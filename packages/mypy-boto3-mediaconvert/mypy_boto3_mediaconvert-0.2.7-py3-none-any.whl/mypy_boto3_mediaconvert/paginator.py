"Main interface for mediaconvert Paginators"
...
class DescribeEndpointsPaginator(Boto3Paginator):
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :type Mode: string
        :param Mode: Optional field, defaults to DEFAULT. Specify DEFAULT for this operation to return your
        endpoints if any exist, or to create an endpoint for you and return it if one doesn't already exist.
        Specify GET_ONLY to return your endpoints if any exist, or an empty list if none exist.
...
class ListJobTemplatesPaginator(Boto3Paginator):
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :type ListBy: string
        :param ListBy: Optional. When you request a list of job templates, you can choose to list them
        alphabetically by NAME or chronologically by CREATION_DATE. If you don't specify, the service will
        list them by name.

        :type Order: string
        :param Order: When you request lists of resources, you can optionally specify whether they are
                ...
                'JobTemplates': [
                    {
                            ...
                            'Inputs': [
                                {
                                    ...
                                    'AudioSelectors': {
                                        'string': {
                                            ...
                                            |'CYM'|'FRY'|'WOL'|'XHO'|'YID'|'YOR'|'ZHA'|'ZUL'|'ORJ'|'QPC'
                                            |'TNG',
                                            'Offset': 123,
                                            'Pids': [
                                                123,
                                    ...
                                    'CaptionSelectors': {
                                        'string': {
                                            ...
                                            'SourceSettings': {
                                                ...
                                                'SourceType':
                                                'ANCILLARY'|'DVB_SUB'|'EMBEDDED'|'SCTE20'|'SCC'|'TTML'|'STL'
                                                |'SRT'|'SMI'|'TELETEXT'|'NULL_SOURCE'|'IMSC',
                            ...
                            'OutputGroups': [
                                {
                                    ...
                                    'OutputGroupSettings': {
                                            ...
                                            'CaptionLanguageMappings': [
                                                {
                                                    ...
                                                    'CustomLanguageCode': 'string',
                                                    'LanguageCode':
                                                    'ENG'|'SPA'|'FRA'|'DEU'|'GER'|'ZHO'|'ARA'|'HIN'|'JPN'
                                                    |'RUS'|'POR'|'ITA'|'URD'|'VIE'|'KOR'|'PAN'|'ABK'|'AAR'
                                                    |'AFR'|'AKA'|'SQI'|'AMH'|'ARG'|'HYE'|'ASM'|'AVA'|'AVE'
                                                    |'AYM'|'AZE'|'BAM'|'BAK'|'EUS'|'BEL'|'BEN'|'BIH'|'BIS'
                                                    |'BOS'|'BRE'|'BUL'|'MYA'|'CAT'|'KHM'|'CHA'|'CHE'|'NYA'
                                                    |'CHU'|'CHV'|'COR'|'COS'|'CRE'|'HRV'|'CES'|'DAN'|'DIV'
                                                    |'NLD'|'DZO'|'ENM'|'EPO'|'EST'|'EWE'|'FAO'|'FIJ'|'FIN'
                                                    |'FRM'|'FUL'|'GLA'|'GLG'|'LUG'|'KAT'|'ELL'|'GRN'|'GUJ'
                                                    |'HAT'|'HAU'|'HEB'|'HER'|'HMO'|'HUN'|'ISL'|'IDO'|'IBO'
                                                    |'IND'|'INA'|'ILE'|'IKU'|'IPK'|'GLE'|'JAV'|'KAL'|'KAN'
                                                    |'KAU'|'KAS'|'KAZ'|'KIK'|'KIN'|'KIR'|'KOM'|'KON'|'KUA'
                                                    |'KUR'|'LAO'|'LAT'|'LAV'|'LIM'|'LIN'|'LIT'|'LUB'|'LTZ'
                                                    |'MKD'|'MLG'|'MSA'|'MAL'|'MLT'|'GLV'|'MRI'|'MAR'|'MAH'
                                                    |'MON'|'NAU'|'NAV'|'NDE'|'NBL'|'NDO'|'NEP'|'SME'|'NOR'
                                                    |'NOB'|'NNO'|'OCI'|'OJI'|'ORI'|'ORM'|'OSS'|'PLI'|'FAS'
                                                    |'POL'|'PUS'|'QUE'|'QAA'|'RON'|'ROH'|'RUN'|'SMO'|'SAG'
                                                    |'SAN'|'SRD'|'SRB'|'SNA'|'III'|'SND'|'SIN'|'SLK'|'SLV'
                                                    |'SOM'|'SOT'|'SUN'|'SWA'|'SSW'|'SWE'|'TGL'|'TAH'|'TGK'
                                                    |'TAM'|'TAT'|'TEL'|'THA'|'BOD'|'TIR'|'TON'|'TSO'|'TSN'
                                                    |'TUR'|'TUK'|'TWI'|'UIG'|'UKR'|'UZB'|'VEN'|'VOL'|'WLN'
                                                    |'CYM'|'FRY'|'WOL'|'XHO'|'YID'|'YOR'|'ZHA'|'ZUL'|'ORJ'
                                                    |'QPC'|'TNG',
                                        ...
                                        'Type':
                                        'HLS_GROUP_SETTINGS'|'DASH_ISO_GROUP_SETTINGS'|'FILE_GROUP_SETTINGS'
                                        |'MS_SMOOTH_GROUP_SETTINGS'|'CMAF_GROUP_SETTINGS'
                                    },
                                    'Outputs': [
                                        {
                                            'AudioDescriptions': [
                                                {
                                                    ...
                                                    'CodecSettings': {
                                                        'AacSettings': {
                                                            ...
                                                            'CodingMode':
                                                            'AD_RECEIVER_MIX'|'CODING_MODE_1_0'
                                                        ...
                                                        'Ac3Settings': {
                                                            'Bitrate': 123,
                                                            'BitstreamMode':
                                                            'COMPLETE_MAIN'|'COMMENTARY'|'DIALOGUE'
                                                            |'EMERGENCY'|'HEARING_IMPAIRED'
                                                            |'MUSIC_AND_EFFECTS'|'VISUALLY_IMPAIRED'
                                                            |'VOICE_OVER',
                                                            'CodingMode':
                                                            'CODING_MODE_1_0'|'CODING_MODE_1_1'
                                                            |'CODING_MODE_2_0'|'CODING_MODE_3_2_LFE',
                                                            'Dialnorm': 123,
                                                        ...
                                                        'Eac3AtmosSettings': {
                                                            ...
                                                            'NONE'|'FILM_STANDARD'|'FILM_LIGHT'
                                                            |'MUSIC_STANDARD'|'MUSIC_LIGHT'|'SPEECH',
                                                            ...
                                                            'MeteringMode':
                                                            'LEQ_A'|'ITU_BS_1770_1'|'ITU_BS_1770_2'
                                                        ...
                                                        'Eac3Settings': {
                                                            ...
                                                            'Bitrate': 123,
                                                            'BitstreamMode':
                                                            'COMPLETE_MAIN'|'COMMENTARY'|'EMERGENCY'
                                            ...
                                            'CaptionDescriptions': [
                                                {
                                                    ...
                                                    'DestinationSettings': {
                                                        ...
                                                        'DestinationType':
                                                        'BURN_IN'|'DVB_SUB'|'EMBEDDED'
                                                        |'EMBEDDED_PLUS_SCTE20'|'IMSC'
                                                        |'SCTE20_PLUS_EMBEDDED'|'SCC'|'SRT'|'SMI'|'TELETEXT'
                                                        |'TTML'|'WEBVTT',
                                            ...
                                            'ContainerSettings': {
                                                'Container':
                                                'F4V'|'ISMV'|'M2TS'|'M3U8'|'CMFC'|'MOV'|'MP4'|'MPD'|'MXF'
                                                |'RAW',
                                                'F4vSettings': {
                                                ...
                                                'M2tsSettings': {
                                                    ...
                                                    'PatInterval': 123,
                                                    'PcrControl':
                                                    'PCR_EVERY_PES_PACKET'|'CONFIGURED_PCR_PERIOD',
                                                    'PcrPid': 123,
                                                    'PmtInterval': 123,
                                                    'PmtPid': 123,
                                            ...
                                            'VideoDescription': {
                                                ...
                                                'CodecSettings': {
                                                    'Codec':
                                                    'FRAME_CAPTURE'|'H_264'|'H_265'|'MPEG2'|'PRORES',
                                                    ...
                                                    'H264Settings': {
                                                        ...
                                                        'Bitrate': 123,
                                                        'CodecLevel':
                                                        'AUTO'|'LEVEL_1'|'LEVEL_1_1'|'LEVEL_1_2'|'LEVEL_1_3'
                                                        |'LEVEL_2'|'LEVEL_2_1'|'LEVEL_2_2'|'LEVEL_3'
                                                        |'LEVEL_3_1'|'LEVEL_3_2'|'LEVEL_4'|'LEVEL_4_1'
                                                        |'LEVEL_4_2'|'LEVEL_5'|'LEVEL_5_1'|'LEVEL_5_2',
                                                        'CodecProfile':
                                                        'BASELINE'|'HIGH'|'HIGH_10BIT'|'HIGH_422'
                                                        |'HIGH_422_10BIT'|'MAIN',
                                                        ...
                                                        'HrdBufferSize': 123,
                                                        'InterlaceMode':
                                                        'PROGRESSIVE'|'TOP_FIELD'|'BOTTOM_FIELD'
                                                    ...
                                                    'H265Settings': {
                                                        ...
                                                        'Bitrate': 123,
                                                        'CodecLevel':
                                                        'AUTO'|'LEVEL_1'|'LEVEL_2'|'LEVEL_2_1'|'LEVEL_3'
                                                        |'LEVEL_3_1'|'LEVEL_4'|'LEVEL_4_1'|'LEVEL_5'
                                                        |'LEVEL_5_1'|'LEVEL_5_2'|'LEVEL_6'|'LEVEL_6_1'
                                                        |'LEVEL_6_2',
                                                        'CodecProfile':
                                                        'MAIN_MAIN'|'MAIN_HIGH'|'MAIN10_MAIN'|'MAIN10_HIGH'
                                                        |'MAIN_422_8BIT_MAIN'|'MAIN_422_8BIT_HIGH'
                                                        |'MAIN_422_10BIT_MAIN'|'MAIN_422_10BIT_HIGH',
                                                        'DynamicSubGop': 'ADAPTIVE'|'STATIC',
                                                        'FlickerAdaptiveQuantization': 'DISABLED'|'ENABLED',
                                                        'FramerateControl':
                                                        'INITIALIZE_FROM_SOURCE'|'SPECIFIED',
                                                    ...
                                                    'ProresSettings': {
                                                        'CodecProfile':
                                                        'APPLE_PRORES_422'|'APPLE_PRORES_422_HQ'
                                                ...
                                                'VideoPreprocessors': {
                                                    ...
                                                    'NoiseReducer': {
                                                        'Filter':
                                                        'BILATERAL'|'MEAN'|'GAUSSIAN'|'LANCZOS'|'SHARPEN'
                                                    ...
                                                    'TimecodeBurnin': {
                                                        'FontSize': 123,
                                                        'Position':
                                                        'TOP_CENTER'|'TOP_LEFT'|'TOP_RIGHT'|'MIDDLE_LEFT'
                        ...
                        'SECONDS_10'|'SECONDS_12'|'SECONDS_15'|'SECONDS_20'|'SECONDS_30'|'SECONDS_60'
                        |'SECONDS_120'|'SECONDS_180'|'SECONDS_240'|'SECONDS_300'|'SECONDS_360'|'SECONDS_420'
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **JobTemplates** *(list) --* List of Job templates.
              ...
              - *(dict) --* A job template is a pre-made set of encoding instructions that you can use to
              quickly create a job.

                - **AccelerationSettings** *(dict) --* Accelerated transcoding can significantly speed up
                jobs with long, visually complex content.

                  - **Mode** *(string) --* Specify the conditions when the service will run your job with
                ...
                - **LastUpdated** *(datetime) --* The timestamp in epoch seconds when the Job template was
                last updated.

                - **Name** *(string) --* A name you create for each job template. Each name must be unique
                within your account.

                - **Priority** *(integer) --* Relative priority on the job.

                - **Queue** *(string) --* Optional. The queue that jobs created from this template are
                assigned to. If you don't specify this, jobs will go to the default queue.

                - **Settings** *(dict) --* JobTemplateSettings contains all the transcode settings saved in
                the template that will be applied to jobs created from it.
                  ...
                  to the input Ad Avail PTS time.

                  - **AvailBlanking** *(dict) --* Settings for ad avail blanking. Video can be blanked or
                  overlaid with an image, and audio muted during SCTE-35 triggered ad avails.

                    - **AvailBlankingImage** *(string) --* Blanking image to be used. Leave empty for solid
                  ...
                  - **Esam** *(dict) --* Settings for Event Signaling And Messaging (ESAM).
                    ...
                    transcoder uses the manifest conditioning instructions that you provide in the setting
                    MCC XML (mccXml).

                      - **MccXml** *(string) --* Provide your ESAM ManifestConfirmConditionNotification XML
                    ...
                    message is less than this value, then the transcoder places the SCTE-35 marker at the
                    beginning of the stream.

                    - **SignalProcessingNotification** *(dict) --* Specifies an ESAM
                    SignalProcessingNotification XML as per OC-SP-ESAM-API-I03-131025. The transcoder uses
                    the signal processing instructions that you provide in the setting SCC XML (sccXml).

                      - **SccXml** *(string) --* Provide your ESAM SignalProcessingNotification XML document
                      inside your JSON job settings. Form the XML document as per OC-SP-ESAM-API-I03-131025.
                      The transcoder will use the signal processing instructions in the message that you
                      supply. Provide your ESAM SignalProcessingNotification XML document inside your JSON
                      job settings. For your MPEG2-TS file outputs, if you want the service to place SCTE-35
                      markers at the insertion points you specify in the XML document, you must also enable
                      SCTE-35 ESAM (scte35Esam). Note that you can either specify an ESAM XML document or
                  ...
                  include multiple inputs when referencing a job template.
                    ...
                    - *(dict) --* Specified video input in a template.
                      ...
                      - **AudioSelectorGroups** *(dict) --* Specifies set of audio selectors within an input
                      to combine. An input may have multiple audio selector groups. See "Audio Selector
                      Group":#inputs-audio_selector_group for more information.
                        ...
                        - *(string) --*
                          ...
                          - *(dict) --* Group of Audio Selectors
                            ...
                            - **AudioSelectorNames** *(list) --* Name of an Audio Selector within the same
                            input to include in the group. Audio selector names are standardized, based on
                            their order within the input (e.g., "Audio Selector 1"). The audio selector name
                            parameter can be repeated to add any number of audio selectors to the group.

                              - *(string) --*

                      - **AudioSelectors** *(dict) --* Use Audio selectors (AudioSelectors) to specify a
                      track or set of tracks from the input that you will use in your outputs. You can use
                      mutiple Audio selectors per input.
                        ...
                        - *(string) --*
                          ...
                          - *(dict) --* Selector for Audio
                            ...
                            - **CustomLanguageCode** *(string) --* Selects a specific language code from
                            within an audio source, using the ISO 639-2 or ISO 639-3 three-letter language
                            code

                            - **DefaultSelection** *(string) --* Enable this setting on one audio selector
                            to set it as the default for the job. The service uses this default for outputs
                            where it can't find the specified input audio. If you don't set a default, those
                            outputs have no audio.

                            - **ExternalAudioFileInput** *(string) --* Specifies audio data from an external
                            file source.

                            - **LanguageCode** *(string) --* Selects a specific language code from within an
                            audio source.

                            - **Offset** *(integer) --* Specifies a time delta in milliseconds to offset the
                            audio from the input video.

                            - **Pids** *(list) --* Selects a specific PID from within an audio source (e.g.
                            257 selects PID 0x101).

                              - *(integer) --*

                            - **ProgramSelection** *(integer) --* Use this setting for input streams that
                            contain Dolby E, to have the service extract specific program data from the
                            track. To select multiple programs, create multiple selectors with the same
                            Track and different Program numbers. In the console, this setting is visible
                            when you set Selector type to Track. Choose the program number from the dropdown
                            list. If you are sending a JSON file, provide the program ID, which is part of
                            the audio metadata. If your input file has incorrect metadata, you can choose
                            All channels instead of a program number to have the service ignore the program
                            IDs and include all the programs in the track.

                            - **RemixSettings** *(dict) --* Use these settings to reorder the audio channels
                            of one input to match those of another input. This allows you to combine the two
                            files into a single output, one after the other.

                              - **ChannelMapping** *(dict) --* Channel mapping (ChannelMapping) contains the
                              group of fields that hold the remixing value for each channel. Units are in
                              dB. Acceptable values are within the range from -60 (mute) through 6. A
                              ...
                              - **ChannelsIn** *(integer) --* Specify the number of audio channels from your
                              input that you want to use in your output. With remixing, you might combine or
                              split the data in these channels, so the number of channels in your final
                              output might be different.

                              - **ChannelsOut** *(integer) --* Specify the number of channels in this output
                            ...
                            - **Tracks** *(list) --* Identify a track from the input audio to include in
                            this selector by entering the track index number. To include several tracks in a
                            single audio selector, specify multiple tracks as follows. Using the console,
                            enter a comma-separated list. For examle, type "1,2,3" to include tracks 1
                            through 3. Specifying directly in your JSON job file, provide the track numbers
                            in an array. For example, "tracks": [1,2,3].

                              - *(integer) --*

                      - **CaptionSelectors** *(dict) --* Use Captions selectors (CaptionSelectors) to
                      specify the captions data from the input that you will use in your outputs. You can
                      use mutiple captions selectors per input.

                        - *(string) --*

                          - *(dict) --* Set up captions in your outputs by first selecting them from your
                          input here.

                            - **CustomLanguageCode** *(string) --* The specific language to extract from
                            source, using the ISO 639-2 or ISO 639-3 three-letter language code. If input is
                            SCTE-27, complete this field and/or PID to select the caption language to
                            extract. If input is DVB-Sub and output is Burn-in or SMPTE-TT, complete this
                            field and/or PID to select the caption language to extract. If input is DVB-Sub
                            that is being passed through, omit this field (and PID field); there is no way
                            to extract a specific language with pass-through captions.

                            - **LanguageCode** *(string) --* The specific language to extract from source.
                            If input is SCTE-27, complete this field and/or PID to select the caption
                            language to extract. If input is DVB-Sub and output is Burn-in or SMPTE-TT,
                            complete this field and/or PID to select the caption language to extract. If
                            input is DVB-Sub that is being passed through, omit this field (and PID field);
                            there is no way to extract a specific language with pass-through captions.

                            - **SourceSettings** *(dict) --* If your input captions are SCC, TTML, STL, SMI,
                            SRT, or IMSC in an xml file, specify the URI of the input captions source file.
                            If your input captions are IMSC in an IMF package, use TrackSourceSettings
                            instead of FileSoureSettings.

                              - **AncillarySourceSettings** *(dict) --* Settings for ancillary captions
                              source.

                                - **Convert608To708** *(string) --* Specify whether this set of input
                                captions appears in your outputs in both 608 and 708 format. If you choose
                                Upconvert (UPCONVERT), MediaConvert includes the captions data in two ways:
                                it passes the 608 data through using the 608 compatibility bytes fields of
                                the 708 wrapper, and it also translates the 608 data into 708.

                                - **SourceAncillaryChannelNumber** *(integer) --* Specifies the 608 channel
                                number in the ancillary data track from which to extract captions. Unused
                                for passthrough.

                                - **TerminateCaptions** *(string) --* By default, the service terminates any
                                unterminated captions at the end of each input. If you want the caption to
                              ...
                              - **EmbeddedSourceSettings** *(dict) --* Settings for embedded captions Source
                                ...
                                - **Source608TrackNumber** *(integer) --* Specifies the video track index
                                used for extracting captions. The system only supports one input video
                              ...
                              - **FileSourceSettings** *(dict) --* If your input captions are SCC, SMI, SRT,
                              STL, TTML, or IMSC 1.1 in an xml file, specify the URI of the input caption
                              source file. If your caption source is IMSC in an IMF package, use
                              TrackSourceSettings instead of FileSoureSettings.
                                ...
                                - **SourceFile** *(string) --* External caption file used for loading
                                captions. Accepted file extensions are 'scc', 'ttml', 'dfxp', 'stl', 'srt',
                                'xml', and 'smi'.

                                - **TimeDelta** *(integer) --* Specifies a time delta in seconds to offset
                                the captions from the source file.

                              - **SourceType** *(string) --* Use Source (SourceType) to identify the format
                              of your input captions. The service cannot auto-detect caption format.

                              - **TeletextSourceSettings** *(dict) --* Settings specific to Teletext caption
                              sources, including Page number.

                                - **PageNumber** *(string) --* Use Page Number (PageNumber) to specify the
                                three-digit hexadecimal page number that will be used for Teletext captions.
                                Do not use this setting if you are passing through teletext from the input
                                source to output.

                              - **TrackSourceSettings** *(dict) --* Settings specific to caption sources
                              that are specified by track number. Currently, this is only IMSC captions in
                              an IMF package. If your caption source is IMSC 1.1 in a separate xml file, use
                              FileSourceSettings instead of TrackSourceSettings.

                                - **TrackNumber** *(integer) --* Use this setting to select a single
                                captions track from a source. Track numbers correspond to the order in the
                                captions source file. For IMF sources, track numbering is based on the order
                                that the captions appear in the CPL. For example, use 1 to select the
                                captions asset that is listed first in the CPL. To include more than one
                                captions track in your job outputs, create multiple input captions
                                selectors. Specify one track per selector.

                      - **Crop** *(dict) --* Use Cropping selection (crop) to specify the video area that
                      the service will include in the output video frame. If you specify a value here, it
                      will override any value that you specify in the output setting Cropping selection
                      (crop).
                        ...
                        - **X** *(integer) --* The distance, in pixels, between the rectangle and the left
                        edge of the video frame. Specify only even numbers.

                        - **Y** *(integer) --* The distance, in pixels, between the rectangle and the top
                        edge of the video frame. Specify only even numbers.

                      - **DeblockFilter** *(string) --* Enable Deblock (InputDeblockFilter) to produce
                      smoother motion in the output. Default is disabled. Only manaully controllable for
                      MPEG2 and uncompressed video inputs.

                      - **DenoiseFilter** *(string) --* Enable Denoise (InputDenoiseFilter) to filter noise
                      from the input. Default is disabled. Only applicable to MPEG2, H.264, H.265, and
                      uncompressed video inputs.

                      - **FilterEnable** *(string) --* Use Filter enable (InputFilterEnable) to specify how
                      the transcoding service applies the denoise and deblock filters. You must also enable
                      ...
                      This is true even if you use the API to enable them in (InputDeblockFilter) and
                      (InputDeblockFilter). * Force - The in put is filtered regardless of input type.

                      - **FilterStrength** *(integer) --* Use Filter strength (FilterStrength) to adjust the
                      magnitude the input filter settings (Deblock and Denoise). The range is -5 to 5.
                      Default is 0.

                      - **ImageInserter** *(dict) --* Enable the image inserter feature to include a graphic
                      overlay on your video. Enable or disable this feature for each input individually.
                      This setting is disabled by default.

                        - **InsertableImages** *(list) --* Specify the images that you want to overlay on
                        your video. The images must be PNG or TGA files.

                          - *(dict) --* Settings that specify how your still graphic overlay appears.

                            - **Duration** *(integer) --* Specify the time, in milliseconds, for the image
                            to remain on the output video. This duration includes fade-in time but not
                            fade-out time.

                            - **FadeIn** *(integer) --* Specify the length of time, in milliseconds, between
                            the Start time that you specify for the image insertion and the time that the
                            image appears at full opacity. Full opacity is the level that you specify for
                            the opacity setting. If you don't specify a value for Fade-in, the image will
                            appear abruptly at the overlay start time.

                            - **FadeOut** *(integer) --* Specify the length of time, in milliseconds,
                            between the end of the time that you have specified for the image overlay
                            Duration and when the overlaid image has faded to total transparency. If you
                            don't specify a value for Fade-out, the image will disappear abruptly at the end
                            of the inserted image duration.

                            - **Height** *(integer) --* Specify the height of the inserted image in pixels.
                            If you specify a value that's larger than the video resolution height, the
                            service will crop your overlaid image to fit. To use the native height of the
                            image, keep this setting blank.

                            - **ImageInserterInput** *(string) --* Specify the HTTP, HTTPS, or Amazon S3
                            location of the image that you want to overlay on the video. Use a PNG or TGA
                            file.

                            - **ImageX** *(integer) --* Specify the distance, in pixels, between the
                            inserted image and the left edge of the video frame. Required for any image
                            overlay that you specify.

                            - **ImageY** *(integer) --* Specify the distance, in pixels, between the
                            overlaid image and the top edge of the video frame. Required for any image
                            overlay that you specify.

                            - **Layer** *(integer) --* Specify how overlapping inserted images appear.
                            Images with higher values for Layer appear on top of images with lower values
                            for Layer.

                            - **Opacity** *(integer) --* Use Opacity (Opacity) to specify how much of the
                            underlying video shows through the inserted image. 0 is transparent and 100 is
                            fully opaque. Default is 50.

                            - **StartTime** *(string) --* Specify the timecode of the frame that you want
                            the overlay to first appear on. This must be in timecode (HH:MM:SS:FF or
                            HH:MM:SS;FF) format. Remember to take into account your timecode source
                            settings.

                            - **Width** *(integer) --* Specify the width of the inserted image in pixels. If
                            you specify a value that's larger than the video resolution width, the service
                            will crop your overlaid image to fit. To use the native width of the image, keep
                            this setting blank.

                      - **InputClippings** *(list) --* (InputClippings) contains sets of start and end times
                      that together specify a portion of the input to be used in the outputs. If you provide
                      ...
                      specify more than one input clip, the transcoding service creates the job outputs by
                      stringing the clips together in the order you specify them.

                        - *(dict) --* To transcode only portions of your input (clips), include one Input
                        clipping (one instance of InputClipping in the JSON job file) for each input clip.
                        All input clips you specify will be included in every output of the job.

                          - **EndTimecode** *(string) --* Set End timecode (EndTimecode) to the end of the
                          portion of the input you are clipping. The frame corresponding to the End timecode
                          value is included in the clip. Start timecode or End timecode may be left blank,
                          but not both. Use the format HH:MM:SS:FF or HH:MM:SS;FF, where HH is the hour, MM
                          is the minute, SS is the second, and FF is the frame number. When choosing this
                          ...
                          - **StartTimecode** *(string) --* Set Start timecode (StartTimecode) to the
                          beginning of the portion of the input you are clipping. The frame corresponding to
                          the Start timecode value is included in the clip. Start timecode or End timecode
                          may be left blank, but not both. Use the format HH:MM:SS:FF or HH:MM:SS;FF, where
                          HH is the hour, MM is the minute, SS is the second, and FF is the frame number.
                          When choosing this value, take into account your setting for Input timecode
                          source. For example, if you have embedded timecodes that start at 01:00:00:00 and
                          you want your clip to begin five minutes into the video, use 01:05:00:00.

                      - **Position** *(dict) --* Use Selection placement (position) to define the video area
                      in your output frame. The area outside of the rectangle that you specify here is
                      black. If you specify a value here, it will override any value that you specify in the
                      output setting Selection placement (position). If you specify a value here, this will
                      override any AFD values in your input, even if you set Respond to AFD (RespondToAfd)
                      to Respond (RESPOND). If you specify a value here, this will ignore anything that you
                      ...
                      - **ProgramNumber** *(integer) --* Use Program (programNumber) to select a specific
                      program from within a multi-program transport stream. Note that Quad 4K is not
                      currently supported. Default is the first program within the transport stream. If the
                      program you specify doesn't exist, the transcoding service will use this default.

                      - **PsiControl** *(string) --* Set PSI control (InputPsiControl) for transport stream
                      inputs to specify which data the demux process to scans. * Ignore PSI - Scan all PIDs
                      for audio and video. * Use PSI - Scan only PSI data.

                      - **TimecodeSource** *(string) --* Use this Timecode source setting, located under the
                      input settings (InputTimecodeSource), to specify how the service counts input video
                      frames. This input frame count affects only the behavior of features that apply to a
                      ...
                      - **TimecodeStart** *(string) --* Specify the timecode that you want the service to
                      use for this input's initial frame. To use this setting, you must set the Timecode
                      source setting, located under the input settings (InputTimecodeSource), to Specified
                      start (SPECIFIEDSTART). For more information about timecodes, see
                      https://docs.aws.amazon.com/console/mediaconvert/timecode.

                      - **VideoSelector** *(dict) --* Selector for video.

                        - **ColorSpace** *(string) --* If your input video has accurate color space
                        metadata, or if you don't know about color space, leave this set to the default
                        value Follow (FOLLOW). The service will automatically detect your input color space.
                        If your input video has metadata indicating the wrong color space, specify the
                        accurate color space here. If your input video is HDR 10 and the SMPTE ST 2086
                        Mastering Display Color Volume static metadata isn't present in your video stream,
                        ...
                        settings. For more information about MediaConvert HDR jobs, see
                        https://docs.aws.amazon.com/console/mediaconvert/hdr.

                        - **ColorSpaceUsage** *(string) --* There are two sources for color metadata, the
                        input file and the job input settings Color space (ColorSpace) and HDR master
                        display information settings(Hdr10Metadata). The Color space usage setting
                        determines which takes precedence. Choose Force (FORCE) to use color metadata from
                        the input job settings. If you don't specify values for those settings, the service
                        defaults to using metadata from your input. FALLBACK - Choose Fallback (FALLBACK) to
                        use color metadata from the source when it is present. If there's no color metadata
                        in your input file, the service defaults to using values you specify in the input
                        settings.

                        - **Hdr10Metadata** *(dict) --* Use these settings to provide HDR 10 metadata that
                        is missing or inaccurate in your input video. Appropriate values vary depending on
                        the input video and must be provided by a color grader. The color grader generates
                        ...
                        coordinate. Related settings - When you specify these values, you must also set
                        Color space (ColorSpace) to HDR 10 (HDR10). To specify whether the the values you
                        specify here take precedence over the values in the metadata of your input file, set
                        Color space usage (ColorSpaceUsage). To specify whether color metadata is included
                        ...
                        MediaConvert HDR jobs, see https://docs.aws.amazon.com/console/mediaconvert/hdr.
                          ...
                          - **BluePrimaryX** *(integer) --* HDR Master Display Information must be provided
                          ...
                          - **BluePrimaryY** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **GreenPrimaryX** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **GreenPrimaryY** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **MaxContentLightLevel** *(integer) --* Maximum light level among all samples in
                          the coded video sequence, in units of candelas per square meter. This setting
                          doesn't have a default value; you must specify a value that is suitable for the
                          content.

                          - **MaxFrameAverageLightLevel** *(integer) --* Maximum average light level of any
                          frame in the coded video sequence, in units of candelas per square meter. This
                          setting doesn't have a default value; you must specify a value that is suitable
                          for the content.

                          - **MaxLuminance** *(integer) --* Nominal maximum mastering display luminance in
                          units of of 0.0001 candelas per square meter.

                          - **MinLuminance** *(integer) --* Nominal minimum mastering display luminance in
                          units of of 0.0001 candelas per square meter

                          - **RedPrimaryX** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **RedPrimaryY** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **WhitePointX** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                          - **WhitePointY** *(integer) --* HDR Master Display Information must be provided
                          by a color grader, using color grading tools. Range is 0 to 50,000, each increment
                          represents 0.00002 in CIE1931 color coordinate. Note that this setting is not for
                          color correction.

                        - **Pid** *(integer) --* Use PID (Pid) to select specific video data from an input
                        ...
                        - **Rotate** *(string) --* Use Rotate (InputRotate) to specify how the service
                        rotates your video. You can choose automatic rotation or specify a rotation. You can
                        specify a clockwise rotation of 0, 90, 180, or 270 degrees. If your input video
                        container is .mov or .mp4 and your input has rotation metadata, you can choose
                        Automatic to have the service rotate your video according to the rotation specified
                        in the metadata. The rotation must be within one degree of 90, 180, or 270 degrees.
                        If the rotation metadata specifies any other rotation, the service will default to
                        no rotation. By default, the service does no rotation, even if your input video has
                        rotation metadata. The service doesn't pass through rotation metadata.

                  - **MotionImageInserter** *(dict) --* Overlay motion graphics on top of your video. The
                  motion graphics that you specify here appear on all outputs in all output groups.

                    - **Framerate** *(dict) --* If your motion graphic asset is a .mov file, keep this
                    setting unspecified. If your motion graphic asset is a series of .png files, specify the
                    frame rate of the overlay in frames per second, as a fraction. For example, specify 24
                    fps as 24/1. Make sure that the number of images in your series matches the frame rate
                    and your intended overlay duration. For example, if you want a 30-second overlay at 30
                    fps, you should have 900 .png images. This overlay frame rate doesn't need to match the
                    frame rate of the underlying video.

                      - **FramerateDenominator** *(integer) --* The bottom of the fraction that expresses
                      your overlay frame rate. For example, if your frame rate is 24 fps, set this value to
                      1.

                      - **FramerateNumerator** *(integer) --* The top of the fraction that expresses your
                      overlay frame rate. For example, if your frame rate is 24 fps, set this value to 24.

                    - **Input** *(string) --* Specify the .mov file or series of .png files that you want to
                    overlay on your video. For .png files, provide the file name of the first file in the
                    series. Make sure that the names of the .png files end with sequential numbers that
                    specify the order that they are played in. For example, overlay_000.png,
                    overlay_001.png, overlay_002.png, and so on. The sequence must start at zero, and each
                    image file name must have the same number of digits. Pad your initial file names with
                    ...
                    - **InsertionMode** *(string) --* Choose the type of motion graphic asset that you are
                    providing for your overlay. You can choose either a .mov file or a series of .png files.

                    - **Offset** *(dict) --* Use Offset to specify the placement of your motion graphic
                    overlay on the video frame. Specify in pixels, from the upper-left corner of the frame.
                    If you don't specify an offset, the service scales your overlay to the full size of the
                    frame. Otherwise, the service inserts the overlay at its native resolution and scales
                    the size up or down with any video scaling.

                      - **ImageX** *(integer) --* Set the distance, in pixels, between the overlay and the
                      left edge of the video frame.

                      - **ImageY** *(integer) --* Set the distance, in pixels, between the overlay and the
                      top edge of the video frame.

                    - **Playback** *(string) --* Specify whether your motion graphic overlay repeats on a
                    loop or plays only once.

                    - **StartTime** *(string) --* Specify when the motion overlay begins. Use timecode
                    format (HH:MM:SS:FF or HH:MM:SS;FF). Make sure that the timecode you provide here takes
                    into account how you have set up your timecode configuration under both job settings and
                    input settings. The simplest way to do that is to set both to start at 0. If you need to
                    set up your job to follow timecodes embedded in your source that don't start at zero,
                    make sure that you specify a start time that is after the first embedded timecode. For
                  ...
                  - **NielsenConfiguration** *(dict) --* Settings for your Nielsen configuration. If you
                  don't do Nielsen measurement and analytics, ignore these settings. When you enable Nielsen
                  configuration (nielsenConfiguration), MediaConvert enables PCM to ID3 tagging for all
                  outputs in the job. To enable Nielsen configuration programmatically, include an instance
                  of nielsenConfiguration in your JSON job specification. Even if you don't include any
                  children of nielsenConfiguration, you still enable the setting.

                    - **BreakoutCode** *(integer) --* Nielsen has discontinued the use of breakout code
                    functionality. If you must include this property, set the value to zero.

                    - **DistributorId** *(string) --* Use Distributor ID (DistributorID) to specify the
                    distributor ID that is assigned to your organization by Neilsen.

                  - **OutputGroups** *(list) --* (OutputGroups) contains one group of settings for each set
                  of outputs that share a common package type. All unpackaged files (MPEG-4, MPEG-2 TS,
                  Quicktime, MXF, and no container) are grouped in a single output group as well. Required
                  in (OutputGroups) is a group of settings that apply to the whole group. This required
                  object depends on the value you set for (Type) under (OutputGroups)>(OutputGroupSettings).
                  ...
                  MS_SMOOTH_GROUP_SETTINGS, MsSmoothGroupSettings * CMAF_GROUP_SETTINGS, CmafGroupSettings
                    ...
                    - *(dict) --* Group of outputs
                      ...
                      - **CustomName** *(string) --* Use Custom Group Name (CustomName) to specify a name
                      for the output group. This value is displayed on the console and can make your job
                      settings JSON more human-readable. It does not affect your outputs. Use up to twelve
                      ...
                      - **OutputGroupSettings** *(dict) --* Output Group settings, including type
                        ...
                        Output Group may only contain a single video, audio, or caption output.
                          ...
                          - **ClientCache** *(string) --* When set to ENABLED, sets #EXT-X-ALLOW-CACHE:no
                          ...
                          - **Destination** *(string) --* Use Destination (Destination) to specify the S3
                          output location and the output filename base. Destination accepts format
                          identifiers. If you do not specify the base filename in the URI, the service will
                          use the filename of the input file. If your job has multiple inputs, the service
                          uses the filename of the first input file.

                          - **DestinationSettings** *(dict) --* Settings associated with the destination.
                          Will vary based on the type of destination
                            ...
                            - **S3Settings** *(dict) --* Settings associated with S3 destination
                              ...
                              as they are uploaded to Amazon S3.
                                ...
                                - **EncryptionType** *(string) --* Specify how you want your data keys
                                managed. AWS uses data keys to encrypt your content. AWS also encrypts the
                                data keys themselves, using a customer master key (CMK), and then stores the
                                encrypted data keys alongside your encrypted content. Use this setting to
                                specify which AWS service manages the CMK. For simplest set up, choose
                                Amazon S3 (SERVER_SIDE_ENCRYPTION_S3). If you want your master key to be
                                managed by AWS Key Management Service (KMS), choose AWS KMS
                                (SERVER_SIDE_ENCRYPTION_KMS). By default, when you choose AWS KMS, KMS uses
                                the AWS managed customer master key (CMK) associated with Amazon S3 to
                                encrypt your data keys. You can optionally choose to specify a different,
                                customer managed CMK. Do so by specifying the Amazon Resource Name (ARN) of
                                the key for the setting KMS ARN (kmsKeyArn).

                                - **KmsKeyArn** *(string) --* Optionally, specify the customer master key
                                (CMK) that you want to use to encrypt the data key that AWS uses to encrypt
                                your output content. Enter the Amazon Resource Name (ARN) of the CMK. To use
                                this setting, you must also set Server-side encryption
                                (S3ServerSideEncryptionType) to AWS KMS (SERVER_SIDE_ENCRYPTION_KMS). If you
                                set Server-side encryption to AWS KMS but don't specify a CMK here, AWS uses
                                the AWS managed CMK associated with Amazon S3.

                          - **Encryption** *(dict) --* DRM settings.

                            - **ConstantInitializationVector** *(string) --* This is a 128-bit, 16-byte hex
                            value represented by a 32-character text string. If this parameter is not set
                            then the Initialization Vector will follow the segment number by default.

                            - **EncryptionMethod** *(string) --* Specify the encryption scheme that you want
                            the service to use when encrypting your CMAF segments. Choose AES-CBC subsample
                            (SAMPLE-AES) or AES_CTR (AES-CTR).

                            - **InitializationVectorInManifest** *(string) --* When you use DRM with CMAF
                            outputs, choose whether the service writes the 128-bit encryption initialization
                            vector in the HLS and DASH manifests.

                            - **SpekeKeyProvider** *(dict) --* If your output group type is CMAF, use these
                            settings when doing DRM encryption with a SPEKE-compliant key provider. If your
                            output group type is HLS, DASH, or Microsoft Smooth, use the SpekeKeyProvider
                            settings instead.

                              - **CertificateArn** *(string) --* If you want your key provider to encrypt
                              the content keys that it provides to MediaConvert, set up a certificate with a
                              master key using AWS Certificate Manager. Specify the certificate's Amazon
                              Resource Name (ARN) here.

                              - **DashSignaledSystemIds** *(list) --* Specify the DRM system IDs that you
                              want signaled in the DASH manifest that MediaConvert creates as part of this
                              CMAF package. The DASH manifest can currently signal up to three system IDs.
                              For more information, see https://dashif.org/identifiers/content_protection/.

                                - *(string) --*

                              - **HlsSignaledSystemIds** *(list) --* Specify the DRM system ID that you want
                              signaled in the HLS manifest that MediaConvert creates as part of this CMAF
                              package. The HLS manifest can currently signal only one system ID. For more
                              ...
                              SPEKE-compliant DRM key provider uses to provide keys for encrypting your
                              content.

                            - **StaticKeyProvider** *(dict) --* Use these settings to set up encryption with
                            a static key provider.

                              - **KeyFormat** *(string) --* Relates to DRM implementation. Sets the value of
                              the KEYFORMAT attribute. Must be 'identity' or a reverse DNS string. May be
                              omitted to indicate an implicit value of 'identity'.

                              - **KeyFormatVersions** *(string) --* Relates to DRM implementation. Either a
                              single positive integer version value or a slash delimited list of version
                              values (1/2/3).

                              - **StaticKeyValue** *(string) --* Relates to DRM implementation. Use a
                              32-character hexidecimal string to specify Key Value (StaticKeyValue).

                              - **Url** *(string) --* Relates to DRM implementation. The location of the
                              license server used for protecting content.

                            - **Type** *(string) --* Specify whether your DRM encryption key is static or
                            from a key provider that follows the SPEKE standard. For more information about
                            SPEKE, see
                            https://docs.aws.amazon.com/speke/latest/documentation/what-is-speke.html.

                          - **FragmentLength** *(integer) --* Length of fragments to generate (in seconds).
                          Fragment length must be compatible with GOP size and Framerate. Note that
                          fragments will end on the next keyframe after this number of seconds, so actual
                          fragment length may be longer. When Emit Single File is checked, the fragmentation
                          is internal to a single output file and it does not cause the creation of many
                          ...
                          - **MinBufferTime** *(integer) --* Minimum time of initially buffered media that
                          is needed to ensure smooth playout.

                          - **MinFinalSegmentLength** *(float) --* Keep this setting at the default value of
                          0, unless you are troubleshooting a problem with how devices play back the end of
                          your video asset. If you know that player devices are hanging on the final segment
                          of your video because the length of your final segment is too short, use this
                          ...
                          a value for this setting, the encoder will combine any final segment that is
                          shorter than the length that you specify with the previous segment. For example,
                          your segment length is 3 seconds and your final segment is .5 seconds without a
                          minimum final segment length; when you set the minimum final segment length to 1,
                          your final segment is 3.5 seconds.

                          - **MpdProfile** *(string) --* Specify whether your DASH profile is on-demand or
                          main. When you choose Main profile (MAIN_PROFILE), the service signals
                          urn:mpeg:dash:profile:isoff-main:2011 in your .mpd DASH manifest. When you choose
                          On-demand (ON_DEMAND_PROFILE), the service signals
                          ...
                          - **SegmentControl** *(string) --* When set to SINGLE_FILE, a single output file
                          is generated, which is internally segmented using the Fragment Length and Segment
                          Length. When set to SEGMENTED_FILES, separate segment files will be created.

                          - **SegmentLength** *(integer) --* Use this setting to specify the length, in
                          seconds, of each individual CMAF segment. This value applies to the whole package;
                          that is, to every output in the output group. Note that segments end on the first
                          keyframe after this number of seconds, so the actual segment length might be
                          slightly longer. If you set Segment control (CmafSegmentControl) to single file,
                          the service puts the content of each output in a single file that has metadata
                          that marks these segments. If you set it to segmented files, the service creates
                          multiple files for each output, each with the content of one segment.

                          - **StreamInfResolution** *(string) --* Include or exclude RESOLUTION attribute
                          for video in EXT-X-STREAM-INF tag of variant manifest.

                          - **WriteDashManifest** *(string) --* When set to ENABLED, a DASH MPD manifest
                          will be generated for this output.

                          - **WriteHlsManifest** *(string) --* When set to ENABLED, an Apple HLS manifest
                          will be generated for this output.
                        ...
                        (OutputGroups)>(OutputGroupSettings) to DASH_ISO_GROUP_SETTINGS.
                          ...
                          - **Encryption** *(dict) --* DRM settings.
                            ...
                            - **PlaybackDeviceCompatibility** *(string) --* This setting can improve the
                            compatibility of your output with video players on obsolete devices. It applies
                            only to DASH H.264 outputs with DRM encryption. Choose Unencrypted SEI
                            (UNENCRYPTED_SEI) only to correct problems with playback on older devices.
                            Otherwise, keep the default setting CENC v1 (CENC_V1). If you choose Unencrypted
                            SEI, for that output, the service will exclude the access unit delimiter and
                            will leave the SEI NAL units unencrypted.

                            - **SpekeKeyProvider** *(dict) --* If your output group type is HLS, DASH, or
                            Microsoft Smooth, use these settings when doing DRM encryption with a
                            SPEKE-compliant key provider. If your output group type is CMAF, use the
                            SpekeKeyProviderCmaf settings instead.
                              ...
                              - **SystemIds** *(list) --* Relates to SPEKE implementation. DRM system
                              identifiers. DASH output groups support a max of two system ids. Other group
                              types support one system id. See
                          ...
                          - **SegmentLength** *(integer) --* Length of mpd segments to create (in seconds).
                          Note that segments will end on the next keyframe after this number of seconds, so
                          actual segment length may be longer. When Emit Single File is checked, the
                          segmentation is internal to a single output file and it does not cause the
                          creation of many output files as in other output types.

                          - **WriteSegmentTimelineInRepresentation** *(string) --* If you get an HTTP error
                          in the 400 range when you play back your DASH output, enable this setting and run
                          your transcoding job again. When you enable this setting, the service writes
                          precise segment durations in the DASH manifest. The segment duration information
                          appears inside the SegmentTimeline element, inside SegmentTemplate at the
                          Representation level. When you don't enable this setting, the service writes
                        ...
                        (OutputGroups)>(OutputGroupSettings) to HLS_GROUP_SETTINGS.
                          ...
                          - **AdMarkers** *(list) --* Choose one or more ad marker types to decorate your
                          Apple HLS manifest. This setting does not determine whether SCTE-35 markers appear
                          ...
                          - **DirectoryStructure** *(string) --* Indicates whether segments should be placed
                          in subdirectories.
                          ...
                          - **Encryption** *(dict) --* DRM settings.
                            ...
                            - **EncryptionMethod** *(string) --* Encrypts the segments with the given
                            encryption scheme. Leave blank to disable. Selecting 'Disabled' in the web
                            interface also disables encryption.

                            - **InitializationVectorInManifest** *(string) --* The Initialization Vector is
                            a 128-bit number used in conjunction with the key for encrypting blocks. If set
                            to INCLUDE, Initialization Vector is listed in the manifest. Otherwise
                            Initialization Vector is not in the manifest.

                            - **OfflineEncrypted** *(string) --* Enable this setting to insert the
                            EXT-X-SESSION-KEY element into the master playlist. This allows for offline
                          ...
                          - **MinSegmentLength** *(integer) --* When set, Minimum Segment Size is enforced
                          by looking ahead and back within the specified range for a nearby avail and
                          extending the segment size if needed.

                          - **OutputSelection** *(string) --* Indicates whether the .m3u8 manifest file
                          should be generated for this HLS output group.

                          - **ProgramDateTime** *(string) --* Includes or excludes EXT-X-PROGRAM-DATE-TIME
                          tag in .m3u8 manifest files. The value is calculated as follows: either the
                          program date and time are initialized using the input timecode source, or the time
                          is initialized using the input timecode source and the date is initialized using
                          the timestamp_offset.

                          - **ProgramDateTimePeriod** *(integer) --* Period of insertion of
                          EXT-X-PROGRAM-DATE-TIME entry, in seconds.

                          - **SegmentControl** *(string) --* When set to SINGLE_FILE, emits program as a
                          single media resource (.ts) file, uses #EXT-X-BYTERANGE tags to index segment for
                          playback.
                          ...
                          - **TimestampDeltaMilliseconds** *(integer) --* Provides an extra millisecond
                        ...
                        (OutputGroups)>(OutputGroupSettings) to MS_SMOOTH_GROUP_SETTINGS.
                          ...
                          - **FragmentLength** *(integer) --* Use Fragment length (FragmentLength) to
                          ...
                          (MsSmoothManifestEncoding) to specify the encoding format for the server and
                          client manifest. Valid options are utf8 and utf16.

                        - **Type** *(string) --* Type of output group (File group, Apple HLS, DASH ISO,
                        Microsoft Smooth Streaming, CMAF)

                      - **Outputs** *(list) --* This object holds groups of encoding settings, one group of
                      settings per output.

                        - *(dict) --* An output object describes the settings for a single output file or
                        stream in an output group.
                          ...
                          encoding settings.
                            ...
                            - *(dict) --* Description of audio output
                              ...
                              ISO-IEC 13818-1: 0 = Undefined, 1 = Clean Effects, 2 = Hearing Impaired, 3 =
                          ...
                          - **CaptionDescriptions** *(list) --* (CaptionDescriptions) contains groups of
                          captions settings. For each output that has captions, include one instance of
                          (CaptionDescriptions). (CaptionDescriptions) can contain multiple groups of
                          captions settings.

                            - *(dict) --* Description of Caption output

                              - **CaptionSelectorName** *(string) --* Specifies which "Caption
                              Selector":#inputs-caption_selector to use from each input when generating
                          ...
                          transport stream, m2ts * Quicktime, mov * MXF container, mxf * MPEG-4 container,
                          mp4 * No Container, the service will use codec extensions (e.g. AAC, H265, H265,
                          AC3)

                          - **NameModifier** *(string) --* Use Name modifier (NameModifier) to have the
                          service add a string to the end of each output filename. You specify the base
                          filename as part of your destination URI. When you create multiple outputs in the
                          same output group, Name modifier (NameModifier) is required. Name modifier also
                          accepts format identifiers. For DASH ISO outputs, if you use the format
                          identifiers $Number$ or $Time$ in one output, you must use them in the same way in
                          ...
                          - **OutputSettings** *(dict) --* Specific settings for this type of output.
                            ...
                            - **HlsSettings** *(dict) --* Settings for HLS output groups
                              ...
                              - **AudioOnlyContainer** *(string) --* Use this setting only in audio-only
                              outputs. Choose MPEG-2 Transport Stream (M2TS) to create a file in an MPEG2-TS
                              container. Keep the default value Automatic (AUTOMATIC) to create an
                              audio-only file in a raw container. Regardless of the value that you specify
                              here, if this output has video, the service will place the output into an
                              MPEG2-TS container.

                              - **AudioRenditionSets** *(string) --* List all the audio groups that are used
                              with the video output stream. Input all the audio GROUP-IDs that are
                              ...
                              that the client may try to play back by default. Represented as an EXT-X-MEDIA
                              in the HLS manifest with DEFAULT=NO, AUTOSELECT=YES Alternate Audio, not Auto
                              Select Alternate rendition that the client will not try to play back by
                              default. Represented as an EXT-X-MEDIA in the HLS manifest with DEFAULT=NO,
                              AUTOSELECT=NO

                              - **IFrameOnlyManifest** *(string) --* When set to INCLUDE, writes I-Frame
                          ...
                          - **Preset** *(string) --* Use Preset (Preset) to specifiy a preset for your
                          transcoding settings. Provide the system or custom preset name. You can specify
                          either Preset (Preset) or Container settings (ContainerSettings), but not both.
                            ...
                            * Choose None to remove all AFD values from this output. * Choose Fixed to
                            ...
                            ProresSettings * FRAME_CAPTURE, FrameCaptureSettings
                              ...
                              (VideoDescription)>(CodecSettings) to the value H_264.
                                ...
                                For example, 24000 / 1001 = 23.976 fps. Use FramerateDenominator to specify
                              ...
                              (VideoDescription)>(CodecSettings) to the value PRORES.
                                ...
                                jobs that use frame rate conversion, specify the frame rate as a fraction.
                                For example, 24000 / 1001 = 23.976 fps. Use FramerateNumerator to specify
                  ...
                  information from inputs.
                    ...
                    - **Anchor** *(string) --* If you use an editing platform that relies on an anchor
                    timecode, use Anchor Timecode (Anchor) to specify a timecode that will match the input
                    video frame to the output video frame. Use 24-hour format with frame number,
                    (HH:MM:SS:FF) or (HH:MM:SS;FF). This setting ignores frame rate conversion. System
                    behavior for Anchor Timecode varies depending on your setting for Source
                    (TimecodeSource). * If Source (TimecodeSource) is set to Specified Start
                    (SPECIFIEDSTART), the first input frame is the specified value in Start Timecode
                    (Start). Anchor Timecode (Anchor) and Start Timecode (Start) are used calculate output
                    timecode. * If Source (TimecodeSource) is set to Start at 0 (ZEROBASED) the first frame
                    is 00:00:00:00. * If Source (TimecodeSource) is set to Embedded (EMBEDDED), the first
                    frame is the timecode value on the first input frame of the input.

                    - **Source** *(string) --* Use Source (TimecodeSource) to set how timecodes are handled
                    within this job. To make sure that your video, audio, captions, and markers are
                    ...
                    that is in the input video. If no embedded timecode is in the source, the service will
                    use Start at 0 (ZEROBASED) instead. * Start at 0 (ZEROBASED) - Set the timecode of the
                    initial frame to 00:00:00:00. * Specified Start (SPECIFIEDSTART) - Set the timecode of
                    the initial frame to a value other than zero. You use Start timecode (Start) to provide
                    this value.

                    - **Start** *(string) --* Only use when you set Source (TimecodeSource) to Specified
                    start (SPECIFIEDSTART). Use Start timecode (Start) to specify the timecode for the
                    ...
                    timecode date without affecting the time and frame number. Provide the new date as a
                    string in the format "yyyy-mm-dd". To use Time stamp offset, you must also enable Insert
                    program-date-time (InsertProgramDateTime) in the output settings. For example, if the
                    date part of your timecodes is 2002-1-25 and you want to change it to one year later,
                    set Timestamp offset (TimestampOffset) to 2003-1-25.

                  - **TimedMetadataInsertion** *(dict) --* Enable Timed metadata insertion
                  (TimedMetadataInsertion) to include ID3 tags in your job. To include timed metadata, you
                  must enable it here, enable it in each output container, and specify tags and timecodes in
                  ID3 insertion (Id3Insertion) objects.

                    - **Id3Insertions** *(list) --* Id3Insertions contains the array of Id3Insertion
                    instances.

                      - *(dict) --* To insert ID3 tags in your output, specify two values. Use ID3 tag (Id3)
                      to specify the base 64 encoded string and use Timecode (TimeCode) to specify the time
                      when the tag should be inserted. To insert multiple ID3 tags in your output, create
                      multiple instances of ID3 insertion (Id3Insertion).

                        - **Id3** *(string) --* Use ID3 tag (Id3) to provide a tag value in base64-encode
                        format.

                        - **Timecode** *(string) --* Provide a Timecode (TimeCode) in HH:MM:SS:FF or
                        HH:MM:SS;FF format.

                - **StatusUpdateInterval** *(string) --* Specify how often MediaConvert sends STATUS_UPDATE
                ...
                - **Type** *(string) --* A job template can be of two types: system or custom. System or
          ...
          - *(dict) --*
            ...
            - **Jobs** *(list) --* List of jobs
              ...
              http://docs.aws.amazon.com/mediaconvert/latest/ug/what-is.html
                ...
                - **AccelerationStatus** *(string) --* Describes whether the current job is running with
                accelerated transcoding. For jobs that have Acceleration (AccelerationMode) set to DISABLED,
                ...
                states. AccelerationStatus is IN_PROGRESS initially, while the service determines whether
                the input files and job settings are compatible with accelerated transcoding. If they are,
                AcclerationStatus is ACCELERATED. If your input files and job settings aren't compatible
                with accelerated transcoding, the service either fails your job or runs it without
                accelerated transcoding, depending on how you set Acceleration (AccelerationMode). When the
                service runs your job without accelerated transcoding, AccelerationStatus is
                NOT_ACCELERATED.

                - **Arn** *(string) --* An identifier for this resource that is unique within all of AWS.

                - **BillingTagsSource** *(string) --* Optional. Choose a tag type that AWS Billing and Cost
                Management will use to sort your AWS Elemental MediaConvert costs on any billing report that
                you set up. Any transcoding outputs that don't have an associated tag will appear in your
                billing report unsorted. If you don't choose a valid value for this field, your job outputs
                will appear on the billing report unsorted.

                - **CreatedAt** *(datetime) --* The time, in Unix epoch format in seconds, when the job got
                created.
                ...
                - **JobPercentComplete** *(integer) --* An estimate of how far your job has progressed. This
                estimate is shown as a percentage of the total time from when your job leaves its queue to
                when your output files appear in your output Amazon S3 bucket. AWS Elemental MediaConvert
                provides jobPercentComplete in CloudWatch STATUS_UPDATE events and in the response to GetJob
                and ListJobs requests. The jobPercentComplete estimate is reliable for the following input
                containers: Quicktime, Transport Stream, MP4, and MXF. For some jobs, the service can't
                provide information about job progress. In those cases, jobPercentComplete returns a null
                value.

                - **JobTemplate** *(string) --* The job template that the job is created from, if it is
                created from a job template.

                - **Messages** *(dict) --* Provides messages from the service about jobs that you have
                already successfully submitted.

                  - **Info** *(list) --* List of messages that are informational only and don't indicate a
                  problem with your job.

                    - *(string) --*

                  - **Warning** *(list) --* List of messages that warn about conditions that might cause
                  your job not to run or to fail.
                ...
                - **Queue** *(string) --* Optional. When you create a job, you can specify a queue to send
                it to. If you don't specify, the job will go to the default queue. For more about queues,
                see the User Guide topic at http://docs.aws.amazon.com/mediaconvert/latest/ug/what-is.html

                - **RetryCount** *(integer) --* The number of times that the service automatically attempted
                to process your job after encountering an error.

                - **Role** *(string) --* The IAM role you use for creating this job. For details about
                ...
                - **Settings** *(dict) --* JobSettings contains all the transcode settings for a job.
                  ...
                  - **Inputs** *(list) --* Use Inputs (inputs) to define source file used in the transcode
                  ...
                  together to create the output.
                    ...
                    - *(dict) --* Specifies media input
                      ...
                      - **DecryptionSettings** *(dict) --* Settings for decrypting any input files that you
                      encrypt before you upload them to Amazon S3. MediaConvert can decrypt files only when
                      you use AWS Key Management Service (KMS) to encrypt the data key that you use to
                      encrypt your content.

                        - **DecryptionMode** *(string) --* Specify the encryption mode that you used to
                        encrypt your input files.

                        - **EncryptedDecryptionKey** *(string) --* Warning! Don't provide your encryption
                        key in plaintext. Your job settings could be intercepted, making your encrypted
                        content vulnerable. Specify the encrypted version of the data key that you used to
                        encrypt your content. The data key must be encrypted by AWS Key Management Service
                        (KMS). The key can be 128, 192, or 256 bits.

                        - **InitializationVector** *(string) --* Specify the initialization vector that you
                        used when you encrypted your content before uploading it to Amazon S3. You can use a
                        16-byte initialization vector with any encryption mode. Or, you can use a 12-byte
                        initialization vector with GCM or CTR. MediaConvert accepts only initialization
                        vectors that are base64-encoded.

                        - **KmsKeyRegion** *(string) --* Specify the AWS Region for AWS Key Management
                      ...
                      - **FileInput** *(string) --* Specify the source file for your transcoding job. You
                      can use multiple inputs in a single job. The service concatenates these inputs, in the
                      order that you specify them in the job, to create the outputs. If your input format is
                      IMF, specify your input by providing the path to your CPL. For example,
                      "s3://bucket/vf/cpl.xml". If the CPL is in an incomplete IMP, make sure to use
                      *Supplemental IMPs* (SupplementalImps) to specify any supplemental IMPs that contain
                      ...
                      - **SupplementalImps** *(list) --* Provide a list of any necessary supplemental IMPs.
                      You need supplemental IMPs if the CPL that you're using for your input is in an
                      incomplete IMP. Specify either the supplemental IMP directories with a trailing slash
                      or the ASSETMAP.xml files. For example ["s3://bucket/ov/",
                      "s3://bucket/vf2/ASSETMAP.xml"]. You don't need to specify the IMP that contains your
                      input CPL, because the service automatically detects it.
                ...
                - **SimulateReservedQueue** *(string) --* Enable this setting when you run a test job to
                estimate how many reserved transcoding slots (RTS) you need. When this is enabled,
                MediaConvert runs your job from an on-demand queue with similar performance to what you will
                see with one RTS in a reserved queue. This setting is disabled by default.

                - **Status** *(string) --* A job's status can be SUBMITTED, PROGRESSING, COMPLETE, CANCELED,
                or ERROR.
                ...
                - **Timing** *(dict) --* Information about when jobs are submitted, started, and finished is
                specified in Unix epoch format in seconds.

                  - **FinishTime** *(datetime) --* The time, in Unix epoch format, that the transcoding job
                  finished

                  - **StartTime** *(datetime) --* The time, in Unix epoch format, that transcoding for the
                  job began.

                  - **SubmitTime** *(datetime) --* The time, in Unix epoch format, that you submitted the
                  job.

                - **UserMetadata** *(dict) --* User-defined metadata that you want to associate with an
...
class ListPresetsPaginator(Boto3Paginator):
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :type Category: string
        :param Category: Optionally, specify a preset category to limit responses to only presets from that
        category.
                            ...
                            'AudioDescriptions': [
                                {
                                    ...
                                    'CodecSettings': {
                                        'AacSettings': {
                                            ...
                                            'CodecProfile': 'LC'|'HEV1'|'HEV2',
                                            'CodingMode':
                                            'AD_RECEIVER_MIX'|'CODING_MODE_1_0'|'CODING_MODE_1_1'
                                        ...
                                        'Ac3Settings': {
                                            'Bitrate': 123,
                                            'BitstreamMode':
                                            'COMPLETE_MAIN'|'COMMENTARY'|'DIALOGUE'|'EMERGENCY'
                                            |'HEARING_IMPAIRED'|'MUSIC_AND_EFFECTS'|'VISUALLY_IMPAIRED'
                                            |'VOICE_OVER',
                                            'CodingMode':
                                        ...
                                        'Codec':
                                        'AAC'|'MP2'|'WAV'|'AIFF'|'AC3'|'EAC3'|'EAC3_ATMOS'|'PASSTHROUGH',
                                        'Eac3AtmosSettings': {
                                            'Bitrate': 123,
                                            'BitstreamMode': 'COMPLETE_MAIN',
                                            'CodingMode': 'CODING_MODE_9_1_6',
                                            'DialogueIntelligence': 'ENABLED'|'DISABLED',
                                            'DynamicRangeCompressionLine':
                                            'NONE'|'FILM_STANDARD'|'FILM_LIGHT'|'MUSIC_STANDARD'
                                            |'MUSIC_LIGHT'|'SPEECH',
                                            'DynamicRangeCompressionRf':
                                            'NONE'|'FILM_STANDARD'|'FILM_LIGHT'|'MUSIC_STANDARD'
                                            |'MUSIC_LIGHT'|'SPEECH',
                                    ...
                                    'CustomLanguageCode': 'string',
                                    'LanguageCode':
                                    'ENG'|'SPA'|'FRA'|'DEU'|'GER'|'ZHO'|'ARA'|'HIN'|'JPN'|'RUS'|'POR'|'ITA'
                                    |'URD'|'VIE'|'KOR'|'PAN'|'ABK'|'AAR'|'AFR'|'AKA'|'SQI'|'AMH'|'ARG'|'HYE'
                                    |'ASM'|'AVA'|'AVE'|'AYM'|'AZE'|'BAM'|'BAK'|'EUS'|'BEL'|'BEN'|'BIH'|'BIS'
                                    |'BOS'|'BRE'|'BUL'|'MYA'|'CAT'|'KHM'|'CHA'|'CHE'|'NYA'|'CHU'|'CHV'|'COR'
                                    |'COS'|'CRE'|'HRV'|'CES'|'DAN'|'DIV'|'NLD'|'DZO'|'ENM'|'EPO'|'EST'|'EWE'
                                    |'FAO'|'FIJ'|'FIN'|'FRM'|'FUL'|'GLA'|'GLG'|'LUG'|'KAT'|'ELL'|'GRN'|'GUJ'
                                    |'HAT'|'HAU'|'HEB'|'HER'|'HMO'|'HUN'|'ISL'|'IDO'|'IBO'|'IND'|'INA'|'ILE'
                                    |'IKU'|'IPK'|'GLE'|'JAV'|'KAL'|'KAN'|'KAU'|'KAS'|'KAZ'|'KIK'|'KIN'|'KIR'
                                    |'KOM'|'KON'|'KUA'|'KUR'|'LAO'|'LAT'|'LAV'|'LIM'|'LIN'|'LIT'|'LUB'|'LTZ'
                                    |'MKD'|'MLG'|'MSA'|'MAL'|'MLT'|'GLV'|'MRI'|'MAR'|'MAH'|'MON'|'NAU'|'NAV'
                                    |'NDE'|'NBL'|'NDO'|'NEP'|'SME'|'NOR'|'NOB'|'NNO'|'OCI'|'OJI'|'ORI'|'ORM'
                                    |'OSS'|'PLI'|'FAS'|'POL'|'PUS'|'QUE'|'QAA'|'RON'|'ROH'|'RUN'|'SMO'|'SAG'
                                    |'SAN'|'SRD'|'SRB'|'SNA'|'III'|'SND'|'SIN'|'SLK'|'SLV'|'SOM'|'SOT'|'SUN'
                                    |'SWA'|'SSW'|'SWE'|'TGL'|'TAH'|'TGK'|'TAM'|'TAT'|'TEL'|'THA'|'BOD'|'TIR'
                                    |'TON'|'TSO'|'TSN'|'TUR'|'TUK'|'TWI'|'UIG'|'UKR'|'UZB'|'VEN'|'VOL'|'WLN'
                            ...
                            'CaptionDescriptions': [
                                {
                                    ...
                                    'DestinationSettings': {
                                        ...
                                        'DestinationType':
                                        'BURN_IN'|'DVB_SUB'|'EMBEDDED'|'EMBEDDED_PLUS_SCTE20'|'IMSC'
                                        |'SCTE20_PLUS_EMBEDDED'|'SCC'|'SRT'|'SMI'|'TELETEXT'|'TTML'
                                        |'WEBVTT',
                                        'DvbSubDestinationSettings': {
                                            'Alignment': 'CENTERED'|'LEFT',
                                            'BackgroundColor': 'NONE'|'BLACK'|'WHITE',
                            ...
                            'VideoDescription': {
                                ...
                                'CodecSettings': {
                                    ...
                                    'H264Settings': {
                                        ...
                                        'Bitrate': 123,
                                        'CodecLevel':
                                        'AUTO'|'LEVEL_1'|'LEVEL_1_1'|'LEVEL_1_2'|'LEVEL_1_3'|'LEVEL_2'
                                        |'LEVEL_2_1'|'LEVEL_2_2'|'LEVEL_3'|'LEVEL_3_1'|'LEVEL_3_2'|'LEVEL_4'
                                    ...
                                    'H265Settings': {
                                        ...
                                        'Bitrate': 123,
                                        'CodecLevel':
                                        'AUTO'|'LEVEL_1'|'LEVEL_2'|'LEVEL_2_1'|'LEVEL_3'|'LEVEL_3_1'
                                        |'LEVEL_4'|'LEVEL_4_1'|'LEVEL_5'|'LEVEL_5_1'|'LEVEL_5_2'|'LEVEL_6'
                                        |'LEVEL_6_1'|'LEVEL_6_2',
                                        'CodecProfile':
                                        'MAIN_MAIN'|'MAIN_HIGH'|'MAIN10_MAIN'|'MAIN10_HIGH'
                                ...
                                'VideoPreprocessors': {
                                    ...
                                    'NoiseReducer': {
                                        'Filter':
                                        'BILATERAL'|'MEAN'|'GAUSSIAN'|'LANCZOS'|'SHARPEN'|'CONSERVE'
                                        |'SPATIAL'|'TEMPORAL',
                                        'FilterSettings': {
                                            'Strength': 123
                                        },
                                    ...
                                    'TimecodeBurnin': {
                                        'FontSize': 123,
                                        'Position':
                                        'TOP_CENTER'|'TOP_LEFT'|'TOP_RIGHT'|'MIDDLE_LEFT'|'MIDDLE_CENTER'
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Presets** *(list) --* List of presets
              ...
              - *(dict) --* A preset is a collection of preconfigured media conversion settings that you
              want MediaConvert to apply to the output during the conversion process.
                ...
                - **LastUpdated** *(datetime) --* The timestamp in epoch seconds when the preset was last
                updated.

                - **Name** *(string) --* A name you create for each preset. Each name must be unique within
                your account.

                - **Settings** *(dict) --* Settings for preset

                  - **AudioDescriptions** *(list) --* (AudioDescriptions) contains groups of audio encoding
                  settings organized by audio codec. Include one instance of (AudioDescriptions) per output.
                  (AudioDescriptions) can contain multiple groups of encoding settings.
                    ...
                    - *(dict) --* Description of audio output
                      ...
                      Ignore these settings unless you need to comply with a loudness standard.
                        ...
                        - **Algorithm** *(string) --* Choose one of the following audio normalization
                        algorithms: ITU-R BS.1770-1: Ungated loudness. A measurement of ungated average
                        loudness for an entire piece of content, suitable for measurement of short-form
                        content under ATSC recommendation A/85. Supports up to 5.1 audio channels. ITU-R
                        BS.1770-2: Gated loudness. A measurement of gated average loudness compliant with
                        the requirements of EBU-R128. Supports up to 5.1 audio channels. ITU-R BS.1770-3:
                        Modified peak. The same loudness measurement algorithm as 1770-2, with an updated
                        true peak measurement. ITU-R BS.1770-4: Higher channel count. Allows for more audio
                        channels than the other algorithms, including configurations such as 7.1.

                        - **AlgorithmControl** *(string) --* When enabled the output audio is corrected
                        using the chosen algorithm. If disabled, the audio will be measured but not
                        adjusted.

                        - **CorrectionGateLevel** *(integer) --* Content measuring above this level will be
                        corrected to the target level. Content measuring below this level will not be
                        corrected. Gating only applies when not using real_time_correction.

                        - **LoudnessLogging** *(string) --* If set to LOG, log each output's audio track
                        loudness to a CSV file.

                        - **PeakCalculation** *(string) --* If set to TRUE_PEAK, calculate and log the
                        TruePeak for each output's audio track loudness.

                        - **TargetLkfs** *(float) --* When you use Audio normalization
                        (AudioNormalizationSettings), optionally use this setting to specify a target
                        loudness. If you don't specify a value here, the encoder chooses a value for you,
                        based on the algorithm that you choose for Algorithm (algorithm). If you choose
                        algorithm 1770-1, the encoder will choose -24 LKFS; otherwise, the encoder will
                        choose -23 LKFS.

                      - **AudioSourceName** *(string) --* Specifies which audio data to use from each input.
                      In the simplest case, specify an "Audio Selector":#inputs-audio_selector by name based
                      on its order within each input. For example if you specify "Audio Selector 3", then
                      the third audio selector will be used from each input. If an input does not have an
                      "Audio Selector 3", then the audio selector marked as "default" in that input will be
                      used. If there is no audio selector marked as "default", silence will be inserted for
                      the duration of that input. Alternatively, an "Audio Selector
                      Group":#inputs-audio_selector_group name may be specified, with similar
                      default/silence behavior. If no audio_source_name is specified, then "Audio Selector
                      1" will be chosen automatically.

                      - **AudioType** *(integer) --* Applies only if Follow Input Audio Type is unchecked
                      (false). A number between 0 and 255. The following are defined in ISO-IEC 13818-1: 0 =
                      Undefined, 1 = Clean Effects, 2 = Hearing Impaired, 3 = Visually Impaired Commentary,
                      4-255 = Reserved.

                      - **AudioTypeControl** *(string) --* When set to FOLLOW_INPUT, if the input contains
                      an ISO 639 audio_type, then that value is passed through to the output. If the input
                      contains no ISO 639 audio_type, the value in Audio Type is included in the output.
                      Otherwise the value in Audio Type is included in the output. Note that this field and
                      ...
                      object. The following lists the codec enum, settings object pairs. * AAC, AacSettings
                      * MP2, Mp2Settings * WAV, WavSettings * AIFF, AiffSettings * AC3, Ac3Settings * EAC3,
                      Eac3Settings * EAC3_ATMOS, Eac3AtmosSettings

                        - **AacSettings** *(dict) --* Required when you set (Codec) under
                        (AudioDescriptions)>(CodecSettings) to the value AAC. The service accepts one of two
                        mutually exclusive groups of AAC settings--VBR and CBR. To select one of these
                        modes, set the value of Bitrate control mode (rateControlMode) to "VBR" or "CBR". In
                        VBR mode, you control the audio quality with the setting VBR quality (vbrQuality).
                        In CBR mode, you use the setting Bitrate (bitrate). Defaults and valid values depend
                        on the rate control mode.

                          - **AudioDescriptionBroadcasterMix** *(string) --* Choose BROADCASTER_MIXED_AD
                          when the input contains pre-mixed main audio + audio description (AD) as a stereo
                          pair. The value for AudioType will be set to 3, which signals to downstream
                          systems that this stream contains "broadcaster mixed AD". Note that the input
                          received by the encoder must contain pre-mixed audio; the encoder does not perform
                          the mixing. When you choose BROADCASTER_MIXED_AD, the encoder ignores any values
                          you provide in AudioType and FollowInputAudioType. Choose NORMAL when the input
                          does not contain pre-mixed audio + audio description (AD). In this case, the
                          encoder will use any values you provide for AudioType and FollowInputAudioType.

                          - **Bitrate** *(integer) --* Specify the average bitrate in bits per second. The
                          set of valid values for this setting is: 6000, 8000, 10000, 12000, 14000, 16000,
                          20000, 24000, 28000, 32000, 40000, 48000, 56000, 64000, 80000, 96000, 112000,
                          128000, 160000, 192000, 224000, 256000, 288000, 320000, 384000, 448000, 512000,
                          576000, 640000, 768000, 896000, 1024000. The value you set is also constrained by
                          the values that you choose for Profile (codecProfile), Bitrate control mode
                          (codingMode), and Sample rate (sampleRate). Default values depend on Bitrate
                          control mode and Profile.

                          - **CodecProfile** *(string) --* AAC Profile.

                          - **CodingMode** *(string) --* Mono (Audio Description), Mono, Stereo, or 5.1
                          channel layout. Valid values depend on rate control mode and profile. "1.0 - Audio
                          Description (Receiver Mix)" setting receives a stereo description plus control
                          track and emits a mono AAC encode of the description track, with control data
                          emitted in the PES header as per ETSI TS 101 154 Annex E.

                          - **RateControlMode** *(string) --* Rate Control Mode.

                          - **RawFormat** *(string) --* Enables LATM/LOAS AAC output. Note that if you use
                          LATM/LOAS AAC in an output, you must choose "No container" for the output
                          container.

                          - **SampleRate** *(integer) --* Sample rate in Hz. Valid values depend on rate
                          control mode and profile.

                          - **Specification** *(string) --* Use MPEG-2 AAC instead of MPEG-4 AAC audio for
                          raw or MPEG-2 Transport Stream containers.

                          - **VbrQuality** *(string) --* VBR Quality Level - Only used if rate_control_mode
                          is VBR.

                        - **Ac3Settings** *(dict) --* Required when you set (Codec) under
                        (AudioDescriptions)>(CodecSettings) to the value AC3.

                          - **Bitrate** *(integer) --* Specify the average bitrate in bits per second. Valid
                          bitrates depend on the coding mode.

                          - **BitstreamMode** *(string) --* Specify the bitstream mode for the AC-3 stream
                          that the encoder emits. For more information about the AC3 bitstream mode, see
                          ...
                          - **DynamicRangeCompressionProfile** *(string) --* If set to FILM_STANDARD, adds
                          dynamic range compression signaling to the output bitstream as defined in the
                          Dolby Digital specification.

                          - **LfeFilter** *(string) --* Applies a 120Hz lowpass filter to the LFE channel
                          prior to encoding. Only valid with 3_2_LFE coding mode.

                          - **MetadataControl** *(string) --* When set to FOLLOW_INPUT, encoder metadata
                          ...
                          - **SampleRate** *(integer) --* This value is always 48000. It represents the
                          sample rate in Hz.

                        - **AiffSettings** *(dict) --* Required when you set (Codec) under
                        (AudioDescriptions)>(CodecSettings) to the value AIFF.

                          - **BitDepth** *(integer) --* Specify Bit depth (BitDepth), in bits per sample, to
                          choose the encoding quality for this audio track.

                          - **Channels** *(integer) --* Specify the number of channels in this output audio
                          track. Valid values are 1 and even numbers up to 64. For example, 1, 2, 4, 6, and
                          so on, up to 64.
                        ...
                        (AudioDescriptions)>(CodecSettings) to the value EAC3_ATMOS.
                          ...
                          - **BitstreamMode** *(string) --* Specify the bitstream mode for the E-AC-3 stream
                          that the encoder emits. For more information about the EAC3 bitstream mode, see
                          ATSC A/52-2012 (Annex E).

                          - **CodingMode** *(string) --* The coding mode for Dolby Digital Plus JOC (Atmos)
                          is always 9.1.6 (CODING_MODE_9_1_6).

                          - **DialogueIntelligence** *(string) --* Enable Dolby Dialogue Intelligence to
                          adjust loudness based on dialogue analysis.

                          - **DynamicRangeCompressionLine** *(string) --* Specify the absolute peak level
                          for a signal with dynamic range compression.

                          - **DynamicRangeCompressionRf** *(string) --* Specify how the service limits the
                          audio dynamic range when compressing the audio.

                          - **LoRoCenterMixLevel** *(float) --* Specify a value for the following Dolby
                          Atmos setting: Left only/Right only center mix (Lo/Ro center). MediaConvert uses
                          this value for downmixing. How the service uses this value depends on the value
                          that you choose for Stereo downmix (Eac3AtmosStereoDownmix). Valid values: 3.0,
                          1.5, 0.0, -1.5, -3.0, -4.5, and -6.0.

                          - **LoRoSurroundMixLevel** *(float) --* Specify a value for the following Dolby
                          Atmos setting: Left only/Right only (Lo/Ro surround). MediaConvert uses this value
                          for downmixing. How the service uses this value depends on the value that you
                          choose for Stereo downmix (Eac3AtmosStereoDownmix). Valid values: -1.5, -3.0,
                          -4.5, -6.0, and -60. The value -60 mutes the channel.

                          - **LtRtCenterMixLevel** *(float) --* Specify a value for the following Dolby
                          Atmos setting: Left total/Right total center mix (Lt/Rt center). MediaConvert uses
                          this value for downmixing. How the service uses this value depends on the value
                          that you choose for Stereo downmix (Eac3AtmosStereoDownmix). Valid values: 3.0,
                          1.5, 0.0, -1.5, -3.0, -4.5, and -6.0.

                          - **LtRtSurroundMixLevel** *(float) --* Specify a value for the following Dolby
                          Atmos setting: Left total/Right total surround mix (Lt/Rt surround). MediaConvert
                          uses this value for downmixing. How the service uses this value depends on the
                          value that you choose for Stereo downmix (Eac3AtmosStereoDownmix). Valid values:
                          -1.5, -3.0, -4.5, -6.0, and -60. The value -60 mutes the channel.

                          - **MeteringMode** *(string) --* Choose how the service meters the loudness of
                          your audio.

                          - **SampleRate** *(integer) --* This value is always 48000. It represents the
                          sample rate in Hz.

                          - **SpeechThreshold** *(integer) --* Specify the percentage of audio content that
                          must be speech before the encoder uses the measured speech loudness as the overall
                          program loudness.

                          - **StereoDownmix** *(string) --* Choose how the service does stereo downmixing.

                          - **SurroundExMode** *(string) --* Specify whether your input audio has an
                          additional center rear surround channel matrix encoded into your left and right
                          surround channels.

                        - **Eac3Settings** *(dict) --* Required when you set (Codec) under
                        (AudioDescriptions)>(CodecSettings) to the value EAC3.

                          - **AttenuationControl** *(string) --* If set to ATTENUATE_3_DB, applies a 3 dB
                          ...
                          - **CodingMode** *(string) --* Dolby Digital Plus coding mode. Determines number
                          of channels.
                          ...
                          - **LfeControl** *(string) --* When encoding 3/2 audio, controls whether the LFE
                          channel is enabled
                          ...
                          uses this value for downmixing. How the service uses this value depends on the
                          value that you choose for Stereo downmix (Eac3StereoDownmix). Valid values: 3.0,
                          1.5, 0.0, -1.5, -3.0, -4.5, -6.0, and -60. The value -60 mutes the channel. This
                          setting applies only if you keep the default value of 3/2 - L, R, C, Ls, Rs
                          (CODING_MODE_3_2) for the setting Coding mode (Eac3CodingMode). If you choose a
                          different value for Coding mode, the service ignores Left only/Right only center
                          (loRoCenterMixLevel).

                          - **LoRoSurroundMixLevel** *(float) --* Specify a value for the following Dolby
                          Digital Plus setting: Left only/Right only (Lo/Ro surround). MediaConvert uses
                          this value for downmixing. How the service uses this value depends on the value
                          that you choose for Stereo downmix (Eac3StereoDownmix). Valid values: -1.5, -3.0,
                          -4.5, -6.0, and -60. The value -60 mutes the channel. This setting applies only if
                          ...
                          - **LtRtCenterMixLevel** *(float) --* Specify a value for the following Dolby
                          Digital Plus setting: Left total/Right total center mix (Lt/Rt center).
                          MediaConvert uses this value for downmixing. How the service uses this value
                          depends on the value that you choose for Stereo downmix (Eac3StereoDownmix). Valid
                          values: 3.0, 1.5, 0.0, -1.5, -3.0, -4.5, -6.0, and -60. The value -60 mutes the
                          channel. This setting applies only if you keep the default value of 3/2 - L, R, C,
                          Ls, Rs (CODING_MODE_3_2) for the setting Coding mode (Eac3CodingMode). If you
                          choose a different value for Coding mode, the service ignores Left total/Right
                          total center (ltRtCenterMixLevel).

                          - **LtRtSurroundMixLevel** *(float) --* Specify a value for the following Dolby
                          Digital Plus setting: Left total/Right total surround mix (Lt/Rt surround).
                          MediaConvert uses this value for downmixing. How the service uses this value
                          depends on the value that you choose for Stereo downmix (Eac3StereoDownmix). Valid
                          ...
                          - **PassthroughControl** *(string) --* When set to WHEN_POSSIBLE, input DD+ audio
                          ...
                          - **PhaseControl** *(string) --* Controls the amount of phase-shift applied to the
                          ...
                          - **StereoDownmix** *(string) --* Choose how the service does stereo downmixing.
                          This setting only applies if you keep the default value of 3/2 - L, R, C, Ls, Rs
                          (CODING_MODE_3_2) for the setting Coding mode (Eac3CodingMode). If you choose a
                          different value for Coding mode, the service ignores Stereo downmix
                          (Eac3StereoDownmix).
                        ...
                        (AudioDescriptions)>(CodecSettings) to the value MP2.
                          ...
                          - **Channels** *(integer) --* Set Channels to specify the number of channels in
                          this output audio track. Choosing Mono in the console will give you 1 output
                        ...
                        (AudioDescriptions)>(CodecSettings) to the value WAV.
                          ...
                          - **Format** *(string) --* The service defaults to using RIFF for WAV outputs. If
                          your output audio is likely to exceed 4 GB in file size, or if you otherwise need
                      ...
                      doesn't specify a language code. For all outputs, you can use an ISO 639-2 or ISO
                      ...
                      - **RemixSettings** *(dict) --* Advanced audio remixing settings.
                        ...
                        - **ChannelMapping** *(dict) --* Channel mapping (ChannelMapping) contains the group
                        of fields that hold the remixing value for each channel. Units are in dB. Acceptable
                        values are within the range from -60 (mute) through 6. A setting of 0 passes the
                        ...
                        - **ChannelsIn** *(integer) --* Specify the number of audio channels from your input
                        that you want to use in your output. With remixing, you might combine or split the
                        data in these channels, so the number of channels in your final output might be
                        different.

                        - **ChannelsOut** *(integer) --* Specify the number of channels in this output after
                        remixing. Valid values: 1, 2, 4, 6, 8... 64. (1 and even numbers to 64.)

                      - **StreamName** *(string) --* Specify a label for this output audio stream. For
                      example, "English", "Director commentary", or "track_2". For streaming outputs,
                  ...
                  - **CaptionDescriptions** *(list) --* Caption settings for this preset. There can be
                  multiple caption settings in a single output.
                    ...
                    - *(dict) --* Caption Description for preset
                      ...
                      the output captions metadata. If your output captions format is DVB-Sub or Burn in,
                      the encoder uses this language information when automatically selecting the font
                      ...
                      data is Embedded or Teletext.
                        ...
                        - **BurninDestinationSettings** *(dict) --* Burn-In Destination Settings.
                          ...
                          - **Alignment** *(string) --* If no explicit x_position or y_position is provided,
                          ...
                          coordinates. This option is not valid for source captions that are STL,
                          ...
                          - **BackgroundColor** *(string) --* Specifies the color of the rectangle behind
                          the captions. All burn-in and DVB-Sub font settings must match.
                          ...
                          - **FontResolution** *(integer) --* Font resolution in DPI (dots per inch);
                          default is 96 dpi. All burn-in and DVB-Sub font settings must match.

                          - **FontScript** *(string) --* Provide the font script, using an ISO 15924 script
                          code, if the LanguageCode is not sufficient for determining the script type. Where
                          LanguageCode or CustomLanguageCode is sufficient, use "AUTOMATIC" or leave unset.
                          This is used to help determine the appropriate font for rendering burn-in
                          captions.

                          - **FontSize** *(integer) --* A positive integer indicates the exact font size in
                          points. Set to 0 for automatic font size selection. All burn-in and DVB-Sub font
                          settings must match.
                          ...
                          - **OutlineSize** *(integer) --* Specifies font outline size in pixels. This
                          ...
                          - **TeletextSpacing** *(string) --* Only applies to jobs with input captions in
                          Teletext or STL formats. Specify whether the spacing between letters in your
                          captions is set by the captions grid or varies depending on letter width. Choose
                          fixed grid to conform to the spacing specified in the captions file more
                          accurately. Choose proportional to make the text easier to read if the captions
                          are closed caption.
                          ...
                          x_position is provided, the horizontal caption position will be determined by the
                          ...
                          captions starting 10 pixels from the top of the output. If no explicit y_position
                          is provided, the caption will be positioned towards the bottom of the output. This
                          option is not valid for source captions that are STL, 608/embedded or teletext.
                          These source settings are already pre-defined by the caption stream. All burn-in
                          and DVB-Sub font settings must match.

                        - **DestinationType** *(string) --* Specify the format for this set of captions on
                        this output. The default format is embedded without SCTE-20. Other options are
                        embedded with SCTE-20, burn-in, DVB-sub, IMSC, SCC, SRT, teletext, TTML, and
                        web-VTT. If you are using SCTE-20, choose SCTE-20 plus embedded
                        (SCTE20_PLUS_EMBEDDED) to create an output that complies with the SCTE-43 spec. To
                        ...
                        - **DvbSubDestinationSettings** *(dict) --* DVB-Sub Destination Settings
                          ...
                          code, if the LanguageCode is not sufficient for determining the script type. Where
                          LanguageCode or CustomLanguageCode is sufficient, use "AUTOMATIC" or leave unset.
                          This is used to help determine the appropriate font for rendering DVB-Sub
                          captions.
                        ...
                        - **EmbeddedDestinationSettings** *(dict) --* Settings specific to
                        embedded/ancillary caption outputs, including 608/708 Channel destination number.

                          - **Destination608ChannelNumber** *(integer) --* Ignore this setting unless your
                          input captions are SCC format and your output captions are embedded in the video
                          ...
                          1 and 3. For more information, see
                          https://docs.aws.amazon.com/console/mediaconvert/dual-scc-to-embedded.

                          - **Destination708ServiceNumber** *(integer) --* Ignore this setting unless your
                          input captions are SCC format and you want both 608 and 708 captions embedded in
                          your output stream. Optionally, specify the 708 service number for each output
                          captions channel. Choose a different number for each channel. To use this setting,
                          also set Force 608 to 708 upconvert (Convert608To708) to Upconvert (UPCONVERT) in
                          your input captions selector settings. If you choose to upconvert but don't
                          specify a 708 service number, MediaConvert uses the number that you specify for CC
                          channel number (destination608ChannelNumber) for the 708 service number. For more
                          information, see
                          https://docs.aws.amazon.com/console/mediaconvert/dual-scc-to-embedded.

                        - **ImscDestinationSettings** *(dict) --* Settings specific to IMSC caption outputs.

                          - **StylePassthrough** *(string) --* Keep this setting enabled to have
                          MediaConvert use the font style and position information from the captions source
                          in the output. This option is available only when your input captions are CFF-TT,
                          IMSC, SMPTE-TT, or TTML. Disable this setting for simplified output captions.

                        - **SccDestinationSettings** *(dict) --* Settings for SCC caption output.

                          - **Framerate** *(string) --* Set Framerate (SccDestinationFramerate) to make sure
                          that the captions and the video are synchronized in the output. Specify a frame
                        ...
                        - **TeletextDestinationSettings** *(dict) --* Settings for Teletext caption output
                          ...
                          - **PageNumber** *(string) --* Set pageNumber to the Teletext page number for the
                          destination captions for this output. This value must be a three-digit hexadecimal
                          string; strings ending in -FF are invalid. If you are passing through the entire
                          set of Teletext data, do not use this field.

                          - **PageTypes** *(list) --* Specify the page types for this Teletext page. If you
                          don't specify a value here, the service sets the page type to the default value
                          Subtitle (PAGE_TYPE_SUBTITLE). If you pass through the entire set of Teletext
                          data, don't use this field. When you pass through a set of Teletext pages, your
                          output has the same page types as your input.

                            - *(string) --* A page type as defined in the standard ETSI EN 300 468, Table 94

                        - **TtmlDestinationSettings** *(dict) --* Settings specific to TTML caption outputs,
                        including Pass style information (TtmlStylePassthrough).

                          - **StylePassthrough** *(string) --* Pass through style and position information
                          from a TTML-like input source (TTML, SMPTE-TT, CFF-TT) to the CFF-TT output or
                          TTML output.

                      - **LanguageCode** *(string) --* Specify the language of this captions output track.
                      For most captions output formats, the encoder puts this language information in the
                      output captions metadata. If your output captions format is DVB-Sub or Burn in, the
                      encoder uses this language information to choose the font language for rendering the
                      captions text.
                  ...
                  - **ContainerSettings** *(dict) --* Container specific settings.
                    ...
                    - **Container** *(string) --* Container for this output. Some containers require a
                    container settings object. If not specified, the default object will be created.

                    - **F4vSettings** *(dict) --* Settings for F4v container

                      - **MoovPlacement** *(string) --* If set to PROGRESSIVE_DOWNLOAD, the MOOV atom is
                      relocated to the beginning of the archive as required for progressive downloading.
                      Otherwise it is placed normally at the end.

                    - **M2tsSettings** *(dict) --* MPEG-2 TS container settings. These apply to outputs in a
                    File output group when the output's container (ContainerType) is MPEG-2 Transport Stream
                    (M2TS). In these assets, data is organized by the program map table (PMT). Each
                    transport stream program contains subsets of data, including audio, video, and metadata.
                    Each of these subsets of data has a numerical label called a packet identifier (PID).
                    Each transport stream program corresponds to one MediaConvert output. The PMT lists the
                    types of data in a program along with their PID. Downstream systems and players use the
                    program map table to look up the PID for each type of data it accesses and then uses the
                    PIDs to locate specific data within the asset.

                      - **AudioBufferModel** *(string) --* Selects between the DVB and ATSC buffer models
                      for Dolby Digital audio.

                      - **AudioFramesPerPes** *(integer) --* The number of audio frames to insert for each
                      PES packet.

                      - **AudioPids** *(list) --* Specify the packet identifiers (PIDs) for any elementary
                      audio streams you include in this output. Specify multiple PIDs as a JSON array.
                      Default is the range 482-492.

                        - *(integer) --*

                      - **Bitrate** *(integer) --* Specify the output bitrate of the transport stream in
                      ...
                      - **DvbNitSettings** *(dict) --* Inserts DVB Network Information Table (NIT) at the
                      specified table repetition interval.

                        - **NetworkId** *(integer) --* The numeric value placed in the Network Information
                        Table (NIT).

                        - **NetworkName** *(string) --* The network name text placed in the
                        network_name_descriptor inside the Network Information Table. Maximum length is 256
                        characters.

                        - **NitInterval** *(integer) --* The number of milliseconds between instances of
                        this table in the output transport stream.

                      - **DvbSdtSettings** *(dict) --* Inserts DVB Service Description Table (NIT) at the
                      specified table repetition interval.

                        - **OutputSdt** *(string) --* Selects method of inserting SDT information into
                        output stream. "Follow input SDT" copies SDT information from input stream to output
                        stream. "Follow input SDT if present" copies SDT information from input stream to
                        output stream if SDT information is present in the input, otherwise it will fall
                        ...
                        - **SdtInterval** *(integer) --* The number of milliseconds between instances of
                        this table in the output transport stream.

                        - **ServiceName** *(string) --* The service name placed in the service_descriptor in
                      ...
                      - **DvbSubPids** *(list) --* Specify the packet identifiers (PIDs) for DVB subtitle
                      data included in this output. Specify multiple PIDs as a JSON array. Default is the
                      range 460-479.
                      ...
                      specified table repetition interval.
                        ...
                        - **TdtInterval** *(integer) --* The number of milliseconds between instances of
                        this table in the output transport stream.

                      - **DvbTeletextPid** *(integer) --* Specify the packet identifier (PID) for DVB
                      teletext data you include in this output. Default is 499.

                      - **EbpAudioInterval** *(string) --* When set to VIDEO_AND_FIXED_INTERVALS, audio EBP
                      markers will be added to partitions 3 and 4. The interval between these additional
                      markers will be fixed, and will be slightly shorter than the video EBP marker
                      interval. When set to VIDEO_INTERVAL, these additional markers will not be inserted.
                      Only applicable when EBP segmentation markers are is selected (segmentationMarkers is
                      EBP or EBP_LEGACY).

                      - **EbpPlacement** *(string) --* Selects which PIDs to place EBP markers on. They can
                      either be placed only on the video PID, or on both the video PID and all audio PIDs.
                      Only applicable when EBP segmentation markers are is selected (segmentationMarkers is
                      EBP or EBP_LEGACY).

                      - **EsRateInPes** *(string) --* Controls whether to include the ES Rate field in the
                      PES header.

                      - **ForceTsVideoEbpOrder** *(string) --* Keep the default value (DEFAULT) unless you
                      know that your audio EBP markers are incorrectly appearing before your video EBP
                      markers. To correct this problem, set this value to Force (FORCE).

                      - **FragmentTime** *(float) --* The length, in seconds, of each fragment. Only used
                      with EBP markers.

                      - **MaxPcrInterval** *(integer) --* Specify the maximum time, in milliseconds, between
                      Program Clock References (PCRs) inserted into the transport stream.

                      - **MinEbpInterval** *(integer) --* When set, enforces that Encoder Boundary Points do
                      not come within the specified time interval of each other by looking ahead at input
                      video. If another EBP is going to come in within the specified time interval, the
                      current EBP is not emitted, and the segment is "stretched" to the next marker. The
                      lookahead value does not add latency to the system. The Live Event must be configured
                      elsewhere to create sufficient latency to make the lookahead accurate.

                      - **NielsenId3** *(string) --* If INSERT, Nielsen inaudible tones for media tracking
                      will be detected in the input audio and an equivalent ID3 tag will be inserted in the
                      output.

                      - **NullPacketBitrate** *(float) --* Value in bits per second of extra null packets to
                      insert into the transport stream. This can be used if a downstream encryption system
                      requires periodic null packets.

                      - **PatInterval** *(integer) --* The number of milliseconds between instances of this
                      table in the output transport stream.

                      - **PcrControl** *(string) --* When set to PCR_EVERY_PES_PACKET, a Program Clock
                      ...
                      - **PcrPid** *(integer) --* Specify the packet identifier (PID) for the program clock
                      reference (PCR) in this output. If you do not specify a value, the service will use
                      the value for Video PID (VideoPid).

                      - **PmtInterval** *(integer) --* Specify the number of milliseconds between instances
                      of the program map table (PMT) in the output transport stream.

                      - **PmtPid** *(integer) --* Specify the packet identifier (PID) for the program map
                      table (PMT) itself. Default is 480.

                      - **PrivateMetadataPid** *(integer) --* Specify the packet identifier (PID) of the
                      private metadata stream. Default is 503.

                      - **ProgramNumber** *(integer) --* Use Program number (programNumber) to specify the
                      program number used in the program map table (PMT) for this output. Default is 1.
                      Program numbers and program map tables are parts of MPEG-2 transport stream
                      containers, used for organizing data.

                      - **RateMode** *(string) --* When set to CBR, inserts null packets into transport
                      ...
                      - **Scte35Esam** *(dict) --* Include this in your job settings to put SCTE-35 markers
                      in your HLS and transport stream outputs at the insertion points that you specify in
                      an ESAM XML document. Provide the document in the setting SCC XML (sccXml).

                        - **Scte35EsamPid** *(integer) --* Packet Identifier (PID) of the SCTE-35 stream in
                        the transport stream generated by ESAM.

                      - **Scte35Pid** *(integer) --* Specify the packet identifier (PID) of the SCTE-35
                      stream in the transport stream.
                      ...
                      - **SegmentationStyle** *(string) --* The segmentation style parameter controls how
                      segmentation markers are inserted into the transport stream. With avails, it is
                      possible that segments may be truncated, which can influence where future segmentation
                      markers are inserted. When a segmentation style of "reset_cadence" is selected and a
                      ...
                      segment will likely be truncated as well. However, all segments after that will have a
                      duration of $segmentation_time seconds. Note that EBP lookahead is a slight exception
                      to this rule.

                      - **SegmentationTime** *(float) --* Specify the length, in seconds, of each segment.
                      Required unless markers is set to _none_.

                      - **TimedMetadataPid** *(integer) --* Specify the packet identifier (PID) for timed
                      metadata in this output. Default is 502.

                      - **TransportStreamId** *(integer) --* Specify the ID for the transport stream itself
                      in the program map table for this output. Transport stream IDs and program map tables
                      are parts of MPEG-2 transport stream containers, used for organizing data.

                      - **VideoPid** *(integer) --* Specify the packet identifier (PID) of the elementary
                    ...
                    - **M3u8Settings** *(dict) --* Settings for TS segments in HLS
                      ...
                      - **AudioPids** *(list) --* Packet Identifier (PID) of the elementary audio stream(s)
                      in the transport stream. Multiple values are accepted, and can be entered in ranges
                      ...
                      - **PcrControl** *(string) --* When set to PCR_EVERY_PES_PACKET a Program Clock
                      Reference value is inserted for every Packetized Elementary Stream (PES) header. This
                      parameter is effective only when the PCR PID is the same as the video or audio
                      elementary stream.

                      - **PcrPid** *(integer) --* Packet Identifier (PID) of the Program Clock Reference
                      (PCR) in the transport stream. When no value is given, the encoder will assign the
                      same value as the Video PID.

                      - **PmtInterval** *(integer) --* The number of milliseconds between instances of this
                      table in the output transport stream.

                      - **PmtPid** *(integer) --* Packet Identifier (PID) for the Program Map Table (PMT) in
                      the transport stream.

                      - **PrivateMetadataPid** *(integer) --* Packet Identifier (PID) of the private
                      metadata stream in the transport stream.

                      - **ProgramNumber** *(integer) --* The value of the program number field in the
                      Program Map Table.

                      - **Scte35Pid** *(integer) --* Packet Identifier (PID) of the SCTE-35 stream in the
                      transport stream.

                      - **Scte35Source** *(string) --* For SCTE-35 markers from your input-- Choose
                      Passthrough (PASSTHROUGH) if you want SCTE-35 markers that appear in your input to
                      also appear in this output. Choose None (NONE) if you don't want SCTE-35 markers in
                      this output. For SCTE-35 markers from an ESAM XML document-- Choose None (NONE) if you
                      don't want manifest conditioning. Choose Passthrough (PASSTHROUGH) and choose Ad
                      markers (adMarkers) if you do want manifest conditioning. In both cases, also provide
                      the ESAM XML as a string in the setting Signal processing notification XML (sccXml).

                      - **TimedMetadata** *(string) --* Applies only to HLS outputs. Use this setting to
                      specify whether the service inserts the ID3 timed metadata from the input in this
                      output.

                      - **TimedMetadataPid** *(integer) --* Packet Identifier (PID) of the timed metadata
                      stream in the transport stream.

                      - **TransportStreamId** *(integer) --* The value of the transport stream ID field in
                      the Program Map Table.

                      - **VideoPid** *(integer) --* Packet Identifier (PID) of the elementary video stream
                      in the transport stream.

                    - **MovSettings** *(dict) --* Settings for MOV Container.

                      - **ClapAtom** *(string) --* When enabled, include 'clap' atom if appropriate for the
                      video output settings.

                      - **CslgAtom** *(string) --* When enabled, file composition times will start at zero,
                      composition times in the 'ctts' (composition time to sample) box for B-frames will be
                      negative, and a 'cslg' (composition shift least greatest) box will be included per
                      14496-1 amendment 1. This improves compatibility with Apple players and tools.

                      - **Mpeg2FourCCControl** *(string) --* When set to XDCAM, writes MPEG2 video streams
                      into the QuickTime file using XDCAM fourcc codes. This increases compatibility with
                      Apple editors and players, but may decrease compatibility with other players. Only
                      applicable when the video codec is MPEG2.

                      - **PaddingControl** *(string) --* If set to OMNEON, inserts Omneon-compatible padding

                      - **Reference** *(string) --* Always keep the default value (SELF_CONTAINED) for this
                      setting.

                    - **Mp4Settings** *(dict) --* Settings for MP4 container. You can create audio-only AAC
                    outputs with this container.
                      ...
                      - **FreeSpaceBox** *(string) --* Inserts a free-space box immediately after the moov
                      box.
                    ...
                    - **MpdSettings** *(dict) --* Settings for MP4 segments in DASH
                      ...
                      output from your audio and video. Choose Raw (RAW) for captions in a single XML file
                      in a raw container. Choose Fragmented MPEG-4 (FRAGMENTED_MP4) for captions in XML
                      format contained within fragmented MP4 files. This set of fragmented MP4 files is
                      ...
                      in your input video file. Choose Passthrough (PASSTHROUGH) if you want SCTE-35 markers
                  ...
                  output.
                    ...
                    - **AfdSignaling** *(string) --* This setting only applies to H.264, H.265, and MPEG2
                    outputs. Use Insert AFD signaling (AfdSignaling) to specify whether the service includes
                    ...
                    - **AntiAlias** *(string) --* The anti-alias filter is automatically applied to all
                    outputs. The service no longer accepts the value DISABLED for AntiAlias. If you specify
                    ...
                    FrameCaptureSettings
                      ...
                      - **Codec** *(string) --* Specifies the video codec. This must be equal to one of the
                      enum values defined by the object VideoCodec.

                      - **FrameCaptureSettings** *(dict) --* Required when you set (Codec) under
                      (VideoDescription)>(CodecSettings) to the value FRAME_CAPTURE.

                        - **FramerateDenominator** *(integer) --* Frame capture will encode the first frame
                        of the output stream, then one frame every framerateDenominator/framerateNumerator
                        ...
                        - **FramerateNumerator** *(integer) --* Frame capture will encode the first frame of
                        the output stream, then one frame every framerateDenominator/framerateNumerator
                        seconds. For example, settings of framerateNumerator = 1 and framerateDenominator =
                        3 (a rate of 1/3 frame per second) will capture the first frame, then 1 frame every
                        3s. Files will be named as filename.NNNNNNN.jpg where N is the 0-based frame
                        sequence number zero padded to 7 decimal places.

                        - **MaxCaptures** *(integer) --* Maximum number of captures (encoded jpg output
                        files).
                      ...
                      (VideoDescription)>(CodecSettings) to the value H_264.
                        ...
                        - **Bitrate** *(integer) --* Specify the average bitrate in bits per second.
                        Required for VBR and CBR. For MS Smooth outputs, bitrates must be unique when
                        rounded down to the nearest multiple of 1000.

                        - **CodecLevel** *(string) --* Specify an H.264 level that is consistent with your
                        output video settings. If you aren't sure what level to specify, choose Auto (AUTO).

                        - **CodecProfile** *(string) --* H.264 Profile. High 4:2:2 and 10-bit profiles are
                        only available with the AVC-I License.

                        - **DynamicSubGop** *(string) --* Choose Adaptive to improve subjective video
                        quality for high-motion content. This will cause the service to use fewer B-frames
                        (which infer information based on other frames) for high-motion portions of the
                        video and more B-frames for low-motion portions. The maximum number of B-frames is
                        ...
                        - **FieldEncoding** *(string) --* Choosing FORCE_FIELD disables PAFF encoding for
                        interlaced outputs.

                        - **FlickerAdaptiveQuantization** *(string) --* Adjust quantization within each
                        frame to reduce flicker or 'pop' on I-frames.

                        - **FramerateControl** *(string) --* If you are using the console, use the Framerate
                        setting to specify the frame rate for this output. If you want to keep the same
                        frame rate as the input video, choose Follow source. If you want to do frame rate
                        conversion, choose a frame rate from the dropdown list or choose Custom. The
                        ...
                        FramerateControl to specify which value the service uses for the frame rate for this
                        output. Choose INITIALIZE_FROM_SOURCE if you want the service to use the frame rate
                        from the input. Choose SPECIFIED if you want the service to use the frame rate you
                        specify in the settings FramerateNumerator and FramerateDenominator.

                        - **FramerateConversionAlgorithm** *(string) --* When set to INTERPOLATE, produces
                        smoother motion during frame rate conversion.

                        - **FramerateDenominator** *(integer) --* When you use the API for transcode jobs
                        that use frame rate conversion, specify the frame rate as a fraction. For example,
                        ...
                        - **FramerateNumerator** *(integer) --* Frame rate numerator - frame rate is a
                        ...
                        - **GopClosedCadence** *(integer) --* Frequency of closed GOPs. In streaming
                        applications, it is recommended that this be set to 1 so a decoder joining
                        mid-stream will receive an IDR frame as quickly as possible. Setting this value to 0
                        will break output segmenting.

                        - **GopSize** *(float) --* GOP Length (keyframe interval) in frames or seconds. Must
                        be greater than zero.

                        - **GopSizeUnits** *(string) --* Indicates if the GOP Size in H264 is specified in
                        frames or seconds. If seconds the system will convert the GOP Size into a frame
                        count at run time.

                        - **HrdBufferInitialFillPercentage** *(integer) --* Percentage of the buffer that
                        should initially be filled (HRD buffer model).

                        - **HrdBufferSize** *(integer) --* Size of buffer (HRD buffer model) in bits. For
                        example, enter five megabits as 5000000.

                        - **InterlaceMode** *(string) --* Use Interlace mode (InterlaceMode) to choose the
                        scan line type for the output. * Top Field First (TOP_FIELD) and Bottom Field First
                        (BOTTOM_FIELD) produce interlaced output with the entire output having the same
                        field polarity (top or bottom first). * Follow, Default Top (FOLLOW_TOP_FIELD) and
                        Follow, Default Bottom (FOLLOW_BOTTOM_FIELD) use the same field polarity as the
                        source. Therefore, behavior depends on the input scan type, as follows. - If the
                        source is interlaced, the output will be interlaced with the same polarity as the
                        source (it will follow the source). The output could therefore be a mix of "top
                        field first" and "bottom field first". - If the source is progressive, the output
                        will be interlaced with "top field first" or "bottom field first" polarity,
                        depending on which of the Follow options you chose.

                        - **MaxBitrate** *(integer) --* Maximum bitrate in bits/second. For example, enter
                        five megabits per second as 5000000. Required when Rate control mode is QVBR.

                        - **MinIInterval** *(integer) --* Enforces separation between repeated (cadence)
                        ...
                        to the scene change I-frame. GOP stretch requires enabling lookahead as well as
                        setting I-interval. The normal cadence resumes for the next GOP. This setting is
                        only used when Scene Change Detect is enabled. Note: Maximum GOP stretch = GOP size
                        + Min-I-interval - 1

                        - **NumberBFramesBetweenReferenceFrames** *(integer) --* Number of B-frames between
                        reference frames.

                        - **NumberReferenceFrames** *(integer) --* Number of reference frames to use. The
                        encoder may use more than requested if using B-frames and/or interlaced encoding.

                        - **ParControl** *(string) --* Using the API, enable ParFollowSource if you want the
                        service to use the pixel aspect ratio from the input. Using the console, do this by
                        ...
                        - **QualityTuningLevel** *(string) --* Use Quality tuning level
                        (H264QualityTuningLevel) to specifiy whether to use fast single-pass, high-quality
                        ...
                        define Rate control mode.
                          ...
                          - **MaxAverageBitrate** *(integer) --* Use this setting only when Rate control
                          mode is QVBR and Quality tuning level is Multi-pass HQ. For Max average bitrate
                          values suited to the complexity of your input video, the service limits the
                          average bitrate of the video part of this output to the value that you choose.
                          That is, the total size of the video element is less than or equal to the value
                          you set multiplied by the number of seconds of encoded output.

                          - **QvbrQualityLevel** *(integer) --* Required when you use QVBR rate control
                          mode. That is, when you specify qvbrSettings within h264Settings. Specify the
                          target quality level for this output, from 1 to 10. Use higher numbers for greater
                          quality. Level 10 results in nearly lossless compression. The quality level for
                          most broadcast-quality transcodes is between 6 and 9.

                        - **RateControlMode** *(string) --* Use this setting to specify whether this output
                        has a variable bitrate (VBR), constant bitrate (CBR) or quality-defined variable
                        bitrate (QVBR).

                        - **RepeatPps** *(string) --* Places a PPS header on each encoded picture, even if
                        repeated.

                        - **SceneChangeDetect** *(string) --* Enable this setting to insert I-frames at
                        scene changes that the service automatically detects. This improves video quality
                        and is enabled by default. If this output uses QVBR, choose Transition detection
                        (TRANSITION_DETECTION) for further video quality improvement. For more information
                        about QVBR, see https://docs.aws.amazon.com/console/mediaconvert/cbr-vbr-qvbr.
                        ...
                        - **SlowPal** *(string) --* Enables Slow PAL rate conversion. 23.976fps and 24fps
                        input is relabeled as 25fps, and audio is sped up correspondingly.

                        - **Softness** *(integer) --* Softness. Selects quantizer matrix, larger values
                        reduce high-frequency content in the encoded image.

                        - **SpatialAdaptiveQuantization** *(string) --* Adjust quantization within each
                        frame based on spatial variation of content complexity.

                        - **Syntax** *(string) --* Produces a bitstream compliant with SMPTE RP-2027.

                        - **Telecine** *(string) --* This field applies only if the Streams > Advanced >
                        ...
                        Advanced > Interlaced Mode field (interlace_mode) to identify the scan type for the
                        output: Progressive, Interlaced, Hard Telecine or Soft Telecine. - Hard: produces
                        29.97i output from 23.976 input. - Soft: produces 23.976; the player converts this
                        output to 29.97i.

                        - **TemporalAdaptiveQuantization** *(string) --* Adjust quantization within each
                        frame based on temporal variation of content complexity.

                        - **UnregisteredSeiTimecode** *(string) --* Inserts timecode for each frame as 4
                      ...
                      - **H265Settings** *(dict) --* Settings for H265 codec
                        ...
                        - **AlternateTransferFunctionSei** *(string) --* Enables Alternate Transfer Function
                        SEI message for outputs using Hybrid Log Gamma (HLG) Electro-Optical Transfer
                        Function (EOTF).
                        ...
                        - **CodecProfile** *(string) --* Represents the Profile and Tier, per the HEVC
                        (H.265) specification. Selections are grouped as [Profile] / [Tier], so "Main/High"
                        ...
                        - **GopSizeUnits** *(string) --* Indicates if the GOP Size in H265 is specified in
                        frames or seconds. If seconds the system will convert the GOP Size into a frame
                        count at run time.
                        ...
                        - **InterlaceMode** *(string) --* Choose the scan line type for the output. Choose
                        Progressive (PROGRESSIVE) to create a progressive output, regardless of the scan
                        type of your input. Choose Top Field First (TOP_FIELD) or Bottom Field First
                        ...
                        (FOLLOW_BOTTOM_FIELD) to create an interlaced output with the same field polarity as
                        the source. If the source is interlaced, the output will be interlaced with the same
                        polarity as the source (it will follow the source). The output could therefore be a
                        mix of "top field first" and "bottom field first". If the source is progressive,
                        your output will be interlaced with "top field first" or "bottom field first"
                        polarity, depending on which of the Follow options you chose. If you don't choose a
                        ...
                        - **QualityTuningLevel** *(string) --* Use Quality tuning level
                        (H265QualityTuningLevel) to specifiy whether to use fast single-pass, high-quality
                        singlepass, or high-quality multipass video encoding.
                        ...
                        define Rate control mode.
                          ...
                          - **QvbrQualityLevel** *(integer) --* Required when you use QVBR rate control
                          mode. That is, when you specify qvbrSettings within h265Settings. Specify the
                          target quality level for this output, from 1 to 10. Use higher numbers for greater
                          quality. Level 10 results in nearly lossless compression. The quality level for
                          most broadcast-quality transcodes is between 6 and 9.
                        ...
                        - **SampleAdaptiveOffsetFilterMode** *(string) --* Specify Sample Adaptive Offset
                        (SAO) filter strength. Adaptive mode dynamically selects best strength based on
                        content
                        ...
                        - **TemporalIds** *(string) --* Enables temporal layer identifiers in the encoded
                        bitstream. Up to 3 layers are supported depending on GOP structure: I- and P-frames
                        form one layer, reference B-frames can form a second layer and non-reference
                        b-frames can form a third layer. Decoders can optionally decode only the lower
                        temporal layers to generate a lower frame rate output. For example, given a
                        bitstream with temporal IDs and with b-frames = 1 (i.e. IbPbPb display order), a
                        decoder could decode all the frames for full frame rate output or only the I and P
                        frames (lowest temporal layer) for a half frame rate output.

                        - **Tiles** *(string) --* Enable use of tiles, allowing horizontal as well as
                        ...
                        doesn't matter in your workflow, ignore this setting. Use this setting only with
                        CMAF or DASH outputs, or with standalone file outputs in an MPEG-4 container (MP4
                      ...
                      (VideoDescription)>(CodecSettings) to the value MPEG2.
                        ...
                        - **CodecLevel** *(string) --* Use Level (Mpeg2CodecLevel) to set the MPEG-2 level
                        for the video output.

                        - **CodecProfile** *(string) --* Use Profile (Mpeg2CodecProfile) to set the MPEG-2
                        ...
                        - **GopSizeUnits** *(string) --* Indicates if the GOP Size in MPEG2 is specified in
                        frames or seconds. If seconds the system will convert the GOP Size into a frame
                        count at run time.
                        ...
                        field polarity (top or bottom first). * Follow, Default Top (FOLLOW_TOP_FIELD) and
                        Follow, Default Bottom (FOLLOW_BOTTOM_FIELD) use the same field polarity as the
                        source. Therefore, behavior depends on the input scan type. - If the source is
                        ...
                        and "bottom field first". - If the source is progressive, the output will be
                        interlaced with "top field first" or "bottom field first" polarity, depending on
                        which of the Follow options you chose.

                        - **IntraDcPrecision** *(string) --* Use Intra DC precision (Mpeg2IntraDcPrecision)
                        to set quantization precision for intra-block DC coefficients. If you choose the
                        value auto, the service will automatically select the precision based on the
                        ...
                        - **QualityTuningLevel** *(string) --* Use Quality tuning level
                        (Mpeg2QualityTuningLevel) to specifiy whether to use single-pass or multipass video
                        encoding.

                        - **RateControlMode** *(string) --* Use Rate control mode (Mpeg2RateControlMode) to
                        specifiy whether the bitrate is variable (vbr) or constant (cbr).
                        ...
                        - **Telecine** *(string) --* Only use Telecine (Mpeg2Telecine) when you set
                        Framerate (Framerate) to 29.970. Set Telecine (Mpeg2Telecine) to Hard (hard) to
                        produce a 29.97i output from a 23.976 input. Set it to Soft (soft) to produce 23.976
                      ...
                      (VideoDescription)>(CodecSettings) to the value PRORES.
                        ...
                        - **CodecProfile** *(string) --* Use Profile (ProResCodecProfile) to specifiy the
                        ...
                        - **FramerateNumerator** *(integer) --* When you use the API for transcode jobs that
                        use frame rate conversion, specify the frame rate as a fraction. For example, 24000
                        / 1001 = 23.976 fps. Use FramerateNumerator to specify the numerator of this
                        ...
                        - **ParControl** *(string) --* Use (ProresParControl) to specify how the service
                        determines the pixel aspect ratio. Set to Follow source (INITIALIZE_FROM_SOURCE) to
                        use the pixel aspect ratio from the input. To specify a different pixel aspect
                        ratio: Using the console, choose it from the dropdown menu. Using the API, set
                        ProresParControl to (SPECIFIED) and provide for (ParNumerator) and (ParDenominator).
                        ...
                        - **Telecine** *(string) --* Only use Telecine (ProresTelecine) when you set
                        Framerate (Framerate) to 29.970. Set Telecine (ProresTelecine) to Hard (hard) to
                        produce a 29.97i output from a 23.976 input. Set it to Soft (soft) to produce 23.976
                        output and leave converstion to the player.

                    - **ColorMetadata** *(string) --* Choose Insert (INSERT) for this setting to include
                    color metadata in this output. Choose Ignore (IGNORE) to exclude color metadata from
                    this output. If you don't specify a value, the service sets this to Insert by default.

                    - **Crop** *(dict) --* Use Cropping selection (crop) to specify the video area that the
                    service will include in the output video frame.
                      ...
                      - **Y** *(integer) --* The distance, in pixels, between the rectangle and the top edge
                      of the video frame. Specify only even numbers.

                    - **DropFrameTimecode** *(string) --* Applies only to 29.97 fps outputs. When this
                    feature is enabled, the service will use drop-frame timecode on outputs. If it is not
                    possible to use drop-frame timecode, the system will fall back to non-drop-frame. This
                    setting is enabled by default when Timecode insertion (TimecodeInsertion) is enabled.

                    - **FixedAfd** *(integer) --* Applies only if you set AFD Signaling(AfdSignaling) to
                    Fixed (FIXED). Use Fixed (FixedAfd) to specify a four-bit AFD value which the service
                    will write on all frames of this video output.

                    - **Height** *(integer) --* Use the Height (Height) setting to define the video
                    resolution height for this output. Specify in pixels. If you don't provide a value here,
                    ...
                    - **RespondToAfd** *(string) --* Use Respond to AFD (RespondToAfd) to specify how the
                    service changes the video itself in response to AFD values in the input. * Choose
                    Respond to clip the input video frame according to the AFD value, input display aspect
                    ratio, and output display aspect ratio. * Choose Passthrough to include the input AFD
                    values. Do not choose this when AfdSignaling is set to (NONE). A preferred
                    implementation of this workflow is to set RespondToAfd to (NONE) and set AfdSignaling to
                    (AUTO). * Choose None to remove all input AFD values from this output.

                    - **ScalingBehavior** *(string) --* Specify how the service handles outputs that have a
                    different aspect ratio from the input aspect ratio. Choose Stretch to output
                    (STRETCH_TO_OUTPUT) to have the service stretch your video image to fit. Keep the
                    setting Default (DEFAULT) to have the service letterbox your video instead. This setting
                    overrides any value that you specify for the setting Selection placement (position) in
                    this output.

                    - **Sharpness** *(integer) --* Use Sharpness (Sharpness) setting to specify the strength
                    ...
                    - **TimecodeInsertion** *(string) --* Applies only to H.264, H.265, MPEG2, and ProRes
                    outputs. Only enable Timecode insertion when the input frame rate is identical to the
                    output frame rate. To include timecodes in this output, set Timecode insertion
                    (VideoTimecodeInsertion) to PIC_TIMING_SEI. To leave them out, set it to DISABLED.
                    Default is DISABLED. When the service inserts timecodes in an output, by default, it
                    uses any embedded timecodes from the input. If none are present, the service will set
                    the timecode for the first output frame to zero. To change this default behavior, adjust
                    the settings under Timecode configuration (TimecodeConfig). In the console, these
                    settings are located under Job > Job settings > Timecode configuration. Note - Timecode
                    source under input settings (InputTimecodeSource) does not affect the timecodes that are
                    inserted in the output. Source under Job settings > Timecode configuration
                    (TimecodeSource) does.

                    - **VideoPreprocessors** *(dict) --* Find additional transcoding features under
                    Preprocessors (VideoPreprocessors). Enable the features at each output individually.
                    These features are disabled by default.

                      - **ColorCorrector** *(dict) --* Enable the Color corrector (ColorCorrector) feature
                      if necessary. Enable or disable this feature for each output individually. This
                      setting is disabled by default.

                        - **Brightness** *(integer) --* Brightness level.

                        - **ColorSpaceConversion** *(string) --* Specify the color space you want for this
                        output. The service supports conversion between HDR formats, between SDR formats,
                        and from SDR to HDR. The service doesn't support conversion from HDR to SDR. SDR to
                        HDR conversion doesn't upgrade the dynamic range. The converted video has an HDR
                        format, but visually appears the same as an unconverted output.

                        - **Contrast** *(integer) --* Contrast level.

                        - **Hdr10Metadata** *(dict) --* Use these settings when you convert to the HDR 10
                        color space. Specify the SMPTE ST 2086 Mastering Display Color Volume static
                        metadata that you want signaled in the output. These values don't affect the pixel
                        values that are encoded in the video stream. They are intended to help the
                        downstream video player display content in a way that reflects the intentions of the
                        the content creator. When you set Color space conversion (ColorSpaceConversion) to
                        HDR 10 (FORCE_HDR10), these settings are required. You must set values for Max frame
                        ...
                        values for the other HDR 10 metadata settings are defined by the P3D65 color space.
                      ...
                      - **Deinterlacer** *(dict) --* Use Deinterlacer (Deinterlacer) to produce smoother
                      motion and a clearer picture.

                        - **Algorithm** *(string) --* Only applies when you set Deinterlacer
                        (DeinterlaceMode) to Deinterlace (DEINTERLACE) or Adaptive (ADAPTIVE). Motion
                        adaptive interpolate (INTERPOLATE) produces sharper pictures, while blend (BLEND)
                        produces smoother motion. Use (INTERPOLATE_TICKER) OR (BLEND_TICKER) if your source
                        file includes a ticker, such as a scrolling headline at the bottom of the frame.

                        - **Control** *(string) --* - When set to NORMAL (default), the deinterlacer does
                        not convert frames that are tagged in metadata as progressive. It will only convert
                        those that are tagged as some other type. - When set to FORCE_ALL_FRAMES, the
                        deinterlacer converts every frame to progressive - even those that are already
                        tagged as progressive. Turn Force mode on only if there is a good chance that the
                        metadata has tagged frames as progressive when they are not progressive. Do not turn
                        on otherwise; processing frames that are already progressive into progressive will
                        probably result in lower quality video.

                        - **Mode** *(string) --* Use Deinterlacer (DeinterlaceMode) to choose how the
                      ...
                      - **NoiseReducer** *(dict) --* Enable the Noise reducer (NoiseReducer) feature to
                      remove noise from your video output if necessary. Enable or disable this feature for
                      each output individually. This setting is disabled by default.

                        - **Filter** *(string) --* Use Noise reducer filter (NoiseReducerFilter) to select
                        one of the following spatial image filtering functions. To use this setting, you
                        must also enable Noise reducer (NoiseReducer). * Bilateral preserves edges while
                        reducing noise. * Mean (softest), Gaussian, Lanczos, and Sharpen (sharpest) do
                        convolution filtering. * Conserve does min/max noise reduction. * Spatial does
                        frequency-domain filtering based on JND principles. * Temporal optimizes video
                        quality for complex motion.

                        - **FilterSettings** *(dict) --* Settings for a noise reducer filter

                          - **Strength** *(integer) --* Relative strength of noise reducing filter. Higher
                          values produce stronger filtering.

                        - **SpatialFilterSettings** *(dict) --* Noise reducer filter settings for spatial
                        filter.

                          - **PostFilterSharpenStrength** *(integer) --* Specify strength of post noise
                          reduction sharpening filter, with 0 disabling the filter and 3 enabling it at
                          maximum strength.

                          - **Speed** *(integer) --* The speed of the filter, from -2 (lower speed) to 3
                          (higher speed), with 0 being the nominal value.

                          - **Strength** *(integer) --* Relative strength of noise reducing filter. Higher
                          values produce stronger filtering.

                        - **TemporalFilterSettings** *(dict) --* Noise reducer filter settings for temporal
                        filter.

                          - **AggressiveMode** *(integer) --* Use Aggressive mode for content that has
                          complex motion. Higher values produce stronger temporal filtering. This filters
                          highly complex scenes more aggressively and creates better VQ for low bitrate
                          outputs.

                          - **Speed** *(integer) --* The speed of the filter (higher number is faster). Low
                          setting reduces bit rate at the cost of transcode time, high setting improves
                          transcode time at the cost of bit rate.

                          - **Strength** *(integer) --* Specify the strength of the noise reducing filter on
                          this output. Higher values produce stronger filtering. We recommend the following
                          value ranges, depending on the result that you want: * 0-2 for complexity
                          reduction with minimal sharpness loss * 2-8 for complexity reduction with image
                          preservation * 8-16 for a high level of complexity reduction

                      - **TimecodeBurnin** *(dict) --* Timecode burn-in (TimecodeBurnIn)--Burns the output
                      timecode and specified prefix into the output.

                        - **FontSize** *(integer) --* Use Font Size (FontSize) to set the font size of any
                        burned-in timecode. Valid values are 10, 16, 32, 48.

                        - **Position** *(string) --* Use Position (Position) under under Timecode burn-in
                        (TimecodeBurnIn) to specify the location the burned-in timecode on output video.

                        - **Prefix** *(string) --* Use Prefix (Prefix) to place ASCII characters before any
                        burned-in timecode. For example, a prefix of "EZ-" will result in the timecode
                        "EZ-00:00:00:00". Provide either the characters themselves or the ASCII code
                        equivalents. The supported range of characters is 0x20 through 0x7e. This includes
                        letters, numbers, and all special characters represented on a standard English
                        keyboard.

                    - **Width** *(integer) --* Use Width (Width) to define the video resolution width, in
                    pixels, for this output. If you don't provide a value here, the service will use the
                    input width.

                - **Type** *(string) --* A preset can be of two types: system or custom. System or built-in
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Queues** *(list) --* List of queues.
              ...
              - *(dict) --* You can use queues to manage the resources that are available to your AWS
              account for running multiple transcoding jobs at the same time. If you don't specify a queue,
              the service sends all jobs through the default queue. For more information, see
              https://docs.aws.amazon.com/mediaconvert/latest/ug/working-with-queues.html.

                - **Arn** *(string) --* An identifier for this resource that is unique within all of AWS.

                - **CreatedAt** *(datetime) --* The timestamp in epoch seconds for when you created the
                queue.

                - **Description** *(string) --* An optional description that you create for each queue.

                - **LastUpdated** *(datetime) --* The timestamp in epoch seconds for when you most recently
                updated the queue.

                - **Name** *(string) --* A name that you create for each queue. Each name must be unique
                within your account.

                - **PricingPlan** *(string) --* Specifies whether the pricing plan for the queue is
                on-demand or reserved. For on-demand, you pay per minute, billed in increments of .01
                minute. For reserved, you pay for the transcoding capacity of the entire queue, regardless
                of how much or how little you use it. Reserved pricing requires a 12-month commitment.

                - **ProgressingJobsCount** *(integer) --* The estimated number of jobs with a PROGRESSING
                status.

                - **ReservationPlan** *(dict) --* Details about the pricing plan for your reserved queue.
                Required for reserved queues and not applicable to on-demand queues.

                  - **Commitment** *(string) --* The length of the term of your reserved queue pricing plan
                  commitment.

                  - **ExpiresAt** *(datetime) --* The timestamp in epoch seconds for when the current
                  pricing plan term for this reserved queue expires.

                  - **PurchasedAt** *(datetime) --* The timestamp in epoch seconds for when you set up the
                  current pricing plan for this reserved queue.

                  - **RenewalType** *(string) --* Specifies whether the term of your reserved queue pricing
                  plan is automatically extended (AUTO_RENEW) or expires (EXPIRE) at the end of the term.

                  - **ReservedSlots** *(integer) --* Specifies the number of reserved transcode slots (RTS)
                  for this queue. The number of RTS determines how many jobs the queue can process in
                  parallel; each RTS can process one job at a time. When you increase this number, you
                  extend your existing commitment with a new 12-month commitment for a larger number of RTS.
                  The new commitment begins when you purchase the additional capacity. You can't decrease
                  the number of RTS in your reserved queue.

                  - **Status** *(string) --* Specifies whether the pricing plan for your reserved queue is
                  ACTIVE or EXPIRED.

                - **Status** *(string) --* Queues can be ACTIVE or PAUSED. If you pause a queue, the service
                won't begin processing jobs in that queue. Jobs that are running when you pause the queue
                continue to run until they finish or result in an error.

                - **SubmittedJobsCount** *(integer) --* The estimated number of jobs with a SUBMITTED
                status.

                - **Type** *(string) --* Specifies whether this on-demand queue is system or custom. System
                queues are built in. You can't modify or delete system queues. You can create and modify
                custom queues.
