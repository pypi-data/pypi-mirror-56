...
import numpy as np
from azureml.studio.core.utils.missing_value_utils import is_na, has_na, drop_na
...
def normalize_dataframe_for_output(data: pd.DataFrame) -> pd.DataFrame:
    for column_name in data.columns:
        data[column_name] = normalize_column_for_output(data[column_name])
    return data
    
def normalize_column_for_output(column: pd.Series) -> pd.Series:
    if pd.core.dtypes.common.is_timedelta64_dtype(column.dropna()) or is_type(column, datetime.timedelta, pd.Timedelta):
        return pd.to_timedelta(column)
    if pd.core.dtypes.common.is_datetimelike(column.dropna()) or is_type(column, datetime.datetime, datetime.date, pd.Timestamp):
        return pd.to_datetime(column)
    if column.dtype.name == "bool" or pd.api.types.infer_dtype(column) == "boolean" or is_type(column,bool):
...
def is_type(column: pd.Series, *types) -> bool:
    ...
    for check_type in types:
        if drop_na.apply(lambda x: isinstance(x, check_type)).astype(bool).all():
            return True
    return False
