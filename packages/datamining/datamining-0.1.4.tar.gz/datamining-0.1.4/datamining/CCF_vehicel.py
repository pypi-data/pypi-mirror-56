...
path = './CCF_vehicel_seal_predict/Train/'
train_sales_data = pd.read_csv(path+'train_sales_data.csv', encoding="utf-8")
train_search_data = pd.read_csv(path+'train_search_data.csv', encoding="utf-8")
train_user_reply_data = pd.read_csv(path+'train_user_reply_data.csv', encoding="utf-8")
test = pd.read_csv(path+'evaluation_public.csv', encoding="utf-8")
...
# 根据月份添加权重值
a = 6; b = 4
data['weightMonth'] = data['regMonth'].map({1:a, 2:a, 3:a, 4:a, 5:b, 6:b, 7:b, 8:b, 9:b, 10:b, 11:b, 12:b,})
...
df_lgb.to_csv("ccf_car_sales.csv", index=False)