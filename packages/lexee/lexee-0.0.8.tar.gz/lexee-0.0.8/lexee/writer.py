...
class LexeeWriter:
    ...
    def save(path, lexeeDoc:lexee.LexeeDoc):
        file = open(path,"w") 
        lingoWriter.writeConfig(file, lexeeDoc.config)
        [lingoWriter.writeImport(file, lexeeDoc.imports[key]) for key in lexeeDoc.imports.keys()]
        [lingoWriter.writeExpression(file, expression) for expression in lexeeDoc.doc]

    @staticmethod
    def writeConfig(file, config:dict): 
        print("config", end=" {", file=file)
        lingoWriter.braces.append("\t")
        print("")
        for key in [key for key in config.__dict__.keys() if key not in ["identity","current","depends_on"]] :
            print("".join(lingoWriter.braces), end=key, file=file)
            print(" =", end=" ", file=file)
            print(json.dumps(config.__dict__[key], indent=5), file=file)
        print("".join(lingoWriter.braces), end="", file=file)
        lingoWriter.braces.pop()
        print("}", file=file)
        print("", file=file)
    ...
    def writeExpression(file, expression:lexee.expression.LexeeExpression): 
        ...
        print("".join(lingoWriter.braces), end="output", file=file)
        print(" =", end=" ", file=file)
