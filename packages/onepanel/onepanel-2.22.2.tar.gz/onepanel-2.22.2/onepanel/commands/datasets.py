...
class DatasetIdentifier:
    ...
    class InvalidPath(Exception):
        ...
        def __init__(self, path):
            Exception.__init__(self, 'Invalid dataset path {}. Please use <account_uid>/datasets/<uid>'.format(path))
...
class UploadDataset:
    ...
    def start_upload(self, directory_stats):
        data = {
    ...
    def upload(self, version, path='/', quiet=False, delete=False):
        provider_storage_push_to_dir = version['data']['version']['path']

        facade = CloudStorageUtility.get_facade(self.account_uid, 'datasets', self.dataset_uid)
    ...
    def __call__(self, vc, account_uid, dataset_uid, path='/', delete=False, directory=None, quiet=False):
        """
        :param vc:
        ...
        try:
            self.start_upload(upload_info['directory_stats'])
            self.upload_started(upload_info['version'])
            self.upload(upload_info['version'], path=path, delete=delete, quiet=quiet)
...
@click.pass_context
@login_required
def datasets_push(ctx, account, dataset, path, message, name, update_version, quiet, background, watch, in_background, delete):
    _datasets_push(ctx, account, dataset, path, message, name, update_version, quiet, background, watch, in_background, delete)


def _datasets_push(ctx, account, dataset, path, message, name, update_version, quiet, background, watch, in_background, delete):
    ...
    try:
        ...
        dataset_update(vc, vc.account_uid, vc.dataset_uid, path=path, delete=delete, quiet=quiet)
