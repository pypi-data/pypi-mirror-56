...
import oktaauth.okta_awscli as ow
...
of roles assigned to you.""" % self.role)
    ...
    def list_roles(self, assertion):
        """ Function to list roles (fetch) """
        roles = self.__extract_available_roles_from(assertion)
        if self.role:
            predefined_role = self.__find_predefiend_role_from(roles)
            if predefined_role:
...
of roles assigned to you.""" % self.role)
        ...
        finalrole = []
        for option in range(len(role_options)):
            finalrole.append(roles[option])

        return finalrole

    def update_role(self, assertion, request):
        """ Function to list roles (fetch) """
