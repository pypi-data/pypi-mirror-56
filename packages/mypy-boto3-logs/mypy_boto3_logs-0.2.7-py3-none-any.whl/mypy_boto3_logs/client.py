...
class Client(BaseClient):
    ...
    def associate_kms_key(self, logGroupName: str, kmsKeyId: str) -> None:
        ...
        Associating an AWS KMS CMK with a log group overrides any existing associations between the log
        group and a CMK. After a CMK is associated with a log group, all newly ingested data for the log
        group is encrypted using the CMK. This association is stored as long as the data encrypted with the
        CMK is still within Amazon CloudWatch Logs. This enables Amazon CloudWatch Logs to decrypt this data
        ...
        **Request Syntax**
        ...
        :param kmsKeyId: **[REQUIRED]**
          ...
          The Amazon Resource Name (ARN) of the CMK to use when encrypting log data. For more information,
          see `Amazon Resource Names - AWS Key Management Service (AWS KMS)
    ...
    def create_export_task(
        ...
        separate out log data for each export task, you can specify a prefix to be used as the Amazon S3 key
    ...
    def create_log_group(
        ...
        * Log group names consist of the following characters: a-z, A-Z, 0-9, '_' (underscore), '-'
        ...
        encrypted with the CMK is still within Amazon CloudWatch Logs. This enables Amazon CloudWatch Logs
        to decrypt this data whenever it is requested.

        If you attempt to associate a CMK with the log group but the CMK does not exist or the CMK is
        disabled, you will receive an ``InvalidParameterException`` error.
    ...
    def describe_log_streams(
        ...
        results by prefix. You can also control how the results are ordered.
        ...
        **Request Syntax**
        ...
        :param orderBy:
          ...
          CloudWatch Logs. This number is expressed as the number of milliseconds after Jan 1, 1970 00:00:00
          UTC. lastEventTimeStamp updates on an eventual consistency basis. It typically updates in less
            ...
            - **logStreams** *(list) --*
              ...
              - *(dict) --*
                ...
                - **lastEventTimestamp** *(integer) --*
                  ...
                  expressed as the number of milliseconds after Jan 1, 1970 00:00:00 UTC. The
    ...
    def describe_metric_filters(
        ...
        """
        Lists the specified metric filters. You can list all the metric filters or filter the results by log
        name, prefix, metric name, or metric namespace. The results are ASCII-sorted by filter name.
    ...
    def describe_queries(
        ...
        """
        Returns a list of CloudWatch Logs Insights queries that are scheduled, executing, or have been
        executed recently in this account. You can request all queries, or limit it to queries of a specific
    ...
    def disassociate_kms_key(self, logGroupName: str) -> None:
        """
        Disassociates the associated AWS Key Management Service (AWS KMS) customer master key (CMK) from the
        specified log group.

        After the AWS KMS CMK is disassociated from the log group, AWS CloudWatch Logs stops encrypting
        newly ingested data for the log group. All previously ingested data remains encrypted, and AWS
        CloudWatch Logs requires permissions for the CMK whenever the encrypted data is requested.
    ...
    def filter_log_events(
        ...
        By default, this operation returns as many log events as can fit in 1 MB (up to 10,000 log events),
        or all the events found within the time range that you specify. If the results include a token, then
        there are more log events available, and you can get additional results by specifying the token in a
        subsequent call.
        ...
        **Request Syntax**
        ...
        :param logStreamNamePrefix:
          ...
          ``logStreamNamePrefix`` does not match any log stream names specified in ``logStreamNames`` , the
        ...
        :param interleaved:
          ...
          If the value is true, the operation makes a best effort to provide responses that contain events
          from multiple log streams within the log group, interleaved in a single response. If the value is
           ...
           **IMPORTANT:** Starting on June 17, 2019, this parameter will be ignored and the value will be
           assumed to be true. The response from this operation will always interleave events from multiple
           log streams within a log group.
    ...
    def get_log_events(
        ...
        """
        Lists log events from the specified log stream. You can list all the log events or filter using a
        time range.

        By default, this operation returns as many log events as can fit in a response size of 1MB (up to
        10,000 log events). You can get additional log events by specifying one of the tokens in a
        subsequent call.
        ...
        **Request Syntax**
        ...
        :param startTime:
          ...
          The start of the time range, expressed as the number of milliseconds after Jan 1, 1970 00:00:00
          UTC. Events with a timestamp equal to this time or later than this time are included. Events with
        ...
        :param limit:
          ...
          The maximum number of log events returned. If you don't specify a value, the maximum is as many
    ...
    def get_log_group_fields(
        ...
        """
        Returns a list of the fields that are included in log events in the specified log group, along with
    ...
    def get_log_record(
        ...
        """
        Retrieves all the fields and values of a single log event. All fields are retrieved, even if the
        ...
        **Request Syntax**
        ...
        :param logRecordPointer: **[REQUIRED]**
          ...
          response of a ``GetQueryResults`` operation. In that response, the value of the ``@ptr`` field for
    ...
    def get_query_results(self, queryId: str) -> ClientGetQueryResultsResponseTypeDef:
        ...
        identifier for the log record. You can use the value of ``@ptr`` in a operation to get the full log
        record.
    ...
    def put_destination(
        ...
        A destination encapsulates a physical resource (such as an Amazon Kinesis stream) and enables you to
        ...
        user cannot call  PutSubscriptionFilter against this destination. To enable this, the destination
    ...
    def put_destination_policy(self, destinationName: str, accessPolicy: str) -> None:
        ...
        `IAM policy document <https://docs.aws.amazon.com/IAM/latest/UserGuide/policies_overview.html>`__
        that is used to authorize claims to register a subscription filter against a given destination.
    ...
    def put_log_events(
        ...
        You must include the sequence token obtained from the response of the previous call. An upload in a
        newly created log stream does not require a sequence token. You can also get the sequence token
        using  DescribeLogStreams . If you call ``PutLogEvents`` twice within a narrow time period using the
        ...
        * None of the log events in the batch can be older than 14 days or older than the retention period
        of the log group.

        * The log events in the batch must be in chronological ordered by their timestamp. The timestamp is
        the time the event occurred, expressed as the number of milliseconds after Jan 1, 1970 00:00:00 UTC.
        (In AWS Tools for PowerShell and the AWS SDK for .NET, the timestamp is specified in .NET format:
        ...
        **Request Syntax**
        ...
        :param sequenceToken:
          ...
          The sequence token obtained from the response of the previous ``PutLogEvents`` call. An upload in
          a newly created log stream does not require a sequence token. You can also get the sequence token
          using  DescribeLogStreams . If you call ``PutLogEvents`` twice within a narrow time period using
          the same value for ``sequenceToken`` , both calls may be successful, or one may be rejected.
    ...
    def put_metric_filter(
        ...
        allow you to configure rules to extract metric data from log events ingested through  PutLogEvents .
    ...
    def put_resource_policy(
        ...
        such as Amazon Route 53. An account can have up to 10 resource policies per region.
    ...
    def put_retention_policy(self, logGroupName: str, retentionInDays: int) -> None:
        """
        Sets the retention of the specified log group. A retention policy allows you to configure the number
    ...
    def put_subscription_filter(
        ...
        PutLogEvents and have them delivered to a specific destination. Currently, the supported
        destinations are:
        ...
        * An AWS Lambda function that belongs to the same account as the subscription filter, for
        ...
        There can only be one subscription filter associated with a log group. If you are updating an
        existing filter, you must specify the correct name in ``filterName`` . Otherwise, the call fails
        because you cannot associate a second filter with a log group.
        ...
        **Request Syntax**
        ...
        :param filterName: **[REQUIRED]**
          ...
          correct name in ``filterName`` . Otherwise, the call fails because you cannot associate a second
    ...
    def start_query(
        ...
        """
        Schedules a query of a log group using CloudWatch Logs Insights. You specify the log group and time
        ...
        Queries time out after 15 minutes of execution. If your queries are timing out, reduce the time
    ...
    def stop_query(self, queryId: str) -> ClientStopQueryResponseTypeDef:
        ...
        **Request Syntax**
        ...
        :param queryId: **[REQUIRED]**
          ...
          The ID number of the query to stop. If necessary, you can use ``DescribeQueries`` to find this ID
          number.
    ...
    def tag_log_group(self, logGroupName: str, tags: Dict[str, str]) -> None:
        ...
        <https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/log-group-tagging.html>`__ in the *Amazon
    ...
    def test_metric_filter(
        ...
        **Request Syntax**
        ...
        :param filterPattern: **[REQUIRED]**
          ...
          example, a log event may contain timestamps, IP addresses, strings, and so on. You use the filter
