...
_ClientCreateGroupResponseResourceQueryTypeDef = TypedDict(
    "_ClientCreateGroupResponseResourceQueryTypeDef", {"Type": str, "Query": str}, total=False
)
...
class ClientGetGroupQueryResponseGroupQueryResourceQueryTypeDef(
    ...
    The resource query which determines which AWS resources are members of the associated resource
    group.
...
class ClientListGroupResourcesResponseQueryErrorsTypeDef(
    ...
    operations on CloudFormation stack-based queries. The error occurs if the CloudFormation stack
    on which the query is based either does not exist, or has a status that renders the stack
    inactive. A ``QueryError`` occurrence does not necessarily mean that AWS Resource Groups could
    not complete the operation, but the resulting group might have no member resources.
    ...
    - **Message** *(string) --*
      ...
      ``CLOUDFORMATION_STACK_INACTIVE`` , the message typically states that the CloudFormation stack
...
class ClientSearchResourcesResourceQueryTypeDef(_ClientSearchResourcesResourceQueryTypeDef):
    """
...
class ClientUpdateGroupQueryResourceQueryTypeDef(_ClientUpdateGroupQueryResourceQueryTypeDef):
    """
...
class ListGroupResourcesPaginateFiltersTypeDef(_ListGroupResourcesPaginateFiltersTypeDef):
    """
...
class SearchResourcesPaginateResourceQueryTypeDef(_SearchResourcesPaginateResourceQueryTypeDef):
    """
