...
class Resource:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._schema_fields = list(self._schema().fields.keys())
    ...
    def __getattribute__(self, name):
        ...
        try:
            ...
            schema = super().__getattribute__("_schema")()
    ...
    def _session(self) -> requests.Session:
        ...
        self._request_session.mount('%s://%s' % (conn.scheme, conn.host), HTTPAdapter(max_retries=5))
        if conn.basic_auth:
    ...
    def from_dict(cls, input_dict: dict, *args, strict: bool = False) -> "Resource":
        ...
        try:
            resource = schema.load(input_dict, unknown=unknown_policy)
        except Exception as exc:
    ...
    def _find(cls, *args, connection: HostConnection = None, **kwargs) -> Optional["Resource"]:
        ...
        matching resource. If 0 are found or if more than 1 is found, an error
        will be raised or returned. If there is exactly 1 matching record, then
        it will be returned.
        ...
        resource._get()  # pylint: disable=protected-access
        return resource
    ...
    def _should_skip_field(
        ...
        warning = (
            "%s.%s is not a %sable field so it is not being sent." %
            (self.__class__.__name__, field, action)
        )
