...
class CleverConnector():
    ...
    def make_call(self, url):
        next = ""
        collected_objects = []
        users_call = bool(re.search('/(users|students|teachers|school_admins|district_admins)', url))
        if users_call:
        ...
        if users_call:
            extracted_objects = [self.process_user(u) for u in extracted_objects]
    ...
    def translate(self, group_filter, user_filter):
        ...
        if user_filter == 'admins':
            return [url.replace(user_filter, 'school_admins'),
                    url.replace(user_filter, 'district_admins')]

        elif user_filter == 'users':
            url_list = [url.replace(user_filter, 'students'),
                        url.replace(user_filter, 'teachers')]
            if not group_filter:
                url_list.extend([url.replace(user_filter, 'school_admins'),
                                 url.replace(user_filter, 'district_admins')])
            return url_list
        else:
            return [url]
