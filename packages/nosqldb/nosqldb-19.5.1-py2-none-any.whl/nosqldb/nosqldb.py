    ...
    def _get_store(host, port, store_config, proxy_config, should_start):
        ...
        if (transport is None):
            ...
            raise ConnectionException(
                ...
                ' and port: ' + port)
        if (proxy_config is not None):
            transport.setTimeout(proxy_config.get_request_timeout())
...
class Row(dict):
    ...
    def get_identity_value(self):
        """
        Returns the identity value generated for the row.
        """
        return self._identity_value

    def set_identity_value(self, identity_value):
        """
        Sets the identity value generated for the row.
        """
...
class Store:
    ...
    def put(self, table, row, write_opts=None):
        ...
        try:
            ...
            identity_generated_value = None
            if result.identityGeneratedValue:
                identity_generated_value =\
                    ((json.loads(result.identityGeneratedValue,
                                 parse_float=decimal.Decimal)).values())[0]
            row.set_identity_value(identity_generated_value)
            if (data is not None):
