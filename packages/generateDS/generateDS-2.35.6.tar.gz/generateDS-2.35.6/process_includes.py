...
def get_fixed_schema_nsmap(node, prefix='xs'):
    ...
    if None in nsmap:
        nsmap.pop(None)
    if 'xsd' in nsmap and prefix != 'xsd':
...
def make_names_unique(root,
    ...
    # Get all the top level xs:complexType definitions in this schema doc.
...
def fixup_refs(root, inserts, rename_data):
    ...
    for node in inserts:
        if node.tag == ct_tag:
            ...
            fixup_refs_simpletype(node, rename_data)
    ...
    if type_name:
        ...
        if prefix:
            ...
            if namespace:
                ...
                if unique_name and unique_name != type_name:
