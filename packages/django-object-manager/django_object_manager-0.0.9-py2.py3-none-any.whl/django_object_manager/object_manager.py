...
class ObjectManager:
    ...
    _data = {}
    _registered_models_by_name = {}
    _registered_models_by_model = {}
    ...
    def register(cls, model, data):
        ...
        cls._data[name] = data
        if model in cls._registered_models_by_model:
            raise ValueError('Model already registered')
        cls._registered_models_by_name[name] = model
        cls._registered_models_by_model[model] = model
    ...
    def _get_model(self, name):
        return self._registered_models_by_name[name]

    def _is_custom(self, _model, **kwargs):
        fields = {field.name: field for field in _model._meta.get_fields()}
        for name in kwargs:
