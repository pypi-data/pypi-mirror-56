    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param Ascending:
          ...
          ``true`` . To list jobs in reverse chronological order, enter ``false`` .
              ...
              - *(dict) --*
                ...
                - **Input** *(dict) --*
                  ...
                  - **InputCaptions** *(dict) --*
                    ...
                    * **Embedded:** Embedded captions are included in the same file as the audio and
                    ...
                    (EIA-608`` , first non-empty channel only), ``CEA-708 (EIA-708`` , first
                    ...
                    * **Sidecar:** Sidecar captions are kept in a separate metadata file from the
                    ...
                    element only), ``ebu-tt`` , ``scc`` , ``smpt`` , ``srt`` , ``ttml`` (first div
                    element only), and ``webvtt``   Valid outputs include: ``dfxp`` (first div
                    ...
                    - **MergePolicy** *(string) --*
                      ...
                      * **MergeRetain:** Elastic Transcoder transcodes both embedded and sidecar
                      ...
                      ``CaptionSources`` is empty, Elastic Transcoder omits all sidecar captions
                    ...
                    - **CaptionSources** *(list) --*
                      ...
                      - *(dict) --*
                        ...
                        - **TimeOffset** *(string) --*
                          ...
                          associated video file, the ``TimeOffset`` tells Elastic Transcoder how
                ...
                - **Inputs** *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    - **Interlaced** *(string) --*
                      ...
                      If you want to specify whether the input file is interlaced, enter one of the
                      following values:
                    ...
                    - **Container** *(string) --*
                      ...
                      If you want to specify the container type for the input file, enter one of the
                      following values:

                       ``3gp`` , ``aac`` , ``asf`` , ``avi`` , ``divx`` , ``flv`` , ``m4a`` ,
                       ``mkv`` , ``mov`` , ``mp3`` , ``mp4`` , ``mpeg`` , ``mpeg-ps`` , ``mpeg-ts``
                    ...
                    - **InputCaptions** *(dict) --*
                      ...
                      * **Embedded:** Embedded captions are included in the same file as the audio
                      ...
                      ``CEA-608 (EIA-608`` , first non-empty channel only), ``CEA-708 (EIA-708`` ,
                      first non-empty channel only), and ``mov-text``   Valid outputs include:
                      ``mov-text``   Elastic Transcoder supports a maximum of one embedded format
                      per output.
                      ...
                      * **Sidecar:** Sidecar captions are kept in a separate metadata file from the
                      ...
                      maximum of 20 sidecar captions per file. Valid input values include: ``dfxp``
                      (first div element only), ``ebu-tt`` , ``scc`` , ``smpt`` , ``srt`` , ``ttml``
                      (first div element only), and ``webvtt``   Valid outputs include: ``dfxp``
                      (first div element only), ``scc`` , ``srt`` , and ``webvtt`` .
                      ...
                      - **CaptionSources** *(list) --*
                        ...
                        - *(dict) --*
                          ...
                          - **TimeOffset** *(string) --*
                            ...
                            the associated video file, the ``TimeOffset`` tells Elastic Transcoder
                ...
                - **Output** *(dict) --*
                  ...
                  - **Key** *(string) --*
                    ...
                    the Amazon S3 bucket specified by the ``OutputBucket`` object in the pipeline
                  ...
                  - **ThumbnailPattern** *(string) --*
                    ...
                    * **``{count}`` (Required)** : If you want to create thumbnails, you must
                    include ``{count}`` in the ``ThumbnailPattern`` object. Wherever you specify
                    ``{count}`` , Elastic Transcoder adds a five-digit sequence number (beginning
                    with **00001** ) to thumbnail file names. The number indicates where a given
                    thumbnail appears in the sequence of thumbnails for a transcoded file.
                    ...
                    * **``{resolution}`` (Optional)** : If you want Elastic Transcoder to include
                    the resolution in the file name, include ``{resolution}`` in the
                  ...
                  - **SegmentDuration** *(string) --*
                    ...
                    each media segment is stored in a separate ``.ts`` file. For ``HLSv4`` ,
                    ``MPEG-DASH`` , and ``Smooth`` playlists, all media segments for an output are
                    ...
                    not evenly divisible by ``SegmentDuration`` , the duration of the last segment
                  ...
                  - **Status** *(string) --*
                    ...
                    ``Outputs:Status`` is always the same as ``Job:Status`` . If you specified more
                    than one output:
                    ...
                    * When all of the outputs reach a terminal status, ``Job:Status`` changes to
                    ...
                    ``Outputs:Status`` for one or more outputs is ``Error`` , the terminal status
                  ...
                  - **Watermarks** *(list) --*
                    ...
                    - *(dict) --*
                      ...
                      - **InputKey** *(string) --*
                        ...
                        Transcoder checks the pipeline specified by ``Pipeline`` ; the ``Input
                        Bucket`` object in that pipeline identifies the bucket.
                      ...
                      - **Encryption** *(dict) --*
                        ...
                        - **Mode** *(string) --*
                          ...
                          * **s3-aws-kms:** Amazon S3 calls the Amazon Key Management Service, which
                          creates and manages the keys that are used for encrypting your files. If
                          you specify ``s3-aws-kms`` and you don't want to use the default key, you
                        ...
                        - **Key** *(string) --*
                          ...
                          The data encryption key that you want Elastic Transcoder to use to encrypt
                          your output file, or that was used to encrypt your input file. The key
                        ...
                        - **KeyMd5** *(string) --*
                          ...
                          The MD5 digest of the key that you used to encrypt your input file, or
                          that you want Elastic Transcoder to use to encrypt your output file.
                          Elastic Transcoder uses the key digest as a checksum to make sure your key
                          was not corrupted in transit. The key MD5 must be base64-encoded, and it
                          must be exactly 16 bytes long before being base64-encoded.
                  ...
                  - **AlbumArt** *(dict) --*
                    ...
                    - **Artwork** *(list) --*
                      ...
                      with an audio file, to a maximum of 20. Valid formats are ``.jpg`` and
                      ``.png``
                      ...
                      - *(dict) --*
                        ...
                        - **SizingPolicy** *(string) --*
                          ...
                          * ``Fill:`` Elastic Transcoder scales the output art so it matches the
                          ...
                          matches or exceeds the other value. Elastic Transcoder centers the output
                          art and then crops it in the dimension (if any) that exceeds the maximum
                          value.
                          ...
                          * ``ShrinkToFit:`` Elastic Transcoder scales the output art down so that
                          ...
                          ``MaxWidth`` and ``MaxHeight`` without exceeding either value. If you
                          ...
                          * ``ShrinkToFill`` Elastic Transcoder scales the output art down so that
                          its dimensions match the values that you specified for at least one of
                          ``MaxWidth`` and ``MaxHeight`` without dropping below either value. If you
                          specify this option, Elastic Transcoder does not scale the art up.

                        - **PaddingPolicy** *(string) --*

                          When you set ``PaddingPolicy`` to ``Pad`` , Elastic Transcoder may add
                          white bars to the top and bottom and/or left and right sides of the output
                ...
                - **Outputs** *(list) --*
                  ...
                  into only one format. Do not use both the ``Outputs`` and ``Output`` syntaxes in
                  ...
                  - *(dict) --*
                    ...
                    - **ThumbnailPattern** *(string) --*
                      ...
                      * **``{resolution}`` (Optional)** : If you want Elastic Transcoder to include
                      ...
                      When creating thumbnails, Elastic Transcoder automatically saves the files in
                      the format (.jpg or .png) that appears in the preset that you specified in the
                      ``PresetID`` value of ``CreateJobOutput`` . Elastic Transcoder also appends
                    ...
                    - **SegmentDuration** *(string) --*
                      ...
                      ``HLSv4`` , ``MPEG-DASH`` , and ``Smooth`` playlists, all media segments for
                    ...
                    - **Status** *(string) --*
                      ...
                      job, ``Outputs:Status`` is always the same as ``Job:Status`` . If you
                      ...
                      * When all of the outputs reach a terminal status, ``Job:Status`` changes to
                      ...
                      If ``Outputs:Status`` for one or more outputs is ``Error`` , the terminal
                      status for ``Job:Status`` is also ``Error`` .
                    ...
                    - **Watermarks** *(list) --*
                      ...
                      watermarks in the same location, the second watermark that you add covers the
                      first one, the third one covers the second, and the fourth one covers the
                      third.
                      ...
                      - **Artwork** *(list) --*
                        ...
                        - *(dict) --*
                          ...
                          - **InputKey** *(string) --*
                            ...
                            pipeline specified by ``PipelineId`` ; the ``InputBucket`` object in
                          ...
                          - **SizingPolicy** *(string) --*
                            ...
                            * ``ShrinkToFill`` Elastic Transcoder scales the output art down so that
                            its dimensions match the values that you specified for at least one of
                            ``MaxWidth`` and ``MaxHeight`` without dropping below either value. If
                        ...
                        - *(dict) --*
                          ...
                          - **Pattern** *(string) --*
                            ...
                            * ``{language}`` is a literal value that Elastic Transcoder replaces
                            ...
                            For example, suppose you're transcoding into srt format. When you enter
                            "Sydney-{language}-sunrise", and the language of the captions is English
                            (en), the name of the first caption file is be
                ...
                - **OutputKeyPrefix** *(string) --*
                  ...
                  recommend that you add a / or some other delimiter to the end of the
                  ``OutputKeyPrefix`` .
                  ...
                  - *(dict) --*
                    ...
                    - **OutputKeys** *(list) --*
                      ...
                      * If your output is not ``HLS`` or does not have a segment duration set, the
                      name of the output file is a concatenation of ``OutputKeyPrefix`` and
                      ``Outputs:Key`` : OutputKeyPrefix``Outputs:Key``

                      * If your output is ``HLSv3`` and has a segment duration set, or is not
                      included in a playlist, Elastic Transcoder creates an output playlist file
                      with a file extension of ``.m3u8`` , and a series of ``.ts`` files that
                      include a five-digit sequential counter beginning with 00000:
                      OutputKeyPrefix``Outputs:Key`` .m3u8 OutputKeyPrefix``Outputs:Key`` 00000.ts
                      ...
                      * If your output is ``HLSv4`` , has a segment duration set, and is included in
                      ...
                      OutputKeyPrefix``Outputs:Key`` _v4.m3u8 OutputKeyPrefix``Outputs:Key``
                      _iframe.m3u8 OutputKeyPrefix``Outputs:Key`` .ts
                      ...
                      in the playlist. For ``Smooth`` playlists, the ``Audio:Profile`` ,
                      ``Video:Profile`` , and ``Video:FrameRate`` to ``Video:KeyframesMaxDist``
                    ...
                    - **HlsContentProtection** *(dict) --*
                      ...
                      - **KeyStoragePolicy** *(string) --*
                        ...
                        an Amazon S3 bucket. If you choose ``WithVariantPlaylists`` ,
                ...
                - **UserMetadata** *(dict) --*
                  ...
                  You specify metadata in ``key/value`` pairs, and you can add up to 10
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param Status: **[REQUIRED]**
          ...
          To get information about all of the jobs associated with the current AWS account that have
          a given status, specify the following status: ``Submitted`` , ``Progressing`` ,
          ``Complete`` , ``Canceled`` , or ``Error`` .
              ...
              - *(dict) --*
                ...
                - **ContentConfig** *(dict) --*
                  ...
                  * **Permissions** : A list of the users and/or predefined Amazon S3 groups you
                  want to have access to transcoded files and playlists, and the type of access that
                  ...
                  * **StorageClass** : The Amazon S3 storage class, Standard or ReducedRedundancy,
                  ...
                  - **Permissions** *(list) --*
                    ...
                    Amazon S3 groups you want to have access to transcoded files and playlists, and
                    the type of access you want them to have. You can grant permissions to a maximum
                ...
                - **ThumbnailConfig** *(dict) --*
                  ...
                  Information about the Amazon S3 bucket in which you want Elastic Transcoder to
                  save thumbnail files. Either you specify both ``ContentConfig`` and
                  ``ThumbnailConfig`` , or you specify ``OutputBucket`` .
                  ...
                  * ``Permissions`` : A list of the users and/or predefined Amazon S3 groups you
                  want to have access to thumbnail files, and the type of access that you want them
                  to have.
          ...
          - *(dict) --*
            ...
            - **Presets** *(list) --*
              ...
              - *(dict) --*
                ...
                from one format to another. Elastic Transcoder includes some default presets for
                common formats, for example, several iPod and iPhone versions. You can also create
                your own presets for formats that aren't included among the default presets. You
                specify which preset you want to use when you create a job.
                ...
                - **Container** *(string) --*
                  ...
                  ``fmp4`` , ``gif`` , ``mp3`` , ``mp4`` , ``mpg`` , ``mxf`` , ``oga`` , ``ogg`` ,
                  ``ts`` , and ``webm`` .
                ...
                - **Audio** *(dict) --*
                  ...
                  - **AudioPackingMode** *(string) --*
                    ...
                    * ``1, 2, or auto``  **channels with no audio input:** Audio omitted from the
                    output

                    * ``1``  **channel with any input with audio:** One track with one channel,
                    downmixed if necessary

                    * ``2``  **channels with one track with one channel:** One track with two
                    identical channels
                    ...
                    * ``auto``  **channels with one track with multiple channels:** One track with
                    ...
                    for every channel in your output. Your output can have up to eight
                    ...
                    * ``auto``  **channels with one track with multiple channels:** Up to eight
                    tracks with one channel each, plus MOS tracks until there are eight tracks in
                    all
                  ...
                  - **CodecOptions** *(dict) --*
                    ...
                    - **Profile** *(string) --*
                      ...
                      * ``auto`` : If you specify ``auto`` , Elastic Transcoder selects the profile
                ...
                - **Video** *(dict) --*
                  ...
                  - **Codec** *(string) --*
                    ...
                    ``mpeg2`` , ``vp8`` , and ``vp9`` . You can only specify ``vp8`` and ``vp9``
                  ...
                  - **CodecOptions** *(dict) --*
                    ...
                    * 4.1 - 32768
                    ...
                    render color. ``Bt.601`` is the standard for standard definition video, while
                    ``Bt.709`` is the standard for high definition video.
                    ...
                    the resolution changes from the list below, Elastic Transcoder applies the
                    ...
                    * *HD to Standard, 1920x1080 to 720x576* - Elastic Transcoder applies
                    ...
                    ``AppliedColorSpaceConversion`` parameter included in your job response. If your
                    job does not have an ``AppliedColorSpaceConversion`` in its response, no
                  ...
                  - **KeyframesMaxDist** *(string) --*
                    ...
                    ``KeyframesMaxDist`` of 90. The output file then needs to have a ratio of 1:3.
                    Valid outputs would have ``FrameRate`` of 30, 25, and 10, and
                  ...
                  - **BitRate** *(string) --*
                    ...
                    values depend on the values of ``Level`` and ``Profile`` . If you specify
                    ...
                    less than or equal to the maximum H.264-compliant value listed for your level
                    and profile:
                  ...
                  - **MaxFrameRate** *(string) --*
                    ...
                    maximum frame rate that you want Elastic Transcoder to use when the frame rate
                    of the input video is greater than the desired maximum frame rate of the output
                    video. Valid values include: ``10`` , ``15`` , ``23.97`` , ``24`` , ``25`` ,
                    ``29.97`` , ``30`` , ``60`` .
                  ...
                  - **Resolution** *(string) --*
                    ...
                    .. warning::
                      ...
                      that you use the values ``MaxWidth`` , ``MaxHeight`` , ``SizingPolicy`` ,
                  ...
                  - **SizingPolicy** *(string) --*
                    ...
                    * ``Stretch`` : Elastic Transcoder stretches the output video to match the
                    ...
                    proportions of the input video and the output video are different, the output
                  ...
                  - **PaddingPolicy** *(string) --*
                    ...
                    bars to the top and bottom and/or left and right sides of the output video to
                    make the total size of the output video match the values that you specified for
                  ...
                  - **Watermarks** *(list) --*
                    ...
                    - *(dict) --*
                      ...
                      Settings for the size, location, and opacity of graphics that you want Elastic
                      Transcoder to overlay over videos that are transcoded using this preset. You
                      can specify settings for up to four watermarks. Watermarks appear in the
                      specified size and location, and with the specified opacity for the duration
                      ...
                      - **MaxWidth** *(string) --*
                        ...
                        * integer percentage (%): The range of valid values is 0 to 100. Use the
                        ...
                        calculation. If you specify the value in pixels, it must be less than or
                      ...
                      - **SizingPolicy** *(string) --*
                        ...
                        * **Stretch** : Elastic Transcoder stretches the watermark to match the
                        values that you specified for ``MaxWidth`` and ``MaxHeight`` . If the
                        relative proportions of the watermark and the values of ``MaxWidth`` and
                        ...
                        * **ShrinkToFit** : Elastic Transcoder scales the watermark down so that its
                        ...
                        ``MaxWidth`` and ``MaxHeight`` without exceeding either value. If you
                        specify this option, Elastic Transcoder does not scale the watermark up.
                      ...
                      - **HorizontalAlign** *(string) --*
                        ...
                        * **Right** : The right edge of the watermark is aligned with the right
                        border of the video.
                      ...
                      - **HorizontalOffset** *(string) --*
                        ...
                        * integer percentage (%): The range of valid values is 0 to 100.
                        ...
                        the left border of the output video.
                         ...
                         ``Left`` or ``Right`` . If you specify an offset that causes the watermark
                         to extend beyond the left or right border and Elastic Transcoder has not
                         added black bars, the watermark is cropped. If Elastic Transcoder has added
                         black bars, the watermark extends into the black bars. If the watermark
                      ...
                      - **Opacity** *(string) --*
                        ...
                        specified a value of 0 for ``Opacity`` . The .jpg file format doesn't
                      ...
                      - **Target** *(string) --*
                        ...
                        * **Content** : ``HorizontalOffset`` and ``VerticalOffset`` values are
                        ...
                        Elastic Transcoder, if any. In addition, ``MaxWidth`` and ``MaxHeight`` , if
                ...
                - **Thumbnails** *(dict) --*
                  ...
                  - **Resolution** *(string) --*
                    ...
                    .. warning::
                      ...
                      you use the values ``MaxWidth`` , ``MaxHeight`` , ``SizingPolicy`` , and
                      ``PaddingPolicy`` instead of ``Resolution`` and ``AspectRatio`` . The two
                  ...
                  - **SizingPolicy** *(string) --*
                    ...
                    * ``Fit`` : Elastic Transcoder scales thumbnails so they match the value that
                    you specified in thumbnail MaxWidth or MaxHeight settings without exceeding the
                    other value.

                    * ``Fill`` : Elastic Transcoder scales thumbnails so they match the value that
                    you specified in thumbnail ``MaxWidth`` or ``MaxHeight`` settings and matches or
                    ...
                    * ``Stretch`` : Elastic Transcoder stretches thumbnails to match the values that
                    you specified for thumbnail ``MaxWidth`` and ``MaxHeight`` settings. If the
                    relative proportions of the input video and thumbnails are different, the
                    thumbnails will be distorted.
                    ...
                    * ``Keep`` : Elastic Transcoder does not scale thumbnails. If either dimension
                    ...
                    ``MaxWidth`` and ``MaxHeight`` settings, Elastic Transcoder crops the
                    thumbnails.
                  ...
                  - **PaddingPolicy** *(string) --*
                    ...
                    When you set ``PaddingPolicy`` to ``Pad`` , Elastic Transcoder may add black
                    bars to the top and bottom and/or left and right sides of thumbnails to make the
