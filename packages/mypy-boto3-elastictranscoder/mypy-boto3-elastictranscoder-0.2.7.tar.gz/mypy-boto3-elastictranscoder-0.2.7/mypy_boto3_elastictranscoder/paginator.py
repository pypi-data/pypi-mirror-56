            ...
            - **Jobs** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Input** *(dict) --*
                  ...
                  A section of the request or response body that provides information about the file that is
                  being transcoded.

                  - **Key** *(string) --*

                    The name of the file to transcode. Elsewhere in the body of the JSON block is the the ID
                    of the pipeline to use for processing the job. The ``InputBucket`` object in that
                    pipeline tells Elastic Transcoder which Amazon S3 bucket to get the file from.

                    If the file name includes a prefix, such as ``cooking/lasagna.mpg`` , include the prefix
                    in the key. If the file isn't in the specified bucket, Elastic Transcoder returns an
                    error.

                  - **FrameRate** *(string) --*

                    The frame rate of the input file. If you want Elastic Transcoder to automatically detect
                    the frame rate of the input file, specify ``auto`` . If you want to specify the frame
                    rate for the input file, enter one of the following values:
                  ...
                  - **AspectRatio** *(string) --*
                    ...
                    detect the aspect ratio of the input file, specify ``auto`` . If you want to specify the
                  ...
                  - **Encryption** *(dict) --*
                    ...
                    - **Mode** *(string) --*
                      ...
                      * **s3-aws-kms:** Amazon S3 calls the Amazon Key Management Service, which creates and
                      manages the keys that are used for encrypting your files. If you specify
                      ``s3-aws-kms`` and you don't want to use the default key, you must add the AWS-KMS key
                      that you want to use to your pipeline.
                    ...
                    - **Key** *(string) --*
                      ...
                      file, or that was used to encrypt your input file. The key must be base64-encoded and
                      it must be one of the following bit lengths before being base64-encoded:
                    ...
                    - **KeyMd5** *(string) --*
                      ...
                      digest as a checksum to make sure your key was not corrupted in transit. The key MD5
                    ...
                    - **InitializationVector** *(string) --*
                      ...
                      Elastic Transcoder to use to encrypt your output files. The initialization vector must
                  ...
                  - **TimeSpan** *(dict) --*
                    ...
                    - **StartTime** *(string) --*
                      ...
                      sssss.SSS (maximum value: 86399.999). If you don't specify a value, Elastic Transcoder
                      starts at the beginning of the input file.

                    - **Duration** *(string) --*

                      The duration of the clip. The format can be either HH:mm:ss.SSS (maximum value:
                      23:59:59.999; SSS is thousandths of a second) or sssss.SSS (maximum value: 86399.999).
                      If you don't specify a value, Elastic Transcoder creates an output file from StartTime
                  ...
                  - **InputCaptions** *(dict) --*
                    ...
                    * **Sidecar:** Sidecar captions are kept in a separate metadata file from the audio and
                    ...
                    Elastic Transcoder does not support OCR (Optical Character Recognition), does not accept
                    pictures as a valid input for captions, and is not available for audio-only transcoding.
                    Elastic Transcoder does not preserve text formatting (for example, italics) during the
                    transcoding process.
                    ...
                    - **MergePolicy** *(string) --*
                      ...
                      * **MergeOverride:** Elastic Transcoder transcodes both embedded and sidecar captions
                      ...
                      appear in a sidecar file, Elastic Transcoder uses the sidecar captions and ignores the
                  ...
                  - *(dict) --*
                    ...
                    - **InputCaptions** *(dict) --*
                      ...
                      * **Sidecar:** Sidecar captions are kept in a separate metadata file from the audio
                      and video data. Sidecar captions require a player that is capable of understanding the
                      relationship between the video file and the sidecar file. Elastic Transcoder supports
                      only one sidecar caption per language, to a maximum of 20 sidecar captions per file.
                      Valid input values include: ``dfxp`` (first div element only), ``ebu-tt`` , ``scc`` ,
                      ``smpt`` , ``srt`` , ``ttml`` (first div element only), and ``webvtt``   Valid outputs
                      ...
                      - **MergePolicy** *(string) --*
                        ...
                        * **MergeOverride:** Elastic Transcoder transcodes both embedded and sidecar
                        captions into outputs. If captions for a language are embedded in the input file and
                        also appear in a sidecar file, Elastic Transcoder uses the sidecar captions and
                ...
                - **Output** *(dict) --*
                  ...
                  - **ThumbnailPattern** *(string) --*
                    ...
                    Whether you want Elastic Transcoder to create thumbnails for your videos and, if so, how
                    you want Elastic Transcoder to name the files.

                    If you don't want Elastic Transcoder to create thumbnails, specify "".

                    If you do want Elastic Transcoder to create thumbnails, specify the information that you
                    want to include in the file name for each thumbnail. You can specify the following
                    ...
                    * **``{resolution}`` (Optional)** : If you want Elastic Transcoder to include the
                    ...
                    (.jpg or .png) that appears in the preset that you specified in the ``PresetID`` value
                    of ``CreateJobOutput`` . Elastic Transcoder also appends the applicable file name
                    extension.
                  ...
                  - **SegmentDuration** *(string) --*
                    ...
                    of each segment in seconds. For ``HLSv3`` format playlists, each media segment is stored
                    in a separate ``.ts`` file. For ``HLSv4`` , ``MPEG-DASH`` , and ``Smooth`` playlists,
                    all media segments for an output are stored in a single file. Each segment is
                    approximately the length of the ``SegmentDuration`` , though individual segments might
                    be shorter or longer.

                    The range of valid values is 1 to 60 seconds. If the duration of the video is not evenly
                    divisible by ``SegmentDuration`` , the duration of the last segment is the remainder of
                  ...
                  - **Status** *(string) --*
                    ...
                    * When Elastic Transcoder starts to process the first output, ``Outputs:Status`` for
                    that output and ``Job:Status`` both change to Progressing. For each output, the value of
                  ...
                  - **Watermarks** *(list) --*
                    ...
                    preset cause Elastic Transcoder to place all watermarks in the same location, the second
                    watermark that you add covers the first one, the third one covers the second, and the
                    ...
                    - *(dict) --*
                      ...
                      - **PresetWatermarkId** *(string) --*
                        ...
                        the current output. In that preset, the value of Watermarks Id tells Elastic
                  ...
                  - **Composition** *(list) --*
                    ...
                    - *(dict) --*
                      ...
                      Settings for one clip in a composition. All jobs in a playlist must have the same clip
                      settings.
                      ...
                      - *(dict) --*
                        ...
                        - **Pattern** *(string) --*
                          ...
                          * ``{language}`` is a literal value that Elastic Transcoder replaces with the two-
                          ...
                          "Sydney-{language}-sunrise", and the language of the captions is English (en), the
                  ...
                  - **AppliedColorSpaceConversion** *(string) --*
                    ...
                    If Elastic Transcoder used a preset with a ``ColorSpaceConversionMode`` to transcode the
                    output file, the ``AppliedColorSpaceConversion`` parameter shows the conversion used. If
                    no ``ColorSpaceConversionMode`` was defined in the preset, this parameter is not be
                      ...
                      - **Artwork** *(list) --*
                        ...
                        - *(dict) --*
                          ...
                          - **SizingPolicy** *(string) --*
                            ...
                            * ``Fit:`` Elastic Transcoder scales the output art so it matches the value that
                            you specified in either ``MaxWidth`` or ``MaxHeight`` without exceeding the
                            other value.
                          ...
                          - **PaddingPolicy** *(string) --*
                            ...
                            to make the total size of the output art match the values that you specified for
                            ``MaxWidth`` and ``MaxHeight`` .
                ...
                - **Playlists** *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    recommend that you create only one master playlist per output format. The maximum number
                    of master playlists in a job is 30.

                    - **Name** *(string) --*

                      The name that you want Elastic Transcoder to assign to the master playlist, for
                      example, nyc-vacation.m3u8. If the name includes a ``/`` character, the section of the
                      name before the last ``/`` must be identical for all ``Name`` objects. If you create
                    ...
                    - **OutputKeys** *(list) --*
                      ...
                      * If your output is ``HLSv3`` and has a segment duration set, or is not included in a
                      ...
                      ``.m3u8`` , and a series of ``.ts`` files that include a five-digit sequential counter
                      beginning with 00000: OutputKeyPrefix``Outputs:Key`` .m3u8
                      ...
                      * If your output is ``HLSv4`` , has a segment duration set, and is included in an
                      ...
                      extension of ``_v4.m3u8`` . If the output is video, Elastic Transcoder also creates an
                      output file with an extension of ``_iframe.m3u8`` : OutputKeyPrefix``Outputs:Key``
                      ...
                      If you include more than one output in a playlist, any segment duration settings, clip
                      settings, or caption settings must be the same for all outputs in the playlist. For
                      ``Smooth`` playlists, the ``Audio:Profile`` , ``Video:Profile`` , and
                    ...
                    - **HlsContentProtection** *(dict) --*
                      ...
                      - **InitializationVector** *(string) --*
                        ...
                        The series of random bits created by a random bit generator, unique for every
                        encryption operation, that you want Elastic Transcoder to use to encrypt your output
                        files. The initialization vector must be base64-encoded, and it must be exactly 16
                    ...
                    - **PlayReadyDrm** *(dict) --*
                      ...
                      - **InitializationVector** *(string) --*
                        ...
                        The series of random bits created by a random bit generator, unique for every
                        encryption operation, that you want Elastic Transcoder to use to encrypt your files.
                        The initialization vector must be base64-encoded, and it must be exactly 8 bytes
                        long before being base64-encoded. If no initialization vector is provided, Elastic
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param Status: **[REQUIRED]**
          ...
          status, specify the following status: ``Submitted`` , ``Progressing`` , ``Complete`` ,
          ``Canceled`` , or ``Error`` .
            ...
            - **Pipelines** *(list) --*
              ...
              - *(dict) --*
                ...
                - **AwsKmsKeyArn** *(string) --*
                  ...
                  provide a key with your job because a default key, known as an AWS-KMS key, is created for
                  you automatically. You need to provide an AWS-KMS key only if you want to use a
                ...
                - **Notifications** *(dict) --*
                  ...
                  * **Error** (optional): The Amazon SNS topic that you want to notify when Elastic
                  ...
                  - **Error** *(string) --*
                    ...
                    The Amazon SNS topic that you want to notify when Elastic Transcoder encounters an error
                    condition.
                ...
                - **ContentConfig** *(dict) --*
                  ...
                  * **StorageClass** : The Amazon S3 storage class, Standard or ReducedRedundancy, that you
                  ...
                  - **Permissions** *(list) --*
                    ...
                    groups you want to have access to transcoded files and playlists, and the type of access
                    you want them to have. You can grant permissions to a maximum of 30 users and/or
                    ...
                    If you omit ``Permissions`` , Elastic Transcoder grants full control over the transcoded
                    files and playlists to the owner of the role specified by ``Role`` , and grants no other
            ...
            - **Presets** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Container** *(string) --*
                  ...
                  , ``gif`` , ``mp3`` , ``mp4`` , ``mpg`` , ``mxf`` , ``oga`` , ``ogg`` , ``ts`` , and
                  ``webm`` .
                ...
                - **Audio** *(dict) --*
                  ...
                  - **CodecOptions** *(dict) --*
                    ...
                    - **Signed** *(string) --*
                      ...
                      You can only choose whether an audio sample is signed when you specify ``pcm`` for the
                      value of Audio:Codec.
                ...
                - **Video** *(dict) --*
                  ...
                  - **CodecOptions** *(dict) --*
                    ...
                    * 4.1 - 32768
                    ...
                    your ``SegmentDuration`` is null, and you are using one of the resolution changes from
                    the list below, Elastic Transcoder applies the following color space conversions:
                  ...
                  - **KeyframesMaxDist** *(string) --*
                    ...
                    frames. The value is an integer formatted as a string; valid values are between 1 (every
                    frame is a key frame) and 100000, inclusive. A higher value results in higher
                  ...
                  - **Resolution** *(string) --*
                    ...
                    * We recommend that you specify a resolution for which the product of width and height
                    is less than or equal to the applicable value in the following list (*List - Max width x
                    height value* ):
                  ...
                  - **AspectRatio** *(string) --*
                    ...
                    input file, Elastic Transcoder adds pillarboxing (black bars on the sides) or
                  ...
                  - **SizingPolicy** *(string) --*
                    ...
                    * ``Fill`` : Elastic Transcoder scales the output video so it matches the value that you
                    specified in either ``MaxWidth`` or ``MaxHeight`` and matches or exceeds the other
                    ...
                    * ``Keep`` : Elastic Transcoder does not scale the output video. If either dimension of
                    the input video exceeds the values that you specified for ``MaxWidth`` and ``MaxHeight``
                  ...
                  - **PaddingPolicy** *(string) --*
                    ...
                    top and bottom and/or left and right sides of the output video to make the total size of
                    the output video match the values that you specified for ``MaxWidth`` and ``MaxHeight``
                    .

                  - **Watermarks** *(list) --*

                    Settings for the size, location, and opacity of graphics that you want Elastic
                    Transcoder to overlay over videos that are transcoded using this preset. You can specify
                    settings for up to four watermarks. Watermarks appear in the specified size and
                    ...
                    - *(dict) --*
                      ...
                      - **MaxWidth** *(string) --*
                        ...
                        * integer percentage (%): The range of valid values is 0 to 100. Use the value of
                        ...
                        that are added by Elastic Transcoder, if any, in the calculation. If you specify the
                        value in pixels, it must be less than or equal to the value of ``MaxWidth`` .
                      ...
                      - **SizingPolicy** *(string) --*
                        ...
                        * **Fit** : Elastic Transcoder scales the watermark so it matches the value that you
                        specified in either ``MaxWidth`` or ``MaxHeight`` without exceeding the other value.
                ...
                - **Thumbnails** *(dict) --*
                  ...
                  - **Resolution** *(string) --*
                    ...
                    .. warning::
                      ...
                      To better control resolution and aspect ratio of thumbnails, we recommend that you use
                      the values ``MaxWidth`` , ``MaxHeight`` , ``SizingPolicy`` , and ``PaddingPolicy``
                  ...
                  - **SizingPolicy** *(string) --*
                    ...
                    * ``Fill`` : Elastic Transcoder scales thumbnails so they match the value that you
                    ...
                    other value. Elastic Transcoder centers the image in thumbnails and then crops in the
                    dimension (if any) that exceeds the maximum value.
                    ...
                    * ``ShrinkToFill`` : Elastic Transcoder scales thumbnails down so that their dimensions
                    ...
                    without dropping below either value. If you specify this option, Elastic Transcoder does
                    not scale thumbnails up.

                  - **PaddingPolicy** *(string) --*

                    When you set ``PaddingPolicy`` to ``Pad`` , Elastic Transcoder may add black bars to the
                    top and bottom and/or left and right sides of thumbnails to make the total size of the
                    thumbnails match the values that you specified for thumbnail ``MaxWidth`` and
                    ``MaxHeight`` settings.
