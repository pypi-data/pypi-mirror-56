...
class WireMode(IntEnum):
    """
    The wire mode describes the way, the leds are organized on the board.
    Two ways of wiring are supported. *LTR* and *ZIGZAG*. The WireMode class
    is an Enum. Whenever you have to specifiy a mode, you simply write
    ``WireMode.LTR`` or ``WireMode.ZIGZAG``

    **WireMde.LTR  (Left-To-Right)**

    In the mode WireMode.LTR, all rows are going from left to right. The cable
    for the data line (blue) goes all the way back from the end of one row to
    ...
    **WireMode.ZIGZAG**
    ...
    In the mode WireMode.ZIGZAG all even rows go from left to right where as the
    odd rows go from right to left. Organizing leds in this way saves a lot of
    cable for the data line (blue).
...
class Display:  # pylint: disable=too-many-public-methods
    """Creates a new instance of a led display. The Display class manages a
    virtual representation of the color state of the LEDs on the physical panel.
    The class offers many methods to set and change colors. The physical LEDs
    are updated via the :meth:`~ledwall.components.Display.update` method.
    Because there are different ways to connect the arduino (or ESP) to your
    computer, the transmission of the data is managed by an instance of a
    :class:`~ledwall.components.Sender`. This library offers several
    Implementations (:class:`~ledwall.components.SerialSender`,
    :class:`~ledwall.components.MqttSender`, :class:`~ledwall.components.UDPSender`).

    The methods to manipulate the color state of the pixels pay respect to the
    wiring mode you used. There are two ways to layout of LEDs on the board
    specified by the WireMode parameter in the constructor.
    ...
    .. code-block:: python
        ...
        s = SerialSender()              # Creating a serial sender with the
    ...
    :param mode:    The mode that the LEDs are organized. Left-to-Right or
                    Zig-Zag. Defaults to WireMode.LTR.
    :type mode: WireMode

    :param panel_id: An unique id for this panel. Currently not used.
    :type panel_id: str

    :param async_mode:  If True, the update() method of the provided
                        sender will be called asynchronously.
    :type async_mode: boolean
    ...
    def get_colors(self) -> List[TColor]:
        """
        Returns an array of all colors in natural
        order. The wiring mode is ignored.
        """
        return self[:]
    ...
    def set_pixel(self, x: int, y: int, color: TColor, update: Optional[bool] = False):
        ...
        Returns True, if setting the pixel was successful, Fales else. The
        color is specified by a color instance. The method also accepts
        tuples or arrays as colors. The length has to be at least 3 and must
        contain the value for red, green, and blue.
        ...
        :param x:
            The x position of the pixel. Must be a value with the
            following constraint: ``0 <= x < width``
        ...
        :param color:
            The color for the pixel. If you want to deactivate or
            clear a pixel, just use black (0,0,0) as color value.
    ...
    def get_pixel(self, x, y):
        ...
        Returns the color for the pixel located at (x,y). The x value must
        be within the the range: ``0 <= x < width``. The y value must be
        with the range: ``0 <= y < height``

        :param x:
            The x position of the pixel. Must be a value with the following
            constraint: ``0 <= x < width``
        :type x: int
    ...
    def write_bitmask(self, row, value, color1=(266, 165.0), color0=(0, 0, 0)):
        """
        Sets the pixel color regarding the bitmask of `value`. `color1` is used
        for 1 and `color0` for 0 in the bitmask.
        """
    ...
    def horizontal_line(self, row, color, update=False):
        ...
        :param color:
            The color for the row. If you want to deactivate
            or clear a row, just use black (0,0,0)
            as color value.
        :type color: Color, tuple, list
    ...
    def vertical_line(self, column, color, update=False):
        """
        Drawing a vertical line in the specified color. See also
        :meth:`~ledwall.components.Display.horizontal_line`
        ...
        :param color:
            The color for the column. If you want to deactivate
            or clear a row, just use black (0,0,0)
            as color value.
    ...
    def shift_row_left(self, row, update=False):
        """
        Shifts row one pixel to the left. The outermost left pixel will
        be placed to the right. So actually it rotates the pixels in the row.
        The method pays respect to the :attr:`~ledwall.components.Display.mode`
        property.
    ...
    def shift_left(self, update=False):
        ...
        The method shift pixels to the left visually, because the method
        takes the mode into account. This means, if the mode is
        :attr:`~ledwall.components.Display.MODE_ZIGZAG` and the row number
        is odd, then the pixels are shifted physically to the right. But
        because this row reads left to right, the pixels are shifted
        visually to the left.

        The pixel to the outermost left will be placed at the last column.
        So this method implements a rotation of the pixels.
    ...
    def move(self, count=1):
        ...
        .. warning::
            This method does not take the ``mode`` into account. So the
            visual effect differs depending on the mode.
    ...
    def shift_row_right(self, row, update=False):
        ...
        The method shift pixels to the right visually, because the method
        takes the mode into account. This means, if the mode is
        :attr:`~ledwall.components.Display.MODE_ZIGZAG` and the row number
        is odd, then the pixels are shifted physically to the left. But
        because this row reads left to right, the pixels are shifted visually
        to the right.

        The pixel to the outermost right will be placed at the first column.
        So this method implements a rotation of the pixels.

        :param row:
            The number of the row to be shifted. The row value must be with
            the range: ``0 <= row < height``
        :type row: int
    ...
    def shift_right(self, update=False):
        ...
        takes the mode into account. This means, if the mode is
        :attr:`~ledwall.components.Display.MODE_ZIGZAG` and the row number
        is odd, then the pixela are shifted physically to the left. But
        because this row reads left to right, the pixels are shifted visually to
        the right.
    ...
    def as_rectangle(self):
        """
        Returns a rectangle, with the dimensions
        of the dispplay.
        """
    ...
    def fill_rect(self, x, y, w, h, color, update=False):
        ...
        :param color:
            The color for the rectangle. If you want to deactivate
            or clear a rectangle, just use black (0,0,0) as color value.
        :type color: Color, tuple, list
        :param update: If True, the display will be updated.
        :type update: boolean
        :rtype: None
    ...
    def fill(self, color, update=False):
        ...
        :param color:
            The color for the display. If you want to deactivate or
            clear the panel, just use black (0,0,0) as color value.
        :type color: Color, tuple, list
    ...
    def update(self, update=True):
        ...
        The internal frame buffer is send to the arduino board and the
        LED stripe gets updated.

        The method takes framerate into account. If the time between
        this update and the last update is less then the milliseconds
        per frame, the method will sleep 'til the end of the frame and
        update the data at the end of the frame.

        Every time the LED display is updated, the frame number will
        be increased by one.

        .. note::
           If you are interested to see the time consumed by transferring
           the date to the arduino, you can read the transmissionInfo
           property after updating. The following code snippet shows
           the basic usage:
    ...
    def load_image(self, path, update=False, transparent_color=None):
        ...
        :param path:
            The relative or absolute path to the image. The method uses the PIL library
