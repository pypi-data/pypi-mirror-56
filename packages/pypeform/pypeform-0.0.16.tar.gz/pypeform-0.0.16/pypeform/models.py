...
class Category(object):
    ...
    def __init__(self):
        ...
        self.set_categories_for_fields()

    def set_categories_for_fields(self) -> None:
        """
...
class ActionGraph(object):
    ...
    def add_by_ref(self, source_ref, target_ref, condition):
        self._map[source_ref].append(Action(source_ref, target_ref, condition))
    ...
    def _get_all_children(self, field, limit, with_children):
        ...
        while stack and (len(stack) < limit or limit == -1):
            ...
            for action in filter(lambda x: x.relevant, self._map[popped_field.ref]):
            ...
            for field in popped_field.children:
                stack.append(field)
                for action in filter(lambda x: x.relevant, self._map[field.ref]):
