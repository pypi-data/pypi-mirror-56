...
class Client(BaseClient):
    ...
    def associate_entity_to_thing(
        ...
        A thing can be associated with only one device at a time. If you associate a thing with a new device
        id, its previous association will be removed.
    ...
    def create_system_instance(
        ...
        This action validates the system instance, prepares the deployment-related resources. For Greengrass
        deployments, it updates the Greengrass group that is specified by the ``greengrassGroupName``
        parameter. It also adds a file to the S3 bucket specified by the ``s3BucketName`` parameter. You
        ...
        For cloud deployments, this action requires a ``flowActionsRoleArn`` value. This is an IAM role that
        has permissions to access AWS services, such as AWS Lambda and AWS IoT, that the flow uses when it
        executes.
    ...
    def deploy_system_instance(
        ...
        If the system or any workflows and entities have been updated before this action is called, then the
        deployment will create a new Amazon Simple Storage Service resource file and then deploy it.
          ...
          - *(dict) --*
            ...
            - **description** *(dict) --*
              ...
              - **validatedNamespaceVersion** *(integer) --*
                ...
                The version of the user's namespace against which the workflow was validated. Use this value
            ...
            - **description** *(dict) --*
              ...
              - **validatedDependencyRevisions** *(list) --*
                ...
                - *(dict) --*
                  ...
                  An object that contains the ID and revision number of a workflow or system that is part of
                  a deployment.
              ...
              - **flowActionsRoleArn** *(string) --*
                ...
                flow execution in a cloud deployment. This role must have read and write permissionss to AWS
                Lambda and AWS IoT and to any other AWS services that the flow uses.
    ...
    def search_entities(
        ...
        **Request Syntax**
        ...
        :param filters:
          ...
          ``SEMANTIC_TYPE_PATH`` and ``REFERENCED_ENTITY_ID`` . ``REFERENCED_ENTITY_ID`` filters on entities
          that are used by the entity in the result set. For example, you can filter on the ID of a property
          ...
          - *(dict) --*
            ...
            An object that filters an entity search. Multiple filters function as OR criteria in the search.
            For example a search that includes a ``NAMESPACE`` and a ``REFERENCED_ENTITY_ID`` filter
        ...
        **Request Syntax**
        ...
        :param filters:
          ...
          - *(dict) --*
            ...
            search. For example a search that includes a GREENGRASS_GROUP_NAME and a STATUS filter searches
            for system instances in the specified Greengrass group that have the specified status.
    ...
    def upload_entity_definitions(
        ...
        ``syncWithPublicNamespace`` parameter is set to ``true`` , the user's namespace will synchronize
