...
class BertServer():
    ...
    def modify_conf(self, gpu_index=0):
        ...
        if not self.with_gpu_flag:
            ...
            conf_str = re.sub('model_data_path.*"', self.model_path_str, conf_str)
            conf_str = re.sub('enable_memory_optimization: 0', 'enable_memory_optimization: 1', conf_str)
    ...
    def run(self, gpu_index=0, port=8866):
        ...
        if self.with_gpu_flag == True:
            ...
            run_cmd += '--port=' + str(port) + ' ' + '--resource_file=resource.prototxt.' + str(gpu_index) + ' '
