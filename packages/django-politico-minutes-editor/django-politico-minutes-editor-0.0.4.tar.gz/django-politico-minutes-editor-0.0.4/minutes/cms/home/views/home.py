...
class HomeView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        if role == "ADV":
            url = reverse_lazy("cms:interstitial-list")
            return url
        elif role == "ADM":
            url = reverse_lazy("cms:home-admin")
            return url
        else:
            # Uncomment and replace when there are more verticals...
            # url = reverse_lazy("cms:vertical-list")
            # return url

            vertical_id = Vertical.objects.get(slug="2020").id
            url = reverse_lazy(
                "cms:vertical", kwargs={"vertical": vertical_id}
            )
            return url
...
class HomeAdminView(CMSBaseView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["breadcrumbs"] = {
            "home": {"name": "Minutes", "url": reverse("cms:home")}
        }
