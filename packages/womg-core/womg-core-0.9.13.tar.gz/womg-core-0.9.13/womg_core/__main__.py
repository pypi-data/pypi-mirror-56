...
if not str(pathlib.Path.cwd()).endswith('src'):
  src_path = pathlib.Path.cwd() / "src" / "womg_core"
  os.sys.path.insert(0, str(src_path))
if str(pathlib.Path.cwd()).endswith('examples'):
  src_path = pathlib.Path.cwd().parent / "src" / "womg_core"
  os.sys.path.insert(0, str(src_path))
#print(pathlib.Path.cwd())
'''
##################################################
...
def womg_main(numb_topics=15,
    ...
    try:
        set_seed(seed)
        print('starting network model', flush=True)
        ...
        print('network model finished', flush=True)

        topic_model = LDA(numb_topics=numb_topics,
                          numb_docs=numb_docs,
                          docs_path=docs_path,
                          items_descr_path=items_descr_path)
        print(numb_docs, docs_path, items_descr_path)
        topic_model.fit()
...
@click.option('--docs', metavar='D', default=None,
                    help='Number of docs to be generated',
                    type=int)
