...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_19_correct_schema(self):
    ...
    def test_20_insert_business_payload(self):
    ...
    def test_21_stats_on_business_payload_data(self):
    ...
    # def test_19b_get_business_payload_data(self):
    #     business_records_no = wait_for_records_in_data_set(
    #         url_get_data_set_records=self.hrefs_v2.get_data_set_records_href(TestAIOpenScaleClient.business_data_set_id),
    #         headers=self.ai_client._get_headers(),
    #         data_set_records=self.business_payload_records,
    ...
    def test_22_run_business_application(self):
    ...
    def test_24_get_drift_runs(self):
        ...
        self.assertEqual(response.status_code, 200, msg="Unable to get drift runs. Reason: {}".format(response.text))
        self.assertGreater(len(response.json()['runs']), 0, msg="No drift runs")

        drift_run_details = response.json()['runs']
        final_drift_run_details = wait_for_monitor_instance(
            run_drift_url,
            headers=TestAIOpenScaleClient.ai_client._get_headers(),
            run_id=drift_run_details[0]['metadata']['id'],
            waiting_timeout=120)

        print("Drift run details: {}".format(final_drift_run_details))

    def test_25_get_drift_measurements(self):
        query = '?start=2018-01-23T12:46:55.677590Z&end={}Z'.format(datetime.utcnow().isoformat())
    ...
    def test_26_run_correlation_monitor(self):
        time.sleep(10)
    ...
    def test_27_check_correlation_run(self):
    ...
    def test_28_check_correlations_metrics(self):
