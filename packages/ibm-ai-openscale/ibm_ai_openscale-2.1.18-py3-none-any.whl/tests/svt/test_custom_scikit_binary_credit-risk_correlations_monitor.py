...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_19_validate_correlation_prerequisites(self):
        ...
        # check drift with filter
        # check kpi with filter
    ...
    def test_21_run_correlation_monitor(self):
        ...
        print(self.hrefs_v2.get_monitor_instance_run_href(TestAIOpenScaleClient.corr_monitor_instance_id))
        response = request_session.get(
    ...
    def test_22_wait_for_correlation_finish(self):
        ...
        while status != 'finished' and status != "error" and elapsed_time < 120:
            response = request_session.get(
                url=self.hrefs_v2.get_monitor_instance_run_details_href(
                    TestAIOpenScaleClient.corr_monitor_instance_id,
                    TestAIOpenScaleClient.corr_run_id),
                headers=TestAIOpenScaleClient.ai_client._get_headers()
            )
        ...
        print(response.json())
        self.assertEqual(status, 'finished', msg="Correlation computation failed. Reason: {}".format(response.json()))
