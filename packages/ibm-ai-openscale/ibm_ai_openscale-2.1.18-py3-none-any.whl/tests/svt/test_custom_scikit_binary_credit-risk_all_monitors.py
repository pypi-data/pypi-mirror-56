...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_02_bind_custom(self):
        ...
        if TestAIOpenScaleClient.binding_uid is None:
            ...
            TestAIOpenScaleClient.binding_uid = self.ai_client.data_mart.bindings.add("Custom ML engine", CustomMachineLearningInstance(self.credentials))
    ...
    def test_10_stats_on_payload_logging_table(self):
        wait_for_payload_table(subscription=self.subscription, payload_records=160)
        ...
        assert_payload_logging_python_table_content(python_table_content=python_table_content,
                                                    fields=['prediction', 'probability', 'Job', 'InstallmentPlans'])
