...
"""+str(cs("default (if arg not given): "+yesterday.strftime("%Y-%m-%d %H:%M:%S"), "lightgrey13")), default=yesterday.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    if log_group is None:
        ...
        print(cs("printing a list of log groups:", "DarkSlateGray3"))
        print()
        try:
        ...
        cmd = f"aws logs filter-log-events --log-group-name {log_group} --start-time {time_to_epoch(start_time)} --output json"
