...
#catch numba performance warnings
def fxn():
    warnings.warn('performance',numba.NumbaPerformanceWarning)
...
class HighDensityRegions():
    ...
    def optimize(self,X,y=None,cost=None, bins_per_dimension=21, number_of_random_simulations=500,           number_of_realizations=10, smoothing_parameter=2.0,
                 agg_func='count'):
...
#             bins3 = [np.concatenate([[-np.inf],_bins_,[np.inf]]) for _bins_ in bins3]
            ...
            pdf_combos[count] = np.flipud(np.fft.fftshift(np.real(np.fft.ifftn(np.fft.fftn(data)*fftkernel))))
            count+=1
        ...
        dim_combos = np.array(dim_combos).astype(np.int)
        args = [dim_combos,pdf_combos,bin_combos,min_lim,max_lim,param_to_dim,self.num_dim,widths]
        self.args = args
...
#             self.kde_target[count] = np.flipud(self.kde_target[count])
    ...
    def prune_clusters(self,X,pad=1.0,limit=0):
        ...
        reduce = [i for i in range(mask.shape[1]) if np.sum(mask[:,i]==1,axis=0)==0]
        self.num_clusters = self.num_clusters - len(reduce)
        mean = np.delete(self.centroids_, reduce, axis=0).flatten()
        
        variance = self.variance_.flatten()
        
        self.kmldata.best_weight_vector = np.concatenate([mean,variance]).reshape(-1,1)
        
        
    def get_kde_estimate(self,pad=1.0):
        
        dim_combos,pdf_combos,bin_combos,fftkernel,min_lim,max_lim,param_to_dim,num_dim,widths = self.kmldata.args
        combo_len = len(self.dim_combos)

        self.kde_estimate = np.zeros((combo_len,self.bins_per_dim,self.bins_per_dim))
        var = self.variance_*pad
        mean = self.centroids_
