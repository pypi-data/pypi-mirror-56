...
class HighDensityRegions():
    ...
    def optimize(self,X,y=None,cost=None, bins_per_dimension=41, number_of_random_simulations=1000, number_of_realizations=10, smoothing_parameter=1.0, agg_func='count'):
...
#        self.get_kde_estimate
    ...
    def get_kde_estimate(self,pad=1.0):
        ...
        for i,j in self.dim_combos:
            ...
            pdf=data1/np.sum(data1)
#            pdf = np.fft.fftshift(np.real(np.fft.ifftn(np.fft.fftn(data1)*fftkernel)))
#            self.kde_estimate[count] = np.flipud(pdf)
            self.kde_estimate[count] = pdf
            count+=1
