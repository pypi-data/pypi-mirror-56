...
class UpdateVersion:
    ...
    def get_version(self):
        ...
        for index, part in enumerate(v):
            ...
            index_check = index - 1
            print(next_part >= self.max_value and index != len(v) - 1, len(v))

            if next_part >= self.max_value and index != len(v) - 1:
                if index_check <= 0 and index != len(v) - 1:
                    self.version_parts[index_check] = "0"
                continue

            self.version_parts[index] = str(next_part)
    ...
    def save_new_version(self, version=None):
