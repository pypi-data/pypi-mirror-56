...
from pyplan.classes.Model import Model
...
class CalcEngine(object):
    ...
    def initializeModel(self, modeFileName=None, skipStartMonitor=False):
        if os.getenv('USE_MULTIPROCESS', '1') == "1":
            ...
            if not skipStartMonitor:
                if self.mode == "fixed":
                    Timer(5, self._monitorFixed).start()
                else:
    ...
    def _startEnvironment(self, modelPath):
        envPath = os.path.join(modelPath, ".env")

        # only use vevn if exists requirements.txt
        if os.path.isfile(os.path.join(modelPath, "requirements.txt")):

            if platform == "linux" or platform == "linux2" or platform == "darwin":
                if not os.path.isdir(envPath):
                    print("Creating venv...")
                    os.system('python3 -m venv "' + envPath +
                              '" --system-site-packages --symlinks')

                if os.path.isfile(os.path.join(modelPath, "requirements.txt")):
                    print("Installing librearies...")
                    command = f'{envPath}/bin/pip3 install -r {modelPath}/requirements.txt'
                    os.system(command)

                print(f'{envPath}/bin/python3')
                multiprocessing.set_executable(f'{envPath}/bin/python3')
                ...
                if not os.path.isdir(envPath):
                    os.system('python -m venv "' + envPath +
                              '" --system-site-packages --symlinks')

                if os.path.isfile(os.path.join(modelPath, "requirements.txt")):
                    command = 'cd "' + modelPath + '" & "' + envPath + \
    ...
    def _monitorFixed(self):
        """Monitor python proces for prevent oom. used only with fixed engine """
        pass
