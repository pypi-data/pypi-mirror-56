...
from pyplan.classes.BaseNode import BaseNode
from pyplan.classes.Evaluator import Evaluator
from pyplan.classes.Intellisense import Intellisense
from pyplan.classes.IOModule import IOModule
from pyplan.classes.PyplanFunctions import Selector
from pyplan.classes.wizards import (CalculatedField, SelectColumns, SelectRows, DataframeIndex, DataframeGroupby,sourcecsv)
from pyplan.common.classes.indexValuesReq import IndexValuesReq
...
class Model(object):
    ...
    def openModel(self, fileName=None, textModel=None):
        ...
        try:
            ppxarray = ""
            base_path = environ.Path(__file__) - 2
            ppxarray = os.path.join(base_path,"extras","pyplan_xarray_extensions.ppl")

            if os.path.isfile(ppxarray):
                self.importModule('pyplan_library', ppxarray, "2")
    ...
    def createSystemNodes(self, fileName):
        ...
        node = self.createNode(identifier="cub_sys_userid",
                               moduleId=self.modelNode.identifier)
        node.system = True
        node = self.createNode(
            identifier="cub_sys_foldername", moduleId=self.modelNode.identifier)
        node.system = True
        node = self.createNode(
            identifier="cub_sys_companyid", moduleId=self.modelNode.identifier)
        node.system = True
    ...
    def createSymlinks(self, path):
        ...
        venv_path="/venv/lib/python3.7/site-packages"
        package_paths = getsitepackages()
        if len(package_paths)>0:
            venv_path = package_paths[0]
        os.system(f"ln -s {user_lib_path} {venv_path}")
       
        # Add model libs to system paths  (FOR BACKWARD COMPATIBILITY)
        model_lib_path = os.path.join(path, "libs")
...
result = {
    ...
    def getToolbars(self, extraPath):
        ...
        base_path = environ.Path(__file__) - 2
        fileName = os.path.join( base_path ,"extras", "toolbars.json")
    ...
    def installLibrary(self, lib, target):
        ...
        def _install(command):
            ...
            symlink_path = os.path.join(target,"dummy","dummy.ppl")
            self.createSymlinks(symlink_path)
            importlib.invalidate_caches()

        cmd = f"pip install --user {lib}"

        # Set python base path
        os.environ["PYTHONUSERBASE"] =  os.path.join(target,".venv")
