...
#   Balance Sheet
IEX_BALANCE_SHEET_URL = prepend_iex_url(endpoint='stock') + '{symbol}/balance-sheet'
...
#   Book
IEX_BOOK_URL = prepend_iex_url(endpoint='stock') + '{symbol}/book?'
def book(symbol):
...
IEX_CASH_FLOW_URL = prepend_iex_url(endpoint='stock') + '{symbol}/cash-flow'
...
# CEO Compensation
IEX_CEO_COMPENSATION_URL = prepend_iex_url(endpoint='stock') + '{symbol}/ceo-compensation'
def ceo_compensation(symbol):
    url = replace_url_var(IEX_CEO_COMPENSATION_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Collections
IEX_COLLECTION_URL = prepend_iex_url(endpoint='stock') + 'market/collection/{collectionType}?collectionName={collectionName}'
...
#   Company
IEX_COMPANY_URL = prepend_iex_url(endpoint='stock') + '{symbol}/company?'
def company(symbol):
    url = replace_url_var(IEX_COMPANY_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Delayed Quote
IEX_DELAYED_QUOTE_URL = prepend_iex_url(endpoint='stock') + '{symbol}/delayed-quote?'
def delayed_quote(symbol):
    url = replace_url_var(IEX_DELAYED_QUOTE_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Dividends
IEX_DIVIDENDS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/dividends/{scope}?'
def dividends(symbol, scope):
    url = replace_url_var(IEX_DIVIDENDS_URL, symbol=symbol, scope=scope)
    return get_iex_json_request(url)

#   Earnings
IEX_EARNINGS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/earnings'
...
#   Earnings Today
IEX_TODAY_EARNINGS_URL = prepend_iex_url(endpoint='stock') + 'market/today-earnings?'
def today_earnings():
...
# DEPRECATED
"""#   Effective Spread
IEX_EFFECTIVE_SPREAD_URL = prepend_iex_url(endpoint='stock') + '{symbol}/effective-spread?'
def effective_spread(symbol):
    url = replace_url_var(IEX_EFFECTIVE_SPREAD_URL, symbol=symbol)
    return get_iex_json_request(url)"""

#   Estimates
IEX_ESTIMATES_URL = prepend_iex_url(endpoint='stock') + '{symbol}/estimates?'
def estimates(symbol):
    url = replace_url_var(IEX_ESTIMATES_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Financials
IEX_FINANCIALS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/financials?'
...
#   Fund Ownership
IEX_FUND_OWNERSHIP_URL = prepend_iex_url(endpoint='stock') + '{symbol}/fund-ownership?'
def fund_ownership(symbol):
    url = replace_url_var(IEX_FUND_OWNERSHIP_URL, symbol=symbol)
    return get_iex_json_request(url)

IEX_HISTORICAL_URL = prepend_iex_url(endpoint='stock') + '{symbol}/chart'
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
#IEX_CHART_URL = prepend_iex_url(endpoint='stock') + '{symbol}/chart'
...
#   Income Statement
IEX_INCOME_STATEMENT_URL = prepend_iex_url(endpoint='stock') + '{symbol}/income'
...
#   Insider Roster
IEX_INSIDER_ROSTER_URL = prepend_iex_url(endpoint='stock') + '{symbol}/insider-roster?'
def insider_roster(symbol):
    url = replace_url_var(IEX_INSIDER_ROSTER_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Insider Summary
IEX_INSIDER_SUMMARY_URL = prepend_iex_url(endpoint='stock') + '{symbol}/insider-summary?'
def insider_summary(symbol):
    url = replace_url_var(IEX_INSIDER_SUMMARY_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Insider Transactions
IEX_INSIDER_TRANSACTIONS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/insider-transactions?'
def insider_transactions(symbol):
    url = replace_url_var(IEX_INSIDER_TRANSACTIONS_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Institutional Ownership
IEX_INSTITUTIONAL_OWNERSHIP_URL = prepend_iex_url(endpoint='stock') + '{symbol}/institutional-ownership?'
def institutional_ownership(symbol):
    url = replace_url_var(IEX_INSTITUTIONAL_OWNERSHIP_URL, symbol=symbol)
    return get_iex_json_request(url)

#   IPO Calendar
IEX_UPCOMING_IPOS_URL = prepend_iex_url(endpoint='stock') + 'market/upcoming-ipos?'
def ipo_upcoming():
    return get_iex_json_request(IEX_UPCOMING_IPOS_URL)

IEX_TODAY_IPOS_URL = prepend_iex_url(endpoint='stock') + 'market/today-ipos?'
def ipo_today():
    return get_iex_json_request(IEX_TODAY_IPOS_URL)

#   Key Stats
IEX_STATS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/stats'
...
#   Largest Trades
IEX_LARGEST_TRADES_URL = prepend_iex_url(endpoint='stock') + '{symbol}/largest-trades?'
def largest_trades(symbol):
    url = replace_url_var(IEX_LARGEST_TRADES_URL, symbol=symbol)
    return get_iex_json_request(url)

#   List
IEX_MARKET_LIST_URL = prepend_iex_url(endpoint='stock') + '{symbol}/list/{list_type}?'
...
#   Logo
IEX_LOGO_URL = prepend_iex_url(endpoint='stock') + '{symbol}/logo?'
def logo(symbol):
    url = replace_url_var(IEX_LOGO_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Market Volume (U.S.)
IEX_MARKET_VOLUME_URL = prepend_iex_url(endpoint='stock') + 'market/volume?'
...
#   News
IEX_NEWS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/news'
...
#   OHLC
IEX_OHLC_URL = prepend_iex_url(endpoint='stock') + '{symbol}/ohlc?'
def ohlc(symbol):
...
#   Peers
IEX_PEERS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/peers?'
def peers(symbol):
    url = replace_url_var(IEX_PEERS_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Previous Day Prices
IEX_PREVIOUS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/previous?'
def previous(symbol):
    url = replace_url_var(IEX_PREVIOUS_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Price
IEX_PRICE_URL = prepend_iex_url(endpoint='stock') + '{symbol}/price?'
def price(symbol):
    url = replace_url_var(IEX_PRICE_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Price Target
IEX_PRICE_TARGET_URL = prepend_iex_url(endpoint='stock') + '{symbol}/price-target?'
def price_target(symbol):
    url = replace_url_var(IEX_PRICE_TARGET_URL, symbol=symbol)
    return get_iex_json_request(url)

#   Quote
IEX_QUOTE_URL = prepend_iex_url(endpoint='stock') + '{symbol}/quote'
...
#   Recommended Trends
IEX_RECOMMENDED_TRENDS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/recommendation-trends?'
...
# DEPRECATED
"""#   Relavent Stocks
IEX_RELEVANT_URL = prepend_iex_url(endpoint='stock') + '{symbol}/relevant?'
def relevant(symbol):
    url = replace_url_var(IEX_RELEVANT_URL, symbol=symbol)
    return get_iex_json_request(url)"""

#   Sector Performance
IEX_SECTOR_PERFORMANCE_URL = prepend_iex_url(endpoint='stock') + 'market/sector-performance?'
def sector_performance():
    return get_iex_json_request(IEX_SECTOR_PERFORMANCE_URL)

#   Splits
IEX_SPLITS_URL = prepend_iex_url(endpoint='stock') + '{symbol}/splits'
...
#   Volume by Venue
IEX_VOLUME_BY_VENUE_URL = prepend_iex_url(endpoint='stock') + '{symbol}/volume-by-venue'
