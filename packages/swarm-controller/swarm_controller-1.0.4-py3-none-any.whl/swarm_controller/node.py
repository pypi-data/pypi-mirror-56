...
class SwarmNode:
    ...
    def update_node_spec(self):
        try:
            ...
            if e.status_code == 503:
                click.echo("Attempted to update node - NOT a swarm manager.")
            else:
                raise
    ...
    def set_available(self):
        self.refresh()
        self.refresh_node_spec()
        self.node_spec["Availability"] = "Available"
        self.update_node_spec()
    ...
    def check_split_brain(self):
        ...
        if managers and self.swarm.get_max_quorum() > managers:
            if managers == 1:
                # from the docs: Only use --force in situations
                # where the swarm will no longer be used after the
                # manager leaves, such as in a single-node swarm
                self.docker_client.leave_swarm(force=True)
            else:
                self.drain()
                self.demote()
