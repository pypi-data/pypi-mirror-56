...
class Mssql_Dst(Mssql_Source):
    ...
    def run_import(self,dst_db,titleStr,sql_datastr_list):
        ...
        try:
            ...
            mssql_execute(dst_db,titleStr+"\n"+ sql_str_data)
        finally:
            lock.release()
            taskcount-=1

    def update_data(self,dst_table,key_list,dst_title,dst_data):
        for row_dict in dst_data:
            dict_source_row={}
            #获取数据单行
            for title in dst_title:
                dict_source_row[title]=row_dict[title]
            #对每行数据进行处理
            dst_row_dict=self.row_process(dict_source_row)
            sql_str_data=self.get_select_row_str(dst_title, dst_row_dict)+"\n"
            sql_str_data_list.append(sql_str_data)
        sql_str="select * from (" + "Union All\n".join(sql_str_data_list) + ") ut"
    ...
    def import_into_dst(self,source_data,dst_table,dst_title=None,source_title=None,batch=1000):
        ...
        if dst_title is None:
            ...
            dst_title_tmp=self.get_sql_title(sql_str)
            dst_title={}
            if bool(source_data):
            ...
            dst_title_temp=self.get_sql_title(sql_str)
            dst_title_dict={}
            for title in dst_title:
        ...
        #插入数据的Insert头部语句
        title_str=self.get_insert_title_str(dst_table,dst_title)
        #待插入的数据
        sql_str_data_list=[]
        #行计数 
        rowcount=0
        ...
        for row in source_data:
            ...
            sql_str_data=self.get_select_row_str(dst_title, dst_row_dict)+"\n"
            sql_str_data_list.append(sql_str_data)
            if len(sql_str_data_list)==batch:
