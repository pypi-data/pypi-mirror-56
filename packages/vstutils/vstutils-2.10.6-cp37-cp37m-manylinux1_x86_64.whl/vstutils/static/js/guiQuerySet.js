...
pushed=true;}}
if(!pushed){bulk_data[field_name].push({data_type:obj.data_type,filter_name:field.options.additionalProperties.value_field,filter_values:[obj.id],});}}}
return bulk_data;}
_loadPrefetchData(prefetch_fields,instances){let promises=[];let bulk_data=this._getBulkDataForPrefetch(prefetch_fields,instances);for(let key in bulk_data){if(bulk_data.hasOwnProperty(key)){for(let index=0;index<bulk_data[key].length;index++){let item=bulk_data[key][index];let filters={};filters[item.filter_name]=item.filter_values;let bulk={method:'get',data_type:item.data_type,filters:this.makeQueryString(filters),};promises.push(this.sendQuery(bulk).then(res=>{this._setPrefetchValue(res,bulk_data,instances,key);}).catch(error=>{debugger;}));}}}
return Promise.all(promises);}
_setPrefetchValue(res,bulk_data,instances,field_name){if(res.status!="200"){return;}
let prefetch_data=res.data.results;let props=this.model.fields[field_name].options.additionalProperties;let view_field=props.view_field;let value_field=props.value_field;for(let index=0;index<instances.length;index++){let instance=instances[index];for(let j in prefetch_data){if(prefetch_data[j][value_field]==instance.data[field_name]){instance.data[field_name]={value:instance.data[field_name],prefetch_value:prefetch_data[j][view_field],};}}}}};