...
pushed=true;}}
if(!pushed){bulk_data[field_name].push({instances_ids:[instance.getPkValue()],data_type:obj.data_type,filter_name:field.options.additionalProperties.value_field,filter_values:[obj.id],});}}}
return bulk_data;}
...
let prefetch_data=res.data.results;let props=this.model.fields[field_name].options.additionalProperties;let view_field=props.view_field;let value_field=props.value_field;for(let index=0;index<instances.length;index++){let instance=instances[index];if(!bulk_data_item.instances_ids.includes(instance.getPkValue())){continue;}
for(let j in prefetch_data){if(prefetch_data[j][value_field]==instance.data[field_name]){instance.data[field_name]={value:instance.data[field_name],prefetch_value:prefetch_data[j][view_field],};}}}}};