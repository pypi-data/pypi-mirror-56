    ...
    async def trace_middleware(request: Request, handler):
        ...
        try:
            with tracer.new_trace(
                    request.headers.get(config.trace_id_header_name), config.tracing_config.sampled
            ) as span:
                ...
                try:
                    ...
                    if trace_id:
                        response.headers[config.trace_id_header_name] = request.headers.get(config.trace_id_header_name)
                    return response
                finally:
