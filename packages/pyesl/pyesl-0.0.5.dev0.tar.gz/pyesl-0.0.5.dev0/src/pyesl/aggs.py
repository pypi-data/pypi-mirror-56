...
class Groupby(AggsBase):
    ...
    def __init__(self, field: str = 'key', name: str = None, size: int = 1024, **params):
        self._field = field
        _name = self._format_name(name or 'agg_{}'.format(field))
        _body = {
            _name: {
                'terms': params
            }
...
class Step(AggsBase):
    ...
    def __init__(self, field: str = 'ts', name: str = 'agg_ts', step_s=3600, **params):
        self._field = field
        _name = self._format_name(name or 'agg_'.format(field))
        self._step_s = step_s
        _body = {
...
class FieldCalculation(AggsBase):
    ...
    def __init__(
        ...
        _name = self._format_name('cal_{}{}{}'.format(func, sorted(fields.keys()), expr))
...
class Calculations(AggsBase):
    ...
    def __init__(self, *calculations: FieldCalculation):
