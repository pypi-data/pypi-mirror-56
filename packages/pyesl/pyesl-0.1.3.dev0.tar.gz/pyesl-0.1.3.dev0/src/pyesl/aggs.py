...
class FieldCalculation(AggsBase):
    ...
    def __init__(
        ...
        if not self._is_script:
            _name = self._format_name('{}_{}'.format(func, base64.urlsafe_b64encode(
                hashlib.md5(str(sorted(fields.keys())).encode("utf-8")).digest()).decode()))
            _body = {
                _name: {
                    self._func: params
                }
            ...
            _name = self._format_name('{}_{}'.format(func, base64.urlsafe_b64encode(
                hashlib.md5(expr.encode("utf-8")).digest()).decode()))
            _body = {
                _name: {
                    self._func: params
                }
