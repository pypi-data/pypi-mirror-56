...
class FitActivity(fitparse.FitFile):
  ...
  # This can go away, I think. In Activity, when auto-detecting
  # events from speed data, we can compare to file events.
  ...
  def __init__(self, file_obj, remove_stopped_periods=False):
    ...
    # TO ACTIVITY?
    # Adds detected events based on speed, in addition to timer events.
    # Pass a blocked dataframe into Activity and remove stopped periods there?
    # Basically, always add events to the filereader dataframe, then do the
    # speed logic in Activity. That way Activities will always pass through
    # the same slow-speed filter, even when manually entered as DataFrames.
    ...
    # TO ACTIVITY (partial)
    # Split this between Activity and this class.
    # Keep the manual start/stop logic here; base blocks on that.
    # Move the auto-detect speed stuff to Activity.
    # Same goes for the event detection.
