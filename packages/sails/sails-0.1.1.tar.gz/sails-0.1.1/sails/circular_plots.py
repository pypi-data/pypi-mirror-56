...
class CircosGroup(object):
    ...
    def find_region_idx(self, region):
        """Find a region index from a name.  Returns None if not present"""
        if region in self.regions:
            return self.regions[region]['analysisid']
        return None

    def find_region_from_idx(self, idx):
        """Find a region index from an index.  Returns None if not present"""
    ...
    def circular_regions(self):
        """Return a list of the regions in circular plotting order"""
        return [x[0] for x in sorted(self.regions.items(),
                                     key=lambda x: x[1]['offset'])]

    @property
    def circular_shortnames(self):
        """Return a list of the short names in circular plotting order"""
    ...
    def netmat_regions(self):
        """Return a list of the regions in netmat plotting order"""
    ...
    def netmat_shortnames(self):
        """Return a list of the short names in netmat plotting order"""
...
class CircosHandler(object):
    ...
    def colour_mapping(self, normalise=True):
        ...
        If normalise is True, these will be scaled from the standard 0-255
    ...
    def from_csv_files(cls, group_filename, region_filename, analysis_column='AnalysisID'):
        """
        This routine requires two CSV files.

        The group file must contain a list of groups; in Circos
        terms these are known as the 'chromosomes'.  This file
    ...
    def ticks(self):
        ...
        # A header
        s.append('show_ticks          = yes')
        s.append('show_tick_labels    = yes')
        s.append('')
        s.append('<ticks>')
    ...
    def links(self, data, linkcolors=None, defaultlinkcolor='black'):
        """
        data: an (N x N) matrix containing thickness (and np.nan where no connection is desired)

        linkcolors: One of (defaults to None):
          * a tuple of three strings (zerocolor, negcolor, poscolor); e.g. ('black', 'blue', 'red)
          * None (in which case ('black', 'blue', 'red') will be assumed)
          * a numpy array of dtype object (N, N) with a specific color string in each entry
        ...
        defaultlinkcolor: defaults to 'black'.  Only used is linkcolors is a numpy array
    ...
    def gen_circos_files(self, data, karyotype_name, output_name,
        ...
        """
        data: an (N x N) matrix containing thickness (and np.nan where no connection is desired)
        karyotype_name: string containing the filename of your karyotype file on disk
        output_name: output basename of configuration files (with no extension)

        Optionally, you can override:
        radius: in pixels (defaults to 1500)
        circos_path: where to find the circos configuration files (defaults to /etc/circos)
        output_name: Directory in which to store output

        You can also pass named arguments linkcolors and defaultlinkcolor which will
        be passed down to the `links` routine.

        Note that if you are creating plots for publication using Circos, you should
        cite the relevant publication:
