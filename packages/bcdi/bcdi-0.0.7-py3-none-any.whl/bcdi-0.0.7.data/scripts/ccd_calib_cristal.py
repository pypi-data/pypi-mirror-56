...
hkls = [(0, 0, 0)]  # list of tuples of hkls, for each scan. Put [(0,0,0)] for a mesh in direct beam.
...
photon_threshold = 20
setup = 'CRISTAL'  #
frames_to_exclude = [[9, 10, 21, 32, 88, 89, 90, 91, 99, 100, 101, 102, 103, 104, 105, 106, 107,
                      108, 109, 110, 11, 112, 113, 114, 115, 116, 117, 118, 119, 120]]  # [[list(3 + 7*np.linspace(0, 6, 7, dtype=int))],[]]
#  list of lists of frames to exclude, leave it as [[],[],...] otherwise, there should be as many sublists as scans

use_rawdata = 1  # 0 to draw a 3*3 square around the COM and mask the rest, 1 to use the raw data
...
for idx in range(nb_scans):
    h5file = h5py.File(specdir + 'S' + str(scan_nb[idx]) + 'det/data/S' + str(scan_nb[idx]) + '.nxs', 'r')
    ccdn = h5file['test_00'+str(scan_nb[idx])]['scan_data']['data_06'][:]  # ndarray if mesh
    print('Scan', scan_nb[idx], ' : ', ccdn.shape[0]*ccdn.shape[1], ' frames, ',
          len(frames_to_exclude[idx]), ' frames to exclude')
...
for idx in range(nb_scans):
    ...
    raw_mgomega = h5file['test_'+str('{:04d}'.format(scan_nb[idx]))]['CRISTAL']['I06-C-C07-EX-MG_OMEGA']['positon_pre'][:] / 1e6
    raw_delta = h5file['test_'+str('{:04d}'.format(scan_nb[idx]))]['scan_data']['actuator_1_1'][:]
    raw_gamma = h5file['test_'+str('{:04d}'.format(scan_nb[idx]))]['scan_data']['actuator_2_1'][:]
    ccdn = h5file['test_'+str('{:04d}'.format(scan_nb[idx]))]['scan_data']['data_06'][:]  # ndarray if mesh
    ccdn = np.reshape(ccdn, (ccdn.shape[0]*ccdn.shape[1], ccdn.shape[2], ccdn.shape[3]))
    ...
    for index in range(ccdn.shape[0]):
        ...
        if index not in frames_to_exclude[idx]:
            ...
            if use_rawdata == 0:
                ...
                data[total_offset + index - index_offset,
                     int(np.rint(y0))-1:int(np.rint(y0))+2, int(np.rint(x0))-1:int(np.rint(x0))+2] = 1000
            else:
...
if imgpbcnt == 0:
    ...
    param, eps = xu.analysis.sample_align.area_detector_calib(
        gamma, delta, data, ['z+', 'y-'], 'x+', plot=True, start=start_variable,
        fix=fix_variable, plotlog=True, debug=False)
else:
    ...
    qconv = xu.experiment.QConversion(['y-'], ['z+', 'y-'], r_i=beam_direction)  # for ID01
    # pwidth1,pwidth2,distance,tiltazimuth,tilt,detector_rotation,outerangle_offset,sampletilt,sampletiltazimuth,wavelength
    hxrd = xu.HXRD([1, 0, 0], [0, 0, 1], wl=wl, qconv=qconv)
    param, eps = xu.analysis.area_detector_calib_hkl(mgomega, gamma, delta, data, hkl, hxrd, material,
                                                     ['z+', 'y-'], 'x+',
                                                     start=start_variable, fix=fix_variable, plotlog=True, debug=False)
