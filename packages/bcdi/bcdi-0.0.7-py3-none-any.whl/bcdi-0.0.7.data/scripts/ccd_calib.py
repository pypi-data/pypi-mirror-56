...
hkls = [(0, 0, 0), (1, 1, 1)]  # list of tuples of hkls, for each scan. Put [(0,0,0)] for a mesh in direct beam.
...
frames_to_exclude = [[10, 11, 12, 13, 14], [3, 10, 17, 24, 31, 38, 45]]  # [[list(3 + 7*np.linspace(0, 6, 7, dtype=int))],[]]
...
for idx in range(nb_scans):
    ccdn = specfile['/' + str(scan_nb[idx]) + '.1/measurement/' + counter].value  # ndarray if mesh
    print('Scan', scan_nb[idx], ' : ', len(ccdn), ' frames, ', len(frames_to_exclude[idx]), ' frames to exclude')
...
for idx in range(nb_scans):
    ...
    raw_eta = specfile['/' + str(scan_nb[idx]) + '.1/instrument/positioners/eta'].value  # float
    raw_delta = specfile['/' + str(scan_nb[idx]) + '.1/measurement/del'].value  # ndarray if mesh
    raw_nu = specfile['/' + str(scan_nb[idx]) + '.1/measurement/nu'].value  # ndarray if mesh
    ccdn = specfile['/' + str(scan_nb[idx]) + '.1/measurement/' + counter].value  # ndarray if mesh
...
if imgpbcnt == 0:
    ...
    param, eps = xu.analysis.sample_align.area_detector_calib(
        nu, delta, data, ['z-', 'y-'], 'x+', plot=True, start=start_variable,
    ...
    param, eps = xu.analysis.area_detector_calib_hkl(eta, nu, delta, data, hkl, hxrd, material, ['z-', 'y-'], 'x+',
