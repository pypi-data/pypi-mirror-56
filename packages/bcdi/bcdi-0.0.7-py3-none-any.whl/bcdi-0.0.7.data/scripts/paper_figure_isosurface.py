...
field_of_view = 500  # in nm, should not be larger than the total width at the moment (no padding)
...
##########################
file_path = filedialog.askopenfilename(initialdir=datadir, title="Select simulation file", filetypes=[("NPZ", "*.npz")])
...
##########################
file_path = filedialog.askopenfilename(initialdir=datadir, title="Select phased file", filetypes=[("NPZ", "*.npz")])
print('Opening ', file_path)
npzfile = np.load(file_path)
amp = npzfile['amp']
phase = npzfile['phase']
...
plt.figure()
plt.imshow(new_phase[numz//2, :, :], cmap=my_cmap, vmin=-phase_range, vmax=phase_range)
plt.title('Phase after subpixel shift')
plt.pause(0.1)
del new_phase
plt.figure()
plt.imshow(new_strain[numz//2, :, :], cmap=my_cmap, vmin=-strain_range, vmax=strain_range)
plt.title('Strain after subpixel shift')
plt.pause(0.1)
new_amp = new_amp / new_amp.max()  # need to renormalize after subpixel shift interpolation
...
plt0 = ax0.imshow(masked_array[numz//2-pixel_FOV:numz//2+pixel_FOV, numy//2-pixel_FOV:numy//2+pixel_FOV, numx // 2],
...
plt1 = ax1.imshow(masked_array[numz//2-pixel_FOV:numz//2+pixel_FOV, numy // 2, numx//2-pixel_FOV:numx//2+pixel_FOV],
                  vmin=-phase_range, vmax=phase_range, cmap=my_cmap)
...
plt2 = ax2.imshow(masked_array[numz // 2, numy//2-pixel_FOV:numy//2+pixel_FOV, numx//2-pixel_FOV:numx//2+pixel_FOV],
                  vmin=-phase_range, vmax=phase_range, cmap=my_cmap)
ax2.invert_yaxis()
...
fig, ax0 = plt.subplots(1, 1)
plt0 = ax0.imshow(
    masked_array[numz // 2 - pixel_FOV:numz // 2 + pixel_FOV, numy // 2 - pixel_FOV:numy // 2 + pixel_FOV, numx // 2],
...
fig, ax1 = plt.subplots(1, 1)
plt1 = ax1.imshow(
    masked_array[numz // 2 - pixel_FOV:numz // 2 + pixel_FOV, numy // 2, numx // 2 - pixel_FOV:numx // 2 + pixel_FOV],
    vmin=-strain_range, vmax=strain_range, cmap=my_cmap)
ax1.xaxis.set_major_locator(ticker.MultipleLocator(pixel_spacing))
ax1.yaxis.set_major_locator(ticker.MultipleLocator(pixel_spacing))
ax1.tick_params(labelbottom=False, labelleft=False, top=True, right=True, direction=tick_direction,
...
fig, ax2 = plt.subplots(1, 1)
plt2 = ax2.imshow(
    masked_array[numz // 2, numy // 2 - pixel_FOV:numy // 2 + pixel_FOV, numx // 2 - pixel_FOV:numx // 2 + pixel_FOV],
    vmin=-strain_range, vmax=strain_range, cmap=my_cmap)
ax2.invert_yaxis()
ax2.xaxis.set_major_locator(ticker.MultipleLocator(pixel_spacing))
ax2.yaxis.set_major_locator(ticker.MultipleLocator(pixel_spacing))
...
plt.colorbar(plt2, ax=ax2)
plt.savefig(savedir + 'phased_strain_XY' + comment + '_colorbar.png', bbox_inches="tight")
