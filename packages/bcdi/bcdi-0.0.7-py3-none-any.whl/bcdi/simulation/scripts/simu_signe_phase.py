...
area_nanowire = np.where((ygrid < radius_NW) &
                         (ygrid > -radius_NW) &
                         ...
                         (ygrid < np.tan(alpha)*(xgrid + radius_NW)) &
                         (ygrid > -np.tan(alpha)*(xgrid + radius_NW)), 1, 0)

area_core = np.where((ygrid < radius_core) &
                     (ygrid > -radius_core) &
                     ...
                     (ygrid < np.tan(alpha)*(xgrid + radius_core)) &
                     (ygrid > -np.tan(alpha)*(xgrid + radius_core)), 1, 0)
...
plt0 = ax0.imshow(nanowire[half_window - 100:half_window+100, half_window-100:half_window+100],
                  cmap=my_cmap, vmin=0, vmax=35)
...
ax0.tick_params(labelbottom='off', labelleft='off', direction='in', top='on', right='on',
...
if save_colorbar == 1:
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    ax0.tick_params(labelbottom='on', labelleft='on', labelsize=12, direction='in', top='on', right='on',
                    length=tick_length, width=tick_width)
    plt.pause(0.5)
...
alpha = -radius_core*radius_core*misfit*(1+nu)/(2*radius_NW*radius_NW*(1-nu))  # dimensionless
...
fig, ax0 = plt.subplots(1, 1)
plt0 = ax0.imshow(displacement[half_window - 100:half_window+100, half_window-100:half_window+100],
                  cmap=my_cmap, vmin=-phase_range, vmax=phase_range)
ax0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
...
print('Min(log10(intensity)', np.log10(intensity).min())  # should be same as mathematica
print('Max(log10(intensity)', np.log10(intensity).max())  # should be same as mathematica

fig, ax0 = plt.subplots(1, 1)
plt0 = ax0.imshow(np.log10(intensity), extent=(qx.min(), qx.max(), qy.min(), qy.max()),
                  cmap='jet', vmin=-7, vmax=4)
ax0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
...
if save_colorbar == 1:
    ...
    plt.xlabel('Qx')
    plt.ylabel('Qy')
    ax0.tick_params(labelbottom='on', labelleft='on', labelsize=12, direction='out', top='on', right='on',
                    length=tick_length, width=tick_width)
    plt.pause(0.5)
    plt.savefig(savedir + 'FFT_positive' + comment + '_ortho_colorbar_jet.png', bbox_inches="tight")

fig, ax0 = plt.subplots(1, 1)
plt0 = ax0.imshow(np.log10(intensity[half_window - 20:half_window+20, half_window-20:half_window+20]),
                  cmap='jet', vmin=-7, vmax=4)
ax0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
plt.savefig(savedir + 'FFT_positive' + comment + '_ortho_zoom_jet.png', bbox_inches="tight")
...
ax0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
plt.savefig(savedir + 'FFT_negative' + comment + '_ortho_zoom_jet.png', bbox_inches="tight")
...
ax0.tick_params(labelbottom='off', labelleft='off', top='on', right='on', labelsize=12, direction='out',
                length=tick_length, width=tick_width)
plt.pause(0.5)
...
if save_colorbar == 1:
    ...
    ax0.tick_params(labelbottom='on', labelleft='on', labelsize=12, direction='out', top='on', right='on',
                    length=tick_length, width=tick_width)
    plt.pause(0.5)
    plt.savefig(savedir + 'FFT_positive' + comment + '_colorbar.png', bbox_inches="tight")
...
plt0 = ax0.imshow(np.log10(intensity), extent=(qx.min(), qx.max(), qy.min(), qy.max()),
                  cmap=my_cmap, vmin=colorbar_range[0], vmax=colorbar_range[1])
ax0.tick_params(labelbottom='off', labelleft='off', direction='out', top='on', right='on',
                length=tick_length, width=tick_width)
plt.pause(0.5)
...
plt0 = x0.imshow(np.log10(intensity[half_window - 20:half_window+20, half_window-20:half_window+20]),
                 cmap=my_cmap, vmin=colorbar_range[0], vmax=colorbar_range[1])
x0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
...
# kinematic sums
#######################
nanowire_zoom = nanowire[half_window - 50:half_window+50, half_window-50:half_window+50]
plt.figure()
...
grid_x = grid_x[half_window - 50:half_window+50, half_window-50:half_window+50]
grid_y = grid_y[half_window - 50:half_window+50, half_window-50:half_window+50]
...
for ii in range(len(qy)):
    for jj in range(len(qx)):
        Fhk1[ii, jj] = Fhk1[ii, jj] + (nanowire_zoom*np.exp(+1j*(qx1[ii, jj]*grid_x + qy1[ii, jj]*grid_y))).sum()

intensity = abs(Fhk1)**2
fig, ax0 = plt.subplots(1, 1)
ax0.imshow(np.log10(intensity), extent=(qx.min(), qx.max(), qy.min(), qy.max()),
           cmap=my_cmap, vmin=colorbar_range[0], vmax=colorbar_range[1])
ax0.tick_params(labelbottom='off', labelleft='off', direction='out', top='on', right='on',
                length=tick_length, width=tick_width)
plt.pause(0.5)
...
if save_colorbar == 1:
    ...
    ax0.tick_params(labelbottom='on', labelleft='on', labelsize=12, direction='out', top='on', right='on',
                    length=tick_length, width=tick_width)
    plt.pause(0.5)
    plt.savefig(savedir + 'Kinematic_+1j_+ux' + comment + '_colorbar.png', bbox_inches="tight")
...
x0.tick_params(labelbottom='off', labelleft='off', bottom='off', left='off', top='off', right='off')
plt.pause(0.5)
plt.savefig(savedir + 'GeSi_NW_kinsum_+1j_+ux' + comment + '_zoom.png', bbox_inches="tight")
