...
photon_threshold = 1  # for noise removal, in detector counts:   data[data <= photon_threshold] = 0
debug = 0  # 1 to show summed data and central frame before cropping
centering = 0  # Bragg peak determination, 0 for max, 1 for center of mass, 0 is better usually
...
hxrd.Ang2Q.init_area('z-', 'y+', cch1=cch1, cch2=-cch2, Nch1=nch1, Nch2=nch2, pwidth1=pixelsize, pwidth2=pixelsize,
                     distance=sdd, detrot=-0.385, tiltazimuth=237.2, tilt=1.316)
...
new_qx, new_qz, new_qy = np.meshgrid(np.linspace(limit_q[0], limit_q[1], num=output_size[0], endpoint=True),
...
for idx in range(nb_scan):
    ...
    if setup == 'CRISTAL':
        ...
        spec_file = SpecFile(specdir + spec_name)
    qx, qz, qy, _, data, _, _ = gridmap(specfile=spec_file, scan_nb=scans[idx], mydetector=detector,
                                        region=roi, myflatfield=flatfield, myhotpixels=hotpix_array,
                                        mysetup=setup, myrocking_angle=rocking_angle,
                                        follow_bragg=follow_delta, myenergy=energy, myoffsets=offsets)
    data[data < photon_threshold] = 0
    rgi = RegularGridInterpolator((qx, qz, qy), data, method='linear', bounds_error=False, fill_value=0)
    new_data = rgi(np.concatenate((new_qx.reshape((1, new_qx.size)), new_qz.reshape((1, new_qz.size)),
                                  new_qy.reshape((1, new_qy.size)))).transpose())
    sum_data = sum_data + new_data.reshape((output_size[0], output_size[1], output_size[2])).astype(data.dtype)
...
plt.figure()
plt.subplot(2, 2, 1)
plt.contourf(new_qx[:, 0, 0], new_qz[0, :, 0], np.log10(abs(sum_data.sum(axis=2))).T, 150, cmap=my_cmap, vmin=0)
# plt.colorbar(ticks=[1, 2, 3, 4, 5])
plt.xlabel(r"Q$_x$ ($1/\AA$)")
plt.ylabel(r"Q$_z$ ($1/\AA$)")
plt.axis('scaled')
...
plt.subplot(2, 2, 2)
plt.contourf(new_qx[:, 0, 0], new_qy[0, 0, :], np.log10(abs(sum_data.sum(axis=1))).T, 150, cmap=my_cmap, vmin=0)
plt.xlabel(r"Q$_x$ ($1/\AA$)")
plt.ylabel(r"Q$_y$ ($1/\AA$)")
# plt.colorbar(ticks=[1, 2, 3, 4, 5])
plt.axis('scaled')
...
plt.subplot(2, 2, 3)
plt.contourf(new_qy[0, 0, :], new_qz[0, :, 0], np.log10(abs(sum_data.sum(axis=0))), 150, cmap=my_cmap, vmin=0, vmax=5.5)
plt.xlabel(r"Q$_y$ ($1/\AA$)")
plt.ylabel(r"Q$_z$ ($1/\AA$)")
# plt.colorbar(ticks=[1, 2, 3, 4, 5])
plt.axis('scaled')
