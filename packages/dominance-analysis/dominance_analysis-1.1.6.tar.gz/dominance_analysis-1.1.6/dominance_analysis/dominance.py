...
#Bala changes start
		if(self.data_format==0):
			iplot(incremental_rsquare_df[['Features','incremental_r2']].set_index("Features").iplot(asFigure=True,kind='bar',title="Incremental "+("Pseudo " if (self.objective!=1) else " ")+"R Squared for Top "+ str(self.top_k) +" Variables ",yTitle="Incremental R2",xTitle="Estimators"))
			...
			iplot(incremental_rsquare_df[['Features','incremental_r2']].set_index("Features").iplot(asFigure=True,kind='bar',title="Incremental "+("Pseudo " if (self.objective!=1) else " ")+"R Squared of " +" Variables ",yTitle="Incremental R2",xTitle="Estimators"))
...
#Bala changes end#Bala changes end
	...
	def predict_complete_dominance(self):
		...
		if(len(cd_df)):
			...
			return pd.DataFrame(data=None,columns=['Predictors','Dominating'])
	...
	def dominance_level(self):
		...
		comp_dom.rename(columns={'Dominating':'Completely Dominating'},inplace=True)
