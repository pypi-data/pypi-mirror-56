...
class Client(BaseClient):
    ...
    def create_app(
        ...
        """
        Creates an application. An application consists of one or more server groups. Each server group
          ...
          ::

            {
                'appSummary': {
                    ...
                    'replicationStatus':
                    'READY_FOR_CONFIGURATION'|'CONFIGURATION_IN_PROGRESS'|'CONFIGURATION_INVALID'
                    |'READY_FOR_REPLICATION'|'VALIDATION_IN_PROGRESS'|'REPLICATION_PENDING'
                    |'REPLICATION_IN_PROGRESS'|'REPLICATED'|'DELTA_REPLICATION_IN_PROGRESS'
                    |'DELTA_REPLICATED'|'DELTA_REPLICATION_FAILED'|'REPLICATION_FAILED'
                    |'REPLICATION_STOPPING'|'REPLICATION_STOP_FAILED'|'REPLICATION_STOPPED',
                    'replicationStatusMessage': 'string',
                    'latestReplicationTime': datetime(2015, 1, 1),
                    'launchStatus':
                    'READY_FOR_CONFIGURATION'|'CONFIGURATION_IN_PROGRESS'|'CONFIGURATION_INVALID'
                    |'READY_FOR_LAUNCH'|'VALIDATION_IN_PROGRESS'|'LAUNCH_PENDING'|'LAUNCH_IN_PROGRESS'
                    |'LAUNCHED'|'DELTA_LAUNCH_IN_PROGRESS'|'DELTA_LAUNCH_FAILED'|'LAUNCH_FAILED'
    ...
    def create_replication_job(
        ...
        """
        Creates a replication job. The replication job schedules periodic replication runs to replicate your
        ...
        **Request Syntax**
        ...
        :param numberOfRecentAmisToKeep:
          ...
          The maximum number of SMS-created AMIs to retain. The oldest will be deleted once the maximum
          number is reached and a new AMI is created.
        ...
        :param kmsKeyId:
          ...
          * ARN referring to KMS key alias
          ...
          If encrypted is *true* but a KMS key id is not specified, the customer's default KMS key for EBS
          is used.
    ...
    def delete_replication_job(self, replicationJobId: str) -> Dict[str, Any]:
        ...
        After you delete a replication job, there are no further replication runs. AWS deletes the contents
        of the Amazon S3 bucket used to store AWS SMS artifacts. The AMIs created by the replication runs
        are not deleted.
    ...
    def generate_change_set(
        ...
        """
        Generates a target change set for a currently launched stack and writes it to an Amazon S3 object in
        the customer’s Amazon S3 bucket.
    ...
    def generate_template(
        ...
        """
        Generates an Amazon CloudFormation template based on the current launch configuration and writes it
                ...
                'replicationJobList': [
                    {
                        ...
                        'latestAmiId': 'string',
                        'state':
                        'PENDING'|'ACTIVE'|'FAILED'|'DELETING'|'DELETED'|'COMPLETED'|'PAUSED_ON_FAILURE'
                        |'FAILING',
                        'statusMessage': 'string',
                        'description': 'string',
                        'numberOfRecentAmisToKeep': 123,
            ...
            - **replicationJob** *(dict) --*
              ...
              - **kmsKeyId** *(string) --*
                ...
                * ARN referring to KMS key alias
                ...
                If encrypted is *true* but a KMS key id is not specified, the customer's default KMS key for
                EBS is used.
    ...
    def import_server_catalog(self, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        ...
        servers that you want to import.
    ...
    def start_on_demand_replication_run(
        ...
        """
        Starts an on-demand replication run for the specified replication job. This replication run starts
        immediately. This replication run is in addition to the ones already scheduled.
