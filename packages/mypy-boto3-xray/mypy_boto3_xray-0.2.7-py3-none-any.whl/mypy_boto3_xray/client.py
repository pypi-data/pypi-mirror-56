...
class Client(BaseClient):
    ...
    def batch_get_traces(
        ...
        """
        Retrieves a list of traces specified by ID. Each trace is a collection of segment documents that
        originates from a single request. Use ``GetTraceSummaries`` to get a list of trace IDs.
              ...
              - *(dict) --*
                ...
                - **Segments** *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    compiled from documents uploaded with  PutTraceSegments , or an ``inferred`` segment for
                    a downstream service, generated from a subsegment sent by the service that called it.
    ...
    def create_sampling_rule(
        ...
        with  GetSamplingRules , and evaluate each rule in ascending order of *priority* for each request.
        If a rule matches, the service records a trace, borrowing it from the reservoir size. After 10
        seconds, the service reports back to X-Ray with  GetSamplingTargets to get updated versions of each
        in-use rule. The updated rule contains a trace quota that the service can use instead of borrowing
        from the reservoir.
    ...
    def get_service_graph(
        ...
        services. Root services are applications that use the AWS X-Ray SDK. Downstream services can be
    ...
    def get_time_series_service_statistics(
        ...
        **Request Syntax**
        ...
        :param EntitySelectorExpression:
          ...
          service, and edge functions. If no selector expression is specified, edge statistics are returned.
    ...
    def get_trace_summaries(
        ...
        A filter expression can target traced requests that hit specific service nodes or edges, have
        errors, or come from a known user. For example, the following filter expression targets traces that
        ...
        For a full list of indexed fields and keywords that you can use in filter expressions, see `Using
        Filter Expressions <https://docs.aws.amazon.com/xray/latest/devguide/xray-console-filters.html>`__
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextToken** *(string) --*
              ...
              retrieve the next page. The first page contains the most most recent results, closest to the
        ...
        **Request Syntax**
        ...
        :param KeyId:
          ...
          * **ARN** - The full Amazon Resource Name of the key ID or alias. For example,
          ``arn:aws:kms:us-east-2:123456789012:key/ae4aa6d49-a4d8-9df9-a475-4ff6d7898456`` . Use this format
        ...
        :param Type: **[REQUIRED]**
          ...
          The type of encryption. Set to ``KMS`` to use your own key for encryption. Set to ``NONE`` for
          default encryption.
    ...
    def put_trace_segments(
        ...
        """
        Uploads segment documents to AWS X-Ray. The X-Ray SDK generates segment documents and sends them to
        the X-Ray daemon, which uploads them in batches. A segment document can be a completed segment, an
        in-progress segment, or an array of subsegments.

        Segments must include the following fields. For the full segment document schema, see `AWS X-Ray
        Segment Documents
        <https://docs.aws.amazon.com/xray/latest/devguide/xray-api-segmentdocuments.html>`__ in the *AWS
        ...
        * ``in_progress`` - Set to ``true`` instead of specifying an ``end_time`` to record that a segment
        has been started, but is not complete. Send an in progress segment when your application receives a
        request that will take a long time to serve, to trace the fact that the request was received. When
        the response is sent, send the complete segment to overwrite the in-progress segment.
