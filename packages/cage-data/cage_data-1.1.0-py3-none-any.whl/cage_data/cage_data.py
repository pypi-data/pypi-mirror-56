...
class cage_data:
    ...
    def create(self, path, nev_mat_file, rhd_file, 
               empty_channels = [], bad_EMG = [],
               do_notch = 1, has_analog = 0):
        """
        ...
        if path[-1] == '/':
            ...
            if self.has_EMG == 1:
                self.rhd_file = ''.join((path, '/', rhd_file))
        self.parse_nev_mat_file(self.nev_mat_file, empty_channels, has_analog)
        if self.has_EMG == 1:
            try:
    ...
    def parse_nev_mat_file(self, filename, empty_channels, has_analog):
        """
