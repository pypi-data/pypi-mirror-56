...
class PTAB2ProceedingResourceTestCase(TestCase):
    def setUp(self):
        ...
        self.inst_company = Company()
        self.inst = PTAB2Proceeding()

        self.company1 = self.inst_company.create({
        ...
        self.company2 = self.inst_company.create({
            'applicant_full_name': 'YEDA RESEARCH CO. LTD.',
            'exchange': 'NYSE',
            'parent_company': 'Teva Pharmaceuticals',
            'ticker': 'TEVA',
        })
        ...
        self.ptab2_proceeding2 = self.inst.create({
            ...
            'petitioner_company_id': self.company2['id'],
        })
    ...
    def test_upsert_with_empty_company_id(self):
        data = {
            **self.valid_data,
            **{'company_id': None},
        }
        ...
        resp = self.inst.upsert(data)
        self.assertEqual(3, len(self.inst.read({})))
        self.assertIsNone(resp['company_id'])

    @parameterized.expand([
        ('company_id',),
        ('petitioner_company_id',),
    ])
    def test_upsert_violates_fk_for_invalid_company_id(self, field):
        invalid_company_id = 912839123
        data = {
            **self.valid_data,
            **{field: invalid_company_id},
        }
        self.assertRaises(
            IntegrityError,
            self.inst.upsert,
    ...
    def test_upsert_with_valid_company_id(self):
        data = {
            **self.valid_data,
            **{'company_id': self.company1['id'],
               'petitioner_company_id': self.company2['id'],
        ...
        resp = self.inst.upsert(data)
        self.assertEqual(self.company1['id'], resp['company_id'])
