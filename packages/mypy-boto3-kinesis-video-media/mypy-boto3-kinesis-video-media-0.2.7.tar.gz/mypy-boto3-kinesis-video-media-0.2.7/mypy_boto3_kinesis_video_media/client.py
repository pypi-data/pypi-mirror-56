...
class Client(BaseClient):
    ...
    def get_media(
        ...
        returns a stream of chunks in order by fragment number.
        ...
        When you put media data (fragments) on a stream, Kinesis Video Streams stores each incoming fragment
        and related metadata in what is called a "chunk." For more information, see `PutMedia
        <https://docs.aws.amazon.com/kinesisvideostreams/latest/dg/API_dataplane_PutMedia.html>`__ . The
        ...
        * Kinesis Video Streams sends media data at a rate of up to 25 megabytes per second (or 200 megabits
        ...
        .. note::
          ...
          If an error is thrown after invoking a Kinesis Video Streams media API, in addition to the HTTP
        ...
        **Request Syntax**
        ::
          ...
          response = client.get_media(
              ...
              StartSelector={
                  'StartSelectorType':
                  'FRAGMENT_NUMBER'|'SERVER_TIMESTAMP'|'PRODUCER_TIMESTAMP'|'NOW'|'EARLIEST'
                  |'CONTINUATION_TOKEN',
        ...
        :param StreamName:
          ...
          The Kinesis video stream name from where you want to get the media content. If you don't specify
          the ``streamName`` , you must specify the ``streamARN`` .
