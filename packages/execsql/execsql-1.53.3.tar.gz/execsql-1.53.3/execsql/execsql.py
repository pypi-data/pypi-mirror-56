...
def exception_info():
	...
	try:
		...
		# UnicodeDecode error.  'repr()' will escape bad characters, but also newlines.
			...
			if tqd:
				...
				if sys.version_info < (3,):
					...
					if paramlist is None:
						...
						curs.execute(encoded_sql.decode(self.encoding), paramlist)
				subvars.add_substitution("$LAST_ROWCOUNT", curs.rowcount)
...
class SqlServerDatabase(Database):
	...
	def populate_table(self, schema_name, table_name, rowsource, column_list, tablespec_src):
		...
		for line in rowsource:
			...
			add_line = True
			if not conf.empty_rows:
				add_line = not all([c is None for c in lt])
			if add_line:
				try:
...
class OracleDatabase(Database):
	...
	def column_exists(self, table_name, column_name, schema_name=None):
		...
		#sql = "select column_name from all_tab_columns where table_name='%s' and column_name='%s'" % (table_name, column_name)
		try:
			curs.execute(sql)
		except ErrInfo:
			raise
	...
	def view_exists(self, view_name, schema_name=None):
		...
		#sql = "select view_name from sys.all_views where view_name = '%s'" % view_name
		try:
			curs.execute(sql)
		except ErrInfo:
			raise
...
class CondParser(CondTokens, object):
	...
	def match_any_metacommand(self, metacmd_list):
		...
		# dictionary of all named grous and their values.
...
metacommands.append(MetaCommand(ins_table_rxs(r'^\s*EXPORT\s+', ins_fn_rxs(r'\s+(?P<tee>TEE\s+)?(?P<append>APPEND\s+)?TO\s+', r'\s+AS\s+(?P<format>CSV|TAB|TSV|TABQ|TSVQ|UNITSEP|US|TXT|TXT-ND|PLAIN|ODS|JSON|XML|VALUES|HTML|LATEX|RAW|B64|FEATHER)(?:\s+DESCRIP(?:TION)?\s+"(?P<description>[^"]*)")?\s*$')),
...
def x_logwritemessages(**kwargs):
	...
	conf.tee_write_log = setting in ('yes', 'on', 'true')

metacommands.append(MetaCommand(r'^\s*LOG_WRITE_MESSAGES\s+(?P<setting>Yes|No|On|Off|True|False)\s*$', x_logwritemessages))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+LOG_WRITE_MESSAGES\s+(?P<setting>Yes|No|On|Off|True|False)\s*$', x_logwritemessages))
...
def x_make_export_dirs(**kwargs):
	global conf
	setting = kwargs['setting'].lower()
	conf.make_export_dirs = setting in ('yes', 'on', 'true')

metacommands.append(MetaCommand(r'^\s*MAKE_EXPORT_DIRS\s+(?P<setting>Yes|No|On|Off|True|False)\s*$', x_make_export_dirs))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+MAKE_EXPORT_DIRS\s+(?P<setting>Yes|No|On|Off|True|False)\s*$', x_make_export_dirs))
...
def x_quote_all_text(**kwargs):
	global conf
	setting = kwargs['setting'].lower()
	conf.quote_all_text = setting in ('yes', 'on', 'true')

metacommands.append(MetaCommand(r'^\s*CONFIG\s+QUOTE_ALL_TEXT\s+(?P<setting>Yes|No|On|Off|True|False)\s*$', x_quote_all_text))
...
def x_connect_ora(**kwargs):
	...
	enc = kwargs['encoding']
	if enc:
		new_db = OracleDatabase(kwargs['server'], kwargs['db_name'], kwargs['user'], need_passwd=need_pwd,
				port=portno, encoding=enc, password=pw)
	else:
		new_db = OracleDatabase(kwargs['server'], kwargs['db_name'], kwargs['user'], need_passwd=need_pwd,
				port=portno, password=pw)
...
metacommands.append(MetaCommand(r'^CONNECT\s+TO\s+ORACLE\s*\(\s*SERVER\s*=\s*(?P<server>[A-Z0-9][A-Z0-9_\-\.]*)\s*,\s*DB\s*=\s*(?P<db_name>[A-Z][A-Z0-9_\-]*)(?:\s*,\s*USER\s*=\s*(?P<user>[A-Z][A-Z0-9_\-@\.]*)\s*,\s*NEED_PWD\s*=\s*(?P<need_pwd>TRUE|FALSE))?(?:\s*,\s*PORT\s*=\s*(?P<port>\d+))?(?:\s*,\s+PASSWORD\s*=\s*(?P<password>[^\s\)]+))?(?:\s*,\s*ENCODING\s*=\s*(?P<encoding>[A-Z][A-Z0-9_-]+))?\s*\)\s+AS\s+(?P<db_alias>[A-Z][A-Z0-9_]*)\s*$', x_connect_ora))
...
metacommands.append(MetaCommand(r'\s*APPEND\s+SCRIPT\s+(?P<script1>\w+)\s+TO\s+(?P<script2>\w+)\s*$', x_appendscript))
...
metacommands.append(MetaCommand(r'^\s*EXEC(?:UTE)?\s+SCRIPT\s+(?P<script_id>\w+)(?:(?:\s+WITH)?(?:\s+ARG(?:UMENT)?S?)?\s*\(\s*(?P<argexp>#?\w+\s*=\s*(?:(?:[^"\'\[][^,\)]*)|(?:"[^"]*")|(?:\'[^\']*\')|(?:\[[^\]]*\]))(?:\s*,\s*#?\w+\s*=\s*(?:(?:[^"\'\[][^,\)]*)|(?:"[^"]*")|(?:\'[^\']*\')|(?:\[[^\]]*\])))*)\s*\))?(?:\s+(?P<looptype>WHILE|UNTIL)\s*\(\s*(?P<loopcond>.+)\s*\))?\s*$', x_executescript))
...
def x_empty_strings(**kwargs):
	...
	if not flag in ('yes', 'no', 'on', 'off', 'true', 'false'):
	...
	conf.empty_strings = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*EMPTY_STRINGS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_empty_strings))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+EMPTY_STRINGS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_empty_strings))
...
def x_empty_rows(**kwargs):
	flag = kwargs['yesno'].lower()
	if not flag in ('yes', 'no', 'on', 'off', 'true', 'false'):
		raise ErrInfo(type="cmd", command_text=kwargs["metacommandline"], other_msg=u"Unrecognized value for EMPTY_ROWS metacommand: %s" % flag)
	conf.empty_rows = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*CONFIG\s+EMPTY_ROWS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_empty_rows))
...
def x_boolean_int(**kwargs):
	...
	conf.boolean_int = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*BOOLEAN_INT\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_boolean_int))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+BOOLEAN_INT\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_boolean_int))
...
metacommands.append(MetaCommand(r'^\s*BOOLEAN_WORDS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_boolean_words))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+BOOLEAN_WORDS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_boolean_words))
...
def x_clean_col_hdrs(**kwargs):
	...
	conf.clean_col_hdrs = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*CLEAN_COLUMN_HEADERS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_clean_col_hdrs))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+CLEAN_COLUMN_HEADERS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_clean_col_hdrs))
...
def x_import_common_cols_only(**kwargs):
	...
	conf.import_common_cols_only = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*IMPORT_COMMON_COLUMNS_ONLY\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_import_common_cols_only))
metacommands.append(MetaCommand(r'^\s*IMPORT_ONLY_COMMON_COLUMNS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_import_common_cols_only))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+IMPORT_COMMON_COLUMNS_ONLY\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_import_common_cols_only))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+IMPORT_ONLY_COMMON_COLUMNS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_import_common_cols_only))
...
def x_write_warnings(**kwargs):
	...
	conf.write_warnings = flag in ('yes', 'on', 'true')
	return None

metacommands.append(MetaCommand(r'^\s*WRITE_WARNINGS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_write_warnings))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+WRITE_WARNINGS\s+(?P<yesno>YES|NO|ON|OFF|TRUE|FALSE)\s*$', x_write_warnings))
...
def x_consolewait_onerror(**kwargs):
	flag = kwargs["onoff"].lower()
	conf.gui_wait_on_error_halt = flag in ('on', 'yes', 'true')

metacommands.append(MetaCommand(r'^\s*CONSOLE\s+WAIT_WHEN_ERROR\s+(?P<onoff>ON|OFF|YES|NO|TRUE|FALSE)\s*$', x_consolewait_onerror))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+CONSOLE\s+WAIT_WHEN_ERROR\s+(?P<onoff>ON|OFF|YES|NO|TRUE|FALSE)\s*$', x_consolewait_onerror))


#****	CONSOLE WAIT_WHEN_DONE
def x_consolewait_whendone(**kwargs):
	flag = kwargs["onoff"].lower()
	conf.gui_wait_on_exit = flag in ('on', 'yes', 'true')

metacommands.append(MetaCommand(r'^\s*CONSOLE\s+WAIT_WHEN_DONE\s+(?P<onoff>ON|OFF|YES|NO|TRUE|FALSE)\s*$', x_consolewait_whendone))
metacommands.append(MetaCommand(r'^\s*CONFIG\s+CONSOLE\s+WAIT_WHEN_DONE\s+(?P<onoff>ON|OFF|YES|NO|TRUE|FALSE)\s*$', x_consolewait_whendone))
...
def substitute_vars(command_str, localvars = None):
    ...
    # After completing subs, looks for any deferred-variable bracketing flag "!{  }!"
    # and replaces with "regular" sub variable flags "!! !!"
	...
	# Define pattern for deferred substitution "!{somevar}!"
	defer_rx = re.compile(r'(!{([$@&~#]?[a-z0-9_]+)}!)', re.I)
	m = defer_rx.findall(cmdstr)
    # Substitute any deferred substitution variables with regular substition var flags "!!somevar!!"
	if m is not None:
		for dv in m:
			rep = "!!" +  dv[1] + "!!"
