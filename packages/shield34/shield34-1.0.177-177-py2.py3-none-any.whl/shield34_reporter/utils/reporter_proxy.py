            ...
            if block_run_report_container.proxyServer is None:
                ...
                if os.path.exists(proxy_file_path):
                    try:
                        ...
                        browser_mob_server.command += ['--address=127.0.0.1',"--ttl=3600"]
                ...
                if chained_proxy is not None:
                    ...
                    Shield34Listener.logger.console("Creating Shield34 Proxy Server sub-instance on port {} through system proxy {}".format(proxy_server_port,chained_proxy))
                else:
                    proxy = browser_mob_server.create_proxy(
                        params={"trustAllServers": "true", "port": sub_instance_port,
                                "bindAddress": "127.0.0.1"})
                    Shield34Listener.logger.console("Creating Shield34 Proxy Server sub-instance on port {}".format(proxy_server_port))
