...
class BusinessMediaFetcher:
  ...
  def __init__(self):
    try:
      ...
      self.logger.exception('Error initializing BusinessMedia')
      

  ### Utility to build fetching url
  def build_url(self, type, user_id, access_token, date_from, date_to):
    ...
    self.logger.debug('built url %s%s%s'%(host,path,historic))
  ...
  def parse_and_insert(self, conn, user_id, data, is_story=False):
    ...
    cursor = conn.cursor() 
    self.logger.debug("insert_user cursor created")
    cursor.execute(command)
    self.logger.debug("insert_user cursor executed")
    conn.commit()
    self.logger.debug("insert_user conn commited")
    cursor.close()
    self.logger.debug("insert_user cursor closed")
    
    cursor = conn.cursor()
    self.logger.debug("insert_media cursor created")
    for item in data:
...
#   r = requests.post(url, body)
  ...
  ### Main function
  def process_business_media(self, conn, user_id, access_token, date_from_param = None, date_to_param = None):
    try:
        ...
        print('BusinessMediaFetcher process_business_media dates :')
        print('%s - %s'%(str(date_from), str(date_to)))
        print('BusinessMediaFetcher process_business_media logger:')
        print(self.logger)
        self.logger.info("##############################################")
        self.logger.info("Getting media for %s" % user_id)      
        paginate = True
        url = self.build_url('media',user_id,access_token,date_from,date_to)
        has_data = False
        while paginate:
        ...
        url = self.build_url('stories',user_id,access_token,date_from,date_to)
        response = self.utils.request_data(url, access_token)
        if response and response.get('data'):
          has_data = True
