...
class BusinessMediaFetcher:
  ...
  def init(self, logger, conf_file, dates_arr):
  ...
  def get_users(self, conn):
    users = list()
    print('BusinessMediaFetcher get_users logger')
    print(self.logger)
    try:
...
def main_script(args):
  ...
  utils_logger = logging.getLogger('shared')
  utils_logger.setLevel(logging.INFO)

  conf_file = "conf/get_business_media.yaml"
  fetcher = BusinessMediaFetcher()
  fetcher.init(root_logger, conf_file, args)

def main_imported(args):  
  ch = logging.StreamHandler()
  root_logger = logging.getLogger()
  root_logger.setLevel(logging.DEBUG)
  root_logger.addHandler(ch)
  ...
  print('res_package %s'%resource_package)
  print('res_path %s'%resource_path)
  conf_file = pkg_resources.resource_stream(resource_package, resource_path)
  print('conf_file')
  print(conf_file)
  #conf_file = pkg_resources.resource_stream(__name__, 'conf/get_business_media.yaml')
  fetcher = BusinessMediaFetcher()
  fetcher.init(root_logger, conf_file, args)

  
if __name__ == '__main__':
  print('program running as script %s; setting stream+file handlers'%__name__)
  main_script(sys.argv[1:])
else:
  print('program running as import %s; setting stream handler'%__name__)
  main_imported(sys.argv[1:])
