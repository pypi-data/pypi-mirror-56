...
class CompetitorsMediaFetcher:
  ...
  def __init__(self):
    try:
      ...
      self.logger.debug("CompetitorsMediaFetcher conf_file: %s" % conf_file)
      self.logger.debug("CompetitorsMediaFetcher queries: %s" % self.utils.from_config('queries'))
    except:
  ...
  def process_all(self, dates_arr):
    try:    
      ...
      for item in users:
        ...
        access_token = item[2]
        self.process_competitors_media(conn, user_id, competitor_name, access_token, date_from, date_to)
      conn.close()
    except:
  ...
  def request_data(self, user_id, competitor_name, access_token, date_from, date_to):
  ...
  def parse_and_insert(self, conn, base_user_id, competitor_name, data):
    ...
    self.logger.debug("update_competitor query: %s" % self.utils.from_config('queries.update_competitor'))
    comp_values = {"instagram_competitor_id": competitor_id,
                   "instagram_id": base_user_id,
                   "competitor_name": competitor_name}
    self.logger.debug("update_competitor values: %s" % str(comp_values))
...
#   r = requests.post(url, body)
  ...
  ### Main function
  def process_competitors_media(self, conn, user_id, competitor, access_token, date_from_param = None, date_to_param = None):
    try:
      ...
      response = self.request_data(user_id,competitor,access_token,date_from,date_to)
      if response:
