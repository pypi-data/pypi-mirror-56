...
class Client(BaseClient):
    ...
    def batch_delete_attributes(
        ...
        **Request Syntax**
        ...
        :param DomainName: **[REQUIRED]** The name of the domain in which the attributes are being
        deleted.
    ...
    def batch_put_attributes(
        ...
        """
        The ``BatchPutAttributes`` operation creates or replaces attributes within one or more
        items. By using this operation, the client can perform multiple  PutAttribute operation with
        a single call. This helps yield savings in round trips and latencies, enabling Amazon
        SimpleDB to optimize requests and generally produce better throughput.

        The client may specify the item name with the ``Item.X.ItemName`` parameter. The client may
        specify new attributes using a combination of the ``Item.X.Attribute.Y.Name`` and
        ``Item.X.Attribute.Y.Value`` parameters. The client may specify the first attribute for the
        first item using the parameters ``Item.0.Attribute.0.Name`` and ``Item.0.Attribute.0.Value``
        ...
        Attributes are uniquely identified within an item by their name/value combination. For
        ...
        attribute values. For example, if an item ``I`` has the attributes ``{ 'a', '1' }, { 'b',
        '2'}`` and ``{ 'b', '3' }`` and the requester does a BatchPutAttributes of ``{'I', 'b', '4'
        }`` with the Replace parameter set to true, the final attributes of the item will be ``{
        'a', '1' }`` and ``{ 'b', '4' }`` , replacing the previous values of the 'b' attribute with
        the new value.

        .. warning::

          This operation is vulnerable to exceeding the maximum URL size when making a REST request
          using the HTTP GET method. This operation does not support conditions using
        ...
        **Request Syntax**
        ...
        :param Items: **[REQUIRED]** A list of items on which to perform the operation.
          ...
          - *(dict) --*
            ...
            - **Attributes** *(list) --* **[REQUIRED]** The list of attributes for a replaceable
            item.
    ...
    def create_domain(self, DomainName: str) -> None:
        ...
        **Request Syntax**
        ...
        :type DomainName: string
        :param DomainName: **[REQUIRED]** The name of the domain to create. The name can range
    ...
    def delete_attributes(
        ...
        DomainName: str,
        ItemName: str,
        Attributes: List[ClientDeleteAttributesAttributesTypeDef] = None,
        Expected: ClientDeleteAttributesExpectedTypeDef = None,
    ) -> None:
        """
        ...
        update model, performing a  GetAttributes or  Select operation (read) immediately after a
        ...
        **Request Syntax**
        ...
        :param ItemName: **[REQUIRED]** The name of the item. Similar to rows on a spreadsheet,
        ...
        request to be processed and the attributes to be deleted.
          ...
          - **Exists** *(boolean) --*
            ...
            value in order for the update condition to be satisfied. Specify ``true`` if the
            attribute must exist for the update condition to be satisfied. Specify ``false`` if the
            attribute should not exist in order for the update condition to be satisfied.
    ...
    def domain_metadata(self, DomainName: str) -> ClientDomainMetadataResponseTypeDef:
        ...
        **Request Syntax**
        ...
        :type DomainName: string
        :param DomainName: **[REQUIRED]** The name of the domain for which to display the metadata
        of.
        ...
        :returns:
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ItemNamesSizeBytes** *(integer) --* The total size of all item names in the domain,
            in bytes.

            - **AttributeNameCount** *(integer) --* The number of unique attribute names in the
            domain.

            - **AttributeNamesSizeBytes** *(integer) --* The total size of all unique attribute
            names in the domain, in bytes.

            - **AttributeValueCount** *(integer) --* The number of all attribute name/value pairs in
            the domain.

            - **AttributeValuesSizeBytes** *(integer) --* The total size of all attribute values in
            the domain, in bytes.

            - **Timestamp** *(integer) --* The data and time when metadata was calculated, in Epoch
            (UNIX) seconds.
        """
    ...
    def list_domains(
        ...
        returns domain names up to the limit set by `MaxNumberOfDomains <#MaxNumberOfDomains>`__ . A
        `NextToken <#NextToken>`__ is returned if there are more than ``MaxNumberOfDomains``
        domains. Calling ``ListDomains`` successive times with the ``NextToken`` provided by the
        ...
        **Request Syntax**
        ...
        :param MaxNumberOfDomains: The maximum number of domain names you want returned. The range
        is 1 to 100. The default setting is 100.

        :type NextToken: string
        :param NextToken: A string informing Amazon SimpleDB where to start the next list of domain
        names.
        ...
        :returns:
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextToken** *(string) --* An opaque token indicating that there are more domains
            than the specified ``MaxNumberOfDomains`` still available.
        """
    ...
    def put_attributes(
        ...
        Expected: ClientPutAttributesExpectedTypeDef = None,
    ) -> None:
        """
        ...
        attribute. Setting this value to ``true`` causes the new attribute value to replace the
        existing attribute value(s). For example, if an item has the attributes ``{ 'a', '1' }`` ,
        ``{ 'b', '2'}`` and ``{ 'b', '3' }`` and the requestor calls ``PutAttributes`` using the
        attributes ``{ 'b', '4' }`` with the ``Replace`` parameter set to true, the final attributes
        of the item are changed to ``{ 'a', '1' }`` and ``{ 'b', '4' }`` , which replaces the
        ...
        consistency update model, an immediate  GetAttributes or  Select operation (read)
    ...
    def select(
        ...
        example, if the client asks to retrieve 2500 items, but each individual item is 10 kB in
        size, the system returns 100 items and an appropriate ``NextToken`` so the client can access
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/sdb-2009-04-15/Select>`_
        ...
        :returns:
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            ``MaxNumberOfItems`` were matched, the response size exceeded 1 megabyte, or the
    ...
    @overload
    # pylint: disable=arguments-differ,redefined-outer-name,redefined-builtin
    def get_paginator(self, operation_name: Literal["select"]) -> paginator_scope.SelectPaginator:
        """
