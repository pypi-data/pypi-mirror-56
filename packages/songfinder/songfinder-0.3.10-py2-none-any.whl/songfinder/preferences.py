...
class ParamGen(object):
	def __init__(self, papa, screens=None, **kwargs):
		fenetre = tk.Toplevel()
		with guiHelper.SmoothWindowCreation(fenetre, screens=screens):
			...
			for i, (parameter, config) in enumerate(self._paramDict.items()):
				intVar = tk.IntVar()
				button = tk.Checkbutton(fenetre, text=config['text'], variable=intVar, \
										command = lambda intVar=intVar, parameter=parameter: \
										self.save_setting(intVar, parameter))
				config['button'] = button
				button.grid(row=i, column=0, columnspan=5, sticky='w' )

			self.mot_data.grid(row=len(self._paramDict), column=0, columnspan=2, sticky='w' )
			self.data_str.grid(row=len(self._paramDict), column=2, columnspan=7, sticky='w' )
			self.data_button.grid(row=len(self._paramDict), column=9, columnspan=1, sticky='w' )
			self.reset_button.grid(row=len(self._paramDict)+1, column=0, columnspan=2)
...
class ParamPres(object):
	...
	def select_profil(self, event):
		if self.combobox_profil.get():
			...
			shutil.copy( os.path.join(globalvar.settingsPath, \
								self.settingName + '_' + self.profil), \
								os.path.join(globalvar.settingsPath, \
								self.settingName) )
			globalvar.sett.read()
			self.maj()
	...
	def start_profil(self):
		for filename in os.listdir(globalvar.settingsPath):
			if filename.find(self.settingName + '_') != -1 and \
				filecmp.cmp( os.path.join(globalvar.settingsPath, filename), \
									os.path.join(globalvar.settingsPath, \
									self.settingName) ):
	...
	def get_profil_liste(self):
		...
		self.liste_profil = []
		for _, _, files in os.walk(globalvar.settingsPath):
