...
class ExportBase(object):
	def __init__(self, element, titleLevel, **kwargs):
	...
	def nbLine(self):
		if self._nbLignes == -1:
			self.exportText # pylint: disable=pointless-statement
	...
	def _processNewLine(self, text):
		if settings.LATEXSETTINGS.get('Latex_Parameters', 'saut_lignes') == 'oui':
	...
	def _getDiapos(self):
		...
		if settings.LATEXSETTINGS.get('Latex_Parameters', 'one_chorus') == 'oui':
...
class ExportLatex(ExportBase):
	def __init__(self, element, titleLevel=1, **kwargs):
	...
	def exportText(self):
		...
		# Capo
		if settings.LATEXSETTINGS.get('Latex_Parameters', 'capo') == 'oui' and self.capo:
		...
		# Song per page
		if settings.LATEXSETTINGS.get('Latex_Parameters', 'one_song_per_page') == 'oui':
			text = '%s\n\\clearpage'%text
...
class ExportMarkdown(ExportBase):
	...
	def exportText(self):
		...
		# Capo
		if settings.LATEXSETTINGS.get('Latex_Parameters', 'capo') == 'oui' \
														and self.capo:
...
class ExportHtml(ExportBase):
	def __init__(self, element, titleLevel=1, markdowner=None, htmlStyle=None, htmlStylePath=None, **kwargs):
		ExportBase.__init__(self, element, titleLevel)
		...
		if not htmlStylePath:
			htmlStylePath = os.path.join(globalvar.dataPath, 'htmlTemplates', 'defaultStyle.html')
