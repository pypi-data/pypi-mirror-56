...
cdef unsigned long long fast_document_dbow_neg(
    ...
    unsigned long long next_random, int learn_context, int learn_hidden, REAL_t *context_locks, long double score) nogil:
...
def train_document_dbow(model, doc_words, word_scores, doctag_indexes, alpha, work=None,
    ...
    i = 0
    for token, score in zip(doc_words, word_scores):
                ...
                for j in range(j, k):
                    ...
                    if c.negative:
                        ...
                        c.next_random = fast_document_dbow_neg(c.negative, c.cum_table, c.cum_table_len, c.word_vectors,
                                                               ...
                                                               c.learn_hidden, c.word_locks, c.word_scores[i])
            ...
            for j in range(c.doctag_len):
                ...
                if c.negative:
                    c.next_random = fast_document_dbow_neg(c.negative, c.cum_table, c.cum_table_len, c.doctag_vectors,
                                                           ...
                                                           c.learn_hidden, c.doctag_locks, c.word_scores[i])
