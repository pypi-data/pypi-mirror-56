        ...
        for resource in resources:
            if 'ValueSet' == resource['resourceType']:
                ...
                if 'content' in resource and 'concept' in resource:
                    ...
                    logger.warning("CodeSystem with no concepts: {}".format(resource['url']))
...
class FHIRValueSet(object):
    ...
    def enum(self):
        ...
        if 1 != len(include):
            logger.warning("Ignoring ValueSet with more than 1 includes ({}: {})".format(len(include), include))
            return None
...
class FHIRElementType(object):
    ...
    def parse_from(self, type_dict):
        ...
        if self.code is None and ext_code is not None:
            json_ext = [e for e in ext_code.get('extension', []) if e.get('url') == 'http://hl7.org/fhir/StructureDefinition/structuredefinition-fhir-type']
            if len(json_ext) < 1:
