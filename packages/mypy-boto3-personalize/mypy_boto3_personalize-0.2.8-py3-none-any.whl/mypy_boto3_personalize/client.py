...
class Client(BaseClient):
    ...
    def create_batch_inference_job(
        ...
        **Request Syntax**
        ...
        :param roleArn: **[REQUIRED]**
          ...
          The ARN of the Amazon Identity and Access Management role that has permissions to read and
          write to your input and out Amazon S3 buckets respectively.
    ...
    def create_campaign(
        ...
        <https://docs.aws.amazon.com/personalize/latest/dg/API_RS_GetPersonalizedRanking.html>`__
        ...
        The minimum provisioned TPS (``minProvisionedTPS`` ) specifies the baseline throughput
        provisioned by Amazon Personalize, and thus, the minimum billing charge. If your TPS
        increases beyond ``minProvisionedTPS`` , Amazon Personalize auto-scales the provisioned
        capacity up and down, but never below ``minProvisionedTPS`` , to maintain a 70% utilization.
        There's a short time delay while the capacity is increased that might cause loss of
        transactions. It's recommended to start with a low ``minProvisionedTPS`` , track your usage
        using Amazon CloudWatch metrics, and then increase the ``minProvisionedTPS`` as necessary.
    ...
    def create_dataset(
        ...
        * Users
        ...
        ``Interactions`` dataset is required in order to train a model (also referred to as creating
        a solution).
    ...
    def create_dataset_group(
        ...
        * DELETE PENDING
        ...
        To get the status of the dataset group, call  DescribeDatasetGroup . If the status shows as
        CREATE FAILED, the response includes a ``failureReason`` key, which describes why the
        creation failed.
        ...
        You can specify an AWS Key Management Service (KMS) key to encrypt the datasets in the
        group. If you specify a KMS key, you must also include an AWS Identity and Access Management
        ...
        **Request Syntax**
        ...
        :param roleArn:
          ...
          The ARN of the IAM role that has permissions to access the KMS key. Supplying an IAM role
          is only valid when also specifying a KMS key.
    ...
    def create_dataset_import_job(
        ...
        * CREATE PENDING > CREATE IN_PROGRESS > ACTIVE -or- CREATE FAILED
        ...
        To get the status of the import job, call  DescribeDatasetImportJob , providing the Amazon
        Resource Name (ARN) of the dataset import job. The dataset import is complete when the
    ...
    def create_event_tracker(
        ...
        """
        Creates an event tracker that you use when sending event data to the specified dataset group
        using the `PutEvents
        ...
        dataset stores the event data from the ``PutEvents`` call. The contents of this dataset are
        ...
        When you send event data you include your tracking ID. The tracking ID identifies the
        customer and authorizes the customer to send the data.
    ...
    def create_schema(self, name: str, schema: str) -> ClientCreateSchemaResponseTypeDef:
        ...
        dataset type and has a set of required field and keywords. You specify a schema when you
        call  CreateDataset .
    ...
    def create_solution(
        ...
        CreateSolutionVersion operation. Every time you call ``CreateSolutionVersion`` , a new
        version of the solution is created.

        After creating a solution version, you check its accuracy by calling  GetSolutionMetrics .
        When you are satisfied with the version, you deploy it using  CreateCampaign . The campaign
        ...
        comes from the dataset group that you provide in the request. A recipe specifies the
        training algorithm and a feature transformation. You can specify one of the predefined
        recipes provided by Amazon Personalize. Alternatively, you can specify ``performAutoML`` and
        Amazon Personalize will analyze your data and select the optimum USER_PERSONALIZATION recipe
        for you.
        ...
        * DELETE PENDING > DELETE IN_PROGRESS
        ...
        To get the status of the solution, call  DescribeSolution . Wait until the status shows as
        ACTIVE before calling ``CreateSolutionVersion`` .
        ...
        **Request Syntax**
        ...
        :param recipeArn:
          ...
          The ARN of the recipe to use for model training. Only specified when ``performAutoML`` is
          false.
    ...
    def create_solution_version(
        ...
        * CREATE PENDING > CREATE IN_PROGRESS > ACTIVE -or- CREATE FAILED
        ...
        To get the status of the version, call  DescribeSolutionVersion . Wait until the status
        shows as ACTIVE before calling ``CreateCampaign`` .
        ...
        **Request Syntax**
        ...
        :param trainingMode:
          ...
          data, while the ``UPDATE`` option processes only the data that has changed in comparison
          to the input solution. Choose ``UPDATE`` when you want to incrementally update your
          ...
          .. warning::
            ...
            trained with the  native-recipe-hrnn-coldstart recipe.
    ...
    def delete_campaign(self, campaignArn: str) -> None:
        ...
        be specified in a `GetRecommendations
        <https://docs.aws.amazon.com/personalize/latest/dg/API_RS_GetRecommendations.html>`__
          ...
          - *(dict) --*
            ...
            - **algorithm** *(dict) --*
              ...
              - **defaultHyperParameterRanges** *(dict) --*
                ...
                tunable hyperparameter can have its value determined during hyperparameter
                optimization (HPO).
              ...
              * CREATE FAILED
              ...
              - **dataSource** *(dict) --*
                ...
                - **dataLocation** *(string) --*
                  ...
                  dataset is stored. For example:

                   ``s3://bucket-name/training-data.csv``

              - **roleArn** *(string) --*

                The ARN of the AWS Identity and Access Management (IAM) role that has permissions to
    ...
    def describe_recipe(self, recipeArn: str) -> ClientDescribeRecipeResponseTypeDef:
        ...
        * Feature transformation information for modifying the input data before training.
        ...
        create a solution with the  CreateSolution API. ``CreateSolution`` trains a model by using
        the algorithm in the specified recipe and a training dataset. The solution, when deployed as
          ...
          - *(dict) --*
            ...
            - **solution** *(dict) --*
              ...
              - **performAutoML** *(boolean) --*
                ...
                be specified). When false (the default), Amazon Personalize uses ``recipeArn`` for
                training.
          ...
          - *(dict) --*
            ...
            - **solutionVersion** *(dict) --*
              ...
              - **performAutoML** *(boolean) --*
                ...
                solution configuration. When false (the default), Amazon Personalize uses
                ``recipeArn`` .
              ...
              - **trainingHours** *(float) --*
                ...
                model. This field is visible only after Amazon Personalize successfully trains a
                model.
              ...
              - **trainingMode** *(string) --*
                ...
                data, while the ``UPDATE`` option processes only the training data that has changed
                since the creation of the last solution version. Choose ``UPDATE`` when you want to
                start recommending items added to the dataset without retraining the model.

                .. warning::

                  The ``UPDATE`` option can only be used after you've created a solution version
                  with the ``FULL`` option and the training solution uses the
          ...
          - *(dict) --*
            ...
            - **eventTrackers** *(list) --*
              ...
              - *(dict) --*
                ...
                call the  DescribeEventTracker API.
          ...
          - *(dict) --*
            ...
            - **solutionVersions** *(list) --*
              ...
              - *(dict) --*
                ...
                call the  DescribeSolutionVersion API.
    ...
    def update_campaign(
        ...
        campaign status using the  DescribeCampaign API.
