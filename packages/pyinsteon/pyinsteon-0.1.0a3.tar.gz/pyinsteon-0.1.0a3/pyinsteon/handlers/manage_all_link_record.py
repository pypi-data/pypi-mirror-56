...
class ManageAllLinkRecordCommand(OutboundHandlerBase):
    ...
    async def async_send(self, action: ManageAllLinkRecordAction,
                         controller: bool, group: int, target: Address,
                         data1: int = 0, data2: int = 0, data3: int = 0,
                         in_use: bool = True, high_water_mark: bool = False,
                         bit5: bool = True, bit4: bool = False):
        """Send Manage All-Link Record command."""
        flags = create(in_use=in_use, controller=controller, hwm=high_water_mark,
                       bit5=bit5, bit4=bit4)
        return await super().async_send(action=action, flags=flags, group=group, target=target,
