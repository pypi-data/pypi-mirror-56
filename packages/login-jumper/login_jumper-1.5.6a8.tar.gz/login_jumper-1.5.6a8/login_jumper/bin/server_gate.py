...
from login_jumper.bin.jumper_expect import server_login
...
def login_choice(host=None, action='login'):
    # 登录堡垒机
    child = jumper_login()
    if not child:
    ...
    if action == 'query':
        ...
        if not host:
            logger.error('Login host could not be None')
        login_in(child, host)
