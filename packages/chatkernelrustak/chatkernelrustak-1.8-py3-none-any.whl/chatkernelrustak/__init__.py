...
def check_server(host):
	...
	except (json.decoder.JSONDecodeError,requests.exceptions.ConnectionError,requests.exceptions.MissingSchema):
		return False
	return response["worked"]


def parse_host(host):
	if (("://" in host and "http://" in host) or ("://" in host and "https://" in host)) and (host.startswith("http://") or host.startswith("https://")):
		return True
	return False
...
def get_last_message(host):
	...
	except (requests.exceptions.ConnectionError, json.decoder.JSONDecodeError,requests.exceptions.MissingSchema):
