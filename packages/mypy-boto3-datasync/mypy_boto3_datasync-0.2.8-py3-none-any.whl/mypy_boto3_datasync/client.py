...
class Client(BaseClient):
    ...
    def cancel_task_execution(self, TaskExecutionArn: str) -> Dict[str, Any]:
        ...
        When you cancel a task execution, the transfer of some files are abruptly interrupted. The
        contents of files that are transferred to the destination might be incomplete or
        inconsistent with the source files. However, if you start a new task execution on the same
        task and you allow the task execution to complete, file content on the destination is
        complete and consistent. This applies to other unexpected failures that interrupt a task
        execution. In all of these cases, AWS DataSync successfully complete the transfer when you
    ...
    def create_agent(
        ...
        such as the AWS Region that you want to activate the agent in. You activate the agent in the
        AWS Region where your target locations (in Amazon S3 or Amazon EFS) reside. Your tasks are
        created in this AWS Region.

        You can activate the agent in a VPC (Virtual private Cloud) or provide the agent access to a
        VPC endpoint so you can run tasks without going over the public Internet.

        You can use an agent for more than one location. If a task uses multiple agents, all of them
        need to have status AVAILABLE for the task to run. If you use multiple agents for a source
        location, the status of all the agents must be AVAILABLE for the task to run.

        Agents are automatically updated by AWS on a regular basis, using a mechanism that ensures
        ...
        **Request Syntax**
        ...
        :param ActivationKey: **[REQUIRED]**
          ...
          request with redirects that enable you to get the agent IP address (port 80).
          Alternatively, you can get it from the AWS DataSync console.

          The redirect URL returned in the response provides you the activation key for your agent
          in the query string parameter ``activationKey`` . It might also include other
        ...
        :param Tags:
          ...
          The key-value pair that represents the tag that you want to associate with the agent. The
        ...
        :param VpcEndpointId:
          ...
          VPC endpoint, see `Creating a VPC Endpoint Service Configuration
        ...
        :param SubnetArns:
          ...
          private. When you start a task that is associated with an agent created in a VPC, or one
          that has access to an IP address in a VPC, then the task is also private. In this case,
          DataSync creates four network interfaces for each task in your subnet. For a data transfer
    ...
    def create_location_efs(
        ...
        **Request Syntax**
        ...
        :param Subdirectory:
          ...
          A subdirectory in the location’s path. This subdirectory in the EFS file system is used to
          read data from the EFS source location or write data to the EFS destination. By default,
        ...
        :param Ec2Config: **[REQUIRED]**
          ...
          * Security group M (which you associate with the mount target) must allow inbound access
          for the Transmission Control Protocol (TCP) on the NFS port (2049) from security group S.
          ...
          * Security group S (provided to DataSync to access EFS) should have a rule that enables
          outbound connections to the NFS port on one of the file system’s mount targets. You can
          enable outbound connections either by IP address (CIDR range) or security group. For
          information about security groups and mount targets, see Security Groups for Amazon EC2
          Instances and Mount Targets in the *Amazon EFS User Guide.*
    ...
    def create_location_nfs(
        ...
        **Request Syntax**
        ...
        :param Subdirectory: **[REQUIRED]**
          ...
          exported by the NFS server, or a subdirectory of that path. The path should be such that
          ...
          appears in the results, and any subdirectory of that directory. Ensure that the NFS export
          ...
          to read all the data. To ensure this, either configure the NFS export with
          ``no_root_squash,`` or ensure that the permissions for all of the files that you want
          DataSync allow read access for all users. Doing either enables the agent to read the
        ...
        :param ServerHostname: **[REQUIRED]**
          ...
          The name of the NFS server. This value is the IP address or Domain Name Service (DNS) name
          of the NFS server. An agent that is installed on-premises uses this host name to mount the
        ...
        :param OnPremConfig: **[REQUIRED]**
          ...
          Contains a list of Amazon Resource Names (ARNs) of agents that are used to connect to an
          NFS server.
        ...
        :param MountOptions:
          ...
          - **Version** *(string) --*
            ...
            * **`NFSv4.1 <https://tools.ietf.org/html/rfc5661>`__ ** - stateful protocol version
            that supports sessions, directory delegations, and parallel data processing. Version 4.1
    ...
    def create_location_s3(
        ...
        **Request Syntax**
        ...
        :param S3StorageClass:
          ...
          used as a task destination. For more information about S3 storage classes, see `Amazon S3
          Storage Classes <https://aws.amazon.com/s3/storage-classes/>`__ in the *Amazon Simple
          Storage Service Developer Guide* . Some storage classes have behaviors that can affect
    ...
    def create_location_smb(
        ...
        """
        Defines a file system on an Server Message Block (SMB) server that can be read from or
        written to.
        ...
        **Request Syntax**
        ...
        :param Subdirectory: **[REQUIRED]**
          ...
          exported by the SMB server, or a subdirectory of that path. The path should be such that
          it can be mounted by other SMB clients in your network.
          ...
          share, and who has the appropriate permissions for all of the files and directories that
          you want DataSync to access, or use credentials of a member of the Backup Operators group
          to mount the share. Doing either enables the agent to access the data. For the agent to
        ...
        :param ServerHostname: **[REQUIRED]**
          ...
          of the SMB server. An agent that is installed on-premises uses this hostname to mount the
          SMB server in a network.
    ...
    def create_task(
        ...
        you create a task, AWS DataSync populates them with service defaults.
        ...
        to the AVAILABLE state without waiting for the AWS location to become mounted. If required,
        ...
        issues are often caused by either a misconfigured firewall or a mistyped NFS server host
        name.
        ...
        **Request Syntax**
        ...
        :param CloudWatchLogGroupArn:
          ...
          *Amazon CloudWatch User Guide.*
          ...
          Task in the *AWS DataSync User Guide.*
        ...
        :param Options:
          ...
          task that occurs when you call ``StartTaskExecution`` . You can configure these options to
          preserve metadata such as user ID (UID) and group ID (GID), file permissions, data
          ...
          - **OverwriteMode** *(string) --*
            ...
            replaced by a source file, even if the destination file differs from the source file. If
            you modify files in the destination and you sync the files, you can use this value to
          ...
          - **Atime** *(string) --*
            ...
            file was read or written to). If you set ``Atime`` to BEST_EFFORT, DataSync attempts to
            preserve the original ``Atime`` attribute on all source files (that is, the version
            before the PREPARING phase). However, ``Atime`` 's behavior is not fully standard across
          ...
          - **PreserveDeletedFiles** *(string) --*
            ...
            file system should be preserved. This option can affect your storage cost. If your task
            deletes objects, you might incur minimum storage duration charges for certain storage
            classes. For detailed information, see  using-storage-classes in the *AWS DataSync User
            Guide* .
          ...
          - **PreserveDevices** *(string) --*
            ...
            character devices in the source file system, and recreate the files with that device
            name and metadata on the destination.
          ...
          - **TaskQueueing** *(string) --*
            ...
            A value that determines whether tasks should be queued before executing the tasks. If
            set to ``ENABLED`` , the tasks will be queued. The default is ``ENABLED`` .
        ...
        :param Excludes:
          ...
          - *(dict) --*
            ...
            - **Value** *(string) --*
              ...
              patterns are delimited by "|" (that is, a pipe), for example: ``/folder1
              |/folder2``
        ...
        :param Tags:
          ...
          The key-value pair that represents the tag that you want to add to the resource. The value
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Status** *(string) --*
              ...
              properly and is available to use. The Running status is the normal running status for
              an agent. If the status is OFFLINE, the agent's VM is turned off or the agent is in an
              unhealthy state. When the issue that caused the unhealthy state is resolved, the agent
            ...
            - **PrivateLinkConfig** *(dict) --*
              ...
              - **PrivateLinkEndpoint** *(string) --*
                ...
                in a `PrivateLink
                <https://docs.aws.amazon.com/vpc/latest/userguide/endpoint-service.html>`__ . An
                agent that is configured with this endpoint will not be accessible over the public
                Internet.
              ...
              - **SecurityGroupArns** *(list) --*
                ...
                The Amazon Resource Names (ARNs) of the security groups that are configured for the
                EC2 resource that hosts an agent activated in a VPC or an agent that has access to a
                VPC endpoint.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Ec2Config** *(dict) --*
              ...
              group that you provide needs to be able to communicate with the security group on the
          ...
          - *(dict) --*
            ...
            - **MountOptions** *(dict) --*
              ...
              - **Version** *(string) --*
                ...
                specify a version, DataSync defaults to ``AUTOMATIC`` . That is, DataSync
                automatically selects a version based on negotiation with the NFS server.

                You can specify the following NFS versions:

                * **`NFSv3 <https://tools.ietf.org/html/rfc1813>`__ ** - stateless protocol version
                that allows for asynchronous writes on the server.

                * **`NFSv4.0 <https://tools.ietf.org/html/rfc3530>`__ ** - stateful,
                firewall-friendly protocol version that supports delegations and pseudo filesystems.

                * **`NFSv4.1 <https://tools.ietf.org/html/rfc5661>`__ ** - stateful protocol version
                that supports sessions, directory delegations, and parallel data processing. Version
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **S3StorageClass** *(string) --*
              ...
              `Amazon S3 Storage Classes <https://aws.amazon.com/s3/storage-classes/>`__ in the
              *Amazon Simple Storage Service Developer Guide* . Some storage classes have behaviors
            ...
            - **S3Config** *(dict) --*
              ...
              S3 in the *AWS DataSync User Guide* .
          ...
          - *(dict) --*
            ...
            - **MountOptions** *(dict) --*
              ...
              - **Version** *(string) --*
                ...
                you don't specify a version, DataSync defaults to ``AUTOMATIC`` . That is, DataSync
                automatically selects a version based on negotiation with the SMB server.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Status** *(string) --*
              ...
              For detailed information about task execution statuses, see Understanding Task
              Statuses in the *AWS DataSync User Guide.*
            ...
            - **CloudWatchLogGroupArn** *(string) --*
              ...
              For more information on these groups, see Working with Log Groups and Log Streams in
            ...
            - **Options** *(dict) --*
              ...
              the task that occurs when you call ``StartTaskExecution`` . You can configure these
              options to preserve metadata such as user ID (UID) and group (GID), file permissions,
              ...
              - **OverwriteMode** *(string) --*
                ...
                For detailed information, see  using-storage-classes in the *AWS DataSync User
                Guide* .

              - **Atime** *(string) --*

                A file metadata value that shows the last time a file was accessed (that is, when
                the file was read or written to). If you set ``Atime`` to BEST_EFFORT, DataSync
                ...
                standard across platforms, so AWS DataSync can only do this on a best-effort basis.
              ...
              - **PreserveDeletedFiles** *(string) --*
                ...
                source file system should be preserved. This option can affect your storage cost. If
                your task deletes objects, you might incur minimum storage duration charges for
                certain storage classes. For detailed information, see  using-storage-classes in the
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Options** *(dict) --*
              ...
              StartTaskExecution operation. Behavior includes preserving metadata such as user ID
              (UID), group ID (GID), and file permissions, and also overwriting files in the
              ...
              in  StartTaskExecution , the default value is used. You can override the defaults
            ...
            - **Includes** *(list) --*
              ...
              list should contain a single filter string that consists of the patterns to include.
            ...
            - **EstimatedFilesToTransfer** *(integer) --*
              ...
              calculated during the PREPARING phase, before the TRANSFERRING phase. This value is
              the expected number of files to be transferred. It's calculated based on comparing the
            ...
            - **FilesTransferred** *(integer) --*
              ...
              for some location types, so don't use it as an indicator for a correct file number or
    ...
    def list_agents(
        ...
        If you have more agents than are returned in a response (that is, the response returns only
        a truncated list of your agents), the response contains a marker that you can specify in
            ...
            - **Locations** *(list) --*
              ...
              - *(dict) --*
                ...
                - **LocationUri** *(string) --*
                  ...
                  For Amazon S3, it's the prefix path that you mount to and treat as the root of the
                  location.
    ...
    def start_task_execution(
        ...
        """
        Starts a specific invocation of a task. A ``TaskExecution`` value represents an individual
        run of a task. Each task can have at most one ``TaskExecution`` at a time.
        ...
        **Request Syntax**
        ...
        :param OverrideOptions:
          ...
          StartTaskExecution , the default value is used. You can override the defaults options on
          each task execution by specifying an overriding ``Options`` value to  StartTaskExecution .
        ...
        :param Includes:
          ...
          A list of filter rules that determines which files to include when running a task. The
          pattern should contain a single filter string that consists of the patterns to include.
          The patterns are delimited by "|" (that is, a pipe). For example: ``"/folder1
          |/folder2"``
    ...
    def update_task(
        ...
        **Request Syntax**
        ...
        :param Schedule:
          ...
          Specifies a schedule used to periodically transfer files from a source to a destination
          location. You can configure your task to execute hourly, daily, weekly or on specific days
          of the week. You control when in the day or hour you want the task to execute. The time
          you specify is UTC time. For more information, see  task-scheduling .
