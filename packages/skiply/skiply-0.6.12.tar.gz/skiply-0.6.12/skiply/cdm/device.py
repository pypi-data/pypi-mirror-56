...
from skiply.cdm import Contract, Service, AssociationContractService
...
def get_devices_to_monitor(hours_start, hours_end, power_threshold, standard_monitoring):
    ...
    try:
        if standard_monitoring:
            results = session.query(Device).filter(Device.entity_id.notin_(session.query(Contract, Service).filter(AssociationContractService.contract_id == Contract.id, AssociationContractService.service_id == Service.id, Service.service_code == "CUSTOM_MONITORING"))).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, func.concat("0x", func.substr(Device.device_battery_level, 5, 4)) <= power)).all()
    except:
