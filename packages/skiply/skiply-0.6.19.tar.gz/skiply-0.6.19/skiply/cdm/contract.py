...
def get_contract_with_service_code(service_code):
    session = db_session()
    try:
        results = session.query(Contract, Service).filter(AssociationContractService.contract_id == Contract.id, AssociationContractService.service_id == Service.id).filter(Service.service_code == service_code).first()
    except:
        print("DB Request get_contract(contract_id) Failed")
        results=None
    finally:
