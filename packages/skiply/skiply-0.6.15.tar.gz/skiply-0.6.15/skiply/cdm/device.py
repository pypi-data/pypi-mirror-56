...
def get_devices_to_monitor(hours_start, hours_end, power_threshold, standard_monitoring):
    ...
    try:
        if standard_monitoring:
            ...
            res_notin = session.query(skiply.cdm.contract.Contract, Service).filter(skiply.cdm.associationContractService.AssociationContractService.contract_id == skiply.cdm.contract.Contract.id, skiply.cdm.associationContractService.AssociationContractService.service_id == skiply.cdm.service.Service.id, skiply.cdm.service.Service.service_code == "CUSTOM_MONITORING")
