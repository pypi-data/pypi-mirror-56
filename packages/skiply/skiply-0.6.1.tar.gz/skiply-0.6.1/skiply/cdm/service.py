...
class Service(SkiplyBase):
    ...
    contracts = relationship('Contract', secondary=AssociationContractService)
...
def get_service(service_id):
    ...
    try:
        ...
        print("DB Request get_service(service_id) Failed")
        results=None
    finally:
        session.close()
...
def get_service_from_code(service_code):
    session = db_session()
    try:
        results = session.query(Service).filter(Service.service_code == service_code).first()
    except:
        print("DB Request get_service_from_code(service_code) Failed")
        results=None
    finally:
        session.close()
