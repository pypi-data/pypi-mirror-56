...
def get_devices_to_monitor(hours_start, hours_end, power_threshold, standard_monitoring):
    ...
    try:
        if standard_monitoring:
            print("get_devices_to_monitor(hours_start, hours_end, power_threshold, standard_monitoring) : Starnd monitoring")
            results = session.query(Device).filter(Device.entity_id.notin_(session.query(skiply.cdm.contract.Contract, Service).filter(skiply.cdm.associationContractService.AssociationContractService.contract_id == skiply.cdm.contract.Contract.id, skiply.cdm.associationContractService.AssociationContractService.service_id == skiply.cdm.service.Service.id, skiply.cdm.service.Service.service_code == "CUSTOM_MONITORING"))).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, func.concat("0x", func.substr(Device.device_battery_level, 5, 4)) <= power)).all()
    except:
