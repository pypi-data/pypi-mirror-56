...
def get_devices_to_monitor(hours_start, hours_end, power_threshold):
    ...
    print(deltatime_end)
    power = hex(power_threshold)
    print(power)
    if (!power.startswith("0x")):
        power = "0x"+power
    while (len(power) < 6):
        power[:2] + '-' + power[2:]
    print(power)
    ...
    session = db_session()
    try:
        results = session.query(Device).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, func.substr(Device.device_battery_level, 5, 4) <= power)).all()
    except:
