...
class ClientCreateBudgetBudgetTypeDef(
    ...
    - **PlannedBudgetLimits** *(dict) --*
      ...
      the current month and include the next 11 months. The ``key`` is the start of the month, ``UTC``
      in epoch seconds.
    ...
    - **TimeUnit** *(string) --* **[REQUIRED]**
      ...
      The length of time until a budget resets the actual and forecasted spend. ``DAILY`` is available
      only for ``RI_UTILIZATION`` , ``RI_COVERAGE`` , ``Savings_Plans_Utilization`` , and
    ...
    - **BudgetType** *(string) --* **[REQUIRED]**
      ...
      Whether this budget tracks costs, usage, RI utilization, RI coverage, Savings Plans utilization,
...
class ClientDescribeBudgetPerformanceHistoryTimePeriodTypeDef(
    ...
    - **End** *(datetime) --*
      ...
      After the end date, AWS deletes the budget and all associated notifications and subscribers. You
      can change your end date with the ``UpdateBudget`` operation.
    """
