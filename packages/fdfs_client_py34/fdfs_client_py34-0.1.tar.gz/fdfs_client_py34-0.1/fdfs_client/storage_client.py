...
#     recv_buffer, remain_bytes = tcp_recv_response(store_conn, th.pkg_len)
...
def tcp_recv_file(conn, local_filename, file_size, buffer_size=1024):
    ...
    with open(local_filename, 'wb+') as f:
        try:
            recv_buffer, total_file_size = tcp_recv_response(conn, file_size)
            f.write(recv_buffer)
        except ConnectionError as e:
            raise ConnectionError('[-] Error: while downloading file(%s).' % e.args)
        except IOError as e:
...
class Storage_client(object):
    ...
    def _storage_do_download_file(self, tracker_client, store_serv, file_buffer, offset, download_size,
        ...
        try:
            ...
            # print(type(down_fmt), type(offset), type(download_size), type(store_serv.group_name), type(remote_filename))
            send_buffer = struct.pack(
                down_fmt, offset, download_size, store_serv.group_name, str.encode(remote_filename))
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            # if th.status == 2:
            #    raise DataError('[-] Error: remote file %s is not exist.' % 
            ...
            self.pool.release(store_conn)
        ret_dic = {
            'Remote file_id': store_serv.group_name + __os_sep__.encode() + str.encode(remote_filename),
