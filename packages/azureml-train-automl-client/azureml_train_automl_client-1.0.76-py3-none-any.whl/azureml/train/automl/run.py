...
class AutoMLRun(Run):
            ...
            try:
                ...
                if automl_settings.spark_context is not None:
                    try:
                        ...
                        raise ScenarioNotSupportedException("azureml-train-automl-runtime must be installed in order "
                                                            "to continue an ADB experiment. Please install this "
    ...
    def get_output(self,
        ...
        if curr_run is None:
            raise NotFoundException.create_without_pii(
                'Could not find a model with a valid score for metric \'{0}\'. '
                'Either the requested run failed or all runs failed.'.format(metric))
    ...
    def _get_run_internal(
        ...
        if iteration is not None and metric is not None:
            raise ConfigException.create_without_pii('Cannot spcify both metric and iteration.')
    ...
    def _download_automl_model(self, curr_run: Run) -> Pipeline:
        ...
        try:
            curr_run.download_file(name=constants.MODEL_PATH, output_file_path=self.local_model_path)
        except Exception as e:
            raise ClientException.create_without_pii(str(e)) from None
        ...
        try:
            ...
            logging.warn("Cannot retrieve model since azureml-train-automl-runtime is not installed locally. "
    ...
    def _download_automl_onnx_model(
        ...
        try:
            curr_run.download_file(name=model_name, output_file_path=self.local_model_path)
        except Exception as e:
            raise ClientException.create_without_pii(str(e)) from None
        ...
        try:
            ...
            raise OptionalDependencyMissingException("azureml-train-automl-runtime must be installed"
                                                     "to enable return_onnx_model. Please install this dependency "
                                                     "to enable returning a model from this method.")
    ...
    def summary(self):
        ...
        primary_metric = properties[PRIMARY_METRIC_STR]
