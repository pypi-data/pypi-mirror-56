...
class sqlite:
    ...
    def __setconn(self):
        if not self.__conn:
            try:
                if '/' in self.__config['db']:
                    self.__conn = sqlite3.connect(self.__config['db'])
                else:
                    self.__conn = sqlite3.connect(os.path.split(os.path.realpath(__file__))[0]+"/sqlitedata/"+self.__config['db'])
