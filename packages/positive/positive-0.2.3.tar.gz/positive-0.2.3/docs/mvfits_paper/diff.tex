...
%DIF DEL mvf-a167b20.tex   Thu Mar 28 20:01:54 2019
%DIF ADD mvf.tex           Tue Apr  9 10:14:51 2019
...
\author{L. London}
\DIFaddbegin \affiliation{MIT-Kavli Institute for Astrophysics and Space Research and LIGO Laboratory, 77 Massachusetts Avenue, 37-664H, Cambridge, MA 02139, USA}
\DIFaddend \affiliation{\Cardiff}
...
%
In the coming years, expectations for frequent \gw{} detections of increasing \snr{} are high \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{TheLIGOScientific:2016pea,Abbott:2016nhf}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{TheLIGOScientific:2016pea,Abbott:2016nhf,LIGOScientific:2018mvr}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
...
%
Most prominently, there is ongoing interest in efficient and accurate signal models for \bbh{} \imr{} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Blackman:2017dfb, London:2017bcn, Hannam:2013oca}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Blackman:2017dfb, London:2017bcn, Hannam:2013oca, Varma:2018mmi, Pan:2013rra, Cotesta:2018fcv}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
As the merger of isolated \bh{s} is expected to result in a perturbed Kerr \bh{}, there is related interest in having \DIFaddbegin \DIFadd{accurate and }\DIFaddend computationally efficient models for perturbative parameters, namely those that enable evaluation of the related \rd{} radiation \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Berti:2005ys}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Berti:2005ys,TheLIGOScientific:2016wfe}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
...
%
These solutions allow \grad{} from \DIFdelbegin \DIFdel{generic perturbations to be well }\DIFdelend \DIFaddbegin \DIFadd{a perturbed Kerr BH at sufficiently late times to be }\DIFaddend approximated by a spectral (multipolar) sum which combines the complex \qnm{} amplitude, $A\lmn$, with \DIFdelbegin \DIFdel{the spin $-2$ weighted -2 }\DIFdelend spheroidal harmonics, $_{-2}S\lmn$\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, of spin weight -2.
}\DIFaddend %
\begin{align}
	%
	\label{hrd}
...
\end{align}
%
\par In the first and second lines of \eqn{hrd}, we relate the observable \gw{} polarizations, $h_+$ and $h_\times$, to the analytically understood morphology of the time domain \rd{} waveform \DIFaddbegin \DIFadd{where $r$ is the distance from the source}\DIFaddend .
%
Here, the labels $\ell$ and $m$ are polar and azimuthal eigenvalues of \tk{'s} angular equations, where total mass\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the speed of light \DIFaddbegin \DIFadd{and the gravitational constant }\DIFaddend are set to unity (i.e. \DIFdelbegin \DIFdel{$M=c=1$}\DIFdelend \DIFaddbegin \DIFadd{$M=c=G=1$}\DIFaddend ).
%
...
%
In the \eob{} formalism, $h\LM$ is modeled such that, after its peak (near merger), the effective functional form reduces (asymptotically) to \eqn{hrd}'s second line \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Cotesta:2018fcv, Buonanno:2000ef,
Bohe:2016gbl,Pan:2013rra, Bohe:2016gbl, Nagar:2018zoe}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Cotesta:2018fcv, Buonanno:2000ef,
Bohe:2016gbl,Pan:2013rra, Bohe:2016gbl, Nagar:2018zoe,Babak:2016tgq}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
This view currently comes with the added assumption that ${_{-2}}S_{\ell m n} = {_{-2}}Y_{\ell m}$, where ${_{-2}}Y_{\ell m}$ are the \DIFdelbegin \DIFdel{spins weighted -2 spherical harmonics .
}\DIFdelend \DIFaddbegin \DIFadd{spherical harmonics of spin weight -2.
}\DIFaddend %
...
%
\par \DIFdelbegin \DIFdel{Both Phenom and }%DIFDELCMD < \eob{} %%%
\DIFdel{approaches either use phenomenological }\DIFdelend \DIFaddbegin \DIFadd{Phenomenological }\DIFaddend models of remnant \bh{} mass and spin \DIFaddbegin \DIFadd{are used by both Phenom and }\eob{} \DIFadd{approaches either }\DIFaddend to interpolate over tables of \qnm{} frequencies \DIFdelbegin \DIFdel{, or }\DIFdelend \DIFaddbegin \DIFadd{or in the evaluation of }\DIFaddend phenomenological models for \qnm{} frequencies\DIFdelbegin \DIFdel{are used directly}\DIFdelend .
%
...
%
%
\par In this combined context, it is clear that the modeling of \qnm{} frequencies, $\cw\lmn$, and spherical-spheroidal mixing coefficients, $\sigma\LMlmn$, are relevant for a range of \gw{} signal models \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(e.g. \mbox{%DIFAUXCMD
\cite{Mehta:2019wxm}}\hspace{0pt}%DIFAUXCMD
).
}\DIFaddend %
...
%
\par For \DIFdelbegin \DIFdel{both the }\DIFdelend \DIFaddbegin \DIFadd{$(l,m)\neq(2,2)$ }\DIFaddend \qnm{} frequencies and the spherical-sphoidal mixing coefficients, we present the first models which treat \qnm{s} rotating with and against the rotation of the \bh{} as being a part of a single solution parameterized by dimensionless \bh{} spin ranging from -1 to 1.
%
This perspective reflects the empirical observation that the remnant spin of \bbh{} mergers smoothly connects regions of positive and negative spin relative to the direction of the initial \DIFdelbegin \DIFdel{oribital }\DIFdelend \DIFaddbegin \DIFadd{orbital }\DIFaddend angular momentum \cite{Husa:2015iqa,London:2018gaq}.
%
\DIFaddbegin \DIFadd{Outside of }\bbh{} \DIFadd{mergers, these negative spin }\qnm{s} \DIFadd{generally correspond to perturbations that are counter-rotating relative to the }\bh{} \DIFadd{spin.
%DIF > 
}\DIFaddend \par For the \qnm{} frequencies, it is well known that for nearly extremal \bh{s} (i.e. $\jf \rightarrow 1$) some of the frequencies have zero-damping (i.e. $\tau\lmn \rightarrow \infty$) \cite{Yang:2012pj,Zimmerman:2015trm}.
%
In the context of \gw{} \da{}, where source parameters are estimated using routines which sample over the space of all possible \bh{} masses and spins \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{TheLIGOScientific:2016wfe}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , it is useful to have accurate physical behavior in the extremal limit\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{TheLIGOScientific:2016wfe}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .
%
...
%
In \DIFdelbegin \DIFdel{section }\DIFdelend \sec{meth}, we outline the \gmvp{} and \gmvr{} algorithms.
%
...
%
\par If we consider a scalar function, $f$, of \DIFdelbegin \DIFdel{$N$ }\DIFdelend \DIFaddbegin \DIFadd{$N+1$ }\DIFaddend variables sampled in $j$, $\vec{x}_j = \{x_{\alpha j}\}_{\alpha=0}^{N}$, then $f(\vec{x}_j)$ can be represented (possibly inaccurately) as a sum over \DIFdelbegin \DIFdel{$K$ }\DIFdelend \DIFaddbegin \DIFadd{$K+1$ }\DIFaddend linearly independent basis functions, $\phi_k(\vec{x}_j)$:
%
\begin{align}
  \label{lin1}
  ...
  \begin{algorithmic}[1]
    ...
    \While{not $done$}
      ...
      \For { $\mlam$ in $\mlam^{bulk}$ }
        \State \DIFdelbegin \DIFdel{$\mlam_{trial} = \mlam_{kept} \cup \mlam$ }\DIFdelend \DIFaddbegin \DIFadd{$\mlam_{trial} = \mlam_{kept} \cup \{\mlam\}$ }\DIFaddend {\hskip0.525in} (add $\mlam$ to $\mlam_{kept}$)
    ...
    \EndWhile
    \vskip 10pt
    \State {\bf Output:} \DIFdelbegin \DIFdel{$\{ \mlam_{kept} \}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mlam_{kept}$ }\DIFaddend (the Greedy Basis)
    %
  \end{algorithmic}
  %
\end{algorithm}
...
%
The study of \DIFdelbegin \DIFdel{smooth scalar functions (e.g. $f(\vec{x})$) }\DIFdelend \DIFaddbegin \DIFadd{continuous scalar functions }\DIFaddend often centers about \DIFdelbegin \DIFdel{the Taylor series expansion }\DIFdelend \DIFaddbegin \DIFadd{their expansion in terms of monomials}\DIFaddend .
%
\DIFdelbegin \DIFdel{In that instance, it is clear that any infinitely differentiable scalar function of many variables}\DIFdelend \DIFaddbegin \DIFadd{This is made rigorous by the Generalised Stone-Weierstrass Theorem \mbox{%DIFAUXCMD
\cite{Hassani:2013}}\hspace{0pt}%DIFAUXCMD
. Consider a continuous function $f(\vec{x}) \equiv f(x_0, x_1, ..., x_N)$ in $N+1$ variables. Then $f(\vec{x})$ }\DIFaddend can be represented \DIFdelbegin \DIFdel{in terms of its derivatives by
}\DIFdelend \DIFaddbegin \DIFadd{as
}\DIFaddend %
\begin{align}
  \label{eq:mvt}
  f( \vec{x}\DIFdelbegin \DIFdel{+ }%DIFDELCMD < \vec{h} %%%
\DIFdelend ) &= \DIFdelbegin \DIFdel{e^{ \vec{x} \cdot \vec{\nabla}' } \, f(}%DIFDELCMD < \vec{x}%%%
\DIFdel{\,') |_{\vec{x}\,'=\vec{h}}
  }\DIFdelend \DIFaddbegin \DIFadd{\sum_{\substack{d_i=0 \\ i \in \{0 \ldots N\}}}^{\infty} c_{d_0 d_1 \ldots d_N} x^{d_0} x^{d_1} \ldots x^{d_N}
  }\DIFaddend \\ \nonumber
  &\approx \sum\DIFdelbegin \DIFdel{_{k=0}}\DIFdelend \DIFaddbegin \DIFadd{_{\substack{d_i=0 \\ i \in \{0 \ldots N\}}}}\DIFaddend ^{K} \DIFdelbegin \DIFdel{\, \frac{1}{k!} \, (}%DIFDELCMD < \vec{x} %%%
\DIFdel{\cdot }%DIFDELCMD < \vec{\nabla}%%%
\DIFdel{')^{k} \, f(}%DIFDELCMD < \vec{x}%%%
\DIFdel{\,') |_{\vec{x}\,'=\vec{h}}
}\DIFdelend \DIFaddbegin \DIFadd{c_{d_0 d_1 \ldots d_N} x^{d_0} x^{d_1} \ldots x^{d_N}
}\DIFaddend \end{align}
%
From the first to second line of \eqn{eq:mvt} \DIFdelbegin \DIFdel{, we have used the definition of the exponential function (i.e. its series expansion).
%DIF < 
In the second line, the }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend equality has been replaced by an approximation as we have limited the linear representation to \DIFdelbegin \DIFdel{$K+1$ terms. }\DIFdelend \DIFaddbegin \DIFadd{monomials with combined exponents no greater than $K(N+1)$. This implication follows from the Stone-Weierstrass Theorem \mbox{%DIFAUXCMD
\cite{Lang:1983} }\hspace{0pt}%DIFAUXCMD
and tells us that a continuous function may be uniformly approximated by a polynomial.
}\DIFaddend %
\par This \DIFdelbegin \DIFdel{latter point }\DIFdelend is key to the perspective of \gmvp{}:
%
given training data thought to be related to a \DIFdelbegin \DIFdel{smooth }\DIFdelend \DIFaddbegin \DIFadd{continuous }\DIFaddend multivariate function, it may, particularly on small scales, be well approximated by a \DIFdelbegin \DIFdel{truncated series expansion }\DIFdelend \DIFaddbegin \DIFadd{polynomial }\DIFaddend in an appropriate coordinate basis.
%
% GMVP
\hspace{1cm}
{\scriptsize
...
%
%
\par \DIFdelbegin %DIFDELCMD < \eqn{eq:mvt}%%%
\DIFdel{'s truncated expansion happens to be a polynomial in at most $N$ variables.
%DIF < 
}\DIFdelend In this setting, the uncertainty of which and how many basis terms to include makes this a problem ripe for the application of linear modeling driven by a greedy process, namely \eqn{eq:linsol1} and Alg. (\ref{alg:pgreedy}) .
%
%
...
\begin{figure*}[htb]
  %
  \begin{tabular}{ll}
    {\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{fits_w.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.49\textwidth,trim={0 0.2in 0 0.4in},clip]{fits_w.pdf}\DIFaddendFL } & {\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.49\textwidth]{fits_tau.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.49\textwidth,trim={0 0.2in 0 0.4in},clip]{fits_tau.pdf}\DIFaddendFL }
    % \\
    % \includegraphics[width=0.49\textwidth]{res_w.pdf} & \includegraphics[width=0.49\textwidth]{res_tau.pdf}
    \\
    \DIFdelbeginFL \DIFdelFL{\hspace{0.35cm}}%DIFDELCMD < \includegraphics[width=0.43\textwidth]{res_cw_0.pdf} %%%
\DIFdelendFL \DIFaddbeginFL {\includegraphics[width=0.49\textwidth,trim={0 0.25in 0 0.9in},clip]{res_cw_0.pdf}} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{\hspace{0.3cm}}%DIFDELCMD < \includegraphics[width=0.43\textwidth]{res_cw_1.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL {\includegraphics[width=0.49\textwidth,trim={0 0.25in 0 0.9in},clip]{res_cw_1.pdf}}
...
%
Each term is an element of the tensor-product of the \DIFdelbegin \DIFdel{power-sets of each model dimension}\DIFdelend \DIFaddbegin \DIFadd{sets of powers of each coordinate of a chosen representation of the models domain}\DIFaddend . That is \DIFdelbegin \DIFdel{$\mlam_{bulk} = \{x_0,x_1 .. x_N, ... x_0^1, x_0^2, x_0^3, ...  , (x_0^K x_1^K ... x_N^K) \}$}\DIFdelend \DIFaddbegin \DIFadd{$\mlam_{bulk} = \{x_0^{d_0} x_1^{d_1} \ldots x_N^{d_N} \mid d_i \in \{0,\ldots,K\}, i \in \{0,\ldots,N\}\}$}\DIFaddend .
%
...
%
%
\par Despite the apparent universality of \eqn{eq:mvt}, there are many cases where $K$ must be orders of magnitude greater than 1 in order for $\vec{f}$ to be accurately represented by a \DIFdelbegin \DIFdel{low order multinomial.
%DIF < 
Worst, in cases where the underlying dataset is best described by a rational function, no polynomialof the same family describing the numerator and denominator will yield satisfactory results in low order}\DIFdelend \DIFaddbegin \DIFadd{polynomial}\DIFaddend .
%
...
\end{align}
%
where, $\hat{P}$ is the pseudo-inverse of the matrix whose elements are $\psi_k(\vec{x}_j)$, $\vec\alpha = ( z_0, z_1, ... z_{R+V-1},z_{R+V} )$, and \DIFdelbegin \DIFdel{$\vec{g} = ( g_0, g_1, ... g_{R+V-1},g_{R+V} )$}\DIFdelend \DIFaddbegin \DIFadd{$\vec{g} = ( g_j )$}\DIFaddend .
%
...
\end{align}
%
In practice, one solves \eqn{eq:pinv3} for $\vec{\alpha}^{(n+1)}$, and then uses the related $a_r^{(n+1)}$ and $b_v^{(n+1)}$ to calculate \DIFdelbegin \DIFdel{$g^{(n+1)}$ }\DIFdelend \DIFaddbegin \DIFadd{$g_j^{(n+1)}$ }\DIFaddend via
%
\begin{align}
  \label{eq:rat4}
  g\DIFaddbegin \DIFadd{_j}\DIFaddend ^{(n+1)} =  \DIFdelbegin \DIFdel{\frac{ \sum_{r=0}^{R} a^{(n+1)}_r \, \phi_{r}(\vec{x}) }{ 1\;-\;  \sum_{v=1}^{V} b^{(n+1)}_v \, \phi_{v}(\vec{x}) } }\DIFdelend \DIFaddbegin \DIFadd{\frac{ \sum_{r=0}^{R} a^{(n+1)}_r \, \phi_{r}(\vec{x_j}) }{ 1\;-\;  \sum_{v=1}^{V} b^{(n+1)}_v \, \phi_{v}(\vec{x_j}) } }\DIFaddend \; .
\end{align}
%
Subsequently, \DIFdelbegin \DIFdel{$g^{(n+1)}$ }\DIFdelend \DIFaddbegin \DIFadd{$g_j^{(n+1)}$ }\DIFaddend is then fed back into \eqn{eq:pinv3} for further refinement.
%
The refinement process is to terminate when a measure of model error (e.g. the $L^2$ norm \DIFdelbegin \DIFdel{$||g^{(0)}-g^{(n)}||$}\DIFdelend \DIFaddbegin \DIFadd{$||\vec{g}^{(0)}-\vec{g}^{(n)}||$}\DIFaddend ) passes a predetermined threshold. \DIFaddbegin \DIFadd{For the results presented in \sec{results} the following model representation error was used
}\DIFaddend %
\DIFaddbegin \begin{align}
  \DIFadd{\label{eq:raterr}
  \epsilon^{(n)}  = \left|\frac{\operatorname{Var}\left(\vec{g}^{(n)} - \vec{g}^{(0)}\right)}{\operatorname{Var}\left(\vec{g}^{(0)}\right)}\right|
}\end{align}
\DIFaddend %
%DIF < 
\DIFaddbegin \DIFadd{where $\operatorname{Var}$ is the variance. This model representation error is similar to the square of the root-mean-squared-deviation normalized by the variance of the data set.
}\DIFaddend % \hspace{1cm}
{\scriptsize
\begin{algorithm}[H]
  %
...
\end{align}
%
appropriately linearizes the sharp behavior of each frequency near $\jf=1$ while also mapping $-1 \le \jf \le 1$ onto $0 \le \kappa \le 1$. \DIFaddbegin \DIFadd{Note that under the mapping $j_f \mapsto \kappa$ the orientation is reversed such that as $j_f \rightarrow 1$ from below $\kappa \rightarrow 0$ from above.
}\DIFaddend %
...
\begin{align}
  \label{eq:zd}
  \cw_{\lmn} = m/2 + \kappa \sum\DIFdelbegin \DIFdel{_{j=1}}\DIFdelend \DIFaddbegin \DIFadd{_{j=0}}\DIFaddend ^{J} c_j \kappa\DIFdelbegin \DIFdel{^k }\DIFdelend \DIFaddbegin \DIFadd{^j }\DIFaddend \; .
\end{align}
%
...
\begin{align}
  \label{eq:nzd}
  \cw_{\lmn} = \sum\DIFdelbegin \DIFdel{_{j=0}^{J} }\DIFdelend \DIFaddbegin \DIFadd{_{u=0}^{U} }\DIFaddend c\DIFdelbegin \DIFdel{_j }\DIFdelend \DIFaddbegin \DIFadd{_u }\DIFaddend \kappa\DIFdelbegin \DIFdel{^k }\DIFdelend \DIFaddbegin \DIFadd{^u }\DIFaddend \; .
\end{align}
%
\par The polynomial content of \eqn{eq:zd} and \eqn{eq:nzd} is determined by \gmvp{}.
%
\eqns{eq:cw_fit_1}{eq:cw_fit_9} display the resulting polynomial models \DIFaddbegin \DIFadd{to numerical results of Leaver's method~\mbox{%DIFAUXCMD
\cite{Leaver85}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
In particular, the domain map allows most \qnm{} frequencies to be well modeled by \DIFdelbegin \DIFdel{4th }\DIFdelend \DIFaddbegin \DIFadd{5th }\DIFaddend order polynomials which include all lower degree terms; concurrently, the real and imaginary parts of each $\cw\lmn$ are modeled simultaneously.
%
\DIFaddbegin \DIFadd{For the zero-damped }\qnm{s}\DIFadd{, we note that 4th order polynomials were actually modeled, with the remaining power of $\kappa$ resulting from explicit enforcement of the zero-damped condition.
%DIF > 
}\DIFaddend \par \fig{fig:qnm} displays select training points, as well as model fits for $\cw\lmn$'s real and imaginary parts.
%
...
%
In particular, it is shown that the two branches (namely $\jf>0$ and $\jf<0$) naturally form a single family of solutions when accounting for the sign of the \bh{}'s oriented spin \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Husa:2007hp}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Husa:2015iqa}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
...
%
We note that, in the asymptotic vicinity of $\jf=1$, the \qnm{} frequencies and decay times are known to have solutions that are asymptoticly degenerate \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Zimmerman:2015rua}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Zimmerman:2015trm}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
%
{\scriptsize
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \vskip 10pt
    \State \DIFdelbegin \DIFdel{Calculate $\alpha^{(1)}_k$ }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Let}} \DIFadd{$n=0$
    }\State \DIFadd{Calculate $\vec{\alpha}^{(1)}$ }\DIFaddend via \eqn{eq:pinv3}.
    \DIFdelbegin \DIFdel{Implicitly, $n=1$.
    }\DIFdelend \State Calculate the \DIFdelbegin \DIFdel{current model prediction $g^{(1)}$ }\DIFdelend \DIFaddbegin \DIFadd{model prediction $\vec{g}^{(1)}$ }\DIFaddend via \eqn{eq:rat4}.
    \State Calculate the model representation error \DIFdelbegin \DIFdel{, e.
    g.: $\epsilon^{(1)}  = ||var(g^{(n)} - g^{(0)})||/||g^{(0)}||$, where $var$ is the variance.
    }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon^{(1)}$.
    }\DIFaddend \State \DIFaddbegin \textbf{\DIFadd{Let}} \DIFaddend $done = False$
    \While { not $done$ }
      \State \DIFdelbegin \DIFdel{Calculate $\alpha^{n+1}_{k}$  }\DIFdelend \DIFaddbegin \DIFadd{$n=n+1$
      }\State \DIFadd{Calculate $\vec{\alpha}^{(n+1)}$  }\DIFaddend via \eqn{eq:pinv3}.
      \State Calculate the \DIFdelbegin \DIFdel{current model prediction $g^{(n)}$ }\DIFdelend \DIFaddbegin \DIFadd{model prediction $\vec{g}^{(n+1)}$ }\DIFaddend via \eqn{eq:rat4}.
      \State Calculate the model representation error \DIFdelbegin \DIFdel{, e.
      g. $\epsilon^{(n)} = ||g^{(n)} - g^{(0)}||/||g^{(0)}||$.
      }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon^{(n+1)}$.
      }\DIFaddend \State \DIFdelbegin \DIFdel{$done = |\epsilon^{(n-1)}-\epsilon^{(n)}| < tol$
      }%DIFDELCMD < \State %%%
\DIFdel{(Implicitly, $n = n+1$)
    }\DIFdelend \DIFaddbegin \DIFadd{$done = |\epsilon^{(n)}-\epsilon^{(n+1)}| < tol$
    }\DIFaddend \EndWhile
    \vskip 10pt
...
%
here, the sharp feature near $\jf=0$ results from their modeling counter and co-rotating \qnm{} as two different curves. \DIFaddbegin \DIFadd{A residual error is also shown as an orange dashed line for the $\cw_{220}$ model presented in Ref.~\mbox{%DIFAUXCMD
\cite{Nagar:2018zoe}}\hspace{0pt}%DIFAUXCMD
. We observe that the rational model of Ref.~\mbox{%DIFAUXCMD
\cite{Nagar:2018zoe} }\hspace{0pt}%DIFAUXCMD
performs better overall while performing worse for higher $\jf$ compared to our model. While we use a polynomial ansatz, the comparison with Ref.~\mbox{%DIFAUXCMD
\cite{Nagar:2018zoe} }\hspace{0pt}%DIFAUXCMD
...
%
%
\par \fig{fig:ys} displays fits, training data and related residuals. \DIFaddbegin \DIFadd{The training data were compute using Leaver's representation for the spheroidal harmonics to evaluate }\eqn{sigma}\DIFadd{.
}\DIFaddend %
...
%
As with the \qnm{} frequencies, residuals are dominated by small scale oscillations with amplitudes that are fractions of a percent of the central values, and largely result from the model \DIFdelbegin \DIFdel{asatz}\DIFdelend \DIFaddbegin \DIFadd{ansatz}\DIFaddend .
%
...
%
%
\par \gmvp{} has been applied to the modeling of \qnm{} frequencies. The resulting \DIFaddbegin \DIFadd{polynomial }\DIFaddend models have been constrained in the extremal Kerr limit, and perform well compared to similar fits in the literature\DIFaddbegin \DIFadd{. We note our comparison of $\cw_{220}$ from Ref.~\mbox{%DIFAUXCMD
\cite{Nagar:2018zoe} }\hspace{0pt}%DIFAUXCMD
suggests even better fits for $(l,m,n)\neq(2,2,0)$ QNM frequencies might be achievable using a rational ansatz. This is a possible avenue for future improvement}\DIFaddend .
%
