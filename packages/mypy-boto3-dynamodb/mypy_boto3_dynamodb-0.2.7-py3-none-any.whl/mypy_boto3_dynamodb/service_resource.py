...
class ServiceResource(Boto3ServiceResource):
    ...
    def batch_get_item(
        ...
        provisioned throughput is exceeded, or an internal processing failure occurs. If a partial result is
        returned, the operation returns a value for ``UnprocessedKeys`` . You can use this value to retry
        ...
        you want strongly consistent reads instead, you can set ``ConsistentRead`` to ``true`` for any or
        all tables.

        In order to minimize response latency, ``BatchGetItem`` retrieves items in parallel.

        When designing your application, keep in mind that DynamoDB does not return items in any particular
        order. To help parse the response by item, include the primary key values for the items in your
        request in the ``ProjectionExpression`` parameter.
        ...
        **Request Syntax**
        ...
        :param RequestItems: **[REQUIRED]**
          ...
          * ``ExpressionAttributeNames`` - One or more substitution tokens for attribute names in the
          ...
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ReservedWords.html>`__ in the
          *Amazon DynamoDB Developer Guide* ). To work around this, you could specify the following for
          ``ExpressionAttributeNames`` :
          ...
          * ``Keys`` - An array of primary key attribute values that define specific items in the table. For
          ...
          key, you only need to provide the partition key value. For a composite key, you must provide
          ...
          * ``ProjectionExpression`` - A string that identifies one or more attributes to retrieve from the
          ...
          in the expression must be separated by commas. If no attribute names are specified, then all
          attributes are returned. If any of the requested attributes are not found, they do not appear in
          the result. For more information, see `Accessing Item Attributes
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.AccessingItemAttributes.html>`__
          in the *Amazon DynamoDB Developer Guide* .
        ...
        :param ReturnConsumedCapacity:
          ...
          * ``INDEXES`` - The response includes the aggregate ``ConsumedCapacity`` for the operation,
          ...
          some operations, such as ``GetItem`` and ``BatchGetItem`` , do not access any indexes at all. In
          these cases, specifying ``INDEXES`` will only return ``ConsumedCapacity`` information for
          table(s).
    ...
    def batch_write_item(
        ...
        check for unprocessed items and submit a new ``BatchWriteItem`` request with those unprocessed items
        until all items have been processed.

        If *none* of the items can be processed due to insufficient provisioned throughput on all of the
        tables in the request, then ``BatchWriteItem`` returns a ``ProvisionedThroughputExceededException``
        .
        ...
        Amazon EMR, or copy data from another database into DynamoDB. In order to improve performance with
        these large-scale operations, ``BatchWriteItem`` does not behave in the same way as individual
        ``PutItem`` and ``DeleteItem`` calls would. For example, you cannot specify conditions on individual
        put and delete requests, and ``BatchWriteItem`` does not return deleted items in the response.

        If you use a programming language that supports concurrency, you can use threads to write items in
        parallel. Your application must include the necessary logic to manage the threads. With languages
        that don't support threading, you must update or delete the specified items one at a time. In both
        situations, ``BatchWriteItem`` performs the specified put and delete operations in parallel, giving
        you the power of the thread pool approach without having to introduce complexity into your
        application.
          ...
          * ``PutRequest`` - Perform a ``PutItem`` operation on the specified item. The item to be put is
          identified by an ``Item`` subelement:
            ...
            * ``Item`` - A map of attributes and their values. Each entry in this map consists of an
            ...
            Requests that contain empty values are rejected with a ``ValidationException`` exception. If you
            specify any attributes that are part of an index key, then the data types for those attributes
              ...
              - *(dict) --*
                ...
                - **DeleteRequest** *(dict) --*
                  ...
                  - **Key** *(dict) --* **[REQUIRED]**
                    ...
                    delete. All of the table's primary key attributes must be specified, and their data
    ...
    def create_table(
        ...
        """
        The ``CreateTable`` operation adds a new table to your account. In an AWS account, table names must
        be unique within each Region. That is, you can have two tables with same name if you create the
        ...
        tables sequentially. Only one table with secondary indexes can be in the ``CREATING`` state at any
        given time.
        ...
        **Request Syntax**
        ...
        :param KeySchema: **[REQUIRED]**
          ...
          see `Data Model
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DataModel.html>`__ in the
        ...
        :param SSESpecification:
          ...
          - **KMSMasterKeyId** *(string) --*
            ...
            a CMK, use its key ID, Amazon Resource Name (ARN), alias name, or alias ARN. Note that you
            should only provide this parameter if the key is different from the default DynamoDB customer
...
class Table(Boto3ServiceResource):
    ...
    def delete(self, *args: Any, **kwargs: Any) -> TableDeleteResponseTypeDef:
        ...
        the specified table is in the ``DELETING`` state until DynamoDB completes the deletion. If the table
        is in the ``ACTIVE`` state, you can delete it. If a table is in ``CREATING`` or ``UPDATING`` states,
        then DynamoDB returns a ``ResourceInUseException`` . If the specified table does not exist, DynamoDB
        ...
        **Request Syntax**
          ...
          response = table.delete_item(
              ...
              Expected={
                  'string': {
                      ...
                      'Exists': True|False,
                      'ComparisonOperator':
                      'EQ'|'NE'|'IN'|'LE'|'LT'|'GE'|'GT'|'BETWEEN'|'NOT_NULL'|'NULL'|'CONTAINS'
              ...
              * Use ``Value`` to specify a value that DynamoDB will compare against an attribute. If the
              ...
              - **ComparisonOperator** *(string) --*
                ...
                * ``CONTAINS`` : Checks for a subsequence, or value in a set.  ``AttributeValueList`` can
                ...
                supported for lists: When evaluating "``a CONTAINS b`` ", "``a`` " can be a list; however,
                ...
                * ``NOT_CONTAINS`` : Checks for absence of a subsequence, or absence of a value in a set.
                ...
                exact match with any member of the set. NOT_CONTAINS is supported for lists: When evaluating
                "``a NOT CONTAINS b`` ", "``a`` " can be a list; however, "``b`` " cannot be a set, a map,
                or a list.
                ...
                * ``BETWEEN`` : Greater than or equal to the first value, and less than or equal to the
                ...
                the target value is greater than, or equal to, the first element and less than, or equal to,
                the second element. If an item contains an ``AttributeValue`` element of a different type
                than the one provided in the request, the value does not match. For example, ``{"S":"6"}``
                does not compare to ``{"N":"6"}`` . Also, ``{"N":"6"}`` does not compare to ``{"NS":["6",
                "2", "1"]}``
              ...
              - **AttributeValueList** *(list) --*
                ...
                code values. For example, ``a`` is greater than ``A`` , and ``a`` is greater than ``B`` .
        ...
        :param ConditionExpression:
          ...
          * Functions: ``attribute_exists | attribute_not_exists | attribute_type | contains | begins_with |
          size``   These function names are case-sensitive.
    ...
    def get_item(
        ...
        element in the response.
         ...
         might take more time than an eventually consistent read, it always returns the last updated value.
    ...
    def load(self, *args: Any, **kwargs: Any) -> None:
        """
        Calls :py:meth:`DynamoDB.Client.describe_table` to update the attributes of the Table resource. Note
    ...
    def put_item(
        ...
        ExpressionAttributeNames: Dict[str, str] = None,
        ExpressionAttributeValues: Dict[
            str, TablePutItemExpressionAttributeValuesTypeDef
        ] = None,
    ) -> TablePutItemResponseTypeDef:
        """
        ...
        primary key doesn't exist), or replace an existing item if it has certain attribute values. You can
        return the item's attribute values in the same operation, using the ``ReturnValues`` parameter.
        ...
        When you add an item, the primary key attributes are the only required attributes. Attribute values
        cannot be null. String and Binary type attributes must have lengths greater than zero. Set type
        attributes cannot be empty. Requests with empty values will be rejected with a
        ``ValidationException`` exception.

        .. note::

          To prevent a new item from replacing an existing item, use a conditional expression that contains
          the ``attribute_not_exists`` function with the name of the attribute being used as the partition
          key for the table. Since every record must contain that attribute, the ``attribute_not_exists``
          function will only succeed if no matching item exists.

        For more information about ``PutItem`` , see `Working with Items
        <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithItems.html>`__ in the
        *Amazon DynamoDB Developer Guide* .
    ...
    def query(
        ...
        ExpressionAttributeNames: Dict[str, str] = None,
        ExpressionAttributeValues: Dict[
            str, TableQueryExpressionAttributeValuesTypeDef
        ] = None,
    ) -> TableQueryResponseTypeDef:
        """
        ...
        ``Query`` operation will return all of the items from the table or index with that partition key
        ...
        will be empty. Queries that do not return results consume the minimum number of read capacity units
        ...
        .. note::
         ...
         Number, the results are returned in numeric order; otherwise, the results are returned in order of
         UTF-8 bytes. By default, the sort order is ascending. To reverse the order, set the
        ...
        <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Query.html#Query.Pagination>`__ in
        the *Amazon DynamoDB Developer Guide* .
        ...
        **Request Syntax**
        ...
        :param Select:
          ...
          * ``ALL_ATTRIBUTES`` - Returns all of the item attributes from the specified table or index. If
          ...
          entire item from the parent table. If the index is configured to project all item attributes, then
          all of the data can be obtained from the local secondary index, and no fetching is required.
          ...
          * ``SPECIFIC_ATTRIBUTES`` - Returns only the attributes listed in ``AttributesToGet`` . This
          ...
          ``Select`` . If you query or scan a local secondary index and request only attributes that are
          projected into that index, the operation will read only the index and not the table. If any of the
          requested attributes are not projected into the local secondary index, DynamoDB fetches each of
          these attributes from the parent table. This extra fetching incurs additional throughput cost and
          latency. If you query or scan a global secondary index, you can only request attributes that are
          ...
          index. You cannot use both ``Select`` and ``AttributesToGet`` together in a single request, unless
          the value for ``Select`` is ``SPECIFIC_ATTRIBUTES`` . (This usage is equivalent to specifying
        ...
        :param Limit:
          ...
          apply in a subsequent operation, so that you can pick up where you left off. Also, if the
          processed dataset size exceeds 1 MB before DynamoDB reaches this limit, it stops the operation and
          returns the matching values up to the limit, and a key in ``LastEvaluatedKey`` to apply in a
          subsequent operation to continue the operation. For more information, see `Query and Scan
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html>`__ in the
          *Amazon DynamoDB Developer Guide* .
          ...
          - *(string) --*
            ...
            - *(dict) --*
              ...
              * For a ``Query`` operation, ``Condition`` is used for specifying the ``KeyConditions`` to use
              ...
              operators are supported:  ``EQ | LE | LT | GE | GT | BEGINS_WITH | BETWEEN``    ``Condition``
              is also used in a ``QueryFilter`` , which evaluates the query results and returns only the
              desired values.
        ...
        :param ScanIndexForward:
          ...
          DynamoDB reads the results in reverse order by sort key value, and then returns the results to the
          client.
        ...
        :param KeyConditionExpression:
          ...
          * ``sortKeyName``  ``BETWEEN``  ``:sortkeyval1``  ``AND``  ``:sortkeyval2`` - true if the sort key
          value is greater than or equal to ``:sortkeyval1`` , and less than or equal to ``:sortkeyval2`` .

          * ``begins_with (``  ``sortKeyName`` , ``:sortkeyval``  ``)`` - true if the sort key value begins
          with a particular operand. (You cannot use this function with a sort key that is of type Number.)
    ...
    def scan(
        ...
        ExpressionAttributeNames: Dict[str, str] = None,
        ExpressionAttributeValues: Dict[
            str, TableScanExpressionAttributeValuesTypeDef
        ] = None,
        ConsistentRead: bool = None,
    ) -> TableScanResponseTypeDef:
        """
        ...
        and results are returned to the user as a ``LastEvaluatedKey`` value to continue the scan in a
        ...
        <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.Pagination>`__ in
        the *Amazon DynamoDB Developer Guide* .
         ...
         set might not include the changes to data in the table immediately before the operation began. If
         you need a consistent copy of the data, as of the time that the ``Scan`` begins, you can set the
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          processed dataset size exceeds 1 MB before DynamoDB reaches this limit, it stops the operation and
          returns the matching values up to the limit, and a key in ``LastEvaluatedKey`` to apply in a
          subsequent operation to continue the operation. For more information, see `Working with Queries
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html>`__ in the
          *Amazon DynamoDB Developer Guide* .
        ...
        :param Select:
          ...
          * ``SPECIFIC_ATTRIBUTES`` - Returns only the attributes listed in ``AttributesToGet`` . This
          return value is equivalent to specifying ``AttributesToGet`` without specifying any value for
          ``Select`` . If you query or scan a local secondary index and request only attributes that are
          projected into that index, the operation reads only the index and not the table. If any of the
          requested attributes are not projected into the local secondary index, DynamoDB fetches each of
          these attributes from the parent table. This extra fetching incurs additional throughput cost and
          latency. If you query or scan a global secondary index, you can only request attributes that are
          projected into the index. Global secondary index queries cannot fetch attributes from the parent
        ...
        :param TotalSegments:
          ...
          number of application workers that will perform the parallel scan. For example, if you want to use
          four application threads to scan a table or an index, specify a ``TotalSegments`` value of 4.
    ...
    def update(
        ...
        * Create a new global secondary index on the table. After the index begins backfilling, you can use
        ``UpdateTable`` to perform other operations.
         ...
         ``ACTIVE`` to ``UPDATING`` . While it is ``UPDATING`` , you cannot issue another ``UpdateTable``
        ...
        **Request Syntax**
        ...
        :param BillingMode:
          ...
          be set. The initial provisioned capacity values are estimated based on the consumed read and write
          capacity of your table and global secondary indexes over the past 30 minutes.
        ...
        :param GlobalSecondaryIndexUpdates:
          ...
          * ``Delete`` - remove a global secondary index from the table.
          ...
          <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/GSI.OnlineOps.html>`__ in the
          *Amazon DynamoDB Developer Guide* .
        ...
        :param ReplicaUpdates:
          ...
          .. note::
            ...
            <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/globaltables.V2.html>`__ of
            global tables.
    ...
    def update_item(
        ...
        **Request Syntax**
          ...
          - *(string) --*
            ...
            - *(dict) --*
              ...
              - **Action** *(string) --*
                ...
                *set* of values is specified, then those values are subtracted from the old set. For
                example, if the attribute value was the set ``[a,b,c]`` and the ``DELETE`` action specified
                ...
                data type of the attribute:
                  ...
                  * If the existing attribute is a number, and if ``Value`` is also a number, then the
                  ...
                  .. note::
                     ...
                     suppose that the item you want to update does not yet have an attribute named
                     *itemcount* , but you decide to ``ADD`` the number ``3`` to this attribute anyway, even
                     though it currently does not exist. DynamoDB will create the *itemcount* attribute, set
                     its initial value to ``0`` , and finally add ``3`` to it. The result will be a new
                  ...
                  * If the existing data type is a set, and if the ``Value`` is also a set, then the
                  ...
                  action specified ``[3]`` , then the final attribute value would be ``[1,2,3]`` . An error
                  occurs if an Add action is specified for a set attribute and the attribute type specified
                  does not match the existing set type.  Both sets must have the same primitive data type.
                  For example, if the existing data type is a set of strings, the ``Value`` must also be a
                  set of strings. The same holds true for number sets and binary sets.

                This action is only valid for an existing attribute whose data type is number or is a set.
                Do not use ``ADD`` for any other data types.
        ...
        :param ReturnValues:
          ...
          are updated. For ``UpdateItem`` , the valid values are:
        ...
        :param UpdateExpression:
          ...
          * ``SET`` - Adds one or more attributes and values to an item. If any of these attributes already
          ...
          attribute that is of type Number. For example: ``SET myNum = myNum + :val``    ``SET`` supports
          the following functions:
            ...
            * ``list_append (operand, operand)`` - evaluates to a list with a new element added to it. You
            can append the new element to the start or the end of the list by reversing the order of the
            operands.
          ...
          attribute does exist, then the behavior of ``ADD`` depends on the data type of the attribute:
            ...
            * If the existing attribute is a number, and if ``Value`` is also a number, then ``Value`` is
            ...
            .. note::
               ...
               before the update, DynamoDB uses ``0`` as the initial value. Similarly, if you use ``ADD``
               for an existing item to increment or decrement an attribute value that doesn't exist before
               the update, DynamoDB uses ``0`` as the initial value. For example, suppose that the item you
               want to update doesn't have an attribute named ``itemcount`` , but you decide to ``ADD`` the
               number ``3`` to this attribute anyway. DynamoDB will create the ``itemcount`` attribute, set
               its initial value to ``0`` , and finally add ``3`` to it. The result will be a new
               ``itemcount`` attribute in the item, with a value of ``3`` .
            ...
            * If the existing data type is a set and if ``Value`` is also a set, then ``Value`` is added to
            ...
            ``ADD`` action is specified for a set attribute and the attribute type specified does not match
            the existing set type.  Both sets must have the same primitive data type. For example, if the
          ...
          * ``DELETE`` - Deletes an element from a set. If a set of values is specified, then those values
          are subtracted from the old set. For example, if the attribute value was the set ``[a,b,c]`` and
          the ``DELETE`` action specifies ``[a,c]`` , then the final attribute value is ``[b]`` . Specifying
    ...
    def wait_until_exists(self, *args: Any, **kwargs: Any) -> None:
        ...
        which polls. :py:meth:`DynamoDB.Client.describe_table` every 20 seconds until a successful state is
    ...
    def wait_until_not_exists(self, *args: Any, **kwargs: Any) -> None:
        ...
        :py:meth:`DynamoDB.Client.describe_table` every 20 seconds until a successful state is reached. An
        error is returned after 25 failed checks.
