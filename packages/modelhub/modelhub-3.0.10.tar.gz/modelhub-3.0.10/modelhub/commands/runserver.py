...
# REDIS_TIMEOUT at least 5 !!!!!
REDIS_TIMEOUT = 10
WATCH_DOG_MODEL_NAME = "watch_dog"
WATCH_DOG_MODEL_VER = 2
WATCH_DOG_TIMEOUT = 60 
_watch_dog_timeout = os.getenv("WATCH_DOG_TIMEOUT")
ENABLE_BATCHING = os.getenv("ENABLE_BATCHING", "false").lower()
if ENABLE_BATCHING not in ["true", "false"]: ENABLE_BATCHING = "false"
try: WATCH_DOG_TIMEOUT = int(_watch_dog_timeout)
except: pass
...
def start_watch_dog(port=8500, watch_dog_port=8000, serving_models=[]):
    ...
    class HealthCheckServer(BaseHTTPRequestHandler):
        # ### Add timeout here
        # to avoid reponse the unhealthy status or worse after a short time.
        # GPU will be occupied when a large batch task ongoing, and thus may 
        # take a long time to process the watch dog computation, even it is a 
        # very simple 1 + 1.
        dog = WatchDog(run_mode="remote",
                       hostport="127.0.0.1:%d" % port,
                       timeout_seconds=WATCH_DOG_TIMEOUT)
        ...
        for model, versions in models:
            ...
            for v in versions:
                try:
                    ...
                    self.auto_discovery.register(model.name, v, timeout=REDIS_TIMEOUT)
    ...
    def _run_server(self, models, model_config_file, port, watch_dog_port, **kwargs):
        ...
        popen = Popen(
            [
                ...
                "--enable_batching=%s" % ENABLE_BATCHING,
            ] + [
        ...
        try:
            ...
            while True:
                try:
                    wait_timeout = REDIS_TIMEOUT - 2
                    ret = popen.wait(timeout=wait_timeout)
                    # print("out", stderr)
