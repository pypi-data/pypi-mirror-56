...
def _get_requirement_name(req):
    ...
    if isinstance(req, ModuleType):
        ...
        raise Exception("Requirement {} is invalid; must be ModuleType or string".format(req))
    return name
...
def _get_distribution(req_name):
    ...
    try:
        ...
        raise Exception("Module {} was detected as a dependency, but not found as a pip installed package. Use acumos.session.Requirements to declare custom packages or map module names to pip-installable names (e.g. Requirements(req_map=dict(PIL='pillow')) )".format(req_name))
