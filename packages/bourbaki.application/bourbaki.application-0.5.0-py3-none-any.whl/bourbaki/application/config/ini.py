...
def load_ini(file):
    ...
    conf = {s: {k: _dynamic_str_parse(parser.get(s, k)) for k in parser.options(s)} for s in keys}
    ...
    if top_level:
        ...
        if len(update) < len(top_level):
            warn("The keys {} were defined at the top level in {} and also as subsections; they will be overridden by "
...
def dump_ini(conf, file):
    ...
    if (not is_str_mapping(conf)
        or any((not isinstance(v, (str, int, bool, NoneType, float, list))) and (not is_str_mapping(v))
