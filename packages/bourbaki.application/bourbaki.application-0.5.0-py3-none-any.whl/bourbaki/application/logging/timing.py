...
class TimedTaskContext:
    def __init__(self, job_name: str,
                 total_tasks: Opt[int]=None, task_units: Opt[str]=None,
                 logger_or_print_func: Opt[Union[Logger, Callable[[str], type(None)]]]=print,
                 level=PROGRESS, error_level=ERROR,
                 time_units: str = 's',
        ...
        try:
            ...
            raise ValueError("{} is not a valid time units identifer; choose one of {}"
                             .format(time_units, set(unit_names)))
        ...
        if isinstance(logger_or_print_func, Logger) or logger_or_print_func is None:
            ...
            assert callable(logger_or_print_func), "print_func must be callable or a name for a logger"
            self.logger = None
    ...
    def _elapsed_time_message(self, start=None, prefix=True):
        ...
        return "{}elapsed time {}{}".format('{}{}: '.format(self.job_name, " " + self.task_units if self.task_units else '')
                                            if prefix else '',
                                            round(elapsed, self.n_digits), self.time_units)

    def _progress_message(self):
        if self.total:
            return "{}: completed {} of {}".format(self.job_name, self.completed, self.total)
        else:
    ...
    def _timing_info_message(self, prefix=True, estimate=True, completed=None):
        ...
        if estimate and self.total:
            ...
            expected_end = datetime.now() + \
                           timedelta(**{self.timedelta_kw: remaining * rate / self.timedelta_multiplier})
    ...
    def _error_message(self):
        if self.total is not None:
            return "FAILED {}; completed {} {}of {}".format(self.job_name,
                                                            self.completed,
    ...
    def __enter__(self):
        if self.total is None:
            self.print("STARTED {} at {}".format(self.job_name, datetime.now().strftime(self.date_fmt)))
        else:
            self.print("STARTED {} {} {}at {}".format(self.job_name,
                                                      self.total,
                                                      self.task_units + ' ' if self.task_units else '',
