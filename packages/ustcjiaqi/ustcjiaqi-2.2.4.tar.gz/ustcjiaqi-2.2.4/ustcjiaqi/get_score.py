...
def get_verify_data():
    page = requests.get('http://60.167.58.71:88/ustc/TicketValidateData.csv')
    page.encoding = 'utf-8'
    data_split = page.text.replace('\r', '').replace('\n', ',').split(',')
    result = data_split[4:-3]
    nsrsbh = result[0::2]
    yc = result[1::2]
    df = {
        'zjnsrsbh': nsrsbh,
...
def post_verify_data(train, test):
    global user_id
