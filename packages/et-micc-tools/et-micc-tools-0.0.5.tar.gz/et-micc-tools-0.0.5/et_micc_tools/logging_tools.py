...
from et_micc_tools.static_vars import static_vars
...
def get_micc_logger(global_options=None): 
    ...
    into account) and to a log file ``et_micc_tools.log``.
    ...
    if not get_micc_logger.the_logger is None:
        ...
        # to this situation and log to a et_micc_tools.log file in the project directory.
    ...
    if get_micc_logger.the_logger is None:
        ...
        if global_options.clear_log:
            logfile = global_options.project_path / 'et_micc_tools.log'
            if logfile.exists():
                logfile.unlink()
            else: 
                global_options.clear_log = False
                
        # create a new logger object that will write to et_micc_tools.log 
        # in the current project directory
        p = global_options.project_path / 'et_micc_tools.log'
...
def create_logger(filepath,filemode='a'):
    """Create a logger object for et_micc_tools.
