...
class Test_ModeController(unittest.TestCase):
    ...
    def test_0init(self):
        ...
        self.assertEqual(mc.current_setpoint.name, current_setpoint.name)
    ...
    def test_1start_stop(self):
        ...
        def _handler_set_point_name(value):
            self.assertEqual(value.decode("UTF-8").lower(), mc.current_setpoint.name.lower())
            hspn.set()

        def _handler_set_point_temperature(value):
            self.assertEqual(float(value), mc.current_setpoint.set_point)
            hspt.set()
    ...
    def test_2prev_next(self):
        ...
        default_setpoint_temperature = mc.current_setpoint.set_point
        default_setpoint_name = mc.current_setpoint.name.lower()
        ...
        self.assertListEqual(set_point_temperature,
                             [default_setpoint_temperature, 21.5, 23.0, 19.5, default_setpoint_temperature, 19.5, 23.0,
                              21.5, default_setpoint_temperature])
        self.assertListEqual(set_point_names,
                             [default_setpoint_name, 'morning', 'day', 'night', default_setpoint_name, 'night', 'day',
                              'morning', default_setpoint_name])
        self.assertListEqual(program_names,
                             [default_program, 'morning', 'day', 'night', default_program, 'night', 'day',
                              'morning', default_program])

        self.assertIsNotNone(schedule_images[0])
        self.assertIsNone(schedule_images[1])
        self.assertIsNone(schedule_images[2])
        self.assertIsNone(schedule_images[3])
        self.assertIsNotNone(schedule_images[4])
        self.assertIsNone(schedule_images[5])
        self.assertIsNone(schedule_images[6])
        self.assertIsNone(schedule_images[7])
        self.assertIsNotNone(schedule_images[8])
    ...
    def test_3to_default_mode(self):
        ...
        self.assertNotEqual(default_setpoint_name, mc.current_setpoint.name.lower())
        ...
        self.assertEqual(default_setpoint_name, mc.current_setpoint.name.lower())
