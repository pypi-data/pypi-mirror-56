...
class Test_OperationController(unittest.TestCase):
    ...
    def test_2inc(self):
        ...
        self.mqtt_client.subscribe(mc.current_setpoint._topic_pub_temperature, _set_point_handler)

        spc.start()

        self.assertLess(mc.current_setpoint.set_point, spc._max_temp-2)
        for i in range((math.ceil(spc._max_temp-mc.current_setpoint.set_point)+1)*math.ceil(1/spc._step_size)):
            self.mqtt_client.publish(spc._topic_sub_up, spc._command_up)
            sph.wait(1)
            self.assertTrue(sph.is_set())
            sph.clear()
        self.assertEqual(mc.current_setpoint.set_point, spc._max_temp)
    ...
    def test_3dec(self):
        ...
        self.assertGreater(mc.current_setpoint.set_point, spc._min_temp+2)
        for i in range((math.ceil(mc.current_setpoint.set_point-spc._min_temp)+1)*math.ceil(1/spc._step_size)):
            self.mqtt_client.publish(spc._topic_sub_down, spc._command_down)
            sph.wait()
            sph.clear()
        self.assertEqual(mc.current_setpoint.set_point, spc._min_temp)
    ...
    def test_4reset(self):
        ...
        spc.start()
        mc.start()
        default_set_point = mc.current_setpoint
        mc.current_setpoint.set_point = 10
        self.assertEqual(mc.current_setpoint.set_point, 10)
        ...
        self.assertEqual(mc.current_setpoint.set_point, default_set_point.set_point)
        spc.stop()
        mc.stop()
