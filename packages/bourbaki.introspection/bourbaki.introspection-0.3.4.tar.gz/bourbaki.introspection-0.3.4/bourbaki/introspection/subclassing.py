...
# decorators
def subclass_mutator_method(wrapped_method_or_class, call_wrapped_first=True, call_wrapped_manually=False,
                            wrapped_is_method=True, concat_docs=True):
    ...
    def dec(wrapper_method):
        return SubclassMutatorMethod(wrapper_method, wrapped_method_or_class,
...
def subclass_method(wrapped_method_or_class, pass_to_wrapper_as, wrapped_is_method=True):
    def dec(wrapper_method):
        return SubclassMethod(wrapper_method, wrapped_method_or_class,
                              pass_to_wrapper_as=pass_to_wrapper_as, wrapped_is_method=wrapped_is_method)
...
class _SubclassMethodBase:
    ...
    def __init__(self, wrapper, wrapped_method_or_class, *, wrapped_is_method, concat_docs=True):
        ...
        wrapper_names, wrapped_names = validate_overrides(wrapper=self.wrapper_sig,
        ...
        update_wrapper(self, wrapper,
                       assigned=("__name__", "__qualname__", "__module__", "__annotations__", "__kwdefaults__"),
                       updated=()
                       )
    ...
    def get_wrapped_args_kwargs(self, args, kwargs):
        bound = bind_verbosely(self.wrapper_sig, args, kwargs, name=self.wrapper.__qualname__)
        ...
        wrapped_args.extend(allargs[k] for k in self.old_pos if k in allargs and k not in varargnames)

        if new_varpos:
            wrapped_args.extend(allargs[new_varpos])

        wrapped_kwargs = dict((k, allargs[k]) for k in self.old_kw if k in allargs and k not in varargnames)

        if new_varkw is not None:
            wrapped_kwargs.update(allargs[new_varkw])

        # this will throw a nice error if anything is missing
        _ = bind_verbosely(self.wrapped_sig, tuple(wrapped_args), wrapped_kwargs, name=self.wrapped.__qualname__)
...
class SubclassMutatorMethod(_SubclassMethodBase):
    ...
    def __init__(self, wrapper, wrapped_method_or_class, *,
                 wrapped_is_method=True, call_wrapped_first=True, call_wrapped_manually=False, concat_docs=True):
        super().__init__(wrapper, wrapped_method_or_class,
                         wrapped_is_method=wrapped_is_method, concat_docs=concat_docs)

        self.__signature__ = merged_signature(self.wrapper_sig, self.wrapped_sig,
                                              wrapper_is_method=True,
                                              wrapped_is_method=wrapped_is_method,
...
class SubclassMethod(_SubclassMethodBase):
    ...
    def __init__(self, wrapper, wrapped_method_or_class, *, wrapped_is_method=True, pass_to_wrapper_as=None):
        if not isinstance(pass_to_wrapper_as, str):
            if not isinstance(pass_to_wrapper_as, tuple) or not all(isinstance(n, str) for n in pass_to_wrapper_as):
                raise TypeError("`pass_to_wrapper_as` must be str or a tuple of str; got {}"
                                .format(repr(pass_to_wrapper_as)))

        super().__init__(wrapper, wrapped_method_or_class, wrapped_is_method=wrapped_is_method)
        ...
        pass_to_wrapper_kind = (tuple(self.wrapper_sig.parameters[k].kind for k in pass_to_wrapper_as)
                                if pass_multiple else self.wrapper_sig.parameters[pass_to_wrapper_as].kind)

        self.pass_to_wrapper_kind = pass_to_wrapper_kind

        kinds = pass_to_wrapper_kind if pass_multiple else (pass_to_wrapper_kind,)
        if not all(k in (Parameter.POSITIONAL_OR_KEYWORD, Parameter.KEYWORD_ONLY) for k in kinds):
            raise TypeError("wrapped result is specified to be passed to {}, which is of kind {}; only parameter kinds "
                            "{} are allowed here".format(pass_to_wrapper_as,
        ...
        wrapper_call_sig = Signature(p for p in self.wrapper_sig.parameters.values() if p.name not in pass_names)
        self.__signature__ = merged_signature(wrapper_call_sig, self.wrapped_sig,
                                              wrapper_is_method=True,
                                              wrapped_is_method=wrapped_is_method,
                                              return_arg_names=False)
