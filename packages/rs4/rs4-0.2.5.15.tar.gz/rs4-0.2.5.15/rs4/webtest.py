...
class HTTP2Response:
    def __init__ (self, r):
        self.headers = {k.decode (): v.decode () for k, v in r.headers.items ()}
...
class HTTP2:
    ...
    def get (self, url, headers = {}, allow_push = True):
        return self._request ('GET', url, headers)

    def post (self, url, data, headers = {}, allow_push = True):
        return self._request ('POST', url, data, headers)


class HTTP3Response (HTTP2Response):
    def __init__ (self, r):
        self.status = r.headers.get (b':status')
        self.reason = ''
        self.content = r.data
...
class HTTP3 (HTTP2):
    ...
    def get (self, url, headers = {}, allow_push = True, repeat = 1):
        return HTTP3Response (h3requests.get ('{}{}'.format (self.endpoint, url), headers = headers, allow_push = allow_push, repeat = repeat))

    def post (self, url, data, headers = {}, allow_push = True, repeat = 1):
        return HTTP3Response (h3requests.post ('{}{}'.format (self.endpoint, url), self.urlencode (data, False), headers = headers, allow_push = allow_push, repeat = repeat))
