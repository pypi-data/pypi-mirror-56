...
class Py2XML(object):
    ...
    def parse( self, pythonObj, objName=None ):
        ...
        :return:
        '''
        ' processes Python data structure into XML string needs objName if pythonObj is a List '
        if pythonObj == None:
            return ""
    ...
    def _PyDict2XML( self, pyDictObj, objName=None ):
        ...
        ' process Python Dict objects They can store XML attributes and / or children '
    ...
    def _PyList2XML( self, pyListObj, objName=None ):
        ' process Python List objects They have no attributes, just children Lists only hold Dicts or Strings '
        tagStr = ""    # XML string for this level
...
def main():
    ...
    class_attributes = get_attributes(Vertex)
    vert = Vertex()
    class_attributes = get_attributes(vert)
    

    python_object = {'documents': [{'formats': [{'info': {'uri': 'http://www.python.org/newness-of-python.pdf',
                     'pages': '245'}, 'type': 'pdf'}, {'info': {'uri': 'http://www.python.org/newness-of-python.html'},
                      'type': 'web'}], 'copyright': {'url': 'http://www.creativecommons.org/', 'date': 'June 24, 2009', 'type': 'CC'}, 'title': 'The Newness of Python', 'date': 'June 6, 2009', 'text': ['Python is very nice. Very, very nice.'], 'author': 'John Doe'}]}
