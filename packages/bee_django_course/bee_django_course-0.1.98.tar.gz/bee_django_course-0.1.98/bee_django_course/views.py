...
from .models import Course, Section, CourseSectionMid, Video, UserLive, UserCourse, UserCourseSection, \
    ...
    UserLiveComment, UserCourseSectionVideo
from .forms import CourseForm, SectionForm, CourseSectionForm, CourseSectionOrderForm, VideoForm, UploadImageForm, \
    ...
    CourseSectionQuestionSearchForm, UserLiveSearchForm
from .utils import JSONResponse, page_it, can_view_expired_live, live_finish, update_live_mins
...
def test(request):
    from cc import get_live_str
    playurl=get_live_str("15CCDE96E5058FCB9C33DC5901307461")
    return render(request, 'bee_django_course/test_video.html',{"playurl":playurl})
...
class CourseRedirectView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        if self.request.user.has_perm('bee_django_course.view_teach_usercoursessection') \
                or self.request.user.has_perm('bee_django_course.view_all_usercoursesection') \
                or self.request.user.has_perm('bee_django_course.view_child_usercoursesection'):
            # 跳转录播列表
...
class LiveList(ListView):
    ...
    def get_queryset(self):
        if self.request.user.has_perm("bee_django_course.view_all_userlives"):
            ...
            self.queryset = UserLive.objects.filter(user__in=user_collection, status=1)
    ...
    def get_context_data(self, **kwargs):
        ...
        if not user:
            if hasattr(settings, "COURSE_LIVE_PROVIDER_LIST"):
                ...
                provider_list = None
            context["search_form"] = UserLiveSearchForm(provider_list, {"name": name, "provider": provider})
        scope = self.request.GET.get("scope")
        offset = self.request.GET.get("offset")
        title_str = ""
        if scope in ["day", 'week', "month"]:
