...
class Client(BaseClient):
    ...
    def add_tags_to_certificate(
        ...
        the certificate on input by its Amazon Resource Name (ARN). You specify the tag by using a key-value
        pair.

        You can apply a tag to just one certificate if you want to identify a specific characteristic of
        that certificate, or you can apply the same tag to multiple certificates if you want to filter for a
        common relationship among those certificates. Similarly, you can apply the same tag to multiple
        resources if you want to specify a relationship among those resources. For example, you can add the
        same tag to an ACM certificate and an Elastic Load Balancing load balancer to indicate that they are
        both used by the same website. For more information, see `Tagging ACM certificates
        <https://docs.aws.amazon.com/acm/latest/userguide/tags.html>`__ .

        To remove one or more tags, use the  RemoveTagsFromCertificate action. To view all of the tags that
        ...
        **Request Syntax**
        ...
        :param CertificateArn: **[REQUIRED]**
          ...
          String that contains the ARN of the ACM certificate to which the tag is to be applied. This must
          be of the form:
          ...
          ::

            {
                'Certificate': {
                    ...
                    |'INVALID_PUBLIC_DOMAIN'|'DOMAIN_VALIDATION_DENIED'|'CAA_ERROR'|'PCA_LIMIT_EXCEEDED'
                    ...
                    |'PCA_INVALID_DURATION'|'PCA_ACCESS_DENIED'|'OTHER',
    ...
    def export_certificate(
        ...
        """
        Exports a private certificate issued by a private certificate authority (CA) for use anywhere. The
        exported file contains the certificate, the certificate chain, and the encrypted private 2048-bit
        RSA key associated with the public key that is embedded in the certificate. For security, you must
        ...
        **Request Syntax**
        ...
        :param Passphrase: **[REQUIRED]**
          ...
          Passphrase to associate with the encrypted exported private key. If you want to later decrypt the
          private key, you must have the passphrase. You can use the following OpenSSL command to decrypt a
          private key:
    ...
    def get_certificate(
        ...
        """
        Retrieves a certificate specified by an ARN and its certificate chain . The chain is an ordered list
        of certificates that contains the end entity certificate, intermediate certificates of subordinate
        CAs, and the root certificate in that order. The certificate and certificate chain are base64
        encoded. If you want to decode the certificate to see the individual fields, you can use OpenSSL.
    ...
    def import_certificate(
        ...
        with ACM. Note that `integrated services
        <https://docs.aws.amazon.com/acm/latest/userguide/acm-services.html>`__ allow only certificate types
        and keys they support to be associated with their resources. Further, their support differs
        depending on whether the certificate is imported into IAM or into ACM. For more information, see the
        documentation for each service. For more information about importing certificates into ACM, see
        ...
        .. note::
          ...
          <https://docs.aws.amazon.com/acm/latest/userguide/acm-renewal.html>`__ for certificates that you
          import.
        ...
        * The private key must be unencrypted. You cannot import a private key that is protected by a
        password or a passphrase.

        * If the certificate you are importing is not self-signed, you must enter its certificate chain.

        * If a certificate chain is included, the issuer must be the subject of one of the certificates in
        the chain.
        ...
        * To import a new certificate, omit the ``CertificateArn`` argument. Include this argument only when
        you want to replace a previously imported certifica
        ...
        * When you import a certificate by using an SDK, you must specify the certificate, the certificate
        chain, and the private key files in the manner required by the programming language you're using.

        * The cryptographic algorithm of an imported certificate must match the algorithm of the signing CA.
        For example, if the signing CA key type is RSA, then the certificate key type must also be RSA.

        This operation returns the `Amazon Resource Name (ARN)
        <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`__ of the imported
        certificate.
        ...
        **Request Syntax**
        ...
        :param CertificateArn:
          ...
          <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`__ of an imported
          certificate to replace. To import a new certificate, omit this field.
    ...
    def list_certificates(
        ...
        **Request Syntax**
        ::
          ...
          response = client.list_certificates(
              ...
              Includes={
                  'extendedKeyUsage': [
                      'TLS_WEB_SERVER_AUTHENTICATION'|'TLS_WEB_CLIENT_AUTHENTICATION'|'CODE_SIGNING'
                      |'EMAIL_PROTECTION'|'TIME_STAMPING'|'OCSP_SIGNING'|'IPSEC_END_SYSTEM'|'IPSEC_TUNNEL'
                      |'IPSEC_USER'|'ANY'|'NONE'|'CUSTOM',
                  ],
                  'keyUsage': [
                      'DIGITAL_SIGNATURE'|'NON_REPUDIATION'|'KEY_ENCIPHERMENT'|'DATA_ENCIPHERMENT'
                      |'KEY_AGREEMENT'|'CERTIFICATE_SIGNING'|'CRL_SIGNING'|'ENCIPHER_ONLY'|'DECIPHER_ONLY'
                      |'ANY'|'CUSTOM',
                  ],
                  'keyTypes': [
                      'RSA_2048'|'RSA_1024'|'RSA_4096'|'EC_prime256v1'|'EC_secp384r1'|'EC_secp521r1',
        ...
        :param NextToken:
          ...
          Use this parameter only when paginating results and only in a subsequent request after you receive
          a response with truncated results. Set it to the value of ``NextToken`` from the response you just
          received.
    ...
    def remove_tags_from_certificate(
        ...
        have been applied to a specific ACM certificate, use the  ListTagsForCertificate action.
    ...
    def renew_certificate(self, CertificateArn: str) -> None:
        ...
        with this operation. In order to renew your ACM PCA certificates with ACM, you must first `grant the
        ACM service principal permission to do so
        ...
        <https://docs.aws.amazon.com/acm/latest/userguide/manual-renewal.html>`__ in the ACM User Guide.
    ...
    def request_certificate(
        ...
        specify a fully qualified domain name (FQDN) in the ``DomainName`` parameter. You can also specify
        ...
        requesting a public certificate, each domain name that you specify must be validated to verify that
        you own or control the domain. You can use `DNS validation
        <https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-validate-dns.html>`__ or `email validation
        <https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-validate-email.html>`__ . We recommend that
        you use DNS validation. ACM issues public certificates after receiving approval from the domain
        owner.
        ...
        **Request Syntax**
        ...
        :param DomainName: **[REQUIRED]**
          ...
          certificate. Use an asterisk (*) to create a wildcard certificate that protects several sites in
          the same domain. For example, *.example.com protects www.example.com, site.example.com, and
          images.example.com.

          The first domain name you enter cannot exceed 64 octets, including periods. Each subsequent
          Subject Alternative Name (SAN), however, can be up to 253 octets in length.
        ...
        :param ValidationMethod:
          ...
          <https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-validate-dns.html>`__ or `validate with
          email <https://docs.aws.amazon.com/acm/latest/userguide/gs-acm-validate-email.html>`__ . We
        ...
        :param SubjectAlternativeNames:
          ...
          names that you can add to an ACM certificate is 100. However, the initial limit is 10 domain
          names. If you need more than 10 names, you must request a limit increase. For more information,
          see `Limits <https://docs.aws.amazon.com/acm/latest/userguide/acm-limits.html>`__ .

          The maximum length of a SAN DNS name is 253 octets. The name is made up of multiple labels
          separated by periods. No label can be longer than 63 octets. Consider the following examples:
        ...
        :param IdempotencyToken:
          ...
          times with the same idempotency token within one hour, ACM recognizes that you are requesting only
          one certificate and will issue only one. If you change the idempotency token for each call, ACM
        ...
        :param CertificateAuthorityArn:
          ...
          issue the certificate. If you do not provide an ARN and you are trying to request a private
          certificate, ACM will attempt to issue a public certificate. For more information about private
          CAs, see the `AWS Certificate Manager Private Certificate Authority (PCA)
          <https://docs.aws.amazon.com/acm-pca/latest/userguide/PcaWelcome.html>`__ user guide. The ARN must
          have the following form:
    ...
    def resend_validation_email(
        ...
        then clicking **I Approve** . However, the validation email can be blocked by spam filters.
        Therefore, if you do not receive the original mail, you can request that the mail be resent within
        72 hours of requesting the ACM certificate. If more than 72 hours have elapsed since your original
        request or since your last attempt to resend validation mail, you must request a new certificate.
        For more information about setting up your contact email addresses, see `Configure Email for your
        Domain <https://docs.aws.amazon.com/acm/latest/userguide/setup-email.html>`__ .
        ...
        **Request Syntax**
        ...
        :param CertificateArn: **[REQUIRED]**
          ...
          parameter causes email to be sent to all top-level domains you specified in the certificate
        ...
        :param ValidationDomain: **[REQUIRED]**
          ...
          The base validation domain that will act as the suffix of the email addresses that are used to
          send the emails. This must be the same as the ``Domain`` value or a superdomain of the ``Domain``
          value. For example, if you requested a certificate for ``site.subdomain.example.com`` and specify
          a **ValidationDomain** of ``subdomain.example.com`` , ACM sends email to the domain registrant,
