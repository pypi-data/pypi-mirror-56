...
def get_experiment_kernel(noise_type: str, noise_var: float, sz: tuple = np.array((101, 101))):
    ...
    """
    # if noiseType == 0 / default
    kernel = np.array([[1]])
    noise_types = ['g0', 'g1', 'g2', 'g3', 'g4', 'g1w', 'g2w', 'g3w', 'g4w']
    ...
    if noise_type[0:2] == 'g1':
        ...
        # Define in case we want to combine with white noise
        alpha = 0.001
    ...
    if len(noise_type) > 2 and noise_type[2] == 'w':
        kalpha = np.sqrt((1 - alpha) + alpha * abs(fft2(kernel, (sz[0], sz[1]))) ** 2)
