...
def  run(args=None,sourcedb=None,targetdb=None):
    ...
    logger.info('源数据库的信息{},目标数据库的信息{}'.format(sourcedb,targetdb))
    ...
    for table , table_desc_sql in same_table_source_dict.items():
        ...
        if   (args is not None  and  not args.only_fields) or sourcedb is not None:
        ...
        if (args is not None and not  args.only_index ) or sourcedb is not None:
