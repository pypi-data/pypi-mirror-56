'''
prompt translate for free as in beer
'''
...
def _rl_prompt_tr(*args, **kwargs):
    ''' be nice and throttle'''
    LOGGER.info(' rate limiting 3 calls/2 secs... ')
    return _prompt_tr(*args, **kwargs)


@with_func_attrs(calls=0, call_tick=-1)
def prompt_tr(*args, **kwargs):
    ''' exempt first 200 calls from rate limiting '''

    # increase calls unto 210
    if prompt_tr.calls < 210:
        prompt_tr.calls += 1

    # reset rate limit if the last call was 2 minutes ago
    tick = time()
    if tick - prompt_tr.calls > 120:
        prompt_tr.calls = 1
    prompt_tr.call_tick = tick

    if prompt_tr.calls < 200:
        return _prompt_tr(*args, **kwargs)

    return _rl_prompt_tr(*args, **kwargs)
...
def test_sanity(to_lang):
    ...
    text = 'test ' + numb
    assert numb in prompt_tr(text, to_lang=to_lang)


def test_calls():
    ''' test calls '''
    calls = prompt_tr.calls
    _ = prompt_tr('test ')
    assert prompt_tr.calls == calls + 1
...
def main():  # pragma: no cover
    ...
    for to_lang in ['zh', 'de', 'fr', ]:
        print(f'{to_lang}: {prompt_tr(text, to_lang=to_lang)}')
        if not text1:
            print(f'{to_lang}: {prompt_tr(text1, to_lang=to_lang)}')
