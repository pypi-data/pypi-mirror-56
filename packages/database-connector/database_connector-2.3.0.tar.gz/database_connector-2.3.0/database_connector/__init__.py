...
class Database_Connector(object):
    ...
    def __init__(self, database, config_file):
        ...
        try:
            ...
            raise ValueError("{e}: database: {database}, config_file: {config_file}".format(
    ...
    def connect(self):
        ...
        if self.type == "mysql":
            ...
            return cx_Oracle.connect('{user}/{password}@{host}:{port}/{service}'.format(
                user=self.user, password=self.password, host=self.host, port=self.port, service=self.service))
...
password: {password}
type: {type}
""".format(database=self.database, host=self.host, port=self.port, user=self.user, password=self.password, type=self.type)
