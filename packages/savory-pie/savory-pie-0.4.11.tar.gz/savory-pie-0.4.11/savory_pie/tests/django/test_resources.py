...
class ModelResourceTest(unittest.TestCase):
    ...
    def test_complex_put(self):
        user = User()
        user._meta.get_field().name = 'name'
    ...
    def _dict_compare(self, actual, expected):
        for item1, item2 in zip(sorted(expected.items()), sorted(actual.items())):
    ...
    def test_put_with_missing_required_field(self):
        ...
        errors = json.loads(resp_content)
...
class QuerySetResourceTest(unittest.TestCase):
    ...
    def test_query_set_get(self):
        ...
        self.assertEqual([self.remove_hash(x) for x in data], [
    ...
    def test_get_distinct(self):
        ...
        self.assertEqual([self.remove_hash(x) for x in data['objects']], [
            {'resourceUri': 'uri://users/1', 'name': 'Alice', 'age': 31},
        ])
...
class SchemaResourceTest(unittest.TestCase):
    ...
    def do_assert_equal(self, key, assert_type=''):
        actual = self.do_get()[key]
        expected = user_resource_schema[key]
        if assert_type == 'List':
            actual = sorted(actual)
            expected = sorted(expected)

        getattr(self, 'assert{}Equal'.format(assert_type))(actual, expected)
