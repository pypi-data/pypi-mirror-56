...
from savory_pie.utils import get_exception_message
...
class QuerySetResource(Resource):
    ...
    def post(self, ctx, source_dict):
        ...
        if any(isinstance(field, ReverseField) for field in resource.fields):
...
class ModelResource(with_metaclass(DirtyInitializerMetaClass, Resource)):
    """
    ...
    def put(self, ctx, source_dict, save=True, skip_validation=False):
        ...
        try:
            ...
            for L in traceback.format_exc().splitlines():
                logger.debug(L)
            raise ValidationError(self, {'invalidFieldData': get_exception_message(e)})
