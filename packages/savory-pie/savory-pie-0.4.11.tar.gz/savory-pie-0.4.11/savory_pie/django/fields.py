...
class DjangoField(base_fields.Field):
    def schema(self, ctx, **kwargs):
        ...
        if self._field:
            """
            We found an issue where Savory-Pie Schemas from ForeignKey models weren't coming back 
            correctly after upgrading to Django 1.8. It turns out the underlying ManyToOneRel
            class has been updated and no longer inherits from a common model that included
            all of the default validation attributes so the original logic was no longer rugged
            enough and was failing silently. We now check to ensure an attribute is present
            before attempting to add it to the schema response.
            
            JIRA: https://ruelala.atlassian.net/browse/WEB-2298
            """
            _schema = {}
            if hasattr(self._field, 'blank'):
                _schema['blank'] = self._field.blank
            if hasattr(self._field, 'help_text'):
                _schema['helpText'] = self._field.help_text
            if 'get_default' in dir(self._field):
                _schema['default'] = ctx.formatter.to_api_value(type(self._field.get_default()), self._field.get_default())
            if hasattr(self._field, 'null'):
                _schema['nullable'] = self._field.null
            if hasattr(self._field, 'editable'):
                _schema['readonly'] = not self._field.editable
            if hasattr(self._field, 'unique'):
                _schema['unique'] = self._field.unique
            if hasattr(self._field, 'choices') and self._field.choices:
                _schema['choices'] = self._field.choices
            if hasattr(_schema, 'helpText') and isinstance(_schema['helpText'], Promise):
                _schema['helpText'] = unicode(_schema['helpText'])
        else:
            _schema = {}
        # "adding" to combine dicts is no longer supported in python 3
...
class AttributeFieldWithModel(AttributeField):
    ...
    def set_recursively(self, ctx, root_obj, attrs, source_dict):
        ...
        def get_model_params(target_type, ctx=ctx, source_dict=source_dict, extras=self._extras):
            ...
            params = {}
            fields = [(field.name, field) for field in target_type._meta.fields]
...
class SubModelResourceField(base_fields.SubObjectResourceField, DjangoField):
    ...
    def schema(self, ctx, **kwargs):
        kwargs.update({'schema': {'type': 'related', 'relatedType': 'to_one'}})
    ...
    def pre_save(self, model):
        ...
        '''
        relation = self._get_field(model)
        if relation:
...
class ReverseRelatedManagerField(with_metaclass(ResourceClassUser)):
    """
