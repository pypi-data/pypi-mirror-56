...
def convert_response_bytes_to_string(response):
    if isinstance(response.content, bytes):
        resp_content = response.content.decode(getattr(response, 'charset', 'utf-8'))
    else:
...
class BatchViewTest(TestCase):
    ...
    def test_put_nocontent_batch(self):
        ...
        response_json = json.loads(convert_response_bytes_to_string(response))
    ...
    def test_put_precondition_batch(self):
        ...
        data = json.loads(convert_response_bytes_to_string(response))['data']
        self.assertEqual(len(data), 1)
...
class ViewTest(TestCase):
    ...
    def test_get_success_streaming(self):
        ...
        self.assertEqual(response.status_code, 200)
        content_array = [x.decode(u'utf-8') for x in response.streaming_content]
        self.assertEqual(''.join(content_array), '{"foo": "bar"}')
        self.assertTrue(root_resource.get.called)
        self.assertIsNotNone(root_resource.get.call_args_list[0].request)
    ...
    def test_post_success_returns_resource_when_indicated(self):
        ...
        self.assertContains(response, '{"key": "value"}', status_code=201)
        self.assertIsNotNone(root_resource.post.call_args_list[0].request)
    ...
    def test_post_with_collision_one(self):
        ...
        self.assertContains(response, '{"resource": "http://localhost/api/"}', status_code=409)
    ...
    def test_post_with_exception(self, logger):
        ...
        content = json.loads(convert_response_bytes_to_string(response))
    ...
    def test_set_header(self):
        ...
        self.assertContains(response, '{"foo2": "bar2"}')
...
class DjangoFPramsTest(TestCase):
