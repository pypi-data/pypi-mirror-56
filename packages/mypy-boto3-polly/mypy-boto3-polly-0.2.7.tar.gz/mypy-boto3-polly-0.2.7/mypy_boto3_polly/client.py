"Main interface for polly Client"
...
class Client(BaseClient):
    ...
    def describe_voices(
        ...
        ASCII version of the voice name.
        ...
        You can optionally specify a language code to filter the available voices. For example, if you
        ...
        **Request Syntax**
        ...
        :param Engine:
          ...
          Specifies the engine (``standard`` or ``neural`` ) used by Amazon Polly when processing input text
        ...
        :param IncludeAdditionalLanguageCodes:
          ...
          an additional language. For instance, if you request all languages that use US English (es-US),
          and there is an Italian voice that speaks both Italian (it-IT) and US English, that voice will be
                ...
                'Voices': [
                    {
                        ...
                        |'Conchita'|'Cristiano'|'Dora'|'Emma'|'Enrique'|'Ewa'|'Filiz'|'Geraint'|'Giorgio'
                        |'Gwyneth'|'Hans'|'Ines'|'Ivy'|'Jacek'|'Jan'|'Joanna'|'Joey'|'Justin'|'Karl'
                        |'Kendra'|'Kimberly'|'Lea'|'Liv'|'Lotte'|'Lucia'|'Lupe'|'Mads'|'Maja'|'Marlene'
                        |'Mathieu'|'Matthew'|'Maxim'|'Mia'|'Miguel'|'Mizuki'|'Naja'|'Nicole'|'Penelope'
                        ...
                        'AdditionalLanguageCodes': [
                            ...
                            |'en-US'|'es-ES'|'es-MX'|'es-US'|'fr-CA'|'fr-FR'|'is-IS'|'it-IT'|'ja-JP'|'hi-IN'
                            |'ko-KR'|'nb-NO'|'nl-NL'|'pl-PL'|'pt-BR'|'pt-PT'|'ro-RO'|'ru-RU'|'sv-SE'
                            |'tr-TR',
                        ],
                        'SupportedEngines': [
                            'standard'|'neural',
            ...
            - **Voices** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Name** *(string) --*
                  ...
                  Name of the voice (for example, Salli, Kendra, etc.). This provides a human readable voice
                ...
                - **AdditionalLanguageCodes** *(list) --*
                  ...
                  used for that language. Since Aditi is bilingual and fluent in both Indian English and
          ...
          - *(dict) --*
            ...
            - **LexiconAttributes** *(dict) --*
              ...
              - **LanguageCode** *(string) --*
                ...
                Language code that the lexicon applies to. A lexicon with a language code such as "en" would
                be applied to all English languages (en-GB, en-US, en-AUS, en-WLS, and so on.
          ...
          ::

            {
                'SynthesisTask': {
                    ...
                    'Aditi'|'Amy'|'Astrid'|'Bianca'|'Brian'|'Camila'|'Carla'|'Carmen'|'Celine'|'Chantal'
                    |'Conchita'|'Cristiano'|'Dora'|'Emma'|'Enrique'|'Ewa'|'Filiz'|'Geraint'|'Giorgio'
                    |'Gwyneth'|'Hans'|'Ines'|'Ivy'|'Jacek'|'Jan'|'Joanna'|'Joey'|'Justin'|'Karl'|'Kendra'
                    |'Kimberly'|'Lea'|'Liv'|'Lotte'|'Lucia'|'Lupe'|'Mads'|'Maja'|'Marlene'|'Mathieu'
                    |'Matthew'|'Maxim'|'Mia'|'Miguel'|'Mizuki'|'Naja'|'Nicole'|'Penelope'|'Raveena'
                    |'Ricardo'|'Ruben'|'Russell'|'Salli'|'Seoyeon'|'Takumi'|'Tatyana'|'Vicki'|'Vitoria'
                    |'Zeina'|'Zhiyu',
                    'LanguageCode':
                    'arb'|'cmn-CN'|'cy-GB'|'da-DK'|'de-DE'|'en-AU'|'en-GB'|'en-GB-WLS'|'en-IN'|'en-US'
                    |'es-ES'|'es-MX'|'es-US'|'fr-CA'|'fr-FR'|'is-IS'|'it-IT'|'ja-JP'|'hi-IN'|'ko-KR'|'nb-NO'
    ...
    def put_lexicon(self, Name: str, Content: str) -> Dict[str, Any]:
        ...
        the region, it is overwritten by the new lexicon. Lexicon operations have eventual consistency,
    ...
    def start_speech_synthesis_task(
        ...
        SpeechMarkTypes: List[str] = None,
        TextType: str = None,
    ) -> ClientStartSpeechSynthesisTaskResponseTypeDef:
        """
        Allows the creation of an asynchronous synthesis task, by starting a new ``SpeechSynthesisTask`` .
        This operation requires all the standard information needed for speech synthesis, plus the name of
        an Amazon S3 bucket for the service to store the output of the synthesis task and two optional
        ...
        **Request Syntax**
        ::
          ...
          response = client.start_speech_synthesis_task(
              ...
              VoiceId=
                  ...
                  |'Maxim'|'Mia'|'Miguel'|'Mizuki'|'Naja'|'Nicole'|'Penelope'|'Raveena'|'Ricardo'|'Ruben'
                  |'Russell'|'Salli'|'Seoyeon'|'Takumi'|'Tatyana'|'Vicki'|'Vitoria'|'Zeina'|'Zhiyu'
          )
        :type Engine: string
        :param Engine:

          Specifies the engine (``standard`` or ``neural`` ) for Amazon Polly to use when processing input
          text for speech synthesis. Using a voice that is not supported for the engine selected will result
          in an error.
        ...
        :param LanguageCode:
          ...
          `DescribeVoices <https://docs.aws.amazon.com/polly/latest/dg/API_DescribeVoices.html>`__ operation
          for the ``LanguageCode`` parameter. For example, if no language code is specified, Aditi will use
          Indian English rather than Hindi.
        ...
        :param SampleRate:
          ...
          The valid values for mp3 and ogg_vorbis are "8000", "16000", "22050", and "24000". The default
          value for standard voices is "22050". The default value for neural voices is "24000".
    ...
    def synthesize_speech(
        ...
        see `How it Works <https://docs.aws.amazon.com/polly/latest/dg/how-text-to-speech-works.html>`__ .
        ...
        **Request Syntax**
        ...
        :param Text: **[REQUIRED]**
          ...
          Input text to synthesize. If you specify ``ssml`` as the ``TextType`` , follow the SSML format for
          the input text.
        ...
        :param TextType:
          ...
          information, see `Using SSML <https://docs.aws.amazon.com/polly/latest/dg/ssml.html>`__ .

        :type VoiceId: string
        :param VoiceId: **[REQUIRED]**

          Voice ID to use for the synthesis. You can get a list of available voice IDs by calling the
          `DescribeVoices <https://docs.aws.amazon.com/polly/latest/dg/API_DescribeVoices.html>`__
          operation.
