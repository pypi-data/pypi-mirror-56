...
class Utils(object):
    ...
    default_full_frmttr = '%(asctime)s - %(name)s - %(levelname)s - %(pathname)s - %(message)s'
    ...
    def old_init(self, conf_file_path, trace_enabled = False):
        ...
        self.level = self.conf.get('log_level', self.default_level)
        self.log_file = self.conf.get("log_file", 'logs/default.log')
        # Only add basicConfig if neither file nor console config are present
        # Rely on init_logger to setup everyting
        # File config: initialize directory
        if self.log_file != 'None':
            print('init() found file config, initializing directory')
        ...
        # No file config, no console config
        elif self.conf.get("log_to_console_level") == None:
            print('init() no file/console config, setting basicConfig')
            logging.basicConfig(format=frmt,level=self.level)
        
    def init_logger(self, logger_name='root'):
        print('init_logger() name %s'%logger_name)
        if logger_name != 'root':
        ...
        if self.log_file != 'None':
            ...
            print('init_logger() added file logger')
        console_level = self.conf.get("log_to_console_level")
        if console_level:
            ch = logging.StreamHandler()
            ch.setFormatter(frmttr)
            ch.setLevel(console_level)
            logger.addHandler(ch)
            print('init_logger() added console logger')
