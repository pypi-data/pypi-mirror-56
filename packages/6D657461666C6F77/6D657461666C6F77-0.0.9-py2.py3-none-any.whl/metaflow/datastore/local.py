...
class LocalDataStore(MetaflowDataStore):
    ...
    # The LocalDataStore can be used both to save artifacts and to store execution metadata
    # (used by the client). In the latter case, we reserve a directory for the metadata to be
    # stored.
    METADATA_DIR = '_meta'
    ...
    def get_datastore_root_from_config(cls, echo, create_on_absent=True):
        ...
        if result is None:
            ...
            if top_level_reached:
                if create_on_absent:
                    ...
                    echo('Creating local metaflow directory in current directory (%s)' % orig_path,
        ...
        # echo('Using %s as Metaflow\'s local data and metadata directory' % result,
        #      fg='green', bold=True)
        return result
