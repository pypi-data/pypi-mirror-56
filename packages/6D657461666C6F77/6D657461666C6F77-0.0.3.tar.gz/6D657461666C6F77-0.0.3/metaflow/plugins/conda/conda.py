...
class Conda(object):
    ...
    def __init__(self):
        ...
        if self._bin is None:
            raise InvalidEnvironmentException("No conda installation found. "
                                              "Install conda first.")
        if LooseVersion(self._info()['conda_version']) < LooseVersion('4.7.0'):
            raise InvalidEnvironmentException("Conda version 4.7.0 or newer "
                                              "is required.")
    ...
    def _call(self, args):
        return subprocess.check_output(args, stderr=open(os.devnull, 'wb')).strip()
