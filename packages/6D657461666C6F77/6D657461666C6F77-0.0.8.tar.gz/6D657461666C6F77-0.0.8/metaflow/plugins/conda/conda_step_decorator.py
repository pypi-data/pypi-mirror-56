...
class CondaStepDecorator(StepDecorator):
    ...
    def _resolve_step_environment(self, ds_root, force=False):
        env_id = self._env_id()
        cached_deps = read_conda_manifest(ds_root)
        ...
        if force or env_id not in cached_deps or 'cache_urls' not in cached_deps[env_id]:
            ...
            write_to_conda_manifest(ds_root, env_id, payload)
    ...
    def _architecture(self, decos):
        ...
        if platform.system() == 'Linux':
            ...
            raise InvalidEnvironmentException("@conda decorator is not supported "
    ...
    def step_init(self, flow, graph, step, decos, environment, datastore, logger):
        ...
        def logger_sane(line, **kwargs):
            logger(line)
        self.local_root = LocalDataStore.get_datastore_root_from_config(logger_sane)
