...
from metaflow.metaflow_config import update_var, get_var, \
    METADATA_SERVICE_NUM_RETRIES, METADATA_SERVICE_HEADERS
...
class ServiceMetadataProvider(MetadataProvider):
    ...
    def compute_info(cls, val):
        v = val.rstrip('/')
        update_var('METADATA_SERVICE_URL', v)
        return v

    @classmethod
    def default_info(cls):
        return get_var('METADATA_SERVICE_URL')
    ...
    def _request(cls, monitor, path, data=None):
        url = '%s/%s' % (get_var('METADATA_SERVICE_URL'), path.lstrip('/'))
