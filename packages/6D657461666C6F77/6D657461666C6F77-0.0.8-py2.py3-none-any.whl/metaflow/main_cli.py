...
def status():
    ...
    # Throw an exception
    if path is None:
        raise click.FileError(click.style('"%s"' % DATASTORE_LOCAL_DIR,
                                          fg='red'),
...
def get_tutorial_metadata(tutorial_path):
    ...
    with open(os.path.join(tutorial_path, 'README.md')) as readme:
       ...
       # Skip the leading '#' for first line.
       metadata['episode'] = header_toks[0][len('# Episode'):].strip()
       metadata['title'] = header_toks[1].strip()

       assert lines[1].startswith('-') and lines[4].startswith('-')
       metadata['description'] = lines[2]

       # Can use the following loop to parse other artifacts.
       for idx in range(4, len(lines)):
           line = lines[idx]
           if line.startswith('#### To play this episode:'):
               metadata['play'] = lines[idx+1:]
               break
    return metadata
...
def list():
    ...
    for name in get_all_episodes():
        ...
        echo('* {0: <13} '.format(metadata['episode']),
             fg='cyan',
             nl=False)
...
def print_without_markdown(lines):
...
def info(episode):
    ...
    print_without_markdown(metadata['description'])
    echo('\nTo play:', fg='cyan', bold=True)
    print_without_markdown(metadata['play'])
