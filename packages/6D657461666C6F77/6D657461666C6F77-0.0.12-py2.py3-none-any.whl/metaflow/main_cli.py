...
def list():
    ...
    echo('\nTo pull your favorite episode(s), type: ')
...
def persist_env(env_dict, profile):
    ...
    echo('Configuration successfully written to ', nl=False)
    echo('"%s"' % path, fg='cyan')
...
@click.option('--profile', '-p', default='',
              help="Optional user profile to allow storing multiple "
                   ...
                   "switch between profile configuration(s).")
def aws(profile):
    if click.confirm('Have you setup your AWS credentials and configuration?'):
        env_dict = {}
        ...
        if use_s3:
            ...
            echo('\n\tAWS Batch', fg='cyan')
            job_queue = click.prompt('\tPlease enter the job queue to use for '
                                     'batch')
            default_image =\
                click.prompt('\tPlease enter the default container image to '
                             'use.')
            container_registry =\
        ...
        if use_s3:
            use_conda = click.prompt('\nDo you want to use conda for '
                                     'dependency management?', default=True)
            if use_conda:
        ...
        echo('Please set them up first using ', nl=False)
        echo('"aws configure".', fg='cyan')
