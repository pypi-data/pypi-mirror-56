...
from dsa.dsa_problem import DsaProblem
...
def to_markdown(html_text):
    ...
    for tag in tt:
        tag.name = 'i'
        tag.attrs = {}
    return tomd.convert(html.unescape(soup.decode())).strip()
...
class Codeforces:
    ...
    def get_problem_from_url(self, url):
        problem = Problem()
        problem.src_url, problem.src_status_url = Codeforces.parse(url)
        # print(problem.src_url)
        ...
        if note:
            ...
            for i in range(len(inputs)):
                if i<len(explanations):
                    ...
                    # print(problem.testcases[i].explanation)
        ...
        if submissions_cpp:
            ...
            CLog.info("    Total visible testcases: %s" % len(problem.testcases))
...
if __name__ == '__main__':
    ...
    dsa_problem = cf.get_problem_from_url("1257/D")
