...
class Calendar(sortedcontainers.SortedSet):
    ...
    def filter(self, func=None, *, year=None, semester=None, quarter=None, month=None, week=None, weekday=None):
        ...
        if week is not None: 
            dates = list(filter(lambda date: utils.isocalendar(date)[1] == week, dates))
        if weekday is not None: 
            dates = list(filter(lambda date: utils.weekday(date) == weekday, dates))
    ...
    def weekdays(self):
        ...
        filtered : Calendar
        """
        return self.filter(lambda date: utils.weekday(date) not in [5, 6])
    ...
    def weekends(self):
        ...
        filtered : Calendar
        """
        return self.filter(lambda date: utils.weekday(date) in [5, 6])
    ...
    def dayof(self, date, frequency=None, *, base=1):
        ...
        if frequency.lower() in ["week", "w"]:
            return self.filter(year=date.year, week=utils.isocalendar(date)[1]).index(date) + base
    ...
    def daysto(self, to=None, *, asof=None):
        ...
        if to.lower() in ["we", "week-end", "week end"]: 
            return len(self.filter(lambda d: d.year == asof.year and utils.isocalendar(d)[1] == utils.isocalendar(asof)[1] and d >= asof)) - 1
    ...
    def groupby(self, grouper):
        ...
        if isinstance(grouper, str):
            if grouper.lower() in ["w", "week"]:
                return self.groupby(lambda date: (date.year, utils.isocalendar(date)[1]))
