...
class Documento(DTE):
    ...
    def __init__(self, vals, resumen=False):
        ...
        util.set_from_keys(self, vals, priorizar=['MntIVA', 'IVA'])
...
#                lines['RUTMandante']
            ...
            if self.CodImpAdic:
                lines['CodImpAdic'] = self.CodImpAdic
            else:
    ...
    def Detalle(self, vals=[]):
        ...
        for d in vals:
            ...
            valor_iva = 19 #self.Emisor.ValorIva if self.Emisor else 19
            linea = LineaDetalle(
                            d,
                            self.TpoMoneda,
                            valor_iva=valor_iva,
    ...
    def ImptoReten(self, vals):
        for val in vals:
            tax = {
                 ...
                 'Retencion': val.get('Retencion', self.TasaImp),
            }
            iva = Impuestos(tax)
            impuesto = LineaImpuesto({
                                    'tax_id': iva,
    ...
    @property
    def MntExe(self):
        if not self.resumen and not hasattr(self, '_mnt_exe')\
                and self.Detalle:
            self._compute_monto()
        if not hasattr(self, '_mnt_exe'):
            return 0
        if self.DscRcgGlobal:
            return self._aplicar_dsc_rcg(self._mnt_exe, False, True)
    ...
    @property
    def MntNeto(self):
        if not self.resumen and not hasattr(self, '_mnt_neto')\
                and self.Detalle:
            self._compute_monto()
    ...
    @property
    def MntTotal(self):
        if not hasattr(self, '_mnt_total') and \
         self.MntExe == 0 and self.MntNeto == 0:
            return 0
        elif self.MntExe > 0 and (not hasattr(self, '_mnt_total') \
                                  or self._mnt_total == 0):
            self._compute_monto()
        if (self.MntNeto > 0 and self.MntIVA == 0) or \
                not hasattr(self, '_mnt_total'):
            self._compute_monto()
        if self.DscRcgGlobal:
            return (self.MntNeto + self.MntIVA + self.MntExe)
...
, en su ficha de receptor")
    ...
    def _compute_monto(self):
        monto_iva = 0 if self.Detalle else self.MntIVA
        monto_neto = 0 if self.Detalle else self.MntNeto
        monto_exe = 0 if self.Detalle else self.MntExe
        otros_imp = 0
        activo_fijo = 0
        iva_activo_fijo = 0
        if not hasattr(self, '_lineas_detalle'):
            ...
            _lineas_detalle = self._lineas_detalle
        for line in _lineas_detalle:
            if line.IndExe:
                monto_exe += line.MontoItem
        if not self.resumen and monto_iva == 0:
            for li in self.impuestos:
                if li.tax_id.TasaImp > 0:
                    ...
                    if li.ActivoFijo:
                        activo_fijo += li.base
                        iva_activo_fijo += li.MontoImp
                    elif li.tax_id.es_adicional:
                        otros_imp += li.MontoImp
                    else:
                        monto_iva += li.MontoImp
                        monto_neto += li.base
        if self.DscRcgGlobal:
            monto_neto = self._aplicar_dsc_rcg(monto_neto)
            monto_iva = self._aplicar_dsc_rcg(monto_iva)
            otros_imp = self._aplicar_dsc_rcg(otros_imp)
        #self.MntActivoFijo = activo_fijo
        self.MntIVAActivoFijo = iva_activo_fijo
        self.MntExe = monto_exe
        self.MntNeto = monto_neto
        self.MntIVA = monto_iva
        self.MntTotal = monto_neto + monto_iva + monto_exe + otros_imp
