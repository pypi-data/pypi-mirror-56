...
from . import (Sparse, Tensor, BackendType, ConvolutionMode, PoolingType,
...
from .parameters_serializer import (serialize_parameters,
...
class Model(ModelBase):
    ...
    def __init__(self, filename = None, backend = BackendType.Software):
        ...
        :param filename: path of the YAML file containing the model
         architecture, or a serialized Model.
         If None, an empty sequential model will be created.
        :param backend: Backend to run the model on
        :type filename: str
    ...
    def get_statistics(self):
        ...
        :return: a dictionary of LayerStatistics indexed by layer_name
        """
        self._layers_stats = {}
    ...
    def _check_hardware_compatibility(self, layer_index):
        ...
        if layer.parameters.layer_type == LayerType.FullyConnected:
            ...
            if layer.parameters.activations_params.threshold_fire_bits not in [1, 2, 4]:
                raise ValueError("Unsupported threshold_fire_bits, supported "
                                 "values are [1, 2, 4]")
    ...
    def _build_model(self, filename):
        ...
        for layer_description in layers:
            ...
            if "Parameters" not in layer_description:
               raise ValueError("Invalid model configuration: "
                                "missing 'Parameters' section in layer " + name)
        ...
        # load model weights
        self.load_weights(os.path.dirname(filename), yaml_content["Name"])
    ...
    def predict(self, input, num_classes=None):
        ...
        :param input: a Sparse object of shape (w, h, f) containing the
         events, or a numpy.ndarray of shape (n, h, w, c) containing the n
         images (grayscale (c=1) or RGB (c=3)).
        :param num_classes: optional parameter (defaults to the number of
         neurons in the last layer)
        :type input: Sparse or numpy.ndarray
        :type num_classes: integer

        :return: a numpy.ndarray of shape (n)
        """
        ...
        if isinstance(input, np.ndarray):
            if input.flags['C_CONTIGUOUS']:
                ...
                tensor = Tensor(np.ascontiguousarray(input))
            labels = super(Model, self).predict(tensor, num_classes)
    ...
    def forward(self, input, do_learning, input_labels=None):
        """
        Forwards a set of images or events through the model.

        Forwards input data (images or events) through the model and returns
        the outputs, including spike values. If ``do_learning`` is set to
        ``True``, the learnable layers' weights are updated according to the
        learning rules.

        :param input: a Sparse object of shape (w, h, f) containing the
         events, or a numpy.ndarray of shape (n, h, w, c) containing the n
         images (grayscale (c=1) or RGB (c=3)).
        :param do_learning: a flag indicating if the model can learn
         from this set of events (mandatory, no default value).
        :param input_labels: input labels.
         Must have one label per input, or a single label for all inputs.
         If a label exceeds the defined number of classes, the input will be
         discarded.
        :type input: Sparse or numpy.ndarray
        :type do_learning: bool
        :type input_labels: int or a numpy.ndarray of int32.
        :return: a Sparse tensor of shape (n, x, y, f)

        :raises TypeError: if the input doesn't have the correct type
         (Sparse, numpy.ndarray).
        :raises ValueError: if the input doesn't match the required shape,
         format, etc.
        """
        if input_labels is None:
            input_labels = []
        elif (not isinstance(input_labels, list)
          and not isinstance(input_labels, np.ndarray)):
            input_labels = [input_labels]
        if isinstance(input, Sparse):
            return super(Model, self).forward(input, do_learning, input_labels)
        elif isinstance(input, np.ndarray):
            if input.flags['C_CONTIGUOUS']:
                tensor = Tensor(input)
            else:
                tensor = Tensor(np.ascontiguousarray(input))
            spikes = super(Model, self).forward(
                tensor,
                do_learning,
                input_labels
            )
    ...
    def summary(self):
        ...
        This method prints a summary of the model with details
        of every layer (type, shape, learning type, ...).
        """
        ...
        col_width = 14
        last_col_width = 26
        headers = ['Layer (type)', 'HW', 'Input shape', 'Output shape',
        'Kernel shape', 'Learning (#classes)', '#InConn/#Weights/ThFire']
        simple_sep = "-" * (first_col_width + hw_col_width +
                            col_width * (len(headers) - 4) + 2 * last_col_width)
        double_sep = "=" * (first_col_width + hw_col_width +
                            col_width * (len(headers) - 4) + 2 * last_col_width)
        ...
        def print_layer(index):
            ...
            # layer name (type)
            current_line = get_column_data("{} ({})".format(
                layer.name, str(layer.parameters.layer_type).split(".")[-1]),
                first_col_width)
            ...
            if "learning_type" in params_names:
                learning_type = serialize_learning_type(
                    layer.parameters.learning_type)
            else:
                learning_type = "N/A"
            if "num_classes" in params_names:
                current_line += get_column_data(f"{learning_type} " +
                                                f"({layer.parameters.num_classes})",
                                                last_col_width)
            else:
                current_line += get_column_data(
                    f"{learning_type}", last_col_width)

            # input connections
            if(layer.parameters.layer_type == LayerType.FullyConnected):
                input_connections = str(
            ...
            current_line += get_column_data(f"{input_connections} / " +
                                            f"{str(num_weights_data)} / " +
    ...
    def get_layer_statistics(self, layer):
        ...
        retrieve layer statistics:
            (average sparsity, number of operations and number of possible spikes,
            row_sparsity)

        :return: a ``LayerStatistics`` object
        """
        return LayerStatistics(self, layer)
