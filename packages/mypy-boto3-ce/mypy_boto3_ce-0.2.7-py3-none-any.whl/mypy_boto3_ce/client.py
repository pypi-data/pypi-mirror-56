...
class Client(BaseClient):
    ...
    def get_cost_and_usage(
        ...
        metric, such as ``BlendedCosts`` or ``UsageQuantity`` , that you want the request to return. You can
        also filter and group your data by various dimensions, such as ``SERVICE`` or ``AZ`` , in a specific
        time range. For a complete list of valid dimensions, see the `GetDimensionValues
        ...
        **Request Syntax**
        ...
        :param TimePeriod: **[REQUIRED]**
          ...
          - **End** *(string) --* **[REQUIRED]**
            ...
            example, if ``end`` is ``2017-05-01`` , AWS retrieves cost and usage data from the start date up
          ...
          - **Or** *(list) --*
            ...
            - *(dict) --*
              ...
              * Compound dimension values with logical operations - You can use multiple ``Expression``
              ...
              {"Dimensions": { "Key": "REGION", "Values": [ "us-east-1", "us-west-1" ] }}, {"Tags": { "Key":
              "TagName", "Values": ["Value1"] } } ]}, {"Not": {"Dimensions": { "Key": "USAGE_TYPE",
              "Values": ["DataTransfer"] }}} ] }``
    ...
    def get_cost_and_usage_with_resources(
        ...
        return. You can also filter and group your data by various dimensions, such as ``SERVICE`` or ``AZ``
        ...
        **Request Syntax**
        ...
        :param TimePeriod: **[REQUIRED]**
          ...
          the last 14 days (the start date cannot be earlier than 14 days ago). The start date is inclusive,
        ...
        **Request Syntax**
        ::
          ...
          response = client.get_cost_forecast(
              ...
              Metric=
                  'BLENDED_COST'|'UNBLENDED_COST'|'AMORTIZED_COST'|'NET_UNBLENDED_COST'|'NET_AMORTIZED_COST'
    ...
    def get_dimension_values(
        ...
        **Request Syntax**
        ...
        :param Context:
          ...
          ``RESERVATIONS`` , the resulting dimension values can be used in the ``GetReservationUtilization``
          operation. If the context is set to ``COST_AND_USAGE`` , the resulting dimension values can be
    ...
    def get_reservation_coverage(
        ...
        """
        Retrieves the reservation coverage for your account. This enables you to see how much of your Amazon
        Elastic Compute Cloud, Amazon ElastiCache, Amazon Relational Database Service, or Amazon Redshift
        usage is covered by a reservation. An organization's master account can see the coverage of the
        associated member accounts. For any time period, you can filter data about reservation usage by the
    ...
    def get_reservation_purchase_recommendation(
        ...
        """
        Gets recommendations for which reservations to purchase. These recommendations could help you reduce
        your costs. Reservations provide a discounted hourly rate (up to 75%) compared to On-Demand pricing.

        AWS generates your recommendations by identifying your On-Demand usage during a specific time period
        and collecting your usage into categories that are eligible for a reservation. After AWS has these
        categories, it simulates every combination of reservations in each category of usage to identify the
        best number of each type of RI to purchase to maximize your estimated savings.

        For example, AWS automatically aggregates your Amazon EC2 Linux, shared tenancy, and c4 family usage
        in the US West (Oregon) Region and recommends that you buy size-flexible regional reservations to
        apply to the c4 family usage. AWS recommends the smallest size instance in an instance family. This
        makes it easier to purchase a size-flexible RI. AWS also shows the equal number of normalized units
        so that you can purchase any instance size that you want. For this example, your RI recommendation
        would be for ``c4.large`` because that is the smallest size instance in the c4 instance family.
        ...
        **Request Syntax**
        ...
        :param Filter:
          ...
          * Simple dimension values - You can set the dimension name and values for the filters that you
          plan to use. For example, you can filter for ``REGION==us-east-1 OR REGION==us-west-1`` . The
          ``Expression`` for that looks like this:  ``{ "Dimensions": { "Key": "REGION", "Values": [
          "us-east-1", “us-west-1” ] } }``   The list of dimension values are OR'd together to retrieve cost
          or usage data. You can create ``Expression`` and ``DimensionValues`` objects using either
          ``with*`` methods or ``set*`` methods in multiple lines.
          ...
          * Compound dimension values with logical operations - You can use multiple ``Expression`` types
          ...
          us-east-1 OR REGION == us-west-1) OR (TAG.Type == Type1)) AND (USAGE_TYPE != DataTransfer)`` . The
          ``Expression`` for that looks like this:  ``{ "And": [ {"Or": [ {"Dimensions": { "Key": "REGION",
          ...
          .. note::
            ...
            For ``GetRightsizingRecommendation`` action, a combination of OR and NOT is not supported. OR is
            not supported between different dimensions, or dimensions and tags. NOT operators aren't
