...
class Client(BaseClient):
    ...
    def add_tags_to_stream(self, StreamName: str, Tags: List[str]) -> None:
        """
        Adds or updates tags for the specified Kinesis data stream. Each time you invoke this operation, you
        can specify up to 10 tags. If you want to add more than 10 tags to your stream, you can invoke this
    ...
    def create_stream(self, StreamName: str, ShardCount: int) -> None:
        ...
        emitted from different data sources or *producers* . Scale-out within a stream is explicitly
        supported by means of shards, which are uniquely identified groups of data records in a stream.

        You specify and control the number of shards that a stream is composed of. Each shard can support
        reads up to five transactions per second, up to a maximum data read total of 2 MB per second. Each
        shard can support writes up to 1,000 records per second, up to a maximum data write total of 1 MB
        per second. If the amount of data input increases or decreases, you can add or remove shards.

        The stream name identifies the stream. The name is scoped to the AWS account used by the
        application. It is also scoped by AWS Region. That is, two streams in two different accounts can
        have the same name, and two streams in the same account, but in two different Regions, can have the
        same name.
        ...
        * Create more shards than are authorized for your account.
        ...
        <http://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html>`__ in the *Amazon
        Kinesis Data Streams Developer Guide* . To increase this limit, `contact AWS Support
        ...
        **Request Syntax**
        ...
        :param StreamName: **[REQUIRED]**
          ...
          application that creates the stream. It is also scoped by AWS Region. That is, two streams in two
          different AWS accounts can have the same name. Two streams in the same AWS account but in two
    ...
    def decrease_stream_retention_period(
        ...
        This operation may result in lost data. For example, if the stream's retention period is 48 hours
        and is decreased to 24 hours, any data already in the stream that is older than 24 hours is
        inaccessible.
    ...
    def delete_stream(
        ...
        When you delete a stream, any shards in that stream are also deleted, and any tags are dissociated
        from the stream.
    ...
    def deregister_stream_consumer(
        ...
        consumer that you want to deregister, you can use the  ListStreamConsumers operation to get a list
        of the descriptions of all the consumers that are currently registered with a given data stream. The
        ...
        **Request Syntax**
        ...
        :param ConsumerARN:
          ...
          get a list of the descriptions of all the consumers that are currently registered with a given
    ...
    def describe_stream(
        ...
        enhanced metric configuration, and shard map. The shard map is an array of shard objects. For each
        shard object, there is the hash key and sequence number ranges that the shard spans, and the IDs of
        any earlier shards that played in a role in creating the shard. Every record ingested in the stream
        is identified by a sequence number, which is assigned when the record is put into the stream.

        You can limit the number of shards returned by each call. For more information, see `Retrieving
        Shards from a Stream
        <http://docs.aws.amazon.com/kinesis/latest/dev/kinesis-using-sdk-java-retrieve-shards.html>`__ in
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          The maximum number of shards to return in a single call. The default value is 100. If you specify
          a value greater than 100, at most 100 shards are returned.
    ...
    def describe_stream_summary(
        ...
        retention period, approximate creation time, monitoring, encryption details, and open shard count.
        ...
        **Request Syntax**
        ...
        :param ShardLevelMetrics: **[REQUIRED]**
          ...
          * ``ALL``
          ...
          CloudWatch <http://docs.aws.amazon.com/kinesis/latest/dev/monitoring-with-cloudwatch.html>`__ in
          the *Amazon Kinesis Data Streams Developer Guide* .
    ...
    def get_records(
        ...
        position in the shard from which you want to start reading data records sequentially. If there are
        no records available in the portion of the shard that the iterator points to,  GetRecords returns an
        empty list. It might take multiple calls to get to a portion of the shard that contains records.

        You can scale by provisioning multiple shards per stream while considering service limits (for more
        information, see `Amazon Kinesis Data Streams Limits
        <http://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html>`__ in the *Amazon
        Kinesis Data Streams Developer Guide* ). Your application should have one thread per shard, each
        ...
        returned in ``NextShardIterator`` in subsequent calls to  GetRecords . If the shard has been closed,
        the shard iterator can't return more data and  GetRecords returns ``null`` in ``NextShardIterator``
        . You can terminate the loop when the shard is closed, or when the shard iterator reaches the record
        with the sequence number or other attribute that marks it as the last record to process.

        Each data record can be up to 1 MiB in size, and each shard can read up to 2 MiB per second. You can
        ensure that your calls don't exceed the maximum supported size or throughput by using the ``Limit``
        parameter to specify the maximum number of records that  GetRecords can return. Consider your
        average record size when determining this limit. The maximum number of records that can be returned
        per call is 10,000.

        The size of the data returned by  GetRecords varies depending on the utilization of the shard. The
        maximum size of data that  GetRecords can return is 10 MiB. If a call returns this amount of data,
        subsequent calls made within the next 5 seconds throw ``ProvisionedThroughputExceededException`` .
        If there is insufficient provisioned throughput on the stream, subsequent calls made within the next
        1 second throw ``ProvisionedThroughputExceededException`` .  GetRecords doesn't return any data when
        it throws an exception. For this reason, we recommend that you wait 1 second between calls to
        GetRecords . However, it's possible that the application will get exceptions for longer than 1
        second.

        To detect whether the application is falling behind in processing, you can use the
        ``MillisBehindLatest`` response attribute. You can also monitor the stream using CloudWatch metrics
        and other mechanisms (see `Monitoring
        <http://docs.aws.amazon.com/kinesis/latest/dev/monitoring.html>`__ in the *Amazon Kinesis Data
        Streams Developer Guide* ).

        Each Amazon Kinesis record includes a value, ``ApproximateArrivalTimestamp`` , that is set when a
        stream successfully receives and stores a record. This is commonly referred to as a server-side time
        stamp, whereas a client-side time stamp is set when a data producer creates or sends the record to a
        stream (a data producer is any data source putting data records into a stream, for example with
        PutRecords ). The time stamp has millisecond precision. There are no guarantees about the time stamp
        accuracy, or that the time stamp is always increasing. For example, records in a shard or across a
        stream might have time stamps that are out of order.
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          that is greater than 10,000,  GetRecords throws ``InvalidArgumentException`` .
    ...
    def get_shard_iterator(
        ...
        The position is specified using the sequence number of a data record in a shard. A sequence number
        is the identifier associated with every record ingested in the stream, and is assigned when a record
        is put into the stream. Each stream has one or more shards.
        ...
        request, you can specify the shard iterator type ``AT_TIMESTAMP`` to read records from an arbitrary
        point in time, ``TRIM_HORIZON`` to cause ``ShardIterator`` to point to the last untrimmed record in
        the shard in the system (the oldest data record in the shard), or ``LATEST`` so that you always read
        ...
        iterator for use in your first  GetRecords request and for subsequent reads use the shard iterator
        returned by the  GetRecords request in ``NextShardIterator`` . A new shard iterator is returned by
        every  GetRecords request in ``NextShardIterator`` , which you use in the ``ShardIterator``
        parameter of the next  GetRecords request.

        If a  GetShardIterator request is made too often, you receive a
        ``ProvisionedThroughputExceededException`` . For more information about throughput limits, see
        GetRecords , and `Streams Limits
        <http://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html>`__ in the *Amazon
        Kinesis Data Streams Developer Guide* .

        If the shard is closed,  GetShardIterator returns a valid iterator for the last sequence number of
        ...
        **Request Syntax**
        ...
        :param Timestamp:
          ...
          ``2016-04-04T19:58:46.480-00:00`` or ``1459799926.480`` . If a record with this exact time stamp
          does not exist, the iterator returned is for the next (later) record. If the time stamp is older
    ...
    def increase_stream_retention_period(
        ...
        which records that have not yet expired are accessible. However, it does not make previous, expired
        data (older than the stream's previous retention period) accessible after the operation has been
        called. For example, if a stream's retention period is set to 24 hours and is increased to 168
    ...
    def list_shards(
        ...
        of 100 transactions per second per data stream.

        .. warning::

          This API is a new operation that is used by the Amazon Kinesis Client Library (KCL). If you have a
          fine-grained IAM policy that only allows specific operations, you must update your policy to allow
          calls to this API. For more information, see `Controlling Access to Amazon Kinesis Data Streams
          Resources Using IAM <https://docs.aws.amazon.com/streams/latest/dev/controlling-access.html>`__ .
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          than the number of shards in the data stream, the response includes a pagination token named
          ``NextToken`` . You can specify this ``NextToken`` value in a subsequent call to ``ListShards`` to
          list the next set of shards.

          Don't specify ``StreamName`` or ``StreamCreationTimestamp`` if you specify ``NextToken`` because
          the latter unambiguously identifies the stream.
        ...
        :param ExclusiveStartShardId:
          ...
          Specify this parameter to indicate that you want to list the shards starting with the shard whose
        ...
        :param MaxResults:
          ...
          The maximum number of shards to return in a single call to ``ListShards`` . The minimum value you
        ...
        :param StreamCreationTimestamp:
          ...
          you create a data stream and then delete it, and you later create another data stream with the
          same name, you can use this input parameter to specify which of the two streams you want to list
          the shards for.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Shards** *(list) --*
              ...
              contains the starting and ending hash keys and the starting and ending sequence numbers for
              the shard.
    ...
    def list_stream_consumers(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          that is less than the number of consumers that are registered with the data stream, the response
          ...
          You can optionally specify a value for the ``MaxResults`` parameter when you specify ``NextToken``
          . If you specify a ``MaxResults`` value that is less than the number of consumers that the
          operation returns if you don't specify ``MaxResults`` , the response will contain a new
          ...
          .. warning::
            ...
            expired token in a call to ``ListStreamConsumers`` , you get ``ExpiredNextTokenException`` .
    ...
    def list_streams(
        ...
        for the ``Limit`` parameter, Kinesis Data Streams uses the default limit, which is currently 10.

        You can detect if there are more streams available to list by using the ``HasMoreStreams`` flag from
        the returned output. If there are more streams available, you can request more streams by using the
        name of the last stream returned by the ``ListStreams`` request in the ``ExclusiveStartStreamName``
        parameter in a subsequent request to ``ListStreams`` . The group of stream names returned by the
        subsequent request is then added to the list. You can continue this process until all the stream
    ...
    def list_tags_for_stream(
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          the stream, ``HasMoreTags`` is set to ``true`` . To list additional tags, set
    ...
    def merge_shards(
        ...
        the stream's capacity to ingest and transport data. Two shards are considered adjacent if the union
        of the hash key ranges for the two shards form a contiguous set with no gaps. For example, if you
        have two shards, one with a hash key range of 276...381 and the other with a hash key range of
        382...454, then you could merge these two shards into a single shard that would have a hash key
        range of 276...454. After the merge, the single child shard receives data for all hash key values
        covered by the two parent shards.
         ...
         of excess capacity that is not being used. You must specify the shard to be merged and the adjacent
         shard for a stream. For more information about merging shards, see `Merge Two Shards
         <http://docs.aws.amazon.com/kinesis/latest/dev/kinesis-using-sdk-java-resharding-merge.html>`__ in
         the *Amazon Kinesis Data Streams Developer Guide* .
    ...
    def put_record(
        ...
        """
        Writes a single data record into an Amazon Kinesis data stream. Call ``PutRecord`` to send data into
        the stream for real-time ingestion and subsequent processing, one record at a time. Each shard can
        support writes up to 1,000 records per second, up to a maximum data write total of 1 MB per second.

        You must specify the name of the stream that captures, stores, and transports the data; a partition
        key; and the data blob itself.
        ...
        partition key associated with each data record to determine the shard to which a given data record
        belongs.

        Partition keys are Unicode strings, with a maximum length limit of 256 characters for each key. An
        MD5 hash function is used to map partition keys to 128-bit integer values and to map associated data
        records to shards using the hash key ranges of the shards. You can override hashing the partition
        ...
        in the *Amazon Kinesis Data Streams Developer Guide* .
         ...
         ``PutRecord`` returns the shard ID of where the data record was placed and the sequence number that
         was assigned to the data record.

        Sequence numbers increase over time and are specific to a shard within a stream, not across all
        shards within a stream. To guarantee strictly increasing ordering, write serially to a shard and use
        the ``SequenceNumberForOrdering`` parameter. For more information, see `Adding Data to a Stream
        <http://docs.aws.amazon.com/kinesis/latest/dev/developing-producers-with-sdk.html#kinesis-using-sdk-java-add-data-to-stream>`__
        in the *Amazon Kinesis Data Streams Developer Guide* .
        ...
        You can use  IncreaseStreamRetentionPeriod or  DecreaseStreamRetentionPeriod to modify this
        retention period.
        ...
        **Request Syntax**
        ...
        :param PartitionKey: **[REQUIRED]**
          ...
          uses the partition key as input to a hash function that maps the partition key and associated data
          to a specific shard. Specifically, an MD5 hash function is used to map partition keys to 128-bit
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **SequenceNumber** *(string) --*
              ...
              for the record is unique across all records in the stream. A sequence number is the identifier
    ...
    def put_records(
        ...
        large as 1 MB, up to a limit of 5 MB for the entire request, including partition keys. Each shard
        can support writes up to 1,000 records per second, up to a maximum data write total of 1 MB per
        second.
        ...
        record size limit applies to the total size of the partition key and data blob.

        The data blob can be any type of data; for example, a segment from a log file, geographic/location
        data, website clickstream data, and so on.

        The partition key is used by Kinesis Data Streams as input to a hash function that maps the
        partition key and associated data to a specific shard. An MD5 hash function is used to map partition
        keys to 128-bit integer values and to map associated data records to shards. As a result of this
        hashing mechanism, all data records with the same partition key map to the same shard within the
        ...
        explicitly the shard where the record is stored. For more information, see `Adding Multiple Records
        with PutRecords
        ...
        Kinesis Data Streams attempts to process all records in each ``PutRecords`` request. A single record
        ...
        account ID, stream name, and shard ID of the record that was throttled. For more information about
        partially successful responses, see `Adding Multiple Records with PutRecords
          ...
          - *(dict) --*
            ...
            - **Records** *(list) --*
              ...
              - *(dict) --*
                ...
                is successfully added to a stream includes ``SequenceNumber`` and ``ShardId`` in the result.
                A record that fails to be added to the stream includes ``ErrorCode`` and ``ErrorMessage`` in
                the result.
    ...
    def register_stream_consumer(
        ...
        register can read data from the stream at a rate of up to 2 MiB per second. This rate is unaffected
    ...
    def split_shard(
        ...
        example, the producers sending data into the specific shard are suddenly sending more than
        ...
        beginning and ending hash key, but it can be any hash key value in the range being mapped into the
        shard. For more information, see `Split a Shard
        <http://docs.aws.amazon.com/kinesis/latest/dev/kinesis-using-sdk-java-resharding-split.html>`__ in
        the *Amazon Kinesis Data Streams Developer Guide* .
        ...
        ``StreamStatus`` . If the stream is in the ``ACTIVE`` state, you can call ``SplitShard`` . If a
        stream is in ``CREATING`` or ``UPDATING`` or ``DELETING`` states, ``DescribeStream`` returns a
        ...
        **Request Syntax**
        ...
        :param NewStartingHashKey: **[REQUIRED]**
          ...
          distributed to one of the child shards. All the lower hash key values in the range are distributed
    ...
    def start_stream_encryption(
        ...
        returns immediately and sets the status of the stream to ``UPDATING`` . After the update is
    ...
    def stop_stream_encryption(
        ...
        Stopping encryption is an asynchronous operation. Upon receiving the request, Kinesis Data Streams
        returns immediately and sets the status of the stream to ``UPDATING`` . After the update is
        ...
        read and write data to your stream while its status is ``UPDATING`` . Once the status of the stream
        ...
        Note: It can take up to 5 seconds after the stream is in an ``ACTIVE`` status before all records
        written to the stream are no longer subject to encryption. After you disabled encryption, you can
        verify that encryption is not applied by inspecting the API response from ``PutRecord`` or
        ``PutRecords`` .
    ...
    def subscribe_to_shard(
        ...
        """
        Call this operation from your consumer after you call  RegisterStreamConsumer to register the
        consumer with Kinesis Data Streams. If the call succeeds, your consumer starts receiving events of
        type  SubscribeToShardEvent for up to 5 minutes, after which time you need to call
        ...
        succeeds, and then you call the operation again less than 5 seconds later, the second call generates
        a  ResourceInUseException . If you call the operation a second time more than 5 seconds after the
        first call succeeds, the second call succeeds and the first connection gets shut down.
            ...
            - **EventStream** (:class:`.EventStream`) --
              ...
              - **SubscribeToShardEvent** *(dict) --*
                ...
                - **MillisBehindLatest** *(integer) --*
                  ...
                  The number of milliseconds the read records are from the tip of the stream, indicating how
                  far behind current time the consumer is. A value of zero indicates that record processing
                  is caught up, and there are no new records to process at this moment.
              ...
              - **KMSInvalidStateException** *(dict) --*
                ...
                request. For more information, see `How Key State Affects Use of a Customer Master Key
                <http://docs.aws.amazon.com/kms/latest/developerguide/key-state.html>`__ in the *AWS Key
              ...
              - **KMSThrottlingException** *(dict) --*
                ...
                `Limits
                <http://docs.aws.amazon.com/kms/latest/developerguide/limits.html#requests-per-second>`__ in
    ...
    def update_shard_count(
        ...
        To update the shard count, Kinesis Data Streams performs splits or merges on individual shards. This
        can cause short-lived shards to be created, in addition to the final shards. We recommend that you
        double or halve the shard count, as this results in the fewest number of splits or merges.
        ...
        * Scale up to more than the shard limit for your account
        ...
        <http://docs.aws.amazon.com/kinesis/latest/dev/service-sizes-and-limits.html>`__ in the *Amazon
        Kinesis Data Streams Developer Guide* . To request an increase in the call rate limit, the shard
        limit for this API, or your overall shard limit, use the `limits form
        <https://console.aws.amazon.com/support/v1#/case/create?issueType=service-limit-increase&limitType=
            service-code-kinesis>`__ .
    ...
    ) -> waiter_scope.StreamExistsWaiter:
        """
