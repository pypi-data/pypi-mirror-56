...
class Label:
    ...
    def set_position(self, timestamp, target_position, actual_position):
        """
        ...
        if self.positions.shape[0] < row + 1:
            self.positions.resize((row + 1, 7), refcheck=False)
        a = actual_position
        t = target_position
        item = [row * self.label_resolution, t[0], t[1], t[2], a[0], a[1], a[2]]
