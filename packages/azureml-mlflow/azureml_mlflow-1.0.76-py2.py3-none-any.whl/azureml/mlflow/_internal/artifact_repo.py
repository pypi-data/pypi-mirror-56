    ...
    def log_artifacts(self, local_dir, artifact_path=None):
        ...
        if artifact_path is not None and artifact_path not in [".", "./"]:
            ...
            for dir_name in dirs:
                self.run_artifacts.upload_dir(dir_name, self._run_id)
...
class AdbAzuremlArtifactRepository(ArtifactRepository):
    ...
    def get_dbfs_artifact_repo(self, artifact_uri):
        ...
        # dbfs artifact_uri is in the form of:
    ...
    def get_aml_artifact_repo(self, artifact_uri):
        regex = "(.*)databricks\\/mlflow\\/(.+)\\/(.+)\\/artifacts"
        mo = re.compile(regex).match(artifact_uri)
        experiment_id = mo.group(2)
        run_id = mo.group(3)
        experiment_name = mlflow.tracking.MlflowClient().store.adb_store.get_experiment(experiment_id)._name
