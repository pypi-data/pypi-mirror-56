...
class reversePipe(object):
	...
	def map(self,func):
		return reversePipe(map(lambda x:func(x),self.pipe),self.lg,self)
	def activate(self):
		# print(2)
	...
	def filter(self,func):
		return reversePipe(filter(lambda x:func(x),self.pipe),self.lg,self)
	...
	def chainElements(self,p_level):
		return reversePipe(chainElements(self.pipe,p_level),self.lg,self)
