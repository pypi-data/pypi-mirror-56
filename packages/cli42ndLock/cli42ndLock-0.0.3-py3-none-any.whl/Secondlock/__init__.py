...
class SecondLock():
    ...
    def _loginSecondLockServer(self):
        ...
        response      = requests.request("POST", "https://%s:%d/api/v1/authenticate" % (self._secondlock_host, self._secondlock_port), data=payload, headers=self._headers, timeout=42)
    ...
    def _uploadRSAPublicKeyToSecondLockServer(self):
        ...
        response = requests.request("POST", "https://%s:%d/api/v1/key" % (self._secondlock_host, self._secondlock_port), data=payload, headers=self._headers, timeout=42)
    ...
    def _uploadContentKeyToSecondLockServer(self, checksum, contentkey, email, live_time_in_days=23, max_open=23):
        ...
        payload         = json.dumps({"checksum": checksum.decode("utf-8") , "key": contentkey.decode("utf-8") , "domain": email.split('@')[1], "liveTime": new_date_string, "maxOpen": max_open})
        response        = requests.request("POST", "https://%s:%d/api/v1/content" % (self._secondlock_host, self._secondlock_port), data=payload, headers=self._headers, timeout=42)
    ...
    def _getPublicKeysForUser(self, email):
        ...
        response = requests.request("GET", "https://%s:%d/api/v1/key/%s" % (self._secondlock_host, self._secondlock_port, email), headers=self._headers, timeout=42)
    ...
    def _getContentKeyForFile(self, content_sha_and_device_id_base64):
        ...
        response = requests.request("GET", "https://%s:%d/api/v1/content/%s" % (self._secondlock_host, self._secondlock_port, content_sha_and_device_id_base64.decode()) , headers=self._headers, timeout=42)
    ...
    def encrypt(self, input_file_content, email, live_time_in_days = 365, max_open=100):
        ...
        for device_entry in self._getPublicKeysForUser(email):
            ...
            self._uploadContentKeyToSecondLockServer(content_sha_and_device_id_base64, rsa_encrypted_base64, email, live_time_in_days, max_open)
