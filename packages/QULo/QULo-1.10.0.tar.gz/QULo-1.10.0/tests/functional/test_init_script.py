...
#######################################################################
# Copyright (C) 2016, 2018 David Palao
#
...
#from tests.common.constants import QULOD_PROCESS_NAME#, QULOD_LOCK_FILE_NAME
...
@unittest.skip
class BasicQulodFunctionalityTest(StartStopTest):
    def test_qulo_start_stop(self):
        ...
        # that the qulod daemon is not running before launching it:
        qulod_process = self.find_qulod()
        if qulod_process:
            self.fail("'qulod' process found before starting! Stop it and "\
                      "come back to test.")
        #  Fine, now he can start testing this new promising tool!
        # For that, he launches the "qulo" init script:
