...
class MyCommand(object):
	...
	def run(self, options=(), timeOut=float('inf'), **kwargs):
		...
		if '|' in commandList:
			...
			proc = subprocess.Popen(commandLists[0],  shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			for commandList in commandLists[1:]:
				proc = subprocess.Popen(commandList,  shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=proc.stdout)
		else:
			proc = subprocess.Popen(commandList,  shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
