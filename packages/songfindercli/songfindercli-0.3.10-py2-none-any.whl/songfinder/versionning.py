...
class AddRepo(object): # TODO integrate to repo class
	def __init__(self, paths, exe, screens=None, updateData=None, **kwargs):
		fenetre = tk.Toplevel()
	...
	def _cloneRepo(self, event=0):
		self.update_fen('Récupération des informations ...')
		repo = self._pathEntryFrame.get()
		name = self._userEntryFrame.get()
		mdp = self._passwordEntryFrame.get()
		if not (repo and name and mdp):
			...
			if repo:
				...
				code, out, err = self.__scm.run( options=['clone'] \
...
class Repo(object):
	def __init__(self, path, exe, papa=None, screens=None, **kwargs):
	...
	def receive(self, send=False, showGui=True):
		...
		if out.find('no changes found') != -1 or out.find('aucun changement trouv') != -1:
			...
			return self.__showError('Erreur: Impossible de recupérer les modifications.\nErreur %d:\n%s'\
									%(code, '\n'.join([out,err])))
		elif tkMessageBox.askyesno('Erreur', 'Erreur: Impossible de recupérer les modifications.\nErreur %d:\n%s\n'
	...
	def send(self, what='all', showGui=True):
		...
		if out.find('nothing changed') != -1 \
			...
			return self.__showInfo('Rien à envoyer') \
					or receiveReturn
		elif code != 0:
			self.__update_fen("    échec\n")
