...
from lendsmart_api.objects import PredictionWorkflow, PredictionInference, PredictionSegment, Document, Base, AccountSign, DocumentPointer
...
class Documents(Group):
    ...
    def document_pointer_create(self, data=None):
        ...
        :raises UnexpectedResponseError: If the returned data from the api does
            not look as expected.
        """
        for i, value in enumerate(data["object_meta"]["owner_references"]):
            self._doc_ptr_create_for_three_point_two(value['uid'])
    ...
    def _doc_ptr_create_for_three_point_two(self, id=None):
        """
        Creates a new DocumentPointer, with given inputs.
        ...
        :raises UnexpectedResponseError: If the returned data from the api does
            not look as expected.
        """
        result = self.client.get('/loan_apps/docs/mandatory/{}'.format(id))
        for res in result:
            if not 'id' in res:
                raise UnexpectedResponseError('Unexpected response when creating DocumentPointer'
                    'Client!', json=result)

            c = DocumentPointer(self.client, res['id'], res)
            return c
