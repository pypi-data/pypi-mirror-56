...
def create_parser(parser):
    required_group = parser.add_argument_group(
        "Required arguments", "These options are required to run crYOLO evaluation. In most cases you want to use a runfile to evaluate"
    ...
    optional_group = parser.add_argument_group(
        "Optional arguments", "These options are optional to run crYOLO evaluation. Here you can"
                              "specify specific images and boxes as ground truth that should be "
                              "used to calcuate the statistics."
    )

    optional_group.add_argument("-i", "--images", help="Path folder with test images (ground truth)")
