...
class Wechat:
    ...
    def sendSubscribe(self,access_token,openid,template_id,data,page=''):
        result = None
        send_data = {
            "touser":openid,
            "template_id":template_id,
        ...
        if page:
            send_data['page'] = page
        wechat_url = 'https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token='+access_token
        headers = {'content-type': 'application/json'}
        try:
            r = requests.post(wechat_url, data=json.dumps(send_data), headers=headers)
            r.encoding='utf-8'
