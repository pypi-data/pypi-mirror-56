import yaml
import argparse
from ibmaceflowtester.util import file, log
...
suite = yaml.load(file.read_file_string(test_config), Loader=yaml.FullLoader)
...
for (_, _, files) in os.walk(args.bar_dir, topdown=True):
    for bar_file in [bar for bar in files if re.match(fnmatch.translate("*.bar"), bar)]:
        binary = file.read_file_binary(os.path.join(args.bar_dir, bar_file))
