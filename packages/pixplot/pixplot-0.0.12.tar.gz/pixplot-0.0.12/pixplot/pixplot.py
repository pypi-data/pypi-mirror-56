...
def get_manifest(**kwargs):
  ...
  manifest = {
    ...
    'images': [clean_filename(i.path) for i in kwargs['images']],
  ...
  for label, position_path in get_positions(**kwargs).items():
    ...
    if not exists(out_dir): os.makedirs(out_dir)
    centroid_path = join(out_dir, hash() + '.json')
...
def filter_images(**kwargs):
  ...
  if not image_paths:
    print('\n ! No input images were found. Please check your --images glob')
    sys.exit()
...
def get_atlas_list(**kwargs):
  ...
  for idx, i in enumerate(kwargs['images']):
...
def vectorize_images(imgs):
  ...
  print(' * preparing to vectorize {} images'.format(len(imgs)))
  base = InceptionV3()
...
def get_rasterfairy_projection(**kwargs):
  ...
  sizes = [i.resize_to_max(kwargs['lod_cell_height']).shape for i in kwargs['images']]
  ...
  z = add_z_dim(np.swapaxes(np.array([x, y]), 0, 1))
  path = write_json('rasterfairy', z, **kwargs)
  return path
...
def get_grid_projection(**kwargs):
  ...
  for i, _ in enumerate(kwargs['images']):
    ...
    y = math.floor(i/n)
    l.append([x, y])
  z = add_z_dim(np.array(l))
  path = write_json('grid', z, **kwargs)
  return path
...
def write_json(name, o, precision=4, sub_dir='layouts', **kwargs):
  '''Write json object `o` to disk and return the path to that file'''
  out_dir = os.path.join(kwargs['out_dir'], sub_dir)
  if not os.path.exists(out_dir): os.makedirs(out_dir)
  path = os.path.join(out_dir, name + '-' + hash() + '.json')
  if precision: o = [[round(float(j), precision) for j in i] for i in o]
  with open(path, 'w') as out: json.dump(o, out)
  return path
