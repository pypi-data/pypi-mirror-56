...
Data.prototype.addCells = function(json) {
  ...
  for (var i=0; i<json.atlas.positions.length; i++) { // atlas index
    for (var j=0; j<json.atlas.positions[i].length; j++) { // cell index within atlas
      ...
      if (drawcall.textures)

      var texIdx = Math.floor(i/config.atlasesPerTex),
          worldPos = json.layouts[this.layout].positions[idx], // position of cell in world -1:1
      ...
      this.cells.push(new Cell({
        ...
        idxInAtlas: j, // index of cell within atlas
        texIdx: texIdx, // index of texture among all textures
...
function Cell(obj) {
  ...
  this.atlasIdx = obj.atlasIdx; // atlas index among all atlases
  this.idxInAtlas = obj.idxInAtlas; // idx among cells in atlas
  this.texIdx = obj.texIdx; // texture index among all textures
  this.idxInDrawCall = obj.idInDrawCall; // idx within mesh
...
Cell.prototype.deactivate = function() {
  var atlasOffset = getAtlasOffset(this.atlasIdx)
      d = data.json.atlas.positions[this.atlasIdx][this.idxInAtlas];
  this.dx = d[0] + atlasOffset.x;
  this.dy = d[1] + atlasOffset.y;
  this.texIdx = this.texIdx; // todo
  ['textureIndex', 'offset'].forEach(this.mutateBuffer.bind(this));
}
...
World.prototype.getScene = function() {
  return new THREE.Scene({background: new THREE.Color(0x111111)});
}
...
World.prototype.plot = function() {
  ...
  for (var i=0; i<drawCalls; i++) {
    var start = i * config.cellsPerDrawCall,
        ...
        meshCells = cells.slice(start, end),
...
Filters.prototype.loadFilters = function() {
  if (!data.json.metadata) return; // exit if no metadata provided
