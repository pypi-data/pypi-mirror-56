...
*   gridSpacing: the size of each unit in the LOD grid. Smaller means that more
...
function Config() {
  ...
  this.cellsPerAtlas = (this.size.atlas / this.size.cell)**2;
  this.cellsPerTex = this.cellsPerAtlas * this.atlasesPerTex;
  var indexLimited = webgl.limits.indexedElements;
  var texLimited = (webgl.limits.textureCount-1) * this.cellsPerTex;
  this.cellsPerDrawCall = Math.min(indexLimited, texLimited);
}
...
function Data() {
  ...
  this.layout = 'grid'; // string label for selected layout
  this.layouts = [];
  this.cells = [];
  this.textures = [];
...
Data.prototype.load = function() {
  get(config.data.dir + '/' + config.data.file, function(json) {
    ...
    // set internal data vals
    this.json = json;
    this.atlasCount = json.atlas.count;
    this.textureCount = Math.ceil(json.atlas.count / config.atlasesPerTex);;
    var layouts = Object.keys(json.layouts);
    this.layout = this.layout ? this.layout : layouts[0];
    this.hotspots = new Hotspots();
    layout.setOptions(layouts);
...
// Add all cells to the world
Data.prototype.addCells = function(json) {
  // create all cells
  var idx = 0; // index of cell among all cells
  for (var i=0; i<json.atlas.positions.length; i++) { // atlas index
    for (var j=0; j<json.atlas.positions[i].length; j++) { // cell index within atlas
      var worldPos = json.layouts[this.layout].positions[idx], // position of cell in world -1:1
          atlasPos = json.atlas.positions[i][j], // idx-th cell position in atlas
          size = json.cell_sizes[i][j];
      this.cells.push(new Cell({
        texIdx: 0, // index of texture among all textures
        idx: idx, // index of cell among all cells
        w:  size[0], // width of cell in lod atlas
        h:  size[1], // height of cell in lod atlas
        dx: atlasPos[0], // x offset of cell in atlas
        dy: atlasPos[1], // y offset of cell in atlas
...
function Cell(obj) {
  ...
  this.z = (obj.z || obj.idx%config.data.spread/10);
...
Cell.prototype.updateParentBoundingBox = function() {
  ...
  ['x', 'y'].forEach(function(d) {
    if      (this[d] > bb[d].max) bb[d].max = this[d];
    else if (this[d] < bb[d].min) bb[d].min = this[d];
  }.bind(this))
}
...
Cell.prototype.deactivate = function() {
  var atlasIdx = Math.floor(this.idx/config.cellsPerAtlas),
      idxInAtlas = this.idx%config.cellsPerAtlas,
      d = data.json.atlas.positions[atlasIdx][idxInAtlas];
  this.dx = d[0];
  this.dy = d[1];
  this.texIdx = Math.floor(atlasIdx/config.atlasesPerTex);
  ['textureIndex', 'offset'].forEach(this.mutateBuffer.bind(this));
}
...
Layout.prototype.transition = function(layoutKey) {
  ...
  for (var i=0; i<data.cells.length; i++) {
    var c = data.cells[i],
        p = data.json.layouts[layoutKey].positions[i];
    c.tx = p[0] * config.data.spread;
    c.ty = p[1] * config.data.spread;
    c.tz = (p[2] || 0) * config.data.spread;
  }
  ...
  for (var i=0; i<meshes.length; i++) {
    ...
    data.cells.slice(start, end).forEach(function(cell) {
      meshes[i].geometry.attributes.pos1.array[iter++] = cell.tx;
      meshes[i].geometry.attributes.pos1.array[iter++] = cell.ty;
      meshes[i].geometry.attributes.pos1.array[iter++] = cell.tz;
    })
    meshes[i].geometry.attributes.pos1.needsUpdate = true;
...
World.prototype.plot = function() {
  var group = new THREE.Group(),
      cells = data.cells,
      drawCalls = Math.ceil(data.cells.length / config.cellsPerDrawCall);
  for (var i=0; i<drawCalls; i++) {
    var start = i * config.cellsPerDrawCall,
        end = (i+1) * config.cellsPerDrawCall,
        groupCells = cells.slice(start, end),
        attrs = this.getGroupAttributes(groupCells),
...
Selector.prototype.showModal = function(cellIdx) {
  ...
  // try to fetch metadata for this image and
...
Hotspots.prototype.init = function() {
  var path = data.json.layouts[data.layout].centroids.replace('output/', '');
  get(getCurrentRoute() + path, function(json) {
    this.json = json;
