...
class Client(BaseClient):
    ...
    def list_aws_default_service_quotas(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          from. If additional items exist beyond the specified maximum, the ``NextToken`` element is present
          and has a value (isn't null). Include that value as the ``NextToken`` request parameter in the
          call to the operation to get the next part of the results. You should check ``NextToken`` after
        ...
        :param MaxResults:
          ...
          a value (isn't null). Include that value as the ``NextToken`` request parameter in the call to the
    ...
    def list_requested_service_quota_change_history_by_quota(
        ...
        granularity over the ``ListRequestedServiceQuotaChangeHistory`` command. Once a quota change request
        has reached ``CASE_CLOSED, APPROVED,`` or ``DENIED`` , the history has been kept for 90 days.
    ...
    def list_service_quotas(
        ...
        quotas for the specified service. you'll see the default values are the values that AWS provides for
        the quotas.
    ...
    def request_service_quota_increase(
        ...
        """
        Retrieves the details of a service quota increase request. The response to this command provides the
        details in the  RequestedServiceQuotaChange object.
