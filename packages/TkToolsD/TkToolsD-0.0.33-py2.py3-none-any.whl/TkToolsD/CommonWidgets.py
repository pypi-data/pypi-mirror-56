...
def __getDirOrFileWidget(isFile, root, title='', msgUp='', pathSaved=None, callback=None, enableEmpty=False, showOpen=True, showOpenFolder=False, mustexist=False) :
    ...
    if pathSaved :
        et.set(pathSaved)
    ttk.Entry(column2, textvariable = et).pack(expand=tk.YES, fill=tk.BOTH, side=tk.LEFT, padx=5, pady=5)
    ...
    def btnCallback() :
        ...
        if isFile :
            ...
            ShowChooseDirDialog(onChosen, initialdir=et.get(), parent=getToplevel(root), mustexist=mustexist)
...
返回的控件包含方法如下：
    ...
    '''
    return __getDirOrFileWidget(False, root, title=title, msgUp=msgUp, pathSaved=pathSaved, callback=callback, enableEmpty=enableEmpty, showOpen=showOpen)

def GetFileWidget(root, title='', msgUp='', pathSaved=None, callback=None, enableEmpty=False, showOpen=False, showOpenFolder=True) :
    '''参数说明:
    root:           tk 父节点
    title:          显示在Entry左边的标题
    msgUp:          显示在Entry上面的文本
...
返回的控件包含方法如下：
    ...
    getUpMsg        获取文本框上面的说明文字
    '''
    return __getDirOrFileWidget(True, root, title=title, msgUp=msgUp, pathSaved=pathSaved, callback=callback, enableEmpty=enableEmpty, showOpen=showOpen, showOpenFolder=showOpenFolder)
...
def __testGetDirWid() :
    root = tk.Tk()
    v = GetDirWidget(root, '选择路径', 'test', mustexist=True)
