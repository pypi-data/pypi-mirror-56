            ...
            if field_name in self._iterables:
                ...
                if field_dna["field_type"] == "ForeignKey":
                    ...
                    iterable_fields[field_name] = dict(
                        field_type="ForeignKey",
                        related_to="Thing",  # f"{settings.APP_NAME.title()}Thing",
                    )
        # Remove the iterable_fields from
        #       TODO: Consider not removing the field. It won't appear.
        if iterable_fields:
            for field_name in iterable_fields.keys():
    ...
    def _get_things_of_site(self, site_dna, thing_names=None):
        ...
        for key, value in site_dna.items():
            if isinstance(value, dict):
                ...
                # Thing properties hosted in the "iterate" key should be listed
                # as Many2Many objects.
                iterate_properties = value.get("iterate", [])
