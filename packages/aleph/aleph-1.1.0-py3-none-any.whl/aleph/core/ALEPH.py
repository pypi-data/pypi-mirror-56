...
def generate_library_starter(args):
    ...
    clustering_mode=args.clustering_mode, number_of_ranges=args.number_of_ranges, number_of_clusters=args.number_of_clusters, exclude_sequence=args.exclude_sequence,use_model_as_it_is=args.use_model_as_it_is)
...
def perform_cluster_starter(args: list):
    """
...
def generate_matrix_and_graph(strucc, cvs_list, pdb_model, weight="distance_avg", min_ah=4, min_bs=3, write_pdb=True,
    ...
    try:
        list_total_atoms = Bioinformatics3.get_atoms_list(pdb_model)
        num = 0
        ...
        ah_content, bs_content = Bioinformatics3.percentage_of_secondary_structure(g, n_res)
        ...
        for combi in itertools.product(*combinations):
            if len(set(combi[:len(ref_frags)]))+len(set(combi[len(ref_frags):])) == len(ref_frags)*2:
                for u in range(3):
                    ...
                    if not swap:
                        rmsd,R,t = Bioinformatics3.get_rmsd_and_RT(re1, ta2, None, transform=False, n_iter=3)
                        tm = Bioinformatics3.get_tm_score(re1, ta2)
                    else:
                        rmsd,R,t = Bioinformatics3.get_rmsd_and_RT(ta2, re1, None, transform=False, n_iter=3)
                        tm = Bioinformatics3.get_tm_score(ta2, re1)
                    ...
                    if u > 0:
                        ...
                        if not swap:
                            rmsd2, R2, t2 = Bioinformatics3.get_rmsd_and_RT(re1, ta2, None, transform=False, n_iter=3)
                            tm2 = Bioinformatics3.get_tm_score(re1, ta2)
                        else:
                            rmsd2, R2, t2 = Bioinformatics3.get_rmsd_and_RT(ta2, re1, None, transform=False, n_iter=3)
                            tm2 = Bioinformatics3.get_tm_score(ta2, re1)
...
def generate_library(local_grid=None,remote_grid=None,supercomputer=None,force_core=-1, directory_database=None,
                     ...
                     clustering_mode='no_clustering', number_of_ranges=500, number_of_clusters=7000, exclude_sequence=None, reverse=False, use_model_as_it_is=False):
    global PATH_NEW_BORGES
    global GRID_TYPE_R
    global MAX_PDB_TAR
    global toclientdir
