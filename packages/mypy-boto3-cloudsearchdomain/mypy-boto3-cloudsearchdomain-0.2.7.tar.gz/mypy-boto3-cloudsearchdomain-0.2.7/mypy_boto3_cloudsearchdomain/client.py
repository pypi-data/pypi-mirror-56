...
class Client(BaseClient):
    ...
    def search(
        ...
        * ``simple`` : search all ``text`` and ``text-array`` fields for the specified string. Search for
        ...
        * ``dismax`` : specify search criteria using the simplified subset of the Apache Lucene query parser
        ...
        <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching.html>`__ in the *Amazon
        ...
        configuration service ``DescribeDomains`` action. A domain's endpoints are also displayed on the
        ...
        **Request Syntax**
        ...
        :param cursor:
          ...
          to control the number of hits to include in each response. You can specify either the ``cursor``
          or ``start`` parameter in a request; they are mutually exclusive. To get the first cursor, set the
          cursor value to ``initial`` . In subsequent requests, specify the cursor value returned in the
          hits section of the response.

          For more information, see `Paginating Results
          <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/paginating-results.html>`__ in the
          *Amazon CloudSearch Developer Guide* .

        :type expr: string
        :param expr:

          Defines one or more numeric expressions that can be used to sort results or specify search or
          filter criteria. You can also specify expressions as return fields.

          You specify the expressions in JSON using the form ``{"EXPRESSIONNAME":"EXPRESSION"}`` . You can
          define and use multiple expressions in a search request. For example:

          ``{"expression1":"_score*rating", "expression2":"(1/rank)*year"}``

          For information about the variables, operators, and functions you can use in expressions, see
          `Writing Expressions
        ...
        :param facet:
          ...
          * ``buckets`` specifies an array of the facet values or ranges to count. Ranges are specified
          using the same syntax that you use to search for a range of values. For more information, see
          `Searching for a Range of Values
          <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-ranges.html>`__ in the
          *Amazon CloudSearch Developer Guide* . Buckets are returned in the order they are specified in the
          ...
          * ``size`` specifies the maximum number of facets to include in the results. By default, Amazon
          CloudSearch returns counts for the top 10. The ``size`` parameter is only valid when you specify
          the ``sort`` option; it cannot be used in conjunction with ``buckets`` .
          ...
          * ``sort`` specifies how you want to sort the facets in the results: ``bucket`` or ``count`` .
          ...
          If no facet options are specified, facet counts are computed for all field values, the facets are
          sorted by facet count, and the top 10 facets are returned in the results.

          To count particular buckets of values, use the ``buckets`` option. For example, the following
          request uses the ``buckets`` option to calculate and return facet counts by decade.

          ``{"year":{"buckets":["[1970,1979]","[1980,1989]","[1990,1999]","[2000,2009]","[2010,}"]}}``

          To sort facets by facet count, use the ``count`` option. For example, the following request sets
          the ``sort`` option to ``count`` to sort the facet values by facet count, with the facet values
          that have the most matching documents listed first. Setting the ``size`` option to 3 returns only
          the top three facet values.

          ``{"year":{"sort":"count","size":3}}``

          To sort the facets by value, use the ``bucket`` option. For example, the following request sets
          ...
          <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/faceting.html>`__ in the *Amazon
          CloudSearch Developer Guide* .

        :type filterQuery: string
        :param filterQuery:

          Specifies a structured query that filters the results of a search without affecting how the
          results are scored and sorted. You use ``filterQuery`` in conjunction with the ``query`` parameter
          to filter the documents that match the constraints specified in the ``query`` parameter.
          Specifying a filter controls only which matching documents are included in the results, it has no
          effect on how they are scored and sorted. The ``filterQuery`` parameter supports the full
          structured query syntax.

          For more information about using filters, see `Filtering Matching Documents
          <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/filtering-results.html>`__ in the
          *Amazon CloudSearch Developer Guide* .
        ...
        :param highlight:
          ...
          specified field must be highlight enabled in the domain configuration. The fields and options are
          ...
          * ``format`` : specifies the format of the data in the text field: ``text`` or ``html`` . When
          data is returned as HTML, all non-alphanumeric characters are encoded. The default is ``html`` .
          ...
          * ``post_tag`` : specifies the string to append to an occurrence of a search term. The default for
          ...
          If no highlight options are specified for a field, the returned field text is treated as HTML and
          the first match is highlighted with emphasis tags: ``&lt;em>search-term&lt;/em&gt;`` .
        ...
        :param partial:
          ...
          your users' search experience. For example, rather than displaying no results, you could display
          the partial results and a message indicating that the results might be incomplete due to a
        ...
        :param query: **[REQUIRED]**
          ...
          query parser used for the request and the parser options specified in the ``queryOptions``
          parameter. By default, the ``simple`` query parser is used to process requests. To use the
        ...
        :param queryOptions:
          ...
          Configures options for the query parser specified in the ``queryParser`` parameter. You specify
          ...
          * ``defaultOperator`` : The default operator used to combine individual terms in the search
          ...
          percentage that represents the percentage of terms in the search string (rounded down) that must
          match, rather than a default operator. A value of ``0%`` is the equivalent to OR, and a value of
          ``100%`` is equivalent to AND. The percentage must be specified as a value in the range 0-100
          followed by the percent (%) symbol. For example, ``defaultOperator: 50%`` . Valid values: ``and``
          , ``or`` , a percentage in the range 0%-100% (``dismax`` ). Default: ``and`` (``simple`` ,
          ...
          * ``fields`` : An array of the fields to search when no fields are specified in a search. If no
          fields are specified in a search and this option is not specified, all text and text-array fields
          are searched. You can specify a weight for each field to control the relative importance of each
          field when Amazon CloudSearch calculates relevance scores. To specify a field weight, append a
          caret (``^`` ) symbol and the weight to the field name. For example, to boost the importance of
          the ``title`` field over the ``description`` field you could specify:
          ``"fields":["title^5","description"]`` . Valid values: The name of any configured field and an
          optional numeric value greater than zero. Default: All ``text`` and ``text-array`` fields. Valid
          ...
          * ``operators`` : An array of the operators or special characters you want to disable for the
          ...
          ``near`` disables the ability to use the ~ operator to perform a sloppy phrase search. Disabling
          the ``fuzzy`` operator disables the ability to use the ~ operator to perform a fuzzy search.
          ``escape`` disables the ability to use a backslash (``\\`` ) to escape special characters within
          the search string. Disabling whitespace is an advanced option that prevents the parser from
          tokenizing on whitespace, which can be useful for Vietnamese. (It prevents Vietnamese words from
          being split incorrectly.) For example, you could disable all operators other than the phrase
          ...
          ``phrase`` , ``precedence`` , ``prefix`` , ``whitespace`` . Default: All operators and special
          characters are enabled. Valid for: ``simple`` .

          * ``phraseFields`` : An array of the ``text`` or ``text-array`` fields you want to use for phrase
          searches. When the terms in the search string appear in close proximity within a field, the field
          scores higher. You can specify a weight for each field to boost that score. The ``phraseSlop``
          option controls how much the matches can deviate from the search string and still be boosted. To
          specify a field weight, append a caret (``^`` ) symbol and the weight to the field name. For
          example, to boost phrase matches in the ``title`` field over the ``abstract`` field, you could
          specify: ``"phraseFields":["title^3", "plot"]`` Valid values: The name of any ``text`` or
          ``text-array`` field and an optional numeric value greater than zero. Default: No fields. If you
          don't specify any fields with ``phraseFields`` , proximity scoring is disabled even if
          ...
          * ``explicitPhraseSlop`` : An integer value that specifies how much a match can deviate from the
          ...
          exceed this proximity distance are not considered a match.) For example, to specify a slop of
          three for dismax phrase queries, you would specify ``"explicitPhraseSlop":3`` . Valid values:
          positive integers. Default: 0. Valid for: ``dismax`` .

          * ``tieBreaker`` : When a term in the search string is found in a document's field, a score is
          calculated for that field based on how common the word is in that field compared to other
          documents. If the term occurs in multiple fields within a document, by default only the highest
          scoring field contributes to the document's overall score. You can specify a ``tieBreaker`` value
          to enable the matches in lower-scoring fields to contribute to the document's score. That way, if
          two documents have the same max field score for a particular term, the score for the document that
          has matches in more fields will be higher. The formula for calculating the score with a tieBreaker
          is ``(max field score) + (tieBreaker) * (sum of the scores for the rest of the matching fields)``
        ...
        :param queryParser:
          ...
          Specifies which query parser to use to process the request. If ``queryParser`` is not specified,
          ...
          * ``simple`` : perform simple searches of ``text`` and ``text-array`` fields. By default, the
          ``simple`` query parser searches all ``text`` and ``text-array`` fields. You can specify which
          fields to search by with the ``queryOptions`` parameter. If you prefix a search term with a plus
          sign (+) documents must contain the term to be considered a match. (This is the default, unless
          you configure the default operator with the ``queryOptions`` parameter.) You can use the ``-``
          (NOT), ``|`` (OR), and ``*`` (wildcard) operators to exclude particular terms, find results that
          match any of the specified terms, or search for a prefix. To search for a phrase rather than
          individual terms, enclose the phrase in double quotes. For more information, see `Searching for
          Text <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/searching-text.html>`__ in the
          *Amazon CloudSearch Developer Guide* .
        ...
        :param returnFields:
          ...
          expressions are specified as a comma-separated list. By default, a search response includes all
          return enabled fields (``_all_fields`` ). To return only the document IDs for the matching
          documents, specify ``_no_fields`` . To retrieve the relevance score calculated for each document,
          specify ``_score`` .
        ...
        :param sort:
          ...
          or ``desc`` ) for each field; for example, ``year desc,title asc`` . To use a field to sort
          results, the field must be sort-enabled in the domain configuration. Array type fields cannot be
          used for sorting. If no ``sort`` parameter is specified, results are sorted by their default
          relevance scores in descending order: ``_score desc`` . You can also sort by document ID (``_id
          asc`` ) and version (``_version desc`` ).

          For more information, see `Sorting Results
          <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/sorting-results.html>`__ in the
          *Amazon CloudSearch Developer Guide* .
        ...
        :param start:
          ...
          zero-based; the first result is at index 0. You can specify either the ``start`` or ``cursor``
              ...
              - *(string) --*
                ...
                - *(dict) --*
                  ...
                  - **missing** *(integer) --*
                    ...
                    The number of documents that do not contain a value in the specified field in the result
                    set.
                  ...
                  - **mean** *(string) --*
                    ...
                    If the field is numeric (``int`` , ``int-array`` , ``double`` , or ``double-array`` ),
                    ``mean`` is the string representation of a double-precision 64-bit floating point value.
                    If the field is ``date`` or ``date-array`` , ``mean`` is the string representation of a
                    date with the format specified in `IETF RFC3339 <http://tools.ietf.org/html/rfc3339>`__
                    : yyyy-mm-ddTHH:mm:ss.SSSZ.
    ...
    def suggest(
        ...
        beginning of the field must match the query string to be considered a match.

        For more information about configuring suggesters and retrieving suggestions, see `Getting
        Suggestions
        <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/getting-suggestions.html>`__ in the
        *Amazon CloudSearch Developer Guide* .

        The endpoint for submitting ``Suggest`` requests is domain-specific. You submit suggest requests to
        a domain's search endpoint. To get the search endpoint for your domain, use the Amazon CloudSearch
        configuration service ``DescribeDomains`` action. A domain's endpoints are also displayed on the
        domain dashboard in the Amazon CloudSearch console.
    ...
    def upload_documents(
        ...
        <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/preparing-data.html>`__ in the *Amazon
        CloudSearch Developer Guide* . For more information about uploading data for indexing, see
        `Uploading Data
        <http://docs.aws.amazon.com/cloudsearch/latest/developerguide/uploading-data.html>`__ in the *Amazon
        CloudSearch Developer Guide* .
