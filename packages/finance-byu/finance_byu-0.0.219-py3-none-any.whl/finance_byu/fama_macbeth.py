...
def fm_summary(p):
    ...
    Returns
    -------
    A summary DataFrame with Fama Macbeth standard errors, mean coefficients, and t-statistics.
...
def _numba_assertions(data,t):
    if len(data.drop(t,axis=1).dtypes) > 0:
        assert all([pd.api.types.is_numeric_dtype(d) for d in data.drop(t,axis=1).dtypes]), 'All columns except the time columns must be a numeric type.'
    else:
        assert pd.api.types.is_numeric_dtype(data.drop(t,axis=1).dtypes), 'All columns except the time columns must be a numeric type.'
