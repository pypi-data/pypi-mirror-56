...
def _numba_assertions(data,t):
    if len(data.drop(t,axis=1).dtypes) > 0:
        assert all([(d.kind in 'biufc') for d in data.drop(t,axis=1).dtypes]), 'All columns except the time columns must be a numeric type.'
    else:
        assert data.drop(t,axis=1).dtypes in 'biufc', 'All columns except the time columns must be a numeric type.'
