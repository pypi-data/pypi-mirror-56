...
def test_account_type() -> None:
    assert account.account_type == 'margin'


def test_account_margin_used() -> None:
    assert account.margin_used >= 0


def test_account_buying_power() -> None:
    assert account.buying_power >= 0


def test_account_active_symbols() -> None:
    assert len(account.active_symbols) >= 0


def test_stock_orders() -> None:
    assert len(account.stock_orders) > 0
    for stock_order in account.stock_orders:
        assert stock_order.side in stock_order_sides
        assert stock_order.state in order_states


def test_stock_positions() -> None:
    assert len(account.stock_positions) > 0
    for stock_position in account.stock_positions:
        print(stock_position.symbol, stock_position.quantity)
        assert ALL[stock_position.symbol] == stock_position.instrument_id
        assert stock_position.quantity >= 0


def test_option_orders() -> None:
    assert len(account.option_orders) > 0
    for option_order in account.option_orders:
        assert option_order.state in order_states
        assert option_order.direction in option_directions


def test_option_positions() -> None:
    assert len(account.option_positions) > 0
    for position in account.option_positions:
        assert position.position_type in option_position_types


def test_option_events() -> None:
    assert len(account.option_events) > 0
    for option_event in account.option_events:
...
def test_symbol_history_adbe() -> None:
...
def test_symbol_history_twtr() -> None:
...
def test_symbol_history_sq() -> None:
...
def test_symbol_history_tlry() -> None:
...
def test_symbol_history_shop() -> None:
...
def test_symbol_history_nvda() -> None:
...
def test_symbol_history_lk() -> None:
