...
def add_file_link(f,name):
    ...
    return('<a href='+str(f)+'>'+str(name)+' download</a>\n')

def add_file_link_list(f,name):
    return('<li><a href='+str(f)+'>'+str(name)+' download</a></li>\n')
...
def get_input_file_names(base_dir):
    ...
    for i in range(len(num_files)):
        num_files[i]=num_files[i].strip('_labels.tsv')
...
def add_test_omes(base_dir):
    ...
    if not os.path.isdir(test_dir):
        return()
    test_v_con_str = ""
    test_v_con_str+=add_button_head("How do the consensus groups based on your input datasets differ in the test datasets?")
...
def make_webpage(base_dir):
    ...
    if len(num_files)>0:
        print("found",len(num_files), "categorical files")
        for c in num_files:
            print("\t",c)
