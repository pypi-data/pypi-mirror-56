...
class V2ray:
    ...
    def check(cls):
        ...
        if not os.path.exists("/usr/bin/v2ray/v2ray"):
            if is_ipv6(get_ip()):
                print(ColorStr.yellow("ipv6 network not support install v2ray online, please manual install!"))
                print(ColorStr.fuchsia("u can run 'bash <(curl -L -s https://install.direct/go.sh) -l v2ray-linux-64.zip' to install v2ray locally"))
                return
            else:
