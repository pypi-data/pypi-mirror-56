...
import numpy as np
from band.bert4keras.layers import *
...
class BertModel(object):
    ...
    def variable_mapping(self, variable_names):
        ...
        mapping['Embedding-Token'] = ['bert4keras/embeddings/word_embeddings']
        mapping['Embedding-Position'] = ['bert4keras/embeddings/position_embeddings']
        mapping['Embedding-Segment'] = ['bert4keras/embeddings/token_type_embeddings']
        mapping['Embedding-Norm'] = [
            'bert4keras/embeddings/LayerNorm/gamma',
            'bert4keras/embeddings/LayerNorm/beta',
        ]

        if self.embedding_size != self.hidden_size:
            mapping['Embedding-Mapping'] = [
                'bert4keras/encoder/embedding_hidden_mapping_in/kernel',
                'bert4keras/encoder/embedding_hidden_mapping_in/bias',
            ]
        ...
        for i in range(self.num_hidden_layers):
            try:
                ...
                continue
            if ('bert4keras/encoder/layer_%d/attention/self/query/kernel' % i) in variable_names:
                block_name = 'layer_%d' % i
            else:
                block_name = 'transformer/group_0/inner_group_0'

            mapping['Encoder-%d-MultiHeadSelfAttention' % (i + 1)] = [
                'bert4keras/encoder/%s/attention/self/query/kernel' % block_name,
                'bert4keras/encoder/%s/attention/self/query/bias' % block_name,
                'bert4keras/encoder/%s/attention/self/key/kernel' % block_name,
                'bert4keras/encoder/%s/attention/self/key/bias' % block_name,
                'bert4keras/encoder/%s/attention/self/value/kernel' % block_name,
                'bert4keras/encoder/%s/attention/self/value/bias' % block_name,
                'bert4keras/encoder/%s/attention/output/dense/kernel' % block_name,
                'bert4keras/encoder/%s/attention/output/dense/bias' % block_name,
            ]
            mapping['Encoder-%d-MultiHeadSelfAttention-Norm' % (i + 1)] = [
                'bert4keras/encoder/%s/attention/output/LayerNorm/gamma' % block_name,
                'bert4keras/encoder/%s/attention/output/LayerNorm/beta' % block_name,
            ]
            mapping['Encoder-%d-FeedForward' % (i + 1)] = [
                'bert4keras/encoder/%s/intermediate/dense/kernel' % block_name,
                'bert4keras/encoder/%s/intermediate/dense/bias' % block_name,
                'bert4keras/encoder/%s/output/dense/kernel' % block_name,
                'bert4keras/encoder/%s/output/dense/bias' % block_name,
            ]
            mapping['Encoder-%d-FeedForward-Norm' % (i + 1)] = [
                'bert4keras/encoder/%s/output/LayerNorm/gamma' % block_name,
                'bert4keras/encoder/%s/output/LayerNorm/beta' % block_name,
            ]

        if self.with_pool:
            mapping['Pooler-Dense'] = [
                'bert4keras/pooler/dense/kernel',
                'bert4keras/pooler/dense/bias',
            ]
            if self.with_nsp:
                mapping['NSP-Proba'] = [
    ...
    def load_weights_from_checkpoint(self, checkpoint_file):
        ...
        def load_variable(name):
            ...
            if name in [
                'bert4keras/embeddings/word_embeddings',
