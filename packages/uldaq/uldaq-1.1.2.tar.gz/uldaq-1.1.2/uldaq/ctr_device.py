...
from ctypes import byref, c_double, c_uint, c_ulonglong, Array
...
from .ul_enums import ScanStatus, WaitType, ScanOption, CInScanFlag
...
class CtrDevice:
    ...
    def c_in_scan(self, low_counter_num, high_counter_num, samples_per_counter, rate, options, flags, data):
        # type: (int, int, int, float, ScanOption, CInScanFlag, Array[int]) -> float
        """
        Scans a range counters at the specified rate on the
        device referenced by the :class:`CtrDevice` object and stores the samples.
        ...
        Args:
            ...
            data (Array[int]): The buffer to receive the data being read. Use :class:`create_int_buffer` to
                create the buffer.
        ...
        """
        rate = c_double(rate)
        err = lib.ulCInScan(self.__handle, low_counter_num, high_counter_num, samples_per_counter, byref(rate),
                            options, flags, data)
        if err != 0:
            raise ULException(err)
        return rate.value
    ...
    def scan_wait(self, wait_type, timeout):
        ...
        referenced by the :class:`CtrDevice` object, or the specified timeout elapses.
        ...
        Raises:
            :class:`ULException`
        """
        err = lib.ulCInScanWait(self.__handle, wait_type, None, timeout)
        if err != 0:
            raise ULException(err)
