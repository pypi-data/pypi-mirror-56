...
class InterfaceType(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating the physical connection
    interface used to communicate with a DAQ device. Values can be OR'd together to specify
    multiple interfaces.
    """
...
class DaqEventType(IntEnum):
    ...
    Returned as a bit-wise operable list using :class:`~DaqDeviceInfo.get_event_types`.
    """
...
class MemAccessType(IntEnum):
    ...
    Returned by :class:`~DaqDevice.get_info` as the field types attribute list returned
    in the :class:`MemDescriptor` object.
    """
...
class AiInputMode(IntEnum):
    ...
    analog input methods and with many of the :class:`~AiDevice.get_info` methods."""
...
class AiChanType(IntEnum):
    ...
    Returned by :class:`~AiInfo.get_chan_types`.
    """
    #: Voltage
    VOLTAGE = 1 << 0,
    #: Thermocouple
...
class AInScanFlag(IntEnum):
    ...
    Use as the flags argument value for :func:`~AiDevice.a_in_scan`.
    """
...
class ScanOption(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating optional scan modes.
    These values can be OR'd together to specify multiple options. Used with many
    analog input and output scan functions, as well as a return value
    for :class:`AiInfo.get_scan_options`.
    """
...
class AiChanQueueLimitation(IntEnum):
    """Contains attributes suitable for bit-wise operations indicating device queue limitations.
    Used with :class:`~AiInfo.get_chan_queue_limitations`.
    See also :class:`~AiInfo.get_queue_types` and :class:`~AiInfo.get_max_queue_length` to determine
    queue capabilities"""
...
class DigitalDirection(IntEnum):
    """
    Used with most digital IO methods to specify the direction of programmable ports
    or bits and with  :func:`~DioInfo.get_port_info` as the list returned indicating
    the current direction that each bit in the specified port type is configured for.
...
class DigitalPortIoType(IntEnum):
    """
    Contains attributes indicating the fixed direction or direction programmability
    of the specified digital port. Returned by :func:`~DioInfo.get_port_info`.
    """
...
class DigitalPortType(IntEnum):
    """
    Used with most digital IO methods as the port_type argument to specify the port
    on the device and as a list returned from  :func:`~DioInfo.get_port_types` indicating
...
class DaqInScanFlag(IntEnum):
    """
    Contains attributes indicating conditioning to apply to analog input data before
    it's returned. Use as the flags argument value for :class:`~DaqiDevice.daq_in_scan`.
    """
...
class DaqInChanType(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating channel types
    for the :func:`~DaqiDevice.daq_in_scan` method. Used with synchronous input scanning
    operations as a property in the :class:`DaqInChanDescriptor` object, and as the
    list of bit-wise operable attributes using :func:`DaqiDevice.get_info`.
    """
...
class AOutScanFlag(IntEnum):
    """
    Contains attributes indicating the conditioning to apply to analog output data before it's
    written to the D/A. Use as the flags argument value for :func:`~AoDevice.a_out_scan`.
    """
    #: User data should be in *scaled* format (usually volts). The data is then written
    #: to the device after converting to native device format and calibration factors
    #: (if any) applied.
...
class DaqOutChanType(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating channel types
    for the :func:`~DaqoDevice.daq_out_scan` method. Used with synchronous output scanning
    operations as a property in the :class:`DaqOutChanDescriptor` object, and as the
    list of bit-wise operable attributes using :func:`DaqoDevice.get_info`
    """
...
class CounterDebounceMode(IntEnum):
    """
    Use as the debounce_mode argument for :func:`~CtrDevice.c_config_scan` to set the glitch rejection
...
class CounterDebounceTime(IntEnum):
    """
    Use as the value for the debounce_time argument for :func:`~CtrDevice.c_config_scan` when
    :class:`CounterDebounceMode` is not NONE.
    """
...
class CounterEdgeDetection(IntEnum):
    """
    Use as the value for the edge_detection argument for :func:`~CtrDevice.c_config_scan`.
    """
...
class CounterMeasurementMode(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating counter operation modes.
    Compatible values can be OR'd together. Use as the value for the
    measurement_mode argument for :func:`~CtrDevice.c_config_scan`.
...
class CounterMeasurementType(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating counter operation modes.
    Use individually as the value for the measurement_type argument for
    :func:`~CtrDevice.c_config_scan`.
    Use :class:`~CtrDevice.get_info` with :func:`~CtrInfo.get_measurement_types` to get a list of
    bit-wise operable attributes to check device capabilities.
"""
...
class CounterRegisterType(IntEnum):
    """
    Contains attributes suitable for bit-wise operations indicating counter register
    types available. Used individually for the :class:`~CtrDevice.c_load` register_type argument,
    and as the value returned by to get a list of bit-wise operable attributes using
...
class CounterTickSize(IntEnum):
    """
    Use as the value for the tick_size argument for :class:`~CtrDevice.c_config_scan` when
    :class:`~CounterMeasurementType` is set to :class:`~CounterMeasurementType.PERIOD`,
...
class TmrIdleState(IntEnum):
    """
    Timer idle state. Used as the idle_state argument of :class:`~TmrDevice.pulse_out_start`.
    """
    LOW = 1,  #: Idle low
    HIGH = 2,  #: Idle high
...
class TmrStatus(IntEnum):
    """
    Used with :class:`~TmrDevice.get_pulse_out_status`, as the timer status returned
    for the specified device.
    """
...
class PulseOutOption(IntEnum):
    """
    Used with :class:`~TmrDevice.pulse_out_start` as the options argument value to set
    advanced options for the specified device.
    """
