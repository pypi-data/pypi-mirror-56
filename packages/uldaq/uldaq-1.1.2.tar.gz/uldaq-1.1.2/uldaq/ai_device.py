...
class AiDevice:
    ...
    def a_in(self, channel, input_mode, analog_range, flags):
        ...
        """
        data = c_double()
        err = lib.ulAIn(self.__handle, channel, input_mode, analog_range, flags, byref(data))
        if err != 0:
            raise ULException(err)
        return data.value
    
    def a_in_scan(self, low_channel, high_channel, input_mode, analog_range, samples_per_channel, rate, options, flags,
                  data):
        ...
        Args:
            ...
            flags (AInScanFlag): One or more of the attributes
                (suitable for bit-wise operations) specifying the conditioning
                applied to the data before it is returned.
            data (Array[float]): The buffer to receive the data. Use :class:`create_float_buffer` to
                create the buffer.
        ...
        """
        rate = c_double(rate)
        err = lib.ulAInScan(self.__handle, low_channel, high_channel, input_mode, analog_range,
    ...
    def a_in_load_queue(self, queue):
        ...
        """
        Loads the A/D queue of the device referenced by the :class:`AiDevice` object.

        Args:
            queue (list[AiQueueElement]): A list of AiQueueElement structs, each of which
                contains fields specifying the channel, range, and mode.
    ...
    def set_trigger(self, trig_type, trig_chan, level, variance, retrigger_sample_count):
        ...
        Args:
            trig_type (TriggerType): One of the :class:`TriggerType` attributes that
                determines the type of the external trigger.
            ...
            level (float): The level at or around which the trigger event should be
                detected; ignored if trig_type is set to
                :class:`~TriggerType.POS_EDGE` :class:`~TriggerType.NEG_EDGE`,
                :class:`~TriggerType.HIGH`, :class:`~TriggerType.LOW`,
                :class:`~TriggerType.GATE_HIGH`, :class:`~TriggerType.GATE_LOW`,
                :class:`~TriggerType.RISING`, or :class:`~TriggerType.FALLING`.
    ...
    def t_in_list(self, low_chan, high_chan, scale, flags=TInListFlag.DEFAULT,
        ...
        Args:
            ...
            flags (Optional[TInArrayFlag]): One or more flag values
                (suitable for bit-wise operations) specifying the
                conditioning applied to the data before it is returned.
            ignore_open_connection (bool): If true, exceptions due to open
                thermocouple detection are suppressed and a data value of
