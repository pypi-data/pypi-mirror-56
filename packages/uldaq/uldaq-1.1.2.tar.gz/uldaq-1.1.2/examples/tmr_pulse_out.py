...
"""
    ...
    3. Call DaqDevice.get_tmr_device() to get the TmrDevice object for the timer subsystem
    4. Verify the TmrDevice object is valid
    5. Call DaqDevice.connect() to connect to the device
    6. Call TmrDevice.pulse_out_start() to start the output pulse for the specified timer
    7. Call TmrDevice.get_pulse_out_status() to get the output status and display the status
    8. Call TmrDevice.scan_stop() to stop the scan
    9. Call DaqDevice.disconnect() and DaqDevice.release() before exiting the process
"""
...
def main():
    ...
    try:
        ...
        try:
            print('\n    Outputting {0:.6f} Hz pulse with duty cycle {1:.3f} for timer {2:d}'.format(
                  frequency, duty_cycle, timer_number))
