...
"""
    ...
    3.  Call DaqDevice.get_daqo_device() to get the DaqoDevice object for the DAQ output subsystem
    ...
    9.  Call DaqoDevice.get_scan_status() to get the scan status and display the status.
    10. Call DaqoDevice.scan_stop() to stop the scan
    11. Call DaqDevice.disconnect() and DaqDevice.release() before exiting the process
"""
...
def main():
    ...
    try:
        ...
        if DaqOutChanType.ANALOG in supported_channel_types:
            configure_analog_channels(daq_device, analog_low_channel, analog_high_channel,
                                      analog_range_index, channel_descriptors, amplitudes)
        if DaqOutChanType.DIGITAL in supported_channel_types:
            configure_digital_channels(daq_device, digital_low_port_index, digital_high_port_index,
        ...
        create_output_data(channel_descriptors, samples_per_channel, samples_per_cycle, amplitudes, out_buffer)
        ...
        try:
            while True:
                ...
                print('    Current index:      ', transfer_status.current_index)
                stdout.flush()
                sleep(0.1)
        ...
        if daq_device:
            ...
            for chan in channel_descriptors:
                if chan.type == DaqOutChanType.DIGITAL:
                    dio_device.d_config_port(chan.channel, DigitalDirection.INPUT)
...
def create_output_data(channel_descriptors, samples_per_channel, samples_per_cycle, amplitudes, data_buffer):
