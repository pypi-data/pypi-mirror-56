...
"""
    ...
    3.  Call DaqDevice.get_ctr_device() to get the CtrDevice object for the counter subsystem
    ...
    13. Call DaqDevice.disconnect() and DaqDevice.release() before exiting the process
"""
...
from uldaq import (get_daq_device_inventory, DaqDevice, create_int_buffer, InterfaceType,
                   CInScanFlag, ScanStatus, ScanOption, CounterMeasurementType)
...
def main():
    ...
    try:
        ...
        sample_rate = ctr_device.c_in_scan(low_counter, high_counter, samples_per_channel,
            ...
            while True:
                try:
                    ...
                    for counter_index in range(number_of_counters):
                        counter_value = data[transfer_status.current_index - counter_index]
                        print('    Counter ', (counter_index + low_counter), ':', str(counter_value).rjust(12), sep='')
