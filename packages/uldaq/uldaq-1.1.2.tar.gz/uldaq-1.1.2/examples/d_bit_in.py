...
"""
    ...
    3. Call daq_device.get_dio_device() to get the dio_device object for the digital subsystem
    ...
    8. Call dio_device.d_config_bit to configure the port for bit I/O if the the port is bit
       configurable; otherwise, call dio_device.d_config_port to configure the port for port I/O
    9. Call dio_device.d_bit_in() to read each bit of the digital port
    10. Display the data for each bit
    11. Call daq_device.disconnect() and daq_device.release() before exiting the process.
"""
