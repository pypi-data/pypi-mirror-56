...
from ctypes import c_double, c_uint, byref, Array
...
class DaqoDevice:
    ...
    def daq_out_scan(self, channel_descriptors, samples_per_channel, rate, options, flags, data):
        ...
        """
        Outputs values synchronously to multiple output subsystems, such as analog and
        digital subsystems, on the device referenced by the :class:`DaqoDevice` object. This
        This method only works with devices that support synchronous output.
        ...
        Args:
            ...
            flags (DaqOutScanFlag): One or more attributes (suitable for bit-wise operations) specifying the
                conditioning applied to the data.
            data (Array[float]): The data buffer to be written. Use :class:`create_float_buffer` to
                create the buffer.
        ...
        chan_descriptor_array = _daqo_chan_descriptor_array(number_of_channels, channel_descriptors)
    ...
    def set_trigger(self, trigger_type, trigger_channel, level, variance, retrigger_sample_count):
        ...
        err = lib.ulDaqOutSetTrigger(self.__handle, trigger_type, trigger_channel,
                                     trig_level, trig_variance, retrigger_sample_count)
        if err != 0:
            raise ULException(err)
