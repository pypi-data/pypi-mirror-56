...
from ctypes import c_uint, c_double, byref, Array
from .ul_enums import TriggerType, ScanStatus, WaitType, ScanOption, DaqInScanFlag
...
class DaqiDevice:
    ...
    def set_trigger(self, trigger_type, trigger_channel, level, variance, retrigger_sample_count):
        ...
        Args:
            ...
            level (float): The level at or around which the trigger event should be detected;
                ignored if trig_type is set to
            ...
            variance (float): The degree to which the input signal can vary relative to the
                level parameter; ignored for all types where level is ignored. For
                pattern triggering, this argument serves as the mask value.
            retrigger_sample_count (int): The number of samples per trigger to acquire
                with each trigger event; ignored unless :class:`ScanOption.RETRIGGER`
                is set for the scan.
    ...
    def daq_in_scan(self, channel_descriptors, samples_per_channel, rate, options, flags, data):
        ...
        or counter, on the device referenced by the :class:`DaqiDevice` object and stores
        the samples in an array. This method works only
        ...
        Args:
            ...
            samples_per_channel (int): the number of samples to collect from each
                channel in the scan.
            rate (float): Sample input rate in samples per second.
            options (ScanOption): One or more attributes (suitable for bit-wise operations) specifying the
                optional conditions that will be applied to the scan, such as continuous or external clock.
            flags (DaqInScanFlag): One or more attributes (suitable for bit-wise operations) specifying the
                conditioning applied to the data before it is returned.
            data (Array[float]): The data buffer to receive the data being read. Use :class:`create_float_buffer` to
                create the buffer.
        ...
        rate = c_double(rate)
        chan_descriptor_array = _daqi_chan_descriptor_array(num_channels, channel_descriptors)
    ...
    def scan_wait(self, wait_type, timeout):
        ...
        referenced by the :class:`DaqiDevice` object, or the specified timeout elapses.
        ...
        Raises:
            :class:`ULException`
        """
        err = lib.ulDaqInScanWait(self.__handle, wait_type, None, timeout)
        if err != 0:
            raise ULException(err)
