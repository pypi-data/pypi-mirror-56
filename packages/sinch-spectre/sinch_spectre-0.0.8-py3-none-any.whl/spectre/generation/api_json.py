...
from spectre.generation.utils import make_form, get_form_name, write_to_file
...
def write(content, schema_info):
    ...
    outfile = f"api.json/{schema_info['name']}.json"
    write_to_file(content, outfile)

def generate(schema_info, entities):
    try:
        spec = TEMPLATE
        set_schema_base(spec, schema_info)
        set_schema_models(spec, entities)
        set_schema_resources(spec, entities)
        write(serializer.prettify(spec), schema_info)
    except Exception as e:
        print('An error occured while attempting to generate api.json spec')
        print(e)
