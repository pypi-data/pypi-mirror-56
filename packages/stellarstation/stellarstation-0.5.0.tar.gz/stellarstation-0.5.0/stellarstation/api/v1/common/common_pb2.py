...
Bits = _reflection.GeneratedProtocolMessageType('Bits', (_message.Message,), {
  ...
  ,
  __doc__ = """A set of bits - this is not necessarily aligned to bytes (8-bits).
  
  
  Attributes:
      length:
          The number of bits in this message. Only the most-significant
          ``length`` bits of ``payload`` will be used.
      payload:
          The actual bits in this message. Only the most-significant
          ``length`` bits of ``payload`` will be used.
  """,
