...
def log(tensor: ITensor) -> ITensor:
    ...
    if is_backend(PYTORCH_BACKEND):
        from babilim.tmath.pytorch import log as _log
        return _log(tensor)
    elif is_backend(TF_BACKEND):
        from babilim.tmath.tf import log as _log
        return _log(tensor)
...
def clip(tensor: ITensor, min, max) -> ITensor:
    ...
    """
    if is_backend(PYTORCH_BACKEND):
        from babilim.tmath.pytorch import clip as _clip
        return _clip(tensor, min, max)
    elif is_backend(TF_BACKEND):
        from babilim.tmath.tf import clip as _clip
        return _clip(tensor, min, max)
...
def pow(tensor: ITensor, exponent: ITensor) -> ITensor:
    ...
    """
    if is_backend(PYTORCH_BACKEND):
        from babilim.tmath.pytorch import pow as _pow
        return _pow(tensor, exponent)
    elif is_backend(TF_BACKEND):
        from babilim.tmath.tf import pow as _pow
        return _pow(tensor, exponent)
