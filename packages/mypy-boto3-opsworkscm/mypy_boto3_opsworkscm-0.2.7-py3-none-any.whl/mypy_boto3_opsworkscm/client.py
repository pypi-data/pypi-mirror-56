...
class Client(BaseClient):
    ...
    def associate_node(
        ...
        *MyManagedNode* --engine-attributes "Name=*PUPPET_NODE_CSR* ,Value=*csr-pem* "``
        ...
        not exist. A ``ValidationException`` is raised when parameters of the request are not valid. The
        AssociateNode API call can be integrated into Auto Scaling configurations, AWS Cloudformation
    ...
    def create_backup(
        ...
        """
        Creates an application-level backup of a server. While the server is in the ``BACKING_UP`` state,
        the server cannot be changed, and no additional backup can be created.

        Backups can be created for servers in ``RUNNING`` , ``HEALTHY`` , and ``UNHEALTHY`` states. By
    ...
    def create_server(
        ...
        account. A ``ResourceNotFoundException`` is thrown when you specify a backup ID that is not valid or
        is for a backup that does not exist. A ``ValidationException`` is thrown when parameters of the
         ...
         *Chef Automate:* The default security group opens the Chef server to the world on TCP port 443. If
         a KeyName is present, AWS OpsWorks enables SSH access. SSH is also open to the world on TCP port
         22.

         *Puppet Enterprise:* The default security group opens TCP ports 22, 443, 4433, 8140, 8142, 8143,
         and 8170. If a KeyName is present, AWS OpsWorks enables SSH access. SSH is also open to the world
         on TCP port 22.

        By default, your server is accessible from any IP address. We recommend that you update your
        security group rules to allow access from known IP addresses and address ranges only. To edit
        security group rules, open Security Groups in the navigation pane of the EC2 management console.

        To specify your own domain for a server, and provide your own self-signed or CA-signed certificate
        and private key, specify values for ``CustomDomain`` , ``CustomCertificate`` , and
        ``CustomPrivateKey`` .
        ...
        **Request Syntax**
        ...
        :param CustomDomain:
          ...
          server, create a CNAME DNS record in your preferred DNS service that points the custom domain to
          the endpoint that is generated when the server is created (the value of the CreateServer Endpoint
          attribute). You cannot access the server by using the generated ``Endpoint`` value if the server
          is using a custom domain. If you specify a custom domain, you must also specify values for
          ``CustomCertificate`` and ``CustomPrivateKey`` .

        :type CustomCertificate: string
        :param CustomCertificate:

          A PEM-formatted HTTPS certificate. The value can be be a single, self-signed certificate, or a
        ...
        :param CustomPrivateKey:
          ...
          A private key in PEM format for connecting to the server by using HTTPS. The private key must not
          be encrypted; it cannot be protected by a password or passphrase. If you specify a custom private
          key, you must also specify values for ``CustomDomain`` and ``CustomCertificate`` .

        :type DisableAutomatedBackup: boolean
        :param DisableAutomatedBackup:

          Enable or disable scheduled backups. Valid values are ``true`` or ``false`` . The default value is
          ``true`` .
        ...
        :param EngineVersion:
          ...
          The major release version of the engine that you want to use. For a Chef server, the valid value
          for EngineVersion is currently ``12`` . For a Puppet server, the valid value is ``2017`` .
        ...
        :param EngineAttributes:
          ...
          * ``CHEF_AUTOMATE_ADMIN_PASSWORD`` : The password for the administrative user in the Chef Automate
          web-based dashboard. The password length is a minimum of eight characters, and a maximum of 32.
          The password can contain letters, numbers, and special characters (!/@#$%^&+=_). The password must
        ...
        :param ServerName: **[REQUIRED]**
          ...
          The name of the server. The server name must be unique within your AWS account, within each
          region. Server names must start with a letter; then letters, numbers, or hyphens (-) are allowed,
          up to a maximum of 40 characters.

        :type InstanceProfileArn: string
        :param InstanceProfileArn: **[REQUIRED]**

          The ARN of the instance profile that your Amazon EC2 instances use. Although the AWS OpsWorks
          console typically creates the instance profile for you, if you are using API commands instead, run
        ...
        :param PreferredMaintenanceWindow:
          ...
          The start time for a one-hour period each week during which AWS OpsWorks CM performs maintenance
          on the instance. Valid values must be specified in the following format: ``DDD:HH:MM`` . The
          specified time is in coordinated universal time (UTC). The default value is a random one-hour
          period on Tuesday, Wednesday, or Friday. See ``TimeWindowDefinition`` for more information.
        ...
        :param PreferredBackupWindow:
          ...
          The start time for a one-hour period during which AWS OpsWorks CM backs up application-level data
          on your server if automated backups are enabled. Valid values must be specified in one of the
          following formats:
        ...
        :param ServiceRoleArn: **[REQUIRED]**
          ...
          the AWS OpsWorks management console typically creates the service role for you, if you are using
          the AWS CLI or API commands, run the service-role-creation.yaml AWS CloudFormation template,
          located at
          https://s3.amazonaws.com/opsworks-cm-us-east-1-prod-default-assets/misc/opsworks-cm-roles.yaml.
          ...
          ::

            {
                'Server': {
                    ...
                    'ServiceRoleArn': 'string',
                    'Status':
                    'BACKING_UP'|'CONNECTION_LOST'|'CREATING'|'DELETING'|'MODIFYING'|'FAILED'|'HEALTHY'
                    |'RUNNING'|'RESTORING'|'SETUP'|'UNDER_MAINTENANCE'|'UNHEALTHY'|'TERMINATED',
                    'StatusReason': 'string',
                    'SubnetIds': [
                        'string',
                    ],
          ...
          - *(dict) --*
            ...
            - **Server** *(dict) --*
              ...
              - **CustomDomain** *(string) --*
                ...
                An optional public endpoint of a server, such as ``https://aws.my-company.com`` . You cannot
              ...
              - **EngineAttributes** *(list) --*
                ...
                * ``PUPPET_STARTER_KIT`` : A base64-encoded ZIP file. The ZIP file contains a Puppet starter
                kit, including a README and a required private key. Save this file, unzip it, and then
              ...
              - **SecurityGroupIds** *(list) --*
                ...
                The security group IDs for the server, as specified in the CloudFormation stack. These might
                not be the same security groups that are shown in the EC2 console.
    ...
    def delete_backup(self, BackupId: str) -> Dict[str, Any]:
        ...
        ``ResourceNotFoundException`` is thrown when the backup does not exist. A ``ValidationException`` is
    ...
    def delete_server(self, ServerName: str) -> Dict[str, Any]:
        ...
        instance). When you run this command, the server state is updated to ``DELETING`` . After the server
        is deleted, it is no longer returned by ``DescribeServer`` requests. If the AWS CloudFormation stack
        ...
        ``ResourceNotFoundException`` is thrown when the server does not exist. A ``ValidationException`` is
    ...
    def describe_backups(
        ...
        """
        Describes backups. The results are ordered by time, with newest backups first. If you do not specify
        a BackupId or ServerName, the command returns all backups.
    ...
    def describe_events(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          previous results as the value of the ``nextToken`` parameter. If there are no more results, the
          response object's ``nextToken`` parameter value is ``null`` . Setting a ``nextToken`` value that
          was not returned in your previous results causes an ``InvalidNextTokenException`` to occur.
    ...
    def describe_node_association_status(
        ...
        A ``ResourceNotFoundException`` is thrown when no recent association or disassociation request with
        the specified token is found, or when the server does not exist. A ``ValidationException`` is raised
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **EngineAttributes** *(list) --*
              ...
              Attributes specific to the node association. In Puppet, the attibute PUPPET_NODE_CERT contains
              the signed certificate (the result of the CSR).
    ...
    def describe_servers(
        ...
        """
        Lists all configuration management servers that are identified with your account. Only the stored
        results from Amazon DynamoDB are returned. AWS OpsWorks CM does not query other services.
    ...
    def disassociate_node(
        ...
        """
        Disassociates a node from an AWS OpsWorks CM server, and removes the node from the server's managed
        nodes. After a node is disassociated, the node key pair is no longer valid for accessing the
        configuration manager's API. For more information about how to associate a node, see  AssociateNode
        .
        ...
        **Request Syntax**
        ...
        :param EngineAttributes:
          ...
          Engine attributes that are used for disassociating the node. No attributes are required for
          Puppet.
    ...
    def export_server_engine_attribute(
        ...
        """
        Exports a specified server engine attribute as a base64-encoded string. For example, you can export
        user data that you can use in EC2 to associate nodes with a server.

        This operation is synchronous.

        A ``ValidationException`` is raised when parameters of the request are not valid. A
        ``ResourceNotFoundException`` is thrown when the server does not exist. An ``InvalidStateException``
        is thrown when the server is in any of the following states: CREATING, TERMINATED, FAILED or
        DELETING.
        ...
        **Request Syntax**
        ...
        :param InputAttributes:
          ...
          The list of engine attributes. The list type is ``EngineAttribute`` . An ``EngineAttribute`` list
          item is a pair that includes an attribute name and its value. For the ``Userdata``
          ...
          * **OrganizationName** In Chef, an organization name. AWS OpsWorks for Chef Automate always
          creates the organization ``default`` . In Puppet, this parameter is ignored.

          * **NodeEnvironment** In Chef, a node environment (for example, development, staging, or one-box).
          In Puppet, this parameter is ignored.

          * **NodeClientVersion** In Chef, the version of the Chef engine (three numbers separated by dots,
          such as 13.8.5). If this attribute is empty, OpsWorks for Chef Automate uses the most current
    ...
    def restore_server(
        ...
        ``UNHEALTHY`` , or ``TERMINATED`` state. When you run RestoreServer, the server's EC2 instance is
        deleted, and a new EC2 instance is configured. RestoreServer maintains the existing server endpoint,
        so configuration management of the server's client devices (nodes) should continue to work.
        ...
        **Request Syntax**
        ...
        :param InstanceType:
          ...
          ``^([cm][34]|t2).*`` For example, ``m5.large`` . Valid values are ``m5.large`` , ``r5.xlarge`` ,
          and ``r5.2xlarge`` . If you do not specify this parameter, RestoreServer uses the instance type
          from the specified backup.

        :type KeyPair: string
        :param KeyPair:

          The name of the key pair to set on the new EC2 instance. This can be helpful if the administrator
          no longer has the SSH key.
    ...
    def start_maintenance(
        ...
        """
        Manually starts server maintenance. This command can be useful if an earlier maintenance attempt
        failed, and the underlying cause of maintenance failure has been resolved. The server is in an
        ``UNDER_MAINTENANCE`` state while maintenance is in progress.

        Maintenance can only be started on servers in ``HEALTHY`` and ``UNHEALTHY`` states. Otherwise, an
        ``InvalidStateException`` is thrown. A ``ResourceNotFoundException`` is thrown when the server does
        not exist. A ``ValidationException`` is raised when parameters of the request are not valid.
    ...
    def update_server_engine_attributes(
        ...
        """
        Updates engine-specific attributes on a specified server. The server enters the ``MODIFYING`` state
        when this operation is in progress. Only one update can occur at a time. You can use this command to
        reset a Chef server's public key (``CHEF_PIVOTAL_KEY`` ) or a Puppet server's admin password
        (``PUPPET_ADMIN_PASSWORD`` ).

        This operation is asynchronous.

        This operation can only be called for servers in ``HEALTHY`` or ``UNHEALTHY`` states. Otherwise, an
