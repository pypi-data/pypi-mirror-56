...
class DicomScanner(AbstractScanner):
    ...
    def discover(self, walker, context, container_factory, path_prefix=None, audit_log=None):
        ...
        for path in files:
            sys.stdout.write(f'Scanning {files_scanned}/{file_count} files...'.ljust(80) + '\r')
            sys.stdout.flush()
            ...
            try:
                with walker.open(path, 'rb', buffering=self.config.buffer_size) as f:
                    ...
                    dcm = DicomFile(f, parse=False, session_label_key=self.session_label_key,
                                    decode=self.related_acquisitions, stop_when=_at_stack_id(self.related_acquisitions),
                                    update_in_place=False, specific_tags=tags, force=is_dicom)
                    acquisition = self.resolve_acquisition(context, dcm)
                    sop_uid = self.get_value(dcm, 'SOPInstanceUID', required=True)
                    try:
                        self._before_add_instance(path, sop_uid, context, walker, audit_log)
                ...
                if is_dicom:
                    self.report_file_error(audit_log, path, exc=exc, msg='Not a DICOM - {}'.format(exc))
                else:
        ...
        for session in self.sessions.values():
            ...
            for acquisition in itertools.chain(session.acquisitions.values(), session.secondary_acquisitions.values()):
                ...
                try:
                    self._before_resolve_acquisition(acquisition, walker, container_factory.resolver, audit_log)
    ...
    def _before_add_instance(self, path, sop_uid, context, walker, audit_log):
        ...
        """
        if sop_uid in context.get('sop_uids', []) and self.config.check_unique_uids and self.config.copy_duplicates:
            ...
            self.report_file_error(audit_log, path, msg=message, msg_prefix='Skipping file')
    ...
    def _before_resolve_acquisition(self, acquisition, walker, resolver, audit_log):
        ...
        if self.config.check_unique_uids and self.config.copy_duplicates:
            ...
            if not acquisition_ctx['uid'] in result['acquisitions']:
                return
            files_to_move = itertools.chain.from_iterable(map(lambda k__v: k__v[1].values(), acquisition.files.items()))
            for f in files_to_move:
                message = ('Copying to {} because acquisition '
                           'already exists with this SeriesInstanceUID: {}'
                           ).format(self.config.duplicates_folder, acquisition_ctx['uid'])
                self.report_file_error(audit_log, f, msg=message, msg_prefix='Skipping file')
    ...
    def _get_duplicates_fs(self, walker):
        if not self.config.duplicates_folder:
            if isinstance(walker, walkers.PyFsWalker):
                ...
                self.config.duplicates_folder = fs.path.combine(parent_dir, 'duplicates')
            else:
                log.warning(f"{walker.__class__.__name__} walker does not support copying files, duplicates won't be collected")
                return None
