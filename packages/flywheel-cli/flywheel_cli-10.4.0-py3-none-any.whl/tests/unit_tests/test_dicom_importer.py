...
def mock_dcm(value):
    if isinstance(value, dict):
        return MockDCM(**{key: mock_dcm(val) for key, val in value.items()}, return_value=None)
...
def test_move_uid_duplicates(temp_fs, dicom_data, mocker):
    tmpfs, tmpfs_url = temp_fs(collections.OrderedDict({
        'DICOM': [
            ('002.dcm', dicom_data('16844_1_1_dicoms', 'MR.1.2.840.113619.2.408.5282380.5220731.23348.1516669692.164.dcm')),
            ('001.dcm', dicom_data('16844_1_1_dicoms', 'MR.1.2.840.113619.2.408.5282380.5220731.23348.1516669692.164.dcm')),
        ],
    }))
    ...
    walker = PyFsWalker(tmpfs_url, src_fs=tmpfs)
    dicom_scanner.discover(walker, context, container_factory)
    mock_copy.copy_file.assert_called_once_with(mock.ANY, 'DICOM/002.dcm', mock.ANY, 'DICOM/002.dcm')
    itr = iter(container_factory.walk_containers())
    ...
    resolver.reserved_uids = {
        'acquisitions': [
            '12840113619240852823805220731304241516669014474'
        ]
    }
    container_factory = ContainerFactory(resolver)
    ...
    walker = PyFsWalker(tmpfs_url, src_fs=tmpfs)
    dicom_scanner.discover(walker, context, container_factory)
    mock_copy.copy_file.assert_any_call(mock.ANY, 'DICOM/001.dcm', mock.ANY, 'DICOM/001.dcm')
    mock_copy.copy_file.assert_any_call(mock.ANY, 'DICOM/002.dcm', mock.ANY, 'DICOM/002.dcm')
    itr = iter(container_factory.walk_containers())
    # nothing to import
    try:
        next(itr)
