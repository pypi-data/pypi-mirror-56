...
class TeradataDialect(default.DefaultDialect):
    ...
    def _get_volatile_tables_list(self, connection, table_name=None):
        ...
        return [row['Table Dictionary Name'] for row in res]
        ...
        stmt = select([column('tablename')],
                      from_obj=[text('dbc.tablesvx')]).where(
                        and_(text('DatabaseName (NOT CASESPECIFIC) = ' + self._cast_name(':schema') + ' (NOT CASESPECIFIC)'),
                             ...
                             text("TableKind (NOT CASESPECIFIC) IN ('O' (NOT CASESPECIFIC),"
                                  " 'Q' (NOT CASESPECIFIC), 'T' (NOT CASESPECIFIC))")))

        res = connection.execute(stmt, schema=schema_name, table_name=table_name).fetchone()
    ...
    def has_view(self, connection, view_name, schema=None):
        ...
        res = connection.execute(stmt, schema=schema_name, view_name=view_name).fetchone()
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        if isView or isVolatile:
            ...
            stmt = select(['*'], from_obj=text('dbc.ColumnsV')).where(
                and_(text('DatabaseName (NOT CASESPECIFIC) = ' + schema_name + ' (NOT CASESPECIFIC)'),
                     text('TableName=:table_name'))).order_by(text("ColumnId"))
        ...
        if isView or isVolatile:
            res = [dict(r, **(self._update_column_help_info(r))) for r in res]
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        stmt = select([column('ColumnName'), column('IndexName')],
                      from_obj=[text('dbc.IndicesV')]).where(
                          and_(text('DatabaseName (NOT CASESPECIFIC) = ' + self._cast_name(':schema') + ' (NOT CASESPECIFIC)'),
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        stmt = select([column('ColumnName'), column('IndexName')], from_obj=[text('dbc.IndicesV')]) \
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        try:
            ...
            if ('3720' in str(err) and 'This view does not contain any complete index columns of the underlying table'
                    in str(err)) or \
               ('3823' in str(err) and 'may not be used for Help Index/Constraint/Statistics, Update, Delete or Insert'
                in str(err)):
                pass
            else:
                raise
