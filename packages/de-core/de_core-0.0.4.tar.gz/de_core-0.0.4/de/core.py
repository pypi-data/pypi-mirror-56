...
def file_load_if_exists(args_str: str) -> str:
...
def load_requirements(namespace_name: str, namespace_dict: Dict[str, Union[str, List[str]]], path: str) -> List[str]:
    """ load requirements from *requirements.txt and return list of portions package names.

    :param namespace_name:  name space name/id.
    :param namespace_dict:  namespace environment dict.
    :param path:            folder from where to load the *requirements.txt files.
    :return:                portion package name.
    """
    dev_require: List[str] = list()
    requirements_file = os.path.join(path, REQ_FILE_NAME)
    if os.path.exists(requirements_file):
        dev_require.extend(
            _ for _ in file_content(requirements_file).strip().split('\n') if not _.startswith('#'))
    namespace_dict['docs_require'] = [
        _ for _ in dev_require if _.startswith('sphinx_')]
    namespace_dict['install_require'] = [
        _ for _ in dev_require if not _.startswith('sphinx_')]
    namespace_dict['setup_require'] = [
        _ for _ in namespace_dict['install_require'] if not _.startswith(f'{namespace_name}_')]  # e.g. de_core
    namespace_dict['portions_package_names'] = portions_package_names = [
        _ for _ in dev_require if _.startswith(f'{namespace_name}_')]

    tests_require: List[str] = list()
    requirements_file = os.path.join(path, REQ_TEST_FILE_NAME)
    if os.path.exists(requirements_file):
        tests_require.extend(
            _ for _ in file_content(requirements_file).strip().split('\n') if not _.startswith('#'))
    namespace_dict['tests_require'] = tests_require
...
def namespace_env(namespace_name: str, pkg_root_path: str = "") -> Dict[str, Union[str, List[str]]]:
    ...
    # add string globals like PORTIONS_COMMON_DIR
    for var_name, var_val in globals().items():
        if isinstance(var_val, str):
            nse[var_name] = var_val

    # load requirements from *requirements.txt
    portions_package_names = load_requirements(namespace_name, nse, setup_path)
    ...
    # finally check if optional extend_namespace_env.py exists and if yes then run it for to patch env vars/values
    file = EXTEND_NAMESPACE_FILE_NAME
...
def patch_str(string: str, patch_name: str, replacer: Callable[[str], str]) -> str:
    ...
    :param patch_name:  name/id of the placeholder.
    ...
    """
    beg = 0
    pre = TEMPLATE_PLACEHOLDER_ID_PREFIX + patch_name + TEMPLATE_PLACEHOLDER_ID_SUFFIX
    len_pre = len(pre)
    len_suf = len(TEMPLATE_PLACEHOLDER_ARGS_SUFFIX)
    while True:
        beg = string.find(pre, beg)
        if beg == -1:
            break
        end = string.find(TEMPLATE_PLACEHOLDER_ARGS_SUFFIX, beg)
        if end == -1:
            break       # ignoring any placeholder without suffix
        string = string[:beg] + replacer(string[beg + len_pre: end].strip()) + string[end + len_suf:]
...
def patch_templates(namespace_env_vars: Dict[str, str], exclude_folder: str = '') -> List[str]:
    ...
    for file_path in glob.glob(f"**/{TEMPLATE_FILE_NAME_PREFIX}*.*", recursive=True):
        if not exclude_folder or not file_path.startswith(exclude_folder + os.path.sep):
            ...
            content = patch_str(content, TEMPLATE_INCLUDE_FILE_PLACEHOLDER_ID, file_load_if_exists)
