...
def sqlserver():
	""" where we discover whether sql server is running on same machine or in a cluster, if cluster, nextone"""
	returns (1,"socket|ip","")

def sockconnect(socket="/tmp/mysql.sock",user="root",passwd="kingkong12",db=""):
	""" connect via socket """
	try:
		conn = connector.Connect(unix_socket=socket,user=user,passwd=passwd)
...
def ipconnect(ip="127.0.0.1",port="3306",user="root",passwd="",db=""):
