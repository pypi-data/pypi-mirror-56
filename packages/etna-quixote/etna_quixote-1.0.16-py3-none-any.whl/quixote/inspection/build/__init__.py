...
def gcc(
    ...
    if version:
        cmd += "-" + str(version)
    return inspection._run([cmd, *output_file_option, *src, *options], capture_output=True, env=env)
...
def javac(*source_files: str, options: List[str] = None, env: Mapping[str, str] = None) -> inspection.CompletedProcess:
    ...
    options = options or []
    cmd = "javac"
    return inspection._run([cmd, *source_files, *options], capture_output=True, env=env)
