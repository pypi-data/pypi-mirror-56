...
def rasterSubmit(mapId, r,  bands, timestamp, xMin = None, xMax = None, yMin = None, yMax= None, tileId = None, token = None):
    ...
    if str(type(xMin)) != "<class 'NoneType'>":
        ...
        for i in np.arange(cover.shape[0]):
            ...
            for j in np.arange(r_sub.shape[1]):
                if bands[j] == 'label':
                    body = {'mapId':mapId, 'tileId':{'tileX':tileX, 'tileY':tileY, 'zoom':zoom}, 'timestamp':timestamp, 'Type':bands[j], 'newLabel': r_sub[:,:,j].tolist() }
                elif 'transparent' in bands:
                        body = {'mapId':mapId, 'tileId':{'tileX':tileX, 'tileY':tileY, 'zoom':zoom}, 'timestamp':timestamp, 'Type':bands[j], 'newLabel': r_sub[:,:,bands.index('transparent')].tolist(), 'newData':r_sub[:,:,j].tolist() }
                else:
    ...
    elif str(type(tileId)) == "<class 'dict'>":
            tileX = int(tileId['tileX'])
            tileY = int(tileId['tileY'])
            zoom = int(tileId['zoom'])
            ...
            for j in np.arange(r_sub.shape[1]):
                if bands[j] == 'label':
                    body = {'mapId':mapId, 'tileId':{'tileX':tileX, 'tileY':tileY, 'zoom':zoom}, 'timestamp':timestamp, 'Type':bands[j], 'newLabel': r[:,:,j].tolist() }
                elif 'transparent' in bands:
                        body = {'mapId':mapId, 'tileId':{'tileX':tileX, 'tileY':tileY, 'zoom':zoom}, 'timestamp':timestamp, 'Type':bands[j], 'newLabel': r[:,:,bands.index('transparent')].tolist(), 'newData':r[:,:,j].tolist() }
                else:
                    body = {'mapId':mapId, 'tileId':{'tileX':tileX, 'tileY':tileY, 'zoom':zoom}, 'timestamp':timestamp, 'Type':bands[j], 'newData':r[:,:,j].tolist() }
