...
class Analyzer:
    ...
    def full_analyze_request(
        ...
        execution_order = ExecutionOrder.compute(
            self._registry_data, analyzer=specified_analyzer, interactive=interactive
        )
        ...
        container_output_path: Optional[Path] = None
        for entry in execution_order.analysis_phase:
            dependency = entry.analyzer
            interactive = entry.interactive
            if interactive:
                print(f"Calling `docker exec` into analyzer with name {dependency}")
            ...
            if (
                ...
                self.dependency_mounter.contains(
                    CacheKey(analyzer=dependency, input=analyzer_input)
                )
                and not force
                and not interactive
            ):
                ...
                self._logger.info(
                    f"Analysis for {analyzer_input.to_json()} {dependency} already exists. Keeping old analysis report"
                )
            else:
                container_output_path = self._analyze(
                    dependency, analyzer_input, interactive=interactive
                )
                skipped = False
    ...
    def prepare_mount_volume(
        ...
        with (input_dir / "parameter.json").open("w") as f:
    ...
    def _analyze(
        ...
        cache_key = CacheKey(analyzer=specified_analyzer, input=analyzer_input)
        try:
            with self.prepare_mount_volume(
                specified_analyzer, analyzer_input
            ) as manager:
                manager.copy_input()
