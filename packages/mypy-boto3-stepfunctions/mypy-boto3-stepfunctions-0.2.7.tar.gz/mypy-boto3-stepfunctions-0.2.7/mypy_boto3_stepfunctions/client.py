"Main interface for stepfunctions Client"
...
class Client(BaseClient):
    ...
    def create_activity(
        ...
        any machine that has access to AWS Step Functions. Activities must poll Step Functions using the
        ...
        .. note::
           ...
           ``CreateActivity`` is an idempotent API. Subsequent requests won’t create a duplicate resource if
           it was already created. ``CreateActivity`` 's idempotency check is based on the activity ``name``
           . If a following request has different ``tags`` values, Step Functions will ignore these
           differences and treat it as an idempotent request of the previous. In this case, ``tags`` will
        ...
        **Request Syntax**
        ...
        :param name: **[REQUIRED]**
          ...
          The name of the activity to create. This name must be unique for your AWS account and region for
          90 days. For more information, see `Limits Related to State Machine Executions
        ...
        :param tags:
          ...
          Billing and Cost Management User Guide* , and `Controlling Access Using IAM Tags
    ...
    def create_state_machine(
        ...
        .. note::
           ...
           resource if it was already created. ``CreateStateMachine`` 's idempotency check is based on the
           state machine ``name`` and ``definition`` . If a following request has a different ``roleArn`` or
    ...
    def delete_state_machine(self, stateMachineArn: str) -> Dict[str, Any]:
        ...
        ``DELETING`` and begins the deletion process. Each state machine execution is deleted the next time
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **roleArn** *(string) --*
              ...
              The Amazon Resource Name (ARN) of the IAM role used when creating this state machine. (The IAM
              role maintains security by granting Step Functions access to AWS resources.)
    ...
    def get_activity_task(
        ...
        seconds. If no task is available within 60 seconds, the poll returns a ``taskToken`` with a null
        string.

        .. warning::

          Workers should set their client side socket timeout to at least 65 seconds (5 seconds higher than
          the maximum time the service may hold the poll request).

          Polling with ``GetActivityTask`` can cause latency in some implementations. See `Avoid Latency
          When Polling for Activity Tasks
          <https://docs.aws.amazon.com/step-functions/latest/dg/bp-activity-pollers.html>`__ in the Step
        ...
        **Request Syntax**
        ...
        :param activityArn: **[REQUIRED]**
          ...
          The Amazon Resource Name (ARN) of the activity to retrieve tasks from (assigned when you create
          the task using  CreateActivity .)
    ...
    def get_execution_history(
        ...
        **Request Syntax**
        ...
        :param maxResults:
          ...
          further pages of results. The default is 100 and the maximum allowed page size is 1000. A value of
          0 uses the default.

          This is only an upper limit. The actual number of results returned per call might be fewer than
        ...
        :param nextToken:
          ...
          the next page. Keep all other arguments unchanged. Each pagination token expires after 24 hours.
    ...
    def list_executions(
        ...
        """
        Lists the executions of a state machine that meet the filtering criteria. Results are sorted by
        time, with the most recent execution first.
    ...
    def send_task_failure(
        ...
        **Request Syntax**
        ...
        :param taskToken: **[REQUIRED]**
          ...
          assigned to a worker, or in the `context object
          <https://docs.aws.amazon.com/step-functions/latest/dg/input-output-contextobject.html>`__ when a
    ...
    def send_task_heartbeat(self, taskToken: str) -> Dict[str, Any]:
        ...
        Used by activity workers and task states using the `callback
        <https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-wait-token>`__
        pattern to report to Step Functions that the task represented by the specified ``taskToken`` is
        still making progress. This action resets the ``Heartbeat`` clock. The ``Heartbeat`` threshold is
        specified in the state machine's Amazon States Language definition (``HeartbeatSeconds`` ). This
        action does not in itself create an event in the execution history. However, if the task times out,
        the execution history contains an ``ActivityTimedOut`` entry for activities, or a ``TaskTimedOut``
        entry for for tasks using the `job run
        <https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-sync>`__ or
        `callback
        <https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-wait-token>`__
        pattern.
        ...
        .. note::
          ...
          its maximum allowed duration, regardless of the number of  SendTaskHeartbeat requests received.
          Use ``HeartbeatSeconds`` to configure the timeout interval for heartbeats.
    ...
    def start_execution(
        ...
        .. note::
           ...
           ``StartExecution`` is idempotent. If ``StartExecution`` is called with the same name and input as
           a running execution, the call will succeed and return the same response as the original request.
           If the execution is closed or if the input is different, it will return a 400
        ...
        **Request Syntax**
        ...
        :param name:
          ...
          machine for 90 days. For more information, see `Limits Related to State Machine Executions
          <https://docs.aws.amazon.com/step-functions/latest/dg/limits.html#service-limits-state-machine-executions>`__
          in the *AWS Step Functions Developer Guide* .
    ...
    def update_state_machine(
        ...
        executions will continue to use the previous ``definition`` and ``roleArn`` . You must include at
        least one of ``definition`` or ``roleArn`` or you will receive a ``MissingRequiredParameter`` error.
