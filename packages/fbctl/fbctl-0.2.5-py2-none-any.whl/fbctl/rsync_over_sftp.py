...
class RsyncOverSftp(object):
    def __init__(self, host, port=22, timeout=10, max_attempts=3, **kwargs):
        self.host = host
        self.port = port
        self.username = ''  # remove me
        self.client = paramiko.SSHClient()
        self.client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        self.sftp = None
        for i in range(max_attempts):
            self.client = get_ssh(host, port)
    ...
    def _validate_dst(self, file, src_stat, remote=True):
        ...
        if abs(dst_stat.st_mtime - src_stat.st_mtime) > MTIME_TOLERANCE:
            logger.debug(
                '%s modified time mismatch (source: %s, destination: %s)',
    ...
    def _save(self, src, dst, src_stat, remote=True):
        if remote:
            logger.debug('copying %s to %s@%s:%s', src, self.username, self.host,
                        dst)
            self.sftp.put(src, dst)
            self.sftp.utime(dst,
                            (int(src_stat.st_atime), int(src_stat.st_mtime)))
        else:
            logger.debug('copying %s@%s:%s to %s', self.username, self.host, src,
                        dst)
