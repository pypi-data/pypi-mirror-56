...
from DKCloudCommand.modules.DKCLIRecipeChecker import DKCLIRecipeChecker  # noqa: E402
from DKCloudCommand.modules.DKCloudAPI import DKCloudAPI  # noqa: E402
from DKCloudCommand.modules.DKCloudCommandConfig import DKCloudCommandConfig  # noqa: E402
from DKCloudCommand.modules.DKCloudCommandRunner import DKCloudCommandRunner  # noqa: E402
from DKCloudCommand.modules.DKFileHelper import DKFileHelper  # noqa: E402
from DKCloudCommand.modules.DKKitchenDisk import DKKitchenDisk  # noqa: E402
from DKCloudCommand.modules.DKRecipeDisk import DKRecipeDisk  # noqa: E402

DEFAULT_IP = 'https://cloud.datakitchen.io'
DEFAULT_PORT = '443'
DEFAULT_CONTEXT = 'default'

DK_VERSION = '1.0.145rc3'
...
class Backend(object):
    ...
    def __init__(self, only_check_version=False, current_command=None):
        self.cfg = None
        self.dki = None
    ...
    def init_context(self):
        ...
        if token is None:
            message = '\nLogin failed. You are in context %s, do you want to reconfigure your context? [yes/No]' % dk_context  # noqa: E501
    ...
    def check_version(self, cfg):
        ...
        if latest_version and self.version_to_int(latest_version) > current_version:
            ...
            print(
                '\033[31m Your command line is out of date, new version %s is available. Please update.\033[39m'
                % latest_version
            )
            print('')
    ...
    def setup_cli(self, file_path, general=False, context=False):
        ...
        if general:
            ...
            check_working_path = raw_input(
                'Check current working path against existing contexts?[yes/No] (default No):'
            )
            ...
            hide_context_legend = raw_input(
                'Hide current context legend on each command response?[yes/No] (default No):'
            )
            ...
            skip_recipe_checker = raw_input(
                'Skip checking the recipe locally, before recipe-update like commands?[yes/No] (default No):'  # noqa: E501
            )
...
class AliasedGroup(click.Group):
    ...
    def get_command(self, ctx, cmd_name):
        ...
        found_command = next((
            long_command for long_command, short_command in alias_exceptions.items()
...
class DKClickCommand(click.Command):
    ...
    def __init__(
        ...
        super(DKClickCommand, self).__init__(
            name, context_settings, callback, params, help, epilog, short_help, options_metavar,
            add_help_option
        )
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help='General and context configuration'
)
@click.option(
    '--context', '-c', default=False, is_flag=True, required=False, help='Context configuration'
)
@click.option(
    '--general', '-g', default=False, is_flag=True, required=False, help='General configuration'
)
@click.pass_obj
...
def context_switch(backend, context_name, yes):
    ...
    try:
        ...
        if not backend.cfg.context_exists(context_name):
            if not yes:
                message = '\nContext does not exist. Are you sure you want to create context %s? [yes/No]' % context_name  # noqa: E501
                confirm = raw_input(message)
                if confirm.lower() != 'yes':
                    return
...
def recipe_status(backend):
    ...
    try:
        ...
        check_and_print(
            DKCloudCommandRunner.recipe_status(backend.dki, kitchen, recipe_name, recipe_dir)
        )
    except Exception as e:
...
@click.option(
    ...
    type=str,
    multiple=True,
    help='Get the recipe along with the kitchen. Multiple allowed'
)
...
def kitchen_get(backend, kitchen_name, recipe, all):
    ...
    try:
        ...
        if len(recipe) > 0:
            click.secho(
                "%s - Getting kitchen '%s' and the recipes %s" %
        ...
        check_and_print(
            DKCloudCommandRunner.get_kitchen(backend.dki, kitchen_name, os.getcwd(), recipe, all)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def kitchen_create(backend, parent, description, kitchen):
    ...
    try:
        if not DKCloudCommandRunner.kitchen_exists(backend.dki, parent):
            raise click.ClickException('Parent kitchen %s does not exist. Check spelling.' % parent)

        click.secho(
            '%s - Creating kitchen %s from parent kitchen %s' % (get_datetime(), kitchen, parent),
            fg='green'
        )
        master = 'master'
        if kitchen.lower() != master.lower():
            check_and_print(
                DKCloudCommandRunner.create_kitchen(backend.dki, parent, kitchen, description)
            )
        else:
...
def kitchen_delete(backend, kitchen, yes):
    ...
    try:
        ...
        if not yes:
            confirm = raw_input(
                '\nAre you sure you want to delete the remote copy of the Kitchen %s ? [yes/No]' %
                kitchen
            )
            if confirm.lower() != 'yes':
                return
        ...
        click.secho(
            '%s - Deleting remote copy of kitchen %s. Local files will not change.' %
            (get_datetime(), kitchen),
            fg='green'
        )
        master = 'master'
        if kitchen.lower() != master.lower():
...
@click.option(
    ...
    type=str,
    required=False,
    help='Get the value for an override variable.',
    multiple=True
)
@click.option(
    '--unset', '-u', type=str, required=False, help='Delete an override variable.', multiple=True
)
@click.option(
    '--listall',
    '-la',
...
def kitchen_config(backend, kitchen, add, get, unset, listall):
    ...
    try:
        ...
        check_and_print(
            DKCloudCommandRunner.config_kitchen(backend.dki, use_kitchen, add, get, unset, listall)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option(
    '--source_kitchen', '-sk', type=str, required=False, help='source (from) kitchen name'
)
...
def kitchen_merge_preview(backend, source_kitchen, target_kitchen, clean_previous_run):
    ...
    try:
        ...
        if recipe_dir is not None:
            message = 'Warning: You are inside a recipe. Please go at least one level above. This command is typically used from the level above kitchen level.'  # noqa: E501
        ...
        if kitchen is None and source_kitchen is None:
            raise click.ClickException(
                'You are not in a Kitchen and did not specify a source_kitchen'
            )

        if kitchen is not None and source_kitchen is not None and kitchen != source_kitchen:
            raise click.ClickException(
                'There is a conflict between the kitchen in which you are, and the source_kitchen you have specified'  # noqa: E501
            )
        ...
        kitchens_root = DKKitchenDisk.find_kitchens_root(
            reference_kitchen_names=[use_source_kitchen, target_kitchen]
        )
        if kitchens_root:
        ...
        click.secho(
            '%s - Previewing merge Kitchen %s into Kitchen %s' %
            (get_datetime(), use_source_kitchen, target_kitchen),
            fg='green'
        )
        check_and_print(
...
def kitchen_merge(backend, source_kitchen, target_kitchen, yes):
    ...
    try:
        ...
        if not yes:
            confirm = raw_input(
                'Are you sure you want to merge the \033[1mremote copy of Source Kitchen %s\033[0m into the \033[1mremote copy of Target Kitchen %s\033[0m? [yes/No]'  # noqa: E501
        ...
        click.secho(
            '%s - Merging Kitchen %s into Kitchen %s' %
            (get_datetime(), use_source_kitchen, target_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.kitchen_merge(backend.dki, use_source_kitchen, target_kitchen)
        )

        retry = True
        while retry:
            try:
                DKCloudCommandRunner.update_local_recipes_with_remote(
                    backend.dki, kitchens_root, target_kitchen
                )
                retry = False
            except Exception as e:
...
def recipe_list(backend, kitchen):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            "%s - Getting the list of Recipes for Kitchen '%s'" % (get_datetime(), use_kitchen),
            fg='green'
        )
...
def recipe_create(backend, kitchen, name, template):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            "%s - Creating Recipe %s for Kitchen '%s'" % (get_datetime(), name, use_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.recipe_create(backend.dki, use_kitchen, name, template=template)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.argument('name', required=True)
@click.pass_obj
def recipe_copy(backend, kitchen, source, name):
    """
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            "%s - Creating Recipe %s from recipe %s for Kitchen '%s'" %
            (get_datetime(), name, source, use_kitchen),
            fg='green'
        )
...
def recipe_delete(backend, kitchen, name, yes):
    ...
    try:
        ...
        click.secho(
            "This command will delete the local and remote copy of recipe '%s' for kitchen '%s'. " %
            (name, use_kitchen)
        )
        if not yes:
            confirm = raw_input(
                'Are you sure you want to delete the local and remote copy of recipe %s? [yes/No]' %
                name
            )
            if confirm.lower() != 'yes':
                return
        ...
        click.secho(
            "%s - Deleting Recipe %s for Kitchen '%s'" % (get_datetime(), name, use_kitchen),
            fg='green'
        )
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help="Force through the command's subprompt."
)
        ...
        if recipe_root_dir is None:
            ...
            if recipe is not None:
                if recipe_name != recipe:
                    raise click.ClickException(
                        "\nThe recipe name argument '%s' is inconsistent with the current directory '%s'"  # noqa: E501
...
def recipe_compile(backend, kitchen, recipe, variation):
    ...
    try:
        ...
        click.secho(
            '%s - Get the Compiled Recipe %s.%s in Kitchen %s' %
            (get_datetime(), recipe, variation, use_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.get_compiled_order_run(
                backend.dki, use_kitchen, recipe, variation
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def file_compile(backend, variation, file):
    ...
    try:
        ...
        click.secho(
            '%s - Get the Compiled File of Recipe %s.%s in Kitchen %s' %
            (get_datetime(), recipe_name, variation, kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.get_compiled_file(
                backend.dki, kitchen, recipe_name, variation, file
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option(
    ...
    required=False,
    default=0,
    help='Number of last changes to display'
)
...
def file_history(backend, change_count, filepath):
    ...
    try:
        ...
        check_and_print(
            DKCloudCommandRunner.file_history(backend.dki, kitchen, recipe, filepath, change_count)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def recipe_validate(backend, variation):
    ...
    try:
        ...
        click.secho(
            '%s - Validating recipe/variation %s.%s in Kitchen %s' %
            (get_datetime(), recipe_name, variation, kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.recipe_validate(backend.dki, kitchen, recipe_name, variation)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def recipe_variation_list(backend, kitchen, recipe):
    ...
    try:
        ...
        if not DKCloudCommandRunner.kitchen_exists(backend.dki, use_kitchen):
            raise click.ClickException('Kitchen %s does not exist. Check spelling.' % use_kitchen)

        click.secho(
            '%s - Listing variations for recipe %s in Kitchen %s' %
            (get_datetime(), use_recipe, use_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.recipe_variation_list(
                backend.dki, use_kitchen, use_recipe, get_remote
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def recipe_ingredient_list(backend):
    ...
    try:
        ...
        click.secho(
            '%s - Listing ingredients for recipe %s in Kitchen %s' %
            (get_datetime(), recipe_name, kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.recipe_ingredient_list(backend.dki, kitchen, recipe_name)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def file_diff(backend, kitchen, recipe, filepath):
    ...
    try:
        ...
        click.secho(
            '%s - File Diff for file %s, in Recipe (%s) in Kitchen (%s)' %
            (get_datetime(), filepath, recipe, use_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.file_diff(backend.dki, use_kitchen, recipe, recipe_dir, filepath)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def file_merge(backend, source_kitchen, target_kitchen, filepath):
    ...
    try:
        recipe_dir = DKRecipeDisk.find_recipe_root_dir()
        if recipe_dir is not None:
            message_1 = 'Warning: You are inside a recipe. Please go at least one level above. This command is typically used from the level above kitchen level.'  # noqa: E501
            message_2 = 'Keep in mind that the file path needs to be stated as it is coming from kitchen-merge-preview output.'  # noqa: E501
            message_3 = 'i.e. dk file-merge -sk source_kitchen_name -tk target_kitchen_name %s' % \
                        os.path.normpath('recipe_name/graph.json')
        ...
        click.secho(
            '%s - File Merge for file %s, source kitchen (%s), target kitchen(%s)' %
            (get_datetime(), filepath, use_source_kitchen, target_kitchen),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.file_merge(
                backend.dki, filepath, use_source_kitchen, target_kitchen
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def file_resolve(backend, source_kitchen, target_kitchen, filepath):
    ...
    try:
        ...
        if recipe_dir is not None:
            message_1 = 'Warning: You are inside a recipe. Please go at least one level above. This command is typically used from the level above kitchen level.'  # noqa: E501
            message_2 = 'Keep in mind that the file path needs to be stated as it is coming from kitchen-merge-preview output.'  # noqa: E501
            message_3 = 'i.e. dk file-resolve -sk source_kitchen_name -tk target_kitchen_name %s' % os.path.normpath(  # noqa: E501
                'recipe_name/graph.json'
            )
            click.secho(message_1, fg='yellow')
            click.secho(message_2, fg='yellow')
            click.secho(message_3, fg='yellow')
        ...
        click.secho(
            "%s - File resolve for file %s, source kitchen (%s), target kitchen(%s)" %
            (get_datetime(), filepath, use_source_kitchen, target_kitchen)
        )
        check_and_print(
            DKCloudCommandRunner.file_resolve(
                backend.dki, use_source_kitchen, target_kitchen, filepath
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def file_get(backend, filepath):
    ...
    try:
        ...
        click.secho(
            '%s - Getting File (%s) to Recipe (%s) in kitchen(%s)' %
            (get_datetime(), filepath, recipe, kitchen),
            fg='green'
        )
...
@click.pass_obj
def file_update(backend, kitchen, recipe, message, filepaths):
    """
    ...
    try:
        ...
        for item in filepaths:
            if any(invalid_separator in item for invalid_separator in invalid_separators):
                error_mesage = 'Files to be updated must be delimited by whitespace (i.e. \'file1.txt file2.txt\' as opposed to \'file1.txt,file2.txt\')'  # noqa: E501
                raise click.ClickException(error_mesage)
        ...
        click.secho(
            '%s - Updating File(s) (%s) in Recipe (%s) in Kitchen(%s) with message (%s)' %
            (get_datetime(), filepaths, recipe, use_kitchen, message),
            fg='green'
        )
        check_and_print(
            DKCloudCommandRunner.update_file(
                backend.dki, use_kitchen, recipe, recipe_dir, message, filepaths
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help='Delete remote files to match local'
)
@click.option('--message', '-m', type=str, required=True, help='change message')
@click.pass_obj
...
def file_delete(backend, kitchen, recipe, message, filepath):
    ...
    try:
        ...
        check_and_print(
            DKCloudCommandRunner.delete_file(backend.dki, use_kitchen, recipe, message, filepath)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option('--kitchen', '-k', type=str, required=False, help='Kitchen name')
@click.option(
    '--interval', '-i', type=int, required=False, default=5, help='watching interval, in seconds'
)
@click.pass_obj
def active_serving_watcher(backend, kitchen, interval):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            '%s - Watching Active OrderRun Changes in Kitchen %s' % (get_datetime(), use_kitchen),
            fg='green'
        )
...
@click.pass_obj
def order_run(backend, kitchen, recipe, variation, node, yes, params):
    """
    ...
    Examples:
        ...
        \b
        3) run an order with parameters (Linux version)
        dk or -y -k myKitchen -p '{"my-key":"my-value"}' myVariation

        \b
        4) run an order with parameters (Windows version)
        dk or -y -k myKitchen -p "{\\"my-key\\":\\"my-value\\"}" myVariation
    ...
    try:
        ...
        if not yes:
            confirm = raw_input(
                'Kitchen %s, Recipe %s, Variation %s.\n'
        ...
        check_and_print(
            DKCloudCommandRunner.create_order(
                backend.dki, use_kitchen, recipe, variation, node, params_json
            )
        )
    except Exception as e:
        raise click.ClickException(str(e))
    ...
    if params:
        if params[0] == '@':
            ...
            try:
                ...
                raise click.ClickException(
                    'Invalid parameters format, they must be a file reference or a well formatted JSON string'  # noqa: E501
                )
                pass
    return None
...
def order_delete(backend, kitchen, order_id, yes):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(
                'You must specify either a kitchen or be in a kitchen directory'
            )

        if not yes:
            if order_id is None:
                confirm = raw_input(
                    'Are you sure you want to delete all Orders in kitchen %s ? [yes/No]' %
                    use_kitchen
                )
            else:
                confirm = raw_input(
                    'Are you sure you want to delete Order %s ? [yes/No]' % order_id
                )
            if confirm.lower() != 'yes':
                return
        ...
        if order_id is not None:
            ...
            check_and_print(
                DKCloudCommandRunner.delete_one_order(backend.dki, use_kitchen, order_id)
            )
        else:
            click.secho(
                '%s - Delete all orders in Kitchen %s' % (get_datetime(), use_kitchen), fg='green'
            )
...
def orderrun_stop(backend, kitchen, order_run_id, yes):
    ...
    try:
        ...
        if order_run_id is None:
            raise click.ClickException('invalid order run id %s' % order_run_id)

        if not yes:
            confirm = raw_input(
                'Are you sure you want to stop Order-Run %s ? [yes/No]' % order_run_id
            )
            if confirm.lower() != 'yes':
                return

        click.secho('%s - Stop order run id %s' % (get_datetime(), order_run_id), fg='green')
        check_and_print(
            DKCloudCommandRunner.stop_orderrun(backend.dki, use_kitchen, order_run_id.strip())
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help='display run summary information'
)
@click.option(
    '--nodestatus',
    '-ns',
    ...
    is_flag=True,
    required=False,
    help=' display node status info'
)
@click.option(
    '--full_log', '-fl', default=False, is_flag=True, required=False, help='display full logs'
)
...
@click.option(
    ...
    required=False,
    help=  # noqa: E251
    'Log display threshold. Can be ERROR, INFO, DEBUG, TRACE. Default value is INFO. This does not affect full_log'  # noqa: E501
)
@click.option(
    '--timing', '-t', default=False, is_flag=True, required=False, help='display timing results'
)
@click.option(
    '--test', '-q', default=False, is_flag=True, required=False, help='display test results'
)
@click.option(
    '--runstatus',
    default=False,
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help=' display order run id (single line)'
)
@click.option(
    '--all_things',
    '-at',
...
@click.pass_obj
def orderrun_detail(
        backend, kitchen, summary, nodestatus, runstatus, full_log, log, log_threshold, timing,
        test, all_things, order_id, order_run_id, disp_order_id, disp_orderrun_id
):
    """
    ...
    try:
        ...
        if log_threshold not in log_threshold_options:
            ...
            raise click.ClickException(
                "log_threshold needs to be one of this options: %s" % log_threshold_options_str
            )
        ...
        if not runstatus and not disp_order_id and not disp_orderrun_id:
            ...
            click.secho(
                '%s - Display Order-Run details from kitchen %s' % (get_datetime(), use_kitchen),
                fg='green'
            )
        check_and_print(
...
def delete_orderrun(backend, orderrun_id, kitchen, yes):
    ...
    try:
        ...
        if not yes:
            confirm = raw_input(
                'Are you sure you want to delete Order-Run  %s ? [yes/No]' % orderrun_id
            )
            if confirm.lower() != 'yes':
                return
        ...
        check_and_print(
            DKCloudCommandRunner.delete_orderrun(backend.dki, use_kitchen, orderrun_id.strip())
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def order_resume(backend, orderrun_id, kitchen, yes):
    ...
    try:
        ...
        if not yes:
            confirm = raw_input(
                'Are you sure you want to resume Order-Run %s ? [yes/No]' % orderrun_id
            )
            if confirm.lower() != 'yes':
                return
        ...
        check_and_print(
            DKCloudCommandRunner.order_resume(backend.dki, use_kitchen, orderrun_id.strip())
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
@click.option(
    '--start', '-s', type=int, required=False, default=0, help='Start offset for displaying orders'
)
@click.option(
    '--order_count', '-oc', type=int, required=False, default=5, help='Number of orders to display'
)
@click.option(
    '--order_run_count',
    '-orc',
...
@click.option(
    ...
    required=False,
    default=None,
    help='Filter results for this recipe only'
)
@click.pass_obj
def order_list(backend, kitchen, order_count, order_run_count, start, recipe):
    ...
    try:
        ...
        click.secho(
            '%s - Get Order information for Kitchen %s' % (get_datetime(), use_kitchen), fg='green'
        )
...
@click.pass_obj
def vault_info(backend, global_secrets, kitchen):
    """
    List vault info
    ...
    try:
        if not kitchen:
            global_secrets = True
        click.echo(click.style('%s - Getting the vault info' % get_datetime(), fg='green'))
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help='Configuration inherited from parent kitchen'
)
...
@click.option(
    '--private', '-p', default='False', required=False, help='Whether vault is private or not'
)
@click.option(
    '--inheritable',
    '-i',
    default='False',
    required=False,
    help='Whether vault Values are Inheritable or not'
)
@click.option(
    '--disable', '-d', default=False, is_flag=True, required=False, help='Disable global vault'
)
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def vault_config(
        backend, kitchen, inherit, service, url, token, prefix, private, inheritable, disable, yes
):
    """
    ...
    try:
        ...
        if not kitchen and not (backend.dki.is_user_role('IT')
                                or backend.dki.is_user_role('ADMIN')):
            raise click.ClickException(
                'You do not have IT or ADMIN privileges to run this command for global vault.'
            )
        ...
        if not kitchen:
            if service == 'custom':
                raise click.ClickException(
                    '--service "custom" does not apply for global vault config'
                )
        ...
        if kitchen:
            if disable:
                raise click.ClickException(
                    'Kitchen vault cannot be disabled. Check vault-delete command instead.'
                )
            if inherit:
                if any([service, url, token, prefix, private.lower() == 'true',
                        inheritable.lower() == 'true']):
                    raise click.ClickException(
            ...
            if service == 'default':
                if any([url, token, prefix, private.lower() == 'true',
                        inheritable.lower() == 'true']):
                    raise click.ClickException(
                        'When service is default: url, token, prefix, private and inheritable cannot be set.'
                    )

        click.echo(click.style('%s - Setting the vault info' % get_datetime(), fg='green'))
...
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def vault_delete(backend, kitchen, yes):
    """
    ...
    try:
        if not yes:
            message = 'You are deleting kitchen vault config for kitchen %s!. Are you sure you want to proceed? [yes/No]' % kitchen  # noqa E501
            confirm = raw_input(message)
            if confirm.lower() != 'yes':
                return
        ...
        click.echo(
            click.style(
                '%s - Deleting the vault configuration for kitchen %s' % (get_datetime(), kitchen),
                fg='green'
            )
        )
...
@click.pass_obj
def secret_list(backend, path, global_secrets, kitchen):
    """
    List Secrets
...
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def secret_write(backend, entry, yes, kitchen):
    """
    ...
    try:
        ...
        if kitchen and not DKCloudCommandRunner.is_vault_config_writable(backend.dki, kitchen):
            raise click.ClickException(
                'No vault service configured for kitchen %s, or vault is read only.' % kitchen
            )
        ...
        if secret_exists:
            if not yes:
                confirm = raw_input(
                    'Are you sure you want to overwrite the existing Vault Secret %s ? [yes/No]' %
                    path
                )
                if confirm.lower() != 'yes':
                    return
...
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def secret_delete(backend, path, kitchen, yes):
    """
    Delete a secret
...
@click.argument('path', required=True)
@click.pass_obj
def secret_exists(backend, path, kitchen):
    """
    ...
    try:
        DKCloudCommandRunner.check_secret_path(path)
        if kitchen:
            message = '%s Checking secret in kitchen vault %s' % (get_datetime(), kitchen)
        else:
            message = '%s Checking secret in global vault' % get_datetime()
...
def kitchen_settings_get(backend):
    ...
    try:
        ...
        click.secho(
            "%s - Getting a local copy of kitchen-settings.json" % get_datetime(), fg='green'
        )
...
def kitchen_settings_update(backend, filepath):
    ...
    try:
        ...
        check_and_print(
            DKCloudCommandRunner.kitchen_settings_update(backend.dki, kitchen, filepath)
        )
    except Exception as e:
        raise click.ClickException(str(e))
...
def agent_status(backend):
    ...
    try:
        ...
        click.secho("%s - Getting Agent Status Information" % get_datetime(), fg='green')
        data = DKCloudCommandRunner.agent_status(backend.dki)
        click.secho('-----------')
        click.secho(
            'Agent is Online' if data['agent_status']['available'] else
            'Agent is Offline - Unable to create new orders'
        )
        click.secho('Total available memory: %d MB' % data['agent_status']['mem'])
        click.secho('Total available disk space: %d MB' % data['agent_status']['disk'])
    except Exception as e:
        raise click.ClickException(str(e))
