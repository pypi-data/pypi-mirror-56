...
from DKCloudCommand.tests.integration_tests.BaseTestCloud import BaseTestCloud, WaitLoop
...
class TestCloudAPI(BaseTestCloud):
    ...
    def test_merge_kitchens_improved(self):
        ...
        self.assertTrue(
            merge_info['recipes']['simple']['simple'][0]['filename'] == 'conflicted-file.txt'
        )
    ...
    def test_merge_kitchens_improved_conflicts(self):
        ...
        if setup:
            ...
            sleep(10)
            rv = self._api.update_file(
                parent_kitchen, recipe, 'Changes on parent to cause conflict', conflict_file,
                'top\nparent\nbottom\n'
            )
            self.assertTrue(rv.ok())
        ...
        self.assertTrue(
            '>>>>>>>' in base64.b64decode(conflicts['simple']['simple'][0]['conflict_tags'])
        )
        if TestCloudAPI.pickle_dump:
            pickle.dump(
                rd, open(os.path.normpath("files/merge_kitchens_improved_conflicts.p"), "wb")
            )
    ...
    def test_merge_kitchens_1_fail(self):
        ...
        base_test_kitchen_name = self._add_my_guid(
            base_test_kitchen_name, delete_branch_on_teardown=cleanup
        )
        branched_test_kitchen_name = 'branched-from-base-test-kitchen-API'
        branched_test_kitchen_name = self._add_my_guid(
            branched_test_kitchen_name, delete_branch_on_teardown=cleanup
        )
        recipe_name = 'parallel-recipe-test'
        rpath = recipe_name
        file_name = 'description.json'
        ...
        if setup:
            ...
            self.assertTrue(self._create_kitchen(existing_kitchen_name, base_test_kitchen_name))
            self.assertTrue(
                self._create_kitchen(base_test_kitchen_name, branched_test_kitchen_name)
            )
            base_test_kitchen_dict = OrderedDict()
            base_test_kitchen_dict['test_data'] = 'TIS SHOLD CONFICT'

            sleep(10)
            rc = self._api.update_file(
                base_test_kitchen_name, recipe_name, message, recipe_file_key,
                self._get_the_json_str(base_test_kitchen_dict)
            )
            self.assertTrue(rc.ok())
            ...
            sleep(10)
            rc = self._api.update_file(
                branched_test_kitchen_name, recipe_name, message + ' version2', recipe_file_key,
            ...
            self._check_merge_conflicts(
                rd, recipe_name, rpath, file_name, branched_test_kitchen_name,
        ...
        conflict_key = '%s|%s|%s|%s|%s' % (
            branched_test_kitchen_name, base_test_kitchen_name, recipe_name, recipe_name, file_name
        )
        d2[recipe_name][recipe_name][conflict_key] = {
            'from_kitchen': branched_test_kitchen_name,
            'to_kitchen': base_test_kitchen_name,
            'recipe': recipe_name,
            'filename': file_name,
            'contents': 'THIS SHOULD CONFLICT'
        }
        rc = self._api.merge_kitchens_improved(
            branched_test_kitchen_name, base_test_kitchen_name, d2
        )
        self.assertTrue(rc.ok())
    ...
    def test_merge_kitchens_multi_fail(self):
        ...
        child_test_kitchen_name = self._add_my_guid(child_test_kitchen_name)
        grandchild_test_kitchen_name = 'grandchild-test-kitchen-API'
        grandchild_test_kitchen_name = self._add_my_guid(grandchild_test_kitchen_name)
        ...
        # setup
        self._delete_and_clean_kitchen(grandchild_test_kitchen_name)
        self._delete_and_clean_kitchen(child_test_kitchen_name)
        # test
        # create kitchens
        ...
        sleep(10)
        self.assertTrue(
            self._api.update_file(
                child_test_kitchen_name, recipe_a_name, message % 1, recipe_a_api_file1_key,
                '{"my_attribute": "Kitchen Child Recipe A File 1"}'
            ).ok()
        )
        self.assertTrue(
            self._api.update_file(
                grandchild_test_kitchen_name, recipe_a_name, message % 1, recipe_a_api_file1_key,
                '{"my_attribute": "Kitchen Grandchild Recipe A File 1"}'
            ).ok()
        )
        ...
        self.assertTrue(
            self._api.update_file(
                child_test_kitchen_name, recipe_a_name, message % 2, recipe_a_api_file2_key,
                '{"my_attribute": "Kitchen Child Recipe A File 2", "type":"DKDataSource_NoOp","name":"%s"}'
                % file_name2.replace('.json', '')
            ).ok()
        )
        self.assertTrue(
            self._api.update_file(
                grandchild_test_kitchen_name, recipe_a_name, message % 2, recipe_a_api_file2_key,
                '{"my_attribute": "Kitchen Grandchild Recipe A File 2","type":"DKDataSource_NoOp","name":"%s"}'
                % file_name2.replace('.json', '')
            ).ok()
        )
        # update file 3
        self.assertTrue(
            self._api.update_file(
                child_test_kitchen_name, recipe_b_name, message % 3, recipe_b_api_file3_key,
                'Kitchen Child Recipe B File 3\n'
            ).ok()
        )
        self.assertTrue(
            self._api.update_file(
                grandchild_test_kitchen_name, recipe_b_name, message % 3, recipe_b_api_file3_key,
                'Kitchen Grandchild Recipe B File 3\n'
            ).ok()
        )
        ...
        self._check_merge_conflicts(
            rd, recipe_a_name, recipe_file_key1, file_name1, grandchild_test_kitchen_name,
            child_test_kitchen_name
        )
        self._check_merge_conflicts(
            rd, recipe_a_name, recipe_file_key2, file_name2, grandchild_test_kitchen_name,
            child_test_kitchen_name
        )
        self._check_merge_conflicts(
            rd, recipe_b_name, recipe_file_key3, file_name3, grandchild_test_kitchen_name,
            child_test_kitchen_name
        )
