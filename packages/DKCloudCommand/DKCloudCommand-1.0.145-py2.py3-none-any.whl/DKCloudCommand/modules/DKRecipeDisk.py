...
class DKRecipeDisk:
    ...
    def __init__(self, ignore, recipe_sha=None, recipe=None, path=None):
        self.ignore = ignore
        self.recipe = recipe
        self._recipe_sha = recipe_sha
        self._recipe_path = path
        if recipe:
            self._recipe_name = min(recipe.keys()) if len(recipe.keys()) else ''
    ...
    def write_recipe_state(self, recipe_dir, remote_shas=None):
    ...
    def do_fetch_shas(self, base_dir):
        result = {}
            ...
            for conflict_key, conflict_info in six.iteritems(folder_conflicts):
                if conflict_info['status'] == 'unresolved':
                    ...
                    if from_kitchen is not None and to_kitchen is not None:
                        if from_kitchen != conflict_info['from_kitchen'] or conflict_info[
            ...
            for conflict_key, conflict_info in six.iteritems(folder_conflicts):
                if conflict_info['status'] == 'resolved':
                    ...
                    if add_it:
                        ...
                        print("Found a resolved conflict for from '%s' to '%s', but we are looking for from '%s' to '%s'" % (  # noqa: E501
                            conflict_info['from_kitchen'], conflict_info['to_kitchen'],
                            from_kitchen, to_kitchen
                        ))
    ...
    def save_conflicts_meta(recipe_meta_dir, conflicts_meta):
        ...
        with open(conflicts_file_path, 'w') as conflicts_file:
            conflicts_file.write(
                json.dumps(conflicts_meta, sort_keys=True, indent=2, separators=(',', ': '))
            )
        return True
            ...
            with open(abspath, 'wb') as the_file:
                ...
                if 'json' in file_dict:
                    if isinstance(file_dict['json'], dict) is True:
                        json.dump(file_dict['json'], the_file)
                    else:
...
def compare_sha(ignore, remote_sha, local_sha, start_dir=None, recipe=None):
    same = dict()
    different = dict()
    local_modified = dict()
    ...
    for remote_path in remote_sha:
        for remote_file in remote_sha[remote_path]:
            if remote_path in local_sha:
                local_files_found = [
                    local_file for local_file in local_sha[remote_path]
        ...
        if ignore.ignore(local_path):
            ...
            for local_file in local_files:
                if ignore.ignore(local_file['filename']):
                    ...
                    continue
                remote_files_found = [
                    remote_file for remote_file in remote_sha[local_path]
...
def get_directory_sha(ignore, walk_dir):
