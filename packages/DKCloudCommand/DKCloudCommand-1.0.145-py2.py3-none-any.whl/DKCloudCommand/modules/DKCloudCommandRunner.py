...
class DKCloudCommandRunner(object):
        ...
        if childs is not None:
            for child in childs:
                ...
                DKCloudCommandRunner.tree_view(
                    user, kitchen_list, child, tree_kitchen_dict, depth + 1, output
                )
        return ''.join(output)
            ...
            if item in payload:
                ...
                if 'list' in payload[item]:
                    ...
                    for list_item in sorted_list:
                        override_message = ''
                        if opposite_item and opposite_item in payload and 'list' in payload[
                                opposite_item] and list_item in payload[opposite_item]['list']:
    ...
    def is_vault_config_writable(dk_api, kitchen_name):
        ...
        if backend_response and 'config' in backend_response and kitchen_name in backend_response[
                'config']:
        ...
        try:
            ...
            if not DKKitchenDisk.is_kitchen_root_dir(rp):
                ...
                if recipe_name_found != recipe_name_param:
                    s = "ERROR: DKCloudCommandRunner.get_recipe: You are asking to get recipe '%s', but you are in folder '%s'" % (  # noqa: E501
            ...
            if os.path.exists(os.path.join(rp, recipe_name_param)):
                ...
                if remote_only_recipe_tree is not None:
                    ...
                    for recipe_folder_name, recipe_folder_contents in remote_only_recipe_tree[
                            'recipes'][recipe_name_param].iteritems():
    ...
    def _merge_file(dk_api, kitchen_name, recipe_name, recipe_path, folder_name, file_info):
        ...
        rc = dk_api.merge_file(
            kitchen_name, recipe_name, file_path_without_recipe, base64.b64encode(local_contents),
    ...
    def get_compiled_file(dk_api, kitchen, recipe_name, variation_name, file_path):
        ...
        try:
            ...
            file_data = {'path': recipe_file_path, 'contents': base64.b64encode(contents)}
    ...
    def kitchen_merge(dk_api, from_kitchen, to_kitchen):
        ...
        try:
            ...
            if len(manual_merge_dict) == 0:
                print('Calling Merge ...')
                return DKCloudCommandRunner.merge_kitchens_improved(
                    dk_api, from_kitchen, to_kitchen
                )
            else:
    ...
    def merge_kitchens_improved(dk_api, from_kitchen, to_kitchen):
        ...
        if not md.ok():
            md.set_message(
                'merge_kitchens_improved error from %s to Kitchen %s\nmessage: %s' %
    ...
    def write_recipe_merge_conflicts(merge_info, recipe_name_param, kitchen_dir):
        ...
        if recipe_name_param not in merge_info['conflicts']:
            rc.set(
                rc.DK_FAIL,
                "DKCloudCommandRunner.write_recipe_merge_conflicts: Can't find conflicts for recipe %s."  # noqa: E501
                % recipe_name_param
            )
            return rc
        ...
        for folder_name, folder_contents in recipe_conflicts.iteritems():
            ...
            for conflict in folder_contents:
                ...
                if 'conflict_tags' in conflict:
                    ...
                    if not DKRecipeDisk.add_conflict_to_conflicts_meta(
                            conflict, folder_name, recipe_name_param, kitchen_dir):
                        rc.set(
                            rc.DK_FAIL,
                            "DKCloudCommandRunner.write_recipe_merge_conflicts: Unable to write out conflict meta for %s"  # noqa: E501
                            % file_fullpath
                        )
                        return rc
                else:
                    rc.set(
                        rc.DK_FAIL,
                        "DKCloudCommandRunner.write_recipe_merge_conflicts: Can't find conflict tags for %s"  # noqa: E501
                        % file_fullpath
                    )
                    return rc

        rc.set(
            rc.DK_SUCCESS, "Conflicts for recipe %s written to %s\n" %
    ...
    def write_merge_conflicts(payload):
        ...
        for conflict_recipe_name in merge_info['conflicts']:
            found_recipe = next(
                (recipe for recipe in local_recipes if recipe == conflict_recipe_name), None
            )
            if not found_recipe:
        ...
        for recipe in local_recipes:
            recipe_rc = DKCloudCommandRunner.write_recipe_merge_conflicts(
                merge_info, recipe, kitchen_dir
            )
            if not recipe_rc.ok():
                return recipe_rc
        ...
        for recipe_key, recipe in merge_info['recipes'].iteritems():
            for recipe_name, recipe_folders in recipe.iteritems():
                for this_file in recipe_folders:
                    row = [
                        os.path.join(recipe_name, this_file['filename']), this_file['changes'],
                        '%s%s' %
                        ('+' * int(this_file['additions']), '-' * int(this_file['deletions']))
                    ]
                    x.add_row(row)
                    files_changed += 1
        msg += x.get_string() + '\n'
        ...
        if order_run and DKCloudCommandRunner.LOGS in order_run:
            ...
            for log in logs:
                if not DKCloudCommandRunner._show_log(log, showable_log_types):
                    continue
                line = map(lambda x: log.get(x, ''), items)
                log_line = ' | '.join(line)
    ...
    def _check_no_merge_conflicts(resp):
        if isinstance(resp, dict) and 'merge-kitchen-result' in resp \
                and 'status' in resp['merge-kitchen-result'] \
