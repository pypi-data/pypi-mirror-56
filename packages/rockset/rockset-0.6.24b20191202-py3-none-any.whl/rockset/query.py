...
class SQLSelect(object):
    ...
    def _enforce_build_order(self, build_order, coalesce=False):
        ...
        return (
            SQLSelect(params=self.P).add_from(sq)._set_build_order(build_order)
        )
...
class SubQuery(Query):
    ...
    def sqlexpression(self, **kwargs):
        ...
        sq = SQLSelectSubQuery(child=childsq, alias=self.alias)
...
class FieldDateTimeOpQuery(FieldOpQuery):
    def __init__(self, field, v, op, sqlop):
        ...
        super().__init__(field, checked_decode(v, dt_types), op, sqlop)
...
class FieldIsDefinedQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldIsDefinedQuery,
              self).__init__(field, 'is_defined', post_sqlop='IS NOT NULL')


class FieldIsNullQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldIsNullQuery,
              self).__init__(field, 'not-implemented', post_sqlop='IS NULL')


class FieldExistsQuery(UnaryOpQuery):
    def __init__(self, field):
        super(FieldExistsQuery,
              self).__init__(field, 'not-implemented', pre_sqlop='EXISTS')
        self.field = field
...
for op in (('lt', '<'), ('le', '<='), ('gt', '>'), ('ge', '>=')):
    ...
    def f_datetime(field, v, op=op[0], sqlop=op[1]):
        return FieldDateTimeOpQuery(field, v, op, sqlop)
