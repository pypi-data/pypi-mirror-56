...
try:
    ...
    exit("Samba requires Python >= 3.5")
...
SAMBA_SCRIPT = realpath(__file__)
SAMBA_ROOT = environ.get('SAMBA_ROOT', environ['HOME'])
SAMBA_BIN = join(environ['HOME'], 'bin')
APP_ROOT = abspath(join(SAMBA_ROOT, "apps"))
DOT_ROOT = abspath(join(SAMBA_ROOT, ".samba"))
...
if SAMBA_BIN not in environ['PATH']:
    environ['PATH'] = SAMBA_BIN + ":" + environ['PATH']
...
def install_acme_sh():
    ...
    call("curl https://raw.githubusercontent.com/Neilpang/acme.sh/master/acme.sh | INSTALLONLINE=1  sh", cwd=SAMBA_ROOT, shell=True)
...
def get_config(app):
    ...
    with open(config_file) as f:
        config = json.load(f)["samba"]

        # multiple site can be deployed by having app.json:samba as array
        ...
        if isinstance(config, list):
            ...
            echo("ERROR: '%s' not found in app.json. 'app.json:samba' is an array but app name didn't match domain_name" % app, fg="red")
            exit(1)
...
def deploy_app(app, deltas={}, newrev=None, release=False):
    ...
    if exists(app_path):
        ...
        if not config:
            ...
            echo("ERROR: app.json missing the 'samba.run' processes or 'samba.run' it is empty", fg="red")
            exit(1)
        else:
            # ensure path exist
    ...
    if 'web' in workers:
        ...
        if 'NGINX_SERVER_NAME' in env:
            ...
            if not exists(key) or stat(crt).st_size == 0:
                ...
                call('openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=US/ST=NY/L=New York/O=Samba/OU=Self-Signed/CN={domain:s}" -keyout {key:s} -out {crt:s}'.format(
                    **locals()), shell=True)
...
@click.group()
def cli():
    """ Samba - A very small PaaS to do git push deployments. 
    ...
    def add_helper(key_file):
        if exists(key_file):
            try:
                ...
                setup_authorized_keys(fingerprint, SAMBA_SCRIPT, key)
            except Exception:
...
@cli.command("update")
def cmd_update():
    """ Update Samba to the latest from Github """
    print_title("Updating")
    url = "https://raw.githubusercontent.com/mardix/samba/master/samba.py"
    echo("...downloading 'samba.py' from github")
    unlink(SAMBA_SCRIPT)
    urllib.request.urlretrieve(url, SAMBA_SCRIPT)
    chmod(SAMBA_SCRIPT, stat(SAMBA_SCRIPT).st_mode | S_IXUSR)
...
def cmd_init():
    ...
    if not(stat(SAMBA_SCRIPT).st_mode & S_IXUSR):
        echo("Setting '{}' as executable.".format(SAMBA_SCRIPT), fg='yellow')
        chmod(SAMBA_SCRIPT, stat(SAMBA_SCRIPT).st_mode | S_IXUSR)
...
cat | SAMBA_ROOT="{SAMBA_ROOT:s}" {SAMBA_SCRIPT:s} git-hook {app:s}""".format(**env))
