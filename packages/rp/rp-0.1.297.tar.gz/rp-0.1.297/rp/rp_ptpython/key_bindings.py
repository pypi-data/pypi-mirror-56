            ...
            def go(char):
                ...
                def _(event):
                    ...
                    if True:#not '"' in after_line and not "'" in after_line:
                        ...
                        header_commands={'\\co':'copy','\\da':'delete all','\\pa':'paste','\\ya':'yapf autoformat','\\gg':'go to top','\\GG':'go to bottom','\\vO':'vim open up','\\vo':'vim open down','\\tts':'tabs to spaces','\\de':'debug'}
                        ...
                        for command,header in header_commands.items():
                            ...
                            if before_line.endswith(chopped_command) and char==key:
                                ...
                                if header=='debug':
                                    ...
                                    buffer.cursor_left(99999)
                                    if text.startswith(debug_text):
                                        #Delete 'debug()' from the top
                                        buffer.delete(len('debug()\n'))
                                        buffer.document=Document(buffer.document.text,old_cursor_pos-len(debug_text),buffer.document.selection)
                                    else:
                                        #Insert 'debug()' at the top
                                        buffer.insert_text('debug()\n')
                                        buffer.document=Document(buffer.document.text,old_cursor_pos+len(debug_text),buffer.document.selection)
