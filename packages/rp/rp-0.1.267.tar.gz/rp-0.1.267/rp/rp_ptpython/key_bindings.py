    ...
    if not in_string_or_comment:#This is just for visual purposes, so I can put the lines in a block of code and document ,after_line)it
      ...
      #region misc tweaks
        ...
        if char==' ' and re.fullmatch(r'.*[\(\[\, ]\-',before_line):
            ...
            buffer.insert_text('_')
            refresh_strings_from_buffer()
        if char=='\n' and before_line.endswith('-') and (re.fullmatch(r' *[^\w\(\{].*',after_line) or not after_line.strip()):
            #On \n:   `[_ and -|]`  --->  `[_ and _]\n
            #On \n:   `x and -`  --->  `x and _
            #On \n:   `x+-`  --->  `x+-`
            buffer.delete_before_cursor()
        ...
        if char==' ' and before_line.lstrip()=='for ' and after_line==' in :':
            ...
            buffer.cursor_right(len(' in '))
            return True
        if char in '- \n' and starts_with_any(before_line.lstrip(),'nonlocal ','global '):
        ...
        if (char.isalpha() or char=='_') and not '\n' in text and not after and before.endswith('/.'):
            ...
            buffer.delete_before_cursor(2)
            buffer.insert_text('?.')
        if char in '.([' and before and not '\n' in before and not after and ric.current_candidates and re.fullmatch(r'[\w\.]*[\w]+',before):
...
def load_python_bindings(python_input):
                ...
                def _(event):
                    ...
                    if True:#not '"' in after_line and not "'" in after_line:
                        ...
                        header_jump_commands={
                                         ...
                                         '\\fo':'for ',#go f
                                         '\\el':'el'}#go e
                        header_arg_commands={'\\ed':'editor','\\re':'replace','\\py':'python','\\dl':'delete to line','\\go':'goto','\\al':'align_lines','\\ac':'align_char','\\sw':'strip_whitespace','\\mla':'multi line arguments'}
                        header_commands={'\\co':'copy','\\da':'delete all','\\pa':'paste','\\ya':'yapf autoformat','\\gg':'go to top','\\GG':'go to bottom','\\vO':'vim open up','\\vo':'vim open down'}
                        ...
                        for command,header in header_commands.items():
                            ...
                            if before_line.endswith(chopped_command) and char==key:
                                ...
                                if command in header_jump_commands:
                                ...
                                elif command in header_arg_commands and '`' in before_line:
                                    if header=='replace':
                                        #DEMO: Type
                                        #`foo`bar\r
                                    ...
                                    if header=='python':
                                        ...
                                        if before_line.count('`')==1:#give a lambda that takes one argument
                                            ...
                                            text=buffer.document.text
                                            try:modifier=eval(arg,r_iterm_comm.globa);text=modifier(text);buffer.document=Document(text,buffer.document.cursor_position,buffer.document.selection)
                                            except BaseException as E:buffer.insert_text("\nERROR: "+str(E)+"\n(Undo to make me go away)\n")
                                    if header=='goto':
    ...
    def _(event):# Spacebar event handle spacebar
        ...
        if document.text=='':# What else would we possibly want the spacebar for on an empty input? Spacebar invokes functions, and the default variable is ans.
            import rp.r_iterm_comm as ric
            if callable(eval(token_name,r_iterm_comm.globa)):
                buffer.insert_text('ans()')
                buffer.cursor_left()
    ...
    if os.name != 'nt':#If we are NOT running windows, which screws EVERYTHING up...
        ...
        def run_code_without_destroying_buffer(event):
        ...
        def run_arbitrary_code_without_destroying_buffer(code,event):
            buffer=event.cli.current_buffer
        ...
        def _(event):
            ...
            def main():
                ...
                run_arbitrary_code_without_destroying_buffer(cell_code,event)
            main()
