...
class Buffer(object):
    ...
    def auto_up(self, count=1, go_to_start_of_line_if_history_changes=False,ryan_allow_completion_navigation=True):
        """
        If we're not on the first line (of a multiline input) go a line up,
        otherwise go back in history. (If nothing is selected.)
        """
        if self.complete_state and ryan_allow_completion_navigation:
    ...
    def auto_down(self, count=1, go_to_start_of_line_if_history_changes=False,ryan_allow_completion_navigation=True):
        """
