...
KEY KEY:
	// In this documentation, you might see non-ascii symbols such as ↵ or ⌥. These symbols represent keys that can't otherwise be represented with a signle character. This section is a key to tell you which symbol corresponds to which key, which will be useful for later sections of this documentation.
...
MICROCOMPLETIONS:
	...
	BACKSLASH COMMANDS:
		...
		WITHOUT ARGUMENTS:
			...
			\pu  pudb                    (similar to \de, pudb is an alternative debugger, and this command also toggles a line on the top of the buffer)
			\ed  editor                  (opens up the current buffer in vim)
			\ac  align char              (inserts ‹→›, aka the alignment character)
			\al  align                   (aligns all alignment characters to the same column)
			\sw  strip whitespace        (strips all trailing whitespace)
			\co  copy                    (copies the current buffer to your clipboard)
			\da  delete all              (deletes everything in the current buffer)
			...
			\vO  insert line above       (like O in vim.)
			\vo  insert line below       (like o in vim.)
...
#Guidelines for this document (for ryan to read this section):
	...
	- Contextual completions as well as global completions (the difference between conetxteual: "On ‹x›: ‹y=¦›  –––>  ‹y=x¦›" vs global aka "‹y=x› –––> ‹y=x¦›). Contextual has an "on keypress", and the left ‹› and right ‹› both have cursor positoins. The global ones have no cursor in the left ‹› as it implicitly starts at the last character, and all chacters shwn in tat ‹› are pressed to get the ‹› on the right.
...
Tips and tricks:
	Debugging:
		Debuggers:
			...
			- Because exiting the debugger throws an error, it also prevents the rest of the code in your buffer (after the debugger) from running. Often this is desirable, actually. Of course, if you don't want this, just use continue instead
		ic():
