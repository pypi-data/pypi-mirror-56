...
class Token(object): # Abstract Class
	"""
	Dictionary interface for retrieving morphological data by id
	"""

	def __init__(self, surfaceForm, offset, length, startOffset, endOffset, posType, morphemes, posTag, dictType):
		self.surfaceForm = surfaceForm 	# to be char list type
		self.offset = offset
		self.length = length
		self.startOffset = startOffset
		self.endOffset = endOffset
		self.posType = posType 		# MORPHEME or COMPOUND
	...
	def getSurfaceFormString(self):
		...
		#return ''.join(self.surfaceForm[self.offset:self.length])
		return self.surfaceForm
