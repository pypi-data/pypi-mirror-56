...
def character_category_map(ch):
	...
	# Latin
		...
		return 'SYMBOL'
	elif hex_ch in range(0x00C0, 0x00FF+1) or \
		 hex_ch in range(0x0100, 0x017F+1) or \
		 hex_ch in range(0x0180, 0x0236+1) or \
		 hex_ch in range(0x1E00, 0x1EF9+1):
		return 'ALPHA'

	# CYRILLIC
	elif hex_ch in range(0x0400, 0x04F9+1) or \
		 hex_ch in range(0x0500, 0x050F+1):
		return 'CYRILLIC'
	...
	# HANJA-NUMERIC
	elif hex_ch in [0x4E00, 0x4E8C, 0x4E09, 0x56DB, 0x4E94, 0x516D, 0x4E03, 0x516B, 
...
class CharacterDefinition(object):
	...
	def getCharacterClass(self, ch):
		return character_category_map(ch) # return 'SPACE' or 'HANGUL' or ...

	def isInvoke(self, ch):
	...
	def isHangul(self, ch):
		if ord(ch) > 0xAC00 and ord(ch) < 0xD7A3: # 한글 음절 범위 (가 ~ 힣)
			return True
		return False
