...
def _case_watcher_json(event: dict = None,
...
def sort_input(data: list, sort_by: list) -> list:
    ...
    def _key(event):
        ...
        return tuple((field not in event, _get_field(field, event)) for field in sort_by)
...
def put_data_in_case(
    ...
    if case:
        ...
        if (set_initial_comment):
            initial_comment = create_description(data, customer, comment=False, internal=True)
            create_comment(case["id"], comment=initial_comment, internal=True)
            log.debug("Added internal comment after creating case #{id}".format_map(case))

    event_accociator(case["id"], data)
    log.debug("Associated these events with case {id}:\n{events}"
              .format(id=case["id"], events=[event["id"] for event in data]))

    return case


def print_results(events: list, cases: set, dry: bool):
    stats = "\n\t".join(
...
def autocreate(
    ...
    :param case_watcher_from_field: String that points to where e-mail
    address for case watcher is in the event
    ...
    :param internal_case: Makes new cases only visible internally
    ...
    def _create_description(events: list, customer: dict, comment: bool = False, internal: bool = False) -> str:
    ...
    putter = partial(
        ...
        create_comment=
            _add_comment if not dry else lambda *a, **kw: None,
        create_case=
            _create_case if not dry else lambda *a, **kw: {"id": "DRY CASE"},
        event_accociator=
            _event_accociator if not dry else lambda *a, **kw: None,
