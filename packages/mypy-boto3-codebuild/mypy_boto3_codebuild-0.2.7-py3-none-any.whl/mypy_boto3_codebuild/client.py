"Main interface for codebuild Client"
...
class Client(BaseClient):
            ...
            - **builds** *(list) --*
              ...
              - *(dict) --*
                ...
                - **vpcConfig** *(dict) --*
                  ...
                  The security groups and subnets must belong to the same VPC. You must provide at least one
                        ...
                        'webhook': {
                            ...
                            'filterGroups': [
                                [
                                    {
                                        'type':
                                        'EVENT'|'BASE_REF'|'HEAD_REF'|'ACTOR_ACCOUNT_ID'|'FILE_PATH',
                                        'pattern': 'string',
        ...
        **Request Syntax**
        ::
          ...
          response = client.create_project(
              ...
              source={
                  'type':
                  'CODECOMMIT'|'CODEPIPELINE'|'GITHUB'|'S3'|'BITBUCKET'|'GITHUB_ENTERPRISE'|'NO_SOURCE',
                  'location': 'string',
                  'gitCloneDepth': 123,
                  'gitSubmodulesConfig': {
                      'fetchSubmodules': True|False
        ...
        :param source: **[REQUIRED]**
          ...
          - **location** *(string) --*
            ...
            * For source code settings that are specified in the source action of a pipeline in AWS
            CodePipeline, ``location`` should not be specified. If it is specified, AWS CodePipeline ignores
            it. This is because AWS CodePipeline uses the settings in a pipeline's source action instead of
            this value.
            ...
            * For source code in a GitHub repository, the HTTPS clone URL to the repository that contains
            the source and the build spec. You must connect your AWS account to your GitHub account. Use the
            AWS CodeBuild console to start creating a build project. When you use the console to connect (or
            reconnect) with GitHub, on the GitHub **Authorize application** page, for **Organization
            ...
            * For source code in a Bitbucket repository, the HTTPS clone URL to the repository that contains
            the source and the build spec. You must connect your AWS account to your Bitbucket account. Use
            the AWS CodeBuild console to start creating a build project. When you use the console to connect
            (or reconnect) with Bitbucket, on the Bitbucket **Confirm access to your account** page, choose
            **Grant access** . (After you have connected to your Bitbucket account, you do not need to
            finish creating the build project. You can leave the AWS CodeBuild console.) To instruct AWS
            CodeBuild to use this connection, in the ``source`` object, set the ``auth`` object's ``type``
            value to ``OAUTH`` .
        ...
        :param sourceVersion:
          ...
          * For GitHub: the commit ID, pull request ID, branch name, or tag name that corresponds to the
          ...
          format ``pr/pull-request-ID`` (for example ``pr/25`` ). If a branch name is specified, the
          ...
          * For Amazon Simple Storage Service (Amazon S3): the version ID of the object that represents the
          ...
          For more information, see `Source Version Sample with CodeBuild
          <https://docs.aws.amazon.com/codebuild/latest/userguide/sample-source-version.html>`__ in the *AWS
        ...
        :param secondarySourceVersions:
          ...
          build level, then they take precedence over these ``secondarySourceVersions`` (at the project
          level).
          ...
          - *(dict) --*
            ...
            - **sourceVersion** *(string) --* **[REQUIRED]**
              ...
              * For GitHub: the commit ID, pull request ID, branch name, or tag name that corresponds to the
              version of the source code you want to build. If a pull request ID is specified, it must use
              the format ``pr/pull-request-ID`` (for example, ``pr/25`` ). If a branch name is specified,
          ...
          - *(dict) --*
            ...
            - **path** *(string) --*
              ...
              * If ``type`` is set to ``S3`` , this is the path to the output artifact. If ``path`` is not
              ...
              and ``name`` is set to ``MyArtifact.zip`` , the output artifact is stored in the output bucket
        ...
        :param cache:
          ...
          - **modes** *(list) --*
            ...
            If you use a ``LOCAL`` cache, the local cache mode. You can use one or more local cache modes at
            the same time.
            ...
            * ``LOCAL_CUSTOM_CACHE`` mode caches directories you specify in the buildspec file. This mode is
        ...
        :param environment: **[REQUIRED]**
          ...
          - **type** *(string) --* **[REQUIRED]**
            ...
            * The environment type ``LINUX_CONTAINER`` with compute type ``build.general1.2xlarge`` is
            ...
            (Central), EU (Ireland), EU (London), EU (Frankfurt), Asia Pacific (Tokyo), Asia Pacific
            (Seoul), Asia Pacific (Singapore), Asia Pacific (Sydney), China (Beijing), and China (Ningxia).

            * The environment type ``LINUX_GPU_CONTAINER`` is available only in regions US East (N.
            Virginia), US East (N. Virginia), US West (Oregon), Canada (Central), EU (Ireland), EU (London),
        ...
        :param timeoutInMinutes:
          ...
          How long, in minutes, from 5 to 480 (8 hours), for AWS CodeBuild to wait before it times out any
          build that has not been marked as completed. The default is 60 minutes.
        ...
        :param encryptionKey:
          ...
          The AWS Key Management Service (AWS KMS) customer master key (CMK) to be used for encrypting the
        ...
        :param logsConfig:
          ...
          - **cloudWatchLogs** *(dict) --*
            ...
            Information about Amazon CloudWatch Logs for a build project. Amazon CloudWatch Logs are enabled
            by default.
            ...
            - **project** *(dict) --*
              ...
              - **source** *(dict) --*
                ...
                - **location** *(string) --*
                  ...
                  * For source code in a GitHub repository, the HTTPS clone URL to the repository that
                  contains the source and the build spec. You must connect your AWS account to your GitHub
                  account. Use the AWS CodeBuild console to start creating a build project. When you use the
                  console to connect (or reconnect) with GitHub, on the GitHub **Authorize application**
                  page, for **Organization access** , choose **Request access** next to each repository you
                  want to allow AWS CodeBuild to have access to, and then choose **Authorize application** .
                  (After you have connected to your GitHub account, you do not need to finish creating the
                  build project. You can leave the AWS CodeBuild console.) To instruct AWS CodeBuild to use
                  this connection, in the ``source`` object, set the ``auth`` object's ``type`` value to
                  ``OAUTH`` .
                  ...
                  * For source code in a Bitbucket repository, the HTTPS clone URL to the repository that
                  ...
                  Bitbucket account. Use the AWS CodeBuild console to start creating a build project. When
                  you use the console to connect (or reconnect) with Bitbucket, on the Bitbucket **Confirm
                  access to your account** page, choose **Grant access** . (After you have connected to your
                  Bitbucket account, you do not need to finish creating the build project. You can leave the
                  AWS CodeBuild console.) To instruct AWS CodeBuild to use this connection, in the
                  ``source`` object, set the ``auth`` object's ``type`` value to ``OAUTH`` .
              ...
              - **artifacts** *(dict) --*
                ...
                - **name** *(string) --*
                  ...
                  * If ``type`` is set to ``S3`` , this is the name of the output artifact object. If you
                  set the name to be a forward slash ("/"), the artifact is stored in the root of the output
                  bucket.
              ...
              - **cache** *(dict) --*
                ...
                - **modes** *(list) --*
                  ...
                  * ``LOCAL_SOURCE_CACHE`` mode caches Git metadata for primary and secondary sources. After
                  the cache is created, subsequent builds pull only the change between commits. This mode is
                  a good choice for projects with a clean working directory and a source that is a large Git
                  repository. If you choose this option and your project does not use a Git repository
                  (GitHub, GitHub Enterprise, or Bitbucket), the option is ignored.

                  * ``LOCAL_DOCKER_LAYER_CACHE`` mode caches existing Docker layers. This mode is a good
                  choice for projects that build or pull large Docker images. It can prevent the performance
                  ...
                  * ``LOCAL_CUSTOM_CACHE`` mode caches directories you specify in the buildspec file. This
                  mode is a good choice if your build scenario is not suited to one of the other three local
              ...
              - **environment** *(dict) --*
                ...
                - **type** *(string) --*
                  ...
                  * The environment type ``LINUX_CONTAINER`` with compute type ``build.general1.2xlarge`` is
                  ...
                  Pacific (Seoul), Asia Pacific (Singapore), Asia Pacific (Sydney), China (Beijing), and
                  China (Ningxia).
                ...
                - **image** *(string) --*
                  ...
                  * For an image digest: ``registry/repository@digest`` . For example, to specify an image
                  with the digest "sha256:cbbf2f9a99b47fc460d422812b6a5adff7dfee951d8fa2e4a98caa0382cfbdbf,"
                  use
                  ``registry/repository@sha256:cbbf2f9a99b47fc460d422812b6a5adff7dfee951d8fa2e4a98caa0382cfbdbf``
                  .
                ...
                - **privilegedMode** *(boolean) --*
                  ...
                  Enables running the Docker daemon inside a Docker container. Set to true only if the build
                  project is used to build Docker images. Otherwise, a build that attempts to interact with
                  the Docker daemon fails. The default setting is ``false`` .
              ...
              - **encryptionKey** *(string) --*
                ...
                The AWS Key Management Service (AWS KMS) customer master key (CMK) to be used for encrypting
    ...
    def create_webhook(
        ...
        """
        For an existing AWS CodeBuild build project that has its source code stored in a GitHub or Bitbucket
        repository, enables AWS CodeBuild to start rebuilding the source code every time a code change is
        pushed to the repository.

        .. warning::

          If you enable webhooks for an AWS CodeBuild project, and the project is used as a build step in
          AWS CodePipeline, then two identical builds are created for each commit. One build is triggered
        ...
        **Request Syntax**
        ...
        :param filterGroups:
          ...
          For a build to be triggered, at least one filter group in the ``filterGroups`` array must pass.
                ...
                - *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    - **pattern** *(string) --*
                      ...
                      the pattern ``^refs/heads/`` triggers a build when the head reference is a branch with
                      a reference name ``refs/heads/branch-name`` .

                    - **excludeMatchedPattern** *(boolean) --*

                      Used to indicate that the ``pattern`` determines which webhook events do not trigger a
                      build. If true, then a webhook event that does not match the ``pattern`` triggers a
    ...
    def delete_webhook(self, projectName: str) -> Dict[str, Any]:
        """
        For an existing AWS CodeBuild build project that has its source code stored in a GitHub or Bitbucket
        repository, stops AWS CodeBuild from rebuilding the source code every time a code change is pushed
        to the repository.
    ...
    def import_source_credentials(
        ...
        """
        Imports the source repository credentials for an AWS CodeBuild project that has its source code
        ...
        **Request Syntax**
        ...
        :param authType: **[REQUIRED]**
          ...
          repository. An OAUTH connection is not supported by the API and must be created using the AWS
          CodeBuild console.
                ...
                'platforms': [
                    {
                        ...
                        'languages': [
                            {
                                'language':
                                'JAVA'|'PYTHON'|'NODE_JS'|'RUBY'|'GOLANG'|'DOCKER'|'ANDROID'|'DOTNET'|'BASE'
                                |'PHP',
                                'images': [
                                    {
                                        'name': 'string',
    ...
    def start_build(
        ...
        **Request Syntax**
        ...
        :param sourceVersion:
          ...
          A version of the build input to be built, for this build only. If not specified, the latest
          version is used. If specified, must be one of:
          ...
          * For Amazon Simple Storage Service (Amazon S3): the version ID of the object that represents the
          ...
          If ``sourceVersion`` is specified at the project level, then this ``sourceVersion`` (at the build
        ...
        :param environmentVariablesOverride:
          ...
          A set of environment variables that overrides, for this build only, the latest ones already
          ...
          - *(dict) --*
            ...
            - **value** *(string) --* **[REQUIRED]**
              ...
              .. warning::
                ...
                We strongly discourage the use of environment variables to store sensitive values,
                especially AWS secret key IDs and secret access keys. Environment variables can be displayed
                in plain text using the AWS CodeBuild console and the AWS Command Line Interface (AWS CLI).
        ...
        :param buildspecOverride:
          ...
          A build spec declaration that overrides, for this build only, the latest one already defined in
          the build project.
        ...
        :param reportBuildStatusOverride:
          ...
          use this option with a source provider other than GitHub, GitHub Enterprise, or Bitbucket, an
          invalidInputException is thrown.
        ...
        :param idempotencyToken:
          ...
          request. The token is included in the StartBuild request and is valid for 12 hours. If you repeat
          the StartBuild request with the same token, but change a parameter, AWS CodeBuild returns a
                ...
                - *(dict) --*
                  ...
                  - **value** *(string) --*
                    ...
                    .. note::
                      ...
                      During a build, the value of a variable is available starting with the ``install``
                      phase. It can be updated between the start of the ``install`` phase and the end of the
                      ``post_build`` phase. After the ``post_build`` phase ends, the value of exported
                      variables cannot change.
    ...
    def update_project(
        ...
        **Request Syntax**
        ...
        :param timeoutInMinutes:
          ...
          The replacement value in minutes, from 5 to 480 (8 hours), for AWS CodeBuild to wait before timing
    ...
    def update_webhook(
        ...
        **Request Syntax**
        ...
        :param filterGroups:
          ...
          build. A filter group must pcontain at least one ``EVENT``  ``WebhookFilter`` .
