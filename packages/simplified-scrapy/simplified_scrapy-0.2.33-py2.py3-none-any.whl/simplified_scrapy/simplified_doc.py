...
class SimplifiedDoc():
  ...
  def getParent(self,tag=None,attr=None,value=None,html=None,start=None,end=None):
    if(not html):html=self.html
    if(not html and self.last):html=self.last.innerHtml
    return getParent(tag,attr,value,html,start,end)
    
  def getChildren(self,html=None,tag=None,start=None,end=None):
    if(not html):html=self.html
    if(not html and self.last):html=self.last.innerHtml
    return getChildren(html,tag,start,end)

  def getNexts(self,attr,value,html,tag=None,start=None,end=None):
    if(not html):html=self.html
    if(not html and self.last):html=self.last.innerHtml
    return getNexts(attr,value,html,tag,start,end)