...
def getParent(tag=None,attr=None,value=None,html=None,start=None,end=None):
  ele = _getElement(tag,attr,value,html,start,end)
  if(ele):
    start = ele[1]
...
def _checkSingle(html,start=None,end=None):
  if(not end):
    end = html.find('>',start)
    return html[end-1:end]=='/'
  return False
def getChildren(html,tag=None,start=None,end=None):
  if(tag):
...
def _getNextTag(html,start):
  ...
  return None
def getNexts(attr,value,html,tag=None,start=None,end=None):
  ele = _getElement(tag,attr,value,html,start,end)
  if(ele):
    start = ele[2]
...
# print(getNexts(attr="name",value="overview",html='''
# <p><h2>Overview1</h2><br /><a name="overview"></a><h2>Overview2</h2><br /><h3>Overview3</h3></p>
# <h2>Overview</h2><br />
# <p>Accounting is the language of business. Accounting concepts come into play when you&rsquo;re checking your bank
#   balance online or filling out IRD tax forms.</p>
# '''))