...
class ZMQ(Singleton):
    ...
    def __init__(self):
        ...
        for _server_key in self._server_confs.keys():
            ...
            _socket = self._context.socket(zmq.ROUTER)
    ...
    def restore_socket_connection(self, server_key, force_new_socket=False):
        ...
        try:
            ...
            if self._instances[server_key]['retries_left'] > 0:
                ...
                events = yield from self._poller.poll(ZEROMQ_RETRY_TIMEOUT)
    ...
    def send_and_recv(self, server_key, message):
        ...
        try:
            ...
            # Log.info(_instance['retries_left'])
            ...
            while _instance['retries_left']:
                ...
                try:
                    ...
                    while True:
                        ...
                        events = yield from self._poller.poll(timeout=1)
                        ...
                        if _instance['socket'] in _socks:
                            
                            # _instance['socket'].recv() # discard delimiter
                            # _response = _instance['socket'].recv_json() # actual message
                            _response = yield from _instance['socket'].recv()

                            Log.info(_response)

                            # Log.info(_response[1])

                            _response = json.loads(_response[1])
