    ...
    if full or isinstance(view, str) and '/' in view:
        ...
        try:
            if not callable(view) or resolve:
                view, routed_args, routed_kwargs = \
                    resolve_viewspec(view, *args, **kwargs)
...
def response_to_str(response) -> str:
    ...
    try:
        ...
        # ...or a list of strings?
              ...
              and all(isinstance(i, str) for i in response.content)):
...
def resolve_viewspec(viewspec, *args, **kwargs):
    ...
    if isinstance(viewspec, str) and ':' in viewspec:
        ...
        rc = delegated_route.routecollectionfactory(*delegate_args,
                                                    **delegate_kwargs)
        return resolve_viewspec(remainder,
                                _routecollection=rc,
                                request=request,
                                method=method,
                                *args, **kwargs)
...
def _get_args_for_route(route: Route,
                        request: Request,
                        args: Tuple,
    ...
    route_kwargs = {}
    route_args = list(chain(
        ((a(request) if isinstance(a, RouteArg) else a)
            for a in route.view_args),
        (mutable_args.pop(0) for a in route.pattern.segments
            if a.converter and not a.name)))

    required_kwargs = chain(route.view_kwargs.keys(),
                            (s.name
                             for s in route.pattern.segments
                             if s.converter and s.name))
