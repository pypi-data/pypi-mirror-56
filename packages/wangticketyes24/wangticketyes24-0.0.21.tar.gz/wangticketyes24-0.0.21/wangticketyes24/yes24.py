...
class PerfTime(object):
	...
	def isPrefered(self, filter_time):
		if filter_time is None:
			return False

		if self.timeinfo == filter_time:
			return True
...
class Yes24(object):
	...
	def _getPerfTime(self, date, filter_time=None):
		...
		for li in soup.findAll('li'):
			...
			if myPerfTime.isPrefered(filter_time):
...
function fdc_PreCheckPromotionBase(jpIdPromotion, jpIdCode, jpIdAuth, jpIsOverlap, jpIsLimit, jpLimitNo, jpGroupIndex, jpSelObj)
	"""
	def _getPromotion(self, seldate, idtime, promo):
		if promo == "":
			return
		...
		for tr in trs:
			if 'idpromotion' in tr.attrs:
				...
				if promo in promotionstr:
					...
					self.sel.change_element_int_value_by_id(selpromoid, limitno)
					break


	def getPromotion(self, seldate, idtime, promo):
		EXCEPT_COUNT = 0
		while True:
			if EXCEPT_COUNT > 100:
				raise Exception("TOO MUCH EXCEPTION...")
			...
			try:
				result = self._getPromotion(seldate, idtime, promo)
			except:
				print(traceback.format_exc())
	...
	def myOrderView(self, idorder):
		...
		whattd = {'class':'ri'} if self.canceltype == "BANK" else {'style':'display:none;'}
	...
	def browser_wait_last_page(self, payment_method=1, deposit=0, captcha_self=False):
		self.sel.goLastPage()
		self.sel.prepare_lastpage(payment_method=payment_method)
	...
	def getDeliveryInfo(self):
		...
		if mobile1 == '' or mobile2 == '' or mobile3 == '':
			print("no mobile number... abort")
			exit()
		if mailh == '' or maild == '':
			print("no email... abort")
			exit()
		if zipcode == '' or address1 == '' or address2 == '':
			print("no address... abort")
			exit()
