...
class ClientCreateServerTagsTypeDef(_ClientCreateServerTagsTypeDef):
    ...
    you might create a tag called ``Group`` and assign the values ``Research`` and ``Accounting`` to
    that group.
...
class ClientDescribeServerResponseServerTypeDef(_ClientDescribeServerResponseServerTypeDef):
    ...
    - **EndpointType** *(string) --*
      ...
      The type of endpoint that your SFTP server is connected to. If your SFTP server is connected
    ...
    - **IdentityProviderType** *(string) --*
      ...
      ``SERVICE_MANAGED`` means that you are using this server to store and access SFTP user
      credentials within the service. A value of ``API_GATEWAY`` indicates that you have integrated
      an API Gateway endpoint that will be invoked for authenticating your user into the service.

    - **LoggingRole** *(string) --*

      This property is an AWS Identity and Access Management (IAM) entity that allows the server to
      turn on Amazon CloudWatch logging for Amazon S3 events. When set, user activity can be viewed
    ...
    - **State** *(string) --*
      ...
      indicates that the server can accept jobs and transfer files. A ``State`` value of ``OFFLINE``
      ...
      or ``STOP_FAILED`` can indicate an error condition.
...
class ClientDescribeUserResponseUserSshPublicKeysTypeDef(
    ...
    Provides information about the public Secure Shell (SSH) key that is associated with a user
    account for a specific server (as identified by ``ServerId`` ). The information returned
    includes the date the key was imported, the public key contents, and the public key ID. A user
    can store more than one SSH public key associated with their user name on a specific SFTP
    server.
...
class ClientDescribeUserResponseUserTypeDef(_ClientDescribeUserResponseUserTypeDef):
    ...
    - **Arn** *(string) --*
      ...
      This property contains the unique Amazon Resource Name (ARN) for the user that was requested
      to be described.
    ...
    - **HomeDirectoryMappings** *(list) --*
      ...
      Logical directory mappings that you specified for what S3 paths and keys should be visible to
      your user and how you want to make them visible. You will need to specify the "``Entry`` " and
      "``Target`` " pair, where ``Entry`` shows how the path is made visible and ``Target`` is the
      actual S3 path. If you only specify a target, it will be displayed as is. You will need to
      also make sure that your AWS IAM Role provides access to paths in ``Target`` .

      In most cases, you can use this value instead of the scope down policy to lock your user down
    ...
    - **HomeDirectoryType** *(string) --*
      ...
      the SFTP server. If you set it to ``PATH`` , the user will see the absolute Amazon S3 bucket
      paths as is in their SFTP clients. If you set it ``LOGICAL`` , you will need to provide
      mappings in the ``HomeDirectoryMappings`` for how you want to make S3 paths visible to your
      user.
    ...
    - **SshPublicKeys** *(list) --*
      ...
      - *(dict) --*
        ...
        Provides information about the public Secure Shell (SSH) key that is associated with a user
        account for a specific server (as identified by ``ServerId`` ). The information returned
        includes the date the key was imported, the public key contents, and the public key ID. A
        user can store more than one SSH public key associated with their user name on a specific
        SFTP server.
    ...
    - **Tags** *(list) --*
      ...
      This property contains the key-value pairs for the user requested. Tag can be used to search
    ...
    - **UserName** *(string) --*
      ...
      This property is the name of the user that was requested to be described. User names are used
      for authentication purposes. This is the string that will be used by your user when they log
      in to your SFTP server.
    """
    ...
    - **User** *(dict) --*
      ...
      - **SshPublicKeys** *(list) --*
        ...
        - *(dict) --*
          ...
          key ID. A user can store more than one SSH public key associated with their user name on a
...
class ClientImportSshPublicKeyResponseTypeDef(_ClientImportSshPublicKeyResponseTypeDef):
    ...
    public key associated with that user. A user can have more than one key on each server that they
    are associated with.
...
class ClientListServersResponseServersTypeDef(_ClientListServersResponseServersTypeDef):
    ...
    - **IdentityProviderType** *(string) --*
      ...
      The authentication method used to validate a user for the server that was specified. This can
      include Secure Shell (SSH), user name and password combinations, or your own custom
    ...
    - **State** *(string) --*
      ...
      This property describes the condition of the SFTP server for the server that was described. A
      value of ``ONLINE`` > indicates that the server can accept jobs and transfer files. A
      ``State`` value of ``OFFLINE`` means that the server cannot perform file transfer operations.
    ...
    - **UserCount** *(integer) --*
      ...
      This property is a numeric value that indicates the number of users that are assigned to the
      SFTP server you specified with the ``ServerId`` .
    """
    ...
    - **Servers** *(list) --*
      ...
      - *(dict) --*
        ...
        - **State** *(string) --*
          ...
          files. A ``State`` value of ``OFFLINE`` means that the server cannot perform file transfer
          operations.
...
class ClientListUsersResponseUsersTypeDef(_ClientListUsersResponseUsersTypeDef):
    ...
    - **Arn** *(string) --*
      ...
      This property is the unique Amazon Resource Name (ARN) for the user that you want to learn
      about.

    - **HomeDirectory** *(string) --*

      This value specifies the location that files are written to or read from an Amazon S3 bucket
      for the user you specify by their ARN.

    - **HomeDirectoryType** *(string) --*

      The type of landing directory (folder) you mapped for your users' home directory. If you set
      it to ``PATH`` , the user will see the absolute Amazon S3 bucket paths as is in their SFTP
    ...
    - **Role** *(string) --*
      ...
      The role in use by this user. A *role* is an AWS Identity and Access Management (IAM) entity
      that, in this case, allows the SFTP server to act on a user's behalf. It allows the server to
      inherit the trust relationship that enables that user to perform file operations to their
      Amazon S3 bucket.
    ...
    - **Users** *(list) --*
      ...
      - *(dict) --*
        ...
        - **Role** *(string) --*
          ...
          entity that, in this case, allows the SFTP server to act on a user's behalf. It allows the
          server to inherit the trust relationship that enables that user to perform file operations
