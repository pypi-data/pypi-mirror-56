...
class Client(BaseClient):
    ...
    def create_server(
        ...
        When you make updates to your server or when you work with users, use the service-generated
        ...
        **Request Syntax**
        ...
        :param IdentityProviderType:
          ...
          , which allows you to store and access SFTP user credentials within the AWS Transfer for SFTP
          service. Use the ``API_GATEWAY`` value to integrate with an identity provider of your choosing.
    ...
    def create_user(
        ...
        """
        Creates a user and associates them with an existing Secure File Transfer Protocol (SFTP) server. You
        can only create and associate users with SFTP servers that have the ``IdentityProviderType`` set to
        ...
        Management (IAM) role. You can also optionally add a scope-down policy, and assign metadata with
        ...
        **Request Syntax**
        ...
        :param HomeDirectoryType:
          ...
          The type of landing directory (folder) you want your users' home directory to be when they log
          into the SFTP server. If you set it to ``PATH`` , the user will see the absolute Amazon S3 bucket
          paths as is in their SFTP clients. If you set it ``LOGICAL`` , you will need to provide mappings
          in the ``HomeDirectoryMappings`` for how you want to make S3 paths visible to your user.

        :type HomeDirectoryMappings: list
        :param HomeDirectoryMappings:

          Logical directory mappings that specify what S3 paths and keys should be visible to your user and
          how you want to make them visible. You will need to specify the "``Entry`` " and "``Target`` "
          pair, where ``Entry`` shows how the path is made visible and ``Target`` is the actual S3 path. If
          you only specify a target, it will be displayed as is. You will need to also make sure that your
          AWS IAM Role provides access to paths in ``Target`` . The following is an example.

           ``'[ "/bucket2/documentation", { "Entry": "your-personal-report.pdf", "Target":
           "/bucket3/customized-reports/${transfer:UserName}.pdf" } ]'``

          In most cases, you can use this value instead of the scope down policy to lock your user down to
        ...
        :param Policy:
          ...
          .. note::
            ...
            For scope-down policies, AWS Transfer for SFTP stores the policy as a JSON blob, instead of the
            ...
            For more information, see
            "https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html" in the *AWS Security
        ...
        :param Role: **[REQUIRED]**
          ...
          this role will determine the level of access you want to provide your users when transferring
          files into and out of your Amazon S3 bucket or buckets. The IAM role should also contain a trust
        ...
        :param Tags:
          ...
          Key-value pairs that can be used to group and search for users. Tags are metadata attached to
        ...
        :param UserName: **[REQUIRED]**
          ...
          ``ServerId`` . This user name must be a minimum of 3 and a maximum of 32 characters long. The
          following are valid characters: a-z, A-Z, 0-9, underscore, and hyphen. The user name can't start
          with a hyphen.
    ...
    def delete_ssh_public_key(
        ...
        **Request Syntax**
        ...
        :param ServerId: **[REQUIRED]**
          ...
          A system-assigned unique identifier for a Secure File Transfer Protocol (SFTP) server instance
    ...
    def import_ssh_public_key(
        ...
        """
        Adds a Secure Shell (SSH) public key to a user account identified by a ``UserName`` value assigned
        to a specific server, identified by ``ServerId`` .
    ...
    def list_servers(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          When additional results are obtained from the ``ListServers`` command, a ``NextToken`` parameter
          is returned in the output. You can then pass the ``NextToken`` parameter in a subsequent command
    ...
    def list_tags_for_resource(
        ...
        """
        Lists all of the tags associated with the Amazon Resource Number (ARN) you specify. The resource can
        be a user, server, or role.
    ...
    def list_users(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          returned in the output. You can then pass in a subsequent command to the ``NextToken`` parameter
          to continue listing additional users.

        :type ServerId: string
        :param ServerId: **[REQUIRED]**

          A system-assigned unique identifier for a Secure File Transfer Protocol (SFTP) server that has
    ...
    def start_server(self, ServerId: str) -> None:
        ...
        It has no impact on an SFTP server that is already ``ONLINE`` . An ``ONLINE`` server can accept and
        process file transfer jobs.

        The state of ``STARTING`` indicates that the server is in an intermediate state, either not fully
        able to respond, or not fully online. The values of ``START_FAILED`` can indicate an error
        condition.
    ...
    def stop_server(self, ServerId: str) -> None:
        ...
        accept and process file transfer jobs. Information tied to your server such as server and user
        properties are not affected by stopping your server. Stopping a server will not reduce or impact
        your Secure File Transfer Protocol (SFTP) endpoint billing.

        The state of ``STOPPING`` indicates that the server is in an intermediate state, either not fully
        able to respond, or not fully offline. The values of ``STOP_FAILED`` can indicate an error
        condition.
    ...
    def tag_resource(self, Arn: str, Tags: List[ClientTagResourceTagsTypeDef]) -> None:
        ...
        **Request Syntax**
        ...
        :param Tags: **[REQUIRED]**
          ...
          Key-value pairs assigned to ARNs that you can use to group and search for resources by type. You
          can attach this metadata to user accounts for any purpose.
    ...
    def test_identity_provider(
        ...
        """
        If the ``IdentityProviderType`` of the server is ``API_Gateway`` , tests whether your API Gateway is
        set up successfully. We highly recommend that you call this operation to test your authentication
        method as soon as you create your server. By doing so, you can troubleshoot issues with the API
        Gateway integration to ensure that your users can successfully use the service.
    ...
    def untag_resource(self, Arn: str, TagKeys: List[str]) -> None:
        ...
        **Request Syntax**
        ...
        :param Arn: **[REQUIRED]**
          ...
          This is the value of the resource that will have the tag removed. An Amazon Resource Name (ARN) is
    ...
    def update_server(
        ...
        **Request Syntax**
        ...
        :param EndpointType:
          ...
          The type of endpoint that you want your SFTP server to connect to. You can choose to connect to
          the public internet or a virtual private cloud (VPC) endpoint. With a VPC endpoint, your SFTP
        ...
        :param LoggingRole:
          ...
          A value that changes the AWS Identity and Access Management (IAM) role that allows Amazon S3
          events to be logged in Amazon CloudWatch, turning logging on or off.

        :type ServerId: string
        :param ServerId: **[REQUIRED]**

          A system-assigned unique identifier for an SFTP server instance that the user account is assigned
          to.
    ...
    def update_user(
        ...
        **Request Syntax**
        ...
        :param HomeDirectory:
          ...
          A parameter that specifies the landing directory (folder) for a user when they log in to the
        ...
        :param HomeDirectoryType:
          ...
          The type of landing directory (folder) you want your users' home directory to be when they log
          into the SFTP serve. If you set it to ``PATH`` , the user will see the absolute Amazon S3 bucket
          paths as is in their SFTP clients. If you set it ``LOGICAL`` , you will need to provide mappings
          in the ``HomeDirectoryMappings`` for how you want to make S3 paths visible to your user.
        ...
        :param Policy:
          ...
          Allows you to supply a scope-down policy for your user so you can use the same AWS Identity and
          Access Management (IAM) role across multiple users. The policy scopes down user access to portions
        ...
        :param Role:
          ...
          this role will determine the level of access you want to provide your users when transferring
          files into and out of your Amazon S3 bucket or buckets. The IAM role should also contain a trust
          relationship that allows the Secure File Transfer Protocol (SFTP) server to access your resources
        ...
        :param UserName: **[REQUIRED]**
          ...
          A unique string that identifies a user and is associated with a server as specified by the
          ``ServerId`` . This is the string that will be used by your user when they log in to your SFTP
          server. This user name is a minimum of 3 and a maximum of 32 characters long. The following are
