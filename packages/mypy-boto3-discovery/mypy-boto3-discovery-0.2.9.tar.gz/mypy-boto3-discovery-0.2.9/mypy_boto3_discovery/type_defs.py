...
_ClientDeleteTagstagsTypeDef = TypedDict("_ClientDeleteTagstagsTypeDef", {"key": str, "value": str})
...
class ClientDescribeAgentsResponseagentsInfoTypeDef(_ClientDescribeAgentsResponseagentsInfoTypeDef):
    ...
    - **hostName** *(string) --*
      ...
      The name of the host where the agent or connector resides. The host can be a server or virtual
      machine.
...
_ClientDescribeAgentsfiltersTypeDef = TypedDict(
    "_ClientDescribeAgentsfiltersTypeDef", {"name": str, "values": List[str], "condition": str}
)
...
class ClientDescribeContinuousExportsResponsedescriptionsTypeDef(
    ...
    - **status** *(string) --*
      ...
      * INACTIVE - the continuous export has been stopped. Data is no longer being exported to the
      customer bucket.
    ...
    - **statusDetail** *(string) --*
      ...
      * ACCESS_DENIED - You don’t have permission to start Data Exploration in Amazon Athena.
      Contact your AWS administrator for help. For more information, see `Setting Up AWS Application
      Discovery Service
      <http://docs.aws.amazon.com/application-discovery/latest/userguide/setting-up.html>`__ in the
      ...
      * DELIVERY_STREAM_LIMIT_FAILURE - You reached the limit for Amazon Kinesis Data Firehose
      delivery streams. Reduce the number of streams or request a limit increase and try again. For
      more information, see `Kinesis Data Streams Limits
      <http://docs.aws.amazon.com/streams/latest/dev/service-sizes-and-limits.html>`__ in the Amazon
      Kinesis Data Streams Developer Guide.

      * FIREHOSE_ROLE_MISSING - The Data Exploration feature is in an error state because your IAM
      User is missing the AWSApplicationDiscoveryServiceFirehose role. Turn on Data Exploration in
      Amazon Athena and try again. For more information, see `Step 3\\: Provide Application
      Discovery Service Access to Non-Administrator Users by Attaching Policies
      ...
      * FIREHOSE_STREAM_DOES_NOT_EXIST - The Data Exploration feature is in an error state because
      your IAM User is missing one or more of the Kinesis data delivery streams.

      * INTERNAL_FAILURE - The Data Exploration feature is in an error state because of an internal
      failure. Try again later. If this problem persists, contact AWS Support.

      * S3_BUCKET_LIMIT_FAILURE - You reached the limit for Amazon S3 buckets. Reduce the number of
      Amazon S3 buckets or request a limit increase and try again. For more information, see `Bucket
      Restrictions and Limitations
      <http://docs.aws.amazon.com/AmazonS3/latest/dev/BucketRestrictions.html>`__ in the Amazon
      Simple Storage Service Developer Guide.

      * S3_NOT_SIGNED_UP - Your account is not signed up for the Amazon S3 service. You must sign up
...
class ClientDescribeExportConfigurationsResponseexportsInfoTypeDef(
    ...
    - **isTruncated** *(boolean) --*
      ...
      exported data is incomplete for the requested time range. To address this, select a smaller
      time range for the export by using ``startDate`` and ``endDate`` .
...
_ClientDescribeExportTasksfiltersTypeDef = TypedDict(
    "_ClientDescribeExportTasksfiltersTypeDef", {"name": str, "values": List[str], "condition": str}
)
...
class ClientDescribeImportTasksResponsetasksTypeDef(_ClientDescribeImportTasksResponsetasksTypeDef):
    ...
    - **name** *(string) --*
      ...
      A descriptive name for an import task. You can use this name to filter future requests related
      to this import task, such as identifying applications and servers that were included in this
      import task. We recommend that you use a meaningful name for each import task.
    ...
    - **status** *(string) --*
      ...
      The status of the import task. An import can have the status of ``IMPORT_COMPLETE`` and still
      have some records fail to import from the overall request. More information can be found in
      the downloadable archive defined in the ``errorsAndFailedEntriesZip`` field, or in the
    ...
    - **errorsAndFailedEntriesZip** *(string) --*
      ...
      This field also includes authorization information so you can confirm the authenticity of the
      compressed archive before you download it.

      If some records failed to be imported we recommend that you correct the records in the failed
      entries file and then imports that failed entries file. This prevents you from having to
      correct and update the larger original file and attempt importing it again.
    """
...
_ClientDescribeImportTasksfiltersTypeDef = TypedDict(
    "_ClientDescribeImportTasksfiltersTypeDef", {"name": str, "values": List[str]}, total=False
)
...
_ClientListConfigurationsfiltersTypeDef = TypedDict(
    "_ClientListConfigurationsfiltersTypeDef", {"name": str, "values": List[str], "condition": str}
)
...
_ClientStartExportTaskfiltersTypeDef = TypedDict(
    "_ClientStartExportTaskfiltersTypeDef", {"name": str, "values": List[str], "condition": str}
)
...
class ClientStartImportTaskResponsetaskTypeDef(_ClientStartImportTaskResponsetaskTypeDef):
    ...
    An array of information related to the import task request including status information, times,
...
_DescribeAgentsPaginatefiltersTypeDef = TypedDict(
    "_DescribeAgentsPaginatefiltersTypeDef", {"name": str, "values": List[str], "condition": str}
)
...
class DescribeExportTasksPaginatefiltersTypeDef(_DescribeExportTasksPaginatefiltersTypeDef):
    """
