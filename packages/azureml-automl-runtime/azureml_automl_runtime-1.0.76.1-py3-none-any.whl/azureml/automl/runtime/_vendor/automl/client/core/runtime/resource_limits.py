...
class SafeEnforceLimits:
    ...
    def wrap(self, func: 'Callable[..., T]') -> 'Callable[..., Tuple[Optional[T], Optional[BaseException], float]]':
        """
        ...
        def wrapped(*args: Any, **kwargs: Any) -> Tuple[Optional[T], Optional[BaseException], float]:
            ...
            if self.limiter is not None:
                result, exit_status, wall_clock_time = self.limiter.execute(func, *args, **kwargs)
            else:
                start = time.time()
                try:
