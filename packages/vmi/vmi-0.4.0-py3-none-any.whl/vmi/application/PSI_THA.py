...
def update_cup_orientation():
    ...
    cup_cs.setAxis(0, -original_LR * cup_cs.axis(0))
...
def update_xray_pelvis():
    cs = pelvis_view.cameraCS()
    pelvis_left_top = pelvis_view.pickPt_FocalPlane([0, 0])
    pelvis_right_top = pelvis_view.pickPt_FocalPlane([pelvis_view.width(), 0])
    pelvis_left_bottom = pelvis_view.pickPt_FocalPlane([0, pelvis_view.height()])

    xray_left_top = xray_view.pickPt_FocalPlane([0, 0])
    xray_right_top = xray_view.pickPt_FocalPlane([xray_view.width(), 0])
    xray_left_bottom = xray_view.pickPt_FocalPlane([0, xray_view.height()])

    global xray_cs
    xray_cs = xray_view.cameraCS()
    xray_cs.setAxis(0, xray_cs.axis(0) * np.linalg.norm(xray_left_top - xray_right_top) /
                    np.linalg.norm(pelvis_left_top - pelvis_right_top))
    xray_cs.setAxis(1, xray_cs.axis(1) * np.linalg.norm(xray_left_top - xray_left_bottom) /
    ...
    ctray_size[2] = vmi.imSize_Vt(voi_volume.data(), cs.axis(2))

    pt = vmi.imCenter(voi_volume.data()) - 0.5 * ctray_size[2] * cs.axis(2)
    ctray_cs.setOrigin(vmi.ptOnPlane(cs.origin(), pt, cs.axis(2)))
...
def update_ctray():
    image_blend = vmi.imReslice_Blend(voi_volume.data(), cs=ctray_cs, size=ctray_size,
...
def update_cup_camera_sagittal():
    ...
    cs.orthogonalize([0])
    cs.setOrigin(cup_cs.origin() - 2.5 * cup_radius * cs.axis(0) - 2.5 * cup_radius * cs.axis(1))
...
def update_guide_path():
    ...
    for cs in css:
        cs.setOrigin(cs.mpt([0, 0, cup_radius]))
...
def LeftButtonPressRelease(**kwargs):
    global spcut_radius, pelvis_cs, cup_radius, cup_cs, cup_RA, cup_RI, hole_radius, xray_image, bone_value
    ...
    elif kwargs['picked'] is export_pelvis_box:
        p = vmi.askDirectory('导出骨盆')
        if p is not None:
            export_pelvis_box.draw_text('请耐心等待')
            # p = pathlib.Path(p)

            # bone_range = [bone_value - 10, 4000]

            # p_mask = vmi.imExtract_SizeRank(voi_volume.data(), [200, 4000])
            # p_mask = vmi.imThreshold(p_mask, [1, 1], replace_in=1, replace_out=0)
            # p_image = vmi.imMask(voi_image, p_mask, -1100)
            # p_pd = vmi.imIsosurface(p_mask, 1)
            # p_pd = vmi.pdSmooth_WindowedSinc(p_pd, 50)

            # f_mask = vmi.imThreshold(voi_volume.data(), [bone_value, 4000], replace_in=0, replace_out=1, mask=True)
            # f_image = vmi.imMask(voi_image, f_mask, -1100, True)
            # f_mask = vmi.imExtract_SizeRank(f_image, bone_range)
            # voi_volume.setData(f_image)
            #
            # f1_mask = vmi.imThreshold(f_mask, [1, 1], replace_in=1, replace_out=0)
            # f1_image = vmi.imMask(voi_image, f1_mask, -1100)
            # f1_pd = vmi.imIsosurface(vmi.imResample_Isotropic(f1_image), bone_value)
            # print(f1_pd)
            #
            # f2_mask = vmi.imThreshold(f_mask, [2, 2], replace_in=1, replace_out=0)
            # f2_image = vmi.imMask(voi_image, f2_mask, -1100)
            # f2_pd = vmi.imIsosurface(vmi.imResample_Isotropic(f2_image), bone_value)
            # print(f2_pd)
            #
            # LR = f1_pd.GetCenter()[0] > f2_pd.GetCenter()[0]

            # vmi.imSave_NIFTI(p_image, str(p / (patient_name + ' Pelvis.nii')))
            # vmi.pdSaveFile_STL(p_pd, str(p / (patient_name + ' Pelvis.stl')))

            # vmi.imSave_NIFTI(f1_image, str(p / (patient_name + ' Femur {}.nii'.format('L' if LR else 'R'))))
            # vmi.pdSaveFile_STL(f1_pd, str(p / (patient_name + ' Femur {}.stl'.format('L' if LR else 'R'))))
            #
            # vmi.imSave_NIFTI(f2_image, str(p / (patient_name + ' Femur {}.nii'.format('R' if LR else 'L'))))
            # vmi.pdSaveFile_STL(f2_pd, str(p / (patient_name + ' Femur {}.stl'.format('R' if LR else 'L'))))

            export_pelvis_box.draw_text('导出骨盆')
...
if __name__ == '__main__':
    ...
    main.excludeKeys += ['main', 'i', 'box', 'prop', 'original_slice_menu', 'voi_image']

    if vmi.askYesNo('打开存档 ?'):
        ...
        export_pelvis_box = vmi.TextBox(voi_view, text='导出骨盆', pickable=True,
                                        size=[0.24, 0.04], pos=[1, 0.1], anchor=[1, 0])
        export_pelvis_box.mouse['LeftButton']['PressRelease'] = [LeftButtonPressRelease]
        ...
        guide_radius_boxs = [vmi.TextBox(guide_view, text='{} mm'.format(guide_radiuss[i]), pickable=True,
                                         size=[0.06, 0.04], pos=[0.06, 0.12 + i * 0.04], anchor=[0, 0]) for i in
                             range(4)]
        guide_angle_boxs = [vmi.TextBox(guide_view, text='{}°'.format(guide_angles[i]), pickable=True,
                                        size=[0.06, 0.04], pos=[0.12, 0.12 + i * 0.04], anchor=[0, 0]) for i in
                            range(4)]
        ...
        for v in [original_view, voi_view, *spcut_y_views, *spcut_z_views, pelvis_view, xray_view, ctray_view,
                  guide_view]:
            v.setEnabled(False)
        original_view.setEnabled(True)

    # 视图布局
    for v in [original_view, voi_view, *spcut_y_views, *spcut_z_views, pelvis_view, xray_view, ctray_view, guide_view]:
    ...
    original_view.setParent(main._ScrollAreaWidget)
    ...
    # main.layout.addWidget(xray_view, 0, 5, 3, 1)
    # main.layout.addWidget(ctray_view, 0, 6, 3, 1)
