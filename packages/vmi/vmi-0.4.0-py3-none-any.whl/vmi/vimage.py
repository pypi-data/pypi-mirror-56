...
def imVTK_Array(ary: np.ndarray, origin: Union[np.ndarray, List[float]],
    ...
    image = imVTK_SITK(sitk.GetImageFromArray(ary), data_scalar_type=types[str(ary.dtype)])
...
def imSave_NIFTI(image: vtk.vtkImageData, file=None) -> None:
    """保存vtkImageData到NIFTII文件"""
    if file is None:
        file = vmi.askSaveFile('nii', '*.nii', '导出 (Export) NIFTII')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.nii'
...
def pdOpenFile_NIFTI(file=None) -> Optional[vtk.vtkImageData]:
    """读取NIFTII文件返回vtkImageData，读取失败则返回None"""
    if file is None:
        file = vmi.askOpenFile('*.nii', '导入 (Import) NIFTII')
        if file is None:
            return
    with tempfile.TemporaryDirectory() as p:
        p = pathlib.Path(p) / '.nii'
