...
def output_upload():
    ...
    if not file_db.exists(image_hash):
        file_db.put(image_bytes)

    # 数据项，引用图像文件特征码
    data_dict = {**series_info, 'seriesImage': image_hash['md5']}
    data_hash = {'md5': hashlib.md5(pickle.dumps(data_dict)).hexdigest()}

    # 上传数据项，避免重复上传完全一样的数据项
    if data_db.find_one(data_hash) is None:
        data_db.insert_one(data_dict)

    if vmi.askYesNo('上传成功 md5: {} 复制到剪贴板？'.format(data_hash['md5'])):
        vmi.app.clipboard().setText(data_hash['md5'])
