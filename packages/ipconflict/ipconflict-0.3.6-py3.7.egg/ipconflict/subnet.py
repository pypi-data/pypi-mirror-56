...
def print_results(subnet_a, subnet_b, overlapping_ips, ip_only=False, print_conflicts=False):
    if not ip_only:
...
def check_conflicts(subnets, quiet=True):
    conflicts = []
    rtree = radix.Radix()
    iterator = tqdm(enumerate(subnets), unit='subnet', total=len(subnets), disable=quiet)
    for idx, subnet_a in iterator:
        ...
        for cidr in set_a.iter_cidrs():
            ...
            if (rtree.search_covered(cidr) or rtree.search_covering(cidr)) and not matched:
                for subnet_b in subnets[:idx]:
                    ...
                    if overlapping_ips:
                        iterator.write(u'conflict found: {} <-> {}'.format(subnet_a, subnet_b))
