...
def getvar(variable_name):
    try:
        confing = load_project_config()
        return config.get(SECTION_VARIABLES, variable_name)
    except configparser.NoSectionError:
        raise KeyError(f'no section {SECTION_VARIABLES} found in your configs')
    except configparser.NoOptionError:
        raise KeyError(f'no variable {variable_name} found in your configs')
...
def prefix_path(path, default_prefix=None):
    ...
    """
    warnings.warn('please use the getvar() method for more flexible variables.'
...
def _notify(message, subject):
    ...
    if cfg[SECTION_MAIL].getboolean('authenticate', fallback=False):
        ...
        # optional
        usernam = cfg[SECTION_MAIL]['username']
        password = cfg[SECTION_MAIL]['password']
