from . import fetch_lyrics, process_lyrics, write_lyrics_to_file

def main(api_key, artist_search_term, dropna=True, check_lyrics_state=True, 
	save_raw_df_path='', save_processed_df_path='', save_processed_hdf_key='default', 
	save_txt_path='lyrics.txt'):
	...
	wirte_lyrics_to_file.main(df=df, save_path=save_txt_path)
...
if __name__ == '__main__':
	...
	parser.add_argument('--save_processed_hdf_key', default='default', type=bool,
						help='where to save the text file with everything')
	parser.add_argument('--save_txt_path', default='./lyrics.txt', type=bool,
						help='where to save the text file with everything')
	args = parser.parse_args()
	main(api_key=args.api_key, artist_search_term=args.artist_search_term, dropna=args.dropna, 
		check_lyrics_state=args.check_lyrics_state, save_raw_df_path=args.save_raw_df_path, 
		save_processed_df_path=args.save_processed_df_path, 
		save_processed_hdf_key=args.save_processed_hdf_key, save_txt_path=args.save_txt_path)