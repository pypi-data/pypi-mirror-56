...
class _SCMCubeIntegrationTester(object):
    ...
    def test_get_scm_timeseries_cubes(self, test_cube, regions_to_get):
        ...
        for label, weights in mocked_weights.items():
            ...
            ] = "{} (more info at gitlab.com/znicholls/netcdf-scm)".format(
...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_get_lat_lon_shape_cube_nlat_nlon_coords(
        self, test_cube, test_cmip6_output_hfds_nlon_nlat_coords_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_nlon_nlat_coords_file)
        caplog.set_level(logging.DEBUG, logger="netcdf_scm.iris_cube_wrappers")
        assert test_cube.lat_lon_shape == (384, 320)
        message_idx = caplog.messages.index(
            "co-ordinates make sense once we ignore `['nlat', 'nlon']`"
        )
        assert caplog.records[message_idx].levelno == logging.DEBUG

    def test_get_lat_lon_shape_cube_i_j_coords(
        self, test_cube, test_cmip6_output_hfds_i_j_coords_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_i_j_coords_file)
        caplog.set_level(logging.DEBUG, logger="netcdf_scm.iris_cube_wrappers")
        assert test_cube.lat_lon_shape == (291, 360)
        message_idx = caplog.messages.index(
            "co-ordinates make sense once we ignore `['j', 'i']`"
        )
        assert caplog.records[message_idx].levelno == logging.DEBUG

    def test_get_lat_lon_shape_cube_vertex_coords(
        self, test_cube, test_cmip6_output_hfds_vertex_coords_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_vertex_coords_file)
        caplog.set_level(logging.DEBUG, logger="netcdf_scm.iris_cube_wrappers")
        assert test_cube.lat_lon_shape == (291, 360)
        message_idx = caplog.messages.index(
            "co-ordinates make sense once we ignore `['j', 'i']` and single "
            "value co-ordinates `['depth']`"
        )
        assert caplog.records[message_idx].levelno == logging.DEBUG

    def test_get_lat_lon_shape_cube_sftlf_gn(
        self, test_cube, test_cmip6_output_sftlf_gn_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_sftlf_gn_file)
        assert test_cube.lat_lon_shape == (90, 144)
        assert not any(
            [
    ...
    def test_load_hfds_data_with_nlon_nlat_coords(
        self, test_cube, test_cmip6_output_hfds_nlon_nlat_coords_file, caplog
    ):
        test_cube.load_data_from_path(test_cmip6_output_hfds_nlon_nlat_coords_file)
        ...
        assert obs_time_points[0] == cftime.DatetimeNoLeap(
            2021, 7, 15, 12, 0, 0, 0, 6, 196
        )
        assert obs_time_points[-1] == cftime.DatetimeNoLeap(
            2021, 9, 15, 0, 0, 0, 0, 5, 258
        )
        ...
        np.testing.assert_allclose(
            ts.filter(region="World|El Nino N3.4", month=8).values.squeeze(),
            52.35971841,
            rtol=0.01,
        )
