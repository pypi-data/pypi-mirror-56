...
class SCMCube:  # pylint:disable=too-many-public-methods
    ...
    def lat_lon_shape(self):
        ...
        ------
        AssertionError
            No lat lon slice can be deduced (if this happens, please raise an issue at
            https://gitlab.com/znicholls/netcdf-scm/issues so we can address your use
            case).
        """
        return self._get_cube_lat_lon_shape()

    def _get_cube_lat_lon_shape(self):
        # # we can't just do this as it doesn't work for non-orthogonal co-ordinates...
        # lat_lon_slice = next(self.cube.slices([self.lat_name, self.lon_name]))

        non_lat_lon_dims = self._get_non_lat_lon_dims()
        lat_lon_slice = next(self.cube.slices_over(non_lat_lon_dims))

        if not lat_lon_slice.shape:  # pragma: no cover # emergency valve
            raise AssertionError(
                "No lat lon slice data found, please raise an issue at "
                "https://gitlab.com/znicholls/netcdf-scm/issues"
            )

        return lat_lon_slice.shape

    def _get_non_lat_lon_dims(self):
        coords_match_shape = len(self.cube.coords()) == len(self.cube.shape)
        if coords_match_shape:
            # we can happily assume that no funny business is required
            return [
        ...
        # try and make sense of what we have
        # firstly try dropping out any `cell index` co-ordinates
        cell_index_coords = [
            c.var_name
            for c in self.cube.coords()
            if c.standard_name is None and c.long_name.startswith("cell index")
        ]
        coords_stripped = [
            c for c in self.cube.coords() if c.var_name not in cell_index_coords
        ]
        coords_stripped_match_shape = len(coords_stripped) == len(self.cube.shape)
        if coords_stripped_match_shape:
            logger.debug(
                "co-ordinates make sense once we ignore `%s`", cell_index_coords,
            )
            return [
                c.standard_name if c.standard_name is not None else c.long_name
                for c in coords_stripped
                if c.standard_name not in [self.lat_name, self.lon_name]
            ]

        # try just looking at the dimensions which have more than one value i.e.
        # ignore scalar dimensions
        multi_value_dims = [c for c in coords_stripped if c.points.shape != (1,)]

        if len(multi_value_dims) == len(  # pragma: no cover # emergency valve
            self.cube.shape
        ):
            single_value_coords = [
                c.var_name for c in coords_stripped if c.points.shape == (1,)
            ]
            logger.debug(
                "co-ordinates make sense once we ignore `%s` and "
                "single value co-ordinates `%s`",
                cell_index_coords,
                single_value_coords,
            )
            return [
                c.standard_name if c.standard_name is not None else c.long_name
                for c in multi_value_dims
                if c.standard_name not in [self.lat_name, self.lon_name]
            ]

        # throw a warning as we don't know what's going on here
        logger.warning(
            "we don't know what we're looking at, `_get_non_lat_lon_dims` "
            "might not work"
        )
    ...
    def load_data_in_directory(self, directory=None, process_warnings=True):
        ...
        ------
        ValueError
            If the files in the directory are not from the same run (i.e. their
            filenames are not identical except for the timestamp) or if the files
            don't form a continuous timeseries.
        """
    ...
    def get_metadata_cube(self, metadata_variable, cube=None):
        ...
        cube : :obj:`SCMCube`
            Optionally, pass in an already loaded metadata cube to link it to
            currently loaded cube.

        Returns
        -------
        type(self)
            instance of self which has been loaded from the file containing the
            metadata variable of interest.
    ...
    def get_scm_timeseries_cubes(
        ...
        def crunch_timeseries(region, weights):
            ...
            if region == "World" and self.netcdf_scm_realm in ("land", "ocean"):
                ...
                # https://gitlab.com/znicholls/netcdf-scm/issues/103
    ...
    def _add_metadata_to_region_timeseries_cube(self, scmcube, region):
        ...
        ] = "{} (more info at gitlab.com/znicholls/netcdf-scm)".format(__version__)
    ...
    def get_area_weights(self, areacell_scmcube=None):
        ...
        areacell_scmcube : :obj:`ScmCube`
            :obj:`ScmCube` containing areacell data. If ``None``, we calculate the
            weights using iris.
