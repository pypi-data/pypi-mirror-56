...
def test_wrangling_blend_models(tmpdir, caplog, test_cmip6_crunch_output):
    ...
    assert len(glob(join(output_dir, "*.mat"))) > 100
...
def test_wrangling_blended_models_default_drs_error(tmpdir, test_cmip6_crunch_output):
    ...
    assert str(result.exception) == (
        ...
        "gitlab.com/znicholls/netcdf-scm/ with your use case if you need this feature."
    )


def test_wrangling_drs_replication(tmpdir, test_cmip6_crunch_output):
    input_dir = join(test_cmip6_crunch_output, "Lmon/CMIP6/CMIP/CNRM-CERFACS")
    output_dir = str(tmpdir)
...
def test_wrangling_annual_mean_file(tmpdir, test_data_root_dir):
    ...
    assert (
        ...
        "gitlab.com/znicholls/netcdf-scm/issues to discuss how to handle "
...
def test_wrangling_units_specs(
    ...
    input_dir = join(test_cmip6_crunch_output, "Omon/CMIP6/CMIP/CCCma")
    output_dir = str(tmpdir)
...
def test_wrangling_mag_file(tmpdir, test_cmip6_crunch_output, caplog):
    ...
    input_dir = join(
        test_cmip6_crunch_output, "Lmon/CMIP6/ScenarioMIP/IPSL/IPSL-CM6A-LR"
    )
    output_dir = str(tmpdir)
...
def test_wrangling_in_file(tmpdir, test_cmip6_crunch_output, caplog):
    ...
    input_dir = join(
        test_cmip6_crunch_output, "Amon/CMIP6/CMIP/IPSL/IPSL-CM6A-LR/historical"
    )
    output_dir = str(tmpdir)
...
def test_wrangling_in_file_operations(
    ...
    input_dir = join(
        test_cmip6_crunch_output, "Amon/CMIP6/CMIP/IPSL/IPSL-CM6A-LR/piControl"
    )
    output_dir = str(tmpdir)
...
def test_wrangling_with_operation_single_timepoint(
    ...
    input_dir = join(
        ...
        "Amon/CMIP6/CMIP/NCAR/CESM2/historical/r7i1p1f1/Amon/tas",
    )
    output_dir = str(tmpdir)
