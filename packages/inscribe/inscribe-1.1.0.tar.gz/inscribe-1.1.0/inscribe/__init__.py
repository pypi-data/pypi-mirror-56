...
class Client(object):
    ...
    def create_customer(self, customer_name: str) -> dict:
        ...
        :param customer_name: A unique name for the customer
        """
        url = "/customers"
    ...
    def update_customer(self, customer_id: str, customer_name: str, notes: str = None, approved: bool = None) -> dict:
        """
    ...
    def get_customers(self, page_number: int = 1, limit_per_page: int = None, search_query: str = None,
         is_approved: bool = None, order_by: str = None, start_date: str = None, end_date: str = None, customer_owner_uuid: str = None) -> dict:
        """
        ...
        :param customer_owner_uuid: Returns only customers owned by this user UUID. This user must be in your organization and you must have access to their customers. 

        """
        url = f"/customers?customersPageNumber={page_number}"
        if limit_per_page is not None:
            url = url + f"&customerLimitPerPage={limit_per_page}"
        if search_query is not None:
            url = url + f"&customersSearchQuery={search_query}"
        if is_approved is not None:
            url = url + f"&isApproved={is_approved}"
        if order_by is not None:
            url = url + f"&orderBy={order_by}"
        if start_date is not None:
            url = url + f"&startDate={start_date}"
        if end_date is not None:
            url = url + f"&endDate={end_date}"
        if customer_owner_uuid is not None:
            url = url + f"&customerOwner={customer_owner_uuid}"
        return self._get(url)
    ...
    def retrieve_document_results(self, customer_id: str, document_id: str) -> dict:
        """
        ...
        :param customer_id: id of the customer you want to retrieve document results.
        :param document_id: id of the document you want to retrieve results.
        """
    ...
    def _process_pusher_message(
        ...
        success_callback(self.retrieve_document_results(json_data["customer_id"], json_data["document_id"]))
