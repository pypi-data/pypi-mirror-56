...
def mmseqs_parser_7flds(alignFile, querySeqLenFile, targetSeqLenFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    ...
    """
    if debug:
        print("mmseqs_parser_7flds :: START")
        print("BLAST-formatted file: {:s}".format(alignFile))
        print("Query proteome: {:s}".format(querySeqLenFile))
        print("Target proteome: {:s}".format(targetSeqLenFile))
