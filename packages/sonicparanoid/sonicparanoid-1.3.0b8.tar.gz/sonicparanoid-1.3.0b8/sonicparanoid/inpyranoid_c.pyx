...
def rewrite_clusters_c(inTbl: str, mergedClstrs: List[str], mergedIds: List[str], skipDict: Dict[str, None], debug: bool=False) -> str:
    ...
    if debug:
        ...
        print("Merged Clusters:\t{:d}".format(len(mergedClstrs)))
        print("Merged IDs:\t{:d}".format(len(mergedIds)))
        print("IDs to be skipped:\t{:d}".format(len(skipDict)))
    ...
    if not os.path.isdir(tblDir):
        sys.stderr.write("\nERROR: the directory with ortholog tables\n{:s}\nwas not found.".format(tblDir))
        sys.exit(-2)

    # define the output paths
    outTbl: str = os.path.join(tblDir, "table.{:s}".format(pairName))
    tmpTbl: str = os.path.join(tblDir, "table.{:s}.tmp".format(pairName))
    ...
    os.remove(tmpTbl)
    # set final path
    finalTblPath: str = os.path.join(tblDir, "table.{:s}".format(pairName))
    ...
    while True:
        ...
        rdCnt += 1
        ofd.write("{:d}\t{:s}".format(rdCnt, line.decode()))
...
def write_sql_c(inTbl: str, outDir: str=os.getcwd(), debug: bool=False) -> str:
    ...
    while True:
        ...
        outSqlLnStart = "{:s}\t{:s}".format(clstrId, flds[1].decode())
