...
class ServerJsonDataSource(Source):
    ...
    def get_data(self):
        ...
        if request.status_code is not 200:
            raise DataSourceException('Could not reach the data. HTTP response code: {}, response: {}'.format(str(request.status_code), str(request.content)))
        else:
            return request.json()
...
class ESFileDumpSource(Source):
    ...
    def get_data(self):
        with io.open(self.file_path, FileJsonDataSource.FILE_OPEN_METHOD, encoding=self.encoding_str) as data_file:
            return list(map(itemgetter('_source'), filter(lambda x: x['_type'] == 'mention', map(json.loads, data_file))))
