...
#########################################################
def plotSignatureBasedFigures(mutationType,subsSignature2PropertiesListDict,sample2Signature2NumberofMutationsDict,outputDir,jobname,isFigureAugmentation,numberofSimulations,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus):
    if (occupancy_type==NUCLEOSOMEOCCUPANCY):
    ...
    for signature in subsSignature2PropertiesListDict:
        #[cutoff numberofMutations  averageProbability]
        signatureBasedNumberofMutations = subsSignature2PropertiesListDict[signature][1]
...
def occupancyAverageSignalFigures(outputDir,jobname,figureAugmentation,numberofSimulations,sample_based,mutationTypes,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus):
    ...
    if checkValidness(SIGNATUREBASED,outputDir,jobname,occupancy_type):
        if (SBS96 in mutationTypes):
            #Subs Signatures
            plotSignatureBasedFigures(SBS96,subsSignature2PropertiesListDict,sample2SubsSignature2NumberofMutationsDict,outputDir,jobname,isFigureAugmentation,numberofSimulations,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus)
        if (ID in mutationTypes):
            #Indels Signatures
            plotSignatureBasedFigures(ID,indelsSignature2PropertiesListDict,sample2IndelsSignature2NumberofMutationsDict,outputDir,jobname,isFigureAugmentation,numberofSimulations,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus)
        if (DBS in mutationTypes):
            # Dinucs Signatures
            plotSignatureBasedFigures(DBS,dinucsSignature2PropertiesListDict,sample2DinucsSignature2NumberofMutationsDict,outputDir,jobname,isFigureAugmentation,numberofSimulations,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus)
    #############################################################################################################################################
