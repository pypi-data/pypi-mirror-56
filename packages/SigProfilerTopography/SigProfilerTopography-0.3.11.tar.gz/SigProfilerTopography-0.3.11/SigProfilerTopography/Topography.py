...
def check_download_chrbased_npy_nuclesome_files(nucleosome_file,chromNamesList):
    ...
    # print('chrombased_npy_path:%s' %chrombased_npy_path)

    if (nucleosome_file==DEFAULT_NUCLEOSOME_OCCUPANCY_FILE1):
        cell_line='Gm12878'
    elif (nucleosome_file==DEFAULT_NUCLEOSOME_OCCUPANCY_FILE2):
        cell_line='K562'

    for chrLong in chromNamesList:
        chrbased_npy_array_path= os.path.join(chrombased_npy_path,chrLong + '_signal_wgEncodeSydhNsome'+ cell_line + 'Sig.npy')
        if not os.path.exists(chrbased_npy_array_path):
            try:
                # print("bash -c '" + 'wget -r -l1 -c -nc --no-parent -nd -P ' + chrombased_npy_path + ' ftp://alexandrovlab-ftp.ucsd.edu/pub/tools/SigProfilerTopography/lib/nucleosome/chrbased/' + chrLong + '_signal_wgEncodeSydhNsome' + cell_line + 'Sig.npy' + "'")
                print('Downloading %s_signal_wgEncodeSydhNsome_%sSig.npy under %s' %(chrLong,cell_line,chrbased_npy_array_path))
                os.system("bash -c '" + 'wget -r -l1 -c -nc --no-parent -nd -P ' + chrombased_npy_path + ' ftp://alexandrovlab-ftp.ucsd.edu/pub/tools/SigProfilerTopography/lib/nucleosome/chrbased/' + chrLong + '_signal_wgEncodeSydhNsome' + cell_line + 'Sig.npy' + "'")
            except:
...
def runReplicationTimeAnalysis(genome,outputDir,jobname,numofSimulations,sample_based,replicationTimeFilename,chromSizesDict,chromNamesList,computation_type,subsSignature2PropertiesListDict,indelsSignature2PropertiesListDict,dinucsSignature2PropertiesListDict):
    ...
    replicationTimeAnalysis(computation_type,sample_based,genome,chromSizesDict,chromNamesList,outputDir,jobname,numofSimulations,replicationTimeFilename,subsSignature2PropertiesListDict,indelsSignature2PropertiesListDict,dinucsSignature2PropertiesListDict)
    ###############################################
...
def runAnalyses(genome,
    ...
    if (full_mode==True):
        ...
        if sample_based:
            ...
            appendDictionaryUnderDataDirectory(mutationType2NumberofMutationsDict, outputDir, jobname,
                                               MutationType2NumberofMutatiosDictFilename)
        ##################################################################################
    ...
    #For GM12878 and K562 cell lines nucleosome occupancy we will be using chromosome based npy arrays
...
if __name__== "__main__":
    ...
    runAnalyses(genome, inputDir, outputDir, jobname, numberofSimulations,
                          ...
                          id_probabilities_file_path=id_probabilities,
                           ...
                           computation_type='COMPUTATION_CHROMOSOMES_SEQUENTIAL_ALL_SIMULATIONS_PARALLEL',
                           epigenomics=False, nucleosome=True, replication_time=False, strand_bias=False,
                           processivity=False, sample_based=False, new_simulations_enforced=False, full_mode=False)
##############################################################