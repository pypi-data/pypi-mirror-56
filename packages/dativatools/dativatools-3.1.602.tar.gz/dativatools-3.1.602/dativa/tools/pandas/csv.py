...
class CSVHandler:
    ...
    def _get_file(self, file):
        return path.join(self.base_path, file)
    ...
    def _get_csv_bytes(self, df):
        if self.compression not in ['gzip', 'infer']:
            raise ValueError("Only gzip compression is supported for writing dataframes")
    ...
    def save_df(self, df, file, **kwargs):
        ...
        bytes_to_write = self._get_csv_bytes(df)
        if self.aes_key:
            if not AES:
