...
def as_numpy_dtype(dtype):
  """Returns a `onp.dtype` based on this `dtype`."""
...
def is_bool(dtype):
  ...
  # We use `kind` because:
  # np.issubdtype(np.uint8, onp.bool) == True.
  return onp.dtype(dtype).kind == 'b'
...
def is_complex(dtype):
  ...
  return np.issubdtype(onp.dtype(dtype), np.complex)
...
def is_floating(dtype):
  ...
  return np.issubdtype(onp.dtype(dtype), np.floating)
...
def is_integer(dtype):
  ...
  return np.issubdtype(onp.dtype(dtype), np.integer)
...
def size(dtype):
  ...
  if hasattr(dtype, 'size') and hasattr(dtype, 'as_numpy_dtype'):
    return dtype.size
  return onp.dtype(dtype).itemsize
