...
from tensorflow_probability.python.internal.backend.numpy import ops as tensor_shape
...
from tensorflow_probability.python.internal.backend.numpy import v1 as check_ops
...
from tensorflow_probability.python.internal.backend.numpy import linear_operator_algebra
...
class LinearOperator(module.Module):
  ...
  * Operators that take advantage of special structure, while providing a
  ...
  ```
  _ops.TensorShape(operator.shape) = [B1,...,Bb] + [M, N],  b >= 0,
  _ops.TensorShape(x.shape) =   [B1,...,Bb] + [N, R]
  ```
  ...
  ```
  _ops.TensorShape(operator.shape) = [B1,...,Bb] + [M, N],  b >= 0,
  _ops.TensorShape(rhs.shape) =   [B1,...,Bb] + [M, R]
  ```
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def shape(self):
    ...
    `_ops.TensorShape(A.shape) = [B1,...,Bb, M, N]`, then this returns
    `TensorShape([B1,...,Bb, M, N])`, equivalent to `_ops.TensorShape(A.shape)`.
  ...
  def shape_tensor(self, name="shape_tensor"):
    ...
    If this operator acts like the batch matrix `A` with
    `_ops.TensorShape(A.shape) = [B1,...,Bb, M, N]`, then this returns a `Tensor` holding
    ...
    with self._name_scope(name):
      ...
      if _ops.TensorShape(self.shape).is_fully_defined():
        return linear_operator_util.shape_tensor(_ops.TensorShape(self.shape).as_list())
      else:
  ...
  def batch_shape(self):
    ...
    If this operator acts like the batch matrix `A` with
    `_ops.TensorShape(A.shape) = [B1,...,Bb, M, N]`, then this returns
    `TensorShape([B1,...,Bb])`, equivalent to `_ops.TensorShape(A.shape)[:-2]`

    Returns:
      `TensorShape`, statically determined, may be undefined.
    """
    # Derived classes get this "for free" once .shape is implemented.
    return _ops.TensorShape(self.shape)[:-2]
  ...
  def tensor_rank(self, name="tensor_rank"):
    ...
    # Derived classes get this "for free" once .shape() is implemented.
    with self._name_scope(name):
      return _ops.TensorShape(self.shape).ndims
  ...
  def domain_dimension(self):
    ...
    If this operator acts like the batch matrix `A` with
    `_ops.TensorShape(A.shape) = [B1,...,Bb, M, N]`, then this returns `N`.

    Returns:
      `Dimension` object.
    """
    # Derived classes get this "for free" once .shape is implemented.
    if _ops.TensorShape(self.shape).rank is None:
      return tensor_shape.Dimension(None)
    else:
      return _ops.TensorShape(self.shape).dims[-1]
  ...
  def range_dimension(self):
    ...
    """
    # Derived classes get this "for free" once .shape is implemented.
    if _ops.TensorShape(self.shape).dims:
      return _ops.TensorShape(self.shape).dims[-2]
    else:
      return tensor_shape.Dimension(None)
  ...
  def matmul(self, x, adjoint=False, adjoint_arg=False, name="matmul"):
    ...
    with self._name_scope(name):
      ...
      tensor_shape.dimension_at_index(
          _ops.TensorShape(self.shape), self_dim).assert_is_compatible_with(
  ...
  def matvec(self, x, adjoint=False, name="matvec"):
    ...
    with self._name_scope(name):
      ...
      self_dim = -2 if adjoint else -1
      tensor_shape.dimension_at_index(
          _ops.TensorShape(self.shape), self_dim).assert_is_compatible_with(_ops.TensorShape(x.shape)[-1])
  ...
  def solvevec(self, rhs, adjoint=False, name="solve"):
    ...
    with self._name_scope(name):
      ...
      self_dim = -1 if adjoint else -2
      tensor_shape.dimension_at_index(
          _ops.TensorShape(self.shape), self_dim).assert_is_compatible_with(_ops.TensorShape(rhs.shape)[-1])
