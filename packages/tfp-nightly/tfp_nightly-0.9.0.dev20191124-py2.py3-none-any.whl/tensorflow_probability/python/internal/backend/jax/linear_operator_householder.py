...
class LinearOperatorHouseholder(linear_operator.LinearOperator):
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _check_reflection_axis(self, reflection_axis):
    ...
    if (_ops.TensorShape(reflection_axis.shape).ndims is not None and
        _ops.TensorShape(reflection_axis.shape).ndims < 1):
      raise ValueError(
  ...
  def _shape(self):
    ...
    d_shape = _ops.TensorShape(self._reflection_axis.shape)
