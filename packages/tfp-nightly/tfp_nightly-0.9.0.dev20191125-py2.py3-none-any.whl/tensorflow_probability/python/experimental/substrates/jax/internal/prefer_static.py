...
from tensorflow_probability.python.internal.backend import jax as nptf
...
def smart_where(condition, x_fn, y_fn):
  ...
  if cond_static is not None:
    ...
    elif isinstance(cond_static, (np.ndarray, onp.generic)):
...
def is_numpy(x):
  ...
  return isinstance(x, (np.ndarray, onp.generic))
