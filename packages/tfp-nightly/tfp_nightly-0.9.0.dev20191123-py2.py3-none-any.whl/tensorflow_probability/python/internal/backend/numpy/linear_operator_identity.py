...
class LinearOperatorIdentity(BaseLinearOperatorIdentity):
  ...
  # Note that _ops.TensorShape(y.shape) is compatible with _ops.TensorShape(operator.shape) because _ops.TensorShape(operator.shape)
  ...
  ```
  _ops.TensorShape(operator.shape) = [B1,...,Bb] + [N, N],  with b >= 0
  _ops.TensorShape(x.shape) =   [C1,...,Cc] + [N, R],
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _possibly_broadcast_batch_shape(self, x):
    ...
    bshape = _ops.broadcast_static_shape_as_tensorshape(_ops.TensorShape(x.shape), special_shape)
