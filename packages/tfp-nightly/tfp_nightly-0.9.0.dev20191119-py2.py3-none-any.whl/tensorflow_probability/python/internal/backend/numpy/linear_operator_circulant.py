...
from tensorflow_probability.python.internal.backend.numpy import numpy_signal as fft_ops
...
class _BaseLinearOperatorCirculant(linear_operator.LinearOperator):
  ...
  def block_shape(self):
    return _ops.TensorShape(self.spectrum.shape)[-self.block_depth:]
  ...
  def _vectorize_then_blockify(self, matrix):
    ...
    if (_ops.TensorShape(vec.shape).is_fully_defined() and
  ...
  def _trace(self):
    ...
    if _ops.TensorShape(self.spectrum.shape).is_fully_defined():
      spec_rank = _ops.TensorShape(self.spectrum.shape).ndims
    ...
    # Real diag part "re_d".
    # Suppose _ops.TensorShape(spectrum.shape) = [B1,...,Bb, N1, N2]
    # _ops.TensorShape(self.shape) = [B1,...,Bb, N, N], with N1 * N2 = N.
    # _ops.TensorShape(re_d_value.shape) = [B1,...,Bb]
...
class LinearOperatorCirculant(_BaseLinearOperatorCirculant):
  ...
  Fourier transforms.  Here we consider `_ops.TensorShape(A.shape) = [N, N]` and ignore batch
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `_ops.TensorShape(H.shape) = [B1,...,Bb, N]`.  We say that `H` is a Hermitian spectrum
  ...
  * This operator is self-adjoint if and only if `H` is real.
  ...
  and `_ops.TensorShape(x.shape) = [N, R]`.  Then
...
class LinearOperatorCirculant2D(_BaseLinearOperatorCirculant):
  ...
  If `_ops.TensorShape(H.shape) = [N0, N1]`, (`N0 * N1 = N`):
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `_ops.TensorShape(H.shape) = [B1,...,Bb, N0, N1]`, we say that `H` is a Hermitian
...
class LinearOperatorCirculant3D(_BaseLinearOperatorCirculant):
  ...
  If `_ops.TensorShape(H.shape) = [N0, N1, N2]`, (`N0 * N1 * N2 = N`):
  Loosely speaking, matrix multiplication is equal to the action of a
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `_ops.TensorShape(H.shape) = [B1,...,Bb, N0, N1, N2]`, we say that `H` is a Hermitian
