...
class HashableWeakRef(weakref.ref):
  ...
  def __hash__(self):
    ...
    if not isinstance(x, onp.ndarray):
      return id(x)
    if isinstance(x, onp.generic):
      raise ValueError('Unable to weakref onp.generic')
...
class Bijector(tf.Module):
  ...
  **WARNING** Handing of non-injective transforms is subject to change.
  ...
  def __call__(self, value, name=None, **kwargs):
    ...
    from tensorflow_probability.python.experimental.substrates.jax.bijectors import chain  # pylint: disable=g-import-not-at-top
    from tensorflow_probability.python.experimental.substrates.jax.distributions import distribution  # pylint: disable=g-import-not-at-top
    from tensorflow_probability.python.experimental.substrates.jax.distributions import transformed_distribution  # pylint: disable=g-import-not-at-top
  ...
  def _maybe_get_static_event_ndims(self, event_ndims):
    ...
    if isinstance(event_ndims_, (onp.generic, np.ndarray)):
