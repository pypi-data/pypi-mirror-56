...
class monitoring2:
	...
	def seek_by_inn(self, inn, rows = 50, page = True, sidx = 'orgId', sord = 'desc'):
		'''
		Поиск организации по ИНН, ОГРН
		'''
		form = {'rows' : rows, 'page' : page , 'sidx' : sidx, 'sord' : sord, 'orgTag' : int(inn)}
		print(form)
...
class prodRf:
	...
	def __init__(self, login , password , URL = 'https://org.1-ofd.ru', crpt = False):
		self.log = []
		self.lk_login = login
