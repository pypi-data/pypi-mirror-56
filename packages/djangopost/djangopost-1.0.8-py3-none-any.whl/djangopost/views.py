...
def ArticleListDashboard(request):
    ...
    article_filter = ArticleModel.objects.by_author(request.user)
    query = request.GET.get("query")
    if query:
        match = ArticleModel.objects.by_author(request.user).filter(Q(title__icontains=query))
        if match:
...
def CategoryListDashboard(request):
    ...
    category_list = CategoryModel.objects.filter_publish()
    query = request.GET.get("query")
    if query:
        match = CategoryModel.objects.filter_publish().filter(Q(title__icontains=query))
        if match:
            message = f"Search Results for: {query}."
...
def ArticleListView(request):
    ...
    article_filter = ArticleModel.objects.filter_publish().filter(is_promote=False)
