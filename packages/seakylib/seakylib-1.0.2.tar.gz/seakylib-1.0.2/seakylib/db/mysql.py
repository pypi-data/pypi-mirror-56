...
class MyModel:
    ...
    def update(self, data_new, data_old=None, key=None, cols=None, force_condition=None,
        ...
        if items_miss:
            if action_mark_miss:
                ...
                is_ok, sql = self.delete(data=items_miss, batch=True, key=key,
    ...
    def add(self, data, expandable=False, check=True):
        ...
        :param data:
        :param expandable:
        :param check: assure the data is not in db, called by update() with expandable set to False
        :return:
        '''
        if check:
            status, ex_obj = self.get_obj(data)
    ...
    def add_all(self, datas, block=1000, timed=True, now=None, time_col='time_insert', dryrun=False, **kwargs):
        ...
        :return:
        '''
        if timed and now and time_col in self.cols_name:
    ...
    def _add_all(self, datas, mode='save', get_ids=True):
        '''
        :param datas:
        :param mode: 'add' is operated one by one, return obj. 'save' is batch insert into db, no return by default
    ...
    def delete(self, data=None, skip_miss=True, batch=True, key=None, condition=None, force_condition=None,
        ...
        :param skip_miss:   skip error that data not in db
        ...
        if isinstance(condition, str):
            ...
            self.session.execute(sql)
            self.commit()
            return True, 'delete with condition done.'
    ...
    def call_procedure(self, name):
        res = self.session.execute('call {}()'.format(name)).fetchall()
        self.commit()
        return True, res
