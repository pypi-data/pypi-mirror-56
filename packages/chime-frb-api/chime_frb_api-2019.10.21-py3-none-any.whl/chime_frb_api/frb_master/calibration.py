...
class Calibration(API):
    ...
    def get_calibration(self, utc_date: str = None, source_name: str = None) -> JSON:
        """
        Fetches all CHIME/FRB calibration data product for a specific utc_date,
    ...
    def get_calibration_in_timerange(
        ...
        if isinstance(source_name, str):
            url += "/{}".format(source_name)
        url += "/{}-{}".format(start_utc_date, stop_utc_date)
        return self.get(url)
    ...
    def get_nearest_calibration(
        ...
        url = "/v1/calibration/nearest/{}/{}/{}".format(utc_date, ra, dec)
        return self.get(url)

    def get_calibration_spectrum(self, calprod_filepath: str = None) -> JSON:
        """
        ...
        calprod_filepath: str
            The path of the .npz file containing the 16K BF/Jy calibration spectrum
            and calibration diagnostic information on the CHIME/FRB Archiver.
        ...
        return self.post(url=url, json=payload)
