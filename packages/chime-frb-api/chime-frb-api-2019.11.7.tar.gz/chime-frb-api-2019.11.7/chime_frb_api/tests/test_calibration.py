...
master = frb_master.FRBMaster(base_url="http://localhost:8001")
...
def test_get_calibration_spectrum():
    with pytest.raises(HTTPError):
        master.calibration.get_calibration_spectrum(calprod_filepath="test")


def test_bad_get_calibration_spectrum():
    with pytest.raises(AttributeError):
        master.calibration.get_calibration_spectrum()
