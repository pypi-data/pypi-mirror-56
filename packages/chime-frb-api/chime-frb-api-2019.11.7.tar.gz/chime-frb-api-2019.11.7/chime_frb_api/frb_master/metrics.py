...
class Metrics(API):
    ...
    def overview(self) -> JSON:
        ...
        """
        return self.get(url="/v1/metrics/overview")
    ...
    def configure(
        ...
        return self.post(url="/v1/metrics/configure", json=payload)
    ...
    def reconfigure(self, category: str = None, names: [str] = None):
        ...
        return self.patch(url="/v1/metrics/configure", json=payload)
    ...
    def add(
        ...
        if patch:
            return self.patch(url="/v1/metrics", json=payload)
        else:
            return self.post(url="/v1/metrics", json=payload)
    ...
    def get_metrics(self, category: str = None, metric: str = None) -> JSON:
        ...
        -------
            metrics: list
                ...
                {
                    "created_at" : Creation time of the metric
                    "updated_at" : Last time the metric was updated
        ...
        if not category:
            raise AttributeError("category is required")
        url = "/v1/metrics/{}".format(category)
        if metric:
            url = "{}/{}".format(url, metric)
    ...
    def search(
        ...
        ----------
            ...
            timestamps : list
                Bounds of time range to search
                Valid values are "min", "max", dateutil.parser parseable str
        ...
        return self.post(url="/v1/metrics/search", json=payload)
