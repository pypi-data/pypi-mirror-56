...
from platformids import RTE, ISSTR, RTE_DIST, isJython, osname, \
    fetch_platform_distribution, fetch_platform_os, \
    fetch_category, fetch_ostype, fetch_dist, \
    rte2num, RTE_GENERIC, PlatformIDsError
...
bash_map = {
    ...
    'dist': "DIST",
    'distrel': "DISTREL",
    'distrel_hexrelease': "distrel_hexrelease",
...
if osname in ('nt', 'cygwin'):
    #
    # specials for ostype NT
    #
    bash_map.update({
...
class PlatformParametersError(PlatformIDsError):
    pass
...
class PlatformParameters(object):
    ...
    def __init__(self, *args, **kargs):
        ...
        """
        self.category = self.ostype = self.distrel_hexrelease = self.wProductType = self.wSuiteMask = 0
        ...
        if _myargs:
            ...
            _myargs.pop(0)
        if _myargs:
            self.distrel_hexrelease = self.distrel = _myargs[0] #: distrel - cumulated sub-vector
            # _myargs.pop(0)
        ...
        self.distrel_hexrelease = self.distrel = kargs.get('distrel', self.distrel)
    ...
    def __and__(self, other):
        ...
        """
        if not self.distrel_hexrelease:
            self.distrel_hexrelease = self.get_hexversion()

        if isinstance(other, int):
            return self.distrel_hexrelease & other

        elif isinstance(other, PlatformParameters):
            return self.distrel_hexrelease & other.get_hexversion()

        elif isinstance(other, dict):
            # use other as init parameters - simply trust or pass exception
            return self.distrel_hexrelease & PlatformParameters(**other).get_hexversion()

        elif isinstance(other, tuple):
            return self.distrel_hexrelease & PlatformParameters(*other).get_hexversion()

        raise PlatformIDsError("type not supported: other = " + str(other))
    ...
    def __eq__(self, other):
        ...
        *PlatformParameters*, and *dict*. In case of
        ...
        if type(other) in ISINT:
            ...
            if self.distrel_hexrelease == other:
                return True
            elif (
                self.distrel_hexrelease & RTE_DIST == other
    ...
    def __repr__(self):
        res = (
            ...
            '"distrel_version": %s, "osrel_version": %s, "distrel_hexrelease": %s, "os_id": %s, "osrel_id": %s}') % (
                str(self.category),
                str(self.ostype),
                str(self.dist),
                str(self.distrel),
...
#                     sys.stdout.write(str(JSONData(self.get_json(select=_select))))
    ...
    def __iand__(self, other):
        ...
        if isinstance(other, int):
            self.distrel_hexrelease &= other
            return self

        elif isinstance(other, PlatformParameters):
            self.distrel_hexrelease &= other.get_hexversion()
            return self

        elif isinstance(other, dict):
            self.distrel_hexrelease &= PlatformParameters(**other).get_hexversion()
            return self
    ...
    def __int__(self):
        ...
        if not self.distrel_hexrelease:
            self.distrel_hexrelease = self.get_hexversion()
        return self.distrel_hexrelease
    ...
    def scan(self):
        ...
        including *Jython*. 
        Internally calls *platformids.fetch_platform_distribution()*.
        ...
        * distrel_hexrelease
        ...
        # numeric bitmasks   
        self.category = fetch_category()
        self.ostype = fetch_ostype()
        self.dist = fetch_dist()
        self.distrel = self.distrel_hexrelease = RTE

        #
        # native platform string representation
        #
        # platform info - OS release
        self.os_id, self.os_version, self.os_version_num, self.osrel_id,  = fetch_platform_os()
  
        # platform info - distribution release
        self.distribution = fetch_platform_distribution()
        (
