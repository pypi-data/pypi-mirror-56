...
class Property_Base(object):
    ...
    def __init__(self, node, id, name=None, settable=False, retained=True, qos=1, unit=None, data_type=None, data_format=None, value=None, set_value=None, tags=[], meta={}):
    ...
    def publish_meta(self, retain=True, qos=1):
        ...
        if len(main_keys) > 0:
            self.publish ("/".join((self.topic, "$meta", "$mainkey-ids")), ','.join(main_keys), retain, qos)

        for main_key in main_keys:
            settings = self.meta [main_key]
            self.publish ("/".join((self.topic, "$meta", main_key, "$key")), settings ['name'], retain, qos)
            self.publish ("/".join((self.topic, "$meta", main_key, "$value")), settings ['value'], retain, qos)
            ...
            if 'subkeys' in settings:
                ...
                self.publish ("/".join((self.topic, "$meta", main_key, "$subkey-ids")), ','.join(sub_keys), retain, qos)

                for sub_key in sub_keys:
                    self.publish ("/".join((self.topic, "$meta", main_key, sub_key, "$key")), settings ['subkeys'] [sub_key] ['name'], retain, qos)
                    self.publish ("/".join((self.topic, "$meta", main_key, sub_key, "$value")), settings ['subkeys'] [sub_key] ['value'], retain, qos)
