...
class ComponentTest:
    ...
    def test_list(self, count=1, entries=5, total_from_res=len,
                  ...
                  single_parent=True, assert_empty_children=True):
        ...
        single_parent: boolean, default False
            If True and the component has a parent component,
            the same is used for all entries. Then, is is also
            asserted that the list contains a total of 0 elements
            at the start.
            If False and the component has a parent component,
            a separate parent is generated for each entry.
        assert_empty_children: bpolean, default True
            If true, the test asserts that the list of children
            is empty after creating an entry.
        """
        print("# # # test list")

        if single_parent and self.comp.parent is not None:
            child_inst = Instance(self.comp)
            child_inst.__enter__()
        else:
            child_inst = None
        ...
        for valids in self.comp.get_valids(Endpoints.LIST, count=count,
                                           ...
                                           generator=generator):
            if single_parent and self.comp.parent is not None:
                parent_inst = Instance(self.comp.parent)
                parent = parent_inst.__enter__()
            else:
                parent_inst, parent = None, None

            initial = total_from_res(self.comp.endpoints.list(valids, parent, limit, offset))

            if single_parent and self.comp.parent is not None and assert_empty_children:
                assert initial == 0, \
                        "Initial list of parent's children not empty"

            elements = []
            for i in range(entries):
                elements.append(Instance(self.comp, parent))
                elements[-1].__enter__()
                current = total_from_res(self.comp.endpoints.list(valids, parent, limit, offset))
            ...
            final_size = total_from_res(self.comp.endpoints.list(valids, parent, limit, offset))
            assert final_size == initial, \
                    "Final list size not the same as the initial one"

            if parent_inst is not None:
                parent_inst.__exit__(None, None, None)

        if child_inst is not None:
            child_inst.__exit__(None, None, None)
    ...
    def test_pagination(self,
        ...
        single_parent: boolean, default False
            If True and the component has a parent component,
            the same is used for all entries.
            If False and the component has a parent component,
            a separate parent is generated for each entry.
        """
        print("# # # test list pagination")

        def list(values, parent, limit=None, offset=None):
            return self.comp.endpoints.list(valids, parent, limit, offset)
        ...
        for valids in self.comp.get_valids(Endpoints.LIST,
            ...
            initial = total_from_res(list(valids, parent))
            total = initial + entries
            elements = [Instance(self.comp) for _ in range(total - initial)]
            for element in elements:
                element.__enter__()

            assert total_from_res(list(valids, parent)) == total, \
                    "Total list size not as expected"

            assert total_from_res(list(valids, parent, 4)) == total, \
                    "Total list size not as expected"
            assert size_from_res(list(valids, parent, 3)) == 3, \
                    "Entry count not as specified by limit"
            assert size_from_res(list(valids, parent, 2, total - 1)) == 1, \
                    "Only single elements is expected for an offset of total-1"

            for element in elements:
                element.__exit__(None, None, None)

            assert total_from_res(list(valids, parent)) == initial, \
    ...
    def test_child_delete(self):
        """Test invalid deletion of parent component.

        This test assumes that this component has a parent and checks
        that its deletion is not allowed if it has any children.
        """
        with Instance(self.comp.parent) as parent:
            with Instance(self.comp, parent) as child:
