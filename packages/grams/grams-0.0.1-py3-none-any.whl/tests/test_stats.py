...
from kiwi.stats import Distro, Sample
from kiwi.utils import generate_samples, randints

from .data import (no_word_data, one_word_data, small_data, small_uniform_data,
...
class StatsTestSuite(unittest.TestCase):
    ...
    def test_sample_one_word(self):
        ...
        samples_distro = Distro(dict(samples))
    ...
    def test_distro_similarity(self):
        ...
        for test_data in (no_word_data, one_word_data, small_data,
            ...
            with self.subTest(data=test_data.words):
                first_distro = Distro(test_data.word_to_freq)
                second_distro = Distro(test_data.word_to_freq)
                third_distro = Distro(
                    tuple(zip(test_data.words,
                              randints(len(test_data.words)))))
        ...
        first_distro = Distro(word_to_freq=(("apples", 100000), ("mangos", 1)))
        second_distro = Distro((("apples", 1), ("mangos", 100000)))
        ...
        for word in small_data.words:
            ...
            # between a Distro(n random ints of sum k) and Distro(n 0s of sum 0)
            # should be 0.5.
            ...
            with self.subTest(words=running_words, freqs=freqs):
                self.assertAlmostEqual(first_distro.similarity(second_distro),
                                       (1 if running_words_types > 1 else 0),
                                       1)

    def test_distro_removedups(self):

        with self.assertRaises(ValueError):
            #Distro.is_valid_wordfreq("a", 0)
            tuple(Distro.remove_dups((("a", 0), )))

        with self.assertRaises(ValueError):
            tuple(Distro.remove_dups((("a", float('inf')), )))

        wordfreq = (("a", 1), )
        self.assertEqual(tuple(Distro.remove_dups(wordfreq)), wordfreq)
        wordfreq = (*wordfreq, *wordfreq)
        self.assertEqual(tuple(Distro.remove_dups(wordfreq)), (("a", 2), ))

    @unittest.skip("asdf")
    def test_sample_small_uniform(self):
        distro = Distro(small_uniform_data.word_to_freq)
        sample = Sample(distro)
        n_samples = 10000
