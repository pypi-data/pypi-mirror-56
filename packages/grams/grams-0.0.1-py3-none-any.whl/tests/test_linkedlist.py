...
from kiwi.linkedlist import LinkedList, Node
...
class LinkedListTest(unittest.TestCase):
    ...
    def test_replace(self):
        LL = LinkedList(['A', 'p', 'p', 'l', 'e', 's'])
        ll = deepcopy(LL)
        ll.replace(lambda x: x == 'A', 'a')
        assert ll.head.data == 'a'
        assert ll.items() == ['a', 'p', 'p', 'l', 'e', 's']
        assert ll.length() == 6

        ll = deepcopy(LL)
        ll.replace(lambda x: x == 'p', ' ')
        assert ll.items() == ['A', ' ', 'p', 'l', 'e', 's']
        assert ll.length() == 6

        ll = deepcopy(LL)
        ll.replace(lambda x: x == 's', 'sss')
        assert ll.tail.data == 'sss'
        assert ll.items() == ['A', 'p', 'p', 'l', 'e', 'sss']
        assert ll.length() == 6

        ll = deepcopy(LL)
        ll.replace(lambda x: x == 'z', ' ')
        assert ll.tail.data == ' '
        assert ll.items() == ['A', 'p', 'p', 'l', 'e', 's', ' ']
        assert ll.length() == 7

        # test for cases where replace is used as a delete function
        ll = deepcopy(LL)
        ll.replace(lambda x: x == 'A')  # delete head
        assert ll.head.data == 'p'
        assert ll.items() == ['p', 'p', 'l', 'e', 's']
        assert ll.length() == 5

        ll.replace(lambda x: x == 's')  # delete tail
        assert ll.tail.data == 'e'
        assert ll.items() == ['p', 'p', 'l', 'e']
        assert ll.length() == 4

        ll.replace(lambda x: x == 'l')
        assert ll.head.data == 'p'
        assert ll.tail.data == 'e'
        assert ll.items() == ['p', 'p', 'e']
        assert ll.length() == 3

        ll.replace(lambda x: x == 'p')
        ll.replace(lambda x: x == 'p')
        assert ll.head.data == ll.tail.data == 'e'
        assert ll.items() == ['e']
        assert ll.length() == 1
        ...
        ll.replace(lambda x: x == 'e')
        assert ll.head is ll.tail is None
        assert ll.items() == []
        assert ll.length() == 0
