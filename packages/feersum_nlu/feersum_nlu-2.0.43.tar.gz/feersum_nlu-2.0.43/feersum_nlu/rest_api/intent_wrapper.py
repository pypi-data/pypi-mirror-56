"""
Wrapper of Feersum_NLU's Intent Classifier Python API in HTTP API.
"""
...
# === Start of helpers === #
# ======================== #
def intent_clsfr_save_helper(name: str, model_save_mode: wrapper_util.SaveHelperMode,
    ...
    if model_save_mode == wrapper_util.SaveHelperMode.VAPORISE_MODEL:
        ...
        wrapper_util.get_nlpe_engine().vaporise_intent_clsfr(name)
    elif model_save_mode == wrapper_util.SaveHelperMode.SAVE_MODEL:
        # Save the model blob.
        success = wrapper_util.get_nlpe_engine().save_intent_clsfr(name)
    ...
    meta_info = wrapper_util.intent_clsfr_dict[name]  # At this point one can assume that the 'name' key exists.

    meta_info.uuid = str(uuid.uuid4())  # Update the uuid of the local copy.

    success = wrapper_util.save_meta_info_blob(name + '.intent_clsfr_meta_pickle', meta_info,
...
def intent_clsfr_upgrade_meta_info(meta_info):
    ...
    if upgrade_meta_info:
        logging.debug(f"    intent_wrapper.intent_clsfr_load_helper: Upgrading meta_info.")
        meta_info = wrapper_util.IntentClassifierMeta(meta_info.uuid, meta_info.long_name, meta_info.desc,
                                                      meta_info_readonly,
...
def intent_clsfr_load_history_helper(name: str,
    ...
    if meta_info is None:
        ...
        logging.error(f"intent_clsfr_load_history_helper: Model history meta_info for {name} is not available!")
        intent_clsfr_trash_helper(name, trash_cache_only=True)
        return False  # 400, {"error_detail": "Loading of model meta data failed!"}

    if not isinstance(meta_info, wrapper_util.IntentClassifierMeta):
        logging.error(f"intent_clsfr_load_history_helper: Loaded model meta_info of {name} of incorrect class!")
    ...
    if wrapper_util.intent_clsfr_dict.get(name) is not meta_info:
        meta_info = intent_clsfr_upgrade_meta_info(meta_info)
        wrapper_util.intent_clsfr_dict[name] = meta_info

        # Note: If a revision was retrieved from history then the model needs to be RETRAINED! So trash the cached model.
        wrapper_util.get_nlpe_engine().trash_intent_clsfr(name, trash_cache_only=True)
...
def intent_clsfr_load_helper(name: str,
    ...
    # Load the meta info first ...
    meta_info = wrapper_util.load_meta_info_blob(name=name + '.intent_clsfr_meta_pickle',
                                                 look_in_trash=look_in_trash,
                                                 cached_meta_blob=wrapper_util.intent_clsfr_dict.get(name))
    ...
    if not isinstance(meta_info, wrapper_util.IntentClassifierMeta):
        logging.error(f"intent_clsfr_load_helper: Loaded model meta_info of {name} of incorrect class!")
        return False  # 400, {"error_detail": "Loaded model meta_info object of incorrect class!"}
    ...
    if wrapper_util.intent_clsfr_dict.get(name) is not meta_info:
        ...
        wrapper_util.get_nlpe_engine().load_intent_clsfr(name,
                                                         meta_info.word_manifold_name_dict)

    return True


def intent_clsfr_trash_helper(name: str, trash_cache_only: bool = False) -> bool:
    """ Trash the meta info and model info + manage cache. """

    if wrapper_util.intent_clsfr_dict.get(name) is not None:
        del wrapper_util.intent_clsfr_dict[name]

    if not trash_cache_only:
        wrapper_util.trash_meta_info_blob(name + '.intent_clsfr_meta_pickle')

    wrapper_util.get_nlpe_engine().trash_intent_clsfr(name, trash_cache_only=trash_cache_only)

    return True


def intent_clsfr_vaporise_helper(name: str) -> bool:
    """ Vaporise the meta info and model info + manage cache. """

    if wrapper_util.intent_clsfr_dict.get(name) is not None:
        del wrapper_util.intent_clsfr_dict[name]

    wrapper_util.vaporise_meta_info_blob(name + '.intent_clsfr_meta_pickle')
...
def intent_clsfr_create(iname: str,
    ...
    if (load_from_store is True) or (revision_uuid is not None):
        ...
        if revision_uuid is not None:
            success = intent_clsfr_load_history_helper(name, revision_uuid)
        else:
            success = intent_clsfr_load_helper(name, look_in_trash=True)

        meta_info = wrapper_util.intent_clsfr_dict.get(name)
        ...
        success = intent_clsfr_load_helper(name, look_in_trash=False)
        meta_info = wrapper_util.intent_clsfr_dict.get(name)
        ...
        # Create a new model
        wrapper_util.intent_clsfr_dict[name] = wrapper_util.IntentClassifierMeta("",
                                                                                 long_name,
                                                                                 description,
                                                                                 False,
                                                                                 1.0,
        ...
        intent_clsfr_save_helper(name, wrapper_util.SaveHelperMode.VAPORISE_MODEL,
                                 save_in_history=True,
                                 save_in_history_message="Created model.")

        meta_info = wrapper_util.intent_clsfr_dict[name]
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_del(iname: str, auth_token: str, caller_name: Optional[str],
                     vaporise: bool) -> Tuple[int, wrapper_util.JSONType]:
    """ Trash/vaporise the intent classifier model. """
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
    ...
    if not vaporise:
        intent_clsfr_trash_helper(name)
    else:
        intent_clsfr_vaporise_helper(name)
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_details(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_details_all(auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    ...
    for model_info in model_meta_info_list:
        name = model_info[0]  # name is 'modelname_authtoken.intent_clsfr_meta_pickle'

        if name.endswith('.intent_clsfr_meta_pickle'):
            meta_info = model_info[1]  # type: wrapper_util.IntentClassifierMeta
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_class_labels(iname: str, auth_token: str,
    ...
    class_label_list = wrapper_util.get_nlpe_engine().get_intent_clsfr_labels(name)
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_set_params(iname: str, auth_token: str, caller_name: Optional[str],
    ...
    # Ensure meta_info is of type IntentClassifierMeta
    meta_info = wrapper_util.IntentClassifierMeta(meta_info.uuid,
                                                  meta_info.long_name,
                                                  meta_info.desc,
                                                  meta_info.readonly,
                                                  meta_info.weak_match_threshold,
    ...
    success = intent_clsfr_save_helper(name, wrapper_util.SaveHelperMode.DONT_SAVE_MODEL,
                                       save_in_history=True,
    ...
    smpl_training_confusion_dict = engine_utils.smplfy_confusion_matrix(
        wrapper_util.intent_clsfr_dict[name].training_confusion_dict)

    smpl_validation_confusion_dict = engine_utils.smplfy_confusion_matrix(
        wrapper_util.intent_clsfr_dict[name].validation_confusion_dict)

    smpl_testing_confusion_dict = engine_utils.smplfy_confusion_matrix(
        wrapper_util.intent_clsfr_dict[name].testing_confusion_dict)
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_params(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_add_training_samples(iname: str,
                                      auth_token: str, caller_name: Optional[str],
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_training_samples(iname: str, auth_token: str,
                                      index: Optional[int],
                                      length: Optional[int],
                                      caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_remove_training_samples(iname: str,
                                         auth_token: str, caller_name: Optional[str],
                                         json_training_data: Optional[Dict]) -> Tuple[int, wrapper_util.JSONType]:
    # if json_training_samples None then remove ALL the samples !!!
    ...
    # Ensure meta_info is of type IntentClassifierMeta
    # Make sure dict points to latest instance.
    wrapper_util.intent_clsfr_dict[name] = wrapper_util.IntentClassifierMeta(meta_info.uuid,
                                                                             meta_info.long_name,
                                                                             meta_info.desc,
                                                                             meta_info.readonly,
                                                                             meta_info.weak_match_threshold,
    ...
    # Save the named meta_info blob. MIGHT modify the meta_info!
    intent_clsfr_save_helper(name, wrapper_util.SaveHelperMode.DONT_SAVE_MODEL,
                             save_in_history=True,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_update_training_samples(iname: str,
                                         auth_token: str, caller_name: Optional[str],
                                         json_training_data: Dict) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_add_testing_samples(iname: str,
                                     auth_token: str, caller_name: Optional[str],
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_get_testing_samples(iname: str, auth_token: str,
                                     index: Optional[int],
                                     length: Optional[int],
                                     caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_remove_testing_samples(iname: str,
                                        auth_token: str, caller_name: Optional[str],
                                        json_testing_data: Optional[Dict]) -> Tuple[int, wrapper_util.JSONType]:
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_update_testing_samples(iname: str,
                                        auth_token: str, caller_name: Optional[str],
                                        json_testing_data: Optional[Dict]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
...
def _intent_clsfr_train_impl(iname: str, auth_token: str,
    ...
    logging.info(f"intent_wrapper._intent_clsfr_train_impl: Training task STARTED.")
    ...
    # === Generate ExpressionT lists for both 'intent_clsfr_training_samples' & 'intent_clsfr_testing_samples' so that the
    # intent IDs are shared between the training and testing examples!!!
    intent_clsfr_samples = []
    intent_clsfr_samples.extend(meta_info.training_samples)
    intent_clsfr_samples.extend(meta_info.testing_samples)

    intent_clsfr_expressions = \
        wrapper_util.get_nlpe_engine().cnvrt_intent_tuples_to_expressions(intent_clsfr_samples)
    intent_clsfr_training_expressions = intent_clsfr_expressions[:len(meta_info.training_samples)]
    intent_clsfr_testing_expressions = intent_clsfr_expressions[len(meta_info.training_samples):]
    # ===

    # === Do the training ===
    success = wrapper_util.get_nlpe_engine().train_intent_clsfr(name,
    ...
    #     wrapper_util.get_nlpe_engine().train_and_cross_validate_intent_clsfr(name,
    #                                                                          intent_clsfr_training_expressions,
    #                                                                          [],
    #                                                                          word_manifold_dict,
    #                                                                          k=10,
    ...
    if len(intent_clsfr_training_expressions) > training_subset_len:
        training_subset = [intent_clsfr_training_expressions[i] for i in
                           random.sample(range(len(intent_clsfr_training_expressions)),
                                         training_subset_len)]
    else:
        training_subset = intent_clsfr_training_expressions

    # HEAVY CPU
    training_accuracy, training_f1, training_confusion_dict_full_sl, training_confusion_dict_labels = \
        wrapper_util.get_nlpe_engine().test_intent_clsfr(name, training_subset,
    ...
    testing_accuracy, testing_f1, testing_confusion_dict_full_sl, testing_confusion_dict_labels = \
        wrapper_util.get_nlpe_engine().test_intent_clsfr(name, intent_clsfr_testing_expressions,
                                                         weak_match_threshold, 1)
    ...
    success = intent_clsfr_save_helper(name, wrapper_util.SaveHelperMode.SAVE_MODEL,
                                       save_in_history=True,
    ...
    word_manifold_list = []  # type: List[Dict[str, str]]
    for lang, manifold_name in wrapper_util.intent_clsfr_dict[name].word_manifold_name_dict.items():
        word_manifold_list.append({'label': lang, 'word_manifold': manifold_name})
    ...
    logging.info(f"intent_wrapper._intent_clsfr_train_impl: Training task COMPLETED.")
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_train(iname: str, auth_token: str, caller_name: Optional[str],
                       weak_match_threshold: Optional[float],
    ...
    if (immediate_mode is True) or (check_celery_available() is False):
        ...
        logging.info(f"intent_wrapper.intent_clsfr_train: Training synchronously.")
        sync_result = tasks.intent_clsfr_train.apply(kwargs={'iname': iname, 'auth_token': auth_token,
                                                             'weak_match_threshold': weak_match_threshold,
                                                             'requested_word_manifold_dict': requested_word_manifold_dict,
                                                             'callback_url': None})
        logging.info(f"intent_wrapper.intent_clsfr_train: Training done. Returning sync result in response.")
        ...
        logging.info(f"intent_wrapper.intent_clsfr_train: Training synchronously/asynchronously.")
        current_uuid = meta_info.uuid
        async_result = \
            tasks.intent_clsfr_train.apply_async(kwargs={'iname': iname, 'auth_token': auth_token,
                                                         'weak_match_threshold': weak_match_threshold,
                                                         'requested_word_manifold_dict': requested_word_manifold_dict,
        ...
        if async_result.ready():
            ...
            logging.info(f"intent_wrapper.intent_clsfr_train: Async training task busy.")
            response_data = {
                "name": iname,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_retrieve(iname: str, auth_token: str, caller_name: Optional[str],
                          text: Optional[str],
    ...
    if (wrapper_util.intent_retrieve_MAX_TEXT_LENGTH >= 0) and (len(text) > wrapper_util.intent_retrieve_MAX_TEXT_LENGTH):
        text = text[:wrapper_util.intent_retrieve_MAX_TEXT_LENGTH]
    ...
    # [text, intent_id, intent_label, score, text_lang]
    result_list, lang_code = wrapper_util.get_nlpe_engine().retrieve_intents(name,
                                                                             text, lang_code,
                                                                             meta_info.weak_match_threshold,
                                                                             3)
    # result_list = List[Tuple[text, intent_id, intent_label, score, lang_code]]

    end_time = time.time()

    logging.info(f"intent_wrapper.intent_clsfr_retrieve - "
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_curate(iname: str, auth_token: str, caller_name: Optional[str],
...
def _intent_clsfr_tsne_post_impl(iname: str, auth_token: str,
    ...
    :return: The response status and json payload.
    """
    logging.info(f"intent_wrapper._intent_clsfr_tsne_post_impl: Task STARTED.")
    ...
    tsne_list = wrapper_util.get_nlpe_engine().tsne_intent_clsfr(name, n_components, perplexity, learning_rate)
    ...
    logging.info(f"intent_wrapper._intent_clsfr_tsne_post_impl: Task COMPLETED.")
    return 200, {}


@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_tsne_post(iname: str, auth_token: str, caller_name: Optional[str],
                           n_components: int, perplexity: float, learning_rate: float,
                           callback_url: Optional[str] = None) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
    ...
    if check_celery_available() is False:
        ...
        logging.info(f"intent_wrapper.intent_clsfr_tsne_post: TSNE-ing synchronously.")
        sync_result = tasks.intent_clsfr_tsne_post.apply(kwargs={'iname': iname, 'auth_token': auth_token,
                                                                 'n_components': n_components, 'perplexity': perplexity,
                                                                 'learning_rate': learning_rate,
                                                                 'callback_url': None})
        logging.info(f"intent_wrapper.intent_clsfr_tsne_post: Processing done. Returning sync result in response.")
        return sync_result.get()  # result is an EagerResult that we know has already been executed.
    else:
        # WITH Celery - asynchronously or synchronously if Celery is in eager mode.
        logging.info(f"intent_wrapper.intent_clsfr_tsne_post: TSNE-ing synchronously/asynchronously.")
        # current_uuid = meta_info.uuid

        async_result = \
            tasks.intent_clsfr_tsne_post.apply_async(kwargs={'iname': iname, 'auth_token': auth_token,
                                                             'n_components': n_components, 'perplexity': perplexity,
                                                             'learning_rate': learning_rate,
                                                             'callback_url': callback_url})
        ...
        if async_result.ready():
            ...
            logging.info(f"intent_wrapper.intent_clsfr_tsne_post: Async processing task busy.")
            response_data = {}  # type: wrapper_util.JSONType
            # "name": iname,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_tsne_get(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def intent_clsfr_add_online_training_samples(iname: str,
                                             auth_token: str, caller_name: Optional[str],
                                             json_training_data: Dict) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
    ...
    existing_expressions = wrapper_util.get_nlpe_engine().get_model_intent_clsfr_expressions(name)

    online_training_expressions = wrapper_util.get_nlpe_engine().cnvrt_intent_tuples_to_expressions(
    ...
    for text, label, lang_hint in online_training_tuples:
        training_sample_is_duplicate = False

        for model_sample in meta_info.training_samples:
            if (text == model_sample[0]) and (label == model_sample[1]) and (lang_hint == model_sample[2]):
                training_sample_is_duplicate = True
                break  # break from for-loop

        if not training_sample_is_duplicate:
            meta_info.training_samples.append((text, label, lang_hint, str(uuid.uuid4())))
            updated_samples.append(meta_info.training_samples[-1])

    wrapper_util.intent_clsfr_dict[name] = meta_info

    success = wrapper_util.get_nlpe_engine().train_intent_clsfr_online(name, online_training_expressions, [])
