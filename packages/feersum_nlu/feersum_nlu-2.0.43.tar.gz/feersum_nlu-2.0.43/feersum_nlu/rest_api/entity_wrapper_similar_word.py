"""
Wrapper of Feersum_NLU's Similar Word Entity Extractor Python API in HTTP API.
"""
from typing import Tuple, Dict, List, Optional, Set  # noqa # pylint: disable=unused-import
import uuid
...
def sim_word_extr_save_helper(name: str, model_save_mode: wrapper_util.SaveHelperMode,
    ...
    if model_save_mode == wrapper_util.SaveHelperMode.VAPORISE_MODEL:
        ...
        # wrapper_util.get_wrapper_nlpe_engine().vaporise_sim_word_extr(name)
        pass
    elif model_save_mode == wrapper_util.SaveHelperMode.SAVE_MODEL:
        # Save the model blob.
        # success = wrapper_util.get_wrapper_nlpe_engine().save_sim_word_extr(name)
        #
        # if not success:
        #     return False
        pass
    ...
    meta_info = wrapper_util.sim_word_extr_dict[name]  # At this point one can assume that the key exists.

    meta_info.uuid = str(uuid.uuid4())  # Update the uuid of the local copy.

    success = wrapper_util.save_meta_info_blob(name + '.sim_word_extr_meta_pickle', meta_info,
...
def sim_word_extr_upgrade_meta_info(meta_info):
    ...
    if upgrade_meta_info:
        logging.debug(f"    entity_wrapper_similar_word.sim_word_extr_load_helper: Upgrading meta_info.")
        meta_info = wrapper_util.SimWordExtractorMeta(meta_info.uuid,
                                                      meta_info.long_name,
                                                      meta_info.desc,
                                                      meta_info_readonly,
                                                      meta_info.input_entities,
                                                      meta_info.threshold,
                                                      meta_info.word_manifold_name,
                                                      meta_info.spell_correct,
                                                      meta_info.training_stamp)
    return meta_info


def sim_word_extr_load_history_helper(name: str,
    ...
    if meta_info is None:
        ...
        logging.error(f"sim_word_extr_load_history_helper: Model history meta_info for {name} is not available!")
        sim_word_extr_trash_helper(name, trash_cache_only=True)
        return False  # 400, {"error_detail": "Loading of model meta data failed!"}

    if not isinstance(meta_info, wrapper_util.SimWordExtractorMeta):
        logging.error(f"sim_word_extr_load_history_helper: Loaded model meta_info of {name} of incorrect class!")
    ...
    if wrapper_util.sim_word_extr_dict.get(name) is not meta_info:
        meta_info = sim_word_extr_upgrade_meta_info(meta_info)
        wrapper_util.sim_word_extr_dict[name] = meta_info

        # Note: If a revision was retrieved from history then the model needs to be RETRAINED! So trash the cached model.
        # wrapper_util.get_nlpe_engine().trash_sim_word_extr(name, trash_cache_only=True)
...
def sim_word_extr_load_helper(name: str,
    ...
    meta_info = wrapper_util.load_meta_info_blob(name=name + '.sim_word_extr_meta_pickle',
                                                 look_in_trash=look_in_trash,
                                                 cached_meta_blob=wrapper_util.sim_word_extr_dict.get(name))
    ...
    if wrapper_util.sim_word_extr_dict.get(name) is not meta_info:
        ...
        # wrapper_util.get_wrapper_nlpe_engine().load_sim_word_extr(name)

    return True


def sim_word_extr_trash_helper(name: str, trash_cache_only: bool = False) -> bool:
    """ Trash the meta info and model info + manage cache. """

    if wrapper_util.sim_word_extr_dict.get(name) is not None:
        del wrapper_util.sim_word_extr_dict[name]

    if not trash_cache_only:
        wrapper_util.trash_meta_info_blob(name + '.sim_word_extr_meta_pickle')

    # wrapper_util.get_wrapper_nlpe_engine().trash_sim_word_extr(name, trash_cache_only=trash_cache_only)

    return True


def sim_word_extr_vaporise_helper(name: str) -> bool:
    """ Vaporise the meta info and model info + manage cache. """

    if wrapper_util.sim_word_extr_dict.get(name) is not None:
        del wrapper_util.sim_word_extr_dict[name]

    wrapper_util.vaporise_meta_info_blob(name + '.sim_word_extr_meta_pickle')
...
def sim_word_extr_create(iname: str,
                         auth_token: str, caller_name: Optional[str],
                         long_name: Optional[str],
                         description: Optional[str],
                         input_entities: Optional[Set[str]],
                         threshold: Optional[float],
                         word_manifold_name: Optional[str],
    ...
    if (load_from_store is True) or (revision_uuid is not None):
        ...
        if revision_uuid is not None:
            success = sim_word_extr_load_history_helper(name, revision_uuid)
        else:
            success = sim_word_extr_load_helper(name, look_in_trash=True)

        meta_info = wrapper_util.sim_word_extr_dict.get(name)
        ...
        return 200, {"name": iname,
                     ...
                     "similar_words": list(meta_info.input_entities),
                     "threshold": meta_info.threshold,
                     "word_manifold": meta_info.word_manifold_name,
                     "training_stamp": meta_info.training_stamp}
    elif input_entities is not None and \
            threshold is not None and \
            word_manifold_name is not None:
        # === See if there already exists a model with the same name ===
        success = sim_word_extr_load_helper(name, look_in_trash=False)
        meta_info = wrapper_util.sim_word_extr_dict.get(name)
        ...
        wrapper_util.sim_word_extr_dict[name] = wrapper_util.SimWordExtractorMeta("",
                                                                                  long_name,
                                                                                  description,
                                                                                  False,
                                                                                  input_entities, threshold,
                                                                                  word_manifold_name, True,
                                                                                  str(datetime.now(
                                                                                      timezone.utc).astimezone()))

        # Save the named meta_info and model blobs. MIGHT modify the meta_info!
        success = sim_word_extr_save_helper(name, wrapper_util.SaveHelperMode.SAVE_MODEL,
                                            save_in_history=True,
                                            save_in_history_message="Created model.")

        return 200, {"name": iname,
                     "id": wrapper_util.sim_word_extr_dict[name].uuid,
                     "long_name": wrapper_util.sim_word_extr_dict[name].long_name,
                     "desc": wrapper_util.sim_word_extr_dict[name].desc,
                     "readonly": wrapper_util.sim_word_extr_dict[name].readonly,
                     "similar_words": list(wrapper_util.sim_word_extr_dict[name].input_entities),
                     "threshold": wrapper_util.sim_word_extr_dict[name].threshold,
                     "word_manifold": wrapper_util.sim_word_extr_dict[name].word_manifold_name,
                     "training_stamp": wrapper_util.sim_word_extr_dict[name].training_stamp}
    else:
        return 400, {
            "error_detail": "Provide list of words to test against, threshold, word manifold and whether to spell correct."}
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_del(iname: str, auth_token: str, caller_name: Optional[str],
                      vaporise: bool) -> Tuple[int, wrapper_util.JSONType]:
    """ Trash/vaporise the sim word extractor model. """
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.

    name = iname + "_" + auth_token

    sim_word_extr_details = {}  # type: Dict
    ...
    sim_word_extr_details = {"name": name[:-(len(auth_token) + 1)],
                             "id": meta_info.uuid,
                             "long_name": meta_info.long_name,
                             "desc": meta_info.desc,
                             "readonly": meta_info.readonly,
    ...
    if not vaporise:
        sim_word_extr_trash_helper(name)
    else:
        sim_word_extr_vaporise_helper(name)

    return 200, sim_word_extr_details


@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_get_details(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
    ...
    sim_word_extr_details = {"name": iname,
                             "id": meta_info.uuid,
                             "long_name": meta_info.long_name,
                             "desc": meta_info.desc,
                             "readonly": meta_info.readonly,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_get_details_all(auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    ...
    for model_info in model_meta_info_list:
        ...
        if name.endswith('.sim_word_extr_meta_pickle'):
            meta_info = model_info[1]  # type: wrapper_util.SimWordExtractorMeta
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_retrieve(iname: str, auth_token: str, caller_name: Optional[str],
    ...
    if meta_info.word_manifold_name in wrapper_util.nlp_engine.feers_sent_encoder_dict:
        req_word_manifold = meta_info.word_manifold_name
    else:
        req_word_manifold = meta_info.word_manifold_name + "_" + auth_token

    # List[Dict]
    result_list = wrapper_util.get_nlpe_engine().retrieve_similar_entities(text,
                                                                           meta_info.input_entities,
                                                                           meta_info.threshold,
                                                                           req_word_manifold,
                                                                           meta_info.spell_correct, "")
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_set_params(iname: str, auth_token: str, caller_name: Optional[str],
    ...
    similar_words = param_dict.get('similar_words')
    threshold = param_dict.get('threshold')
    word_manifold = param_dict.get('word_manifold')
    ...
    if similar_words is not None:
        meta_info.input_entities = similar_words
    if threshold is not None:
        meta_info.threshold = threshold
    if word_manifold is not None:
        meta_info.word_manifold_name = word_manifold

    wrapper_util.sim_word_extr_dict[name] = meta_info

    # Save the named meta_info blob. MIGHT modify the meta_info!
    success = sim_word_extr_save_helper(name, wrapper_util.SaveHelperMode.DONT_SAVE_MODEL,
                                        save_in_history=True,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def sim_word_extr_get_params(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
