"""
Wrapper of Feersum_NLU's Duckling Entity Extractor Python API in HTTP API.
"""
...
def duckling_extr_save_helper(name: str, model_save_mode: wrapper_util.SaveHelperMode,
    ...
    if model_save_mode == wrapper_util.SaveHelperMode.VAPORISE_MODEL:
        ...
        # wrapper_util.get_wrapper_nlpe_engine().vaporise_duckling_extr(name)
        pass
    elif model_save_mode == wrapper_util.SaveHelperMode.SAVE_MODEL:
        # Save the model blob.
        # success = wrapper_util.get_wrapper_nlpe_engine().save_duckling_extr(name)
        #
        # if not success:
        #     return False
        pass
    ...
    meta_info = wrapper_util.duckling_extr_dict[name]  # At this point one can assume that the key exists.

    meta_info.uuid = str(uuid.uuid4())  # Update the uuid of the local copy.

    success = wrapper_util.save_meta_info_blob(name + '.duckling_extr_meta_pickle', meta_info,
...
def duckling_extr_upgrade_meta_info(meta_info):
    ...
    if upgrade_meta_info:
        logging.debug(f"    entity_wrapper_duckling.duckling_extr_load_helper: Upgrading meta_info.")
        meta_info = wrapper_util.DucklingExtractorMeta(meta_info.uuid,
                                                       meta_info.long_name,
                                                       meta_info.desc,
                                                       meta_info_readonly,
                                                       meta_info.reference_time)
    return meta_info


def duckling_extr_load_history_helper(name: str,
    ...
    if meta_info is None:
        ...
        logging.error(f"duckling_extr_load_history_helper: Model history meta_info for {name} is not available!")
        duckling_extr_trash_helper(name, trash_cache_only=True)
        return False  # 400, {"error_detail": "Loading of model meta data failed!"}

    if not isinstance(meta_info, wrapper_util.DucklingExtractorMeta):
        logging.error(f"duckling_extr_load_history_helper: Loaded model meta_info of {name} of incorrect class!")
    ...
    if wrapper_util.duckling_extr_dict.get(name) is not meta_info:
        meta_info = duckling_extr_upgrade_meta_info(meta_info)
        wrapper_util.duckling_extr_dict[name] = meta_info

        # Note: If a revision was retrieved from history then the model needs to be RETRAINED! So trash the cached model.
        # wrapper_util.get_nlpe_engine().trash_duckling_extr(name, trash_cache_only=True)
...
def duckling_extr_load_helper(name: str,
    ...
    meta_info = wrapper_util.load_meta_info_blob(name=name + '.duckling_extr_meta_pickle',
                                                 look_in_trash=look_in_trash,
                                                 cached_meta_blob=wrapper_util.duckling_extr_dict.get(name))
    ...
    if wrapper_util.duckling_extr_dict.get(name) is not meta_info:
        ...
        # wrapper_util.get_wrapper_nlpe_engine().load_duckling_extr(name)

    return True


def duckling_extr_trash_helper(name: str, trash_cache_only: bool = False) -> bool:
    """ Trash the meta info and model info + manage cache. """

    if wrapper_util.duckling_extr_dict.get(name) is not None:
        del wrapper_util.duckling_extr_dict[name]

    if not trash_cache_only:
        wrapper_util.trash_meta_info_blob(name + '.duckling_extr_meta_pickle')

    # wrapper_util.get_wrapper_nlpe_engine().trash_duckling_extr(name, trash_cache_only=trash_cache_only)

    return True


def duckling_extr_vaporise_helper(name: str) -> bool:
    """ Vaporise the meta info and model info + manage cache. """

    if wrapper_util.duckling_extr_dict.get(name) is not None:
        del wrapper_util.duckling_extr_dict[name]

    wrapper_util.vaporise_meta_info_blob(name + '.duckling_extr_meta_pickle')
...
def duckling_extr_create(iname: str,
    ...
    if (load_from_store is True) or (revision_uuid is not None):
        ...
        if revision_uuid is not None:
            success = duckling_extr_load_history_helper(name, revision_uuid)
        else:
            success = duckling_extr_load_helper(name, look_in_trash=True)

        meta_info = wrapper_util.duckling_extr_dict.get(name)
        ...
        return 200, {"name": iname,
                     ...
                     "reference_time": meta_info.reference_time}
    else:
        # === See if there already exists a model with the same name ===
        success = duckling_extr_load_helper(name, look_in_trash=False)
        meta_info = wrapper_util.duckling_extr_dict.get(name)
        ...
        wrapper_util.duckling_extr_dict[name] = wrapper_util.DucklingExtractorMeta("",
                                                                                   long_name,
                                                                                   description,
                                                                                   False,
                                                                                   reference_time)
        ...
        success = duckling_extr_save_helper(name, wrapper_util.SaveHelperMode.SAVE_MODEL,
                                            save_in_history=True,
                                            save_in_history_message="Created model.")

        return 200, {"name": iname,
                     "id": wrapper_util.duckling_extr_dict[name].uuid,
                     "long_name": wrapper_util.duckling_extr_dict[name].long_name,
                     "desc": wrapper_util.duckling_extr_dict[name].desc,
                     "readonly": wrapper_util.duckling_extr_dict[name].readonly,
                     "reference_time": wrapper_util.duckling_extr_dict[name].reference_time}
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_del(iname: str, auth_token: str, caller_name: Optional[str],
                      vaporise: bool) -> Tuple[int, wrapper_util.JSONType]:
    """ Trash/vaporise the duckling extractor model. """
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.

    name = iname + "_" + auth_token

    duckling_extr_details = {}  # type: Dict
    ...
    duckling_extr_details = {"name": name[:-(len(auth_token) + 1)],
                             "id": meta_info.uuid,
                             "long_name": meta_info.long_name,
                             "desc": meta_info.desc,
                             "readonly": meta_info.readonly,
    ...
    if not vaporise:
        duckling_extr_trash_helper(name)
    else:
        duckling_extr_vaporise_helper(name)

    return 200, duckling_extr_details


@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_get_details(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
    ...
    duckling_extr_details = {"name": iname,
                             "id": meta_info.uuid,
                             "long_name": meta_info.long_name,
                             "desc": meta_info.desc,
                             "readonly": meta_info.readonly,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_get_details_all(auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    ...
    for model_info in model_meta_info_list:
        ...
        if name.endswith('.duckling_extr_meta_pickle'):
            meta_info = model_info[1]  # type: wrapper_util.DucklingExtractorMeta
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_retrieve(iname: str, auth_token: str, caller_name: Optional[str],
    ...
    # List[Dict]
    result_list = wrapper_util.get_nlpe_engine().retrieve_duckling_entities(text, "",
                                                                            reference_time=meta_info.reference_time)
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_set_params(iname: str, auth_token: str, caller_name: Optional[str],
    ...
    reference_time = param_dict.get('reference_time')
    ...
    if reference_time is not None:
        meta_info.reference_time = reference_time

    wrapper_util.duckling_extr_dict[name] = meta_info

    # Save the named meta_info blob. MIGHT modify the meta_info!
    success = duckling_extr_save_helper(name, wrapper_util.SaveHelperMode.DONT_SAVE_MODEL,
                                        save_in_history=True,
...
@wrapper_util.lock_decorator_nlpe
@wrapper_util.auth_decorator
def duckling_extr_get_params(iname: str, auth_token: str, caller_name: Optional[str]) -> Tuple[int, wrapper_util.JSONType]:
    if iname is None:
        return 400, {"error_detail": "No instance name provided!"}  # Bad/Malformed request.
