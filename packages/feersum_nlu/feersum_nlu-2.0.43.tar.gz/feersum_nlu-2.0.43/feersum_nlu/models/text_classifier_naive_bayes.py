"""
FeersumNLU Classifier: Naive Bayes.
"""
...
# class BPEAnalyzer(object):
#     def __init__(self, training_samples: List[str]):
#         with open('_BPEAnalyzer_samples.txt', 'wt') as f:
#             for sample in training_samples:
#                 f.write(sample + '\n')
#
#         spm.SentencePieceTrainer.Train(
#             '--input=_BPEAnalyzer_samples.txt --model_type=bpe --model_prefix=_BPEAnalyzer_model --vocab_size=7500')
#
#         self._sp = spm.SentencePieceProcessor()
#         self._sp.Load("_BPEAnalyzer_model.model")
#
#     def __call__(self, doc):
#         return self._sp.EncodeAsPieces(doc)


class TextClassifierNB(text_classifier_base.TextClassifierBase):
    """
    FeersumNLU Classifier: Naive Bayes.
    """
    ...
    def train(self,
        ...
        _word_ngram_vectorizer = CountVectorizer(analyzer='word', ngram_range=(1, 2), max_features=500000,
                                                 lowercase=True)
        _char_2gram_vectorizer = CountVectorizer(analyzer='char_wb', ngram_range=(2, 2), max_features=500000,
                                                 lowercase=True)
        _char_4gram_vectorizer = CountVectorizer(analyzer='char_wb', ngram_range=(4, 4), max_features=500000,
                                                 lowercase=True)
        _char_6gram_vectorizer = CountVectorizer(analyzer='char_wb', ngram_range=(6, 6), max_features=500000,
                                                 lowercase=True)

        # bpe_training_samples = []  # type: List[str]
        # for sample in training_list:
        #     bpe_training_samples.append(sample[0])
        # for sample in testing_list:
        #     bpe_training_samples.append(sample[0])
        #
        # bpe_analyzer = BPEAnalyzer(bpe_training_samples)
        # _bpe_vectorizer = CountVectorizer(analyzer=bpe_analyzer, ngram_range=(1, 1), max_features=1000000,
        #                                   lowercase=True)
