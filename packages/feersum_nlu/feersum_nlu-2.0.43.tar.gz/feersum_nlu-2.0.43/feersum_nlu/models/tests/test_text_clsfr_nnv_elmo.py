...
class TestTextClassifierNNVCosineElmo(BaseTestCase):
    ...
    def test(self):
        print("Testing TestTextClassifierNNVCosineElmo.test ...", flush=True)

        print("Loading language model...", end='', flush=True)
        language_model = \
            sentence_encoder_elmo.SentenceEncoderElmo("https://tfhub.dev/google/elmo/2?"
        ...
        if matcher is not None:
            ...
            print("len(matcher.get_labels()) =", len(matcher.get_labels()))
            ...
            for query in queries:
                scored_labels, lang_code = matcher.classify(input_text=query,
                                                            lang_code_hint=None, weak_match_threshold=1.0,
                                                            top_n=topn)
            ...
            for idx, scored_label in enumerate(sanity_scored_labels):
                print(idx, "label =", scored_label[0])
                print(idx, "prob =", scored_label[1])
