    ...
    def cruise(self, exit, feedback=None, thruster=None, **entrance):
        ...
        for k in entrance.keys():
            if isinstance(entrance[k], np.ndarray):
                ent.append(torch.from_numpy(entrance[k]).to(self.sc.engine.context))
        ...
        if thruster is None:
            ...
            self.sc.layout[thruster].optimizer.zero_grad()
            self.sc.out_node = exit + (thruster,)
            self.sc.train()
            ext = self.sc(*tuple(ent))
            ext[feedback].backward()
            ext[thruster].optimizer.step()

        ret = {}
        for key in exit:
            if ext[key].size == 1:
                ret[key] = ext[key].detach().numpy()[0]
            else:
                ret[key] = ext[key].detach().numpy()
        return ret
