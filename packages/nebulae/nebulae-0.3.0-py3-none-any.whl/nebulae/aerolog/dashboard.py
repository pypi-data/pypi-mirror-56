...
class DashBoard(object):
    ...
    def _log(self, gauge=True, tachograph=True):
        if gauge:
            boards = {}
            # clustering
            ...
            for k in boards.keys():
                for i, b in enumerate(boards[k]):
                    ...
                    plt.legend()
                    plt.grid(True)
                    plt.savefig(path.join(self.param['log_path'], '%s_%s_%.3g_mile_%d.jpg'
                ...
                if self.max_epoch > 0:
                    plt.legend()
                    plt.grid(True)
                    plt.savefig(path.join(self.param['log_path'], '%s_epoch_%d.jpg' % (k.replace('/', '-'), self.max_epoch)))
                    plt.close()
        if tachograph:
            for k in self.gauge_mile.keys():
                stage, metric = k.split(':')
                df = pd.DataFrame(data={'A': self.trail_mile[k], 'B': self.gauge_mile[k]})
                df.to_csv(path.join(self.param['log_path'], '%s_%s_mile.csv'%(metric.replace('/', '-'), stage)),
                          header=None, index=None)
                df = pd.DataFrame(data={'A': self.trail_epoch[k], 'B': self.gauge_epoch[k][:-1]})
                df.to_csv(path.join(self.param['log_path'], '%s_%s_epoch.csv' % (metric.replace('/', '-'), stage)),
                          header=None, index=None)