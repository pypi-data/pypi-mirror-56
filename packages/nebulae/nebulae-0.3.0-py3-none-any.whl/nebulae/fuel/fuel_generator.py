...
class FuelGenerator(object):
    ...
    def __init__(self, config=None, file_dir=None, file_list=None, dtype=None, is_seq=False):
        ...
        for dt in self.param['dtype']:
            if dt.strip('v') not in Law.VALID_DTYPE:
    ...
    def _write(self, dst_path, patch, data, info_keys, max_frames):
        ...
        for k, key in enumerate(info_keys):
            ...
            if dt.startswith('v'):
                sdt = h5py.special_dtype(vlen=dt[1:])
                hdf5.create_dataset(key, dtype=sdt, data=np.array(data[key]))
            else:
                hdf5[key] = np.array(data[key]).astype(dt)
    ...
    def generate(self, dst_path, height=0, width=0, channel=3, encode='JPEG', shards=1, keep_exif=True):
        ...
        duration = self._file2Byte(dst_path, height, width, channel, encode, shards, keep_exif)
        print('+' + (60 * '-') + '+')
