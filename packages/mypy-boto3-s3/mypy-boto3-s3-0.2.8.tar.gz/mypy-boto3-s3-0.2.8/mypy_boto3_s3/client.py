...
class Client(BaseClient):
    ...
    def abort_multipart_upload(
        ...
        To verify that all parts have been removed, so you don't get charged for the part storage,
        you should call the  ListParts operation and ensure the parts list is empty.
    ...
    def complete_multipart_upload(
        ...
        Multipart Upload request, you must provide the parts list. You must ensure the parts list is
        complete, this operation concatenates the parts you provide in the list. For each part in
        the list, you must provide the part number and the ``ETag`` value, returned after that part
        was uploaded.
        ...
        whitespace characters to keep the connection from timing out. Because a request could fail
        after the initial 200 OK response has been sent, it is important that you check the response
        body to determine whether the request succeeded.

        Note that if ``CompleteMultipartUpload`` fails, applications should be prepared to retry the
        failed requests. For more information, see `Amazon S3 Error Best Practices
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ETag** *(string) --*
              ...
              not an MD5 digest of the object data, it will contain one or more nonhexadecimal
              characters and/or will consist of less than 32 or more than 32 hexadecimal digits.

            - **ServerSideEncryption** *(string) --*

              If you specified server-side encryption either with an Amazon S3-managed encryption
              key or an AWS KMS customer master key (CMK) in your initiate multipart upload request,
              the response includes this header. It confirms the encryption algorithm that Amazon S3
            ...
            - **SSEKMSKeyId** *(string) --*
              ...
              If present, specifies the ID of the AWS Key Management Service (KMS) customer master
    ...
    def copy_object(
        ...
        .. note::
          ...
          object up to 5 GB in size in a single atomic operation using this API. However, for
          copying an object greater than 5 GB, you must use the multipart upload Upload Part - Copy
          API. For conceptual information, see `Copy Object Using the REST Multipart Upload API
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/S3_ACLs_UsingACLs.html>`__ .
        ...
        source object and *write* access to the destination bucket. For more information, see `REST
        Authentication <https://docs.aws.amazon.com/AmazonS3/latest/dev/RESTAuthentication.html>`__
        . Both the Region that you want to copy the object from and the Region that you want to copy
        the object to must be enabled for your account.

        To only copy an object under certain conditions, such as whether the Etag matches or whether
        the object was modified before or after a specified date, use the request parameters
        ...
        You can use this operation to change the storage class of an object that is already stored
        in Amazon S3 using the StorageClass parameter. For more information, see `Storage Classes
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/storage-class-intro.html>`__ .

        The source object that you are copying can be encrypted or unencrypted. If the source object
        is encrypted, it can be encrypted by server-side encryption using AWS-managed encryption
        keys or by using a customer-provided encryption key. When copying an object, you can request
        that Amazon S3 encrypt the target object by using either the AWS-managed encryption keys or
        by using your own encryption key. You can do this regardless of the form of server-side
        encryption that was used to encrypt the source, or even if the source object was not
        ...
        receive a standard Amazon S3 error. If the error occurs during the copy operation, the error
        response is embedded in the 200 OK response. This means that a ``200 OK`` response can
        contain either a success or an error. Design your application to parse the contents of the
        ...
        .. note::
          ...
          If the request is an HTTP 1.1 request, the response is chunk encoded. If it were not, it
          would not contain the content-length, and you would need to read the entire body.
        ...
        * Consideration 2 – If both of the x-amz-copy-source-if-none-match and
        ...
        destination object. For pricing information, see `Amazon S3 Pricing
        <https://aws.amazon.com/s3/pricing/>`__ .

        Following are other considerations when using ``CopyObject`` :

          Versioning

        By default, ``x-amz-copy-source`` identifies the current version of an object to copy. (If
        the current version is a delete marker, Amazon S3 behaves as if the object was deleted.) To
        copy a different version, use the ``versionId`` subresource.
        ...
        If the source object's storage class is GLACIER, then you must restore a copy of this object
        before you can use it as a source object for the copy operation. For more information, see .

          Access Permissions

        When copying an object, you can optionally specify the accounts or groups that should be
        granted specific permissions on the new object. There are two ways to grant the permissions
        using the request headers:

        * Specify a canned ACL with the ``x-amz-acl`` request header. For more information, see
        `Canned ACL <https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#CannedACL>`__
        .

        * Specify access permissions explicitly with the ``x-amz-grant-read`` ,
        ``x-amz-grant-read-acp`` , ``x-amz-grant-write-acp`` , and ``x-amz-grant-full-control``
        headers. These parameters map to the set of permissions that Amazon S3 supports in an ACL.
        For more information, see `Access Control List (ACL) Overview
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html>`__ .
        ...
        To encrypt the target object, you must provide the appropriate encryption-related request
        headers. The one you use depends on whether you want to use AWS-managed encryption keys or
        ...
        * To encrypt the target object using server-side encryption with an AWS-managed encryption
        ...
        .. note::
          ...
          encryption-aws-kms-key-id, Amazon S3 uses the AWS managed customer master key (CMK) in KMS
          to protect the data.
        ...
        * If the source object is encrypted using server-side encryption with customer-provided
        ...
        You also can use the following access control–related headers with this operation. By
        default, all objects are private. Only the owner has full access control. When adding a new
        object, you can grant permissions to individual AWS accounts or to predefined groups defined
        by Amazon S3. These permissions are then added to the Access Control List (ACL) on the
        object. For more information, see `Using ACLs
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/S3_ACLs_UsingACLs.html>`__ . With this
        ...
        * Specify access permissions explicitly — To explicitly grant access permissions to specific
        AWS accounts or groups, use the following headers. Each header maps to specific permissions
        that Amazon S3 supports in an ACL. For more information, see `Access Control List (ACL)
        Overview <https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html>`__ . In the
        header, you specify a list of grantees who get the specific permission. To grant permissions
        explicitly use:
        ...
        **Request Syntax**
        ...
        :param CopySource: **[REQUIRED]** The name of the source bucket, key name of the source
        object, and optional version ID of the source object.  You can either provide this value as
        a string or a dictionary.  The string form is {bucket}/{key} or
        {bucket}/{key}?versionId={versionId} if you want to copy a specific version.  You can also
        provide this value as a dictionary.  The dictionary format is recommended over the string
        format because it is more explicit.  The dictionary format is: {'Bucket': 'bucket', 'Key':
        'key', 'VersionId': 'id'}.  Note that the VersionId key is optional and may be omitted.
        ...
        :param SSECustomerKeyMD5:
          ...
          uses this header for a message integrity check to ensure the encryption key was
        ...
        :param Tagging:
          ...
          The tag-set for the object destination object this value must be used in conjunction with
          the TaggingDirective. The tag-set must be encoded as URL Query parameters
          ...
          - *(dict) --*
            ...
            - **CopyObjectResult** *(dict) --*
              ...
              - **ETag** *(string) --*
                ...
                Returns the ETag of the new object. The ETag reflects only changes to the contents
                of an object, not its metadata. The source and destination ETag is identical for a
    ...
    def create_bucket(
        ...
        minimize costs, or address regulatory requirements. For example, if you reside in Europe,
        you will probably find it advantageous to create buckets in the EU (Ireland) region. For
        more information, see `How to Select a Region for Your Buckets
        ...
        .. note::
          ...
          the us-east-1 region. Accordingly, the signature calculations in Signature Version 4 must
          use us-east-1 as region, even if the location constraint in the request specifies another
          region where the bucket is to be created. If you create a bucket in a region other than US
          East (N. Virginia) region, your application must be able to handle 307 redirect. For more
        ...
        * Specify access permissions explicitly using the ``x-amz-grant-read`` ,
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html>`__ . You specify each
        grantee as a type=value pair, where the type is one of the following:
        ...
        ::
          ...
          response = client.create_bucket(
              ...
              CreateBucketConfiguration={
                  'LocationConstraint':
                  'EU'|'eu-west-1'|'us-west-1'|'us-west-2'|'ap-south-1'|'ap-southeast-1'
    ...
    def create_multipart_upload(
        ...
        If you have configured a lifecycle rule to abort incomplete multipart uploads, the upload
        must complete within the number of days specified in the bucket lifecycle configuration.
        Otherwise, the incomplete multipart upload becomes eligible for an abort operation and
        Amazon S3 aborts the multipart upload. For more information, see `Aborting Incomplete
        ...
        multipart upload requests. For more information about signing, see `Authenticating Requests
        (AWS Signature Version 4)
        <https://docs.aws.amazon.com/AmazonS3/latest/API/sig-v4-authenticating-requests.html>`__ .
        ...
        .. note::
          ...
          Amazon S3 frees up the space used to store the parts and stop charging you for storing
        ...
        KMS) customer master keys (CMKs) or Amazon S3-managed encryption keys. If you choose to
        provide your own encryption key, the request headers you provide in  UploadPart ) and
        UploadPartCopy ) requests must match the headers you used in the request to initiate the
        ...
        permission to the ``kms:Encrypt`` , ``kms:Decrypt`` , ``kms:ReEncrypt*`` ,
        ``kms:GenerateDataKey*`` , and ``kms:DescribeKey`` actions on the key. These permissions are
        required because Amazon S3 must decrypt and read data from the encrypted file parts before
        it completes the multipart upload.

        If your AWS Identity and Access Management (IAM) user or role is in the same AWS account as
        the AWS KMS CMK, then you must have these permissions on the key policy. If your IAM user or
        role belongs to a different account than the key, then you must have the permissions on both
        ...
        * Specify access permissions explicitly with the ``x-amz-grant-read`` ,
        ...
        writes it to disks in its data centers and decrypts it when you access it. The option you
        use depends on whether you want to use AWS-managed encryption keys or provide your own
        encryption key.

        * Use encryption keys managed by Amazon S3 or customer master keys (CMKs) stored in Amazon
        Key Management Service (KMS) – If you want AWS to manage the keys used to encrypt data,
        ...
        .. note::
          ...
          If you specify x-amz-server-side-encryption:aws:kms, but don't provide x-amz-server-side-
          encryption-aws-kms-key-id, Amazon S3 uses the AWS managed CMK in AWS KMS to protect the
          data.
    ...
    def delete_bucket_encryption(self, Bucket: str) -> None:
        ...
        Encryption <https://docs.aws.amazon.com/AmazonS3/latest/dev//bucket-encryption.html>`__ in
        the *Amazon Simple Storage Service Developer Guide* .

        To use this operation, you must have permissions to perform the
        ``s3:PutEncryptionConfiguration`` action. The bucket owner has this permission by default.
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev//using-with-s3-actions.html#using-with-s3-actions-related-to-bucket-subresources>`__
        and `Managing Access Permissions to your Amazon S3 Resources
        <https://docs.aws.amazon.com/AmazonS3/latest/dev//s3-access-control.html>`__ in the *Amazon
    ...
    def delete_bucket_lifecycle(self, Bucket: str) -> None:
        ...
        ``s3:PutLifecycleConfiguration`` action. By default, the bucket owner has this permission
        and the bucket owner can grant this permission to others.
    ...
    def delete_bucket_policy(self, Bucket: str) -> None:
        ...
        Denied`` error. If you have the correct permissions, but you're notusing an identity that
        ...
        .. warning::
          ...
          use this operation, even if the policy explicitly denies the root user the ability to
          perform this action.
    ...
    def delete_bucket_replication(self, Bucket: str) -> None:
        ...
        and can grant it to others. For more information about permissions, see `Permissions Related
    ...
    def delete_bucket_website(self, Bucket: str) -> None:
        ...
        OK`` response upon successfully deleting a website configuration on the specified bucket.
        You will get a ``200 OK`` response if the website configuration you are trying to delete
        does not exist on the bucket. Amazon S3 returns a ``404`` response if the bucket specified
        in the request does not exist.

        This DELETE operation requires the ``S3:DeleteBucketWebsite`` permission. By default, only
        the bucket owner can delete the website configuration attached to a bucket. However, bucket
        owners can grant other users permission to delete the website configuration by writing a
        bucket policy granting them the ``S3:DeleteBucketWebsite`` permission.
    ...
    def delete_object(
        ...
        subresource. Using this subresource permanently deletes the version. If the object deleted
        is a delete marker, Amazon S3 sets the response header, x-amz-delete-marker, to true.
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingMFADelete.html>`__ . To see sample
        ...
        lifecycle ( PutBucketLifecycle ) to enable Amazon S3 to remove them for you. If you want to
        block users or accounts from removing or deleting objects from your bucket you must deny
        them the s3:DeleteObject, s3:DeleteObjectVersion and s3:PutLifeCycleConfiguration actions.
        ...
        **Request Syntax**
        ...
        :param MFA:
          ...
          versionedobject if versioning is configured with MFA Deleteenabled.
    ...
    def delete_objects(
        ...
        operation encountered an error. For a successful deletion, the operation does not return any
        ...
        request will fail, even if there are non versioned objects you are attempting to delete. If
        you provide an invalid token, whether there are versioned keys in the request or not, the
        ...
        uses the header value to ensure that your request body has not be altered in transit.
            ...
            - **Deleted** *(list) --*
              ...
              - *(dict) --*
                ...
                - **DeleteMarker** *(boolean) --*
                  ...
                  Specifies whether the versioned object that was permanently deleted was (true) or
                  was not (false) a delete marker. In a simple DELETE, this header indicates whether
            ...
            - **Errors** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Code** *(string) --*
                  ...
                  meant to be read and understood by programs that detect and handle errors by type.
                  ...
                  *
                    ...
                    * *Description:* The AWS access key ID you provided does not exist in our
                    records.
                  ...
                  *
                    ...
                    * *Description:* Indicates that the version ID specified in the request does not
                  ...
                  *
                    ...
                    * *Description:* The bucket you are attempting to access must be addressed using
                  ...
                  *
                    ...
                    * *Description:* The request signature we calculated does not match the
                    ...
                    <https://docs.aws.amazon.com/AmazonS3/latest/dev/RESTAuthentication.html>`__ and
                    `SOAP Authentication
                ...
                - **Message** *(string) --*
                  ...
                  English. It is intended for a human audience. Simple programs display the message
                  directly to the end user if they encounter an error condition they don't know how
                  or don't care to handle. Sophisticated programs with more exhaustive error
    ...
    def get_bucket_accelerate_configuration(
        ...
        Amazon S3 Transfer Acceleration is a bucket-level feature that enables you to perform faster
        ...
        acceleration state. A bucket has no Transfer Acceleration state, if a state has never been
        set on the bucket.

        For more information on transfer acceleration, see `Transfer Acceleration
        <https://docs.aws.amazon.com/AmazonS3/latest/dev//transfer-acceleration.html>`__ in the
    ...
    def get_bucket_acl(self, Bucket: str) -> ClientGetBucketAclResponseTypeDef:
        ...
        access control list (ACL) of a bucket. To use ``GET`` to return the ACL of the bucket, you
        must have ``READ_ACP`` access to the bucket. If ``READ_ACP`` permission is granted to the
        anonymous user, you can return the ACL of the bucket without using an authorization header.
    ...
    def get_bucket_analytics_configuration(
        ...
        To use this operation, you must have permissions to perform the
        ``s3:GetAnalyticsConfiguration`` action. The bucket owner has this permission by default.
        The bucket owner can grant this permission to others. For more information about
        permissions, see `Permissions Related to Bucket Subresource Operations
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html#using-with-s3-actions-related-to-bucket-subresources>`__
        and `Managing Access Permissions to Your Amazon S3 Resources
          ...
          - *(dict) --*
            ...
            - **AnalyticsConfiguration** *(dict) --*
              ...
              - **Filter** *(dict) --*
                ...
                exactly one prefix, one tag, or one conjunction (AnalyticsAndOperator). If no filter
    ...
    def get_bucket_cors(self, Bucket: str) -> ClientGetBucketCorsResponseTypeDef:
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/cors.html>`__ Enabling Cross-Origin
        Resource Sharing.
    ...
    def get_bucket_encryption(self, Bucket: str) -> ClientGetBucketEncryptionResponseTypeDef:
        ...
        To use this operation, you must have permission to perform the
        ``s3:GetEncryptionConfiguration`` action. The bucket owner has this permission by default.
        The bucket owner can grant this permission to others. For more information about
        permissions, see `Permissions Related to Bucket Subresource Operations
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html#using-with-s3-actions-related-to-bucket-subresources>`__
        and `Managing Access Permissions to Your Amazon S3 Resources
    ...
    def get_bucket_lifecycle(self, Bucket: str) -> ClientGetBucketLifecycleResponseTypeDef:
        ...
        .. warning::
          ...
          For an updated version of this API, see  GetBucketLifecycleConfiguration . If you
          configured a bucket lifecycle using the ``filter`` element, you should the updated version
        ...
        To use this operation, you must have permission to perform the
        ``s3:GetLifecycleConfiguration`` action. The bucket owner has this permission by default.
        The bucket owner can grant this permission to others. For more information about
        permissions, see `Permissions Related to Bucket Subresource Operations
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html#using-with-s3-actions-related-to-bucket-subresources>`__
        and `Managing Access Permissions to Your Amazon S3 Resources
          ...
          - *(dict) --*
            ...
            - **Rules** *(list) --*
              ...
              - *(dict) --*
                ...
                Specifies lifecycle rules for an Amazon S3 bucket. For more information, see `PUT
                Bucket lifecycle
                <https://docs.aws.amazon.com/AmazonS3/latest/API/RESTBucketPUTlifecycle.html>`__ in
                ...
                - **NoncurrentVersionTransition** *(dict) --*
                  ...
                  - **NoncurrentDays** *(integer) --*
                    ...
                    <https://docs.aws.amazon.com/AmazonS3/latest/dev/s3-access-control.html>`__ in
                    the *Amazon Simple Storage Service Developer Guide* .
                ...
                - **AbortIncompleteMultipartUpload** *(dict) --*
                  ...
                  Amazon S3 will wait before permanently removing all parts of the upload. For more
    ...
    def get_bucket_lifecycle_configuration(
        ...
        .. note::
          ...
          you are still using previous version of the lifecycle configuration, it works. For the
            ...
            - **Rules** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Filter** *(dict) --*
                  ...
                  must have exactly one of Prefix, Tag, or And specified.
            ...
            - **LoggingEnabled** *(dict) --*
              ...
              *Amazon Simple Storage Service API Reference* .
              ...
              - **TargetBucket** *(string) --*
                ...
                the same target bucket. In this case you should choose a different TargetPrefix for
              ...
              - **TargetPrefix** *(string) --*
                ...
                buckets in a single bucket, you can use a prefix to distinguish which log files came
                from which bucket.
          ...
          - *(dict) --*
            ...
            - **MetricsConfiguration** *(dict) --*
              ...
              - **Filter** *(dict) --*
                ...
                include objects that meet the filter's criteria. A filter must be a prefix, a tag,
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **TopicConfiguration** *(dict) --*
              ...
              This data type is deperecated. A container for specifying the configuration for
              publication of messages to an Amazon Simple Notification Service (Amazon SNS) topic
              when Amazon S3 detects specified events.

              - **Id** *(string) --*

                An optional unique identifier for configurations in a notification configuration. If
                you don't provide one, Amazon S3 will assign an ID.
              ...
              - **Topic** *(string) --*
                ...
                Amazon SNS topic to which Amazon S3 will publish a message to report the specified
                events for the bucket.

            - **QueueConfiguration** *(dict) --*

              This data type is deprecated. This data type specifies the configuration for
              publishing messages to an Amazon Simple Queue Service (Amazon SQS) queue when Amazon
            ...
            - **CloudFunctionConfiguration** *(dict) --*
              ...
              - **CloudFunction** *(string) --*
                ...
                Lambda cloud function ARN that Amazon S3 can invoke when it detects events of the
                specified type.
    ...
    def get_bucket_notification_configuration(
        ...
        For more information about setting and reading the notification configuration on a bucket,
        see `Setting Up Notification of Bucket Events
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/NotificationHowTo.html>`__ . For more
        information about bucket policies, see `Using Bucket Policies
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-iam-policies.html>`__ .
    ...
    def get_bucket_policy_status(self, Bucket: str) -> ClientGetBucketPolicyStatusResponseTypeDef:
        ...
        a Policy <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html>`__ .
    ...
    def get_bucket_replication(self, Bucket: str) -> ClientGetBucketReplicationResponseTypeDef:
        ...
        .. note::
          ...
          It can take a while to propagate the put or delete a replication configuration to all
          Amazon S3 systems. Therefore, a get request soon after put or delete can return a wrong
          result.

        For information about replication configuration, see `Replication
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/replication.html>`__ .

        This operation requires permissions for the ``s3:GetReplicationConfiguration`` action. For
        more information about permissions, see `Using Bucket Policies and User Policies
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-iam-policies.html>`__ .

        If you include the ``Filter`` element in a replication configuration, you must also include
        the ``DeleteMarkerReplication`` and ``Priority`` elements. The response also returns those
        elements.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ReplicationConfiguration** *(dict) --*
              ...
              A container for replication rules. You can add up to 1,000 rules. The maximum size of
              a replication configuration is 2 MB.

              - **Role** *(string) --*

                The Amazon Resource Name (ARN) of the AWS Identity and Access Management (IAM) role
                that Amazon S3 assumes when replicating objects. For more information, see `How to
                Set Up Replication
                <https://docs.aws.amazon.com/AmazonS3/latest/dev/replication-how-setup.html>`__ in
                the *Amazon Simple Storage Service Developer Guide* .
                ...
                - *(dict) --*
                  ...
                  - **Priority** *(integer) --*
                    ...
                    * Same object qualify tag based filter criteria specified in multiple rules
                    ...
                    https://docs.aws.amazon.com/AmazonS3/latest/dev/replication.html>`__ in the
                    *Amazon S3 Developer Guide* .

                  - **Prefix** *(string) --*

                    An object keyname prefix that identifies the object or objects to which the rule
                    applies. The maximum prefix length is 1,024 characters. To include all objects
                    in a bucket, specify an empty string.
                  ...
                  - **ExistingObjectReplication** *(dict) --*
                    ...
                    A container that specifies information about existing object replication. You
                    can choose whether to enable or disable the replication of existing objects.
                  ...
                  - **Destination** *(dict) --*
                    ...
                    - **Account** *(string) --*
                      ...
                      destination bucket by specifying the ``AccessControlTranslation`` property,
                      this is the account ID of the destination bucket owner. For more information,
                      see `Replication Additional Configuration\\: Change Replica Owner
                      <https://docs.aws.amazon.com/AmazonS3/latest/dev/replication-change-owner.html>`__
                      in the *Amazon Simple Storage Service Developer Guide* .
                    ...
                    - **Metrics** *(dict) --*
                      ...
                      addition, contains configurations related to specific metrics or events. Must
                      be specified together with a ``ReplicationTime`` block.
                  ...
                  - **DeleteMarkerReplication** *(dict) --*
                    ...
                    contain only <Status>Disabled</Status>. For an example configuration, see `Basic
                    Rule Configuration
    ...
    def get_bucket_versioning(self, Bucket: str) -> ClientGetBucketVersioningResponseTypeDef:
        ...
        This implementation also returns the MFA Delete status of the versioning state, i.e., if the
        MFA Delete status is ``enabled`` , the bucket owner must use an authentication device to
    ...
    def get_bucket_website(self, Bucket: str) -> ClientGetBucketWebsiteResponseTypeDef:
        ...
        This GET operation requires the ``S3:GetBucketWebsite`` permission. By default, only the
            ...
            - **RoutingRules** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Condition** *(dict) --*
                  ...
                  to apply. For example, 1. If request is for pages in the ``/docs`` folder,
                  ...
                  - **KeyPrefixEquals** *(string) --*
                    ...
                    the docs/ folder. Required when the parent element ``Condition`` is specified
                    and sibling ``HttpErrorCodeReturnedEquals`` is not specified. If both conditions
                ...
                - **Redirect** *(dict) --*
                  ...
                  - **ReplaceKeyPrefixWith** *(string) --*
                    ...
                    requests for all pages with prefix ``docs/`` (objects in the ``docs/`` folder)
                    to ``documents/`` , you can set a condition block with ``KeyPrefixEquals`` set
                    to ``docs/`` and in the Redirect set ``ReplaceKeyPrefixWith`` to ``/documents``
    ...
    def get_object(
        ...
        object. If you grant ``READ`` access to the anonymous user, you can return the object
        without using an authorization header.

        An Amazon S3 bucket has no directory hierarchy such as you would find in a typical computer
        file system. You can, however, create a logical hierarchy by using object key names that
        imply a folder structure. For example, instead of naming an object ``sample.jpg`` , you can
        name it ``photos/2006/February/sample.jpg`` .

        To get an object from such a logical hierarchy, specify the full key name for the object in
        the ``GET`` operation. For a virtual hosted-style request example, if you have the object
        ...
        object ``photos/2006/February/sample.jpg`` in the bucket named examplebucket, specify the
        resource as ``/examplebucket/photos/2006/February/sample.jpg`` . For more information about
        ...
        For more information, see `Amazon S3 Torrent
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/S3Torrent.html>`__ . For more information
        about returning the ACL of an object, see  GetObjectAcl .

        If the object you are retrieving is stored in the GLACIER or DEEP_ARCHIVE storage classes,
        before you can retrieve the object you must first restore a copy using . Otherwise, this
        operation returns an ``InvalidObjectStateError`` error. For information about restoring
        ...
        GET requests if your object uses server-side encryption with CMKs stored in AWS KMS
        ...
        * x-amz-server-side​-encryption​-customer-key-MD5
        ...
        ``x-amz-tagging-count`` header that provides the count of number of tags associated with the
        object. You can use  GetObjectTagging to retrieve the tag set associated with an object.

         **Permissions**

        You need the ``s3:GetObject`` permission for this operation. For more information, see
        `Specifying Permissions in a Policy
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html>`__ . If the
        object you request does not exist, the error Amazon S3 returns depends on whether you also
        have the ``s3:ListBucket`` permission.

        * If you have the ``s3:ListBucket`` permission on the bucket, Amazon S3 will return an HTTP
        status code 404 ("no such key") error.

        * If you don’t have the ``s3:ListBucket`` permission, Amazon S3 will return an HTTP status
        ...
        For example, you might override the Content-Disposition response header value in your GET
        request.
        ...
        subset of the headers that Amazon S3 accepts when you create an object. The response headers
        that you can override for the GET response are ``Content-Type`` , ``Content-Language`` ,
        ``Expires`` , ``Cache-Control`` , ``Content-Disposition`` , and ``Content-Encoding`` . To
        override these header values in the GET response, you use the following request parameters.
        ...
        *  GetObjectAcl
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/s3-2006-03-01/GetObject>`_
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **WebsiteRedirectLocation** *(string) --*
              ...
              another object in the same bucket or to an external URL. Amazon S3 stores the value of
            ...
            - **StorageClass** *(string) --*
              ...
              all objects except for Standard storage class objects.
    ...
    def get_object_tagging(
        ...
        action. By default, the GET operation returns information about current version of an
        object. For a versioned bucket, you can have multiple versions of an object in your bucket.
        To retrieve tags of any other version, use the versionId query parameter. You also need
    ...
    def get_object_torrent(
        ...
        .. note::
          ...
          encrypted using server-side encryption with customer-provided encryption key.
    ...
    def get_public_access_block(self, Bucket: str) -> ClientGetPublicAccessBlockResponseTypeDef:
        """
        Retrieves the ``PublicAccessBlock`` configuration for an Amazon S3 bucket. In order to use
        this operation, you must have the ``s3:GetBucketPublicAccessBlock`` permission. For more
        ...
        .. warning::
          ...
          object, it checks the ``PublicAccessBlock`` configuration for both the bucket (or the
          bucket that contains the object) and the bucket owner's account. If the
          ``PublicAccessBlock`` settings are different between the bucket and the account, Amazon S3
          ...
          - *(dict) --*
            ...
            - **PublicAccessBlockConfiguration** *(dict) --*
              ...
              - **IgnorePublicAcls** *(boolean) --*
                ...
                this bucket. Setting this element to ``TRUE`` causes Amazon S3 to ignore all public
              ...
              - **RestrictPublicBuckets** *(boolean) --*
                ...
                services and authorized users within this account if the bucket has a public policy.
    ...
    def head_bucket(self, Bucket: str) -> None:
        ...
        it. The operation returns a ``200 OK`` if the bucket exists and you have permission to
        access it. Otherwise, the operation might return responses such as ``404 Not Found`` and
        ``403 Forbidden`` .
    ...
    def head_object(
        ...
        * x-amz-server-side​-encryption​-customer-key-MD5
        ...
        .. note::
          ...
          Encryption request headers, like ``x-amz-server-side-encryption`` , should not be sent for
          GET requests if your object uses server-side encryption with CMKs stored in AWS KMS
          (SSE-KMS) or server-side encryption with Amazon S3–managed encryption keys (SSE-S3). If
        ...
        * If you have the ``s3:ListBucket`` permission on the bucket, Amazon S3 will return a HTTP
        status code 404 ("no such key") error.

        * If you don’t have the ``s3:ListBucket`` permission, Amazon S3 will return a HTTP status
        code 403 ("access denied") error.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **Restore** *(string) --*
              ...
              RestoreObject or an archive copy is already restored.
            ...
            - **ServerSideEncryption** *(string) --*
              ...
              master key (CMK) or an Amazon S3-managed encryption key, the response includes this
              header with the value of the Server-side encryption algorithm used when storing this
            ...
            - **ReplicationStatus** *(string) --*
              ...
              (GetObject) or object metadata (HeadObject) from these buckets, Amazon S3 will return
              ...
              * If requesting object from the source bucket — Amazon S3 will return the
              x-amz-replication-status header if object in your request is eligible for replication.
              For example, suppose in your replication configuration you specify object prefix
              "TaxDocs" requesting Amazon S3 to replicate objects with key prefix "TaxDocs". Then
              any objects you upload with this key name prefix, for example "TaxDocs/document1.pdf",
              is eligible for replication. For any object request with this key name prefix Amazon
              S3 will return the x-amz-replication-status header with value PENDING, COMPLETED or
              FAILED indicating object replication status.

              * If requesting object from the destination bucket — Amazon S3 will return the
              x-amz-replication-status header with value REPLICA if object in your request is a
            ...
            - **ObjectLockLegalHoldStatus** *(string) --*
              ...
              returned if the requester has the ``s3:GetObjectLegalHold`` permission. This header is
              not returned if the specified version of this object has never had a legal hold
    ...
    def list_bucket_analytics_configurations(
        ...
        This operation supports list pagination and does not return more than 100 configurations at
        a time. You should always check the ``IsTruncated`` element in the response. If there are no
        more configurations to list, ``IsTruncated`` is set to false. If there are more
        configurations to list, ``IsTruncated`` is set to true, and there will be a value in
        ``NextContinuationToken`` . You use the ``NextContinuationToken`` value to continue the
        pagination of the list by passing the value in continuation-token in the request to ``GET``
        the next page.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **IsTruncated** *(boolean) --*
              ...
              of true indicates that the list is not complete and the NextContinuationToken will be
            ...
            - **NextContinuationToken** *(string) --*
              ...
              NextContinuationToken is sent when isTruncated is true, which indicates that there are
              more analytics configurations to list. The next request must include this
    ...
    def list_bucket_inventory_configurations(
        ...
        This operation supports list pagination and does not return more than 100 configurations at
        a time. Always check the ``IsTruncated`` element in the response. If there are no more
        configurations to list, ``IsTruncated`` is set to false. If there are more configurations to
        list, ``IsTruncated`` is set to true, and there is a value in ``NextContinuationToken`` .
        You use the ``NextContinuationToken`` value to continue the pagination of the list by
          ...
          - *(dict) --*
            ...
            - **InventoryConfigurationList** *(list) --*
              ...
              - *(dict) --*
                ...
                see `GET Bucket inventory
                <https://docs.aws.amazon.com/AmazonS3/latest/API/RESTBucketGETInventoryConfig.html>`__
                in the *Amazon Simple Storage Service API Reference* .
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextContinuationToken** *(string) --*
              ...
              Use the NextContinuationToken from a previously truncated list response to continue
            ...
            - **MetricsConfigurationList** *(list) --*
              ...
              - *(dict) --*
                ...
                the metrics configuration ID) from an Amazon S3 bucket. If you're updating an
                ...
                erased. For more information, see `PUT Bucket metrics
    ...
    def list_multipart_uploads(
        ...
        ``max-uploads`` parameter in the response. If additional multipart uploads satisfy the list
        criteria, the response will contain an ``IsTruncated`` element with the value true. To list
        the additional multipart uploads, use the ``key-marker`` and ``upload-id-marker`` request
        parameters.

        In the response, the uploads are sorted by key. If your application has initiated more than
        ...
        **Request Syntax**
        ...
        :param Delimiter:
          ...
          ``CommonPrefixes`` . If you don't specify the prefix parameter, then the substring starts
          at the beginning of the key. The keys that are grouped under ``CommonPrefixes`` result
          ...
          - *(dict) --*
            ...
            - **CommonPrefixes** *(list) --*
              ...
              - *(dict) --*
                ...
                the string specified by a delimiter. CommonPrefixes lists keys that act like
                subdirectories in the directory specified by Prefix. For example, if the prefix is
                notes/ and the delimiter is a slash (/) as in notes/summer/july, the common prefix
                is notes/summer/.
            ...
            - **EncodingType** *(string) --*
              ...
              If you specify ``encoding-type`` request parameter, Amazon S3 includes this element in
    ...
    def list_object_versions(
        ...
        **Request Syntax**
        ...
        :param Delimiter:
          ...
          string between the ``prefix`` and the first occurrence of the delimiter are grouped under
          a single result element in CommonPrefixes. These groups are counted as one result against
        ...
        :param MaxKeys:
          ...
          keys but will never contain more. If additional keys satisfy the search criteria, but were
        ...
        :param Prefix:
          ...
          Use this parameter to select only those keys that begin with the specified prefix. You can
          use prefixes to separate a bucket into different groupings of keys. (You can think of
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **IsTruncated** *(boolean) --*
              ...
              follow-up paginated request using the NextKeyMarker and NextVersionIdMarker response
            ...
            - **NextKeyMarker** *(string) --*
              ...
              first key not returned that satisfies the search criteria. Use this value for the
              key-marker request parameter in a subsequent request.
            ...
            - **Delimiter** *(string) --*
              ...
              The delimeter grouping the included keys. A delimiter is a character that you specify
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextMarker** *(string) --*
              ...
              you can use the key name in this field as marker in the subsequent request to get next
              set of objects. Amazon S3 lists objects in alphabetical order Note: This element is
              returned only if you have delimiter request parameter specified. If response does not
              include the NextMaker and it is truncated, you can use the value of the last Key in
              the response as the marker in the subsequent request to get the next set of object
              keys.
    ...
    def list_objects_v2(
        ...
        **Request Syntax**
        ...
        :param ContinuationToken:
          ...
          ContinuationToken indicates Amazon S3 that the list is being continued on this bucket with
          a token. ContinuationToken is obfuscated and is not a real key.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **IsTruncated** *(boolean) --*
              ...
              available to return. If the number of results exceeds that specified by MaxKeys, all
            ...
            - **MaxKeys** *(integer) --*
              ...
              Sets the maximum number of keys returned in the response. The response might contain
            ...
            - **CommonPrefixes** *(list) --*
              ...
              roll up into a common prefix count as a single return when calculating the number of
              returns.
            ...
            - **KeyCount** *(integer) --*
              ...
              less than equals to MaxKeys field. Say you ask for 50 keys, your result will include
            ...
            - **NextContinuationToken** *(string) --*
              ...
              in the bucket that can be listed. The next list requests to Amazon S3 can be continued
              with this NextContinuationToken. NextContinuationToken is obfuscated and is not a real
              key
    ...
    def list_parts(
        ...
        include the upload ID, which you obtain by sending the initiate multipart upload request
        (see  CreateMultipartUpload ). This request returns a maximum of 1,000 uploaded parts. The
        default number of parts returned is 1,000 parts. You can restrict the number of parts
        returned by specifying the ``max-parts`` request parameter. If your multipart upload
        consists of more than 1,000 parts, the response returns an ``IsTruncated`` field with the
        value of true, and a ``NextPartNumberMarker`` element. In subsequent ``ListParts`` requests
        ...
        *  ListMultipartUploads
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/s3-2006-03-01/ListParts>`_
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **AbortDate** *(datetime) --*
              ...
              If the bucket has a lifecycle rule configured with an action to abort incomplete
              multipart uploads and the prefix in the lifecycle rule matches the object name in the
              request, then the response includes this header indicating when the initiated
              multipart upload will become eligible for abort operation. For more information, see
              `Aborting Incomplete Multipart Uploads Using a Bucket Lifecycle Policy
              <https://docs.aws.amazon.com/AmazonS3/latest/dev/mpuoverview.html#mpu-abort-incomplete-mpu-lifecycle-config>`__
              .
            ...
            - **IsTruncated** *(boolean) --*
              ...
              the list was truncated. A list can be truncated if the number of parts exceeds the
            ...
            - **Initiator** *(dict) --*
              ...
              is an AWS account, this element provides the same information as the Owner element. If
              the initiator is an IAM User, then this element provides the user ARN and display
              name.
    ...
    def put_bucket_accelerate_configuration(
        ...
        a bucket-level feature that enables you to perform faster data transfers to Amazon S3.
        ...
        * Suspended – Disables accelerated data transfers to the bucket.
        ...
        The  GetBucketAccelerateConfiguration operation returns the transfer acceleration state of a
        bucket.

        After setting the Transfer Acceleration state of a bucket to Enabled, it might take up to
        thirty minutes before the data transfer rates to the bucket increase.
    ...
    def put_bucket_acl(
        ...
        information, see `Using ACLs
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/S3_ACLs_UsingACLs.html>`__ . To set the ACL
        of a bucket, you must have WRITE_ACP permission.
        ...
        * Specify permissions using request headers
        ...
        Depending on your application needs, you may choose to set the ACL on a bucket using either
        the request body or the headers. For example, if you have an existing application that
        updates a bucket ACL using the request body, then you can continue to use that approach.
        ...
        * Specify a canned ACL with the ``x-amz-acl`` request header. Amazon S3 supports a set of
        ...
        you cannot use other access control specific headers in your request. For more information,
        see `Canned ACL
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#CannedACL>`__ .

        * Specify access permissions explicitly with the ``x-amz-grant-read`` ,
        ``x-amz-grant-read-acp`` , ``x-amz-grant-write-acp`` , and ``x-amz-grant-full-control``
        headers. When using these headers you specify explicit access permissions and grantees (AWS
        accounts or a Amazon S3 groups) who will receive the permission. If you use these ACL
        specific headers, you cannot use x-amz-acl header to set a canned ACL. These parameters map
        to the set of permissions that Amazon S3 supports in an ACL. For more information, see
        ...
        * By Email address:  ``<Grantee xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:type=
            "AmazonCustomerByEmail"><EmailAddress><>Grantees@email.com<></EmailAddress>lt;/Grantee>``
    ...
    def put_bucket_analytics_configuration(
        ...
        are based on the object filters you configure. When selecting data export you specify a
        destination bucket and optional destination prefix where the file is written. You can export
        the data to a destination bucket in a different account. However, the destination bucket
        must be in the same region as the bucket that you are making the PUT analytics configuration
        to. For more information, see `Amazon S3 Analytics – Storage Class Analysis
        ...
        To use this operation, you must have permissions to perform the
        ``s3:PutAnalyticsConfiguration`` action. The bucket owner has this permission by default.
        The bucket owner can grant this permission to others. For more information about
        permissions, see `Permissions Related to Bucket Subresource Operations
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html#using-with-s3-actions-related-to-bucket-subresources>`__
        and `Managing Access Permissions to Your Amazon S3 Resources
        ...
        **Request Syntax**
        ...
        :param AnalyticsConfiguration: **[REQUIRED]**
          ...
          - **Filter** *(dict) --*
            ...
            one prefix, one tag, or one conjunction (AnalyticsAndOperator). If no filter is
            provided, all objects will be considered in any analysis.
    ...
    def put_bucket_cors(
        ...
        To enable cross-origin resource sharing (CORS) on a bucket, you add the ``cors`` subresource
        to the bucket. The ``cors`` subresource is an XML document in which you configure rules that
        identify origins and the HTTP methods that can be executed on your bucket. The document is
        ...
        **Request Syntax**
        ...
        :param CORSConfiguration: **[REQUIRED]**
          ...
          more information, see `Enabling Cross-Origin Resource Sharing
          <https://docs.aws.amazon.com/AmazonS3/latest/dev//cors.html>`__ in the Amazon Simple
    ...
    def put_bucket_encryption(
        ...
        """
        This implementation of the ``PUT`` operation uses the ``encryption`` subresource to set the
        ...
        server-side encryption with Amazon S3-managed keys SSE-S3 or AWS KMS customer master keys
        (CMKs) (SSE-KMS) bucket. For information about the Amazon S3 default encryption feature, see
        ...
        this action. in the *Amazon Simple Storage Service Developer Guide* .
        ...
        **Request Syntax**
        ...
        :param Bucket: **[REQUIRED]**
          ...
          S3-managed keys (SSE-S3) or customer master keys stored in AWS KMS (SSE-KMS). For
          information about the Amazon S3 default encryption feature, see `Amazon S3 Default Bucket
          Encryption <https://docs.aws.amazon.com/AmazonS3/latest/dev/bucket-encryption.html>`__ in
          the *Amazon Simple Storage Service Developer Guide* .
          ...
          - **Rules** *(list) --* **[REQUIRED]**
            ...
            - *(dict) --*
              ...
              - **ApplyServerSideEncryptionByDefault** *(dict) --*
                ...
                If a PUT Object request doesn't specify any server-side encryption, this default
                ...
                - **KMSMasterKeyID** *(string) --*
                  ...
                  KMS master key ID to use for the default encryption. This parameter is allowed if
    ...
    def put_bucket_inventory_configuration(
        ...
        """
        This implementation of the ``PUT`` operation adds an inventory configuration (identified by
        the inventory ID) to the bucket. You can have up to 1,000 inventory configurations per
        bucket.
        ...
        the *destination* bucket. The *destination* bucket must be in the same AWS Region as the
        *source* bucket.
        ...
        object versions or only current versions. For more information, see `Amazon S3 Inventory
        <https://docs.aws.amazon.com/AmazonS3/latest/dev//storage-inventory.html>`__ in the Amazon
        Simple Storage Service Developer Guide.
    ...
    def put_bucket_lifecycle(
        ...
        resource owner, the AWS account that created the resource, can access it. The resource owner
        can optionally grant access permissions to others by writing an access policy. For this
        ...
        permissions. If you want to prevent users or accounts from removing or deleting objects from
        ...
        * ``s3:PutLifecycleConfiguration``
        ...
        Resources <https://docs.aws.amazon.com/AmazonS3/latest/dev//s3-access-control.html>`__ in
        the *Amazon Simple Storage Service Developer Guide* .
            ...
            - *(dict) --*
              ...
              - **Expiration** *(dict) --*
                ...
                - **Date** *(datetime) --*
                  ...
                  Indicates at what date the object is to be moved or deleted. Should be in GMT ISO
                  8601 Format.
                ...
                - **ExpiredObjectDeleteMarker** *(boolean) --*
                  ...
                  Indicates whether Amazon S3 will remove a delete marker with no noncurrent
                  versions. If set to true, the delete marker will be expired; if set to false the
                  policy takes no action. This cannot be specified with Days or Date in a Lifecycle
                  Expiration Policy.
              ...
              - **NoncurrentVersionTransition** *(dict) --*
                ...
                to the ``STANDARD_IA`` , ``ONEZONE_IA`` , ``INTELLIGENT_TIERING`` , ``GLACIER`` , or
    ...
    def put_bucket_lifecycle_configuration(
        ...
        """
        Creates a new lifecycle configuration for the bucket or replaces an existing lifecycle
        configuration. For information about lifecycle configuration, see `Managing Access
        ...
        * Filter identifying a subset of objects to which the rule applies. The filter can be based
        on a key name prefix, object tags, or a combination of both.
        ...
        * One or more lifecycle transition and expiration actions that you want Amazon S3 to perform
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/object-lifecycle-mgmt.html>`__ and
        ...
        policy. For this operation, a user must get the s3:PutLifecycleConfiguration permission.
          ...
          - **Rules** *(list) --* **[REQUIRED]**
            ...
            - *(dict) --*
              ...
              - **NoncurrentVersionTransitions** *(list) --*
                ...
                objects transition to the a specific storage class. If your bucket is
                versioning-enabled (or versioning is suspended), you can set this action to request
                that Amazon S3 transition noncurrent object versions to the a specifc storage class
    ...
    def put_bucket_logging(
        ...
        * By Email address:  ``<Grantee xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        xsi:type=
            "AmazonCustomerByEmail"><EmailAddress><>Grantees@email.com<></EmailAddress></Grantee>``
        The grantee is resolved to the CanonicalUser and, in a response to a GET Object acl request,
        appears as the CanonicalUser.
        ...
        **Request Syntax**
        ...
        :param BucketLoggingStatus: **[REQUIRED]**
          ...
          - **LoggingEnabled** *(dict) --*
            ...
            keys for a bucket. For more information, see `PUT Bucket logging
            <https://docs.aws.amazon.com/AmazonS3/latest/API/RESTBucketPUTlogging.html>`__ in the
            *Amazon Simple Storage Service API Reference* .
            ...
            - **TargetBucket** *(string) --* **[REQUIRED]**
              ...
              Specifies the bucket where you want Amazon S3 to store server access logs. You can
              have your logs delivered to any bucket that you own, including the same bucket that is
              being logged. You can also configure multiple buckets to deliver their logs to the
              same target bucket. In this case you should choose a different TargetPrefix for each
        ...
        :param MetricsConfiguration: **[REQUIRED]**
          ...
          - **Filter** *(dict) --*
            ...
            - **And** *(dict) --*
              ...
              A conjunction (logical AND) of predicates, which is used in evaluating a metrics
              filter. The operator must have at least two predicates, and an object must match all
              of the predicates in order for the filter to apply.
        ...
        **Request Syntax**
        ...
        :param NotificationConfiguration: **[REQUIRED]**
          ...
          - **TopicConfiguration** *(dict) --*
            ...
            This data type is deperecated. A container for specifying the configuration for
            publication of messages to an Amazon Simple Notification Service (Amazon SNS) topic when
    ...
    def put_bucket_notification_configuration(
        ...
        Using this API, you can replace an existing notification configuration. The configuration is
        an XML file that defines the event types that you want Amazon S3 to publish and the
        destination where you want Amazon S3 to publish an event notification when it detects an
        ...
        that the bucket owner has permission to publish to it by sending a test notification. In the
        case of AWS Lambda destinations, Amazon S3 verifies that the Lambda function permissions
        grant Amazon S3 permission to invoke the function from the Amazon S3 bucket. For more
        information, see `Configuring Notifications for Amazon S3 Events
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/NotificationHowTo.html>`__ .

        You can disable notifications by adding the empty NotificationConfiguration element.

        By default, only the bucket owner can configure notifications on a bucket. However, bucket
        owners can use a bucket policy to grant permission to other users to set this configuration
        with ``s3:PutBucketNotification`` permission.
        ...
        .. note::
          ...
          topic. If the message fails, the entire PUT operation will fail, and Amazon S3 will not
          add the configuration to your bucket.

         **Responses**

        If the configuration in the request body includes only one ``TopicConfiguration`` specifying
        only the *s3:ReducedRedundancyLostObject* event type, the response will also include the
        *x-amz-sns-test-message-id* header containing the message ID of the test notification sent
        to topic.
        ...
        :param NotificationConfiguration: **[REQUIRED]**
          ...
          - **TopicConfigurations** *(list) --*
            ...
            - *(dict) --*
              ...
              Simple Notification Service (Amazon SNS) topic when Amazon S3 detects specified
              events.
    ...
    def put_bucket_policy(
        ...
        If you don't have ``PutBucketPolic`` y permissions, Amazon S3 returns a ``403 Access
    ...
    def put_bucket_replication(
        ...
        `Replication <https://docs.aws.amazon.com/AmazonS3/latest/dev/replication.html>`__ in the
        *Amazon S3 Developer Guide* .
        ...
        1,000. Each rule identifies a subset of objects to replicate by filtering the objects in the
        source bucket. To choose additional subsets of objects to replicate, add a rule for each
        subset. All rules must specify the same destination bucket.

        To specify a subset of the objects in the source bucket to apply a replication rule to, add
        the Filter element as a child of the Rule element. You can filter objects based on an object
        key prefix, one or more object tags, or both. When you add the Filter element in the
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html>`__ and
        `Managing Access Permissions to Your Amazon S3 Resources
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/s3-access-control.html>`__ .
        ...
        configuration, see `Replicating Objects Created with SSE Using CMKs stored in AWS KMS
        ...
        * Error code: ``InvalidRequest``
          ...
          * Description: If the <Owner> in <AccessControlTranslation> has a value, the <Account>
          ...
          - **Rules** *(list) --* **[REQUIRED]**
            ...
            - *(dict) --*
              ...
              - **Priority** *(integer) --*
                ...
                The priority associated with the rule. If you specify multiple rules in a
                replication configuration, Amazon S3 prioritizes the rules to prevent conflicts when
                filtering. If two or more rules identify the same object based on a specified
                filter, the rule with higher priority takes precedence. For example:
                ...
                * Same object qualify tag based filter criteria specified in multiple rules
                ...
                For more information, see `Replication <
                https://docs.aws.amazon.com/AmazonS3/latest/dev/replication.html>`__ in the *Amazon
                S3 Developer Guide* .
              ...
              - **SourceSelectionCriteria** *(dict) --*
                ...
                A container that describes additional filters for identifying the source objects
                that you want to replicate. You can choose to enable or disable the replication of
                these objects. Currently, Amazon S3 supports only the filter that you can specify
                for objects created with server-side encryption using a customer master key (CMK)
              ...
              - **Destination** *(dict) --* **[REQUIRED]**
                ...
                - **Account** *(string) --*
                  ...
                  bucket by specifying the ``AccessControlTranslation`` property, this is the
                ...
                - **AccessControlTranslation** *(dict) --*
                  ...
                  owners are not the same), and you want to change replica ownership to the AWS
                  account that owns the destination bucket. If this is not specified in the
    ...
    def put_bucket_tagging(
        ...
        to get your AWS account bill with tag key values included. Then, to see the cost of combined
        resources, organize your billing information according to resources with the same tag key
        values. For example, you can tag several resources with a specific application name, and
        then organize your billing information to see the total cost of that application across
        ...
        .. note::
          ...
          Bucket Tags <https://docs.aws.amazon.com/AmazonS3/latest/dev/CostAllocTagging.html>`__ .
        ...
        * Error code: ``InvalidTagError``
          ...
          * Description: The tag provided was not a valid tag. This error can occur if the tag did
          not pass input validation. For information about tag restrictions, see `User-Defined Tag
          Restrictions
    ...
    def put_bucket_versioning(
         ...
         **Suspended** —Disables versioning for the objects in the bucket. All objects added to the
         bucket receive the version ID null.

        If the versioning state has never been set on a bucket, it has no versioning state; a
        GetBucketVersioning request does not return a versioning state value.

        If the bucket owner enables MFA Delete in the bucket versioning configuration, the bucket
        owner must include the ``x-amz-mfa request`` header and the Status and the ``MfaDelete``
    ...
    def put_bucket_website(
        ...
        rules. For more information, see `Hosting Websites on Amazon S3
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/WebsiteHosting.html>`__ .

        This PUT operation requires the ``S3:PutBucketWebsite`` permission. By default, only the
        bucket owner can configure the website attached to a bucket; however, bucket owners can
        allow other users to set the website configuration by writing a bucket policy that grants
        them the ``S3:PutBucketWebsite`` permission.

        To redirect all website requests sent to the bucket's website endpoint, you add a website
        configuration with the following elements. Because all requests are sent to another website,
        ...
        * Protocol
        ...
        redirect destination. In this case, the website configuration must provide an index document
        for the bucket, because some requests might not be redirected.
        ...
        :param WebsiteConfiguration: **[REQUIRED]**
          ...
          - **IndexDocument** *(dict) --*
            ...
            - **Suffix** *(string) --* **[REQUIRED]**
              ...
              (e.g. if the suffix is index.html and you make a request to samplebucket/images/ the
            ...
            - *(dict) --*
              ...
              - **Condition** *(dict) --*
                ...
                - **HttpErrorCodeReturnedEquals** *(string) --*
                  ...
                  error code equals this value, then the specified redirect is applied. Required
                  when parent element ``Condition`` is specified and sibling ``KeyPrefixEquals`` is
                  not specified. If both are specified, then both must be true for the redirect to
                  be applied.
              ...
              - **Redirect** *(dict) --* **[REQUIRED]**
                ...
                - **ReplaceKeyPrefixWith** *(string) --*
                  ...
                  ``docs/`` and in the Redirect set ``ReplaceKeyPrefixWith`` to ``/documents`` . Not
    ...
    def put_object(
        ...
        Amazon S3 is a distributed system. If it receives multiple write requests for the same
        object simultaneously, it overwrites all but the last object written. Amazon S3 does not
        provide object locking; if you need this, make sure to build it into your application layer
        or use versioning instead.

        To ensure that data is not corrupted traversing the network, use the ``Content-MD5`` header.
        When you use this header, Amazon S3 checks the object against the provided MD5 value and, if
        they do not match, returns an error. Additionally, you can calculate the MD5 while putting
        ...
        .. note::
          ...
          use the ``100-continue`` HTTP status code. For PUT operations, this helps you avoid
          sending the message body if the message is rejected based on the headers (for example,
          because authentication fails or a redirect occurs). For more information on the
          ``100-continue`` HTTP status code, see Section 8.2.3 of
        ...
        encryption keys. For more information, see `Using Server-Side Encryption
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingServerSideEncryption.html>`__ .

          Access Permissions

        You can optionally specify the accounts or groups that should be granted specific
        permissions on the new object. There are two ways to grant the permissions using the request
        headers:
        ...
        * Use encryption keys managed Amazon S3 or customer master keys (CMKs) stored in AWS Key
        Management Service (KMS) – If you want AWS to manage the keys used to encrypt data, specify
        ...
        * Use customer-provided encryption keys – If you want to manage your own encryption keys,
        ...
        For more information on Server-Side Encryption with CMKs stored in KMS (SSE-KMS), see
        `Protecting Data Using Server-Side Encryption with CMKs stored in AWS KMS
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingKMSEncryption.html>`__ .
        ...
        You specify each grantee as a type=value pair, where the type is one of the following:
          ...
          * emailAddress – if the value specified is the email address of an AWS account
          ...
          .. warning::
            ...
            Endpoints <https://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region>`__ in the
        ...
        * Use customer-provided encryption keys – If you want to manage your own encryption keys,
        ...
        Standard storage class provides high durability and high availability. You can specify other
        storage classes depending on the performance needs. For more information, see `Storage
        Classes <https://docs.aws.amazon.com/AmazonS3/latest/dev/storage-class-intro.html>`__ in the
        Amazon Simple Storage Service Developer Guide.

         **Versioning**

        If you enable versioning for a bucket, Amazon S3 automatically generates a unique version ID
        for the object being stored. Amazon S3 returns this ID in the response using the
        ``x-amz-version-id response`` header. If versioning is suspended, Amazon S3 always uses null
        as the version ID for the object stored. For more information about returning the versioning
        state of a bucket, see  GetBucketVersioning . If you enable versioning for a bucket, when
        Amazon S3 receives multiple write requests for the same object simultaneously, it stores all
        of the objects.
        ...
        *  DeleteObject
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/s3-2006-03-01/PutObject>`_
        ...
        **Request Syntax**
        ...
        :param ContentMD5:
          ...
          <https://docs.aws.amazon.com/AmazonS3/latest/dev/RESTAuthentication.html>`__ .
        ...
        :param StorageClass:
          ...
          If you don't specify, Standard is the default storage class. Amazon S3 supports other
          storage classes.
        ...
        :param WebsiteRedirectLocation:
          ...
          If the bucket is configured as a website, redirects requests for this object to another
          object in the same bucket or to an external URL. Amazon S3 stores the value of this header
          in the object metadata. For information about object metadata, see .
          ...
          S3 <https://docs.aws.amazon.com/AmazonS3/latest/dev/WebsiteHosting.html>`__ and `How to
        ...
        :param SSEKMSKeyId:
          ...
          specifies the ID of the AWS Key Management Service (AWS KMS) customer master key (CMK)
          that was used for the object.

          If the value of x-amz-server-side-encryption is aws:kms, this header specifies the ID of
          the AWS KMS CMK that will be used for the object. If you specify
          x-amz-server-side-encryption:aws:kms, but do not provide
          x-amz-server-side-encryption-aws-kms-key-id, Amazon S3 uses the AWS managed CMK in AWS to
          protect the data.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ServerSideEncryption** *(string) --*
              ...
              If you specified server-side encryption either with an AWS KMS customer master key
              (CMK) or Amazon S3-managed encryption key in your PUT request, the response includes
              this header. It confirms the encryption algorithm that Amazon S3 used to encrypt the
              object.
            ...
            - **SSEKMSKeyId** *(string) --*
              ...
              If the x-amz-server-side-encryption is present and has the value of aws:kms, this
              header specifies the ID of the AWS Key Management Service (KMS) customer master key
    ...
    def put_object_acl(
        ...
        Depending on your application needs, you may choose to set the ACL on an object using either
        the request body or the headers. For example, if you have an existing application that
        updates a bucket ACL using the request body, then you can continue to use that approach.
    ...
    def put_object_lock_configuration(
        ...
        .. note::
           ...
           ``DefaultRetention`` requires either Days or Years. You can't specify both at the same
           time.
    ...
    def put_object_retention(
        ...
        **Request Syntax**
        ...
        :param Bucket: **[REQUIRED]**
          ...
          The bucket that contains the object you want to apply this Object Retention configuration
          to.
    ...
    def put_object_tagging(
        ...
        against the tagging subresource that is associated with the object. You can retrieve tags by
        ...
        *
          ...
          * *Cause: The tag provided was not a valid tag. This error can occur if the tag did not
          pass input validation. For more information, see `Object Tagging
          <https://docs.aws.amazon.com/AmazonS3/latest/dev/object-tagging.html>`__ .*
        ...
        *
          ...
          * *Cause: A conflicting conditional operation is currently in progress against this
          resource. Please try again.*
    ...
    def put_public_access_block(
        ...
        """
        Creates or modifies the ``PublicAccessBlock`` configuration for an Amazon S3 bucket. In
        order to use this operation, you must have the ``s3:PutBucketPublicAccessBlock`` permission.
        For more information about Amazon S3 permissions, see `Specifying Permissions in a Policy
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html>`__ .
        ...
        **Request Syntax**
        ...
        :param PublicAccessBlockConfiguration: **[REQUIRED]**
          ...
          - **IgnorePublicAcls** *(boolean) --*
            ...
            Specifies whether Amazon S3 should ignore public ACLs for this bucket and objects in
            this bucket. Setting this element to ``TRUE`` causes Amazon S3 to ignore all public ACLs
    ...
    def restore_object(
        ...
        * ``restore an archive`` - Restore an archived object
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/querying-glacier-archives.html>`__ in the
        *Amazon Simple Storage Service Developer Guide* .

        When making a select request, do the following:

        * Define an output location for the select query's output. This must be an Amazon S3 bucket
        in the same AWS Region as the bucket that contains the archive object that is being queried.
        The AWS account that initiates the job must have permissions to write to the S3 bucket. You
        can specify the storage class and encryption for the output objects stored in the bucket.
        For more information about output, see `Querying Archived Objects
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/querying-glacier-archives.html>`__ in the
        *Amazon Simple Storage Service Developer Guide* . For more information about the ``S3``
        structure in the request body, see the following:

          *  PutObject

          * `Managing Access with ACLs
          <https://docs.aws.amazon.com/AmazonS3/latest/dev/S3_ACLs_UsingACLs.html>`__ in the *Amazon
          Simple Storage Service Developer Guide*
        ...
        * Define the SQL expression for the ``SELECT`` type of restoration for your query in the
        request body's ``SelectParameters`` structure. You can use expressions like the following
        examples.
          ...
          * Assuming that you are not using any headers for data stored in the object, you can
          specify columns with positional headers.  ``SELECT s._1, s._2 FROM Object s WHERE s._3 >
          100``
          ...
          * If you have headers and you set the ``fileHeaderInfo`` in the ``CSV`` structure in the
          ...
          cannot mix ordinal positions with header column names.   ``SELECT s.Id, s.FirstName, s.SSN
          FROM S3Object s``

        For more information about using SQL with Glacier Select restore, see `SQL Reference for
        Amazon S3 Select and Glacier Select
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/s3-glacier-select-sql-reference.html>`__ in
        the *Amazon Simple Storage Service Developer Guide* .

        When making a select request, you can also do the following:

        * To expedite your queries, specify the ``Expedited`` tier. For more information about
        tiers, see "Restoring Archives," later in this topic.
        ...
        * The output results are new Amazon S3 objects. Unlike archive retrievals, they are stored
        until explicitly deleted-manually or through a lifecycle policy.
        ...
        * Amazon S3 accepts a select request even if the object has already been restored. A select
        ...
        object, you must first initiate a restore request. This restores a temporary copy of the
        archived object. In a restore request, you specify the number of days that you want the
        restored copy to exist. After the specified period, Amazon S3 deletes the temporary copy but
        ...
        When restoring an archived object (or using a select request), you can specify one of the
        ...
        * **``Expedited`` ** - Expedited retrievals allow you to quickly access your data stored in
        the GLACIER storage class when occasional urgent requests for a subset of archives are
        required. For all but the largest archived objects (250 MB+), data accessed using Expedited
        retrievals are typically made available within 1–5 minutes. Provisioned capacity ensures
        that retrieval capacity for Expedited retrievals is available when you need it. Expedited
        retrievals and provisioned capacity are not available for the DEEP_ARCHIVE storage class.

        * **``Standard`` ** - Standard retrievals allow you to access any of your archived objects
        within several hours. This is the default option for the GLACIER and DEEP_ARCHIVE retrieval
        requests that do not specify the retrieval option. Standard retrievals typically complete
        within 3-5 hours from the GLACIER storage class and typically complete within 12 hours from
        the DEEP_ARCHIVE storage class.

        * **``Bulk`` ** - Bulk retrievals are Amazon Glacier’s lowest-cost retrieval option,
        enabling you to retrieve large amounts, even petabytes, of data inexpensively in a day. Bulk
        retrievals typically complete within 5-12 hours from the GLACIER storage class and typically
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/restoring-objects.html>`__ in the *Amazon
        Simple Storage Service Developer Guide* .
        ...
        another restore request to the same object, setting a new ``Tier`` request element. When
        issuing a request to upgrade the restore tier, you must choose a tier that is faster than
        the tier that the in-progress restore is using. You must not change any other parameters,
        such as the ``Days`` request element. For more information, see `Upgrading the Speed of an
        In-Progress Restore
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/restoring-objects.html#restoring-objects-upgrade-tier.title.html>`__
        in the *Amazon Simple Storage Service Developer Guide* .

        To get the status of object restoration, you can send a ``HEAD`` request. Operations return
        the ``x-amz-restore`` header, which provides information about the restoration status, in
        the response. You can use Amazon S3 event notifications to notify you when a restore is
        ...
        the restoration period when Amazon S3 is actively processing your current restore request
        for the object.

        If your bucket has a lifecycle configuration with a rule that includes an expiration action,
        the object expiration overrides the life span that you specify in a restore request. For
        example, if you restore an object copy for 10 days, but the object is scheduled to expire in
        3 days, Amazon S3 deletes the object in 3 days. For more information about lifecycle
        configuration, see  PutBucketLifecycleConfiguration and `Object Lifecycle Management
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/object-lifecycle-mgmt.html>`__ in *Amazon
        Simple Storage Service Developer Guide* .
        ...
        * If the object copy is not previously restored, then Amazon S3 returns ``202 Accepted`` in
        the response.
        ...
        *
          ...
          * *Cause: Glacier expedited retrievals are currently not available. Try again later.
          (Returned if there is insufficient capacity to process the Expedited request. This error
          applies only to Expedited retrievals and not to Standard or Bulk retrievals.)*
            ...
            - **InputSerialization** *(dict) --* **[REQUIRED]**
              ...
              - **CSV** *(dict) --*
                ...
                - **AllowQuotedRecordDelimiter** *(boolean) --*
                  ...
                  records should be allowed. Default value is FALSE. Setting this value to TRUE may
                  lower performance.
    ...
    def select_object_content(
        ...
        query language (SQL) statement. In the request, along with the SQL expression, you must also
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/selecting-content-from-objects.html>`__ in
        the *Amazon Simple Storage Service Developer Guide* .
        ...
        You must have ``s3:GetObject`` permission for this operation. Amazon S3 Select does not
        support anonymous access. For more information about permissions, see `Specifying
        Permissions in a Policy
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/using-with-s3-actions.html>`__ in the
        *Amazon Simple Storage Service Developer Guide* .
        ...
        * *GZIP or BZIP2* - CSV and JSON files can be compressed using GZIP or BZIP2. GZIP and BZIP2
        ...
        Amazon S3 Select supports columnar compression for Parquet using GZIP or Snappy. Amazon S3
        ...
        * *Server-side encryption* - Amazon S3 Select supports querying objects that are protected
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/ServerSideEncryptionCustomerKeys.html>`__
        in the *Amazon Simple Storage Service Developer Guide* . For objects that are encrypted with
        Amazon S3 managed encryption keys (SSE-S3) and customer master keys (CMKs) stored in AWS Key
        Management Service (SSE-KMS), server-side encryption is handled transparently, so you don't
        ...
        *Amazon Simple Storage Service Developer Guide* .
        ...
        response. For more information, see  RESTSelectObjectAppendix .
        ...
        * ``Range`` : While you can specify a scan range for a Amazon S3 Select request, see
        SelectObjectContentRequest$ScanRange in the request parameters below, you cannot specify the
        range of bytes of an object to return.

        * GLACIER, DEEP_ARCHIVE and REDUCED_REDUNDANCY storage classes: You cannot specify the
        GLACIER, DEEP_ARCHIVE, or ``REDUCED_REDUNDANCY`` storage classes. For more information,
        about storage classes see `Storage Classes
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingMetadata.html#storage-class-intro>`__
        in the *Amazon Simple Storage Service Developer Guide* .

         **Special Errors**

        For a list of special errors for this operation and for general information about Amazon S3
        errors and a list of error codes, see  ErrorResponses
        ...
        **Request Syntax**
          ...
          - **CSV** *(dict) --*
            ...
            - **FileHeaderInfo** *(string) --*
              ...
              * ``IGNORE`` : First line is a header, but you can't use the header values to indicate
              the column in an expression. You can use column position (such as _1, _2, …) to
        ...
        :param ScanRange:
          ...
          Specifies the byte range of the object to get the records from. A record is processed when
          its first byte is contained by the range. This parameter is optional, but when specified,
          it must not be empty. See RFC 2616, Section 14.35.1 about how to specify the start and end
          of the range.
          ...
          * ``<scanrange><end>50</end></scanrange>`` - process only the records within the last 50
          ...
          - **Start** *(integer) --*
            ...
            ``<scanrange><start>50</start></scanrange>`` means scan from byte 50 until the end of
            the file.
          ...
          - **End** *(integer) --*
            ...
            queried. If only the End parameter is supplied, it is interpreted to mean scan the last
            N bytes of the file. For example; ``<scanrange><end>50</end></scanrange>`` means scan
            the last 50 bytes.
        ...
        :returns:
          ...
          :class:`.EventStream` will yield events based on the structure below, where only one of
          the top level keys will be present for any given event.
    ...
    def upload_part(
        ...
        You must initiate a multipart upload (see  CreateMultipartUpload ) before you can upload any
        part. In response to your initiate request, Amazon S3 returns an upload ID, a unique
        identifier, that you must include in your upload part request.

        Part numbers can be any number from 1 to 10,000, inclusive. A part number uniquely
        identifies a part and also defines its position within the object being created. If you
        upload a new part using the same part number that was used with a previous part, the
        previously uploaded part is overwritten. Each part must be at least 5 MB in size, except the
        last part. There is no size limit on the last part of your multipart upload.

        To ensure that data is not corrupted when traversing the network, specify the
        ``Content-MD5`` header in the upload part request. Amazon S3 checks the part data against
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/mpuoverview.html>`__ in the *Amazon Simple
        Storage Service Developer Guide* .

        For information on the permissions required to use the multipart upload API, go to
        `Multipart Upload API and Permissions
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/mpuAndPermissions.html>`__ in the *Amazon
        Simple Storage Service Developer Guide* .
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingServerSideEncryption.html>`__ in the
        *Amazon Simple Storage Service Developer Guide* .

        Server-side encryption is supported by the S3 Multipart Upload actions. Unless you are using
        a customer-provided encryption key, you don't need to specify the encryption parameters in
        each UploadPart request. Instead, you only need to specify the server side encryption
        ...
        **Request Syntax**
        ...
        :param ContentMD5:
          ...
          when using the command from the CLI. This parameted is required if object lock parameters
          are specified.
    ...
    def upload_part_copy(
        ...
        source by adding the request header ``x-amz-copy-source`` in your request and a byte range
        by adding the request header ``x-amz-copy-source-range`` in your request.
        ...
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/qfacts.html>`__ in the *Amazon Simple
        Storage Service Developer Guide* .

        .. note::

          Instead of using an existing object as part data, you might use the  UploadPart operation
        ...
        * For conceptual information on multipart uploads, go to `Uploading Objects Using Multipart
        Upload <https://docs.aws.amazon.com/AmazonS3/latest/dev/uploadobjusingmpu.html>`__ in the
        *Amazon Simple Storage Service Developer Guide* .

        * For information on permissions required to use the multipart upload API, go to `Multipart
        Upload API and Permissions
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/mpuAndPermissions.html>`__ in the *Amazon
        Simple Storage Service Developer Guide* .

        * For information about copying objects using a single atomic operation vs. the multipart
        upload, go to `Operations on Objects
        <https://docs.aws.amazon.com/AmazonS3/latest/dev/ObjectOperations.html>`__ in the *Amazon
        Simple Storage Service Developer Guide* .

        * For information about using server-side encryption with customer-provided encryption keys
        with the UploadPartCopy operation, see  CopyObject and  UploadPart .
        ...
        * **Consideration 1** - If both of the ``x-amz-copy-source-if-match`` and
        ...
        ``x-amz-copy-source-if-unmodified-since`` condition evaluates to ``false`` ; then, S3
        ...
        * **Consideration 2** - If both of the ``x-amz-copy-source-if-none-match`` and
        ...
        ``x-amz-copy-source-if-modified-since`` condition evaluates to ``true`` ; then, S3 returns
        ...
        By default, ``x-amz-copy-source`` identifies the current version of the object to copy. If
        the current version is a delete marker and you don't specify a versionId in the
        ``x-amz-copy-source`` , Amazon S3 returns a 404 error, because the object does not exist. If
        you specify versionId in the ``x-amz-copy-source`` and the versionId is a delete marker,
        Amazon S3 returns an HTTP 400 error, because you are not allowed to specify a delete marker
        ...
        **Request Syntax**
        ...
        :param CopySourceRange:
          ...
          example, bytes=
              0-9 indicates that you want to copy the first ten bytes of the source. You
    ...
    @overload
    # pylint: disable=arguments-differ,redefined-outer-name,redefined-builtin
    def get_waiter(self, waiter_name: Literal["bucket_exists"]) -> waiter_scope.BucketExistsWaiter:
        """
    ...
    @overload
    # pylint: disable=arguments-differ,redefined-outer-name,redefined-builtin
    def get_waiter(self, waiter_name: Literal["object_exists"]) -> waiter_scope.ObjectExistsWaiter:
        """
