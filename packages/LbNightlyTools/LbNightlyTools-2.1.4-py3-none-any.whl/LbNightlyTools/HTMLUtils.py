...
class ANSIStyle(object):
    ...
    def __eq__(self, other):
        return (isinstance(other, ANSIStyle) and
                (self.style, self.color, self.bgcolor) ==
...
class ANSI2HTML(object):
    ...
    def __init__(self, start_style=''):
        self.current_style = (start_style if isinstance(start_style, ANSIStyle)
...
class TableizeLine(object):
    ...
    '''
    def __init__(self, first_line_no=1, line_id_prefix='L', add_line_nos=False,
                 row_class=None):
    ...
    def __call__(self, line):
        ...
        out = self._line_format.format(n=self.line_no, class_desc=class_desc,
                                       text=line) + newline
        self.line_no += 1
        return out
...
class WrapLine(object):
    ...
    def __init__(self, tag, attrs={}):
        self._line_format = '<{tag}{attrs}>{{text}}</{tag}>'.format(
            tag=tag,
            attrs='' if not attrs else ' ' + ' '.join(
                '{0}="{1}"'.format(item) for item in list(attrs.items())
            )
        )
...
class XTerm2HTML(object):
    ...
    def __init__(self, first_line=1, show_line_no=False,
                 line_prefix='L', is_escaped=False, plugins_function=None):
        '''
        ...
        class.
        '''
        self.actions = [] if is_escaped else [lambda line:
                                              cgi.escape(line, quote=True)]
        self.actions.extend(plugins_function or [])
        self.actions.extend([ANSI2HTML(), WrapLine('pre'),
                             TableizeLine(first_line_no=first_line,
...
def convertFile(src, dst, show_line_no=False,
                line_prefix='L', plugins_function=[]):
    '''
    Small helper to convert a text (ANSI) file to HTML.
    '''
    import codecs
    from os.path import basename
    conv = XTerm2HTML(show_line_no=show_line_no,
        ...
        def mr_link_repl(matchobj):
            ...
            try:
                ...
                if title:
                    ...
                    return ('<a href="https://gitlab.cern.ch/{0}/'
...
def test_special_cases():
    ...
    expected = ('<table class="table table-striped" style="text-align:left">'
...
if __name__ == '__main__':
    if '--convertFile' in sys.argv:
        ...
        conv = XTerm2HTML(show_line_no='--show-line-no' in sys.argv,
                          plugins_function=[lambda line:
                                            str(line, 'utf-8', 'ignore')])
