...
def genConfig(slot='lhcb-release', build_id=0, projects=None, packages=None, platforms=None, build_tool='cmake'):
    '''
...
class _ReleaseTriggerBase(PlainScript):
    ...
    def _gen_release_params(self, stack):
        projects_list = ' '.join(' '.join(pv)
                                 for pv in stack.get('projects', []))
        packages_list = ' '.join(' '.join(pv)
                                 for pv in stack.get('packages', []))
        ...
        if projects_list or packages_list:
            ...
            data = JobParams(projects_list=projects_list,
...
class Poll(_ReleaseTriggerBase):
    ...
    def _clean_stacks_params(self):
        ...
        list(map(os.remove, glob(self.PARAM_FILE_TPL.format('*'))))
...
def createManifestFile(project, version, platform, build_dir):
    ...
    env['CMTPROJECTPATH'] = ':'.join([dirname(dirname(abspath(build_dir))),
    ...
    if not match:
        logging.error('problem with "cmt show projects":\n'
                      '--- output ---\n%s\n--------------',
                      out)
    ...
    if 'DBASE' in out or 'PARAM' in out:
        ...
        def findVersion(pkg):
            v = next((x.split()[3] for x in out
                 if re.match(r'^#.*%s' % pkg, x)))
            if v == 'v*':
                v = '*'
            return v
