...
class Script(BaseScript):
    ...
    def main(self):
        ...
        ensureDirs(os.path.join(self.artifacts_dir, d)
                   for d in ('packs/src', 'checkout'))
        if 'BUILD_URL' in os.environ:
            with open(os.path.join(self.artifacts_dir, 'checkout',
                                   'job_url.txt'), 'w') as f:
        ...
        with chdir(self.build_dir):
            ...
            for projname, deps in slot.dependencies().items():
        ...
        for project in slot.projects:
            if hasattr(project, 'checkout_log'):
                ...
                co_logfile = join(self.artifacts_dir, 'checkout',
                                  '.'.join((project.name, 'log')))
        ...
        with codecs.open(join(self.artifacts_dir, '.htaccess'),
                         'w', 'utf-8') as htaccess_file:
            htaccess_file.write(HTACCESS_DATA)
        for helper in ('extract.php', 'listzip.php'):
            shutil.copy(join(os.path.dirname(__file__), helper),
