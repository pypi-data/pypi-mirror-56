...
DEFAULT_DIR_PERM = (stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP |
                    stat.S_IROTH | stat.S_IXOTH)


def fix_permissions(path, file_perm=DEFAULT_FILE_PERM,
...
class Script(BaseScript):
    ...
    def defineBuildOptions(self):
        ...
        self.parser.set_defaults(jobs=jobs,
    ...
    def _prepareBuildDir(self):
        ...
        if not self.options.no_unpack:
            unpackArtifacts(os.path.join(self.artifacts_dir, 'packs', 'src'),
                            self.build_dir)
            unpackArtifacts(os.path.join(self.artifacts_dir, 'packs',
                                         self.platform),
                            self.build_dir)
            unpackArtifacts(os.path.join(self.artifacts_dir, 'ccache'),
                            self.build_dir)

            if os.path.exists(os.path.join(self.artifacts_dir, 'slot.patch')):
                self.log.warning(
                    'Applying patch file: %s' % os.path.join(
                        self.artifacts_dir,  'slot.patch'))
                log_call(
                    ['patch', '-p1', '-i', os.path.join(self.artifacts_dir,
                                                        'slot.patch')],
                    cwd=self.build_dir)
...
try:
    ...
    pass # __file__ gets defined only with LbScripts > v8r0\n''')
                ...
                cmake_templ = Template('''set(NIGHTLY_BUILD_ROOT ${build_root})
...
string(REPLACE "$${NIGHTLY_BUILD_ROOT}" "$${CMAKE_CURRENT_LIST_DIR}"
    ...
    def _sourcesLists(self, proj):
        ...
        '''
        return sorted(listAllFiles(self._buildDir(proj),
    ...
    def _manifestRequired(self, proj, build_result):
        ...
        '''
        return (self.slot.name == 'lhcb-release' and
    ...
    def _ensureGeneratedManifest(self, proj, build_result):
        ...
        if (not os.path.exists(manifest_file) and
                not os.path.exists(self._buildDir(proj, 'manifest.xml'))):
    ...
    def main(self):
        ...
        with chdir(self.build_dir):
            ...
            for proj, result in self.slot.buildGen(projects=opts.projects,
                ...
                if str(proj.build_tool) == 'CMake':
                    ...
                    collect_retcode = \
                        CBLScript().run(['--debug', '--cmt',
                                         ...
                                         self._buildDir(proj),
                                         join(summary_dir, 'build.log')])
                if (collect_retcode or
                        not os.path.exists(join(summary_dir, 'build.log'))):
                    with open(join(summary_dir, 'build.log'), 'w') as f:
                ...
                report = genBuildReport(
                    ...
                    self._buildDir(proj), self.build_dir,
                    exceptions={'warning': self.slot.warning_exceptions,
                                'error': self.slot.error_exceptions},
                    extra_info={'project': proj.name,
                ...
                for (f, d) in (('sources.list', sources),
                               ('sources_built.list',
                                self._sourcesLists(proj))):
                    self.write(self._summaryDir(proj, f), '\n'.join(d) + '\n')

                if (not isinstance(proj, DataProject) and
                    ...
                    if os.path.exists(inst_area):
                        ...
                        self.log.warning('%s missing, binary package not '
                                         'produced', inst_area)
                ...
                self.send({'type': 'artifacts.build', 'project': proj,
                           'platform': self.platform,
                ...
                if opts.coverity and result.returncode == 0:
                    ...
                    # fix Permissions of artifacts in Coverity builds
                    fix_permissions(self.artifacts_dir)
                    self.send({'type': 'artifacts.coverity', 'project': proj,
                               'platform': self.platform,
                               'path': self.artifacts_dir})
        ...
        self.send({'type': 'job-end',
                   'completed': self.completetime.isoformat()}, sync=True)
...
def genBuildReport(build_log, output_dir,
                   proj_build_root=None, slot_build_root=None,
                   exceptions=None, extra_info=None):
    from os.path import join, dirname, isdir
    from subprocess import check_output, CalledProcessError
    from LbNightlyTools.HTMLUtils import (ANSI2HTML, TableizeLine, WrapLine,
                                          ClassifyByLineNo)
    from LbNightlyTools.BuildLogScanner import (split_build_log,
                                                scan_build_log,
                                                strip_build_root)
    from LbNightlyTools.Scripts.CollectBuildLogs import (
        TABLE_HEAD, TABLE_TAIL, IssueLinker, GitlabUrlFormat,
        report_dict, write_report_index)
    ...
    reports = dict((chunk_id, scan_build_log(lines))
                   for chunk_id, lines in chunks)
    ...
    for chunk_id, chunk in chunks:
        classification = ClassifyByLineNo(
            ((issue.log_range[0] + 1, issue.log_range[1] + 1),
             {'warning': 'alert-warning',
              'error': 'alert-danger'}.get(issue.severity, ''))
            for issue in reports[chunk_id]
        )
        actions = [lambda line: cgi.escape(line, quote=True),
                   ANSI2HTML(),
                   WrapLine('pre'),
                   TableizeLine(line_id_prefix=chunk_id + '_',
                                add_line_nos=True,
                                row_class=classification)]
        if git_info:
            actions.append(IssueLinker(
        ...
        with codecs.open(join(output_dir, chunk_id + '.html'),
                         'w', 'utf-8') as h:
            h.write(TABLE_HEAD)
    ...
    jreport['ignored_issues'] = {severity: {}
                                 for severity in jreport['issues']}
    exceptions = {severity: [(exp, re.compile(exp))
                             for exp in exceptions[severity]]
    ...
    jreport.update({'warnings': len(jreport['issues']['warning']),
