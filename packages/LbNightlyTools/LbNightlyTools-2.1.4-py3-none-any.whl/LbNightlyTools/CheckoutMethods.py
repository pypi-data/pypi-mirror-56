...
def decorate_log_call(func):
    ...
    def wrapped(*args, **kwargs):  # pylint: disable=C0111
        if args:
            ...
            kwargs['logger'].debug('(%s)$ %s',
                                   where, ' '.join(map(repr, cmd)))
...
def ignore(desc, export=False):
    ...
    return {'retcode': 0,
            'stdout': 'checkout not requested for %s' % desc, 'stderr': ''}
...
class GitRepository(object):
    ...
    def __init__(self, path=os.curdir, url=None):
        ...
        if url and 'gitlab.cern.ch' in url:
            ...
            if 'origin/merge-requests' not in proc.communicate()[0]:
                ...
                log_call(['git', 'config', '--add', 'remote.origin.fetch',
                        fetch], cwd=self.path)
                retry_log_call(['git', 'fetch', '-q', 'origin'], cwd=self.path,
                            retry=3, retry_sleep=600)
    ...
    def remotes(self):
        ...
        return dict(m.groups() for m in [_f for _f in map(pattern.match, lines) if _f])
    ...
    def tags(self):
        ...
        return set(tag.strip()
                   for tag in proc.communicate()[0].splitlines())
    ...
    def add_remote(self, name, url, retry=True):
        ...
        retry_log_call(['git', 'fetch', '-q', name], cwd=self.path,
                       retry=3 if retry else 0, retry_sleep=600)
        retry_log_call(['git', 'fetch', '-q', '--tags', name], cwd=self.path,
                       retry=3 if retry else 0, retry_sleep=600)
    ...
    def resolve_commit(self, commit, remote='origin'):
        ...
        if not commit or commit == 'HEAD':
            ...
            if commit not in branches:
                for branch in ['{0}/{1}'.format(remote, commit),
                               '{0}'.format(commit)]:
    ...
    def init_submodules(self):
        ...
        if os.path.exists(os.path.join(self.path, '.gitmodules')):
            retry_log_call(['git', 'submodule', 'update',
                            '--init', '--recursive'],
                           cwd=self.path, retry=3, retry_sleep=600)
            return [os.path.join(self.path, l.split()[1])
...
class GitMergeHandler(GitRepository):
    ...
    def __init__(self, commit, url=None, remote_name=None, path=os.curdir):
        ...
        self.commit, self.commit_id = self.resolve_commit(commit,
                                                          self.remote_name)
        if url is None:
            url = self.remotes()[self.remote_name]
        __log__.getChild('git').debug('merging %s from %s (%s)',
                                      self.commit, self.remote_name, url)
...
def git(proj, url=None, commit=None, export=False, merges=None):
    ...
    '''
    output = {'retcode': 0, 'stdout': '', 'stderr': '',
              'warning': [], 'error': []}
    log = __log__.getChild('git')
    ...
    if merges and any(str(m).startswith('all') for m in merges):
        ...
        elif (hasattr(proj, 'container') and proj.container and
                proj.container.slot):
            slotname = proj.container.slot.name
        all_merges = getAllMergeRequestIDs(gitlab_name,
                                           filters=filters,
                                           labels=[slotname, 'all-slots'])
        ...
        if isinstance(merge, int):
            ...
            if len(merge) == 2:
                if isinstance(merge[0], int):
                    miid, commit = merge
                    return GitLabMergeRequestHandler(miid, path=dest,
                                                     commit=commit)
                else:
                    url, commit = merge
    ...
    try:
        if hasattr(proj, 'slot') and proj.slot and proj.slot.build_id:
            if not gitlabProjectExists('lhcb-nightlies/{}'.format(proj.name)):
                raise RuntimeError('lhcb-nightlies/{} does not exist'
                                   .format(proj.name))
            ...
            repo.add_remote(
                'lhcb-nightlies',
                'ssh://git@gitlab.cern.ch:7999/lhcb-nightlies/{0}.git'
                .format(proj.name), retry=False)
            ...
            if ('remotes/lhcb-nightlies/' + branch_name()) in repo.branches():
                ...
                if (repo.rev_parse('HEAD:') !=
                    repo.rev_parse('remotes/lhcb-nightlies/{}:'.format(
                        branch_name()))):
                    log.debug('content change detected')
                    repo.merge('remotes/lhcb-nightlies/' + branch_name(),
                               extra_args=['-sours', '--no-edit'])
                    repo.push_branch('HEAD:' + branch_name(),
                                     remote='lhcb-nightlies')
                else:
                ...
                repo.push_branch(branch_name(), remote='lhcb-nightlies',
                                 force=True)
                ...
                proc = Popen(['git', 'for-each-ref', '--python',
                              '--sort=-*creatordate',
                              '--format=%(refname),'], cwd=repo.path, stdout=PIPE)
            ...
            if tag_name(previous=True) in repo.tags():
...
def copy(desc, src, export=False):
    ...
    return {'retcode': 0,
            'stdout': 'copied %s from %s' % (desc, src), 'stderr': ''}
...
def unzip(desc, src, export=False):
    ...
    dest = desc.baseDir
    if not os.path.isdir(dest):
        raise RuntimeError('the zipfile %s does not contain %s',
                           src, desc.baseDir)
    log.debug('unpacking of %s from %s completed', desc, src)
    return output
...
def lbscripts(proj, url=None, export=False, merges=None, commit=None):
    ...
    outputs.append(git(proj, url=url, commit=commit, export=export,
                       merges=merges))
    ...
    if isinstance(proj, Package):
        ...
        if not url:
            ...
            if gitlabProjectExists(gitlab_name):
                ...
                kwargs['url'] = ('https://gitlab.cern.ch/{}.git'
                                 ).format(gitlab_name)
