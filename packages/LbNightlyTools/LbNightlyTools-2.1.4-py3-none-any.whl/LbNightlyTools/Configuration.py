...
class Project(object, metaclass=_ProjectMeta):
    '''
    ...
    def baseDir(self):
        ...
        if self.slot and self.slot.with_version_dir:
            ...
            __log__.debug('Using file structure with version directory. Base dir=%s' % bdir)
            return bdir
        return '%s/' % self.name
...
class Package(object):
    ...
    def build(self, **kwargs):
        ...
        if os.path.exists(os.path.join(base, 'Makefile')):
            ...
            env = dict((key, value) for key, value in list(os.environ.items())
...
class DataProject(Project):
    ...
    def checkout(self, **kwargs):
        ...
        co_kwargs = dict([(key, value) for key, value in kwargs.items()
                          if key in ('export')])
        b_kwargs = dict([(key, value) for key, value in kwargs.items()
                         if key in ('jobs')])
...
class Slot(object, metaclass=_SlotMeta):
    '''
    ...
    __slots__ = ('_name', '_projects', 'env', '_build_tool', 'disabled',
                 ...
                 'with_version_dir', 'no_test', 'deployment', '_source')
    __projects__ = []
    __env__ = []
...
def loadFromOldXML(source, slot):
    ...
    data = {'slot': slot, 'env': []}
    try:
        slot_el = next((el for el in doc.findall('slot')
                   if el.attrib.get('name') == slot))
        ...
        data['error_exceptions'] = list(map(el2re,
                                       doc.findall('general/ignore/error')))
        data['warning_exceptions'] = list(map(el2re,
...
def loadConfig(module=None):
    ...
    try:
        ...
        slot_list = getattr(importlib.import_module(module_name), attribute)
...
def findSlot(name, flavour='nightly', server=None, dbname=None):
    ...
    if re.match(r"^[-_0-9a-zA-Z]+\.\d+$", name):
        url = '/'.join([
            server or Dashboard.SERVER_URL,
            dbname or Dashboard.dbName(flavour),
            name
        ])
        __log__.debug('retrieving %s', url)
        return Slot.fromDict(json.load(urllib.request.urlopen(url))['config'])
...
def check_config():
    ...
    parser.set_defaults(module='lhcbnightlyconf',
    ...
    if args.dump_json:
        ...
        json.dump([slots[name].toDict() for name in sorted(slots)],
