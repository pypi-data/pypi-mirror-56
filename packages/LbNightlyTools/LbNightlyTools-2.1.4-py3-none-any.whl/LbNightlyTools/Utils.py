...
class Dashboard(object):
    ...
    def __init__(self, credentials=None, flavour='nightly',
                 server=None, dbname=None):
        '''
        ...
        self._log.debug('preparing connection to dashboard (%s%s)',
                        server, dbname)
    ...
    def lastBuildId(self, slot):
        ...
        rows = list(self.db.view('summaries/lastBuildId',
                                 key=slot, group=True))
...
def postToMergeRequest(name_or_id, mreq_iid, message, new_comment=False,
                       token=None):
    '''
    ...
    '''
    server = gitlab.Gitlab('https://gitlab.cern.ch/',
                           token or os.environ['GITLAB_TOKEN'])
    ...
    try:
        ...
        if new_comment:
            ...
            own_notes = [note for note in mreq.notes.list()
                         if note.author['id'] == server.user.id]
            if own_notes:
                last_note = own_notes[0]
...
def getAllMergeRequestIDs(name_or_id, state='opened',
                          filters=None, labels=None, token=None):
    '''
    ...
    server = gitlab.Gitlab('https://gitlab.cern.ch/',
                           token or os.environ['GITLAB_TOKEN'])
    logging.debug('looking for %s merge requests in project %s',
                  state, name_or_id)
    try:
        if filters:
        ...
        project = server.projects.get(name_or_id)
        mrs = [mr.iid for mr in project.mergerequests.list(state=state,
                                                           all=True)
               if accepted(mr)]
        mrs.sort()
        return mrs
...
def notifyMergeRequest(proj, name_or_id, mreq_iid, success, token=None):
    ...
    '''
    if (not proj or not proj.slot or not proj.slot.build_id or
            (os.environ.get('NO_UPDATE_MR', 'false').lower()
             not in ('false', '0', ''))):
        # noting to notify
        return
...
def cpuinfo():
    ...
    try:
        ...
        for l in open('/proc/cpuinfo'):
            try:
                k, v = list(map(str.strip, l.split(':', 1)))
                if k == 'processor':
                    current = {k: v}
