...
def _check_build_artifacts(root, info):
    ...
    assert_files_exist(artifacts_dir,
                       *[f.format(**info)
                         for f in ['{project}.{version}.{slot}.{config}.zip',
...
def test_simple_build_env_search_path():
    ...
    try:
        ...
        loc = {}
        exec(open(join(artifacts_dir, 'confSummary.py')).read(), {'__file__': '/new/root/confSummary.py'}, loc)
...
def test_explicit_list():
    ...
    try:
        ...
        os.makedirs('build')
        call(['unzip', '-q', '-o', join(
            os.pardir, 'artifacts', 'TestProject.HEAD.testing-slot.src.zip')],
             cwd='build')
