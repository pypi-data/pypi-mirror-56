...
class Issue(object):
    ...
    def __str__(self):
        return ': '.join(str(s)
                         for s in [self.source, self.severity, self.msg])
        ...
        while True:
            ...
            if m:
                ...
                try:
                    ...
                    pass
                reports.append(GCCIssue(
                    m.group(4), (m.group(1), int(m.group(2)), int(m.group(3))),
...
def cmake_output_scanner(lines, reports=None):
    if reports is None:
        reports = []
    diag_rexp = re.compile(r'^CMake (Warning|Error) at (\S+):([0-9]+) \(message\):')
    try:
        ln_iter = enumerate_x(lines, reports2exclusions(reports))
        ln, line = next(ln_iter)
        while True:
            ...
            if m:
                ...
                if len(msg) > 120:
                    msg = msg[:80] + '[...]' + msg[-35:]
                reports.append(CMakeIssue(
                    m.group(1).lower(), (m.group(2), int(m.group(3))),
                    msg, (startln, ln)))
            else:
                ln, line = next(ln_iter)
    except StopIteration:
    ...
    try:
        ...
        while True:
            ...
            if m:
                startln = ln
                ln, line = next(ln_iter)
                reports.append(PythonIssue(
                    'warning', (m.group(1), int(m.group(2))),
