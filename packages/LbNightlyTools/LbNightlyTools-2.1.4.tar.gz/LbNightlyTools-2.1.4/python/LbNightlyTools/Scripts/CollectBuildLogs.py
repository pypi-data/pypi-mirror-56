...
class Script(PlainScript):
    ...
    def defineOpts(self):
        ...
        platform = os.environ.get('BINARY_TAG',
    ...
    def cmake(self):
        ...
        if not logs:
            raise CollectLogsError('lbn-wrapcmd did not produce files in %s' %
                                   path)
    ...
    def cmt(self):
        ...
        if not logs:
            raise CollectLogsError('CMT build did not produce log files in %s'
                                   % path)
            ...
            if n >= l and n < h:
                ...
                line = line.replace(txt,
                                    '<a href="{url}">{txt}</a>'.format(
                                        url=self.url_format(issue.source),
                                        txt=txt))
                break
        self._count += 1
        return line
...
def report_dict(chunks, reports):
    ...
    report = {'sections': [],
              'issues': {severity: [] for severity in Issue.SEVERITIES}}

    for chunk_id, chunk in chunks:
        report['sections'].append({'id': chunk_id,
                                   'desc': chunk_id,
                                   'url': '{}.html'.format(chunk_id)})
        for issue in reports[chunk_id]:
            report['issues'][issue.severity].append(
                {'section_id': chunk_id,
                 'anchor': '{0}_{1}'.format(chunk_id,
                                            issue.log_range[0] + 1),
                 'desc': cgi.escape(str(issue), quote=True),
                 'text': [cgi.escape(remove_colors(line.rstrip()), quote=True)
                          for line in
                           chunk[issue.log_range[0]:issue.log_range[1]]]}
            )
    return report
            ...
            if sum(len(items) for items in report['issues'].values()):
                ...
                for severity in Issue.SEVERITIES:
                    ...
                    if issues:
                        ...
                        h.write('<li class="issue-{0}">{0}s ({1})<ul>\n'
                                .format(severity, len(issues)))
                        h.writelines(
...
class LogToHTML(PlainScript):
    ...
    def defineOpts(self):
        ...
        Options specific to this script.
        '''
        self.parser.add_option('--build-root',
                               help='build root of the project (to strip)')
        self.parser.add_option('--slot-build-root',
                               help='build root of the slot (to strip)')
        self.parser.add_option('--base-url',
                               help='base URL to access source files')
    ...
    def main(self):
        ...
        jreport = genBuildReport(log_file, output_dir,
                                 self.options.build_root,
                                 self.options.slot_build_root)
        json.dump(jreport, open(os.path.join(output_dir, 'report.json'), 'wb'),
                  indent=2)
