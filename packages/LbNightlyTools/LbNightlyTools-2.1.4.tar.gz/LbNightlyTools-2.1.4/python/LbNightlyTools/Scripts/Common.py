...
class RsyncDestination(object):
    ...
    def accept(self, msg):
        ...
        if msg.get('type', '').startswith('artifacts') and 'path' in msg:
            ...
            if self.listener:
                msg = dict(msg)
                msg.update({'type': 'ready.' + msg['type'],
                            'retcode': retcode})
...
class DashboardUpdate(object):
    ...
    def __init__(self, script):
        ...
        self.script_type = {'slot-config': 'checkout',
                            'build-result': 'build',
                            'tests-result': 'test'}[script.json_type]

        self.dashboard = Dashboard(credentials=None,
                                   flavour=opts.flavour,
                                   server=opts.db_url,
                                   dbname=opts.db_name or
                                       Dashboard.dbName(opts.flavour))
    ...
    def checkout_completed(self, config):
        ...
        data = {'config': recursive_update({}, config),  # clone object
                'checkout': {}}
        config = data['config']  # ignore the input data and work on the clone
    ...
    def update_ready_builds(self, msg):
        ...
        def changes(d):
            ...
            d['entries'].append({
                ...
                'scripts': {'repository': os.environ.get('scripts_repository'),
                            'version': os.environ.get('scripts_version')},
                'venv': {'JENKINS_OVERRIDE_PIP_REQUIREMENTS': os.environ.get('JENKINS_OVERRIDE_PIP_REQUIREMENTS'),
                         'JENKINS_RESET_VIRTUALENV': os.environ.get('JENKINS_RESET_VIRTUALENV')}
            })
            return d
    ...
    def project_build_started(self, msg):
        ...
        data = {'builds':
                {self.platform:
                 {msg['project']: {'started': msg['started'],
                                   'build_url': self.build_url}}}}
        self.dashboard.update(self.doc_name, data)
    ...
    def project_build_completed(self, msg):
        ...
        data = {'builds':
                {self.platform:
                 {msg['project']: {'completed': msg['completed'],
                                   'retcode': msg['retcode'],
...
class PeriodicTestMsg(object):
    ...
    def builds_ready(self, msg):
        ...
        self.log.debug('sending the message to the queue that builds '
                       'are ready for slot: %s, project: %s, platform: %s, '
                       'build_id: %s, deployment: %s', self.slot.name, msg['project'].name,
                       self.platform, self.slot.build_id, deployment_target)
        try:
...
class BaseScript(PlainScript):
    ...
    def _setupReceivers(self):
        ...
        if hasattr(self.options, 'rsync_dest') and self.options.rsync_dest:
            self.receivers.append(RsyncDestination(self.options.rsync_dest,
                                                   self))
    ...
    def _setup(self, build_dir=None, json_type=None, make_dirs=True,
        ...
        expandTokensInOptions(opts,
                              ['build_id', 'artifacts_dir', 'rsync_dest'],
                              slot=self.slot.name)
    ...
    def _summaryDir(self, *subdirs):
        ...
        return os.path.join(self.artifacts_dir, self.options.summary_prefix,
                            self.summary_base, self.platform,
        ...
        if os.path.isdir(self._buildDir(proj, '.git')):
            with open(self._summaryDir(proj, 'git-status.txt'), 'wb') as f:
                self.log.debug('running "git status" in %s',
                               self._buildDir(proj))
                Popen(['git', 'status', '--porcelain'], stdout=f,
