...
def show(left, right, width=80, headers=None):
    ...
    print '\n'.join(format_line(a, b) for a, b in izip(left_lines, right_lines))
...
def add_defaults(expected):
    ...
    slot_defaults = {'packages': [],
                     ...
                     'error_exceptions': [],
                     'warning_exceptions': []}
    project_defaults = {'checkout': 'default',
                        'checkout_opts': {},
                        'disabled': False,
                        'env': [],
                        'overrides': {},
                        'dependencies': [],
                        'with_shared': False}
    package_defaults = {'checkout': 'default',
                        'container': 'DBASE'}
...
def test_loadXML():
    ...
    expected = {'slot': 'lhcb-lcg-head',
                ...
                'build_tool': 'cmt',
                'platforms': ['x86_64-slc5-gcc43-dbg',
                              'i686-slc5-gcc43-opt',
                              'x86_64-slc6-gcc46-opt',
                              'x86_64-slc6-gcc46-dbg',
                              'x86_64-slc6-gcc47-opt',
                              'x86_64-slc6-clang32-opt'],
                'preconditions': [{'args': {'path': '/afs/cern.ch/sw/lcg/app/nightlies/dev/${TODAY}/LCG_externals_${CMTCONFIG}.txt',
                             'required': ['AIDA',
                                          'Boost',
                                          'CASTOR',
                                          'CLHEP',
...
def test_loadXML_3():
    ...
    expected = {'slot': 'lhcb-compatibility-x',
                ...
                'build_tool': 'cmt',
                'platforms': ['x86_64-slc5-gcc43-opt',
...
''')
        ...
        try:
            ...
            slot = Configuration.getSlot('hidden-slot', 'lhcbnightlyconf.configuration:hidden_slots')
