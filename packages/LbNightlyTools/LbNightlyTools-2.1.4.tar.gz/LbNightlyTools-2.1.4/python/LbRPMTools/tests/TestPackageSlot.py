...
class Test(unittest.TestCase):
    ...
    def testBinarySpecDo0(self):
        ...
        newspecfilename = os.path.join(artifactdir, 'rpms', self._binspecnamedo0)
        newlines = [ l for l in open(newspecfilename, 'U').readlines()
                     if "%define buildarea" not in l ]
    ...
    def testSharedSpec(self):
        ...
        with ZipFile(os.path.join(artifactdir, 'packs', 'src',
                                  'Brunel.v46r0.lhcb-release.999.src.zip'),
                     'w', ZIP_DEFLATED):
            pass
        ...
        newspecfilename = os.path.join(artifactdir, 'rpms', self._sharedspecname)
        newlines = [ l for l in open(newspecfilename, 'U').readlines()
                     if "%define buildarea" not in l ]
        oldlines = [ l.replace('%TMPDIR%', artifactdir) for l in open(self._fullsharedspecname, 'U').readlines()
        if "%define buildarea" not in l ]

        from difflib import Differ, unified_diff
        d = Differ()
        #        result = list(d.compare(oldlines, newlines))
        result = unified_diff(oldlines, newlines, fromfile=self._fullsharedspecname,
        tofile=newspecfilename, n=0)
        diffFound = False
        import sys
        for l in result:
    ...
    def testGlimpseSpec(self):
        ...
        newspecfilename = os.path.join(artifactdir, 'rpms', self._indexspecname)
        newlines = [ l for l in open(newspecfilename, 'U').readlines()
                     if "%define buildarea" not in l ]
    ...
    def testDatapkgSpec(self):
        ...
        newspecfilename = os.path.join(artifactdir, 'rpms', self._datapkgspecname)
        newlines = [ l for l in open(newspecfilename, 'U').readlines()
                     if "%define buildarea" not in l ]
