...
def get_default_platforms(proj_id, mreq_id):
    ...
    target_dict = { 'master': ['x86_64-centos7-gcc8-opt',],
                    'run2-patches': ['x86_64-slc6-gcc62-opt', 'x86_64-centos7-gcc62-opt', 'x86_64-centos7-gcc7-opt'],
                    '2016-patches': ['x86_64-slc6-gcc62-opt',],
                    '2017-patches': ['x86_64-slc6-gcc62-opt', 'x86_64-centos7-gcc62-opt', 'x86_64-centos7-gcc7-opt'],
                    '2018-patches': ['x86_64-slc6-gcc62-opt', 'x86_64-centos7-gcc62-opt', 'x86_64-centos7-gcc7-opt'] }

    # TODO potentially come up with some better way of derriving this
    return target_dict.get(target, ['x86_64-centos7-gcc8-opt', ])
...
def get_default_model(proj_id, mreq_id):
    ...
    gitlab_server = connect_gitlab()
    target = get_mr_from_id(proj_id, mreq_id).attributes['target_branch']
    if target in ['master', 'run2-patches', '2016-patches', '2017-patches', '2018-patches']:
        return 'lhcb-' + target
...
def parse_build_cmd(command_args, proj_id, mreq_id):
    ...
    MR_IDs, external_commits, platforms, model = parse_build_args(command_args, proj_id, mreq_id)
...
def parse_rebuild_cmd(command_args, proj_id, mreq_id):
    ...
    _MR_IDs, _external_commits, _platforms, _model = find_previous_config(proj_id, mreq_id)
...
def construct_builds(raw_content):
    ...
    if command_exe != rebuild_mr_cmd:
        ...
        with open('gitlab-ref-build.txt', 'w') as _file:
            ...
            _file.write( 'platforms={}'.format( ' '.join(json_data['platforms']) ) )
    ...
    message += 'You can check their build progress here: https://lhcb-nightlies.cern.ch/testing/summary.\n\n'
    message += 'You can compare the builds using a URL like: https://lhcb-nightlies.cern.ch/testing/compare/lhcb-master-mr/MR-BUILD-ID/lhcb-master-ref/REF-BUILD-ID/\n'
