    ...
    def __init__(self, tag, attrs={}):
        self._line_format = u'<{tag}{attrs}>{{text}}</{tag}>'.format(
            ...
            attrs='' if not attrs else ' ' + ' '.join(
                u'{0}="{1}"'.format(item) for item in attrs.items()
            )
        )
...
def test_special_cases():
    ...
    expected = (u'<table class="table table-striped" style="text-align:left">'
