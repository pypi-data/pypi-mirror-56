...
class Timeline:
    ...
    def build_shift_day(self, shift, shiftday):
            ...
            if month_range < startDate.day + 1:
                if red_label_day.month + 1 > 12:
                    self.shiftsList.add(CalendarEvent(
                        ...
                        # name=f'{red_label_day.name} ({self.dayweek_name(startDate)})',
        ...
        self.shiftsList.add(CalendarEvent(
            ...
            # description=f'{shift.shift.name} ({shiftday.daytype.name}) {shiftday.description if shiftday.description else ""}'
