...
class ClientDescribeStreamResponseStreamDescriptionKeySchemaTypeDef(
    ...
    simple primary key (partition key) would be represented by one ``KeySchemaElement`` . A
    composite primary key (partition key and sort key) would require one ``KeySchemaElement`` for
...
class ClientDescribeStreamResponseStreamDescriptionTypeDef(
    ...
    - **LastEvaluatedShardId** *(string) --*
      ...
      in the result set. The only way to know when you have reached the end of the result set is
      when ``LastEvaluatedShardId`` is empty.
    """
...
class ClientDescribeStreamResponseTypeDef(_ClientDescribeStreamResponseTypeDef):
    ...
    - **StreamDescription** *(dict) --*
      ...
      - **LastEvaluatedShardId** *(string) --*
        ...
        is when ``LastEvaluatedShardId`` is empty.
    """
...
class ClientGetRecordsResponseRecordsdynamodbKeysTypeDef(
    ...
    multi-valued set. For example, a book item can have title and authors attributes. Each book has
    one title but can have many authors. The multi-valued attribute is a set; duplicate values are
    not allowed.
    ...
    - **M** *(dict) --*
      ...
      - *(string) --*
        ...
        - *(dict) --*
          ...
          Each attribute in an item is a name-value pair. An attribute can be single-valued or
          multi-valued set. For example, a book item can have title and authors attributes. Each
          book has one title but can have many authors. The multi-valued attribute is a set;
    ...
    - **L** *(list) --*
      ...
      - *(dict) --*
        ...
        Each attribute in an item is a name-value pair. An attribute can be single-valued or
        multi-valued set. For example, a book item can have title and authors attributes. Each book
        has one title but can have many authors. The multi-valued attribute is a set; duplicate
...
class ClientGetRecordsResponseRecordsTypeDef(_ClientGetRecordsResponseRecordsTypeDef):
    ...
    - **eventVersion** *(string) --*
      ...
      The version number of the stream record format. This number is updated whenever the structure
      of ``Record`` is modified.

      Client applications must not assume that ``eventVersion`` will remain at a particular value,
      as this number is subject to change at any time. In general, ``eventVersion`` will only
...
class ClientGetRecordsResponseTypeDef(_ClientGetRecordsResponseTypeDef):
    ...
    - **NextShardIterator** *(string) --*
      ...
      The next position in the shard from which to start sequentially reading stream records. If set
      to ``null`` , the shard has been closed and the requested iterator will not return any more
      data.
    """
