...
import os
import copy
from .constants import OPERATORS, WEEKS, STATESFREE, STATESCOM, TIME_FREQ, RULE
import pickle

# TODO:
#   - improve distance selection, currently only destination cares
#   - improve change contition relaxing some rules. Ideally based on the changing
#             rate of loops, only change rules if the cause is efectivelly identified.
#   - change the way loops are printed, showing counter of states and condition
#             not complied, thi will help to identify if statistics provided or rules
...
def add_column_datetime(df, hours, reference_date, t):
    fr = {1: 'H', 0.5: '30min', 0.25: '15min'}
    freq = fr[t]
    start_date = pd.to_datetime(reference_date)
    hourly_periods = hours
    drange = pd.date_range(start_date, periods=hourly_periods, freq=freq)
...
class Mobility:
    ...
    def setStats(self, stat_ntrip, stat_dest, stat_km):
        ...
        self.df2or = stat_dest.copy()
        self.stat = stat_dest.copy()
        ...
        self.df2 = self.stat.copy()
        self.df3 = stat_km

    def setParams(self, param):
        self.per_str = param['person']
        self.refdate = param['refdate']
        self.numb_weeks = param['numb_weeks']
        self.g_type = param['group']
        self.sequence = days_week_sequence(self.refdate, self.weeks)
        self.numb_dayblocks = param['dayblocks']
        self.t = self.time_freq[self.numb_dayblocks]['t']
        self.day_hrs = self.numb_dayblocks
        self.hours = self.numb_weeks*self.day_hrs*7
        self.groups = {self.per_str: self.g_type}
        self.energy_consumption = param['energy_consumption']
        if self.g_type == 'freetime':
            self.states = STATESFREE
        else:
            self.states = STATESCOM

    def setRules(self, rules):
        self.rules = {self.g_type: RULE}
        for week, dicts in rules.items():
    ...
    def group_trips_week(self):
        ...
        # print('Trips: ', self.Rnd_week_trips)

    def time_purpose_iter(self):  # df*, cutt,cutp, group*, day
        """
        ...
        """
        self.D = self.df2[self.df2['days'] == self.day].copy()  # a smaller table is obtained that only contains the probability distribution of a selected day.
    ...
    def creation_unsorted_trips(self):  # trips,df,group,day
        '''
        It returns a unsorted time dataframe with trips. It includes number of trips, departure time, destination (purpose).
        trips: number of trips to create a tour later (tour: multiple trips).
        df: Dataframe with probability distributions. df must have columns with day_code, time,purpose,group of persons (is only used for time_purpose_iter() function).
        group: string that refers to a grop of persons where the probability distribution of the df to consider (is only used for time_purpose_iter() function).
        day: string refering to a weekday, saturday or sunday (it is only used for time_purpose_iter() function).
        '''
    ...
    def creation_unsorted_trips_commuter(self):  # trips,df,group,day,repeated
        '''
        It returns a unsorted time dataframe with trips. It includes number of trips, departure time, destination (purpose).
        trips: number of trips to create a tour later (tour: multiple trips).
        df: Dataframe with probability distributions. df must have columns with day_code, time,purpose,group of persons (is only used for time_purpose_iter() function).
        group: string that refers to a grop of persons where the probability distribution of the df to consider (is only used for time_purpose_iter() function).
        day: string refering to a weekday, saturday or sunday (it is only used for time_purpose_iter() function).
        repeated: is a list of tuples with departure time and destination (purpose) of thoso trips that habe to be constant. eg: [(),()]
        '''
        self.cutt = []
        self.cutp = []
        self.wdc = pd.DataFrame()
    ...
    def create_tour(self):  # start, prev_dest,db=self.wdc,t
        '''
        ...
        db: is a dataframe of unsorted trips
        start: indicate the last trip time of the previous tour.
        t: is a factor that indicates the fraction of an hour. eg. for 30min is 0.5

        TODO: - to edit the script to be able to model with 30 min or even for 15 min "df.loc[i, 'duration'] = row['departure'] - (prev_triptime+1)"
              - to add the last trip time in the return
        '''
        ...
        # return df,state         self.tour    self.state

    def hwh_edges_create(self):  # df_dist
        '''
        ...
        df_dist: is a dataframe that contains probability distribution of distance for each destination (purpose)
        '''
        self.list_of_KMranges = self.df3['km_range'].values.tolist()
        self.towork = np.random.choice(self.list_of_KMranges, p=self.df3['workplace'].values.tolist())
        self.string_numb = self.towork.split('_')
        self.integers = [int(i) for i in self.string_numb]
        self.value = np.random.randint(low=self.integers[0]+1, high=self.integers[1]+1)
        self.hwh_edges = {('home', 'workplace'): self.value, ('workplace', 'home'): self.value}
        # return hwh_edges

    def doubleedge_distance_create(self):  # df_dist, edge
        '''
        This function returns a dictionary with two edges as dictionary keys and distance traveled in km for edges 'origin' to 'destination' and 'destination' to 'origin'
        where the distance of these two edges is the same.
        df_dist: is a dataframe that contains probability distribution of distance for each destination (purpose)
        edge: is a tuples with an edge ('origin','destination'). eg: ('shopping','leisure')
        '''
        self.list_of_KMranges = self.df3['km_range'].values.tolist()
        self.destination = np.random.choice(self.list_of_KMranges, p=self.df3[self.edge[1]].values.tolist())  # select a range of distances based on destinations and their probability distribution.
        self.string_numb = self.destination.split('_')  # it brakes the range of distances to a list. eg: from '10_20' to ['10','20']
        self.integers = [int(i) for i in self.string_numb]  # the string numbers in the list are converted into integers. eg: [10,20]
        self.value = np.random.randint(low=self.integers[0]+1, high=self.integers[1]+1)  # it returns an integer from a list interval in uniform distribution. eg: "(10,20]" including right endpoint.
        self.doubleedge = {(self.edge[0], self.edge[1]): self.value, (self.edge[1], self.edge[0]): self.value}
        # return doubleedge

    def add_distance_to_tour(self):  # df, df_dist,hwh_edges
        '''
        ...
        df: is a dataframe that represents a day tour and contains trip information in each row. The information required is 'state' and 'purpose'.
        df_dist: is a dataframe that contains probability distribution of distance for each destination (purpose).
        hwh_edges: is a dictionary with home-workplace-home edges and distance.
        '''
        self.edges = list(set(list(tuple(zip(self.tour['state'], self.tour['purpose'])))))  # create a list with al the edges identified in a tour of trips
        # this script creates inverted edge of all edges and remove the one that is repeated in the list of edges.
        self.new_edges = self.edges.copy()
        self.memory = []
        for link in self.edges:
            self.knil = (link[1], link[0])
            if self.knil in self.edges:
                if self.knil not in self.memory:
                    self.memory.append(link)
                    self.new_edges.remove(self.knil)
        # this script call the function that allocate distance to edges other than home-work-home and check if distances are in a similar region.
        self.warningA = 0
        while True:
            self.warningA += 1
            if self.warningA % 100 == 0:
                print('   "add_distance_to_tour" method in loop after %s iter...' % self.warningA)
            if self.groups[self.per_str] == 'commuter':
                self.distance_dict = dict(self.hwh_edges.items())
            else:
                self.distance_dict = {}
            for self.edge in self.new_edges:
                if self.edge not in list(self.distance_dict.keys()):
                    self.doubleedge_distance_create()
                    self.distance_dict.update(self.doubleedge)
            for i, row in self.tour.iterrows():
                self.tour.loc[i, 'distance'] = self.distance_dict[(row['state'], row['purpose'])]
            # TODO:
            ...
            if self.tour['distance'].mean() + 0.5 >= self.tour['distance'].max() - self.tour['distance'].min():  # distance consistency check
                break

    def MeetAllConditions(self):  # operators,states,rules,g_type,weektype,df,flag=False
        self.flag = False
        self.cause = ''
    ...
    def select_tour(self):  # start,prev_dest,df_trips,df_hr,df_dist,n_day,per_str,g_dict,week_dict,states,operators,rules,hwh_edges,day_hrs,t
        '''
        ...
        start: represents the departing hour of the last trip of the tour of the day before.
        df_trips,df_hr,df_dist: are the probability distributions.
        n_day: string of weekday, Saturday or Sunday.
        per_str: string of a group of person.
        g_dict: dictionary with group of person.
        week_dict: dictionary that contains days of the week, kind of weekday and a day code.
        states: a list that contains all the possible locations that a vehicle can be.
        rules: dictionary with constraints that the tour of trips has to meet to be selected.
        operators: a dictionary that represent equality or inequality operator that may apply depends on the constraint contained in rules dictionary. This operator compares values of trips in right hand side (RHS) with rules dict values in left hand side (LHS).
        hwh_edges: dictionary with home-work-home edges and distance.
        day_hrs: this variable represents the number of values in the column "time" of the time_purpose stats, in order to identify if 24 hour a day, or 48 of 30min or 96 for 15min
        '''
        ...
        if self.Rnd_week_trips != 0:
            ...
            while True:
                ...
                if self.flagiter:
                    print('   tour done')
                break
            self.add_distance_to_tour()
            self.start = self.tour['departure'].iloc[-1] - self.day_hrs*self.t
            self.prev_dst = copy.deepcopy(self.state)
        else:
            self.no_trip = True
            self.tour = False
            self.start -= self.day_hrs*self.t
            self.prev_dst = copy.deepcopy(self.prev_dest)
        # return (self.no_trip, self.tour, self.start, self.prev_dst)
    ...
    def loadprofile(self, profilepath):
        self.profile = pd.read_csv(profilepath)
    ...
    def fill_rows(self):  # df,hours,t
        ...
        self.timeseries.loc[:, 'hh'] = np.arange(0, self.hours*self.t, self.t)
        ...
        self.timeseries.loc[self.hours-1, 'state'] = self.db['state'].iloc[-1]
        self.timeseries.loc[self.hours-1, 'hr'] = self.timeseries['hh'][self.hours-1]
        ...
        self.timeseries = add_column_datetime(self.timeseries, self.hours, self.refdate, self.t)
        self.timeseries = self.timeseries[['state', 'distance', 'consumption']]

    def driving_profile_creation(self):
        '''
        This function returns a driving profile
        '''
        ...
        if self.groups[self.per_str] == 'commuter':
            self.hwh_edges_create()
            self.warningD = 0
            while True:
                self.warningD += 1
                if self.warningD % 100 == 0:
                    print('   "driving_profile_creation" method, loop in section "select_tour" not instatiated after %s iter' % self.warningD)
                self.n_day = 1
                self.select_tour()
                if not self.no_trip:
                    break
        ...
        for w in range(self.numb_weeks):
            for n_day in self.sequence:
                self.n_day = n_day
                self.days += 1
                if self.weeks[self.n_day]['week'] == 'weekday' and self.groups[self.per_str] == 'commuter':
                    self.select_tour_commuter()
                else:
                    self.select_tour()
                self.prev_dest = copy.deepcopy(self.prev_dst)
                if not self.no_trip:
                    for idx, row in self.tour.iterrows():
                        self.hr = row['departure'] + self.days*self.day_hrs*self.t - self.t
                        if self.hr < self.hours*self.t-self.t:
                            ...
                            self.profile.loc[self.hr, 'person'] = self.groups[self.per_str]
                        ...
                        elif self.hr == self.hours*self.t-self.t:
                            self.profile.loc[self.hr, 'hr'] = self.hr
                            self.profile.loc[self.hr, 'state'] = row['state']
                            self.profile.loc[self.hr, 'departure'] = row['departure']
                            self.profile.loc[self.hr, 'last_dep'] = row['last_dep']
                            ...
                            break
        if self.profile['hr'].iloc[-1] < self.hours*self.t-self.t:  # verify and test this, not always works...
            self.profile.loc[self.hours*self.t-self.t, 'hr'] = self.hours*self.t-self.t
            self.profile.loc[self.hours*self.t-self.t, 'state'] = self.prev_dest
            self.profile.loc[self.hours*self.t-self.t, 'departure'] = row['departure']
            self.profile.loc[self.hours*self.t-self.t, 'last_dep'] = row['last_dep']
            self.profile.loc[self.hours*self.t-self.t, 'purpose'] = row['purpose']
            self.profile.loc[self.hours*self.t-self.t, 'duration'] = row['duration']
            self.profile.loc[self.hours*self.t-self.t, 'weekday'] = self.weeks[self.n_day]['day']
            self.profile.loc[self.hours*self.t-self.t, 'person'] = self.groups[self.per_str]
            self.profile.loc[self.hours*self.t-self.t, 'distance'] = 0

        self.fill_rows()

        # delete all unnecesary attributes
        self.to_rem = list(self.__dict__.keys())[:]
        for r in ['df1', 'df2or', 'df3', 'kind', 'rules', 'per_str', 'g_type', 'refdate', 'numb_weeks', 'sequence', 'numb_dayblocks', 't', 'day_hrs', 'hours', 'profile', 'energy_consumption', 'timeseries', 'name']:
            self.to_rem.remove(r)
        for attr in self.to_rem:
