...
class Client(BaseClient):
    ...
    def create_delivery_stream(
        ...
        . Attempts to send data to a delivery stream that is not in the ``ACTIVE`` state cause an
        ...
        providers using  PutRecord or  PutRecordBatch , or it can be configured to use an existing Kinesis
        stream as its source. To specify a Kinesis data stream as input, set the ``DeliveryStreamType``
        parameter to ``KinesisStreamAsSource`` , and provide the Kinesis stream Amazon Resource Name (ARN)
        ...
        A delivery stream is configured with a single destination: Amazon S3, Amazon ES, Amazon Redshift,
        ...
        * We strongly recommend that you use the user name and password you provide exclusively with
        Kinesis Data Firehose, and that the permissions for the account are restricted for Amazon Redshift
        ``INSERT`` permissions.
    ...
    def delete_delivery_stream(
        ...
        stream only if it is in one of the following states: ``ACTIVE`` , ``DELETING`` ,
        ``CREATING_FAILED`` , or ``DELETING_FAILED`` . You can't delete a delivery stream that is in the
        ``CREATING`` state. While the deletion request is in process, the delivery stream is in the
        ``DELETING`` state.
        ...
        **Request Syntax**
        ...
        :param AllowForceDelete:
          ...
          Set this to true if you want to delete the delivery stream even if Kinesis Data Firehose is
          unable to retire the grant for the CMK. Kinesis Data Firehose might be unable to retire the grant
          due to a customer error, such as when the CMK or the grant are in an invalid state. If you force
    ...
    def list_delivery_streams(
        ...
        ``Limit`` parameter. To determine whether there are more delivery streams to list, check the value
        of ``HasMoreDeliveryStreams`` in the output. If there are more delivery streams to list, you can
    ...
    def list_tags_for_delivery_stream(
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          with the delivery stream, ``HasMoreTags`` is set to ``true`` in the response. To list additional
    ...
    def put_record(
        ...
        If the ``PutRecord`` operation throws a ``ServiceUnavailableException`` , back off and retry. If
        the exception persists, it is possible that the throughput limits have been exceeded for the
        delivery stream.

        Data records sent to Kinesis Data Firehose are stored for 24 hours from the time they are added to
    ...
    def put_record_batch(
        ...
        By default, each delivery stream can take in up to 2,000 transactions per second, 5,000 records per
        second, or 5 MB per second. If you use  PutRecord and  PutRecordBatch , the limits are an aggregate
        across these two operations for each delivery stream. For more information about limits, see
    ...
    def start_delivery_stream_encryption(
        ...
        after the encryption status changes to ``ENABLED`` before all records written to the delivery
        stream are encrypted. To find out whether a record or a batch of records was encrypted, check the
        ...
        If a delivery stream already has encryption enabled and then you invoke this operation to change
        the ARN of the CMK or both its type and ARN and you get ``ENABLING_FAILED`` , this only means that
        the attempt to change the CMK failed. In this case, encryption remains enabled with the old CMK.
        ...
        combined limit of 25 calls per delivery stream per 24 hours. For example, you reach the limit if
        you call ``StartDeliveryStreamEncryption`` 13 times and ``StopDeliveryStreamEncryption`` 12 times
    ...
    def stop_delivery_stream_encryption(
        ...
        This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Data Firehose
        first sets the encryption status of the stream to ``DISABLING`` , and then to ``DISABLED`` . You
        can continue to read and write data to your stream while its status is ``DISABLING`` . It can take
        up to 5 seconds after the encryption status changes to ``DISABLED`` before all records written to
        the delivery stream are no longer subject to encryption. To find out whether a record or a batch of
    ...
    def tag_delivery_stream(
        ...
        replaced with the value that you specify in the request. Tags are metadata. For example, you can
        add friendly names and descriptions or other types of information that can help you distinguish the
    ...
    def update_destination(
        ...
        data writes to the delivery stream can continue during this process. The updated configurations are
        ...
        **Request Syntax**
        ...
        :param CurrentDeliveryStreamVersionId: **[REQUIRED]**
          ...
          interleaving update and this value is null, then the update destination fails. After the update
          is successful, the ``VersionId`` value is updated. The service then performs a merge of the old
