...
class Client(BaseClient):
    ...
    def create_delivery_stream(
        ...
        delivery stream is ``CREATING`` . After the delivery stream is created, its status is
        ``ACTIVE`` and it now accepts data. If the delivery stream creation fails, the status
        transitions to ``CREATING_FAILED`` . Attempts to send data to a delivery stream that is not
        in the ``ACTIVE`` state cause an exception. To check the state of a delivery stream, use
        ...
        A Kinesis Data Firehose delivery stream can be configured to receive records directly from
        ...
        stream Amazon Resource Name (ARN) and role ARN in the ``KinesisStreamSourceConfiguration``
        parameter.
        ...
        By default, if no ``BufferingHints`` value is provided, Kinesis Data Firehose buffers data
        up to 5 MB or for 5 minutes, whichever condition is satisfied first. ``BufferingHints`` is a
        hint, so there are some cases where the service cannot adhere to these conditions strictly.
        For example, record boundaries might be such that the size is a little over or under the
        configured buffering size. By default, no encryption is performed. We strongly recommend
        ...
        * An Amazon Redshift destination requires an S3 bucket as intermediate location. Kinesis
        Data Firehose first delivers data to Amazon S3 and then uses ``COPY`` syntax to load data
        ...
        * We strongly recommend that you use the user name and password you provide exclusively with
        Kinesis Data Firehose, and that the permissions for the account are restricted for Amazon
        Redshift ``INSERT`` permissions.

        Kinesis Data Firehose assumes the IAM role that is configured as part of the destination.
        The role should allow the Kinesis Data Firehose principal to assume the role, and the role
        should have permissions that allow the service to deliver the data. For more information,
        see `Grant Kinesis Data Firehose Access to an Amazon S3 Destination
        <https://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#using-iam-s3>`__ in
        ...
        **Request Syntax**
        ...
        :param DeliveryStreamName: **[REQUIRED]**
          ...
          The name of the delivery stream. This name must be unique per AWS account in the same AWS
          Region. If the delivery streams are in different accounts or different Regions, you can
          have multiple delivery streams with the same name.
        ...
        :param KinesisStreamSourceConfiguration:
          ...
          KinesisStreamSourceConfiguration containing the Kinesis data stream Amazon Resource Name
          (ARN) and the role ARN for the source stream.

          - **KinesisStreamARN** *(string) --* **[REQUIRED]**

            The ARN of the source Kinesis data stream. For more information, see `Amazon Kinesis
            Data Streams ARN Format
        ...
        :param DeliveryStreamEncryptionConfigurationInput:
          ...
          - **KeyARN** *(string) --*
            ...
            Resource Name (ARN) of the CMK. If you set ``KeyType`` to ``AWS_OWNED_CMK`` , Kinesis
          ...
          - **KeyType** *(string) --* **[REQUIRED]**
            ...
            setting is ``AWS_OWNED_CMK`` . For more information about CMKs, see `Customer Master
            Keys (CMKs)
            <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ .
            When you invoke  CreateDeliveryStream or  StartDeliveryStreamEncryption with ``KeyType``
            set to CUSTOMER_MANAGED_CMK, Kinesis Data Firehose invokes the Amazon KMS operation
            `CreateGrant
            <https://docs.aws.amazon.com/kms/latest/APIReference/API_CreateGrant.html>`__ to create
            a grant that allows the Kinesis Data Firehose service to use the customer managed CMK to
            perform encryption and decryption. Kinesis Data Firehose manages that grant.

            When you invoke  StartDeliveryStreamEncryption to change the CMK for a delivery stream
            that is already encrypted with a customer managed CMK, Kinesis Data Firehose schedules
        ...
        :param ElasticsearchDestinationConfiguration:
          ...
          - **RoleARN** *(string) --* **[REQUIRED]**
            ...
            information, see `Grant Kinesis Data Firehose Access to an Amazon S3 Destination
            <https://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#using-iam-s3>`__
          ...
          - **S3BackupMode** *(string) --*
            ...
            ``FailedDocumentsOnly`` , Kinesis Data Firehose writes any documents that could not be
            indexed to the configured Amazon S3 destination, with ``elasticsearch-failed/`` appended
            to the key prefix. When set to ``AllDocuments`` , Kinesis Data Firehose delivers all
            incoming records to Amazon S3, and also writes failed documents with
            ``elasticsearch-failed/`` appended to the prefix. For more information, see `Amazon S3
            Backup for the Amazon ES Destination
            <https://docs.aws.amazon.com/firehose/latest/dev/basic-deliver.html#es-s3-backup>`__ .
    ...
    def delete_delivery_stream(
        ...
        To check the state of a delivery stream, use  DescribeDeliveryStream . You can delete a
        delivery stream only if it is in one of the following states: ``ACTIVE`` , ``DELETING`` ,
        ``CREATING_FAILED`` , or ``DELETING_FAILED`` . You can't delete a delivery stream that is in
        the ``CREATING`` state. While the deletion request is in process, the delivery stream is in
        the ``DELETING`` state.

        While the delivery stream is in the ``DELETING`` state, the service might continue to accept
        records, but it doesn't make any guarantees with respect to delivering the data. Therefore,
        ...
        **Request Syntax**
        ...
        :param AllowForceDelete:
          ...
          <https://docs.aws.amazon.com/kms/latest/APIReference/API_RevokeGrant.html>`__ operation to
    ...
    def describe_delivery_stream(
        ...
        **Request Syntax**
        ...
        :param ExclusiveStartDestinationId:
          ...
          The ID of the destination to start returning the destination information. Kinesis Data
          ...
          ::

            {
                'DeliveryStreamDescription': {
                    ...
                    'FailureDescription': {
                        'Type':
                        'RETIRE_KMS_GRANT_FAILED'|'CREATE_KMS_GRANT_FAILED'
                        |'KMS_ACCESS_DENIED'|'DISABLED_KMS_KEY'|'INVALID_KMS_KEY'
                        |'KMS_KEY_NOT_FOUND'|'KMS_OPT_IN_REQUIRED'|'UNKNOWN_ERROR',
                        'Details': 'string'
                    },
                    'DeliveryStreamEncryptionConfiguration': {
                        ...
                        'KeyType': 'AWS_OWNED_CMK'|'CUSTOMER_MANAGED_CMK',
                        'Status':
                        'ENABLED'|'ENABLING'|'ENABLING_FAILED'|'DISABLED'
                        |'DISABLING'|'DISABLING_FAILED',
                        'FailureDescription': {
                            'Type':
                            'RETIRE_KMS_GRANT_FAILED'|'CREATE_KMS_GRANT_FAILED'
                            |'KMS_ACCESS_DENIED'|'DISABLED_KMS_KEY'
                            |'INVALID_KMS_KEY'|'KMS_KEY_NOT_FOUND'
                            |'KMS_OPT_IN_REQUIRED'|'UNKNOWN_ERROR',
                            'Details': 'string'
                        }
                    },
                    'DeliveryStreamType': 'DirectPut'|'KinesisStreamAsSource',
                    ...
                    'Destinations': [
                        {
                            ...
                            'ElasticsearchDestinationDescription': {
                                ...
                                'TypeName': 'string',
                                'IndexRotationPeriod':
                                'NoRotation'|'OneHour'|'OneDay'|'OneWeek'
                                |'OneMonth',
                                'BufferingHints': {
                                    'IntervalInSeconds': 123,
                                    'SizeInMBs': 123
            ...
            - **DeliveryStreamDescription** *(dict) --*
              ...
              - **DeliveryStreamEncryptionConfiguration** *(dict) --*
                ...
                - **KeyARN** *(string) --*
                  ...
                  If ``KeyType`` is ``CUSTOMER_MANAGED_CMK`` , this field contains the ARN of the
                  customer managed CMK. If ``KeyType`` is ``AWS_OWNED_CMK`` ,
                ...
                - **KeyType** *(string) --*
                  ...
                  default setting is ``AWS_OWNED_CMK`` . For more information about CMKs, see
                ...
                - **Status** *(string) --*
                  ...
                  is ``ENABLING_FAILED`` or ``DISABLING_FAILED`` , it is the status of the most
              ...
              - **VersionId** *(string) --*
                ...
                changed, and the current version ID is required when updating the destination. This
                is so that the service knows it is applying the changes to the correct version of
                the delivery stream.
                ...
                - *(dict) --*
                  ...
                  - **S3DestinationDescription** *(dict) --*
                    ...
                    - **BufferingHints** *(dict) --*
                      ...
                      - **SizeInMBs** *(integer) --*
                        ...
                        ``IntervalInSeconds`` , and vice versa.
                  ...
                  - **ExtendedS3DestinationDescription** *(dict) --*
                    ...
                    - **S3BackupDescription** *(dict) --*
                      ...
                      - **Prefix** *(string) --*
                        ...
                        The "YYYY/MM/DD/HH" time format prefix is automatically used for delivered
                        Amazon S3 files. You can also specify a custom prefix, as described in
                      ...
                      - **BufferingHints** *(dict) --*
                        ...
                        - **SizeInMBs** *(integer) --*
                          ...
                          if you specify a value for it, you must also specify a value for
                          ``IntervalInSeconds`` , and vice versa.
                        ...
                        - **IntervalInSeconds** *(integer) --*
                          ...
                          Buffer incoming data for the specified period of time, in seconds, before
                          delivering it to the destination. The default value is 300. This parameter
                          is optional but if you specify a value for it, you must also specify a
                          value for ``SizeInMBs`` , and vice versa.
                        ...
                        - **Serializer** *(dict) --*
                          ...
                          - **ParquetSerDe** *(dict) --*
                            ...
                            - **Compression** *(string) --*
                              ...
                              ``SNAPPY`` . Use ``SNAPPY`` for higher decompression speed. Use
                          ...
                          - **OrcSerDe** *(dict) --*
                            ...
                            it in Amazon S3. For more information, see `Apache ORC
                  ...
                  - **RedshiftDestinationDescription** *(dict) --*
                    ...
                    - **CopyCommand** *(dict) --*
                      ...
                      - **CopyOptions** *(string) --*
                        ...
                        COPY command <https://docs.aws.amazon.com/redshift/latest/dg/r_COPY.html>`__
                  ...
                  - **ElasticsearchDestinationDescription** *(dict) --*
                    ...
                    - **DomainARN** *(string) --*
                      ...
                      Kinesis Data Firehose uses either ``ClusterEndpoint`` or ``DomainARN`` to send
                      data to Amazon ES.
    ...
    def list_delivery_streams(
        ...
        **Request Syntax**
        ...
        :param ExclusiveStartDeliveryStreamName:
          ...
          The list of delivery streams returned by this call to ``ListDeliveryStreams`` will start
          with the delivery stream whose name comes alphabetically immediately after the name you
    ...
    def list_tags_for_delivery_stream(
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          associated with the delivery stream, ``HasMoreTags`` is set to ``true`` in the response.
            ...
            - **Tags** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Value** *(string) --*
                  ...
                  length: 256 characters. Valid characters: Unicode letters, digits, white space, _
                  . / = + - % @
    ...
    def put_record(
        ...
        By default, each delivery stream can take in up to 2,000 transactions per second, 5,000
        records per second, or 5 MB per second. If you use  PutRecord and  PutRecordBatch , the
        limits are an aggregate across these two operations for each delivery stream. For more
        information about limits and how to request an increase, see `Amazon Kinesis Data Firehose
        ...
        The data record consists of a data blob that can be up to 1,000 KB in size, and any kind of
        ...
        allows the consumer application to parse individual data items when reading the data from
        the destination.

        The ``PutRecord`` operation returns a ``RecordId`` , which is a unique string assigned to
        each record. Producer applications can use this ID for purposes such as auditability and
        investigation.

        If the ``PutRecord`` operation throws a ``ServiceUnavailableException`` , back off and
        retry. If the exception persists, it is possible that the throughput limits have been
        ...
        added to a delivery stream as it tries to send the records to the destination. If the
    ...
    def put_record_batch(
        ...
        records per second, or 5 MB per second. If you use  PutRecord and  PutRecordBatch , the
        limits are an aggregate across these two operations for each delivery stream. For more
        information about limits, see `Amazon Kinesis Data Firehose Limits
        ...
        as large as 1,000 KB (before 64-bit encoding), up to a limit of 4 MB for the entire request.
        ...
        The  PutRecordBatch response includes a count of failed records, ``FailedPutCount`` , and an
        array of responses, ``RequestResponses`` . Even if the  PutRecordBatch call succeeds, the
        value of ``FailedPutCount`` may be greater than 0, indicating that there are records for
        which the operation didn't succeed. Each entry in the ``RequestResponses`` array provides
        additional information about the processed record. It directly correlates with a record in
        the request array using the same ordering, from the top to the bottom. The response array
        always includes the same number of records as the request array. ``RequestResponses``
        includes both successfully and unsuccessfully processed records. Kinesis Data Firehose tries
        to process all records in each  PutRecordBatch request. A single record failure does not
        ...
        might have failed. If ``FailedPutCount`` is greater than 0, retry the request, resending
        only those records that might have failed processing. This minimizes the possible duplicate
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **FailedPutCount** *(integer) --*
              ...
              than 0 even if the  PutRecordBatch call succeeds. Check ``FailedPutCount`` to
            ...
            - **RequestResponses** *(list) --*
              ...
              The results array. For each record, the index of the response element is the same as
              the index used in the request array.

              - *(dict) --*

                Contains the result for an individual record from a  PutRecordBatch request. If the
    ...
    def start_delivery_stream_encryption(
        ...
        Firehose first sets the encryption status of the stream to ``ENABLING`` , and then to
        ``ENABLED`` . The encryption status of a delivery stream is the ``Status`` property in
        ...
        stream while the encryption status is ``ENABLING`` , but the data is not encrypted. It can
        take up to 5 seconds after the encryption status changes to ``ENABLED`` before all records
        written to the delivery stream are encrypted. To find out whether a record or a batch of
        ...
        operation on it to change the ARN of the CMK or both its type and ARN. In this case, Kinesis
        Data Firehose schedules the grant it had on the old CMK for retirement and creates a grant
        ...
        change the ARN of the CMK or both its type and ARN and you get ``ENABLING_FAILED`` , this
        only means that the attempt to change the CMK failed. In this case, encryption remains
        enabled with the old CMK.

        If the encryption status of your delivery stream is ``ENABLING_FAILED`` , you can invoke
        ...
        limit if you call ``StartDeliveryStreamEncryption`` 13 times and
    ...
    def stop_delivery_stream_encryption(self, DeliveryStreamName: str) -> Dict[str, Any]:
        ...
        This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Data
        Firehose first sets the encryption status of the stream to ``DISABLING`` , and then to
        ...
        ``DISABLED`` before all records written to the delivery stream are no longer subject to
        encryption. To find out whether a record or a batch of records was encrypted, check the
        ...
        ``StopDeliveryStreamEncryption`` , Kinesis Data Firehose schedules the related KMS grant for
    ...
    def update_destination(
        ...
        retained. For example, in the Amazon S3 destination, if  EncryptionConfiguration is not
        ...
        S3 to Amazon Redshift, Kinesis Data Firehose does not merge any parameters. In this case,
        all parameters must be specified.

        Kinesis Data Firehose uses ``CurrentDeliveryStreamVersionId`` to avoid race conditions and
        conflicting merges. This is a required field, and the service updates the configuration only
        if the existing configuration has a version ID that matches. After the update is applied
        ...
        **Request Syntax**
        ...
        :param CurrentDeliveryStreamVersionId: **[REQUIRED]**
          ...
          is an interleaving update and this value is null, then the update destination fails. After
          the update is successful, the ``VersionId`` value is updated. The service then performs a
        ...
        :param ElasticsearchDestinationUpdate:
          ...
          - **TypeName** *(string) --*
            ...
            Kinesis Data Firehose still delivers data to Elasticsearch with the old index name and
            type name. If you want to update your delivery stream with a new index name, provide an
            empty string for ``TypeName`` .
