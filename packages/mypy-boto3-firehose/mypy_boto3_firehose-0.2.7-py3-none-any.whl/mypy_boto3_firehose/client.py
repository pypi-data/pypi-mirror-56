"Main interface for firehose Client"
...
class Client(BaseClient):
    ...
    def create_delivery_stream(
        ...
        This is an asynchronous operation that immediately returns. The initial status of the delivery
        stream is ``CREATING`` . After the delivery stream is created, its status is ``ACTIVE`` and it now
        accepts data. If the delivery stream creation fails, the status transitions to ``CREATING_FAILED`` .
        ...
        can't invoke ``CreateDeliveryStream`` again on it. However, you can invoke the  DeleteDeliveryStream
        operation to delete it.

        A Kinesis Data Firehose delivery stream can be configured to receive records directly from providers
        using  PutRecord or  PutRecordBatch , or it can be configured to use an existing Kinesis stream as
        its source. To specify a Kinesis data stream as input, set the ``DeliveryStreamType`` parameter to
        ``KinesisStreamAsSource`` , and provide the Kinesis stream Amazon Resource Name (ARN) and role ARN
        ...
        A delivery stream is configured with a single destination: Amazon S3, Amazon ES, Amazon Redshift, or
        Splunk. You must specify only one of the following destination configuration parameters:
        ``ExtendedS3DestinationConfiguration`` , ``S3DestinationConfiguration`` ,
        ``ElasticsearchDestinationConfiguration`` , ``RedshiftDestinationConfiguration`` , or
        ``SplunkDestinationConfiguration`` .

        When you specify ``S3DestinationConfiguration`` , you can also provide the following optional
        values: BufferingHints, ``EncryptionConfiguration`` , and ``CompressionFormat`` . By default, if no
        ``BufferingHints`` value is provided, Kinesis Data Firehose buffers data up to 5 MB or for 5
        ...
        might be such that the size is a little over or under the configured buffering size. By default, no
        encryption is performed. We strongly recommend that you enable encryption to ensure secure data
        storage in Amazon S3.

        A few notes about Amazon Redshift as a destination:

        * An Amazon Redshift destination requires an S3 bucket as intermediate location. Kinesis Data
        Firehose first delivers data to Amazon S3 and then uses ``COPY`` syntax to load data into an Amazon
        ...
        * We strongly recommend that you use the user name and password you provide exclusively with Kinesis
        Data Firehose, and that the permissions for the account are restricted for Amazon Redshift
        ``INSERT`` permissions.

        Kinesis Data Firehose assumes the IAM role that is configured as part of the destination. The role
        should allow the Kinesis Data Firehose principal to assume the role, and the role should have
        permissions that allow the service to deliver the data. For more information, see `Grant Kinesis
        Data Firehose Access to an Amazon S3 Destination
        <https://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#using-iam-s3>`__ in the
        *Amazon Kinesis Data Firehose Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/CreateDeliveryStream>`_
        ...
        **Request Syntax**
                      ...
                      'Processors': [
                          {
                              ...
                              'Parameters': [
                                  {
                                      'ParameterName':
                                      'LambdaArn'|'NumberOfRetries'|'RoleArn'|'BufferSizeInMBs'
        ...
        :param DeliveryStreamName: **[REQUIRED]**
          ...
          The name of the delivery stream. This name must be unique per AWS account in the same AWS Region.
          If the delivery streams are in different accounts or different Regions, you can have multiple
        ...
        :param DeliveryStreamEncryptionConfigurationInput:
          ...
          - **KeyType** *(string) --* **[REQUIRED]**
            ...
            <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#master_keys>`__ . When you
            ...
            <https://docs.aws.amazon.com/kms/latest/APIReference/API_CreateGrant.html>`__ to create a grant
            ...
            already encrypted with a customer managed CMK, Kinesis Data Firehose schedules the grant it had
            on the old CMK for retirement.
        ...
        :param ElasticsearchDestinationConfiguration:
          ...
          - **TypeName** *(string) --*
            ...
            The Elasticsearch type name. For Elasticsearch 6.x, there can be only one type per index. If you
            try to specify a new type for an existing index that already has another type, Kinesis Data
        ...
        :param Tags:
          ...
          assign to AWS resources. Tags are metadata. For example, you can add friendly names and
          descriptions or other types of information that can help you distinguish the delivery stream. For
          more information about tags, see `Using Cost Allocation Tags
    ...
    def delete_delivery_stream(
        ...
        To check the state of a delivery stream, use  DescribeDeliveryStream . You can delete a delivery
        stream only if it is in one of the following states: ``ACTIVE`` , ``DELETING`` , ``CREATING_FAILED``
        , or ``DELETING_FAILED`` . You can't delete a delivery stream that is in the ``CREATING`` state.
        While the deletion request is in process, the delivery stream is in the ``DELETING`` state.

        While the delivery stream is in the ``DELETING`` state, the service might continue to accept
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/DeleteDeliveryStream>`_
        ...
        **Request Syntax**
        ...
        :param AllowForceDelete:
          ...
          Set this to true if you want to delete the delivery stream even if Kinesis Data Firehose is unable
          to retire the grant for the CMK. Kinesis Data Firehose might be unable to retire the grant due to
          a customer error, such as when the CMK or the grant are in an invalid state. If you force
          deletion, you can then use the `RevokeGrant
          <https://docs.aws.amazon.com/kms/latest/APIReference/API_RevokeGrant.html>`__ operation to revoke
          the grant you gave to Kinesis Data Firehose. If a failure to retire the grant happens due to an
          AWS KMS issue, Kinesis Data Firehose keeps retrying the delete operation.
    ...
    def describe_delivery_stream(
        ...
        """
        Describes the specified delivery stream and its status. For example, after your delivery stream is
        created, call ``DescribeDeliveryStream`` to see whether the delivery stream is ``ACTIVE`` and
        therefore ready for data to be sent to it.

        If the status of a delivery stream is ``CREATING_FAILED`` , this status doesn't change, and you
        can't invoke  CreateDeliveryStream again on it. However, you can invoke the  DeleteDeliveryStream
        operation to delete it. If the status is ``DELETING_FAILED`` , you can force deletion by invoking
        DeleteDeliveryStream again but with  DeleteDeliveryStreamInput$AllowForceDelete set to true.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/DescribeDeliveryStream>`_
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          The limit on the number of destinations to return. You can have one destination per delivery
          stream.
          ...
          ::

            {
                'DeliveryStreamDescription': {
                    ...
                    'FailureDescription': {
                        'Type':
                        'RETIRE_KMS_GRANT_FAILED'|'CREATE_KMS_GRANT_FAILED'|'KMS_ACCESS_DENIED'
                        |'DISABLED_KMS_KEY'|'INVALID_KMS_KEY'|'KMS_KEY_NOT_FOUND'|'KMS_OPT_IN_REQUIRED'
                        |'UNKNOWN_ERROR',
                        'Details': 'string'
                    },
                                    ...
                                    'Processors': [
                                        {
                                            ...
                                            'Parameters': [
                                                {
                                                    'ParameterName':
                                                    'LambdaArn'|'NumberOfRetries'|'RoleArn'
    ...
    def list_delivery_streams(
        ...
        ``ListDeliveryStreams`` . You can limit the number of delivery streams returned, using the ``Limit``
        parameter. To determine whether there are more delivery streams to list, check the value of
        ``HasMoreDeliveryStreams`` in the output. If there are more delivery streams to list, you can
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/ListDeliveryStreams>`_
        ...
        **Request Syntax**
        ...
        :param DeliveryStreamType:
          ...
          * ``KinesisStreamAsSource`` : The delivery stream uses a Kinesis data stream as a source.
          ...
          This parameter is optional. If this parameter is omitted, delivery streams of all types are
          returned.
    ...
    def list_tags_for_delivery_stream(
        ...
        """
        Lists the tags for the specified delivery stream. This operation has a limit of five transactions
        per second per account.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/ListTagsForDeliveryStream>`_
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          the delivery stream, ``HasMoreTags`` is set to ``true`` in the response. To list additional tags,
    ...
    def put_record(
        ...
        """
        Writes a single data record into an Amazon Kinesis Data Firehose delivery stream. To write multiple
        data records into a delivery stream, use  PutRecordBatch . Applications using these operations are
        referred to as producers.

        By default, each delivery stream can take in up to 2,000 transactions per second, 5,000 records per
        second, or 5 MB per second. If you use  PutRecord and  PutRecordBatch , the limits are an aggregate
        across these two operations for each delivery stream. For more information about limits and how to
        request an increase, see `Amazon Kinesis Data Firehose Limits
        <https://docs.aws.amazon.com/firehose/latest/dev/limits.html>`__ .

        You must specify the name of the delivery stream and the data record when using  PutRecord . The
        ...
        Kinesis Data Firehose buffers records before delivering them to the destination. To disambiguate the
        data blobs at the destination, a common solution is to use delimiters in the data, such as a newline
        (``\\n`` ) or some other character unique within the data. This allows the consumer application to
        ...
        Data records sent to Kinesis Data Firehose are stored for 24 hours from the time they are added to a
        ...
        .. warning::
          ...
          Don't concatenate two or more base64 strings to form the data fields of your records. Instead,
          concatenate the raw data, then perform base64 encoding.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/PutRecord>`_
    ...
    def put_record_batch(
        ...
        """
        Writes multiple data records into a delivery stream in a single call, which can achieve higher
        throughput per producer than when writing single records. To write single data records into a
        delivery stream, use  PutRecord . Applications using these operations are referred to as producers.

        By default, each delivery stream can take in up to 2,000 transactions per second, 5,000 records per
        second, or 5 MB per second. If you use  PutRecord and  PutRecordBatch , the limits are an aggregate
        across these two operations for each delivery stream. For more information about limits, see `Amazon
        Kinesis Data Firehose Limits <https://docs.aws.amazon.com/firehose/latest/dev/limits.html>`__ .

        Each  PutRecordBatch request supports up to 500 records. Each record in the request can be as large
        as 1,000 KB (before 64-bit encoding), up to a limit of 4 MB for the entire request. These limits
        cannot be changed.

        You must specify the name of the delivery stream and the data record when using  PutRecord . The
        data record consists of a data blob that can be up to 1,000 KB in size, and any kind of data. For
        example, it could be a segment from a log file, geographic location data, website clickstream data,
        and so on.
        ...
        didn't succeed. Each entry in the ``RequestResponses`` array provides additional information about
        the processed record. It directly correlates with a record in the request array using the same
        ordering, from the top to the bottom. The response array always includes the same number of records
        as the request array. ``RequestResponses`` includes both successfully and unsuccessfully processed
        records. Kinesis Data Firehose tries to process all records in each  PutRecordBatch request. A
        ...
        reflects the type of error, and is one of the following values: ``ServiceUnavailableException`` or
        ``InternalFailure`` . ``ErrorMessage`` provides more detailed information about the error.

        If there is an internal server error or a timeout, the write might have completed or it might have
        failed. If ``FailedPutCount`` is greater than 0, retry the request, resending only those records
        that might have failed processing. This minimizes the possible duplicate records and also reduces
        the total bytes sent (and corresponding charges). We recommend that you handle any duplicates at the
        destination.
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/PutRecordBatch>`_
    ...
    def start_delivery_stream_encryption(
        ...
        This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Data Firehose
        first sets the encryption status of the stream to ``ENABLING`` , and then to ``ENABLED`` . The
        ...
        ``ENABLING_FAILED`` . You can continue to read and write data to your delivery stream while the
        encryption status is ``ENABLING`` , but the data is not encrypted. It can take up to 5 seconds after
        the encryption status changes to ``ENABLED`` before all records written to the delivery stream are
        encrypted. To find out whether a record or a batch of records was encrypted, check the response
        elements  PutRecordOutput$Encrypted and  PutRecordBatchOutput$Encrypted , respectively.

        To check the encryption status of a delivery stream, use  DescribeDeliveryStream .

        Even if encryption is currently enabled for a delivery stream, you can still invoke this operation
        on it to change the ARN of the CMK or both its type and ARN. In this case, Kinesis Data Firehose
        schedules the grant it had on the old CMK for retirement and creates a grant that enables it to use
        the new CMK to encrypt and decrypt data and to manage the grant.

        If a delivery stream already has encryption enabled and then you invoke this operation to change the
        ARN of the CMK or both its type and ARN and you get ``ENABLING_FAILED`` , this only means that the
        attempt to change the CMK failed. In this case, encryption remains enabled with the old CMK.

        If the encryption status of your delivery stream is ``ENABLING_FAILED`` , you can invoke this
        operation again.
        ...
        combined limit of 25 calls per delivery stream per 24 hours. For example, you reach the limit if you
        call ``StartDeliveryStreamEncryption`` 13 times and ``StopDeliveryStreamEncryption`` 12 times for
        the same delivery stream in a 24-hour period.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/StartDeliveryStreamEncryption>`_
    ...
    def stop_delivery_stream_encryption(
        ...
        This operation is asynchronous. It returns immediately. When you invoke it, Kinesis Data Firehose
        first sets the encryption status of the stream to ``DISABLING`` , and then to ``DISABLED`` . You can
        continue to read and write data to your stream while its status is ``DISABLING`` . It can take up to
        5 seconds after the encryption status changes to ``DISABLED`` before all records written to the
        ...
        ensures that it is finished delivering records to the destination.
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/StopDeliveryStreamEncryption>`_
    ...
    def tag_delivery_stream(
        ...
        """
        Adds or updates tags for the specified delivery stream. A tag is a key-value pair that you can
        define and assign to AWS resources. If you specify a tag that already exists, the tag value is
        replaced with the value that you specify in the request. Tags are metadata. For example, you can add
        friendly names and descriptions or other types of information that can help you distinguish the
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/TagDeliveryStream>`_
    ...
    def untag_delivery_stream(
        ...
        """
        Removes tags from the specified delivery stream. Removed tags are deleted, and you can't recover
        them after this operation successfully completes.
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/UntagDeliveryStream>`_
    ...
    def update_destination(
        ...
        Use this operation to change the destination type (for example, to replace the Amazon S3 destination
        with Amazon Redshift) or change the parameters associated with a destination (for example, to change
        the bucket name of the Amazon S3 destination). The update might not occur immediately. The target
        delivery stream remains active while the configurations are updated, so data writes to the delivery
        stream can continue during this process. The updated configurations are usually effective within a
        few minutes.

        Switching between Amazon ES and other services is not supported. For an Amazon ES destination, you
        can only update to another Amazon ES destination.

        If the destination type is the same, Kinesis Data Firehose merges the configuration parameters
        specified with the destination configuration that already exists on the delivery stream. If any of
        the parameters are not specified in the call, the existing values are retained. For example, in the
        ...
        If the destination type is not the same, for example, changing the destination from Amazon S3 to
        Amazon Redshift, Kinesis Data Firehose does not merge any parameters. In this case, all parameters
        must be specified.

        Kinesis Data Firehose uses ``CurrentDeliveryStreamVersionId`` to avoid race conditions and
        conflicting merges. This is a required field, and the service updates the configuration only if the
        existing configuration has a version ID that matches. After the update is applied successfully, the
        version ID is updated, and can be retrieved using  DescribeDeliveryStream . Use the new version ID
        to set ``CurrentDeliveryStreamVersionId`` in the next call.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/firehose-2015-08-04/UpdateDestination>`_
        ...
        **Request Syntax**
        ...
        :param CurrentDeliveryStreamVersionId: **[REQUIRED]**
          ...
          required, and helps the service perform conditional operations. For example, if there is an
        ...
        :param ElasticsearchDestinationUpdate:
          ...
          - **DomainARN** *(string) --*
            ...
            The ARN of the Amazon ES domain. The IAM role must have permissions for
            ``DescribeElasticsearchDomain`` , ``DescribeElasticsearchDomains`` , and
            ``DescribeElasticsearchDomainConfig`` after assuming the IAM role specified in ``RoleARN`` . For
            more information, see `Amazon Resource Names (ARNs) and AWS Service Namespaces
            <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`__ .
          ...
          - **TypeName** *(string) --*
            ...
            Firehose still delivers data to Elasticsearch with the old index name and type name. If you want
            to update your delivery stream with a new index name, provide an empty string for ``TypeName`` .
