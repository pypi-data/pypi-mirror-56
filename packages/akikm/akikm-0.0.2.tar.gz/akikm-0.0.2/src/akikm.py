...
class KM(object):
    ...
    _vk = {
        ...
        "n": 0x4E, "o": 0x4F, "p": 0x50, "q": 0x51, "r": 0x52, "s": 0x53, "t": 0x54, "u": 0x55, "v": 0x56, "w": 0x57,
        "x": 0x58, "y": 0x59, "z": 0x5A,
        # 数字键
        "0": 0x60, "1": 0x61, "2": 0x62, "3": 0x63, "4": 0x64, "5": 0x65, "6": 0x66, "7": 0x67, "8": 0x68, "9": 0x69,
        # oem
        ',': 0xBC, '.': 0xBE, '-': 0xBD, '[': 0xDB, ']': 0xDD, ';': 0xBA, "'": 0xDE, '\\': 0xDC, ',': 0xbc, '`': 0xC0,
        '=': 0xBB, '/': 0xBF
    }

    _vk_2 = {
        '~': 0xC0, '!': 0x31, '@': 0x32, '#': 0x33, '$': 0x34, '%': 0x35, '^': 0x36, '&': 0x37, '*': 0x38, '(': 0x39,
        ')': 0x30,
    ...
    def keyboard(self, text, delayed=0.0):
        """字符输入"""
        if type(text) == VK:
            self.key_down(text.value)
            time.sleep(delayed)
            self.key_up(text.value)
            return None
        for t in str(text):
    ...
    @staticmethod
    def key_down(vk_code, delayed=0.0):
        """键盘按键按下"""
        win32api.keybd_event(vk_code, win32api.MapVirtualKey(vk_code, 0), 0, 0)
        time.sleep(delayed)

    @staticmethod
    def key_up(vk_code, delayed=0.0):
        """键盘按键释放"""
    ...
    @staticmethod
    def mouse_left_click(delayed=0.0):
        """鼠标左键单击"""
    ...
    @staticmethod
    def mouse_left_move_click(x, y, delayed=0.0):
        """鼠标左键移动到坐标单击"""
        win32api.SetCursorPos([x, y])
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
        time.sleep(delayed)
    ...
    @staticmethod
    def mouse_right_click(delayed=0.0):
        """鼠标右键单击"""
    ...
    @staticmethod
    def mouse_right_move_click(x, y, delayed=0.0):
        """鼠标右键移动到坐标单击"""
        win32api.SetCursorPos([x, y])
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
        time.sleep(delayed)
