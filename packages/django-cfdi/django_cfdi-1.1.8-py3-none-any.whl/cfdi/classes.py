...
from .settings import (
    TMP_DIR, DFACTURE_AUTH, PRODIGIA_AUTH, NTLINK_AUTH, XSLT_PATH,
    STO_AUTH,
)
...
class CFDI:
    ...
    def __init__(self, *args, **kwargs):
        ...
        #NTLink
        self.ntlink_url = "http://ntlink.com.mx/ntfacturacionws/ServicioTimbrado.svc?wsdl"
        self.ntlink_usuario = NTLINK_AUTH["prod"]["usuario"]
        self.ntlink_password = NTLINK_AUTH["prod"]["password"]

        self.ntlink_pruebas_url = "http://pruebas.ntlink.com.mx/Certificador/ServicioTimbrado.svc?wsdl"
        self.ntlink_pruebas_usuario = NTLINK_AUTH["dev"]["usuario"]
        self.ntlink_pruebas_password = NTLINK_AUTH["prod"]["password"]
                    ...
                    for otropago in self.nomina["OtrosPagos"]["otrospagos"]:
                        ...
                        if otropago.get("CompensacionSaldosAFavor"):
                            xml += '<nomina12:CompensacionSaldosAFavor SaldoAFavor="%s" Año="%s" RemanenteSalFav="%s"/>' % (
                                otropago["SubsidioAlEmpleo"]["SaldoAFavor"],
                                otropago["SubsidioAlEmpleo"]["Año"],
                                otropago["SubsidioAlEmpleo"]["RemanenteSalFav"],
                            )
    ...
    def timbrar_xml(self):
        ...
        if self.PAC in ["prodigia", PACS["PRODIGIA"]] or not self.PAC:
            self.PAC = PACS["PRODIGIA"]
            return self._timbrar_prodigia()    
        
        elif self.PAC in ["stoconsulting", PACS["STOCONSULTING"]]:
            self.PAC = PACS["STOCONSULTING"]
            return self._timbrar_stoconsulting()

        elif self.PAC in ["finkok", PACS["FINKOK"]]:
            self.PAC = PACS["FINKOK"]
            return self._timbrar_finkok()

        elif self.PAC in ["prueba", PACS["PRUEBA"]]:
            self.PAC = PACS["PRUEBA"]
            return self._timbrar_prueba()

        elif self.PAC in ["dfacture", PACS["DFACTURE"]]:
            self.PAC = PACS["DFACTURE"]
            return self._timbrar_dfacture()
    ...
    def _timbrar_dfacture(self):
        ...
        try:
            respuesta_timbrado = requests.post(url, data=data, verify=False)
        except Exception as e:
    ...
    def _timbrar_prodigia(self):
        ...
        try:
            respuesta_timbrado = requests.post(url, data=xml.encode("utf-8"), headers=headers)
        except Exception as e:
        ...
        self.fin_timbrado = timezone.now()
        self.fin_conexion_pac = self.inicio_conexion_pac
        if not respuesta_timbrado.status_code in [200, 202] and not self.recuperar_xml:
    ...
    def _timbrar_stoconsulting(self):
        ...
        try:
            respuesta_timbrado = requests.post(url, json=data)#, timeout=20) 
        except Exception as e:
    ...
    def _timbrar_diverza(self):
        ...
        if self.TIMBRADO_PRUEBAS:
            ...
            url = "https://staging.diverza.com/stamp"
        else:
            url = "https://rest.timbrefiscal.mx/stamp"
            if self.emisor_rfc == "DDE9907137SA" or self.emisor_rfc == "DAU080801BTA":
        ...
        #try:
        requests.post(url)
        respuesta_timbrado = requests.post(url, xml.encode("utf-8"), headers={ "x-auth-token":x_auth_token, })
        #except Exception as e: #except (SAXParseException, URLError) as e:
        #    self.cfdi_status = u"Respuesta inesperada del PAC al intentar timbrar: %s" % repr(e)
        #    return False
        self.fin_timbrado = timezone.now()
        self.fin_conexion_pac = self.fin_timbrado = self.fin_timbrado

        if not respuesta_timbrado.status_code == 200:
            try:
                self.cfdi_status = respuesta_timbrado.content.decode("utf-8")
            except:
                self.cfdi_status = respuesta_timbrado.content

            self.cfdi_status = "%s -diverza %s" % (self.cfdi_status, self.TIMBRADO_PRUEBAS)
            return False

        else:
            respuesta_timbrado_content = respuesta_timbrado.content

        if "<tfd:TimbreFiscalDigital" in respuesta_timbrado_content:
            timbre_xml = respuesta_timbrado_content.split("<tfd:TimbreFiscalDigital")[1].split("/>")[0]
            self.cfdi_uuid = timbre_xml.split('UUID="')[1].split('"')[0]
            fecha_cfdi = timbre_xml.split('FechaTimbrado="')[1].split('"')[0]
            self.cfdi_fecha_timbrado = datetime.strptime(fecha_cfdi , "%Y-%m-%dT%H:%M:%S" )
            self.cfdi_sello_digital = timbre_xml.split('selloCFD="')[1].split('"')[0]
    ...
    def cancelar_cfdi(self, dfacture=False):
        if self.TIMBRADO_PRUEBAS:
            return [False, u"Los ambientes en modo de pruebas solo pueden obtener acuses de cancelación de facturas hechas en modo de prueba."]
        ...
        if 'oCertificadoSAT="200' in self.cfdi_xml or \
            ...
            return [True, "CANCELACION EN MODO DE PRUEBAS"]
        ...
        if rfc_pac:
            fncancel = {
                ...
                #'PPD101129EA3':self.cancelar_dfacture,
    ...
    def cancelar_prodigia(self):
        ...
        try:
            msg = client.service.cancelarConOpciones(
                self.prodigia_contrato,
                self.prodigia_usuario,
                self.prodigia_password,
                self.rfc,
        ...
        msg = str(msg)
        if "ancelado exitosamente" in msg or "reviamente cancelado" in msg:
            if "<acuseCancelBase64>" in msg:
                x = msg
                acuse_base64 = x.split("<acuseCancelBase64>")[1].split("</acuseCancelBase64>")[0]
                import base64
                self.acuse_cancelacion = base64.b64decode(acuse_base64)
    ...
    def dfactura_consulta_cfdi(self, tipo_consulta):
        ...
        if tipo_consulta == 5:
            ...
            try:
                self.rfc_receptor = "%s" % unescape(self.cfdi_xml.split("<cfdi:Receptor ")[1].split('Rfc="')[1].split('"')[0])
        ...
        try:
            respuesta = requests.post(url, data=data, verify=False).text
        except Exception as e:
            self.error_conexion = "Hubo un error al intentar conectarse con el PAC: %s" % e
            return [False, self.error_conexion]
    ...
    def cancelar_stoconsulting(self):
        ...
        respuesta = requests.post(url, json=data, timeout=50)
    ...
    def cancelar_diverza(self):
        ...
        respuesta = requests.post(url, headers={ "x-auth-token":x_auth_token, })
        msg = respuesta.text
...
class XmlNewObject:
    def __init__(self, *args, **kwargs):
        ...
        self.lista_etiqueta = []
        self.prefix = kwargs.get("prefix") or "cfdi"
    ...
    def set_lista_etiqueta(self, nombre_elemento, prefix=None, ignorar_prefijo=False):

        if not ignorar_prefijo:
    ...
    def get_elemento(self, nombre, index=0, prefix=None):
        if self.num_elementos <= 0:
            return self.__class__(texto="", prefix=(prefix or self.prefix))
        obj = self.__class__(prefix=(prefix or self.prefix))
