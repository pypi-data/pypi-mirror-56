...
class STGraph(ctypes.Structure):
	_fields_ = [("__unused", ctypes.c_uint64), ("__itype", ctypes.c_uint8), ("T", ctypes.c_uint64), ("base0", ctypes.c_uint64), ("Tm1inv", ctypes.c_float)]

	LOCAL_G = None

	@property
	def base(self):
		if self.LOCAL_G is None:
			#print("CONSTRUCTING LOCAL_G IN STGraph.base()")
			self.LOCAL_G = ctypes.cast(self.base0, ctypes.POINTER(Graph))
		return self.LOCAL_G
...
class Model(ctypes.Structure):
	...
	def slice_edge(self, e, A):
		...
		if len(A) < self.dimension:
			raise TypeError('A must be (at least) ' + str(self.dimension) + ' dimensional')
	...
	def predict(self, observed, progress_hook = 0, iterations = 100):
		if self.destroyed:
			return numpy.array([])
		L = []
		self.__set_ctx_state()
	...
	def sample(self, observed = None, num_samples = None, sampler = Sampler.apx_perturb_and_map, progress_hook = 0, iterations = 100, perturbation = 0.1, burn = 100):
		if self.destroyed:
			return numpy.array([])
	...
	def probv(self, v, x):
		...
		if self.__marginals is None:
			raise RuntimeError('you must call infer first')
...
def train(data, graph, mode = Mode.mrf, inference = Inference.belief_propagation, iters = 1000, seed = 0, k = 3, in_model = None, initial_stepsize = 0.1, opt_progress_hook = 0, progress_hook = 0, opt_regularization_hook = 0, opt_proximal_hook = 0, T = 1, edge_fraction = 0.1, lam = 0, shared_states = False):
	...
	if in_model is None:
		...
		in_model._Model__set_ctx_state()
