...
class Model(ctypes.Structure):
	...
	def slice_edge(self, e, A):
		...
		if len(A) != self.dimension:
			raise TypeError('A must be ' + str(self.dimension) + ' dimensional')
	...
	def predict(self, observed, generic_progress_hook = 0, iterations = 100):
		L = []
		self.set_ctx_state()
	...
	def probv(self, v, x, observed = None, inference = Inference.belief_propagation, iterations = 100, k = 3):
		if observed is not None:
			if not isinstance(observed, numpy.ndarray):
				raise TypeError('observed must be of type numpy.ndarray')
			if len(observed.shape) == 1:
		...
		if not (numpy.array_equal(observed, self.__observed) and self.__marginals is not None and self.__A is not None):
	...
	def prob(self, v, x, w = None, y = None, observed = None, inference = Inference.belief_propagation, iterations = 100, k = 3):
		if observed is not None:
			if not isinstance(observed, numpy.ndarray):
				raise TypeError('observed must be of type numpy.ndarray')
			if len(observed.shape) == 1:
		...
		if w is None or y is None:
			return self.probv(v, x, observed, inference, iterations, k)
		...
		return P[int(x0 * self.states[x1] + x1)]
