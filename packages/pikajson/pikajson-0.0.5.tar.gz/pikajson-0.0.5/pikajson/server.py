...
class PikaConsumer:
    ...
    def __init__(self, handlers, queue, host, port, user, password):
        self._queue = queue
        self._host = host
        self._port = port
        self._user = user
    ...
    def create_from_config(cls, handlers, config_path, config_section='consumer'):
        ...
        return cls(handlers, queue, host, port, user, password)
    ...
    def disconnect(self):
        self._disconnection_cs.acquire()
        if self._connection is not None and not self._connection.is_closed:
            ...
            try:
                ...
                logging.error("rabbit disconnect error: %s" % str(ex),
                              extra={"queue": self._queue, "host": self._host})
        self._disconnection_cs.release()
