...
class PikaPublisher:
    ...
    def __init__(self, queue, host, port, user, password, max_length=None):
        self._queue = queue
        self._host = host
        self._port = port
        self._user = user
    ...
    def create_from_config(cls, config_path, config_section='publisher'):
        ...
        return cls(queue, host, port, user, password, max_length)
        ...
        try:
            ...
            while not sent:
                try:
                    ...
                    logging.error("queue %s is overflowed, retry in %d seconds..." % (self._queue, self.retry_timeout),
                                  extra={"queue": self._queue, "host": self._host})
