...
def startALIXEforARCIMBOLDO(clust_fold, cluster_id, reference_hkl, cell, sg_symbol, tolerance_list=[60.0, 88.0],
    ...
    ent_present = False
    # TODO CM: change the hard_limit to automatic decision depending on hardware
    hard_limit_phs = 1000
    ...
    if mode=='one_step':
        clust_fold = os.path.abspath(os.path.join(basepath_clustfold, str(cluster_id)))
        logging.info('\n Starting ALIXE on folder ' + clust_fold)
        ...
        if os.path.exists(path_for_tuple):  # Then first round was completed
            ...
            logging.info("\n ALIXE first round has run succesfully already on this folder. Returning! ")
            # Return only the merged clusters for expansion
            for cluster in final_dict.keys():
                if final_dict[cluster]['n_phs'] == 1:
                    del final_dict[cluster]
            print "\n"+str(len(final_dict))+' phase clusters will be sent for expansion with SHELXE'
    ...
    if run == 'SHREDDER' or run == 'BORGES':
        ...
        if not skip_copyfiles:
            list_chosen = al.get_files_from_9_EXP_BORGES(wd=basepath, clust_fold=clust_fold,
    ...
    if mode == 'one_step':
        ...
        # if run == 'SHREDDER':
        #     new_for_clust_fold_rel=(ADT.split_string_by_common(basepath, current_cwd))[2]
        #     clust_fold_rel=os.path.join("."+new_for_clust_fold_rel+"/11.5_CLUSTERING/",os.path.basename(clust_fold))
        # elif run == 'BORGES':
        ...
        #     new_for_clust_fold_rel=(ADT.split_string_by_common(basepath, current_cwd))[2]
        #     clust_fold_rel=os.path.join("."+new_for_clust_fold_rel+"/11.5_CLUSTERING/",os.path.basename(clust_fold))
        # # NOTE CM temporary to check for very long paths
        # # print('SHERLOCK len(dictio_fragments.keys()[0])',len(dictio_fragments.keys()[0]))
        # # print('SHERLOCK len(dictio_fragments.keys()[0]+".phs")', len(dictio_fragments.keys()[0]+".phs"))
        # # print('SHERLOCK dictio_fragments.keys()[0]',dictio_fragments.keys()[0])
        # # print('SHERLOCK os.path.basename(dictio_fragments.keys()[0])', os.path.basename(dictio_fragments.keys()[0]))
        # # print('SHERLOCK clust_fold_rel+"/"+os.path.basename(dictio_fragments.keys()[0])+".phs"', clust_fold_rel+"/"+os.path.basename(dictio_fragments.keys()[0])+".phs")
        # # print('SHERLOCK clust_fold_rel',clust_fold_rel)
        # # esasuma=len(clust_fold_rel+"/"+os.path.basename(dictio_fragments.keys()[0])+".phs")
        # # if esasuma>=80:
        # #     print('\n SHERLOCK too long name for clust_fold_rel')
        # #     print('\n SHERLOCK len(clust_fold_rel)',len(clust_fold_rel))
        # #     print('\nSHERLOCK clust_fold_rel', clust_fold_rel)
        # #     quit()
        ...
        # TODO CM: change to the linking function
        # 2.1) Link the ins file
        ...
        if dictio_space_groups[sg_number]['polar'] and sg_number==1: # at the moment I am interested in testing fast!
            ...
            orisub='sxos'
        # NOTE CM: moving to use seed 1 to accelerate
        dict_first_round = al.clustering_phstat_isa_under_a_tolerance(name_phstat='first_round',
                                                                      wd=clust_fold,
                                                                      path_phstat=SELSLIB2.PATH_PHSTAT,
                                                                      tolerance=tolerance,resolution=resolution,
                                                                      seed=1, n_cycles=cycles,
                                                                      orisub=orisub, weight='e',
                                                                      idrotclu=cluster_id,oricheck=True,
                                                                      mapcc=True)
        ...
        for cluster in dict_first_round.keys():
            if dict_first_round[cluster]['n_phs'] > 1:
                ...
                for phs in dict_first_round[cluster]['dictio_result'].keys():  # For each phs in the cluster
                    fullpath_file = os.path.normpath(os.path.join(clust_fold, phs))
                ...
                fullpath_file = os.path.normpath(os.path.join(clust_fold,name_file))
    ...
    if mode == 'two_steps':
        ...
        # NOTE CM: at the moment I am using the topLLG of the solutions in every cluster from the first round
        # I should include other foms and evaluate which ones work better
        ...
        for i in range(len(sorted_list_phis)):
            ...
            # NOTECM: trying to get rid of the relative paths issues
            print('SHERLOCK phs_fullpath',phs_fullpath)
            print('SHERLOCK phs_dest_path',phs_dest_path)
        ...
        # TODOCM: changing to not relative path
        # Next thing to try move to SEED 1 to accelerate
        dict_second_round = al.clustering_phstat_isa_under_a_tolerance(name_phstat='second_round', wd=clust_fold,
                                                                       path_phstat=SELSLIB2.PATH_PHSTAT,
                                                                       tolerance=tolerance, resolution=resolution,
                                                                       seed=1, n_cycles=cycles,
                                                                       orisub=orisub, weight='e')
