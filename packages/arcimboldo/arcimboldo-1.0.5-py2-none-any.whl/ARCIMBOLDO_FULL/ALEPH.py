    ...
    if args.target and os.path.exists(args.target): Bioinformatics3.rename_hetatm_and_icode(args.target)
        ...
        for path in paths:
            generate_library(directory_database=dbtemp,
                             ...
                             swap_superposition=True, reverse=args.reverse)
...
def decompose_by_community_clustering_starter(args):
    ...
    decompose_by_community_clustering(reference=args.pdbmodel, strictness_ah=args.strictness_ah,
                                          ...
                                          min_ah_dist=args.min_ah_dist, max_bs_dist=args.max_bs_dist,
                                          min_bs_dist=args.min_bs_dist, write_graphml=args.write_graphml,
                                          write_pdb=True,  algorithm=args.algorithm)
...
def generate_library_starter(args):
    ...
    dic_json = {'mode': 'library generation', 'submode': 'superposition based on Characteristic Vectors',
                ...
                'local_grid': args.local_grid,'remote_grid': args.remote_grid,'supercomputer': args.supercomputer,}}
    ...
    clustering_mode=args.clustering_mode, number_of_ranges=args.number_of_ranges, number_of_clusters=args.number_of_clusters, exclude_sequence=args.exclude_sequence)
...
def ss_dict_from_graph(g, reference):
    ss_dict = {reference: {'secondary_structure_number': g.vcount(),
                            'ah_number': sum(1 for frag in g.vs() if frag['sstype'] == 'ah'),
                            'bs_number': sum(1 for frag in g.vs() if frag['sstype'] == 'bs')}}
    return ss_dict
    ...
    def __generate_graph(a, min_ah, min_bs, associations):
        ...
        listaFrags = [
            {"sstype": associations[fragl[0]]["result"].lower(),
             ...
             "strictnesses": [t[1] for t in sorted(set([(e[3],e[4]) for fr in fragl for e in a if list(fr) in e[2]]), key=lambda hl: hl[0])]}
            ...
            if o > 0 and len(listaFrags[o - 1]["reslist"]) > 2 and len(listaFrags[o]["reslist"]) > 2 and \
                ...
                if Bioinformatics3.check_continuity(resaN, prevResC):
                    ...
                    if id1_gly >= 0:
                        ...
                        dirco = {"sstype": "coil", "reslist": [listaFrags[o - 1]["reslist"][-1]],
                                 "cvids": [listaFrags[o - 1]["cvids"][-1]],
                                 "cvls": [listaFrags[o - 1]["cvls"][-1]], "sequence": "".join(list(map(lambda x: Bioinformatics3.AADICMAP[x[4]], [listaFrags[o - 1]["reslist"][-1]]))),
                                 "strictnesses": [listaFrags[o - 1]["strictnesses"][-1]]}
...
def annotate_pdb_model_with_aleph(pdb_model, weight="distance_avg", min_ah=4, min_bs=3, write_pdb=True,
                                  strictness_ah=0.45, strictness_bs=0.20, peptide_length=3, is_model=False, only_reformat=True, path_base="./", verbose=True):
    """
    ...
    return generate_matrix_and_graph(strucc, cvs_list, pdb_model, weight=weight, min_ah=min_ah, min_bs=min_bs, write_pdb=write_pdb,
                                     strictness_ah=strictness_ah, strictness_bs=strictness_bs, is_model=is_model, mixed_chains=True, path_base=path_base, verbose=verbose)
...
def parse_pdb_and_generate_cvs(pdbmodel, strucc=None, peptide_length=3, one_model_per_nmr=True, only_reformat=True, process_only_chains=None):
    if not strucc: strucc = Bioinformatics3.get_structure("stru", pdbmodel)
...
def generate_matrix_and_graph(strucc, cvs_list, pdb_model, weight="distance_avg", min_ah=4, min_bs=3, write_pdb=True,
                              ...
                              maximum_distance=None, maximum_distance_bs=None, just_diagonal_plus_one=False, path_base="./", verbose=True):
    """
    ...
    for r in Bioinformatics3.get_list_of_residues(strucc):
        if tuple(r.get_full_id()[1:4]) not in resil and r.get_resname() in Bioinformatics3.AADICMAP:
            toadd.append({"reslist":[r.get_full_id()], "sstype":"coil", "resIdList":[r.get_full_id()], "cvids":[],"strictnesses":[], "cvls":[], "sequence":"".join([Bioinformatics3.AADICMAP[r.get_resname()]])})
    ...
    pdbsearchin = Bioinformatics3.generate_secondary_structure_record(g)
...
def decompose_by_community_clustering(reference, strictness_ah, strictness_bs, peptide_length, pack_beta_sheet,
                                      ...
                                      homogeneity, algorithm="fastgreedy"):
    ...
    dic_json = ss_dict_from_graph(graph_no_coil_reference, reference)
...
def annotate_pdb_model(reference, strictness_ah, strictness_bs, peptide_length, width_pic, height_pic, write_graphml,
    ...
    qc = {}
    for res in Bioinformatics3.get_list_of_residues(structure_reference):
        for atm in res:
    ...
    stren = Bioinformatics3.get_pdb_from_structure(structure_reference, None)
...
def perform_superposition(reference, target, criterium_selection_core="residues", min_rmsd=0.0, max_rmsd=6.0, polyala=False):
    ...
    if isinstance(reference, tuple) and isinstance(target, tuple):
        ...
        if minrmsd < numpy.inf and minrmsd >= min_rmsd and minrmsd <= max_rmsd:
            ...
            if not swap:
                pdbtar, cnv = Bioinformatics3.get_pdb_from_list_of_atoms(Bioinformatics3.get_list_of_atoms(structure_target, sorting=True), renumber=False, uniqueChain=False, applyRt=(minR,mint), polyala=polyala)
            else:
                pdbtar, cnv = Bioinformatics3.get_pdb_from_list_of_atoms(Bioinformatics3.get_list_of_atoms(structure_reference, sorting=True), renumber=False, uniqueChain=False, applyRt=(minR, mint), polyala=polyala)
            ...
            if scd >= 0.9: #<= 40:
                ...
                if remarks_target is not None: pdbtar = remarks_target + "\n" + pdbtar
                return {"suggested": True, "rmsd": minrmsd, "size": mincore,"transf":(minR,mint), "isreversed":swapreverse, "pdb_target": pdbtar, "pdb_core_target": pdbtar, "score_mat_dist":scd, "score_mat_ang":sca}
            else:
...
def __compareSeqReal(seq1, seq2, ssbridge, seq_vaso, structure):
    ...
    for r in range(len(resids)):
        ...
        for r2 in range(len(resids)):
            ...
            if (checkbond >= 2.0 and checkbond <= 2.10):  # S-S bond is 2.05 A
                found = True
        if not found:
            return False
    # if isEqual:
...
def process_structure_fast(pdbsearch, pdbf, pattern, cvs_model, sequence, ncssearch, remove_redundance, identity, ssbridge,
                      peptide_length, connectivity, weight, strictness_ah, strictness_bs, list_chains, processed_cvlist=[]):
    ...
    for i in range(len(fri)):
        ...
        for z in range(Dt.shape[0]-size+1):
            ...
            diff_d = Bioinformatics3.distance_between_matrices([ref_d],[cmp_d], verbose=False)#, stat_test=False)
            diff_t = Bioinformatics3.distance_between_matrices([ref_t],[cmp_t], are_cosins=True, verbose=False)#, stat_test=False)
                ...
                for solu in solutions:
                    ...
                    for n,pos in enumerate(solu):
                        ...
                        if len(set(list(range(g, f))) & set(list(range(s, e)))) > 0:
                            # print("Not acceptable because",set(list(range(g,f)))&set(list(range(s,e))))
                            compatible = False
                            break
                        ...
                        if len(sss) != len(ttt):
                            # print("Have different number of residues",len(sss),len(ttt))
                            compatible = False
                            break
                        ...
                        if ref_frags != tar_frags:
                            # print("Sizes not compatible",ref_frags,tar_frags)
                            compatible = False
                            break
                    ...
                    if compatible:
                        ...
                        fixed_t = Bioinformatics3.distance_between_matrices(ref_tt_frags,cmp_tt_frags, weights_list=ww_frags, are_cosins=True, verbose=False)#, stat_test=True if i==2 and n==1 else False, verbose=True if i==2 and n==1 else False)
                        fixed_d = Bioinformatics3.distance_between_matrices(ref_dd_frags,cmp_dd_frags, weights_list=ww_frags, verbose=False)#, stat_test=True if i==2 and n==1 else False, verbose=True if i==2 and n==1 else False)
                        ...
                        if fixed_d < cutoff_d_jump or fixed_t < cutoff_t_jump:
                            #print("False fixed",fixed_d,cutoff_d_jump,"fixedt",fixed_t,cutoff_t_jump)
                            compatible=False
                ...
                for cc in range(wert[0], wert[1]):
                    for res in cvs_target[cc][4]:
                        if len(list_chains) == len(fri):
                            chains_dict[tr][Bioinformatics3.get_residue(strucc2, res[1], res[2], res[3]).get_unpacked_list()[0].get_full_id()[:-1]] = list_chains[num]
                        else:
                            chains_dict[tr][Bioinformatics3.get_residue(strucc2, res[1], res[2], res[3]).get_unpacked_list()[0].get_full_id()[:-1]] = 'A'
    ...
    for tr,solu in enumerate(solutions):
        if len(solu) == len(fri):
            ...
            if len(sequence) > 0:
                seqr = "".join([Bioinformatics3.AADICMAP[Bioinformatics3.get_residue(strucc2, resi[1], resi[2], resi[3]).get_resname().upper()] for resi in residues])
                # print(sequence)
                # print(seqr)
                # print()
                if not __compareSeqReal(sequence, seqr, ssbridge, residues, strucc2): continue
...
def start_new_process_pdb(pdbsearch, pdbf, idname, DicParameters, doCluster=True, getStructure=False, justBackbone=True, processed_cvlist=[], size_tree=5):
    try:
        nombre = "undefined"
        toremove = ""
        ...
        if DicParameters["exclude_sequence"]:
            structure_read = Bioinformatics3.get_structure("pdb",pdbf)
            seqbychain = {chai.get_id(): "".join(
                [Bioinformatics3.AADICMAP[d.get_parent().get_resname()] for d in Bioinformatics3.get_list_of_atoms(structure_read, type_atom="CA") if
                 d.get_parent().get_resname() in Bioinformatics3.AADICMAP and d.get_parent().get_full_id()[2] == chai.get_id()]) for
            ...
            if len(chain_to_skip) == len(seqbychain.keys()):
                ...
                atoms = [atm for atm in Bioinformatics3.get_list_of_atoms(structure_read) if atm.get_full_id()[2] not in chain_to_skip]
                pdbftext,_ = Bioinformatics3.get_pdb_from_list_of_atoms(atoms)
        ...
        for tuplos in clusts:
            ...
            if justBackbone:
                ppp = Bioinformatics3.get_pdb_from_list_of_atoms(solutions[IdSolution][0], renumber=True, dictio_chains = chains_dict[IdSolution])
                ...
                ppp = Bioinformatics3.get_pdb_from_list_of_atoms(solutions[IdSolution][1], renumber=True, dictio_chains = chains_dict[IdSolution], polyala=False)
...
def create_pdbs(wdir, pdbsearch, pdbfstru, solutions, solred, pdbid, model, DicParameters,
                representative=False, superpose=True, superpose_exclude=1, return_pdbstring=False, nilges=10, ref_pdbid = False):
        ...
        if superpose:
            ...
            if "discarded" in dictio_super and dictio_super["discarded"]:
                ...
                if not dictio_super["isreversed"]:
                    ...
                    pdacc, cnv = Bioinformatics3.get_pdb_from_list_of_atoms(Bioinformatics3.get_list_of_atoms(Bioinformatics3.get_structure("result", io.StringIO(SystemUtility.py2_3_unicode(allpdbtar))), sorting=True), renumber=False, uniqueChain=False, applyRt=dictio_super["transf"])
                    rmsT = dictio_super["rmsd"]

                print("SUPERPOSITION", name, rmsT, "stored in ./library/suggested because score matrix distance is", scd)
                write = False
            ...
            if rmsT >= DicParameters["minRMSD"] and rmsT <= DicParameters["maxRMSD"]:
                if representative and pdbid in dictio_pdb_best and rmsT < dictio_pdb_best[pdbid][1]:
                    ...
                    print("SUPERPOSITION", name, rmsT, dictio_super["size"], title)
                    listrmsd.append((rmsT, name, dictio_super["size"]))
                    write = True
...
def start_new_process(wdir, pdbsearch, pdbstruc, DicParameters, doCluster, backbone, cvs_list_str, superpose, idn,
    ...
    (idname, solList, lisBigSS, solred) = start_new_process_pdb(pdbsearch, pdbstruc, idn, DicParameters, doCluster=doCluster,
                                                                justBackbone=backbone, processed_cvlist=cvs_list_str)
...
def evaluate_pdb(sym, wdir, pdbf, cvs_list_str, pdbstruc, strucc, lisBig, i, pattern, pattern_cvs, highd,
                 ...
                 criterium_selection_core,  exclude_sequence, return_pdbstring=False):
...
def generate_library(local_grid=None,remote_grid=None,supercomputer=None,force_core=-1, directory_database=None,
                     ...
                     clustering_mode='no_clustering', number_of_ranges=500, number_of_clusters=7000, exclude_sequence=None, reverse=False):
    global PATH_NEW_BORGES
    global GRID_TYPE_R
    global MAX_PDB_TAR
    global toclientdir
            ...
            for chain in model.get_list():
                for residue in chain.get_list():
                    if len(listn) == 0 or (chain.get_id(), residue.get_id()) in listn:
                        reference += residue.get_unpacked_list()
            pdbmod, cnv = Bioinformatics3.get_pdb_from_list_of_atoms(reference, renumber=True, uniqueChain=False, remove_non_res_hetatm=True)
    ...
    if targz != None and os.path.exists(targz):
        ...
        for i in range(nPDBs):
            ...
            pdl, dicl = evaluate_pdb(sym, wdir, pdbfile, cvs_list_str, pdbstruc, strucc, lisBig, i, pattern,
                                     ...
                                     exclude_sequence, return_pdbstring=True)
            pdbsol += pdl
            solred.update(dicl)
        infile.close()
        tar.close()
        ...
        for i in range(len(allfiles)):
            ...
            # Multiprocessing
            if supercomputer == None and local_grid == None and remote_grid == None:
                if isinstance(pdbf, str):
