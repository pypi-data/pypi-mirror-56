...
def startARCIMBOLDO_BORGES(BorData, isShredder, input_bor, DicParameters={}, DicGridConn={}, cm=None, sym=None,
    ...
    # SHERLOCK cm CLAUDIA TESTING DATA CORRECT
    ...
    if alixe_mode not in ['one_step', 'two_steps']:
        logging.critical('\n No valid mode for ALIXE. Must be one_step or two_steps')
        sys.exit(0)
    ...
    if not isShredder:
        ...
        if not os.path.exists(mrsumpath):
            ...
            (nqueuetest, convNamestest) = SELSLIB2.startMR_ELLG(DicParameters=DicParameters, cm=cm, sym=sym,
                                                            ...
                                                            highR=res_rot, ellg_target=ellg_target, datacorr=readcorr)
    ...
    for step_i in range(2):
        ...
        for clusi in range(len(orderedClusters)):
            ...
            if swap_model_after_translation!=None:
                ...
                if not os.path.exists(os.path.join(current_directory, "./7_SWAP/" + str(i) + "/")):
                    ...
                    for file_to_swap in list_path_models_to_swap:
                        ...
                        #file_b.close()
                        try:
                            (rmsT,nref,ncom,allAtoms,compStru,pda) = Bioinformatics.getSuperimp(PDBTRANS,PDBTEMP,
                                                                                                "PDBSTRINGBM_RESIDUES_CONSERVED",
                                                                                                algorithm="nigels-core2",backbone=True,superpose_exclude=1,n_iter=None,onlyCA=True)
                        except:
            ...
            if topExp_run is not None:
                ...
                if alixe:
                    print("\nStarting ALIXE phase combination:\n")
                    runstring = ""
                    tuple_phi = ()
                    if isShredder:
                        runstring = "SHREDDER"
                    ...
                    if alixe_mode == 'one_step':
                        ...
                        if tuple_phi[1] != {}:  # If any phi, call SELSLIB2.shelxe_cycle_BORGES
                            ...
                            # TODO: we whould not send more phase clusters than normal solutions to expand, but we could filter this before producing them!
                            # if len(tuple_phi[1].keys())>topExp:
                            #     print("\n SHERLOCK more ALIXE clusters than topExp, do something", topExp)
                            #     quit()
                            failure_all = SELSLIB2.shelxe_cycle_BORGES(lock=lock, DicParameters=DicParameters,
                                                                       ...
                                                                       spaceGroup=spaceGroup, hkl=hkl, ent=ent,
                        ...
                        for clu in evaluated_clusters:
                            if not os.path.exists(os.path.join(current_directory, "./11.5_CLUSTERING/") +
                                ...
                                tuple_phi = ALIXE.startALIXEforARCIMBOLDO(clust_fold=os.path.join(current_directory,
                                                                                                  "./11.5_CLUSTERING/"),
                                                                          cluster_id=clu, reference_hkl=hkl,
                                                                          cell=cell_dim, sg_symbol=spaceGroup,
                                                                          tolerance_list=[60.0, 87.0], resolution=2.0,
                                                                          cycles=3, f_fom=True, run=runstring,
                                                                          mode='one_step', confibor=BorData,
                                                                          input_mode=input_mode)
                            else:
                                print('\n The first step of ALIXE was already completed for rotation cluster ',clu)
                        ...
                        if tuple_phi_second[1] != {} and not os.path.exists(os.path.join(current_directory,'11_EXP_alixe')):
                            ...
                            failure_all = SELSLIB2.shelxe_cycle_BORGES(lock=lock, DicParameters=DicParameters, ClusAll=CluAll, cm=cm,
                                                         ...
                                                         USE_RGR=USE_RGR,isSHREDDER=isShredder, tuple_phi=tuple_phi_second, 
