...
class Client(BaseClient):
        ...
        :returns:
          ...
          **Response Syntax**
          ...
          ::
            ...
            {
                'Availability':
                'AVAILABLE'|'AVAILABLE_RESERVED'|'AVAILABLE_PREORDER'|'UNAVAILABLE'
                |'UNAVAILABLE_PREMIUM'|'UNAVAILABLE_RESTRICTED'|'RESERVED'|'DONT_KNOW'
            }
          **Response Structure**
    ...
    def check_domain_transferability(
        ...
        **Request Syntax**
        ...
        :param AuthCode:
          ...
          If the registrar for the top-level domain (TLD) requires an authorization code to transfer
          the domain, the code that you got from the current registrar for the domain.
    ...
    def disable_domain_transfer_lock(
        ...
        from performing this action unless you intend to transfer the domain to a different
        registrar. Successful submission returns an operation ID that you can use to track the
    ...
    def enable_domain_auto_renew(self, DomainName: str) -> Dict[str, Any]:
        ...
        their renewal policies, see `"Renewal, restoration, and deletion times"
        <http://wiki.gandi.net/en/domains/renew#renewal_restoration_and_deletion_times>`__ on the
        website for our registrar associate, Gandi. Amazon Route 53 requires that you renew before
        the end of the renewal period that is listed on the Gandi website so we can complete
    ...
    def get_domain_detail(self, DomainName: str) -> ClientGetDomainDetailResponseTypeDef:
        ...
        the current AWS account. Contact information for the domain is also returned as part of the
        output.
          ...
          ::

            {
                ...
                'AdminContact': {
                    ...
                    |'AU'|'AW'|'AZ'|'BA'|'BB'|'BD'|'BE'|'BF'|'BG'|'BH'|'BI'|'BJ'|'BL'
                    |'BM'|'BN'|'BO'|'BR'|'BS'|'BT'|'BW'|'BY'|'BZ'|'CA'|'CC'|'CD'|'CF'
                    |'CG'|'CH'|'CI'|'CK'|'CL'|'CM'|'CN'|'CO'|'CR'|'CU'|'CV'|'CX'|'CY'
                    |'CZ'|'DE'|'DJ'|'DK'|'DM'|'DO'|'DZ'|'EC'|'EE'|'EG'|'ER'|'ES'|'ET'
                    |'FI'|'FJ'|'FK'|'FM'|'FO'|'FR'|'GA'|'GB'|'GD'|'GE'|'GH'|'GI'|'GL'
                    |'GM'|'GN'|'GQ'|'GR'|'GT'|'GU'|'GW'|'GY'|'HK'|'HN'|'HR'|'HT'|'HU'
                    |'ID'|'IE'|'IL'|'IM'|'IN'|'IQ'|'IR'|'IS'|'IT'|'JM'|'JO'|'JP'|'KE'
                    |'KG'|'KH'|'KI'|'KM'|'KN'|'KP'|'KR'|'KW'|'KY'|'KZ'|'LA'|'LB'|'LC'
                    |'LI'|'LK'|'LR'|'LS'|'LT'|'LU'|'LV'|'LY'|'MA'|'MC'|'MD'|'ME'|'MF'
                    |'MG'|'MH'|'MK'|'ML'|'MM'|'MN'|'MO'|'MP'|'MR'|'MS'|'MT'|'MU'|'MV'
                    |'MW'|'MX'|'MY'|'MZ'|'NA'|'NC'|'NE'|'NG'|'NI'|'NL'|'NO'|'NP'|'NR'
                    |'NU'|'NZ'|'OM'|'PA'|'PE'|'PF'|'PG'|'PH'|'PK'|'PL'|'PM'|'PN'|'PR'
                    |'PT'|'PW'|'PY'|'QA'|'RO'|'RS'|'RU'|'RW'|'SA'|'SB'|'SC'|'SD'|'SE'
                    |'SG'|'SH'|'SI'|'SK'|'SL'|'SM'|'SN'|'SO'|'SR'|'ST'|'SV'|'SY'|'SZ'
                    |'TC'|'TD'|'TG'|'TH'|'TJ'|'TK'|'TL'|'TM'|'TN'|'TO'|'TR'|'TT'|'TV'
                    |'TW'|'TZ'|'UA'|'UG'|'US'|'UY'|'UZ'|'VA'|'VC'|'VE'|'VG'|'VI'|'VN'
                    ...
                    'ExtraParams': [
                        {
                            ...
                            |'FI_NATIONALITY'|'FI_ORGANIZATION_TYPE'|'IT_PIN'
          ...
          **Response Structure**
          ...
          - *(dict) --*
              ...
              - **ExtraParams** *(list) --*
                ...
                - *(dict) --*
                  ...
                  - **Name** *(string) --*
                    ...
                    top-level domains that require additional parameters and which parameters they
                    require:
            ...
            - **AdminPrivacy** *(boolean) --*
              ...
              ``true`` , WHOIS ("who is") queries return contact information either for Amazon
              Registrar (for .com, .net, and .org domains) or for our registrar associate, Gandi
              (for all other TLDs). If the value is ``false`` , WHOIS queries return the information
            ...
            - **RegistrarName** *(string) --*
              ...
              registered by our registrar associate, Gandi. The value for domains that are
            ...
            - **StatusList** *(list) --*
              ...
              means, go to the `ICANN website <https://www.icann.org/>`__ and search for ``epp
    ...
    def get_domain_suggestions(
        ...
        """
        The GetDomainSuggestions operation returns a list of suggested domain names given a string,
        which can either be a domain name or simply a word or phrase (without spaces).
        ...
        **Request Syntax**
        ...
        :param DomainName: **[REQUIRED]**
          ...
          domain name must contain a top-level domain (TLD), such as .com, that Amazon Route 53
          supports. For a list of TLDs, see `Domains that You Can Register with Amazon Route 53
          <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/registrar-tld-list.html>`__ in
        ...
        :param OnlyAvailable: **[REQUIRED]**
          ...
          without checking whether they're available to be registered. To determine whether the
            ...
            - **SuggestionsList** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Availability** *(string) --*
                  ...
                  The TLD registry didn't reply with a definitive answer about whether the domain
                  name is available. Amazon Route 53 can return this response for a variety of
    ...
    def get_operation_detail(self, OperationId: str) -> ClientGetOperationDetailResponseTypeDef:
        ...
        **Request Syntax**
        ...
        :param OperationId: **[REQUIRED]**
          ...
          The identifier for the operation for which you want to get the status. Amazon Route 53
          ...
          **Response Syntax**
          ...
          ::

            {
                ...
                |'DOMAIN_LOCK'|'ENABLE_AUTORENEW'|'DISABLE_AUTORENEW'|'ADD_DNSSEC'
                |'REMOVE_DNSSEC'|'EXPIRE_DOMAIN'|'TRANSFER_OUT_DOMAIN'|'CHANGE_DOMAIN_OWNER'
                |'RENEW_DOMAIN'|'PUSH_DOMAIN',
    ...
    def list_domains(
        ...
        **Request Syntax**
        ...
        :param Marker:
          ...
          that are associated with the current AWS account is greater than the value that you
          specified for ``MaxItems`` , you can use ``Marker`` to return additional domains. Get the
    ...
    def list_operations(
        ...
        **Request Syntax**
        ...
        :param SubmittedSince:
          ...
          submitted after a specified date and time. Specify the date and time in Coordinated
          Universal time (UTC).
                ...
                'Operations': [
                    {
                        ...
                        |'ENABLE_AUTORENEW'|'DISABLE_AUTORENEW'|'ADD_DNSSEC'
                        |'REMOVE_DNSSEC'|'EXPIRE_DOMAIN'|'TRANSFER_OUT_DOMAIN'
    ...
    def register_domain(
        ...
        * Creates a Amazon Route 53 hosted zone that has the same name as the domain. Amazon Route
        53 assigns four name servers to your hosted zone and automatically updates your domain
        registration with the names of these name servers.

        * Enables autorenew, so your domain registration will renew automatically each year. We'll
        notify you in advance of the renewal date so you can choose whether to renew the
        registration.

        * Optionally enables privacy protection, so WHOIS queries return contact information either
        for Amazon Registrar (for .com, .net, and .org domains) or for our registrar associate,
        Gandi (for all other TLDs). If you don't enable privacy protection, WHOIS queries return the
        ...
        **Request Syntax**
                  ...
                  'ExtraParams': [
                      {
                          ...
                          |'CA_LEGAL_TYPE'|'CA_BUSINESS_ENTITY_TYPE'
                          |'ES_IDENTIFICATION'|'ES_IDENTIFICATION_TYPE'
                          |'ES_LEGAL_FORM'|'FI_BUSINESS_NUMBER'|'FI_ID_NUMBER'
                          |'FI_NATIONALITY'|'FI_ORGANIZATION_TYPE'|'IT_PIN'
                          |'IT_REGISTRANT_ENTITY_TYPE'|'RU_PASSPORT_DATA'
                          |'SE_ID_NUMBER'|'SG_ID_NUMBER'|'VAT_NUMBER'
                          |'UK_CONTACT_TYPE'|'UK_COMPANY_NUMBER',
        ...
        :param DurationInYears: **[REQUIRED]**
          ...
          The number of years that you want to register the domain for. Domains are registered for a
          minimum of one year. The maximum period depends on the top-level domain. For the range of
          valid values for your domain, see `Domains that You Can Register with Amazon Route 53
          <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/registrar-tld-list.html>`__ in
          the *Amazon Route 53 Developer Guide* .
        ...
        :param AdminContact: **[REQUIRED]**
          ...
          - **ContactType** *(string) --*
            ...
            If you choose an option other than ``PERSON`` , you must enter an organization name, and
            you can't enable privacy protection for the contact.
        ...
        :param PrivacyProtectAdminContact:
          ...
          TLDs). If you specify ``false`` , WHOIS queries return the information that you entered
    ...
    def renew_domain(
        ...
        domain is billed to your AWS account.
        ...
        advance. For more information about renewing domain registration, see `Renewing Registration
        for a Domain <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/domain-renew.html>`__
    ...
    def resend_contact_reachability_email(
        ...
        **Request Syntax**
        ...
        :param domainName:
          ...
          The name of the domain for which you want Amazon Route 53 to resend a confirmation email
    ...
    def transfer_domain(
        ...
        """
        This operation transfers a domain from another registrar to Amazon Route 53. When the
        transfer is complete, the domain is registered either with Amazon Registrar (for .com, .net,
        and .org domains) or with our registrar associate, Gandi (for all other TLDs).

        For transfer requirements, a detailed procedure, and information about viewing the status of
        a domain transfer, see `Transferring Registration for a Domain to Amazon Route 53
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/domain-transfer-to-route-53.html>`__
        in the *Amazon Route 53 Developer Guide* .

        If the registrar for your domain is also the DNS service provider for the domain, we highly
        recommend that you consider transferring your DNS service to Amazon Route 53 or to another
        DNS service provider before you transfer your registration. Some registrars provide free DNS
        service when you purchase a domain registration. When you transfer the registration, the
        previous registrar will not renew your domain registration and could end your DNS service at
        any time.
    ...
    def update_domain_contact(
        ...
        If the update is successful, this method returns an operation ID that you can use to track
    ...
    def update_domain_contact_privacy(
        ...
        (registrant, administrator, or tech). If the request succeeds, Amazon Route 53 returns an
        operation ID that you can use with  GetOperationDetail to track the progress and completion
        of the action. If the request doesn't complete successfully, the domain registrant will be
        notified by email.
    ...
    def view_billing(
        ...
        **Request Syntax**
        ...
        :param Marker:
          ...
          billing records that are associated with the current AWS account during the specified
          period is greater than the value that you specified for ``MaxItems`` , you can use
          ``Marker`` to return additional billing records. Get the value of ``NextPageMarker`` from
