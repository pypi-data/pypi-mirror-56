...
class Auto_sinsert():
    def __init__(self,host='',username='',password='',db='',drop_column=["id","update"]):
        red_print("--------------------------------------------------------------------------------------------")
        print('''auto_sinsert = Auto_sinsert(host='192.168.4.201',username='root',password='mysql',db='zhijianju',drop_column=["id","jid","update","entid"])''')
        red_print("--------------------------------------------------------------------------------------------")
        self.host = host
        self.username = username
        self.password = password
...
class Execute_sql():
    def __init__(self,host='',username='',password='',db=''):
        red_print("--------------------------------------------------------------------------------------------")
        print('''execute_sql = Execute_sql(host='192.168.4.201',username='root',password='mysql',db='zhijianju')''')
        red_print("--------------------------------------------------------------------------------------------")
        self.host = host
        self.username = username
        self.password = password
...
class Debug_code():
    def __init__(self,url):
        self.url = url
    def get(self,headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; TencentTraveler 4.0)'}, xpath=''):
    ...
    def post(self,headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; TencentTraveler 4.0)'}, data={}, xpath=''):
...
def getAllName(messageContent):
    ...
    return names
#识别验证码或者电话号码
def image_recognize_url(url,max_lenth=None,max_try=10,headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; TencentTraveler 4.0)'}):
    n = 0
    while n<max_try:
...
def get_photo(url,image_path,headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; TencentTraveler 4.0)'}):
...
def get_pdf(url,image_path,headers={'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; TencentTraveler 4.0)'},data={},proxies={},type_way="get"):
    path = '/'.join(image_path.split('\\')[:-1])
    if not os.path.exists(path):
        os.makedirs(path)
        red_print("创建{}文件夹".format(path))
...
def get_cookies(url,executable_path="E:\chrome download\chromedriver.exe",type="str",proxy=''):
    ops = Options()
    # print('--proxy-server=http://%s' % proxy)
    # ops.add_argument('--user-agent=%s' % ua)
    # print(proxy)
    # ops.add_argument('--proxy-server=http://%s' % "")
    browser = webdriver.Chrome(executable_path=executable_path,chrome_options=ops)
    browser.get(url)
    result_list = browser.get_cookies()
    print(cookie)
    # result_list = eval(cookie)
    if type=="str":
        cookie = ""
...
def get_ip():
    ...
    proxies = {'http':'http://elements:Elements-123.@' + ip + '/'}
    return proxies
