...
class BatchHardMiner(BasePostGradientMiner):
    def __init__(self, use_similarity=False, squared_distances=False, **kwargs):
        ...
        self.max_positive_dist = 0
        self.min_negative_dist = 0
        self.record_these += ["max_positive_dist", "min_negative_dist"]
    ...
    def mine(self, embeddings, labels):
        ...
        self.max_positive_dist = torch.max(hardest_positive_dist).item()
        self.min_negative_dist = torch.min(hardest_negative_dist).item()
