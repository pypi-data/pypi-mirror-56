...
def install():
    ...
    subprocess.check_output("rm -rf ~/.canvas/ExpertModeCTStudio && rm -rf ~/.canvas/Modelhouse && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/dependencies.zip -O && unzip dependencies.zip -d ~/.canvas/ && rm -rf dependencies.zip", shell=True)
    subprocess.check_output("mv ~/.canvas/mysql-connector-java-5.1.38-bin.jar "+app_server_path+"/lib/ && mv ~/.canvas/ojdbc6.jar "+app_server_path+"/lib/", shell=True)
    ...
    if (server_type == 'tomcat'):
        ...
        print 'You have successfully Install Canvas!!! Ready to Rock and Roll'
    else:
        print 'You have successfully Install Canvas!!! Ready to Rock and Roll'
...
def app_deploy(server_type):
    ...
    if (server_type == 'tomcat'):
        ...
        if os.path.exists(os.path.expanduser("~/.canvas")):
            if os.path.exists(os.path.expanduser("~/.canvas/ctmodelhouse.war")) and os.path.exists(os.path.expanduser("~/.canvas/expertctstudio.war")):
                print 'Latest War exist in Canvas home folder'
            else:
                subprocess.check_output("curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
        else:
            subprocess.check_output("mkdir ~/.canvas && mkdir ~/.canvas/template && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
        if os.path.exists(os.path.expanduser(tomcatpath+ "/webapps/expertctstudio.war")):
        ...
        if os.path.exists(os.path.expanduser(tomcatpath+ "/webapps/ctmodelhouse.war")):
            subprocess.check_output(" rm -rf " +tomcatpath+"/webapps/ctmodelhouse.war && rm -rf " +tomcatpath.strip()+"/webapps/ctmodelhouse", shell=True)
        tomcat_status = subprocess.check_output("cp ~/.canvas/expertctstudio.war "+tomcatpath.strip()+"/webapps/", shell=True)
        tomcat_status = subprocess.check_output("cp ~/.canvas/ctmodelhouse.war "+tomcatpath.strip()+"/webapps/", shell=True)
        print 'War Deployed'
        return tomcatpath
    if (server_type == 'jboss'):
        jbosspath=raw_input('Enter Jboss base path(If you dont have JBoss Please install Jboss) : ')
        if not os.path.exists(os.path.expanduser(jbosspath)):
            print "File path not exist && please recheck it once and try again"
            ...
            if not os.path.exists(os.path.expanduser(jbosspath)):
                ...
                exit()
        if not os.path.exists(os.path.expanduser(jbosspath+ "/standalone")):
            print 'unable to find the Standalone folder: '+jbosspath+ '/standalone'
            exit()
        if os.path.exists(os.path.expanduser("~/.canvas")):
            if os.path.exists(os.path.expanduser("~/.canvas/jenkins.war")):
                print 'Latest War Exist in Canvas Home folder!!!'
            else:
                subprocess.check_output("curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
        else:
            subprocess.check_output("mkdir ~/.canvas && mkdir ~/.canvas/template && curl -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/expertctstudio.war -O && curl  -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/ctmodelhouse.war -O && mv expertctstudio.war ~/.canvas/expertctstudio.war &&  mv ctmodelhouse.war ~/.canvas/ctmodelhouse.war && chmod +x ~/.canvas/*", shell=True)
        if os.path.exists(os.path.expanduser(jbosspath+ "/standalone/deployments/expertctstudio.war")):
        ...
        if os.path.exists(os.path.expanduser(jbosspath+ "/standalone/deployments/ctmodelhouse.war")):
        ...
        jboss_status = subprocess.check_output("cp ~/.canvas/expertctstudio.war "+jbosspath.strip()+"/standalone/deployments/", shell=True)
        jboss_status = subprocess.check_output("cp ~/.canvas/ctmodelhouse.war "+jbosspath.strip()+"/standalone/deployments/", shell=True)
        print 'War Deployed'
        return jbosspath
...
def db(server_type,db_type,app_server_path):
    ...
    subprocess.check_output("rm -rf ~/.canvas/tmpl && curl -v -u admin:admin123 http://172.19.29.253:8081/repository/canvas/18/tmpl.zip -O && unzip tmpl.zip -d ~/.canvas/tmpl/", shell=True)
    if not os.path.exists(os.path.expanduser("~/.canvas/template")):
    ...
    subprocess.check_output("perl -p -i -e 's/{ip}/'"+dbip+"'/g' ~/.canvas/template/*", shell=True)
    subprocess.check_output("perl -p -i -e 's/{port}/'"+dbport+"'/g' ~/.canvas/template/*", shell=True)
    subprocess.check_output("perl -p -i -e 's/{user}/'"+dbuser+"'/g' ~/.canvas/template/*", shell=True)
    subprocess.check_output("perl -p -i -e 's/{pass}/'"+dbpass+"'/g' ~/.canvas/template/*", shell=True)
    #subprocess.check_output("cp ~/.canvas/template/", shell=True)
    if (server_type == 'tomcat'):
        if (db_type == 'MySQL'):
            dbflag  = 0
            ...
            if dbflag == 0:
                ...
                f = open(os.path.expanduser('~/.canvas/template/s.txt'), 'r')
                text = f.read().strip()
                f.close()
                with open(app_server_path + '/conf/server.xml', 'r+') as fh:
                    for line in fh:
        ...
        if (db_type == 'OracleSQL'):
            ...
            if dbflag == 0:
                tmp = open("/tmp/temp_server.xml", 'w')
                f = open(os.path.expanduser('~/.canvas/template/os.txt'), 'r')
                text = f.read().strip()
                f.close()
                with open(app_server_path + '/conf/server.xml', 'r+') as fh:
                    for line in fh:
            ...
            if dbflag == 0:
                tmp = open("/tmp/temp_context.xml", 'w')
                f = open(os.path.expanduser('~/.canvas/template/oc.txt'), 'r')
                text = f.read().strip()
                f.close()
                with open(app_server_path + '/conf/context.xml', 'r+') as fh:
                    for line in fh:
    ...
    if (server_type == 'jboss'):
        if (db_type == 'MySQL'):
            ...
            if dbflag == 0:
                ...
                f = open(os.path.expanduser('~/.canvas/template/sa.txt'), 'r')
                text = f.read().strip()
                f.close()
                with open(app_server_path + '/standalone/configuration/standalone.xml', 'r+') as fh:
                    for line in fh:
        ...
        if (db_type == 'OracleSQL'):
            ...
            if dbflag == 0:
                tmp = open("/tmp/temp_standalone.xml", 'w')
                f = open(os.path.expanduser('~/.canvas/template/osa.txt'), 'r')
                text = f.read().strip()
                f.close()
                with open(app_server_path + '/standalone/configuration/standalone.xml', 'r+') as fh:
                    for line in fh:
