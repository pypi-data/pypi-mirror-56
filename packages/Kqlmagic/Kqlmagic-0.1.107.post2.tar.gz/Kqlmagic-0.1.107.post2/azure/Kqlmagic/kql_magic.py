...
logger().debug("kql_magic.py - import display from IPython.core.display")
from IPython.core.display import display
logger().debug("kql_magic.py - import Javascript from IPython.core.magics.display")
...
class Kqlmagic(Magics, Configurable):
    ...
    def __init__(self, shell):
        ...
        logger().debug("Kqlmagic::__init__ - get load mode")
        ...
        if load_mode != "silent" and options.get("show_init_banner"):
            logger().debug("Kqlmagic::__init__ - display banner")
            html_str = (
                """<html>
            <head>
            <style>
    ...
    def execute_query(self, parsed, user_ns: dict, result_set=None):
        ...
        try:
            ...
            if options.get("cache") is not None and options.get("cache") != options.get("use_cache"):
                CacheClient().save(raw_query_result, conn, parametrized_query, **options)
                if options.get("feedback", self.feedback):
