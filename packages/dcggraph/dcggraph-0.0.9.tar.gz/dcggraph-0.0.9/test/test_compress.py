...
class TestCompressNonCyclic(unittest.TestCase):
    ...
    def test_no_predicate(self):
        s = DCGsearch(self.g).compress()

        # no compression
        self.assertEqual(len(s.graph.nodes), 10)
        self.assertEqual(len(s.graph.edges), 8)
    ...
    def test_even_predicate(self):
        ...
        s1 = DCGsearch(self.g, even).compress()

        # did compression
        self.assertEqual(len(s1.graph.nodes), 6)
        self.assertEqual(len(s1.graph.edges), 4)
    ...
    def test_even_memo(self):
        ...
        s1 = DCGsearch(self.g, odd).compress()

        # did compression
        self.assertEqual(len(s1.graph.nodes), 4)
        self.assertEqual(len(s1.graph.edges), 2)
        ...
        really_run = False
        s2 = DCGsearch(self.g, odd).compress()

        # did compression
        self.assertEqual(len(s2.graph.nodes), 0)
        self.assertEqual(len(s2.graph.edges), 0)
...
class TestCyclic(unittest.TestCase):
    ...
    def test_no_predicate(self):
        ...
        # no compression
        self.assertEqual(len(s.graph.nodes), 11)
        self.assertEqual(len(s.graph.edges), 11)
    ...
    def test_even_predicate(self):
        ...
        # did compression
        self.assertEqual(len(s1.graph.nodes), 7)
        self.assertEqual(len(s1.graph.edges), 7)
    ...
    def test_unresolved_even_predicate(self):
        ...
        # did compression
        self.assertEqual(len(s1.graph.nodes), 4)
        self.assertEqual(len(s1.graph.edges), 0)
...
class TestRegressCompressLoop(unittest.TestCase):
    """
    bug fixed: when a subgroup is being compressed out, but it has an edge
    to itself, it caused an infinite loop in compress during edge creation
    """ 
    def test_regress_compress_group_loop(self):
        g = DCGgraph()
        self.g = g
        ...
        def predicate(node, visited, passed, failed):
            if node.name == "n1":
                return PredicateResult.FAIL
            return PredicateResult.PASS

        s1 = DCGsearch(self.g, predicate).compress()

        self.assertEqual(s1.graph.get_node("n0")
                           .edge_out_by_node("n0").to_node, n0)
        self.assertEqual(s1.graph.get_node("n0")
                           .edge_out_by_node("n2").to_node, n2)
        self.assertEqual(s1.graph.get_node("n2")
                           .edge_in_by_node("n0").from_node, n0)
        self.assertEqual(len(s1.graph.nodes), 2)
