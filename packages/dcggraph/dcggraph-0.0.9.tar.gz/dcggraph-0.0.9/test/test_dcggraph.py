...
class TestDCGgraph(unittest.TestCase):
    ...
    def test_basic_graph(self):
        ...
        self.assertEqual(g.has_node("foo"), True)
        self.assertEqual(g.has_node("foo9"), False)
        ...
        with self.assertRaisesRegex(Exception, ".* is not connected from"):
            n0.add_edge_out(e2)
        self.assertEqual(n2.edge_out_by_node("bar2").to_node, n3)
        self.assertEqual(n3.edge_in_by_node("bar").from_node, n2)

        # clone node
        self.assertEqual(len(n0.edges_out), 1)
        self.assertEqual(len(n0.edges_in), 1)
        clone = n0.clone()
        self.assertEqual(clone.name, n0.name)
        self.assertEqual(len(clone.edges_in), 0)
        self.assertEqual(len(clone.edges_out), 0)
