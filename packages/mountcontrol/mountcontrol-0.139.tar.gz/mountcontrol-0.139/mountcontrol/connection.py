...
class Connection(object):
    ...
    COMMANDS = [':AP',
                ...
                ':Ga', ':Gd', ':Gdat', ':Gev', ':Gg', ':Gh', ':GINQ', ':Ginfo', ':Glms',
    ...
    def buildClient(self):
        ...
        """
        if not self.host:
            self.logger.error('no host defined')
            return None
        if not isinstance(self.host, tuple):
            self.logger.error('host entry malformed')
            return None
        ...
        try:
            while True:
                ...
                try:
                    ...
                    self.logger.error(f'{e}, {chunkRaw}')
                    return False, ''
                if not chunk:
                    break
                response += chunk
    ...
    def communicate(self, commandString):
        ...
        if not self.validCommandSet(commandString):
            self.logger.error('unknown commands: {0}'.format(commandString))
            return False, 'wrong commands', 0

        # analysing the command
        numberOfChunks, getData, minBytes = self.analyseCommand(commandString)
        logFormat = 'sending: {0}, getData: {1}, minBytes: {2}, chunks: {3}, host: {4}'
        self.logger.debug(logFormat.format(commandString,
                                           getData,
                                           minBytes,
                                           numberOfChunks,
                                           self.host,
