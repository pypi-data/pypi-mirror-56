...
class ObsSite(object):
    ...
    def location(self, value):
        ...
        if not isinstance(value, (list, tuple)):
            self._location = None
            self.logger.error(f'Malformed value: {value}')
            return
        if len(value) != 3:
            self._location = None
            self.logger.error(f'Malformed value: {value}')
    ...
    def startSlewing(self, slewType='normal'):
        ...
        if response[0].count('#') > 0:
            self.logger.error(f'Slew could not be done, {response}')
            return False
        return True
    ...
    def setTargetAltAz(self,
        ...
        if result.count('0') > 0:
            self.logger.error(f'Coordinates could not be set, {response}')
            return False
        if len(response) != 4:
            self.logger.error(f'Missing return values, {response}')
            return False
