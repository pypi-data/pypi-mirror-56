...
class MgvGraphView(QtWidgets.QGraphicsView):
    ...
    def deleteNodes(self, liste, delfiles):
        ...
        if len(liste):
            ...
            if delfiles:
                a.setText('Remove those nodes and DELETE associated folders ?')
            else:
                a.setText('Remove those nodes and KEEP associated folders ?')
...
class MgvHotBoxBatch(object):
    ...
    def chooseOption(self, x, y):
        ...
        if dist < self.mainMenu.radiusB * self.graphview.globalScale * self.graphview.scale * .12:
            for o in self.mainMenu.options:
                o.unselect()
                o.show()
                o.setScale(1)
...
class MgvTypeEditor(QtWidgets.QMainWindow):
    ...
    def __init__(self, gui, typ):
        ...
        self.root.deleteVersionPushButton.setIcon(QtGui.QIcon(os.path.join(gui.mgvDirectory, 'icons', 'trash-white.svg')))
        self.root.deleteVersionPushButton.setIconSize(QtCore.QSize(16, 16))
        ...
        for i, x in enumerate(vs):
            if x.id == self.type.versionActive.id:
                ct = i
            line = str(x.id)
            if len(x.comment.strip()):
                line += u' - ' + x.comment.strip()
            liste.append(line)

        self.root.versionComboBox.addItems(liste)
        self.root.versionComboBox.setCurrentIndex(vs.index(self.type.versionActive))
        if ct > -1:
            self.root.versionComboBox.setItemData(ct, QtGui.QBrush(QtGui.QColor('#27674a')), QtCore.Qt.BackgroundRole)
        ...
        self.root.versionCommentLineEdit.textChanged.connect(self.versionCommentChanged)
        ...
        self.root.versionCommentLineEdit.setStyleSheet('background-color: #666666;')
    ...
    def versionCommentChanged(self):
        if self.currentVersion:
            self.currentVersion.comment = u'%s' % self.root.versionCommentLineEdit.text()
            i = self.root.versionComboBox.currentIndex()
            txt = u'%s' % self.currentVersion.getId()
            if len(self.currentVersion.comment.strip()):
                txt += u' - ' + self.currentVersion.comment.strip()
            self.root.versionComboBox.setItemText(i, txt)
            if self.type.versionActive is self.currentVersion:
                self.root.versionComboBox.setItemData(i, QtGui.QBrush(QtGui.QColor('#27674a')), QtCore.Qt.BackgroundRole)
...
""".replace('url(:', 'url(' + self.gui.mgvDirectory))
    ...
    def changeVersion(self, num):
        s = self.root.versionComboBox.itemText(num)
        if '-' in s:
            s = s.split('-')[0].strip()
    ...
    def loadVersion(self):
        ...
        self.root.versionCommentLineEdit.setText(self.currentVersion.comment)
        self.currentActionChanged()
    ...
    def delVersion(self):
        if len(self.type.versions) and self.currentVersion is not self.type.versionActive:
            result = QtWidgets.QMessageBox.question(self.root, 'Delete', 'Delete this version ?',
    ...
    def checkCurrentColor(self):
        col = '#27674a' if self.currentVersion is self.type.versionActive else '#555555'
        self.root.versionComboBox.setStyleSheet('QComboBox{background-color: %s}' % col)
        i = self.type.versions.index(self.type.versionActive)
        for j in range(self.root.versionComboBox.count()):
            col = '#27674a' if j == i else '#333333'
            self.root.versionComboBox.setItemData(j, QtGui.QBrush(QtGui.QColor(col)), QtCore.Qt.BackgroundRole)

        if self.currentVersion is not self.type.versionActive:
            self.root.deleteVersionPushButton.setEnabled(True)
            self.root.publishVersionPushButton.setEnabled(True)
            self.root.publishVersionPushButton.setStyleSheet('color: #CCCCCC')
            self.root.deleteVersionPushButton.setIcon(
                QtGui.QIcon(os.path.join(self.gui.mgvDirectory, 'icons', 'trash-white.svg')))
        else:
            self.root.deleteVersionPushButton.setEnabled(False)
            self.root.publishVersionPushButton.setEnabled(False)
            self.root.publishVersionPushButton.setStyleSheet('color: #333333')
            self.root.deleteVersionPushButton.setIcon(
                QtGui.QIcon(os.path.join(self.gui.mgvDirectory, 'icons', 'trash-2.svg')))
...
class MangroveGUI(object):
    ...
    def __init__(self, app):
        ...
        self.toggleIcons()
        self.window.setWindowTitle('Mangrove 1.1')
    ...
    def tabChanged(self, i):
        ...
        if self.currentGraphView:
            self.window.setWindowTitle('%s - Mangrove 1.1' % self.currentGraphView.graph.getName())
            ...
            for node in nodes:
                for x in list(node['versions']):
                    if x['id'] != node['versionActive']:
                        ...
                        x['id'] = self.currentProject.versions_start
                node['oldVersionActive'] = node['versionActive']
                node['versionActive'] = self.currentProject.versions_start
...
KEYx :\t\t\tGraph key number x"""
    ...
    def about(self):
        text = """<p><body>Mangrove v1.1 By Djelloul Bekri</body></p>
...
Delete: remove selected nodes but keep associated folders
Shift + Del: removes selected nodes and associated folders
Movements :
...
""".replace('url(:', 'url(' + self.gui.mgvDirectory))
    ...
    def delVar(self, name):
        ...
        for x in result:
            ...
            for var in vars:
                if var['name'] == name:
                    mgvWrapper.setGraphVar(self.pattern, x, name, delete=True)
        self.selectGraphs()
...
class MgvDownload(object):
    ...
    def selectType(self, i):
        if i > -1:
            ...
            if x['database_id'] == self.gui.database_id.split(':')[-1] or x['author'] == 'djl':
...
class MgvProjectSettings(object):
    ...
    def addPattern(self):
        ...
        self.readProject()
        self.root.patternsTabWidget.setCurrentIndex(self.root.patternsTabWidget.count()-1)
    ...
    def patternNameChanged(self, pattern, widget, index):
        ...
        for w in range(self.root.patternsTabWidget.count()):
            oldname = self.root.patternsTabWidget.tabText(w)
            if oldname == pattern.name:
                self.root.patternsTabWidget.setTabText(w, value)
                break
