...
class MgvGraph(object):
    ...
    def _fillWithGraphTemplate(self, template):
        ...
        for node in self.nodes:
            node.create()
        for node in self.nodes:
            node.inputLinks = [[y.uuid for y in self.nodes if y.name == x][0] for x in node.inputLinks]
        for node in self.nodes:
            node.outputLinks = [[y.uuid for y in self.nodes if y.name == x][0] for x in node.outputLinks]
...
class MgvNodeVersion(object):
    ...
    def setParameter(self, name, value):
        ...
        self.node._ungotvar()
        original = self.node.type.getParameter(name, version_id=self.node.versionActive.typeForceVersion)
...
class MgvTypeVersion(object):
    ...
    def __init__(self, uuid=None, type=None, version_id=0, parameters=None, actions=None, script='', comment=''):
        self.uuid = uuid
        self.type = type
        self.id = version_id
        self.comment = comment
    ...
    def create(self):
        ...
        self.uuid = mgvWrapper.createNode(self.type, self.code, id=self.id, script=self.script, comment=self.comment)
    ...
    def _copy(self, typeVersion):
        ...
        self.comment = typeVersion.comment
