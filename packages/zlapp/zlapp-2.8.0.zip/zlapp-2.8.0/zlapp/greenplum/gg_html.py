...
def gg_html_cdc(max_html_key,conp_src,conp_dst):
    ...
    FROM pg_partitions
    WHERE tablename='t_gg' and schemaname='src' order by  partitionname asc
    """
    ...
    while end<len(quyus):
        ...
        sql="select html_key,page as page from src.t_gg where html_key>%d and quyu in(%s) "%(max_html_key,st)
        print(sql)
        ...
        sql="insert into public.gg_html select distinct on(html_key) * from cdc.gg_html_cdc as a  where not exists(select 1 from  public.gg_html  as b where a.html_key=b.html_key)"
        db_command(sql,dbtype="postgresql",conp=conp_dst)
...
def update_gg_html(conp_src,conp_dst,max_html_key=None):
    ...
    if 'gg_html' not in df['tablename'].values:
        ...
        #max_html_key=53000000
        gg_html_cdc(max_html_key,conp_src,conp_dst)

# conp_src=['gpadmin','since2015','192.168.4.183:5433','base_db','public']
# conp_dst=['postgres','since2015','192.168.4.207','biaost_pre','public']
# gg_html_cdc(55000000,conp_src,conp_dst)
