...
def est_gg_html_algo_quyu(quyu,conp_gp):
  
    sql="""
    CREATE  unlogged  TABLE if not exists etl.gg_html_algo_%s (
    html_key bigint,
    page text
    )
...
def pre_quyu_cdc(quyu,conp_gp):
    ...
    est_gg_html_algo_quyu(quyu,conp_gp)
    sql="truncate table etl.gg_html_algo_%s;"%quyu
    print(sql)
    db_command(sql,dbtype="postgresql",conp=conp_gp)

    sql1="insert into etl.gg_html_algo_%s select html_key,page.tran_page(page) as page from src.t_gg where quyu='%s' and html_key>%d "%(quyu,quyu,max_html_key)
    print(sql1)
    db_command(sql1,dbtype="postgresql",conp=conp_gp)

    df=db_query("select max(html_key),count(*) from etl.gg_html_algo_%s "%quyu,dbtype="postgresql",conp=conp_gp)
    cnt=df.iat[0,1]
...
def insert_into(quyu,conp_gp,conp_app1):
    ...
    select * from dblink('hostaddr=192.168.4.183 port=5433 dbname=base_db user=gpadmin password=since2015',
            $lmf$
            select html_key,page from etl.gg_html_algo_anhui_anqing_ggzy 
          $lmf$) AS testTable ( html_key bigint,page text))
    insert into public.gg_html_algo 

    select * from a   where not exists(select html_key from  gg_html_algo as b where a.html_key=b.html_key)
    """
