...
def test_upsert_template_update():
    ...
    with mock_server(content='{ "version_id": 2 }'):
        result = CliRunner(mix_stderr=False).invoke(upsert_module_workingcopy_template, args="--module-name test --module-version 0.0.1-SNAPSHOT data/create-module-template.json")
        assert result.exit_code == 0, result.output or result.exception
