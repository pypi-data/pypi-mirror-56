...
def validator(string=None, *, accept=None, output=None):
    ...
    def decorator(f):
        ...
        def _m_validator(compiler, schema):
            ...
            local_output_object = output_object
            ...
            invalid_to_default = params.pop('invalid_to_default', False)
            has_invalid_to = 'invalid_to' in params
...
_UNIQUE_CHECK_ERROR_MESSAGE = "unable to check unique for non-hashable types"


def _key_of_scalar(v):
    return v
...
def list_validator(compiler, items=None, minlen=0, maxlen=1024,
                   unique=False, optional=False):
    if items is None:
        inner = None
    else:
        with mark_index():
...
@validator(accept=object, output=object)
def dict_validator(compiler, items=None, optional=False):
    if items is None:
        inners = None
    else:
        inners = []
...
def str_validator(compiler, minlen=0, maxlen=1024 * 1024,
                  strip=False, escape=False, accept_object=False):
    """Validate string
...
def date_validator(compiler, format='%Y-%m-%d', output_object=False):
...
@validator(output=(str, object))
def time_validator(compiler, format='%H:%M:%S', output_object=False):
...
@validator(output=(str, object))
def ipv4_validator(compiler, output_object=False):
    def validate(value):
        try:
...
@validator(output=(str, object))
def ipv6_validator(compiler, output_object=False):
    def validate(value):
        try:
...
@validator(output=(str, object))
def url_validator(compiler, scheme='http https', maxlen=256, output_object=False):
...
@validator(output=(str, object))
def uuid_validator(compiler, version=None, output_object=False):
    if version is None:
        msg = 'invalid uuid'
    else:
        if not 1 <= version <= 5:
...
def create_re_validator(name, r):
...
def create_enum_validator(name, items, string=True):
