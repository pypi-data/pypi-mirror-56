# coding: utf-8
"""
Copyright: 2015-2019 Saito Tsutomu
...
def dual_model(m, ignoreUpBound=False):
    ...
    var_count = [0]
    vvs = [addvars(len(cc),var_count=var_count, lowBound=0 if i!=1 else None)
           for i,cc in enumerate(ccs)]
    nwmdl += coe*lpSum(k*lpDot([c.constant for c in cc],vv)
                       for k,cc,vv in zip([1,-1,-1],ccs,vvs))
    for w in m.variables():
        nwmdl += lpSum(k*c.get(w)*v for k,cc,vv in zip([-1,1,1],ccs,vvs)
                   for c,v in zip(cc,vv) if c.get(w)) == coe*m.objective.get(w,0)
    return nwmdl
...
def graph_from_table(dfnd, dfed, directed=False, multi=False, node_label='id',
...
def networkx_draw(g, dcpos=None, node_label='id', x_label='x', y_label='y', **kwargs):
    ...
    if not dcpos:
        dcpos = {r[node_label]:(r[x_label], r[y_label]) for i, r in g.node.items()}
...
def ortools_vrp(nn, dist, nv=1, capa=1000, demands=None, depo=0):
    ...
    if demands is not None:
        routing.AddDimension(get_callback_index1(manager, routing, demands),
                             0, capa, True, 'Capacity')
...
def tsp1(nodes, dist):
    ...
    n = len(nodes)
    df = pd.DataFrame([(i, j, dist[i, j]) for i in range(n) for j in range(n) if i != j],
...
def chinese_postman(g, weight='weight'):
    ...
    return (sum(d[weight] for (i,j,_),d in g.edges.items()),
...
class MultiKeyDict:
    ...
    """
    ekey = tuple() # 空キー
    def __init__(self, dc={}, conv=None, iskey=None,
            dtype=dict, extend=False, cache=2048):
        """
