...
def cache(func, force_hot: bool = False, history_limit: int = 5, disable_cache: bool = False) -> callable:
    ...
    def memo_wrapper(*args, force_hot_, history_limit_, disable_cache_=False, **kwargs, ) -> functools.partial:
        ...
        if func_name not in shelf["__meta_history"]:
            shelf["__meta_history"][func_name] = {}
        ...
        try:
            ...
            hot = force_hot_ or call_hash not in shelf["__meta_history"][func_name]
        ...
        if hot:
            ...
            shelf = shelve.open(__CACHE_NAME__)
            if call_hash is not None and call_hash not in shelf["__meta_history"][func_name]:
                shelf["__meta_history"][func_name].append(call_hash)
                # Forget old calls
                if len(shelf["__meta_history"][func_name]) > history_limit:
                    for call_to_forget in shelf["__meta_history"][func_name][:-history_limit]:
