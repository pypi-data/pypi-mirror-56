...
try:
    ...
    print(err)
    from IHEWAwapor.WPdownload import download as WaPOR

try:
    from .download import GIS_functions as gis
except ImportError as err:
    print(err)
    from IHEWAwapor.WPdownload.download import GIS_functions as gis
...
def main(APIToken='',
    ...
    for index, row in df_avail.iterrows():
        ...
        filename = 'RET_WAPOR.v%s_l%s-annually-1_%s.tif' % (
            version, level,
