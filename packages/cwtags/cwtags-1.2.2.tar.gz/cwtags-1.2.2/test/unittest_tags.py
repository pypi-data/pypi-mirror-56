...
class TestTagsTC(unittest.TestCase):
    ...
    def test_all_tags(self):
        ...
        for tname in sorted(tagnames):
            ...
            if tname in tagbuilder.HTML_EMPTY_TAGS:
                self.assertIn(tname, text_type(t(id='babar')))
            else:
                self.assertIn(tname, text_type(t()))

    def test_simple_tags(self):
        self.assertEqual(text_type(tag.div(u'toto', Class="css")),
                         u'<div class="css">toto</div>')
        self.assertEqual(text_type(tag.div(u'toto',
    ...
    def test_tag_names(self):
        ...
        self.assertEqual(text_type(tag.div(Class='css')), expected)
        self.assertEqual(text_type(tag.div(class_='css')), expected)
        self.assertEqual(text_type(tag.div(_class='css')), expected)
        self.assertEqual(text_type(tag.div(klass='css')), expected)

    def test_flag_tags(self):
        self.assertEqual(
            text_type(tag.option(u'test', value='5', selected=True)),
            u'<option value="5" selected>test</option>')
        self.assertEqual(
            text_type(tag.option(u'test', value='5', selected=False)),
            u'<option value="5">test</option>')

    def test_none_tags(self):
        self.assertEqual(text_type(tag.div(title=None)),
                         u'<div ></div>')
    ...
    def test_list_tags(self):
        ...
        self.assertIn(text_type(tag.div(class_=classes)), expected)
        self.assertIn(text_type(tag.div(class_=tuple(classes))), expected)

        self.assertIn(text_type(tag.div(class_=set(classes))), expected)
        self.assertIn(text_type(tag.div(class_=frozenset(classes))), expected)

    def test_dict_tags(self):
        classes = {'css1': True, 'css2': False}
        expected = u'<div class="css1"></div>'
        self.assertEqual(text_type(tag.div(class_=classes)), expected)
