...
class BlueGreen:
    ...
    def set_traffic_weight(self, stack_name: str, weight: float):
        ...
        cur_weight_remain = sum(to_update[name] for name in to_update if name != stack_name)
        for name in to_update:
            if name != stack_name and to_update[name] != 0.0:
                new_weights[name] = round(to_update[name] * new_weight_remain / cur_weight_remain)
