"Main interface for route53 Client"
...
# pylint: disable=import-self
import mypy_boto3_route53.waiter as waiter_scope
...
class Client(BaseClient):
    ...
    def associate_vpc_with_hosted_zone(
        ...
        .. note::
          ...
          If you want to associate a VPC that was created by using one AWS account with a private hosted
          zone that was created by using a different account, the AWS account that created the private
          hosted zone must first submit a ``CreateVPCAssociationAuthorization`` request. Then the account
          that created the VPC must submit an ``AssociateVPCWithHostedZone`` request.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/AssociateVPCWithHostedZone>`_
        ...
        ::
          ...
          response = client.associate_vpc_with_hosted_zone(
              ...
              VPC={
                  ...
                  |'eu-central-1'|'ap-east-1'|'me-south-1'|'ap-southeast-1'|'ap-southeast-2'|'ap-south-1'
                  |'ap-northeast-1'|'ap-northeast-2'|'ap-northeast-3'|'eu-north-1'|'sa-east-1'
    ...
    def change_resource_record_sets(
        self,
        HostedZoneId: str,
        ChangeBatch: ClientChangeResourceRecordSetsChangeBatchTypeDef,
    ) -> ClientChangeResourceRecordSetsResponseTypeDef:
        """
        ...
        The request body must include a document with a ``ChangeResourceRecordSetsRequest`` element. The
        request body contains a list of change items, known as a change batch. Change batches are considered
        transactional changes. When using the Amazon Route 53 API to change resource record sets, Route 53
        either makes all or none of the changes in a change batch request. This ensures that Route 53 never
        partially implements the intended changes to the resource record sets in a hosted zone.

        For example, a change batch request that deletes the ``CNAME`` record for www.example.com and
        creates an alias resource record set for www.example.com. Route 53 deletes the first resource record
        set and creates the second resource record set in a single operation. If either the ``DELETE`` or
        the ``CREATE`` action fails, then both changes (plus any other changes in the batch) fail, and the
        ...
        .. warning::
          ...
          than once in a single change batch. If you attempt to delete the same change batch more than once,
          Route 53 returns an ``InvalidChangeBatch`` error.

         **Traffic Flow**

        To create resource record sets for complex routing configurations, use either the traffic flow
        visual editor in the Route 53 console or the API actions for traffic policies and traffic policy
        instances. Save the configuration as a traffic policy, then associate the traffic policy with one or
        more domain names (such as example.com) or subdomain names (such as www.example.com), in the same
        hosted zone or in multiple hosted zones. You can roll back the updates if the new configuration
        isn't performing as expected. For more information, see `Using Traffic Flow to Route DNS Traffic
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/traffic-flow.html>`__ in the *Amazon Route
        53 Developer Guide* .
        ...
        * ``UPSERT`` : If a resource record set does not already exist, AWS creates it. If a resource set
        does exist, Route 53 updates it with the values in the request.

         **Syntaxes for Creating, Updating, and Deleting Resource Record Sets**

        The syntax for a request depends on the type of resource record set that you want to create, delete,
        or update, such as weighted, alias, or failover. The XML elements in your request must appear in the
        order listed in the syntax.

        For an example for each type of resource record set, see "Examples."

        Don't refer to the syntax in the "Parameter Syntax" section, which includes all of the elements for
        every kind of resource record set that you can create, delete, or update by using
        ``ChangeResourceRecordSets`` .

         **Change Propagation to Route 53 DNS Servers**

        When you submit a ``ChangeResourceRecordSets`` request, Route 53 propagates your changes to all of
        the Route 53 authoritative DNS servers. While your changes are propagating, ``GetChange`` returns a
        status of ``PENDING`` . When propagation is complete, ``GetChange`` returns a status of ``INSYNC`` .
        Changes generally propagate to all Route 53 name servers within 60 seconds. For more information,
        see `GetChange <https://docs.aws.amazon.com/Route53/latest/APIReference/API_GetChange.html>`__ .

         **Limits on ChangeResourceRecordSets Requests**

        For information about the limits on a ``ChangeResourceRecordSets`` request, see `Limits
        <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/DNSLimitations.html>`__ in the *Amazon
        Route 53 Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ChangeResourceRecordSets>`_
                  ...
                  'Changes': [
                      {
                          ...
                          'ResourceRecordSet': {
                              ...
                              |'eu-west-2'|'eu-west-3'|'eu-central-1'|'ap-southeast-1'|'ap-southeast-2'
                              |'ap-northeast-1'|'ap-northeast-2'|'ap-northeast-3'|'eu-north-1'|'sa-east-1'
                              |'cn-north-1'|'cn-northwest-1'|'ap-east-1'|'me-south-1'|'ap-south-1',
                              'GeoLocation': {
            ...
            - *(dict) --*
              ...
              - **ResourceRecordSet** *(dict) --* **[REQUIRED]**
                ...
                - **Name** *(string) --* **[REQUIRED]**
                  ...
                  include a trailing dot. If you omit the trailing dot, Amazon Route 53 assumes that the
                ...
                - **Type** *(string) --* **[REQUIRED]**
                  ...
                  .. note::
                    ...
                    However, we no longer recommend that you create resource record sets for which the value
                    of ``Type`` is ``SPF`` . RFC 7208, *Sender Policy Framework (SPF) for Authorizing Use of
                    Domains in Email, Version 1* , has been updated to say, "...[I]ts existence and
                ...
                - **SetIdentifier** *(string) --*
                   ...
                   *Resource record sets that have a routing policy other than simple:* An identifier that
                   differentiates among multiple resource record sets that have the same combination of name
                   and type, such as multiple weighted resource record sets named acme.example.com that have
                   a type of A. In a group of resource record sets that have the same name and type, the
                ...
                - **Weight** *(integer) --*
                   ...
                   *Weighted resource record sets only:* Among resource record sets that have the same
                   ...
                   combination of DNS name and type. Route 53 then responds to queries based on the ratio of
                   a resource's weight to the total. Note the following:
                  ...
                  * For weighted (but not weighted alias) resource record sets, if you set ``Weight`` to
                  ...
                  is different when you associate health checks with weighted resource record sets. For more
                ...
                - **GeoLocation** *(dict) --*
                  ...
                  continent), priority goes to the smallest geographic region. This allows you to route most
                  queries for a continent to one resource and to route queries for a country on that
                  ...
                  .. warning::
                    ...
                    aren't mapped to geographic locations, so even if you create geolocation resource record
                    sets that cover all seven continents, Route 53 will receive some DNS queries from
                    locations that it can't identify. We recommend that you create a resource record set for
                    which the value of ``CountryCode`` is ``*`` , which handles both queries that come from
                    locations for which you haven't created geolocation resource record sets and queries
                    from IP addresses that aren't mapped to a location. If you don't create a ``*`` resource
                ...
                - **Failover** *(string) --*
                   ...
                   *Failover resource record sets only:* To configure failover, you add the ``Failover``
                   ...
                   as the value for ``Failover`` ; for the other resource record set, you specify
                   ``SECONDARY`` . In addition, you include the ``HealthCheckId`` element and specify the
                  ...
                  * When the secondary resource record set is unhealthy, Route 53 responds to DNS queries
                  with the applicable value from the primary resource record set regardless of the health of
                  the primary resource record set.

                  * If you omit the ``HealthCheckId`` element for the secondary resource record set, and if
                  the primary resource record set is unhealthy, Route 53 always responds to DNS queries with
                  the applicable value from the secondary resource record set. This is true regardless of
                ...
                - **MultiValueAnswer** *(boolean) --*
                   ...
                   *Multivalue answer resource record sets only* : To route traffic approximately randomly
                   to multiple resources, such as web servers, create one multivalue answer record for each
                  ...
                  * If you have more than eight healthy records, Route 53 responds to different DNS
                  resolvers with different combinations of healthy records.
                ...
                - **TTL** *(integer) --*
                  ...
                  * If you're creating or updating an alias resource record set, omit ``TTL`` . Amazon Route
                  ...
                  * If you're associating this resource record set with a health check (if you're adding a
                  ``HealthCheckId`` element), we recommend that you specify a ``TTL`` of 60 seconds or less
                  ...
                  * All of the resource record sets in a group of weighted resource record sets must have
                  the same value for ``TTL`` .

                  * If a group of weighted resource record sets includes one or more weighted alias resource
                  record sets for which the alias target is an ELB load balancer, we recommend that you
                  specify a ``TTL`` of 60 seconds for all of the non-alias weighted resource record sets
                ...
                - **ResourceRecords** *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    - **Value** *(string) --* **[REQUIRED]**
                      ...
                      returned. For descriptions about how to format ``Value`` for different record types,
                      see `Supported DNS Resource Record Types
                      <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/ResourceRecordTypes.html>`__
                      in the *Amazon Route 53 Developer Guide* .
                ...
                - **AliasTarget** *(dict) --*
                  ...
                  * Creating geolocation alias resource record sets or latency alias resource record sets in
                  ...
                  * For information about creating failover resource record sets in a private hosted zone,
                  see `Configuring Failover in a Private Hosted Zone
                  <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-private-hosted-zones.html>`__
                  in the *Amazon Route 53 Developer Guide* .
                  ...
                  - **HostedZoneId** *(string) --* **[REQUIRED]**
                    ...
                    Specify the hosted zone ID for your API. You can get the applicable value using the AWS
                    CLI command `get-domain-names
                    <https://docs.aws.amazon.com/cli/latest/reference/apigateway/get-domain-names.html>`__ :
                    ...
                    * For edge-optimized APIs, specify the value of ``distributionHostedZoneId`` .
                    ...
                    ``HostedZoneId`` using the AWS CLI command `describe-vpc-endpoints
                    ...
                    environment must have a regionalized subdomain. For a list of regions and the
                    corresponding hosted zone IDs, see `AWS Elastic Beanstalk
                    <http://docs.aws.amazon.com/general/latest/gr/rande.html#elasticbeanstalk_region>`__ in
                    the "AWS Regions and Endpoints" chapter of the *Amazon Web Services General Reference* .
                    ...
                    * `Elastic Load Balancing
                    <https://docs.aws.amazon.com/general/latest/gr/rande.html#elb_region>`__ table in the
                    "AWS Regions and Endpoints" chapter of the *Amazon Web Services General Reference* : Use
                    the value that corresponds with the region that you created your load balancer in. Note
                    that there are separate columns for Application and Classic Load Balancers and for
                    Network Load Balancers.

                    * **AWS Management Console** : Go to the Amazon EC2 page, choose **Load Balancers** in
                    the navigation pane, select the load balancer, and get the value of the **Hosted zone**
                    ...
                    * **Elastic Load Balancing API** : Use ``DescribeLoadBalancers`` to get the applicable
                    value. For more information, see the applicable guide:
                      ...
                      * Classic Load Balancers: Use `DescribeLoadBalancers
                      <http://docs.aws.amazon.com/elasticloadbalancing/2012-06-01/APIReference/API_DescribeLoadBalancers.html>`__
                      to get the value of ``CanonicalHostedZoneNameId`` .

                      * Application and Network Load Balancers: Use `DescribeLoadBalancers
                      <http://docs.aws.amazon.com/elasticloadbalancing/latest/APIReference/API_DescribeLoadBalancers.html>`__
                    ...
                    * **AWS CLI** : Use ``describe-load-balancers`` to get the applicable value. For more
                    information, see the applicable guide:
                      ...
                      * Classic Load Balancers: Use `describe-load-balancers
                      <http://docs.aws.amazon.com/cli/latest/reference/elb/describe-load-balancers.html>`__
                      to get the value of ``CanonicalHostedZoneNameId`` .

                      * Application and Network Load Balancers: Use `describe-load-balancers
                      <http://docs.aws.amazon.com/cli/latest/reference/elbv2/describe-load-balancers.html>`__
                      to get the value of ``CanonicalHostedZoneId`` .
                    ...
                    <http://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region>`__ table in the "AWS
                  ...
                  - **DNSName** *(string) --* **[REQUIRED]**
                     ...
                     *Alias resource record sets only:* The value that you specify depends on where you want
                     to route queries:

                      Amazon API Gateway custom regional APIs and edge-optimized APIs

                    Specify the applicable domain name for your API. You can get the applicable value using
                    the AWS CLI command `get-domain-names
                    <https://docs.aws.amazon.com/cli/latest/reference/apigateway/get-domain-names.html>`__ :

                    * For regional APIs, specify the value of ``regionalDomainName`` .

                    * For edge-optimized APIs, specify the value of ``distributionDomainName`` . This is the
                    name of the associated CloudFront distribution, such as ``da1b2c3d4e5.cloudfront.net`` .
                    ...
                    Your CloudFront distribution must include an alternate domain name that matches the name
                    of the resource record set. For example, if the name of the resource record set is
                    *acme.example.com* , your CloudFront distribution must include *acme.example.com* as one
                    of the alternate domain names. For more information, see `Using Alternate Domain Names
                    (CNAMEs)
                    <http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/CNAMEs.html>`__ in
                    the *Amazon CloudFront Developer Guide* .

                    You can't create a resource record set in a private hosted zone to route traffic to a
                    CloudFront distribution.
                    ...
                    If the domain name for your Elastic Beanstalk environment includes the region that you
                    deployed the environment in, you can create an alias record that routes traffic to the
                    environment. For example, the domain name ``my-environment.*us-west-2*
                    .elasticbeanstalk.com`` is a regionalized domain name.
                    ...
                    .. warning::
                      ...
                      you can't create a record that routes traffic for example.com to your Elastic
                      Beanstalk environment.

                    For Elastic Beanstalk environments that have regionalized subdomains, specify the
                    ``CNAME`` attribute for the environment. You can use the following methods to get the
                    ...
                    * *AWS Management Console* : For information about how to get the value by using the
                    ...
                    <http://docs.aws.amazon.com/elasticbeanstalk/latest/dg/customdomains.html>`__ in the
                    *AWS Elastic Beanstalk Developer Guide* .

                    * *Elastic Beanstalk API* : Use the ``DescribeEnvironments`` action to get the value of
                    the ``CNAME`` attribute. For more information, see `DescribeEnvironments
                    <http://docs.aws.amazon.com/elasticbeanstalk/latest/api/API_DescribeEnvironments.html>`__
                    ...
                    * *AWS CLI* : Use the ``describe-environments`` command to get the value of the
                    ...
                    <http://docs.aws.amazon.com/cli/latest/reference/elasticbeanstalk/describe-environments.html>`__
                    ...
                    * **AWS Management Console** : Go to the EC2 page, choose **Load Balancers** in the
                    navigation pane, choose the load balancer, choose the **Description** tab, and get the
                    value of the **DNS name** field.  If you're routing traffic to a Classic Load Balancer,
                    get the value that begins with **dualstack** . If you're routing traffic to another type
                    ...
                    * **AWS CLI** : Use ``describe-load-balancers`` to get the value of ``DNSName`` . For
                    ...
                    <http://docs.aws.amazon.com/general/latest/gr/rande.html#s3_region>`__ in the *Amazon
                    Web Services General Reference* . For more information about using S3 buckets for
                    websites, see `Getting Started with Amazon Route 53
                    <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/getting-started.html>`__ in
                    the *Amazon Route 53 Developer Guide.*
                    ...
                    .. note::
                      ...
                      If you're creating an alias record that has the same name as the hosted zone (known as
                      the zone apex), you can't specify the domain name for a record for which the value of
                      ``Type`` is ``CNAME`` . This is because the alias record must have the same type as
                      the record that you're routing traffic to, and creating a CNAME record for the zone
                  ...
                  - **EvaluateTargetHealth** *(boolean) --* **[REQUIRED]**
                     ...
                     *Applies only to alias, failover alias, geolocation alias, latency alias, and weighted
                     alias resource record sets:* When ``EvaluateTargetHealth`` is ``true`` , an alias
                     resource record set inherits the health of the referenced AWS resource, such as an ELB
                    ...
                    automatically contains an ELB load balancer if it includes more than one Amazon EC2
                    instance.) If you set ``EvaluateTargetHealth`` to ``true`` and either no Amazon EC2
                    ...
                    * **Application and Network Load Balancers** : If you specify an ELB Application or
                    ...
                    associated with the load balancer:
                      ...
                      * For an Application or Network Load Balancer to be considered healthy, every target
                      ...
                      group contains only unhealthy targets, the load balancer is considered unhealthy, and
                    ...
                    .. note::
                      ...
                      health checks; they're not Route 53 health checks, but they perform a similar
                      function. Do not create Route 53 health checks for the EC2 instances that you register
                      with an ELB load balancer.

                      S3 buckets

                    There are no special requirements for setting ``EvaluateTargetHealth`` to ``true`` when
                    the alias target is an S3 bucket.

                      Other records in the same hosted zone

                    If the AWS resource that you specify in ``DNSName`` is a record or a group of records
                    (for example, a group of weighted records) but is not another alias record, we recommend
                    that you associate a health check with all of the records in the alias target. For more
                    information, see `What Happens When You Omit Health Checks?
                    <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-complex-configs.html#dns-failover-complex-configs-hc-omitting>`__
                    in the *Amazon Route 53 Developer Guide* .

                    For more information and examples, see `Amazon Route 53 Health Checks and DNS Failover
                    <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover.html>`__ in the
                    *Amazon Route 53 Developer Guide* .

                - **HealthCheckId** *(string) --*

                  If you want Amazon Route 53 to return this resource record set in response to a DNS query
                  only when the status of a health check is healthy, include the ``HealthCheckId`` element
                  ...
                  * By determining the current state of a CloudWatch alarm (CloudWatch metric health checks)
                  ...
                  .. warning::
                    ...
                    record set, for example, the endpoint specified by the IP address in the ``Value``
                    element. When you add a ``HealthCheckId`` element to a resource record set, Route 53
                    checks the health of the endpoint that you specified in the health check.
                  ...
                  * `Configuring Failover in a Private Hosted Zone
                  ...
                  Specifying a value for ``HealthCheckId`` is useful only when Route 53 is choosing between
                  two or more resource record sets to respond to a DNS query, and you want Route 53 to base
                  the choice in part on the status of a health check. Configuring health checks makes sense
                  ...
                  * **Non-alias resource record sets** : You're checking the health of a group of non-alias
                  ...
                  weighted records named www.example.com with a type of A) and you specify health check IDs
                  for all the resource record sets.  If the health check status for a resource record set is
                  healthy, Route 53 includes the record among the records that it responds to DNS queries
                  with. If the health check status for a resource record set is unhealthy, Route 53 stops
                  responding to DNS queries using the value for that resource record set. If the health
                  check status for all resource record sets in the group is unhealthy, Route 53 considers
                  all resource record sets in the group healthy and responds to DNS queries accordingly.

                  * **Alias resource record sets** : You specify the following settings:

                    * You set ``EvaluateTargetHealth`` to true for an alias resource record set in a group
                    of resource record sets that have the same routing policy, name, and type (such as
                    multiple weighted records named www.example.com with a type of A).
                  ...
                  If the health check status is unhealthy, Route 53 stops responding to DNS queries using
                  ...
                  have resource record sets for a state in the United States, for the entire United States,
                  for North America, and a resource record set that has ``*`` for ``CountryCode`` is ``*`` ,
                  which applies to all locations. If the endpoint for the state resource record set is
                  unhealthy, Route 53 checks for healthy resource record sets in the following order until
                  ...
                  * The default resource record set
                  ...
                  .. warning::
                    ...
                    * Create a health check that has the same value for ``FullyQualifiedDomainName`` as the
                    name of a resource record set.
                ...
                - **TrafficPolicyInstanceId** *(string) --*
                  ...
                  resource record set. ``TrafficPolicyInstanceId`` is the ID of the traffic policy instance
                  ...
                  .. warning::
                    ...
                    ``ChangeResourceRecordSets`` , Route 53 doesn't automatically delete the traffic policy
    ...
    def change_tags_for_resource(
        ...
        <https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/cost-alloc-tags.html>`__ in the *AWS
        Billing and Cost Management User Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ChangeTagsForResource>`_
        ...
        **Request Syntax**
        ...
        :param AddTags:
          ...
          or hosted zone and/or the tags that you want to edit ``Value`` for.
    ...
    def create_health_check(
        ...
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_ResourceRecordSet.html#Route53-Type-ResourceRecordSet-HealthCheckId>`__
        in `ChangeResourceRecordSets
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_ChangeResourceRecordSets.html>`__ .
        ...
        You can associate health checks with failover resource record sets in a private hosted zone. Note
        the following:

        * Route 53 health checkers are outside the VPC. To check the health of an endpoint within a VPC by
        ...
        * You can configure a health checker to check the health of an external resource that the instance
        relies on, such as a database server.

        * You can create a CloudWatch metric, associate an alarm with the metric, and then create a health
        check that is based on the state of the alarm. For example, you might create a CloudWatch metric
        that checks the status of the Amazon EC2 ``StatusCheckFailed`` metric, add an alarm to the metric,
        and then create a health check that is based on the state of the alarm. For information about
        creating CloudWatch metrics and alarms by using the CloudWatch console, see the `Amazon CloudWatch
        User Guide
        <http://docs.aws.amazon.com/AmazonCloudWatch/latest/DeveloperGuide/WhatIsCloudWatch.html>`__ .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateHealthCheck>`_
        ...
        **Request Syntax**
          ...
          response = client.create_health_check(
              ...
              HealthCheckConfig={
                  ...
                  'AlarmIdentifier': {
                      ...
                      |'eu-west-1'|'eu-west-2'|'eu-west-3'|'ap-east-1'|'me-south-1'|'ap-south-1'
                      |'ap-southeast-1'|'ap-southeast-2'|'ap-northeast-1'|'ap-northeast-2'|'ap-northeast-3'
                      |'eu-north-1'|'sa-east-1'|'cn-northwest-1'|'cn-north-1',
                      'Name': 'string'
                  },
        ...
        :param CallerReference: **[REQUIRED]**
          ...
          * If you send a ``CreateHealthCheck`` request with the same ``CallerReference`` and settings as a
          previous request, and if the health check doesn't exist, Amazon Route 53 creates the health check.
          If the health check does exist, Route 53 returns the settings for the existing health check.
        ...
        :param HealthCheckConfig: **[REQUIRED]**
          ...
          - **ResourcePath** *(string) --*
            ...
            path can be any value for which your endpoint will return an HTTP status code of 2xx or 3xx when
            the endpoint is healthy, for example, the file /docs/route53-health-check.html. You can also
            include query string parameters, for example, ``/welcome.html?language=jp&login=y`` .
          ...
          - **FullyQualifiedDomainName** *(string) --*
            ...
            * If you specify another value for ``Port`` and any value except ``TCP`` for ``Type`` , Route 53
            ...
            .. note::
              ...
              to the endpoint. If there's no resource record set with a type of A for the name that you
              specify for ``FullyQualifiedDomainName`` , the health check fails with a "DNS resolution
              failed" error.
          ...
          - **Disabled** *(boolean) --*
            ...
            * **Health checks that monitor CloudWatch alarms:** Route 53 stops monitoring the corresponding
            ...
            healthy. If you configured DNS failover, Route 53 continues to route traffic to the
                ...
                'HealthCheck': {
                    ...
                    'HealthCheckConfig': {
                        ...
                        'AlarmIdentifier': {
                            ...
                            |'ap-northeast-3'|'eu-north-1'|'sa-east-1'|'cn-northwest-1'|'cn-north-1',
                            'Name': 'string'
                        },
                        'InsufficientDataHealthStatus': 'Healthy'|'Unhealthy'|'LastKnownStatus'
                    },
            ...
            - **HealthCheck** *(dict) --*
              ...
              - **HealthCheckConfig** *(dict) --*
                ...
                - **IPAddress** *(string) --*
                  ...
                  interval that you specify in ``RequestInterval`` . Using an IP address returned by DNS,
                  Route 53 then checks the health of the endpoint.
                ...
                - **Type** *(string) --*
                  ...
                  * **HTTP_STR_MATCH** : Route 53 tries to establish a TCP connection. If successful, Route
                  53 submits an HTTP request and searches the first 5,120 bytes of the response body for the
                  string that you specify in ``SearchString`` .

                  * **HTTPS_STR_MATCH** : Route 53 tries to establish a TCP connection. If successful, Route
                  53 submits an ``HTTPS`` request and searches the first 5,120 bytes of the response body
                  ...
                  * **CALCULATED** : For health checks that monitor the status of other health checks, Route
                  53 adds up the number of health checks that Route 53 health checkers consider to be
                ...
                - **FullyQualifiedDomainName** *(string) --*
                  ...
                  * If you specify another value for ``Port`` and any value except ``TCP`` for ``Type`` ,
                  ...
                  you choose to specify the endpoint only by ``FullyQualifiedDomainName`` , we recommend
                  ...
                  for ``IPAddress`` . If the value of ``Type`` is ``TCP`` , Route 53 doesn't pass a ``Host``
                  header.
              ...
              - **CloudWatchAlarmConfiguration** *(dict) --*
                ...
                - **ComparisonOperator** *(string) --*
                  ...
                  For the metric that the CloudWatch alarm is associated with, the arithmetic operation that
                ...
                - **Statistic** *(string) --*
                  ...
                  For the metric that the CloudWatch alarm is associated with, the statistic that is applied
                  to the metric.
    ...
    def create_hosted_zone(
        ...
        * For public hosted zones, Amazon Route 53 automatically creates a default SOA record and four NS
        records for the zone. For more information about SOA and NS records, see `NS and SOA Records that
        Route 53 Creates for a Hosted Zone
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/SOA-NSrecords.html>`__ in the *Amazon
        Route 53 Developer Guide* . If you want to use the same name servers for multiple public hosted
        zones, you can optionally associate a reusable delegation set with the hosted zone. See the
        ...
        * If your domain is registered with a registrar other than Route 53, you must update the name
        ...
        see `Migrating DNS Service for an Existing Domain to Amazon Route 53
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/MigratingDNS.html>`__ in the *Amazon Route
        53 Developer Guide* .

        When you submit a ``CreateHostedZone`` request, the initial status of the hosted zone is ``PENDING``
        . For public hosted zones, this means that the NS and SOA records are not yet available on all Route
        53 DNS servers. When the NS and SOA records are available, the status of the zone changes to
        ``INSYNC`` .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateHostedZone>`_
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          servers for your domain to the set of ``NameServers`` that ``CreateHostedZone`` returns in
          ``DelegationSet`` .
        ...
        :param CallerReference: **[REQUIRED]**
          ...
          A unique string that identifies the request and that allows failed ``CreateHostedZone`` requests
          to be retried without the risk of executing the operation twice. You must use a unique
          ``CallerReference`` string every time you submit a ``CreateHostedZone`` request.
        ...
        :param DelegationSetId:
          ...
          53 assigned to the reusable delegation set when you created it. For more information about
          reusable delegation sets, see `CreateReusableDelegationSet
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateReusableDelegationSet.html>`__
          .
          ...
          - *(dict) --*
            ...
            - **HostedZone** *(dict) --*
              ...
              - **LinkedService** *(dict) --*
                ...
                When a hosted zone is created by another service, you can't edit or delete it using Route
                53.
    ...
    def create_query_logging_config(
        ...
        * Create a CloudWatch Logs log group, and make note of the ARN, which you specify when you create a
        query logging configuration. Note the following:
          ...
          * When you create log groups for query logging, we recommend that you use a consistent prefix, for
          example:  ``/aws/route53/*hosted zone name* ``   In the next step, you'll create a resource
          policy, which controls access to one or more log groups and the associated AWS resources, such as
          Route 53 hosted zones. There's a limit on the number of resource policies that you can create, so
          we recommend that you use a consistent prefix so you can use the same resource policy for all the
        ...
        * Create a CloudWatch Logs resource policy, and give it the permissions that Route 53 needs to
        ...
        the ARN for the log group that you created in the previous step. To use the same resource policy for
        all the CloudWatch Logs log groups that you created for query logging configurations, replace the
        hosted zone name with ``*`` , for example:
        ...
        * Creates a log stream for an edge location the first time that the edge location responds to DNS
        queries for the specified hosted zone. That log stream is used to log all queries that Route 53
        ...
        * Begins to send query logs to the applicable log stream.
        ...
        The three-letter code typically corresponds with the International Air Transport Association airport
        code for an airport near the edge location. (These abbreviations might change in the future.) For a
        ...
        Query logs contain only the queries that DNS resolvers forward to Route 53. If a DNS resolver has
        already cached the response to a query (such as the IP address for a load balancer for example.com),
        the resolver will continue to return the cached response. It doesn't forward another query to Route
        53 until the TTL for the corresponding resource record set expires. Depending on how many DNS
        queries are submitted for a resource record set, and depending on the TTL for that resource record
        set, query logs might contain information about only one query out of every several thousand queries
        that are submitted to DNS. For more information about how DNS works, see `Routing Internet Traffic
        to Your Website or Web Application
        <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/welcome-dns-service.html>`__ in the
        *Amazon Route 53 Developer Guide* .

          Log File Format

        For a list of the values in each query log and the format of each value, see `Logging DNS Queries
        <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/query-logs.html>`__ in the *Amazon Route
        53 Developer Guide* .
        ...
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_DeleteQueryLoggingConfig.html>`__ .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateQueryLoggingConfig>`_
        ...
        **Request Syntax**
        ...
        :param CloudWatchLogsLogGroupArn: **[REQUIRED]**
          ...
          <https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_DescribeLogGroups.html>`__
          API action, the `describe-log-groups
          <https://docs.aws.amazon.com/cli/latest/reference/logs/describe-log-groups.html>`__ command, or
          the applicable command in one of the AWS SDKs.
    ...
    def create_reusable_delegation_set(
        ...
        If a hosted zoned ID is specified, ``CreateReusableDelegationSet`` marks the delegation set
        ...
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/white-label-name-servers.html>`__ .

        The process for migrating existing hosted zones to use a reusable delegation set is comparable to
        the process for configuring white label name servers. You need to perform the following steps:
        ...
        * Change TTLs back to their original values.
        ...
        If you want to migrate existing hosted zones to use a reusable delegation set, the existing hosted
        ...
        * For small numbers of hosted zones—up to a few hundred—it's relatively easy to create reusable
        delegation sets until you get one that has four name servers that don't overlap with any of the name
        servers in your hosted zones.

        * For larger numbers of hosted zones, the easiest solution is to use more than one reusable
        delegation set.

        * For larger numbers of hosted zones, you can also migrate hosted zones that have overlapping name
        servers to hosted zones that don't have overlapping name servers, then migrate the hosted zones
        again to use the reusable delegation set.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateReusableDelegationSet>`_
    ...
    def create_traffic_policy(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateTrafficPolicy>`_
          ...
          - *(dict) --*
            ...
            - **TrafficPolicy** *(dict) --*
              ...
              - **Type** *(string) --*
                ...
                The DNS type of the resource record sets that Amazon Route 53 creates when you use a traffic
                policy to create a traffic policy instance.
    ...
    def create_traffic_policy_instance(
        ...
        policy version. In addition, ``CreateTrafficPolicyInstance`` associates the resource record sets
        with a specified domain name (such as example.com) or subdomain name (such as www.example.com).
        Amazon Route 53 responds to DNS queries for the domain or subdomain name by using the resource
        record sets that ``CreateTrafficPolicyInstance`` created.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateTrafficPolicyInstance>`_
    ...
    def create_traffic_policy_version(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateTrafficPolicyVersion>`_
        ...
        **Request Syntax**
        ...
        :param Document: **[REQUIRED]**
          ...
          The definition of this version of the traffic policy, in JSON format. You specified the JSON in
          the ``CreateTrafficPolicyVersion`` request. For more information about the JSON format, see
          `CreateTrafficPolicy
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateTrafficPolicy.html>`__ .
    ...
    def create_vpc_association_authorization(
        ...
        .. note::
          ...
          that you created by using a different account, you must submit one authorization request for each
          VPC.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/CreateVPCAssociationAuthorization>`_
    ...
    def delete_health_check(self, HealthCheckId: str) -> Dict:
        ...
        .. warning::
          ...
          Amazon Route 53 does not prevent you from deleting a health check even if the health check is
          associated with one or more resource record sets. If you delete a health check and you don't
          update the associated resource record sets, the future status of the health check can't be
          predicted and may change. This will affect the routing of DNS queries for your DNS failover
          configuration. For more information, see `Replacing and Deleting Health Checks
          <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/health-checks-creating-deleting.html#health-checks-deleting.html>`__
          in the *Amazon Route 53 Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteHealthCheck>`_
    ...
    def delete_hosted_zone(self, Id: str) -> ClientDeleteHostedZoneResponseTypeDef:
        ...
        If the hosted zone was created by another service, such as AWS Cloud Map, see `Deleting Public
        Hosted Zones That Were Created by Another Service
        <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/DeleteHostedZone.html#delete-public-hosted-zone-created-by-another-service>`__
        in the *Amazon Route 53 Developer Guide* for information about how to delete it. (The process is the
        same for public and private hosted zones that were created by another service.)

        If you want to keep your domain registration but you want to stop routing internet traffic to your
        website or web application, we recommend that you delete resource record sets in the hosted zone
        instead of deleting the hosted zone.

        .. warning::

          If you delete a hosted zone, you can't undelete it. You must create a new hosted zone and update
          the name servers for your domain registration, which can require up to 48 hours to take effect.
          (If you delegated responsibility for a subdomain to a hosted zone and you delete the child hosted
          zone, you must update the name servers in the parent hosted zone.) In addition, if you delete a
          hosted zone, someone could hijack the domain and route traffic to their own resources using your
          domain name.

        If you want to avoid the monthly charge for the hosted zone, you can transfer DNS service for the
        domain to a free DNS service. When you transfer DNS service, you have to update the name servers for
        the domain registration. If the domain is registered with Route 53, see `UpdateDomainNameservers
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_domains_UpdateDomainNameservers.html>`__
        ...
        registrar to update name servers for the domain registration. For more information, perform an
        internet search on "free DNS service."

        You can delete a hosted zone only if it contains only the default SOA record and NS resource record
        sets. If the hosted zone contains other resource record sets, you must delete them before you can
        delete the hosted zone. If you try to delete a hosted zone that contains other resource record sets,
        the request fails, and Route 53 returns a ``HostedZoneNotEmpty`` error. For information about
        ...
        * Use the ``ListHostedZones`` action to get a list of the hosted zones associated with the current
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteHostedZone>`_
    ...
    def delete_query_logging_config(self, Id: str) -> Dict[str, Any]:
        ...
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateQueryLoggingConfig.html>`__ .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteQueryLoggingConfig>`_
    ...
    def delete_reusable_delegation_set(self, Id: str) -> Dict:
        ...
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_GetReusableDelegationSet.html>`__
        request and specify the ID of the reusable delegation set that you want to delete.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteReusableDelegationSet>`_
    ...
    def delete_traffic_policy(self, Id: str, Version: int) -> Dict:
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteTrafficPolicy>`_
    ...
    def delete_traffic_policy_instance(self, Id: str) -> Dict:
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteTrafficPolicyInstance>`_
        ...
        **Request Syntax**
        ...
        :param Id: **[REQUIRED]**
          ...
          .. warning::
            ...
            When you delete a traffic policy instance, Amazon Route 53 also deletes all of the resource
    ...
    def delete_vpc_association_authorization(
        ...
        """
        Removes authorization to submit an ``AssociateVPCWithHostedZone`` request to associate a specified
        VPC with a hosted zone that was created by a different account. You must use the account that
        created the hosted zone to submit a ``DeleteVPCAssociationAuthorization`` request.

        .. warning::

          Sending this request only prevents the AWS account that created the VPC from associating the VPC
          with the Amazon Route 53 hosted zone in the future. If the VPC is already associated with the
          hosted zone, ``DeleteVPCAssociationAuthorization`` won't disassociate the VPC from the hosted
          zone. If you want to delete an existing association, use ``DisassociateVPCFromHostedZone`` .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DeleteVPCAssociationAuthorization>`_
        ...
        **Request Syntax**
        ...
        :param HostedZoneId: **[REQUIRED]**
          ...
          zone that was created with a different AWS account, the ID of the hosted zone.

        :type VPC: dict
        :param VPC: **[REQUIRED]**

          When removing authorization to associate a VPC that was created by one AWS account with a hosted
          zone that was created with a different AWS account, a complex type that includes the ID and region
          of the VPC.
    ...
    def disassociate_vpc_from_hosted_zone(
        ...
        * You can submit a ``DisassociateVPCFromHostedZone`` request using either the account that created
        the hosted zone or the account that created the VPC.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/DisassociateVPCFromHostedZone>`_
    ...
    def get_account_limit(self, Type: str) -> ClientGetAccountLimitResponseTypeDef:
        ...
        that you can create using the account.
        ...
        <https://console.aws.amazon.com/support/home#/case/create?issueType=
            service-limit-increase&limitType=service-code-route53>`__ .

        .. note::

          You can also view account limits in AWS Trusted Advisor. Sign in to the AWS Management Console and
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetAccountLimit>`_
        ...
        **Request Syntax**
        ::
          ...
          response = client.get_account_limit(
              Type=
                  ...
                  |'MAX_TRAFFIC_POLICY_INSTANCES_BY_OWNER'|'MAX_REUSABLE_DELEGATION_SETS_BY_OWNER'
        ...
        :param Type: **[REQUIRED]**
          ...
          * **MAX_HOSTED_ZONES_BY_OWNER** : The maximum number of hosted zones that you can create using the
          current account.

          * **MAX_REUSABLE_DELEGATION_SETS_BY_OWNER** : The maximum number of reusable delegation sets that
          ...
          * **MAX_TRAFFIC_POLICY_INSTANCES_BY_OWNER** : The maximum number of traffic policy instances that
          you can create using the current account. (Traffic policy instances are referred to as traffic
    ...
    def get_change(self, Id: str) -> ClientGetChangeResponseTypeDef:
        ...
        * ``PENDING`` indicates that the changes in this request have not propagated to all Amazon Route 53
        DNS servers. This is the initial status of all change batch requests.
    ...
    def get_checker_ip_ranges(
        ...
        .. warning::
           ...
           ``GetCheckerIpRanges`` still works, but we recommend that you download ip-ranges.json, which
           includes IP address ranges for all AWS services. For more information, see `IP Address Ranges of
           Amazon Route 53 Servers
           <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/route-53-ip-addresses.html>`__ in the
           *Amazon Route 53 Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetCheckerIpRanges>`_
    ...
    def get_geo_location(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetGeoLocation>`_
        ...
        **Request Syntax**
        ...
        :param SubdivisionCode:
          ...
          Amazon Route 53 uses the one- to three-letter subdivision codes that are specified in `ISO
          standard 3166-1 alpha-2 <https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2>`__ . Route 53 doesn't
          support subdivision codes for all countries. If you specify ``subdivisioncode`` , you must also
          specify ``countrycode`` .
    ...
    def get_health_check(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHealthCheck>`_
        ...
        **Request Syntax**
        ...
        :param HealthCheckId: **[REQUIRED]**
          ...
          or update a resource record set, you use this value to specify which health check to use. The
    ...
    def get_health_check_count(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHealthCheckCount>`_
    ...
    def get_health_check_last_failure_reason(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHealthCheckLastFailureReason>`_
        ...
        **Request Syntax**
        ...
        :param HealthCheckId: **[REQUIRED]**
          ...
          The ID for the health check for which you want the last failure reason. When you created the
          health check, ``CreateHealthCheck`` returned the ID in the response, in the ``HealthCheckId``
          element.

          .. note::

            If you want to get the last failure reason for a calculated health check, you must use the
              ...
              - *(dict) --*
                ...
                - **StatusReport** *(dict) --*
                  ...
                  - **CheckedTime** *(datetime) --*
                    ...
                    <https://en.wikipedia.org/wiki/ISO_8601>`__ and Coordinated Universal Time (UTC). For
                    example, the value ``2017-03-27T17:48:16.751Z`` represents March 27, 2017 at
                    17:48:16.751 UTC.
    ...
    def get_health_check_status(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHealthCheckStatus>`_
        ...
        **Request Syntax**
        ...
        :param HealthCheckId: **[REQUIRED]**
          ...
          The ID for the health check that you want the current status for. When you created the health
          check, ``CreateHealthCheck`` returned the ID in the response, in the ``HealthCheckId`` element.

          .. note::

            If you want to check the status of a calculated health check, you must use the Amazon Route 53
            console or the CloudWatch console. You can't use ``GetHealthCheckStatus`` to get the status of a
    ...
    def get_hosted_zone(self, Id: str) -> ClientGetHostedZoneResponseTypeDef:
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHostedZone>`_
                ...
                'VPCs': [
                    {
                        'VPCRegion':
                        'us-east-1'|'us-east-2'|'us-west-1'|'us-west-2'|'eu-west-1'|'eu-west-2'|'eu-west-3'
                        |'eu-central-1'|'ap-east-1'|'me-south-1'|'ap-southeast-1'|'ap-southeast-2'
                        |'ap-south-1'|'ap-northeast-1'|'ap-northeast-2'|'ap-northeast-3'|'eu-north-1'
                        |'sa-east-1'|'ca-central-1'|'cn-north-1',
                        'VPCId': 'string'
                    },
                ]
            }
    ...
    def get_hosted_zone_count(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHostedZoneCount>`_
    ...
    def get_hosted_zone_limit(
        ...
        that you can create in the hosted zone.
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetHostedZoneLimit>`_
        ...
        **Request Syntax**
        ...
        :param Type: **[REQUIRED]**
          ...
          * **MAX_VPCS_ASSOCIATED_BY_ZONE** : The maximum number of Amazon VPCs that you can associate with
          the specified private hosted zone.
    ...
    def get_query_logging_config(
        ...
        For more information about DNS query logs, see `CreateQueryLoggingConfig
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateQueryLoggingConfig.html>`__ and
        `Logging DNS Queries <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/query-logs.html>`__
        .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetQueryLoggingConfig>`_
    ...
    def get_reusable_delegation_set(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetReusableDelegationSet>`_
    ...
    def get_reusable_delegation_set_limit(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetReusableDelegationSetLimit>`_
        ...
        **Request Syntax**
        ...
        :param Type: **[REQUIRED]**
          ...
          Specify ``MAX_ZONES_BY_REUSABLE_DELEGATION_SET`` to get the maximum number of hosted zones that
    ...
    def get_traffic_policy(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetTrafficPolicy>`_
    ...
    def get_traffic_policy_instance(
        ...
        .. note::
          ...
          the traffic policy definition. For more information, see the ``State`` response element.
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetTrafficPolicyInstance>`_
    ...
    def get_traffic_policy_instance_count(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/GetTrafficPolicyInstanceCount>`_
    ...
    def list_geo_locations(
        ...
        for a country (for example, states or provinces), the subdivisions for that country are listed in
        alphabetical order immediately after the corresponding country.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListGeoLocations>`_
        ...
        **Request Syntax**
        ...
        :param StartContinentCode:
          ...
          ``IsTruncated`` is true, and if ``NextContinentCode`` from the previous response has a value,
          enter that value in ``startcontinentcode`` to return the next page of results.
        ...
        :param StartCountryCode:
          ...
          supports for geolocation. If Route 53 has already returned a page or more of results, if
          ``IsTruncated`` is ``true`` , and if ``NextCountryCode`` from the previous response has a value,
        ...
        :param StartSubdivisionCode:
          ...
          locations that Amazon Route 53 supports for geolocation. If Route 53 has already returned a page
          or more of results, if ``IsTruncated`` is ``true`` , and if ``NextSubdivisionCode`` from the
          previous response has a value, enter that value in ``startsubdivisioncode`` to return the next
          page of results.
        ...
        :param MaxItems:
          ...
          request. If more than ``maxitems`` geolocations remain to be listed, then the value of the
    ...
    def list_health_checks(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListHealthChecks>`_
        ...
        **Request Syntax**
        ...
        :param Marker:
          ...
          which is the ID of the first health check that Amazon Route 53 will return if you submit another
          request.

          If the value of ``IsTruncated`` in the previous response was ``false`` , there are no more health
          checks to get.

        :type MaxItems: string
        :param MaxItems:

          The maximum number of health checks that you want ``ListHealthChecks`` to return in response to
          the current request. Amazon Route 53 returns a maximum of 100 items. If you set ``MaxItems`` to a
                ...
                'HealthChecks': [
                    {
                        ...
                        'HealthCheckConfig': {
                            ...
                            'AlarmIdentifier': {
                                ...
                                |'eu-central-1'|'eu-west-1'|'eu-west-2'|'eu-west-3'|'ap-east-1'|'me-south-1'
          ...
          **Response Structure**
          ...
          - *(dict) --*
                ...
                - **HealthCheckConfig** *(dict) --*
                  ...
                  - **Type** *(string) --*
                    ...
                    * **HTTP_STR_MATCH** : Route 53 tries to establish a TCP connection. If successful,
                    Route 53 submits an HTTP request and searches the first 5,120 bytes of the response body
                    for the string that you specify in ``SearchString`` .
            ...
            - **NextMarker** *(string) --*
              ...
              If ``IsTruncated`` is ``true`` , the value of ``NextMarker`` identifies the first health check
              that Amazon Route 53 returns if you submit another ``ListHealthChecks`` request and specify
    ...
    def list_hosted_zones(
        ...
        """
        Retrieves a list of the public and private hosted zones that are associated with the current AWS
        account. The response includes a ``HostedZones`` child element for each hosted zone.

        Amazon Route 53 returns a maximum of 100 items in each response. If you have a lot of hosted zones,
        you can use the ``maxitems`` parameter to list them in groups of up to 100.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListHostedZones>`_
        ...
        **Request Syntax**
        ...
        :param Marker:
          ...
          If the value of ``IsTruncated`` in the previous response was ``true`` , you have more hosted
          zones. To get more hosted zones, submit another ``ListHostedZones`` request.

          For the value of ``marker`` , specify the value of ``NextMarker`` from the previous response,
          which is the ID of the first hosted zone that Amazon Route 53 will return if you submit another
          request.
        ...
        :param MaxItems:
          ...
          more than ``maxitems`` hosted zones, the value of ``IsTruncated`` in the response is ``true`` ,
          and the value of ``NextMarker`` is the hosted zone ID of the first hosted zone that Route 53 will
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **IsTruncated** *(boolean) --*
              ...
              truncated, you can get more hosted zones by submitting another ``ListHostedZones`` request and
            ...
            - **NextMarker** *(string) --*
              ...
              in the next group of hosted zones. Submit another ``ListHostedZones`` request, and specify the
    ...
    def list_hosted_zones_by_name(
        ...
        the domain name using the escaped or Punycoded value, which is the format that Amazon Route 53 saves
        in its database. For example, to create a hosted zone for exämple.com, you specify ex\\344mple.com
        ...
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/DomainNameFormat.html>`__ in the *Amazon
        Route 53 Developer Guide* .
        ...
        * If the value of ``IsTruncated`` in the response is true, there are more hosted zones associated
        with the current AWS account.  If ``IsTruncated`` is false, this response includes the last hosted
        ...
        * The ``NextDNSName`` and ``NextHostedZoneId`` elements in the response contain the domain name and
        the hosted zone ID of the next hosted zone that is associated with the current AWS account. If you
        want to list more hosted zones, make another call to ``ListHostedZonesByName`` , and specify the
        value of ``NextDNSName`` and ``NextHostedZoneId`` in the ``dnsname`` and ``hostedzoneid``
        parameters, respectively.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListHostedZonesByName>`_
        ...
        **Request Syntax**
        ...
        :param DNSName:
          ...
          (Optional) For your first request to ``ListHostedZonesByName`` , include the ``dnsname`` parameter
          only if you want to specify the name of the first hosted zone in the response. If you don't
          ...
          ``dnsname`` and ``hostedzoneid`` parameters. For ``dnsname`` , specify the value of
        ...
        :param MaxItems:
          ...
          The maximum number of hosted zones to be included in the response body for this request. If you
          have more than ``maxitems`` hosted zones, then the value of the ``IsTruncated`` element in the
          response is true, and the values of ``NextDNSName`` and ``NextHostedZoneId`` specify the first
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextHostedZoneId** *(string) --*
              ...
              If ``IsTruncated`` is ``true`` , the value of ``NextHostedZoneId`` identifies the first hosted
              zone in the next group of ``maxitems`` hosted zones. Call ``ListHostedZonesByName`` again and
              specify the value of ``NextDNSName`` and ``NextHostedZoneId`` in the ``dnsname`` and
              ``hostedzoneid`` parameters, respectively.
    ...
    def list_query_logging_configs(
        ...
        """
        Lists the configurations for DNS query logging that are associated with the current AWS account or
        the configuration that is associated with a specified hosted zone.

        For more information about DNS query logs, see `CreateQueryLoggingConfig
        <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateQueryLoggingConfig.html>`__ .
        Additional information, including the format of DNS query logs, appears in `Logging DNS Queries
        <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/query-logs.html>`__ in the *Amazon Route
        53 Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListQueryLoggingConfigs>`_
        ...
        **Request Syntax**
        ...
        :param HostedZoneId:
          ...
          zone, specify the ID in ``HostedZoneId`` .
        ...
        :param NextToken:
          ...
          (Optional) If the current AWS account has more than ``MaxResults`` query logging configurations,
        ...
        :param MaxResults:
          ...
          return in response to the current request. If the current AWS account has more than ``MaxResults``
          configurations, use the value of `NextToken
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_ListQueryLoggingConfigs.html#API_ListQueryLoggingConfigs_RequestSyntax>`__
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextToken** *(string) --*
              ...
              request. Get the value of ``NextToken`` that Amazon Route 53 returned in the previous response
    ...
    def list_resource_record_sets(
        ...
        Lists the resource record sets in a specified hosted zone.
         ...
         ``ListResourceRecordSets`` returns up to 100 resource record sets at a time in ASCII order,
        ...
        Note the trailing dot, which can change the sort order when the record name contains characters that
        appear before ``.`` (decimal 46) in the ASCII table. These characters include the following: ``! " #
        $ % & ' ( ) * + , -``
        ...
        The results begin with the first resource record set in the list whose name is greater than or equal
        to ``Name`` .
        ...
        ``PENDING`` , and that are not yet available on all Route 53 DNS servers.

         **Changing resource record sets**

        To ensure that you get an accurate listing of the resource record sets for a hosted zone at a point
        in time, do not submit a ``ChangeResourceRecordSets`` request while you're paging through the
        results of a ``ListResourceRecordSets`` request. If you do, some pages may display results without
        the latest changes while other pages display results with the latest changes.
        ...
        ``NextRecordName`` , ``NextRecordType`` , and ``NextRecordIdentifier`` (if any) from the response.
        Then submit another ``ListResourceRecordSets`` request, and specify those values for
        ``StartRecordName`` , ``StartRecordType`` , and ``StartRecordIdentifier`` .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListResourceRecordSets>`_
        ...
        **Request Syntax**
        ...
        :param StartRecordType:
          ...
          * **Another resource record set in this hosted zone:** The type of the resource record set that
        ...
        :param StartRecordIdentifier:
           ...
           *Resource record sets that have a routing policy other than simple:* If results were truncated
           for a given DNS name and type, specify the value of ``NextRecordIdentifier`` from the previous
        ...
        :param MaxItems:
          ...
          ``NextRecordType`` elements in the response identify the first resource record set in the next
          group of ``maxitems`` resource record sets.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **IsTruncated** *(boolean) --*
              ...
              A flag that indicates whether more resource record sets remain to be listed. If your results
              were truncated, you can make a follow-up pagination request by using the ``NextRecordName``
              element.
            ...
            - **NextRecordIdentifier** *(string) --*
               ...
               *Resource record sets that have a routing policy other than simple:* If results were
               truncated for a given DNS name and type, the value of ``SetIdentifier`` for the next resource
    ...
    def list_reusable_delegation_sets(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListReusableDelegationSets>`_
        ...
        **Request Syntax**
        ...
        :param Marker:
          ...
          For the value of ``marker`` , specify the value of ``NextMarker`` from the previous response,
          which is the ID of the first reusable delegation set that Amazon Route 53 will return if you
        ...
        :param MaxItems:
          ...
          this request. If you specify a value greater than 100, Route 53 returns only the first 100
    ...
    def list_tags_for_resource(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTagsForResource>`_
              ...
              - **Tags** *(list) --*
                ...
                - *(dict) --*
                  ...
                  - **Key** *(string) --*
                    ...
                    * **Give a name to a health check** : Edit the default ``Name`` tag. In the Amazon Route
                    53 console, the list of your health checks includes a **Name** column that lets you see
    ...
    def list_tags_for_resources(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTagsForResources>`_
    ...
    def list_traffic_policies(
        ...
        current AWS account. Policies are listed in the order that they were created in.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTrafficPolicies>`_
        ...
        **Request Syntax**
        ...
        :param TrafficPolicyIdMarker:
          ...
          If you have more traffic policies than the value of ``MaxItems`` , ``ListTrafficPolicies`` returns
          only the first ``MaxItems`` traffic policies. To get the next group of policies, submit another
        ...
        :param MaxItems:
          ...
          response to this request. If you have more than ``MaxItems`` traffic policies, the value of
          ``IsTruncated`` in the response is ``true`` , and the value of ``TrafficPolicyIdMarker`` is the ID
          of the first traffic policy that Route 53 will return if you submit another request.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **TrafficPolicyIdMarker** *(string) --*
              ...
              If the value of ``IsTruncated`` is ``true`` , ``TrafficPolicyIdMarker`` is the ID of the first
              traffic policy in the next group of ``MaxItems`` traffic policies.
    ...
    def list_traffic_policy_instances(
        ...
        .. note::
          ...
          After you submit an ``UpdateTrafficPolicyInstance`` request, there's a brief delay while Amazon
          Route 53 creates the resource record sets that are specified in the traffic policy definition. For
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTrafficPolicyInstances>`_
        ...
        **Request Syntax**
        ...
        :param HostedZoneIdMarker:
          ...
          If the value of ``IsTruncated`` in the previous response was ``false`` , there are no more traffic
        ...
        :param TrafficPolicyInstanceNameMarker:
          ...
          If the value of ``IsTruncated`` in the previous response was ``true`` , you have more traffic
          policy instances. To get more traffic policy instances, submit another
          ``ListTrafficPolicyInstances`` request. For the value of ``trafficpolicyinstancename`` , specify
          the value of ``TrafficPolicyInstanceNameMarker`` from the previous response, which is the name of
        ...
        :param TrafficPolicyInstanceTypeMarker:
          ...
          If the value of ``IsTruncated`` in the previous response was ``true`` , you have more traffic
          policy instances. To get more traffic policy instances, submit another
          ``ListTrafficPolicyInstances`` request. For the value of ``trafficpolicyinstancetype`` , specify
          the value of ``TrafficPolicyInstanceTypeMarker`` from the previous response, which is the type of
          the first traffic policy instance in the next group of traffic policy instances.
        ...
        :param MaxItems:
          ...
          to a ``ListTrafficPolicyInstances`` request. If you have more than ``MaxItems`` traffic policy
          instances, the value of the ``IsTruncated`` element in the response is ``true`` , and the values
    ...
    def list_traffic_policy_instances_by_hosted_zone(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTrafficPolicyInstancesByHostedZone>`_
        ...
        **Request Syntax**
        ...
        :param TrafficPolicyInstanceNameMarker:
          ...
          ``TrafficPolicyInstanceNameMarker`` from the previous response, which is the name of the first
          traffic policy instance in the next group of traffic policy instances.
        ...
        :param TrafficPolicyInstanceTypeMarker:
          ...
          ``TrafficPolicyInstanceTypeMarker`` from the previous response, which is the type of the first
          traffic policy instance in the next group of traffic policy instances.
        ...
        :param MaxItems:
          ...
          The maximum number of traffic policy instances to be included in the response body for this
          request. If you have more than ``MaxItems`` traffic policy instances, the value of the
          ``IsTruncated`` element in the response is ``true`` , and the values of ``HostedZoneIdMarker`` ,
          ``TrafficPolicyInstanceNameMarker`` , and ``TrafficPolicyInstanceTypeMarker`` represent the first
    ...
    def list_traffic_policy_instances_by_policy(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTrafficPolicyInstancesByPolicy>`_
        ...
        **Request Syntax**
        ...
        :param HostedZoneIdMarker:
          ...
          For the value of ``hostedzoneid`` , specify the value of ``HostedZoneIdMarker`` from the previous
          response, which is the hosted zone ID of the first traffic policy instance that Amazon Route 53
        ...
        :param TrafficPolicyInstanceTypeMarker:
          ...
          ``TrafficPolicyInstanceTypeMarker`` from the previous response, which is the name of the first
          traffic policy instance that Amazon Route 53 will return if you submit another request.
    ...
    def list_traffic_policy_versions(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListTrafficPolicyVersions>`_
        ...
        **Request Syntax**
        ...
        :param TrafficPolicyVersionMarker:
          ...
          ``ListTrafficPolicyVersions`` returns only the first group of ``MaxItems`` versions. To get more
          traffic policy versions, submit another ``ListTrafficPolicyVersions`` request. For the value of
          ``TrafficPolicyVersionMarker`` , specify the value of ``TrafficPolicyVersionMarker`` in the
          previous response.
        ...
        :param MaxItems:
          ...
          response body for this request. If the specified traffic policy has more than ``MaxItems``
          versions, the value of ``IsTruncated`` in the response is ``true`` , and the value of the
          ``TrafficPolicyVersionMarker`` element is the ID of the first version that Route 53 will return if
    ...
    def list_vpc_association_authorizations(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/ListVPCAssociationAuthorizations>`_
        ...
        **Request Syntax**
        ...
        :param NextToken:
           ...
           *Optional* : If a response includes a ``NextToken`` element, there are more VPCs that can be
           associated with the specified hosted zone. To get the next page of results, submit another
           request, and include the value of ``NextToken`` from the response in the ``nexttoken`` parameter
           in another ``ListVPCAssociationAuthorizations`` request.

        :type MaxResults: string
        :param MaxResults:

           *Optional* : An integer that specifies the maximum number of VPCs that you want Amazon Route 53
           to return. If you don't specify a value for ``MaxResults`` , Route 53 returns up to 50 VPCs per
           page.
    ...
    def test_dns_answer(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/TestDNSAnswer>`_
        ...
        **Request Syntax**
        ...
        :param ResolverIP:
          ...
          resolver. If you omit this value, ``TestDnsAnswer`` uses the IP address of a DNS resolver in the
        ...
        :param EDNS0ClientSubnetIP:
          ...
          of a client in the applicable location, for example, ``192.0.2.44`` or
        ...
        :param EDNS0ClientSubnetMask:
          ...
          of bits of the IP address that you want the checking tool to include in the DNS query. For
          example, if you specify ``192.0.2.44`` for ``edns0clientsubnetip`` and ``24`` for
          ``edns0clientsubnetmask`` , the checking tool will simulate a request from 192.0.2.0/24. The
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ResponseCode** *(string) --*
              ...
              returns a response code that describes the error. For a list of possible response codes, see
              `DNS RCODES
    ...
    def update_health_check(
        ...
        Checks
        <http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/health-checks-creating-deleting.html>`__
        in the *Amazon Route 53 Developer Guide* .

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/UpdateHealthCheck>`_
        ...
        **Request Syntax**
        ::
          ...
          response = client.update_health_check(
              ...
              AlarmIdentifier={
                  ...
                  |'eu-west-2'|'eu-west-3'|'ap-east-1'|'me-south-1'|'ap-south-1'|'ap-southeast-1'
                  |'ap-southeast-2'|'ap-northeast-1'|'ap-northeast-2'|'ap-northeast-3'|'eu-north-1'
                  |'sa-east-1'|'cn-northwest-1'|'cn-north-1',
                  'Name': 'string'
              },
              InsufficientDataHealthStatus='Healthy'|'Unhealthy'|'LastKnownStatus',
              ResetElements=[
        ...
        :param HealthCheckVersion:
          ...
          ``HealthCheckVersion`` for the health check that you want to update, and that you include that
          value in your ``UpdateHealthCheck`` request. This prevents Route 53 from overwriting an
          intervening update:

          * If the value in the ``UpdateHealthCheck`` request matches the value of ``HealthCheckVersion`` in
          the health check, Route 53 updates the health check with the new settings.

          * If the value of ``HealthCheckVersion`` in the health check is greater, the health check was
          changed after you got the version number. Route 53 does not update the health check, and it
        ...
        :param IPAddress:
          ...
          checks on. If you don't specify a value for ``IPAddress`` , Route 53 sends a DNS request to
          resolve the domain name that you specify in ``FullyQualifiedDomainName`` at the interval that you
          specify in ``RequestInterval`` . Using an IP address that is returned by DNS, Route 53 then checks
          the health of the endpoint.
          ...
          * **IPv6 address** : eight groups of four hexadecimal values, separated by colons (:), for
          ...
          If the endpoint is an EC2 instance, we recommend that you create an Elastic IP address, associate
          it with your EC2 instance, and specify the Elastic IP address for ``IPAddress`` . This ensures
          that the IP address of your instance never changes. For more information, see the applicable
          documentation:

          * Linux: `Elastic IP Addresses (EIP)
          <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/elastic-ip-addresses-eip.html>`__ in the
          *Amazon EC2 User Guide for Linux Instances*

          * Windows: `Elastic IP Addresses (EIP)
          <https://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/elastic-ip-addresses-eip.html>`__ in the
        ...
        :param ResourcePath:
          ...
          The path that you want Amazon Route 53 to request when performing health checks. The path can be
          any value for which your endpoint will return an HTTP status code of 2xx or 3xx when the endpoint
          is healthy, for example the file /docs/route53-health-check.html. You can also include query
        ...
        :param FullyQualifiedDomainName:
          ...
          * If you specify another value for ``Port`` and any value except ``TCP`` for ``Type`` , Route 53
          ...
          .. note::
            ...
            If you don't specify a value for ``IPAddress`` , Route 53 uses only IPv4 to send health checks
            to the endpoint. If there's no resource record set with a type of A for the name that you
            specify for ``FullyQualifiedDomainName`` , the health check fails with a "DNS resolution failed"
            error.

          If you want to check the health of weighted, latency, or failover resource record sets and you
          choose to specify the endpoint only by ``FullyQualifiedDomainName`` , we recommend that you create
          a separate health check for each endpoint. For example, create a health check for each HTTP server
          that is serving content for www.example.com. For the value of ``FullyQualifiedDomainName`` ,
          specify the domain name of the server (such as ``us-east-2-www.example.com`` ), not the name of
          ...
          .. warning::
            ...
            resource record sets and you then associate the health check with those resource record sets,
        ...
        :param SearchString:
          ...
          If the value of ``Type`` is ``HTTP_STR_MATCH`` or ``HTTP_STR_MATCH`` , the string that you want
          Amazon Route 53 to search for in the response body from the specified resource. If the string
          appears in the response body, Route 53 considers the resource healthy. (You can't change the value
          of ``Type`` when you update a health check.)
        ...
        :param Disabled:
          ...
          * **Health checks that monitor CloudWatch alarms:** Route 53 stops monitoring the corresponding
          ...
          resources. If you want to stop routing traffic to a resource, change the value of `Inverted
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_UpdateHealthCheck.html#Route53-UpdateHealthCheck-request-Inverted>`__
          .
        ...
        :param HealthThreshold:
          ...
          the ``ChildHealthChecks`` and ``ChildHealthCheck`` elements.
        ...
        :param ChildHealthChecks:
          ...
          A complex type that contains one ``ChildHealthCheck`` element for each health check that you want
          to associate with a ``CALCULATED`` health check.
        ...
        :param EnableSNI:
          ...
          A health check can also have that status for other reasons. If SNI is enabled and you're still
          getting the error, check the SSL/TLS configuration on your endpoint and confirm that your
        ...
        :param InsufficientDataHealthStatus:
          ...
          * ``LastKnownStatus`` : Route 53 uses the status of the health check from the last time CloudWatch
        ...
        :param ResetElements:
          ...
          A complex type that contains one ``ResettableElementName`` element for each element that you want
          to reset to the default value. Valid values for ``ResettableElementName`` include the following:

          * ``ChildHealthChecks`` : Amazon Route 53 resets `ChildHealthChecks
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_HealthCheckConfig.html#Route53-Type-HealthCheckConfig-ChildHealthChecks>`__
          to null.
          ...
          * ``Regions`` : Route 53 resets the `Regions
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_HealthCheckConfig.html#Route53-Type-HealthCheckConfig-Regions>`__
          list to the default set of regions.

          * ``ResourcePath`` : Route 53 resets `ResourcePath
          <https://docs.aws.amazon.com/Route53/latest/APIReference/API_HealthCheckConfig.html#Route53-Type-HealthCheckConfig-ResourcePath>`__
          to null.
    ...
    def update_hosted_zone_comment(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/UpdateHostedZoneComment>`_
        ...
        **Request Syntax**
        ...
        :param Comment:
          ...
          The new comment for the hosted zone. If you don't specify a value for ``Comment`` , Amazon Route
          53 deletes the existing value of the ``Comment`` element, if any.
    ...
    def update_traffic_policy_comment(
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/UpdateTrafficPolicyComment>`_
    ...
    def update_traffic_policy_instance(
        ...
        """
        Updates the resource record sets in a specified hosted zone that were created based on the settings
        in a specified traffic policy version.

        When you update a traffic policy instance, Amazon Route 53 continues to respond to DNS queries for
        the root resource record set name (such as example.com) while it replaces one group of resource
        record sets with another. Route 53 performs the following operations:

        * Route 53 creates a new group of resource record sets based on the specified traffic policy. This
        is true regardless of how significant the differences are between the existing resource record sets
        and the new resource record sets.

        * When all of the new resource record sets have been created, Route 53 starts to respond to DNS
        queries for the root resource record set name (such as example.com) by using the new resource record
        sets.

        * Route 53 deletes the old group of resource record sets that are associated with the root resource
        record set name.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/route53-2013-04-01/UpdateTrafficPolicyInstance>`_
        ...
        **Request Syntax**
        ...
        :param TrafficPolicyId: **[REQUIRED]**
          ...
          The ID of the traffic policy that you want Amazon Route 53 to use to update resource record sets
    ...
    ) -> paginator_scope.ListHealthChecksPaginator:
        """
        Get Paginator for `list_health_checks` operation.
        """
    ...
    ) -> paginator_scope.ListHostedZonesPaginator:
        """
        Get Paginator for `list_hosted_zones` operation.
        """
    ...
    ) -> paginator_scope.ListQueryLoggingConfigsPaginator:
        """
        Get Paginator for `list_query_logging_configs` operation.
        """
    ...
    ) -> paginator_scope.ListResourceRecordSetsPaginator:
        """
        Get Paginator for `list_resource_record_sets` operation.
        """
    ...
    ) -> paginator_scope.ListVPCAssociationAuthorizationsPaginator:
        """
        Get Paginator for `list_vpc_association_authorizations` operation.
        """
    ...
    ) -> waiter_scope.ResourceRecordSetsChangedWaiter:
        """
        Get Waiter `resource_record_sets_changed`.
        """
