...
class ModelFieldSequence(factory.LazyFunction):
    def __init__(self, model=None, max_length: int = None, string: str = None):
        assert not (
                model and string), 'You cannot specify both `model` and `string`, but only one of them'
...
MODEL_FIELD_TO_FACTORY_FIELD_MAPPER = {
    ...
    # Geometry
    PointField: lambda field: Point(10, 10)  # FIXME create something unique
}
    ...
    for field in model_cls._meta.fields:
        ...
        if field.__class__ in (ForeignKey, OneToOneField) and field.related_model == model_cls:
            if field.blank:
                ...
                raise NotImplementedError(
                    'We cannot handle ForeignKey or OneToOneField  to "self" which do not have "blank=True"')
            continue
