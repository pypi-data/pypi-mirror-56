...
defaultBasePath = os.path.realpath(
    os.path.normpath(
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../../nomad-meta-info")
        )
    )
)
...
def cascade(
    explodedDir, dictionaryDir, docsDir, args, deleteOldBk=False, continueOnError=True
):
    ...
    if explodedDir:
        for dFile in os.listdir(explodedDir):
            try:
                ...
                if dictionaryDir:
                    dPath = os.path.join(
                        dictionaryDir, d.metadict_name + ".meta_dictionary.json"
                    )
    ...
    if docsDir:
        ...
        for dName, d in sorted(mInfo.dictionaries.items()):
            try:
                ...
                logging.exception(
                    f"Failure when generating documentation for dictionary {dName}."
                )
        indexBody.append("</ul>\n")
        if siteWriter:
            ...
            if regenPaths:
                ...
                siteWriter.writeLayout(
                    indexPath,
                    body=indexBody,
                    basePath=os.path.basename(regenPaths[0]),
                    title="Schemas Index",
                )
            siteWriter.cleanupUnknown()
...
def cascadeCmd(args):
    if (
        not args.exploded_directory
        and not args.dict_directory
        and not args.docs_directory
    ):
        if not args.base_directory:
            args.base_directory = defaultBasePath
        args.exploded_directory = os.path.join(
            args.base_directory, "meta_dictionary_exploded"
        )
    ...
    cascade(
        args.exploded_directory,
        args.dict_directory,
        args.docs_directory,
        deleteOldBk=args.delete_old_bk,
    ...
    for inF in args.inPath:
        try:
            ...
            if outFormat == "exploded":
                ...
                writeFile(
                    os.path.join(target_dir, d.metadict_name + ".meta_dictionary.json"),
                    lambda outF: d.write(outF),
                )
        except:
...
def checkArgs(parser):
    ...
    parser.add_argument(
        "--name-check",
        choices=list(NameCheckLevel),
        help="Constraints on the meta_name.\n strict: only lowercase ascii alphanumeric and underscore ([a-z_][a-zA-Z_]*);\n normal: ascii alphanumeric or underscore;\n weak: unicode alphanumerics and underscore",
        default=NameCheckLevel.strict,
    )
    parser.add_argument(
        "--name-clashes",
        action="append",
        help="Kind of clashes between names that should be checked. It gives the things that should be ignored when comparing, several onew can be given, the default is to ignore all of the listed things (ignore-all) and thus just compare lowercase meta_name stripped of undercores",
        default=["ignore-case", "unique-section-attributes"],
        choices=[
            "ignore-case",
            "ignore-underscores",
            "unique-section-attributes",
...
if __name__ == "__main__":
    ...
    tmpfile = tempfile.NamedTemporaryFile(
        delete=False, prefix="meta-", suffix=".log", encoding="utf8", mode="w"
    )
    tmpfile.close()
    fh = logging.FileHandler(filename=tmpfile.name, mode="w", encoding="utf8")
    fh.setLevel(logging.DEBUG)
    df = logging.Formatter(
    ...
    parser.add_argument(
        "--verbose", help="increases the logging level", action="store_true"
    )
    subparsers = parser.add_subparsers(help="sub-command help")
    # create the parser for the "cascade" command
    parser_cascade = subparsers.add_parser(
        "cascade",
        help="reformat and propagate changes from exploded to single file dictionaries",
    )
    ...
    parser_cascade.add_argument(
        "--exploded-directory", type=str, help="path to the exploded directory"
    )
    parser_cascade.add_argument(
        "--dict-directory",
        type=str,
    ...
    parser_r = subparsers.add_parser(
        "rewrite", help="rewrites a dictionary possibly changing its format"
    )
    ...
    parser_r.add_argument(
        "inPath", type=str, nargs="+", help="an input path to convert"
    )
    parser_r.add_argument(
        "--out-format",
    ...
    parser_doc = subparsers.add_parser(
        "doc", help="writes the documentation on the given dictionary"
    )
