...
class BaseWindow(wx.ScrolledWindow):
  ...
  def OnChange2(self, ctrl, value, write_conf=False):
    ...
    if self.FormatOK(value, fmt4):
      ...
      if write_conf:	# write the new datum to conf; missing: list, dict
        if fmt4 in ("inte", "numb", "bool"):
          x = eval(value)
          setattr(conf, name, x)
        elif fmt4 in ('text', 'rfil'):
      ...
      if self.radio_name == Settings[1]:	# changed for current radio
        if name in ('hot_key_ptt_toggle', 'hot_key_ptt_if_hidden', 'keyupDelay'):
          ...
          value = (value == "True")
          application.Hardware.DisableSyncFreq(value)
...
class Radios(BaseWindow):	# The "Radios" first-level page
  def __init__(self, parent):
    ...
    item = self.AddPushButton(2, "Restart Quisk", self.OnBtnRestart, 1)
    self.NextRow()
    self.FitInside()
...
class RadioHardwareBase(BaseWindow):		# The Hardware page in the second-level notebook for each radio
  def __init__(self, parent, radio_name):
    ...
    #self.MarkCols()
    self.AlwaysMakeControls()		# Controls are always created for this tab. They may be replaced later.
