...
class JSONSchemaRelationTagsTC(CubicWebTC):
    ...
    def test__init(self):
        ...
        for tagged, expected_section in (
            ...
            (('Book', 'author', 'Author', 'subject'), 'inlined'),
            (('Book', 'in_library', 'Library', 'subject'), 'related'),
            # computed relation
    ...
    def test_relations_by_section_inlined(self):
        with self.admin_access.cnx() as cnx:
            entity = self.fake_entity(cnx, 'Book')
            rsection = self.rtag(cnx, entity)
            expected = ['title', 'publication_date', 'author']
            read_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'inlined', 'read')]
            self.assertCountEqual(read_rtypes, expected)
            add_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'inlined', 'add')]
            self.assertCountEqual(add_rtypes, expected)
    ...
    def test_relations_by_section_related(self):
        with self.admin_access.cnx() as cnx:
            ...
            rtypes = [rtype for rtype, _, _ in rsection.relations_by_section(
                entity, 'related', 'read')]
            self.assertCountEqual(['in_library', 'publications', 'topics'],
                                  rtypes)
    ...
    def test_relations_by_section_inlined_permissions(self):
        ...
        with self.new_access(u'bob').cnx() as cnx:
            ...
            read_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'inlined', 'read')]
            self.assertCountEqual(['title', 'publication_date', 'author'],
                                  read_rtypes)
            update_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'inlined', 'update')]
            self.assertCountEqual(['author'], update_rtypes)
    ...
    def test_relations_by_section_related_permissions(self):
        ...
        with self.new_access(u'bob').cnx() as cnx:
            ...
            update_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'related', 'add')]
            self.assertCountEqual(['topics', 'in_library'],
                                  update_rtypes)
            update_rtypes = [
                rtype for rtype, _, _ in rsection.relations_by_section(
                    entity, 'inlined', 'add')]
