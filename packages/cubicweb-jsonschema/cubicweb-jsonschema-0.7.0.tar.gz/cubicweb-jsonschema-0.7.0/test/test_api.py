    ...
    class ApiBaseTC(PyramidCWTest):
        ...
        def setup_database(self):
            with self.admin_access.cnx() as cnx:
                cnx.create_entity('Author', name=u'Ernest Hemingway')
...
def load_tests(loader, tests, ignore):
    ...
    def validate_schema(schema):
        ...
        with open(hyperschema) as f:
            try:
                ...
                warnings.warn(
                    'validation with python-jsonschema failed ({})'.format(
                        str(exc)),
                    RuntimeWarning,
                )
        if not has_ajv:
    ...
    def setUp(test):
        ...
        # Log-in as admin as all views are protected with the "authenticated"
        # permission.
        test.globs.pop('login')()
    ...
    def docfilesuite(fpath):
        return doctest.DocFileSuite(
            ...
            checker=testing.WebTestReponseOutputChecker(),
            optionflags=(
                doctest.REPORT_ONLY_FIRST_FAILURE
                | testing.WEBTEST_RESPONSE
            ),
        )

    pattern = os.environ.get('DOCTESTS_PATTERN', '*') + '.rst'
    for fpath in glob(path.join(here, '*', pattern)):
        tests.addTests(docfilesuite(fpath))
    if pattern in ('*.rst', 'hypermedia-walkthrough.rst'):
        tests.addTests(
