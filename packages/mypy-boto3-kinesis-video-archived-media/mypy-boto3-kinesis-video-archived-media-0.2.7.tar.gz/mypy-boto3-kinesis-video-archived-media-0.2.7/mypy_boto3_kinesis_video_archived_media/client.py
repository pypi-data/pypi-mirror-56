...
class Client(BaseClient):
    ...
    def get_dash_streaming_session_url(
        ...
        * Retrieve the MPEG-DASH URL using ``GetDASHStreamingSessionURL`` . Kinesis Video Streams creates an
        MPEG-DASH streaming session to be used for accessing content in a stream using the MPEG-DASH
        ...
        * Provide the URL (containing the encrypted session token) for the MPEG-DASH manifest to a media
        player that supports the MPEG-DASH protocol. Kinesis Video Streams makes the initialization fragment
        and media fragments available through the manifest URL. The initialization fragment contains the
        codec private data for the stream, and other data needed to set up the video or audio decoder and
        renderer. The media fragments contain encoded video frames or encoded audio samples.
        ...
        * The media player receives the authenticated URL and requests stream metadata and media data
        normally. When the media player requests data, it calls the following actions:
          ...
          * **GetMP4InitFragment:** Retrieves the MP4 initialization fragment. The media player typically
          ...
          "``fytp`` " and "``moov`` " MP4 atoms, and the child atoms that are needed to initialize the media
          player decoder. The initialization fragment does not correspond to a fragment in a Kinesis video
          stream. It contains only the codec private data for the stream and respective track, which the
          media player needs to decode the media frames.
        ...
        For more information about HLS, see `HTTP Live Streaming <https://developer.apple.com/streaming/>`__
        on the `Apple Developer site <https://developer.apple.com>`__ .
        ...
        **Request Syntax**
        ...
        :param PlaybackMode:
          ...
          * **``LIVE`` ** : For sessions of this type, the MPEG-DASH manifest is continually updated with
          ...
          interface typically displays a "live" notification, with no scrubber control for choosing the
          ...
          .. note::
             ...
             there is a gap between fragments (that is, if a fragment is missing). A gap like this might
             cause a media player to halt or cause a jump in playback. In this mode, fragments are not added
             to the MPEG-DASH manifest if they are older than the newest fragment in the playlist. If the
             missing fragment becomes available after a subsequent fragment is added to the manifest, the
             older fragment is not added, and the gap is not filled.

          * **``LIVE_REPLAY`` ** : For sessions of this type, the MPEG-DASH manifest is updated similarly to
          how it is updated for ``LIVE`` mode except that it starts by including fragments from a given
          ...
          to be able to start playback from when an event is detected and continue live streaming media that
          has not yet been ingested as of the time of the session creation. This mode is also useful to
          ...
          * **``ON_DEMAND`` ** : For sessions of this type, the MPEG-DASH manifest contains all the
          ...
          multiple fragments with the same start timestamp, the fragment that has the larger fragment number
          (that is, the newer fragment) is included in the MPEG-DASH manifest. The other fragments are not
          included. Fragments that have different timestamps but have overlapping durations are still
        ...
        :param DisplayFragmentTimestamp:
          ...
          wall-clock time derived from the manifest file may be inaccurate. If DisplayFragmentTimestamp is
          set to ``ALWAYS`` , the accurate fragment timestamp is added to each S element in the manifest
        ...
        :param DisplayFragmentNumber:
          ...
          each S element in the manifest file with the attribute name “kvs:fn”. These fragment numbers can
          be used for logging or for use with other APIs (e.g. ``GetMedia`` and ``GetMediaForFragmentList``
        ...
        :param DASHFragmentSelector:
          ...
          - **FragmentSelectorType** *(string) --*
            ...
            Fragments that have duplicate producer timestamps are deduplicated. This means that if producers
            are producing a stream of fragments with producer timestamps that are approximately equal to the
            true clock time, the MPEG-DASH manifest will contain all of the fragments within the requested
            timestamp range. If some fragments are ingested within the same time range and very different
        ...
        :param MaxManifestFragmentResults:
          ...
          When the ``PlaybackMode`` is ``ON_DEMAND`` , the oldest fragments are returned, up to this maximum
          number.
    ...
    def get_hls_streaming_session_url(
        ...
        * The audio track (if present) of each fragment must contain codec private data in the AAC format
        ...
        Kinesis Video Streams HLS sessions contain fragments in the fragmented MPEG-4 form (also called fMP4
        or CMAF) or the MPEG-2 form (also called TS chunks, which the HLS specification also supports). For
        ...
        * Provide the URL (containing the encrypted session token) for the HLS master playlist to a media
        ...
        initialization fragment contains the codec private data for the stream, and other data needed to set
        up the video or audio decoder and renderer. The media fragments contain H.264-encoded video frames
        or AAC-encoded audio samples.
        ...
        * The media player receives the authenticated URL and requests stream metadata and media data
        normally. When the media player requests data, it calls the following actions:
          ...
          * **GetHLSMediaPlaylist:** Retrieves an HLS media playlist, which contains a URL to access the MP4
          ...
          fragments with the ``GetMP4MediaFragment`` actions. The HLS media playlist also contains metadata
          about the stream that the player needs to play it, such as whether the ``PlaybackMode`` is
          ``LIVE`` or ``ON_DEMAND`` . The HLS media playlist is typically static for sessions with a
          ...
          * **GetTSFragment:** Retrieves MPEG TS fragments containing both initialization and media data for
        ...
        .. note::
          ...
          * A Kinesis video stream can have a maximum of ten active HLS streaming sessions. If a new session
          is created when the maximum number of sessions is already active, the oldest (earliest created)
          session is closed. The number of active ``GetMedia`` connections on a Kinesis video stream does
          not count against this limit, and the number of active HLS sessions does not count against the
        ...
        **Request Syntax**
        ...
        :param PlaybackMode:
          ...
          * **``LIVE`` ** : For sessions of this type, the HLS media playlist is continually updated with
          ...
          .. note::
             ...
             In ``LIVE`` mode, the newest available fragments are included in an HLS media playlist, even if
             there is a gap between fragments (that is, if a fragment is missing). A gap like this might
             cause a media player to halt or cause a jump in playback. In this mode, fragments are not added
             to the HLS media playlist if they are older than the newest fragment in the playlist. If the
             missing fragment becomes available after a subsequent fragment is added to the playlist, the
             older fragment is not added, and the gap is not filled.

          * **``LIVE_REPLAY`` ** : For sessions of this type, the HLS media playlist is updated similarly to
          how it is updated for ``LIVE`` mode except that it starts by including fragments from a given
          start time. Instead of fragments being added as they are ingested, fragments are added as the
          duration of the next fragment elapses. For example, if the fragments in the session are two
          ...
          * **``ON_DEMAND`` ** : For sessions of this type, the HLS media playlist contains all the
          ...
          In all playback modes, if ``FragmentSelectorType`` is ``PRODUCER_TIMESTAMP`` , and if there are
          multiple fragments with the same start timestamp, the fragment that has the larger fragment number
          (that is, the newer fragment) is included in the HLS media playlist. The other fragments are not
          included. Fragments that have different timestamps but have overlapping durations are still
        ...
        :param HLSFragmentSelector:
          ...
          - **FragmentSelectorType** *(string) --*
            ...
            Fragments that have duplicate producer timestamps are deduplicated. This means that if producers
            are producing a stream of fragments with producer timestamps that are approximately equal to the
            true clock time, the HLS media playlists will contain all of the fragments within the requested
            timestamp range. If some fragments are ingested within the same time range and very different
            points in time, only the oldest ingested collection of fragments are returned.
        ...
        :param ContainerFormat:
          ...
          supported packaging on older HLS players. MPEG TS typically has a 5-25 percent packaging overhead.
          This means MPEG TS typically requires 5-25 percent more bandwidth and cost than fMP4.
        ...
        :param DiscontinuityMode:
          ...
          * ``ON_DISCONTIUNITY`` : a discontinuity marker is placed between fragments that have a gap or
          ...
          The default is ``ALWAYS`` when  HLSFragmentSelector is set to ``SERVER_TIMESTAMP`` , and ``NEVER``
        ...
        :param DisplayFragmentTimestamp:
          ...
          The default is ``NEVER`` . When  HLSFragmentSelector is ``SERVER_TIMESTAMP`` , the timestamps will
          be the server start timestamps. Similarly, when  HLSFragmentSelector is ``PRODUCER_TIMESTAMP`` ,
