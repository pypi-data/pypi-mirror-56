...
class Client(BaseClient):
    ...
    def get_dash_streaming_session_url(
        ...
        then open the URL in a media player to view the stream contents.
        ...
        * The video track of each fragment must contain codec private data in the Advanced Video
        Coding (AVC) for H.264 format and HEVC for H.265 format. For more information, see `MPEG-4
        specification ISO/IEC 14496-15 <https://www.iso.org/standard/55980.html>`__ . For
        ...
        * The audio track (if present) of each fragment must contain codec private data in the AAC
        format (`AAC specification ISO/IEC 13818-7 <https://www.iso.org/standard/43345.html>`__ ) or
        the `MS Wave format <http://www-mmsp.ece.mcgill.ca/Documents/AudioFormats/WAVE/WAVE.html>`__
        .

        The following procedure shows how to use MPEG-DASH with Kinesis Video Streams:

        * Get an endpoint using `GetDataEndpoint
        <http://docs.aws.amazon.com/kinesisvideostreams/latest/dg/API_GetDataEndpoint.html>`__ ,
        specifying ``GET_DASH_STREAMING_SESSION_URL`` for the ``APIName`` parameter.

        * Retrieve the MPEG-DASH URL using ``GetDASHStreamingSessionURL`` . Kinesis Video Streams
        ...
        .. note::
           ...
           that you would use with your AWS credentials.
        ...
        * Provide the URL (containing the encrypted session token) for the MPEG-DASH manifest to a
        media player that supports the MPEG-DASH protocol. Kinesis Video Streams makes the
        ...
        * The media player receives the authenticated URL and requests stream metadata and media
        data normally. When the media player requests data, it calls the following actions:
          ...
          * **GetDASHManifest:** Retrieves an MPEG DASH manifest, which contains the metadata for
          the media that you want to playback.
          ...
          * **GetMP4InitFragment:** Retrieves the MP4 initialization fragment. The media player
          ...
          needed to initialize the media player decoder. The initialization fragment does not
          correspond to a fragment in a Kinesis video stream. It contains only the codec private
          data for the stream and respective track, which the media player needs to decode the media
          frames.
        ...
        .. note::
          ...
          * A streaming session URL should not be shared between players. The service might throttle
          ...
          <http://docs.aws.amazon.com/kinesisvideostreams/latest/dg/limits.html>`__ .

          * A Kinesis video stream can have a maximum of ten active MPEG-DASH streaming sessions. If
          a new session is created when the maximum number of sessions is already active, the oldest
          ...
          .. note::
             ...
             The maximum limits for active HLS and MPEG-DASH streaming sessions are independent of
             each other.
        ...
        <https://aws.amazon.com/pricing/>`__ . Charges for both HLS sessions and outgoing AWS data
        apply.
        ...
        .. warning::
          ...
          addition to the HTTP status code and the response body, it includes the following pieces
          of information:
          ...
          * ``x-amz-RequestId`` HTTP header – if you want to report an issue to AWS, the support
          ...
          For more information, see the **Errors** section at the bottom of this topic, as well as
          `Common Errors
          <https://docs.aws.amazon.com/kinesisvideostreams/latest/dg/CommonErrors.html>`__ .
        ...
        **Request Syntax**
        ...
        :param PlaybackMode:
          ...
          * **``LIVE`` ** : For sessions of this type, the MPEG-DASH manifest is continually updated
          with the latest fragments as they become available. We recommend that the media player
          retrieve a new manifest on a one-second interval. When this type of session is played in a
          media player, the user interface typically displays a "live" notification, with no
          ...
          .. note::
             ...
             like this might cause a media player to halt or cause a jump in playback. In this mode,
          ...
          * **``ON_DEMAND`` ** : For sessions of this type, the MPEG-DASH manifest contains all the
          ...
          ``MaxMediaPlaylistFragmentResults`` . The manifest must be retrieved only once for each
          ...
          there are multiple fragments with the same start timestamp, the fragment that has the
          larger fragment number (that is, the newer fragment) is included in the MPEG-DASH
          manifest. The other fragments are not included. Fragments that have different timestamps
          but have overlapping durations are still included in the MPEG-DASH manifest. This can lead
        ...
        :param DisplayFragmentTimestamp:
          ...
          Per the MPEG-DASH specification, the wall-clock time of fragments in the manifest file can
          be derived using attributes in the manifest itself. However, typically, MPEG-DASH
          compatible media players do not properly handle gaps in the media timeline. Kinesis Video
          Streams adjusts the media timeline in the manifest file to enable playback of media with
          discontinuities. Therefore, the wall-clock time derived from the manifest file may be
          inaccurate. If DisplayFragmentTimestamp is set to ``ALWAYS`` , the accurate fragment
          timestamp is added to each S element in the manifest file with the attribute name
        ...
        :param DisplayFragmentNumber:
          ...
          session. If DisplayFragmentNumber is set to ``ALWAYS`` , the Kinesis Video Streams
          fragment number is added to each S element in the manifest file with the attribute name
          “kvs:fn”. These fragment numbers can be used for logging or for use with other APIs (e.g.
          ``GetMedia`` and ``GetMediaForFragmentList`` ). A custom MPEG-DASH media player is
          necessary to leverage these this custom attribute.
        ...
        :param DASHFragmentSelector:
          ...
          - **FragmentSelectorType** *(string) --*
            ...
            FragmentSelector$TimestampRange is included in the media playlist. In addition, the
            fragments with producer timestamps within the ``TimestampRange`` ingested immediately
            ...
            fragments based on server timestamps are included in the MPEG-DASH manifest. This means
            that even if fragments ingested in the past have producer timestamps with values now,
          ...
          - **TimestampRange** *(dict) --*
            ...
            - **EndTimestamp** *(datetime) --*
              ...
              ``LIVE_REPLAY`` mode. If the ``EndTimestamp`` is not set for ``LIVE_REPLAY`` mode then
              the session will continue to include newly ingested fragments until the session
              expires.
    ...
    def get_hls_streaming_session_url(
        ...
        browser or media player to view the stream contents.
        ...
        * The video track of each fragment must contain codec private data in the Advanced Video
        Coding (AVC) for H.264 format or HEVC for H.265 format (`MPEG-4 specification ISO/IEC
        14496-15 <https://www.iso.org/standard/55980.html>`__ ). For information about adapting
        stream data to a given format, see `NAL Adaptation Flags
        <http://docs.aws.amazon.com/kinesisvideostreams/latest/dg/producer-reference-nal.html>`__ .

        * The audio track (if present) of each fragment must contain codec private data in the AAC
        format (`AAC specification ISO/IEC 13818-7 <https://www.iso.org/standard/43345.html>`__ ).
        ...
        also supports). For more information about HLS fragment types, see the `HLS specification
        ...
        * Retrieve the HLS URL using ``GetHLSStreamingSessionURL`` . Kinesis Video Streams creates
        ...
        * Provide the URL (containing the encrypted session token) for the HLS master playlist to a
        ...
        playlist, initialization fragment, and media fragments available through the master playlist
        URL. The initialization fragment contains the codec private data for the stream, and other
        data needed to set up the video or audio decoder and renderer. The media fragments contain
        ...
        * The media player receives the authenticated URL and requests stream metadata and media
        data normally. When the media player requests data, it calls the following actions:
          ...
          * **GetHLSMediaPlaylist:** Retrieves an HLS media playlist, which contains a URL to access
          ...
          the ``PlaybackMode`` is ``LIVE`` or ``ON_DEMAND`` . The HLS media playlist is typically
          static for sessions with a ``PlaybackType`` of ``ON_DEMAND`` . The HLS media playlist is
          continually updated with new fragments for sessions with a ``PlaybackType`` of ``LIVE`` .
          There is a distinct HLS media playlist for the video track and the audio track (if
          ...
          * **GetTSFragment:** Retrieves MPEG TS fragments containing both initialization and media
          ...
          .. note::
           ...
           Data retrieved with this action is billable. For more information, see `Kinesis Video
           Streams pricing <https://aws.amazon.com/kinesis/video-streams/pricing/>`__ .
        ...
        .. note::
          ...
          * A Kinesis video stream can have a maximum of ten active HLS streaming sessions. If a new
          ...
          (earliest created) session is closed. The number of active ``GetMedia`` connections on a
          Kinesis video stream does not count against this limit, and the number of active HLS
          sessions does not count against the active ``GetMedia`` connection limit.
        ...
        **Request Syntax**
        ...
        :param StreamARN:
          ...
          The Amazon Resource Name (ARN) of the stream for which to retrieve the HLS master playlist
          URL.
        ...
        :param PlaybackMode:
          ...
          * **``LIVE`` ** : For sessions of this type, the HLS media playlist is continually updated
          with the latest fragments as they become available. We recommend that the media player
          retrieve a new playlist on a one-second interval. When this type of session is played in a
          media player, the user interface typically displays a "live" notification, with no
          scrubber control for choosing the position in the playback window to display.
          ...
          * **``ON_DEMAND`` ** : For sessions of this type, the HLS media playlist contains all the
          ...
          In all playback modes, if ``FragmentSelectorType`` is ``PRODUCER_TIMESTAMP`` , and if
          there are multiple fragments with the same start timestamp, the fragment that has the
          larger fragment number (that is, the newer fragment) is included in the HLS media
          playlist. The other fragments are not included. Fragments that have different timestamps
          but have overlapping durations are still included in the HLS media playlist. This can lead
          to unexpected behavior in the media player.
        ...
        :param HLSFragmentSelector:
          ...
          - **FragmentSelectorType** *(string) --*
            ...
            When ``FragmentSelectorType`` is set to ``PRODUCER_TIMESTAMP`` and
            GetHLSStreamingSessionURLInput$PlaybackMode is ``ON_DEMAND`` or ``LIVE_REPLAY`` , the
            first fragment ingested with a producer timestamp within the specified
            FragmentSelector$TimestampRange is included in the media playlist. In addition, the
            fragments with producer timestamps within the ``TimestampRange`` ingested immediately
            following the first fragment (up to the
            ...
            used in the MP4 fragments and for deduplication. But the most recently ingested
            fragments based on server timestamps are included in the HLS media playlist. This means
            that even if fragments ingested in the past have producer timestamps with values now,
            they are not included in the HLS media playlist.
        ...
        :param ContainerFormat:
          ...
          ``FRAGMENTED_MP4`` container format packages the media into MP4 fragments (fMP4 or CMAF).
          This is the recommended packaging because there is minimal packaging overhead. The other
          container format option is ``MPEG_TS`` . HLS has supported MPEG TS chunks since it was
          released and is sometimes the only supported packaging on older HLS players. MPEG TS
          typically has a 5-25 percent packaging overhead. This means MPEG TS typically requires
        ...
        :param DiscontinuityMode:
          ...
          typical if  HLSFragmentSelector is set to ``SERVER_TIMESTAMP`` ), the media player
        ...
        :param DisplayFragmentTimestamp:
          ...
          Typically, media players report the playhead position as a time relative to the start of
          the first fragment in the playback session. However, when the start timestamps are
          included in the HLS media playlist, some media players might report the current playhead
          as an absolute time based on the fragment timestamps. This can be useful for creating a
          playback experience that shows viewers the wall-clock time of the media.

          The default is ``NEVER`` . When  HLSFragmentSelector is ``SERVER_TIMESTAMP`` , the
    ...
    def get_media_for_fragment_list(
        ...
        **Request Syntax**
        ...
        :param Fragments: **[REQUIRED]**
          ...
          A list of the numbers of fragments for which to retrieve media. You retrieve these values
          with  ListFragments .
    ...
    def list_fragments(
        ...
        from a request to ``ListFragments`` . However, results are typically available in less than
        one second.
        ...
        **Request Syntax**
        ...
        :param MaxResults:
          ...
          more than the value specified in ``max-results`` , then a  ListFragmentsOutput$NextToken
