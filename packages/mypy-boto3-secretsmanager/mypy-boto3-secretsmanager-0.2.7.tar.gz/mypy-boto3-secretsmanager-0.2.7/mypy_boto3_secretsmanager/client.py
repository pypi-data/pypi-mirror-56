...
class Client(BaseClient):
    ...
    def cancel_rotate_secret(
        ...
        value greater than 0. This will immediately rotate your secret and then enable the automatic
        schedule.

        .. note::

          If you cancel a rotation that is in progress, it can leave the ``VersionStage`` labels in an
          unexpected state. Depending on what step of the rotation was in progress, you might need to remove
          the staging label ``AWSPENDING`` from the partially created version, specified by the
          ``VersionId`` response value. You should also evaluate the partially rotated new version to see if
          it should be deleted, which you can do by removing all staging labels from the new version's
          ``VersionStage`` field.
        ...
        * Attached to the same version as the staging label ``AWSCURRENT``
        ...
        If the staging label ``AWSPENDING`` is attached to a different version than the version with
        ...
        * To list all of the versions currently associated with a secret, use  ListSecretVersionIds .
        ...
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/CancelRotateSecret>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret for which you want to cancel a rotation request. You can specify either the
    ...
    def create_secret(
        ...
        Tags: List[ClientCreateSecretTagsTypeDef] = None,
    ) -> ClientCreateSecretResponseTypeDef:
        """
        ...
        Secrets Manager stores the encrypted secret data in one of a collection of "versions" associated
        with the secret. Each version contains a copy of the encrypted secret data. Each version is
        associated with one or more "staging labels" that identify where the version is in the rotation
        cycle. The ``SecretVersionsToStages`` field of the secret contains the mapping of staging labels to
        the active versions of the secret. Versions without a staging label are considered deprecated and
        are not included in the list.

        You provide the secret data to be encrypted by putting text in either the ``SecretString`` parameter
        or binary data in the ``SecretBinary`` parameter, but not both. If you include ``SecretString`` or
        ``SecretBinary`` then Secrets Manager also creates an initial secret version and automatically
        ...
        .. note::
          ...
          * If you call an operation that needs to encrypt or decrypt the ``SecretString`` or
          ...
          specify a AWS KMS encryption key, Secrets Manager uses the account's default AWS managed customer
          master key (CMK) with the alias ``aws/secretsmanager`` . If this key doesn't already exist in your
          account then Secrets Manager creates it for you automatically. All users and roles in the same AWS
          account automatically have access to use the default CMK. Note that if an Secrets Manager API call
          results in AWS having to create the account's AWS-managed CMK, it can result in a one-time
          ...
          * If the secret is in a different AWS account from the credentials calling an API that requires
          encryption or decryption of the secret value then you must create and use a custom AWS KMS CMK
          because you can't access the default CMK for the account using credentials from a different AWS
          account. Store the ARN of the CMK in the secret when you create the secret or when you update it
          by including it in the ``KMSKeyId`` . If you call an API that must encrypt or decrypt
          ``SecretString`` or ``SecretBinary`` using credentials from a different account then the AWS KMS
          key policy must grant cross-account access to that other account's user or role for both the
        ...
        * kms:GenerateDataKey - needed only if you use a customer-managed AWS KMS key to encrypt the secret.
        You do not need this permission to use the account's default AWS managed CMK for Secrets Manager.

        * kms:Decrypt - needed only if you use a customer-managed AWS KMS key to encrypt the secret. You do
        not need this permission to use the account's default AWS managed CMK for Secrets Manager.
        ...
        * To retrieve all other details for a secret, use  DescribeSecret . This does not include the
        encrypted secure string and secure binary values.

        * To retrieve the list of secret versions associated with the current secret, use  DescribeSecret
        and examine the ``SecretVersionsToStages`` response value.

        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/CreateSecret>`_
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          .. note::
            ...
            Don't end your secret name with a hyphen followed by six characters. If you do so, you risk
            confusion and unexpected results when searching for a secret by partial ARN. This is because
            Secrets Manager automatically adds a hyphen and six random characters at the end of the ARN.

        :type ClientRequestToken: string
        :param ClientRequestToken:

          (Optional) If you include ``SecretString`` or ``SecretBinary`` , then an initial version is
          created as part of the secret, and this parameter specifies a unique identifier for the new
          version.

          .. note::

            If you use the AWS CLI or one of the AWS SDK to call this operation, then you can leave this
            parameter empty. The CLI or SDK generates a random UUID for you and includes it as the value for
            this parameter in the request. If you don't use the SDK and instead generate a raw HTTP request
          ...
          that you generate a `UUID-type <https://wikipedia.org/wiki/Universally_unique_identifier>`__ value
        ...
        :param KmsKeyId:
          ...
          (Optional) Specifies the ARN, Key ID, or alias of the AWS KMS customer master key (CMK) to be used
          to encrypt the ``SecretString`` or ``SecretBinary`` values in the versions stored in this secret.

          You can specify any of the supported ways to identify a AWS KMS key ID. If you need to reference a
          CMK in a different account, you can use only the key ARN or the alias ARN.

          If you don't specify this value, then Secrets Manager defaults to using the AWS account's default
          CMK (the one named ``aws/secretsmanager`` ). If a AWS KMS CMK with that name doesn't yet exist,
          then Secrets Manager creates it for you automatically the first time it needs to encrypt a
          version's ``SecretString`` or ``SecretBinary`` fields.

          .. warning::

            You can use the account's default CMK to encrypt and decrypt only if you call this operation
            using credentials from the same account that owns the secret. If the secret is in a different
            account, then you must create a custom CMK and specify the ARN in this field.

        :type SecretBinary: bytes
        :param SecretBinary:

          (Optional) Specifies binary data that you want to encrypt and store in the new version of the
          ...
          Either ``SecretString`` or ``SecretBinary`` must have a value, but not both. They cannot both be
          empty.
        ...
        :param SecretString:
          ...
          the information as a JSON structure of key/value pairs that the Lambda rotation function knows how
          to parse.

          For storing multiple values, we recommend that you use a JSON text string argument and specify
          key/value pairs. For information on how to format a JSON parameter for the various command line
          tool environments, see `Using JSON for Parameters
          <https://docs.aws.amazon.com/cli/latest/userguide/cli-using-param.html#cli-using-param-json>`__ in
        ...
        :param Tags:
          ...
          (Optional) Specifies a list of user-defined tags that are attached to the secret. Each tag is a
          "Key" and "Value" pair of strings. This operation only appends tags to the existing list of tags.
          ...
          .. warning::
            ...
            * If you check tags in IAM policy ``Condition`` elements as part of your security strategy, then
            adding or removing a tag can change permissions. If the successful completion of this operation
            would result in you losing your permissions for this secret, then this operation is blocked and
            returns an ``Access Denied`` error.

          This parameter requires a JSON text string argument. For information on how to format a JSON
          parameter for the various command line tool environments, see `Using JSON for Parameters
          <https://docs.aws.amazon.com/cli/latest/userguide/cli-using-param.html#cli-using-param-json>`__ in
          the *AWS CLI User Guide* . For example:
    ...
    def delete_resource_policy(
        ...
        * To list all of the currently available secrets, use  ListSecrets .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/DeleteResourcePolicy>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to delete the attached resource-based policy for. You can
    ...
    def delete_secret(
        ...
        """
        Deletes an entire secret and all of its versions. You can optionally include a recovery window
        during which you can restore the secret. If you don't specify a recovery window value, the operation
        defaults to 30 days. Secrets Manager attaches a ``DeletionDate`` stamp to the secret that specifies
        the end of the recovery window. At the end of the recovery window, Secrets Manager deletes the
        secret permanently.

        At any time before recovery window ends, you can use  RestoreSecret to remove the ``DeletionDate``
        and cancel the deletion of the secret.

        You cannot access the encrypted secret information in any secret that is scheduled for deletion. If
        you need to access that information, you must cancel the deletion with  RestoreSecret and then
        retrieve the information.

        .. note::

          * There is no explicit operation to delete a version of a secret. Instead, remove all staging
          labels from the ``VersionStage`` field of a version. That marks the version as deprecated and
          allows Secrets Manager to delete it as needed. Versions that do not have any staging labels do not
          show up in  ListSecretVersionIds unless you specify ``IncludeDeprecated`` .

          * The permanent secret deletion at the end of the waiting period is performed as a background task
          with low priority. There is no guarantee of a specific time after the recovery window for the
        ...
        * To cancel deletion of a version of a secret before the recovery window has expired, use
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/DeleteSecret>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to delete. You can specify either the Amazon Resource Name
        ...
        :param ForceDeleteWithoutRecovery:
          ...
          (Optional) Specifies that the secret is to be deleted without any recovery window. You can't use
          both this parameter and the ``RecoveryWindowInDays`` parameter in the same API call.

          An asynchronous background process performs the actual deletion, so there can be a short delay
          before the operation completes. If you write code to delete and then immediately recreate a secret
          with the same name, ensure that your code includes appropriate back off and retry logic.

          .. warning::

            Use this parameter with caution. This parameter causes the operation to skip the normal waiting
            period before the permanent deletion that AWS would normally impose with the
            ``RecoveryWindowInDays`` parameter. If you delete a secret with the
            ``ForceDeleteWithouRecovery`` parameter, then you have no opportunity to recover the secret. It
            is permanently lost.
    ...
    def describe_secret(self, SecretId: str) -> ClientDescribeSecretResponseTypeDef:
        ...
        are populated with a value are returned in the response.
        ...
        * To list all of the secrets in the AWS account, use  ListSecrets .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/DescribeSecret>`_
    ...
    def get_random_password(
        ...
        """
        Generates a random password of the specified complexity. This operation is intended for use in the
        Lambda rotation function. Per best practice, we recommend that you specify the maximum length and
        include every character type that the system you are generating a password for can support.
        ...
        * secretsmanager:GetRandomPassword
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/GetRandomPassword>`_
        ...
        **Request Syntax**
        ...
        :param ExcludeCharacters:
          ...
          A string that includes characters that should not be included in the generated password. The
        ...
        :param ExcludePunctuation:
          ...
          Specifies that the generated password should not include punctuation characters. The default if
          you do not include this switch parameter is that punctuation characters can be included.
        ...
        :param ExcludeUppercase:
          ...
          not include this switch parameter is that uppercase letters can be included.

        :type ExcludeLowercase: boolean
        :param ExcludeLowercase:

          Specifies that the generated password should not include lowercase letters. The default if you do
          not include this switch parameter is that lowercase letters can be included.
        ...
        :param RequireEachIncludedType:
          ...
          allowed character type. The default value is ``True`` and the operation requires at least one of
    ...
    def get_resource_policy(
        ...
        """
        Retrieves the JSON text of the resource-based policy document that's attached to the specified
        secret. The JSON request string input and response output are shown formatted with white space and
        ...
        * To list all of the currently available secrets, use  ListSecrets .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/GetResourcePolicy>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to retrieve the attached resource-based policy for. You can
          specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
    ...
    def get_secret_value(
        ...
        * kms:Decrypt - required only if you use a customer-managed AWS KMS key to encrypt the secret. You
        do not need this permission to use the account's default AWS managed CMK for Secrets Manager.
        ...
        * To retrieve the non-encrypted details for the secret, use  DescribeSecret .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/GetSecretValue>`_
        ...
        **Request Syntax**
        ...
        :param VersionId:
          ...
          This value is typically a `UUID-type <https://wikipedia.org/wiki/Universally_unique_identifier>`__
        ...
        :param VersionStage:
          ...
          use this parameter then don't specify ``VersionId`` . If you don't specify either a
          ``VersionStage`` or ``VersionId`` , then the default is to perform the operation on the version
    ...
    def list_secret_version_ids(
        ...
        at least one staging label in ``VersionStage`` attached.

        .. note::

          Always check the ``NextToken`` response parameter when calling any of the ``List*`` operations.
          These operations can occasionally return an empty or shorter than expected list of results even
          when there are more results available. When this happens, the ``NextToken`` response parameter
          contains a value to pass to the next call to the same API to request the next part of the list.
        ...
        * To list the secrets in an account, use  ListSecrets .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/ListSecretVersionIds>`_
        ...
        **Request Syntax**
        ...
        :param MaxResults:
          ...
          include this parameter, it defaults to a value that's specific to the operation. If additional
          items exist beyond the maximum you specify, the ``NextToken`` response element is present and has
          a value (isn't null). Include that value as the ``NextToken`` request parameter in the next call
          to the operation to get the next part of the results. Note that Secrets Manager might return fewer
        ...
        :param IncludeDeprecated:
          ...
          (Optional) Specifies that you want the results to include versions that do not have any staging
          labels attached to them. Such versions are considered deprecated and are subject to deletion by
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextToken** *(string) --*
              ...
              values at all, such as when you ask for a filtered view of a very long list. Use this value in
              the ``NextToken`` request parameter in a subsequent call to the operation to continue
              processing and get the next part of the output. You should repeat this until the ``NextToken``
    ...
    def list_secrets(
        ...
        """
        Lists all of the secrets that are stored by Secrets Manager in the AWS account. To list the versions
        currently stored for a specific secret, use  ListSecretVersionIds . The encrypted fields
        ``SecretString`` and ``SecretBinary`` are not included in the output. To get that information, call
        ...
        * To list the versions attached to a secret, use  ListSecretVersionIds .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/ListSecrets>`_
            ...
            - **SecretList** *(list) --*
              ...
              - *(dict) --*
                ...
                - **SecretVersionsToStages** *(dict) --*
                  ...
                  ``SecretVersionId`` that each is attached to. Staging labels are used to keep track of the
    ...
    def put_resource_policy(
        ...
        resource-based policy is optional. Alternatively, you can use IAM identity-based policies that
        specify the secret's Amazon Resource Name (ARN) in the policy statement's ``Resources`` element. You
        can also use a combination of both identity-based and resource-based policies. The affected users
        and roles receive the permissions that are permitted by all of the relevant policies. For more
        information, see `Using Resource-Based Policies for AWS Secrets Manager
        <http://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access_resource-based-policies.html>`__
        . For the complete description of the AWS policy syntax and grammar, see `IAM JSON Policy Reference
        <https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies.html>`__ in the *IAM User
        Guide* .
        ...
        * To list all of the currently available secrets, use  ListSecrets .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/PutResourcePolicy>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to attach the resource-based policy to. You can specify either
        ...
        :param ResourcePolicy: **[REQUIRED]**
          ...
          resource-based policy. The policy in the string identifies who can access or manage this secret
          and its versions. For information on how to format a JSON parameter for the various command line
          tool environments, see `Using JSON for Parameters
          <http://docs.aws.amazon.com/cli/latest/userguide/cli-using-param.html#cli-using-param-json>`__ in
    ...
    def put_secret_value(
        ...
        """
        Stores a new encrypted secret value in the specified secret. To do this, the operation creates a new
        version and attaches it to the secret. The version can contain a new ``SecretString`` value or a new
        ``SecretBinary`` value. You can also specify the staging labels that are initially attached to the
        new version.

        .. note::

          The Secrets Manager console uses only the ``SecretString`` field. To add binary data to a secret
          with the ``SecretBinary`` field you must use the AWS CLI or one of the AWS SDKs.

        * If this operation creates the first version for the secret then Secrets Manager automatically
        attaches the staging label ``AWSCURRENT`` to the new version.

        * If another version of this secret already exists, then this operation does not automatically move
        any staging labels other than those that you explicitly specify in the ``VersionStages`` parameter.

        * If this operation moves the staging label ``AWSCURRENT`` from another version to this version
        (because you included it in the ``StagingLabels`` parameter) then Secrets Manager also automatically
        ...
        * This operation is idempotent. If a version with a ``VersionId`` with the same value as the
        ...
        because you cannot modify an existing version; you can only create new ones.
        ...
        * To list the versions attached to a secret, use  ListSecretVersionIds .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/PutSecretValue>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Resource Name (ARN) or the friendly name of the secret. The secret must already exist.
        ...
        :param ClientRequestToken:
          ...
          .. note::
            ...
            If you use the AWS CLI or one of the AWS SDK to call this operation, then you can leave this
            parameter empty. The CLI or SDK generates a random UUID for you and includes that in the
            request. If you don't use the SDK and instead generate a raw HTTP request to the Secrets Manager
          ...
          function's processing. We recommend that you generate a `UUID-type
          <https://wikipedia.org/wiki/Universally_unique_identifier>`__ value to ensure uniqueness within
          ...
          * If a version with this value already exists and that version's ``SecretString`` and
          ``SecretBinary`` values are different from those in the request then the request fails because you
          cannot modify an existing secret version. You can only create new versions to store new secret
          values.
        ...
        :param SecretString:
          ...
          (Optional) Specifies text data that you want to encrypt and store in this new version of the
          secret. Either ``SecretString`` or ``SecretBinary`` must have a value, but not both. They cannot
          both be empty.

          If you create this secret by using the Secrets Manager console then Secrets Manager puts the
          protected secret text in only the ``SecretString`` parameter. The Secrets Manager console stores
          the information as a JSON structure of key/value pairs that the default Lambda rotation function
          knows how to parse.
        ...
        :param VersionStages:
          ...
          These staging labels are used to track the versions through the rotation process by the Lambda
          rotation function.
    ...
    def restore_secret(self, SecretId: str) -> ClientRestoreSecretResponseTypeDef:
        ...
        the secret accessible to query once again.
        ...
        * To delete a secret, use  DeleteSecret .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/RestoreSecret>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to restore from a previously scheduled deletion. You can
          specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
    ...
    def rotate_secret(
        ...
        RotationRules: ClientRotateSecretRotationRulesTypeDef = None,
    ) -> ClientRotateSecretResponseTypeDef:
        """
        Configures and starts the asynchronous process of rotating this secret. If you include the
        configuration parameters, the operation sets those values for the secret and then immediately starts
        a rotation. If you do not include the configuration parameters, the operation starts a rotation with
        the values already stored in the secret. After the rotation completes, the protected service and its
        clients all use the new version of the secret.

        This required configuration information includes the ARN of an AWS Lambda function and the time
        between scheduled rotations. The Lambda rotation function creates a new version of the secret and
        creates or updates the credentials on the protected service to match. After testing the new
        credentials, the function marks the new secret with the staging label ``AWSCURRENT`` so that your
        clients all immediately begin to use the new version. For more information about rotating secrets
        and how to configure a Lambda function to rotate the secrets for your protected service, see
        `Rotating Secrets in AWS Secrets Manager
        <https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets.html>`__ in the *AWS
        ...
        * The ``AWSPENDING`` staging label is not attached to any version of the secret.
        ...
        If instead the ``AWSPENDING`` staging label is present but is not attached to the same version as
        ...
        * To attach staging labels to or remove staging labels from a version of a secret, use
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/RotateSecret>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to rotate. You can specify either the Amazon Resource Name
          (ARN) or the friendly name of the secret.
        ...
        :param ClientRequestToken:
          ...
          If you use the AWS CLI or one of the AWS SDK to call this operation, then you can leave this
          parameter empty. The CLI or SDK generates a random UUID for you and includes that in the request
          for this parameter. If you don't use the SDK and instead generate a raw HTTP request to the
          Secrets Manager service endpoint, then you must generate a ``ClientRequestToken`` yourself for new
          ...
          ensure that a given secret is not created twice. We recommend that you generate a `UUID-type
          <https://wikipedia.org/wiki/Universally_unique_identifier>`__ value to ensure uniqueness within
          the specified secret.

          Secrets Manager uses this value to prevent the accidental creation of duplicate versions if there
          are failures and retries during the function's processing. This value becomes the ``VersionId`` of
          the new version.
        ...
        :param RotationRules:
          ...
          - **AutomaticallyAfterDays** *(integer) --*
            ...
            last rotation. The service chooses the hour within that 24-hour date window randomly. The minute
            is also chosen somewhat randomly, but weighted towards the top of the hour and influenced by a
    ...
    def tag_resource(
        ...
        secret. This operation only appends tags to the existing list of tags. To remove tags, you must use
        UntagResource .
        ...
        * Do not use the ``aws:`` prefix in your tag names or values because it is reserved for AWS use. You
        can't edit or delete tag names or values with this prefix. Tags with this prefix do not count
        against your tags per secret limit.
        ...
        * If your tagging schema will be used across multiple services and resources, remember that other
        ...
        .. warning::
          ...
          permissions. If successfully completing this operation would result in you losing your permissions
          for this secret, then the operation is blocked and returns an Access Denied error.
        ...
        * To view the list of tags attached to a secret, use  DescribeSecret .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/TagResource>`_
        ...
        **Request Syntax**
        ...
        :param Tags: **[REQUIRED]**
          ...
          JSON parameter for the various command line tool environments, see `Using JSON for Parameters
          <https://docs.aws.amazon.com/cli/latest/userguide/cli-using-param.html#cli-using-param-json>`__ in
          the *AWS CLI User Guide* . For the AWS CLI, you can also use the syntax: ``--Tags
    ...
    def untag_resource(self, SecretId: str, TagKeys: List[str]) -> None:
        ...
        This operation is idempotent. If a requested tag is not attached to the secret, no error is returned
        and the secret metadata is unchanged.

        .. warning::

          If you use tags as part of your security strategy, then removing a tag can change permissions. If
          successfully completing this operation would result in you losing your permissions for this
        ...
        * To view the list of tags attached to a secret, use  DescribeSecret .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/UntagResource>`_
    ...
    def update_secret(
        ...
        *either*  ``SecretString`` or ``SecretBinary`` then it also creates a new version attached to the
        secret.
        ...
        .. note::
          ...
          encrypting and storing only a text string. To encrypt and store binary data as part of the version
          of a secret, you must use either the AWS CLI or one of the AWS SDKs.

        * If a version with a ``VersionId`` with the same value as the ``ClientRequestToken`` parameter
        already exists, the operation results in an error. You cannot modify an existing version, you can
        only create a new version.
        ...
        * kms:GenerateDataKey - needed only if you use a custom AWS KMS key to encrypt the secret. You do
        not need this permission to use the account's AWS managed CMK for Secrets Manager.

        * kms:Decrypt - needed only if you use a custom AWS KMS key to encrypt the secret. You do not need
        ...
        * To list the versions contained in a secret, use  ListSecretVersionIds .
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/UpdateSecret>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret that you want to modify or to which you want to add a new version. You can
          specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
        ...
        :param ClientRequestToken:
          ...
          If you use the AWS CLI or one of the AWS SDK to call this operation, then you can leave this
          parameter empty. The CLI or SDK generates a random UUID for you and includes that in the request.
          If you don't use the SDK and instead generate a raw HTTP request to the Secrets Manager service
          endpoint, then you must generate a ``ClientRequestToken`` yourself for new versions and include
          that value in the request.

          You typically only need to interact with this value if you implement your own retry logic and want
          to ensure that a given secret is not created twice. We recommend that you generate a `UUID-type
          <https://wikipedia.org/wiki/Universally_unique_identifier>`__ value to ensure uniqueness within
          the specified secret.

          Secrets Manager uses this value to prevent the accidental creation of duplicate versions if there
          are failures and retries during the Lambda rotation function's processing.
        ...
        :param KmsKeyId:
          ...
          (Optional) Specifies an updated ARN or alias of the AWS KMS customer master key (CMK) to be used
          to encrypt the protected text in new versions of this secret.

          .. warning::

            You can only use the account's default CMK to encrypt and decrypt if you call this operation
            using credentials from the same account that owns the secret. If the secret is in a different
            account, then you must create a custom CMK and provide the ARN of that CMK in this field. The
            user making the call must have permissions to both the secret and the CMK in their respective
            accounts.

        :type SecretBinary: bytes
        :param SecretBinary:

          (Optional) Specifies updated binary data that you want to encrypt and store in the new version of
          the secret. To use this parameter in the command-line tools, we recommend that you store your
          binary data in a file and then use the appropriate technique for your tool to pass the contents of
          the file as a parameter. Either ``SecretBinary`` or ``SecretString`` must have a value, but not
        ...
        :param SecretString:
          ...
          (Optional) Specifies updated text data that you want to encrypt and store in this new version of
          the secret. Either ``SecretBinary`` or ``SecretString`` must have a value, but not both. They
          ...
          If your command-line tool or SDK requires quotation marks around the parameter, you should use
          single quotes to avoid confusion with the double quotes required in the JSON text. You can also
          'escape' the double quote character in the embedded JSON text by prefacing each with a backslash.
    ...
    def update_secret_version_stage(
        ...
        """
        Modifies the staging labels attached to a version of a secret. Staging labels are used to track a
        version as it progresses through the secret rotation process. You can attach a staging label to only
        one version of a secret at a time. If a staging label to be added is already attached to another
        version, then it is moved--removed from the other version first and then attached to this one. For
        more information about staging labels, see `Staging Labels
        <https://docs.aws.amazon.com/secretsmanager/latest/userguide/terms-concepts.html#term_staging-label>`__
        in the *AWS Secrets Manager User Guide* .

        The staging labels that you specify in the ``VersionStage`` parameter are added to the existing list
        of staging labels--they don't replace it.
        ...
        * To get the list of staging labels that are currently associated with a version of a secret, use ``
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/secretsmanager-2017-10-17/UpdateSecretVersionStage>`_
        ...
        **Request Syntax**
        ...
        :param SecretId: **[REQUIRED]**
          ...
          Specifies the secret with the version whose list of staging labels you want to modify. You can
          specify either the Amazon Resource Name (ARN) or the friendly name of the secret.
        ...
        :param RemoveFromVersionId:
          ...
          Specifies the secret version ID of the version that the staging label is to be removed from. If
          the staging label you are trying to attach to one version is already attached to a different
          version, then you must include this parameter and specify the version that the label is to be
          removed from. If the label is attached and you either do not specify this parameter, or the
          version ID does not match, then the operation fails.

        :type MoveToVersionId: string
        :param MoveToVersionId:

          (Optional) The secret version ID that you want to add the staging label to. If you want to remove
          a label from a version, then do not specify this parameter.

          If the staging label is already attached to a different version of the secret, then you must also
          specify the ``RemoveFromVersionId`` parameter.
    ...
    def get_paginator(
        ...
        """
        Get Paginator for `list_secrets` operation.
        """
