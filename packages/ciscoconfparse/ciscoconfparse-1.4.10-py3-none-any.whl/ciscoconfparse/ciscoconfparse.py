...
class CiscoConfParse(object):
    ...
    def find_objects_dna(self, dnaspec, exactmatch=False):
        ...
        return list(
            [obj for obj in self.ConfigObjs if linespec_re.search(obj.dna)])
    ...
    def find_objects_w_child(self, parentspec, childspec, ignore_ws=False,
        ...
        return list(
            [x for x in self.find_objects(parentspec) if x.re_search_children(childspec, 
                    recurse=recurse)])
    ...
    def _find_line_OBJ(self, linespec, exactmatch=False):
        ...
        return list(
            [obj for obj in self.ConfigObjs if linespec_re.search(obj.text)])
...
class IOSConfigList(MutableSequence):
    ...
    def _banner_mark_regex(self, REGEX):
        ...
        banner_objs = list(
            [obj for obj in self._list if REGEX.search(obj.text)])
    ...
    def _bootstrap_obj_init(self, text_list):
        ...
        for line in text_list:
            ...
            if (indent < max_indent) and is_config_line:
                ...
                stale_parent_idxs = [ii for ii in sorted(
                        list(parents.keys()), reverse=True) if ii >= indent]
