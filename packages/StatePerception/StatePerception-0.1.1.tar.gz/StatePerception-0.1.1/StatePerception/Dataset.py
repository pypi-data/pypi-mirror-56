...
from .HelperFunctions import save_pickle_file, load_pickle_file, ensure_dir
...
class SPDataset():
    ...
    def __init__(
        ...
        if self.data_train is not(None):
            if len(self.data_train) == 0:
                ...
                if self.verbose:
                    print(
                        'WARNING: Training Data List Empty - Setting Training Data to None')
        if self.data_val is not(None):
            if len(self.data_val) == 0:
                self.data_val = None
                if self.verbose:
                    print(
                        'WARNING: Validation Data List Empty - Setting Validation Data to None')
    ...
    def init_from_XY_arrays(
        ...
        X_train = [X_train] if isinstance(X_train, np.ndarray) else X_train
        Y_train = [Y_train] if isinstance(Y_train, np.ndarray) else Y_train
        I_train = [I_train] if isinstance(I_train, np.ndarray) else I_train
        X_val = [X_val] if isinstance(X_train, np.ndarray) else X_val
        Y_val = [Y_val] if isinstance(Y_train, np.ndarray) else Y_val
        I_val = [I_val] if isinstance(I_train, np.ndarray) else I_val
        X_test = [X_test] if isinstance(X_train, np.ndarray) else X_test
        Y_test = [Y_test] if isinstance(Y_train, np.ndarray) else Y_test
        I_test = [I_test] if isinstance(I_train, np.ndarray) else I_test
        # Construct data_train, data_val and data_test. I_* is optional
        if X_train is not(None):
            data_train = [[X_train, Y_train]] if I_train is None else [
                [X_train, Y_train, I_train]]
        else:
            data_train = None
        if X_val is not(None):
            data_val = [[X_val, Y_val]] if I_val is None else [
                [X_val, Y_val, I_val]]
        else:
            data_val = None
        if X_test is not(None):
            data_test = [[X_test, Y_test]] if I_test is None else [
    ...
    def _fit_X_scaler(self):
        if self.X_train is not(None):
            self._X_scaler = self._scaler_fit_on_2d_arrays(
                self.X_scaler, self.X_train)
        else:
            if self.verbose:
    ...
    def _unscale_np_array(self, scaler_in, array_in):
        ...
        if dimensions == 2:
            ...
            array_in_2d = array_in.reshape(
                (array_in.shape[0] * array_in.shape[1], array_in.shape[2]))
            array_out_2d = scaler_in.inverse_transform(array_in_2d)
            array_out = array_out_2d.reshape(
                (array_in.shape[0], array_in.shape[1], array_in.shape[2]))
        return array_out
    ...
    def _get_signal_from_tensor(
        ...
        signal = self._get_tensor(
            dataset=dataset,
            tensor=tensor[0],
            scaled=scaled,
            indices=indices,
            pick_signals=pick_signal)[
            tensor[1]]
        return signal
    ...
    def _get_tensor(
        ...
        tensor = [
            (tensor_[
                indices[0]:indices[1], sig], sig)[0] for tensor_, sig in zip(
                tensor, pick_signals)] if pick_signals is not(None) else [
            tensor_[
                indices[0]:indices[1], :] for tensor_ in tensor]
        # Return
        return tensor
...
#        return Y_out
    ...
    def _validity_vector_transitions(self, dataset, inval_timesteps):
        if dataset == 'train':
            idx_transition = [np.sum(self.n_train_list[:i])
                              for i in range(self.n_meas_train)]
            I = self.I_train
        elif dataset == 'val':
            idx_transition = [np.sum(self.n_val_list[:i])
                              for i in range(self.n_meas_val)]
            I = self.I_val
        elif dataset == 'test':
            idx_transition = [np.sum(self.n_test_list[:i])
                              for i in range(self.n_meas_test)]
            I = self.I_test
    ...
    def summary(self):
        ...
        data_info_1 = [['Overall',
                        self.n_meas_train + self.n_meas_val + self.n_meas_test,
                        self.n_train + self.n_val + self.n_test,
                        100],
                       ['Train',
                        self.n_meas_train,
                        self.n_train,
                        self.n_train / (self.n_samples) * 100],
                       ['Val',
                        self.n_meas_val,
                        self.n_val,
                        self.n_val / (self.n_samples) * 100],
                       ['Test',
                        self.n_meas_test,
                        self.n_test,
                        self.n_test / (self.n_samples) * 100]]

        # column widths: 8, 10, 15, 7, 10
        template_header = '{0:14}|{1:8}{2:13}{3:9}'
        # column widths: 8, 10, 15, 7, 10
        template = '{0:14}|{1:8}{2:13}{3:9.2f}'
        print(
...
#            print('')
    ...
    def get_3D_data(
            ...
            shift=None,
            zero_inval_Y=True,
            set_measurement_transition_invalid=False,
            indices=None,
        ...
        if shift is None:
            shift = timesteps
        # Get Data
        X, Y, I = self._get_XYI(dataset, scaled_X, scaled_Y, indices,
                                pick_signals_X=pick_signals_X, pick_signals_Y=pick_signals_Y)
        ...
        if set_measurement_transition_invalid and timesteps is not(None):
            I_transition = self._validity_vector_transitions(
                dataset, inval_timesteps=timesteps)
            I = I * I_transition
        # Multiply Invalid Reference Y by Zero
        if zero_inval_Y:
            Y = self._set_inval_Y_to_zero(Y, I)
        # Batch Data
        X, Y, I = self._batch_list_of_arrays(
            X, Y, I, timesteps=timesteps, shift=shift)
        # Discard Invalid Samples
        if discard_invalid:
            X, Y, I = self._discard_invalid_samples(
                X, Y, I, method=discard_invalid)
        # Extract last Time Step for Y and I if no Sequence to Sequence
        # Prediction
    ...
    def get_timeseries_generator(
        ...
        SPTSG = SPTimeSeriesGenerator(
            data=[
                *X,
                *I],
            flag_3d_data=flag_3d_X,
            targets=Y,
            flag_3d_targets=flag_3d_Y,
            look_back=timesteps,
    ...
    def _save_dataset_mat(self, savename):
        ...
        savedict = {
            ...
            'Y_val_scaled': self.Y_val_scaled,
            'X_test_scaled': self.X_test_scaled,
            'Y_test_scaled': self.Y_test_scaled,
            'dt': self.dt,
            'X_mean': self.X_mean,
            'X_scale': self.X_scale,
            'Y_mean': self.Y_mean,
            'Y_scale': self.Y_scale,
            'X_signal_names': self.X_signal_names,
            'Y_signal_names': self.Y_signal_names,
            'X_signal_units': self.X_signal_units,
            'Y_signal_units': self.Y_signal_units,
            'name': self.name}
    ...
    def _load_dataset_pickle(self, savename):
        [self.data_train,
         ...
         self.Y_signal_units,
         self.X_scaler,
         self.Y_scaler] = load_pickle_file(savename)
    ...
    def _get_legend_entry(self, tensor, signal, scaled=False):
        ...
        legend_entry = self._get_legend_entries(
            tensor=tensor[0],
            scaled=scaled,
            pick_signals=pick_signal)[
            tensor[1]][0]
    ...
    def _get_legend_entries(self, tensor='X', scaled=True, pick_signals=None):
        ...
        for i_tensor, n_tensor in enumerate(n):
            ...
            for i_signal in range(n_tensor):
                # Get Elements
                name_ = signal_names[i_tensor][i_signal] if signal_names is not(
                    None) else None
                unit_ = signal_units[i_tensor][i_signal] if signal_units is not(
                    None) else None
                bias_ = biases[i_tensor][i_signal] if biases is not(
                    None) and scaled else None
                scale_ = scales[i_tensor][i_signal] if scales is not(
                    None) and scaled else None
                # Get Legend Name
                legend_entry = self._construct_legend_entry(
                    i_signal, name_, unit_, bias_, scale_)
        ...
        if self.data_train is not(None):
            ...
            for i_tensor in range(self.n_X):
                X_train.append(np.concatenate(
                    [meas[0][i_tensor] for meas in self.data_train], axis=0))
            return X_train
        else:
            return None
    ...
    def X_val(self):
        if self.data_val is not(None):
            X_val = []
            for i_tensor in range(self.n_X):
                X_val.append(np.concatenate(
                    [meas[0][i_tensor] for meas in self.data_val], axis=0))
            return X_val
        else:
            return None
        ...
        if self.data_test is not(None):
            ...
            for i_tensor in range(self.n_X):
                X_test.append(np.concatenate(
                    [meas[0][i_tensor] for meas in self.data_test], axis=0))
            return X_test
        else:
            return None
        ...
        if self.data_train is not(None):
            ...
            for i_tensor in range(self.n_Y):
                Y_train.append(np.concatenate(
                    [meas[1][i_tensor] for meas in self.data_train], axis=0))
            return Y_train
        else:
            return None
    ...
    def Y_val(self):
        if self.data_val is not(None):
            Y_val = []
            for i_tensor in range(self.n_Y):
                Y_val.append(np.concatenate(
                    [meas[1][i_tensor] for meas in self.data_val], axis=0))
            return Y_val
        else:
            return None
        ...
        if self.data_test is not(None):
            ...
            for i_tensor in range(self.n_Y):
                Y_test.append(np.concatenate(
                    [meas[1][i_tensor] for meas in self.data_test], axis=0))
            return Y_test
        else:
            return None
    ...
    def I_train(self):
        if self.data_train is not(None):
            return [np.concatenate([meas[2][0]
                                    for meas in self.data_train], axis=0)]
        else:
            return None
    ...
    def I_val(self):
        if self.data_val is not(None):
            return [np.concatenate([meas[2][0]
                                    for meas in self.data_val], axis=0)]
        else:
            return None
    ...
    def I_test(self):
        if self.data_test is not(None):
            return [np.concatenate([meas[2][0]
                                    for meas in self.data_test], axis=0)]
        else:
            return None
    ...
    @property
    def I_idx(self):
        if self.data_train is not(None) or self.data_val is not(
                None) or self.data_test is not(None):
    ...
    def X_train_scaled(self):
        if self.X_train is not(None) and self.X_scaler is not(None):
            X_train_scaled = self._get_scaled_2d_arrays(
                self.X_scaler, self.X_train)
    ...
    def X_val_scaled(self):
        if self.X_val is not(None) and self.X_scaler is not(None):
            X_val_scaled = self._get_scaled_2d_arrays(
                self.X_scaler, self.X_val)
            return X_val_scaled
        else:
            return None
    ...
    def X_test_scaled(self):
        if self.X_test is not(None) and self.X_scaler is not(None):
            X_test_scaled = self._get_scaled_2d_arrays(
                self.X_scaler, self.X_test)
    ...
    def Y_train_scaled(self):
        if self.Y_train is not(None) and self.Y_scaler is not(None):
            Y_train_scaled = self._get_scaled_2d_arrays(
                self.Y_scaler, self.Y_train)
    ...
    def Y_val_scaled(self):
        if self.Y_val is not(None) and self.Y_scaler is not(None):
            Y_val_scaled = self._get_scaled_2d_arrays(
                self.Y_scaler, self.Y_val)
            return Y_val_scaled
        else:
            return None
    ...
    def Y_test_scaled(self):
        if self.Y_test is not(None) and self.Y_scaler is not(None):
            Y_test_scaled = self._get_scaled_2d_arrays(
                self.Y_scaler, self.Y_test)
...
class SPDatasetPlotter():
    ...
    def _plot_dataset(
            ...
            plot_X=True,
            plot_Y=True,
            X_legend_entries=None,
            Y_legend_entries=None,
            downsample=1,
            sharex=True,
            plot_legend=True,
            mark_invalid=False):
        ...
        t = np.arange(
            0, np.int(
                np.ceil(
                    X[0].shape[1] / downsample))) * dt * downsample
        # Dimensions
        ...
        if plot_X:
            for i_X_curr, X_curr in enumerate(X):
                if plot_Y:
                    spx_ = plt.subplot(
                        n_sp_row, n_sp_col, (i_X_curr + 1) * 2 - 1)
                else:
        ...
        if plot_Y:
            for i_Y_curr, Y_curr in enumerate(Y):
                ...
                if plot_X:
                    ...
                    spy_ = plt.subplot(
                        n_sp_row, n_sp_col, (i_Y_curr + 1), sharex=sharedaxis)
                spy.append(spy_)
                plt.plot(t, Y_curr[0, ::downsample, :])
                if mark_invalid:
                    plt.plot(t[Inval_idx], Y_curr[0, ::downsample, :]
                             [Inval_idx], 'o', color='red')
                plt.ylabel('Feature')
                if self.SPDataset.dt is not(None):
                    plt.xlabel('Time t [s]')
                else:
    ...
    def _plot_heatmap(
        ...
        H, xedges, yedges = np.histogram2d(
            sig1, sig2, bins=bins, density=density)
        # Meshgrid
    ...
    def plot_dataset(
            ...
            zero_inval_Y=False):
        # Legend Entries
        X_legend_entries = self.SPDataset._get_legend_entries(
            tensor='X', scaled=scaled_X, pick_signals=pick_signals_X)
        Y_legend_entries = self.SPDataset._get_legend_entries(
            tensor='Y', scaled=scaled_Y, pick_signals=pick_signals_Y)
        # Choose Datset + Mapping
        X, Y, I = self.SPDataset.get_3D_data(dataset, scaled_X=scaled_X, scaled_Y=scaled_Y, indices=indices,
                                             pick_signals_X=pick_signals_X, pick_signals_Y=pick_signals_Y, zero_inval_Y=zero_inval_Y)
        # Plot Data Set
        self._plot_dataset(
            X,
            Y,
    ...
    def plot_scatter(
        ...
        signal_name_axis_1 = self.SPDataset._get_legend_entry(
            tensor=tensor_axis_1, signal=signal_axis_1, scaled=scaled_axis_1)
        signal_name_axis_2 = self.SPDataset._get_legend_entry(
            tensor=tensor_axis_2, signal=signal_axis_2, scaled=scaled_axis_2)
        # Choose Datset + Mapping
        signal_1 = self.SPDataset._get_signal_from_tensor(
            dataset=dataset,
            tensor=tensor_axis_1,
            signal=signal_axis_1,
            indices=indices,
            scaled=scaled_axis_1)
        signal_2 = self.SPDataset._get_signal_from_tensor(
            dataset=dataset,
            tensor=tensor_axis_2,
            signal=signal_axis_2,
            indices=indices,
            scaled=scaled_axis_2)
        # Plot
        self._plot_scatter(
            sig1=signal_1,
            sig2=signal_2,
            name1=signal_name_axis_1,
            name2=signal_name_axis_2,
            marker=marker,
            alpha=alpha)
    ...
    def plot_heatmap(
        ...
        signal_2 = self.SPDataset._get_signal_from_tensor(
            ...
            scaled=scaled_axis_2)
        # Plot
        self._plot_heatmap(sig1=signal_1[:,
                                         0],
                           sig2=signal_2[:,
                                         0],
                           name1=signal_name_axis_1,
    ...
    def plot_distribution(
            ...
            normalize=True):
        # Get Histograms
        hX, hY = self.SPDataset._get_histogram(
            dataset=dataset, bins=bins, normalize=normalize)
        # Plot Hisograms
        if plotX:
            plt.figure()
    ...
    def plot_trainvaltest_distribution(
            self,
            bins=50,
            plotX=True,
            plotY=True,
            relative=True):
        # Get Histograms
        hX_train, hY_train = self.SPDataset._get_histogram(
            dataset='train', bins=bins)
        hX_val, hY_val = self.SPDataset._get_histogram(
            dataset='val', bins=bins)
        hX_test, hY_test = self.SPDataset._get_histogram(
            dataset='test', bins=bins)
#        # Plot Hisograms
#        if plotX:
#            plt.figure()
...
#                axX.grid()
        ...
        if plotY:
            plt.figure()
            axY_ref = plt.subplot(
                self.SPDataset.n_Y, self.SPDataset.n_outputs[0], 1)
            # Loop over Datasets
            for i_D, hY in enumerate([hY_train, hY_val, hY_test]):
                ...
                for i_Y, h_ in enumerate(hY):
                    ...
                    for i_sig, h_sig_ in enumerate(h_):
                        ...
                        axY = plt.subplot(
                            self.SPDataset.n_Y,
                            self.SPDataset.n_outputs[0],
                            i_sp,
                            sharey=axY_ref) if i_sp > 1 else axY_ref
                        # Plot
                        ...
                        axY.grid()
                        # Axes
                        if self.SPDataset.Y_signal_names is not(
                                None) and self.SPDataset.Y_signal_units is not(None):
                            plt.xlabel(
                                self.SPDataset.Y_signal_names[i_Y][i_sig] +
                                ' [' +
                                self.SPDataset.Y_signal_units[i_Y][i_sig] +
                                ']')
...
class SPTimeSeriesGenerator(Sequence):
    ...
    def __getitem__(self, index):
        if index > (
                (len(self.data[0]) - self.look_back) // self.batch_size) + 1:
            raise ValueError(
                'Index for data generator is larger than maximum number of Batches')
#        print('getting item....')
...
# HELP FUNCTIONS #########################################################
    def get_3d_data(self, data, rows):
        ...
        return as_strided(
            ...
            batch_shape,
            batch_strides,
            writeable=False)  # 185 times faster than 3d indexing
