...
class loss_with_convergence_time():
    ...
    def __init__(self,loss_function=mean_squared_error,convergence_time=100):
        ...
        self.__name__ = loss_function.__name__ + '_convtime'+str(convergence_time)
...
class weighted_loss():
    ...
    def __init__(self,loss_function=mean_squared_error,loss_weights=None):
        # Tensor
        self.loss_function = loss_function
        self.loss_weights = tf.expand_dims(tf.expand_dims(tf.constant(value=loss_weights,dtype=tf.float32),axis=0),axis=1)
...
# =============================================================================
mean_squared_error_convtime100 = loss_with_convergence_time(loss_function=mean_squared_error,convergence_time=100)
mse_conv100_w2 = weighted_loss(loss_function=mean_squared_error_convtime100,loss_weights=np.array([0.5,2]))
...
class PCA_Layer(Layer):
    ...
    def build(self, input_shape):
        ...
        # Get Initializers
        kernel_init = tf.constant_initializer(value=self._kernel,dtype=tf.float32)
        ...
        self.kernel = self.add_weight(name='kernel', 
                                      ...
                                      trainable=False)
        super(PCA_Layer, self).build(input_shape)  # Be sure to call this somewhere!
...
class ConstantNormalizationLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        if self._mean is not(None):
            mean_init = tf.constant_initializer(value=self._mean,dtype=tf.float32)
        scale_init = tf.constant_initializer(value=self._scale,dtype=tf.float32)
        
        # Create Constants for Normalization
        if self._mean is not(None):
            self.mean =  self.add_weight(name='mean',  shape=(self._mean.shape[0],),  trainable=False, initializer=mean_init)
        self.scale = self.add_weight(name='scale', shape=(self._scale.shape[0],), trainable=False, initializer=scale_init)
        
        super(ConstantNormalizationLayer, self).build(input_shape)
        
    def call(self, x):
        if self._position is 'input':
            if self._mean is not(None):
                x = x - self.mean
            x = tf.multiply(x, tf.divide(1,self.scale))
        if self._position is 'output':
...
class DerivativeLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        factor_init = tf.constant_initializer(value=self._factor,dtype=tf.float32)
        # Create Constants
        self.dt = self.add_weight(name='dt', shape=(1,), trainable=False, initializer=dt_init)
        self.factor = self.add_weight(name='factor', shape=(1,), trainable=False, initializer=factor_init)
        # Build Super
...
class AddMeanSignalsToSignal(Layer):
    ...
    def __init__(self, ind_mean_signals=[4,5,6,7], ind_target_signal=0, **kwargs):
        # Write Arguments
    ...
    def call(self, x):
        ...
        av = tf.mean(x0[:,:,ind_mean_signals],axis=2)/len(self.ind_mean_signals)
...
class CustomFunctionsLib():
    ...
    def __init__(self):
        self.fundict = {'mean_squared_error_convtime100':mean_squared_error_convtime100,
                        'PCA_Layer':PCA_Layer,
