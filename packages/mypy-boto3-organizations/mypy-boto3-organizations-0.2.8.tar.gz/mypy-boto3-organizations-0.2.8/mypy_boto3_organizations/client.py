...
class Client(BaseClient):
    ...
    def attach_policy(self, PolicyId: str, TargetId: str) -> None:
        ...
        policy affects accounts depends on the type of policy:

        * **Service control policy (SCP)** - An SCP specifies what permissions can be delegated to
        users in affected member accounts. The scope of influence for a policy depends on what you
        ...
        organizational unit (OU). You can attach one SCP to a higher level root or OU, and a
        different SCP to a child OU or to an account. The child policy can further restrict only the
        permissions that pass through the parent filter and are available to the child. An SCP that
        is attached to a child can't grant a permission that the parent hasn't already granted. For
        example, imagine that the parent SCP allows permissions A, B, C, D, and E. The child SCP
        allows C, D, E, F, and G. The result is that the accounts affected by the child SCP are
        allowed to use only C, D, and E. They can't use A or B because the child OU filtered them
        out. They also can't use F and G because the parent OU filtered them out. They can't be
        granted back by the child SCP; child SCPs can only filter the permissions they receive from
        the parent SCP.

        AWS Organizations attaches a default SCP named ``"FullAWSAccess`` to every root, OU, and
        account. This default SCP allows all services and actions, enabling any new child OU or
        account to inherit the permissions of the parent root or OU. If you detach the default
        policy, you must replace it with a policy that specifies the permissions that you want to
        allow in that OU or account.

        For more information about how AWS Organizations policies permissions work, see `Using
    ...
    def create_account(
        ...
        *AWS Organizations User Guide.*
        ...
        .. warning::
          ...
          * When you create an account in an organization using the AWS Organizations console, API,
          ...
          *not* automatically collected. If you must remove an account from your organization later,
          you can do so only after you provide the missing information. Follow the steps at `To
          leave an organization as a member account
        ...
        **Request Syntax**
        ...
        :param RoleName:
          ...
          in the *AWS Organizations User Guide* , and steps 2 and 3 in `Tutorial\\: Delegate Access
          ...
          parameter is a string of characters that can consist of uppercase letters, lowercase
          ...
          - *(dict) --*
            ...
            - **CreateAccountStatus** *(dict) --*
              ...
              - **Id** *(string) --*
                ...
                The `regex pattern <http://wikipedia.org/wiki/regex>`__ for an create account
    ...
    def create_gov_cloud_account(
        ...
        *AWS Organizations User Guide.*
        ...
        created in the new AWS GovCloud (US) account that can be assumed by the AWS GovCloud (US)
        account that is associated with the master account of the commercial organization. For more
        information and to view a diagram that explains how account access works, see `AWS
        Organizations
        <http://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-organizations.html>`__ in
        the *AWS GovCloud User Guide.*
    ...
    def create_organization(
        ...
        root. If you instead choose to create the organization supporting only the consolidated
        billing features by setting the ``FeatureSet`` parameter to ``CONSOLIDATED_BILLING"`` , no
        policy types are enabled by default, and you can't use organization policies.
        ...
        **Request Syntax**
        ...
        :param FeatureSet:
          ...
          * ``ALL`` : In addition to all the features supported by the consolidated billing feature
          set, the master account can also apply any policy type to any member account in the
          ...
          - *(dict) --*
            ...
            - **OrganizationalUnit** *(dict) --*
              ...
              - **Id** *(string) --*
                ...
                ID of the root that contains the OU) followed by a second "-" dash and from 8 to 32
    ...
    def create_policy(
        ...
        **Request Syntax**
        ...
        :param Content: **[REQUIRED]**
          ...
          policy
          <https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_policies_scp.html>`__
          (SCP), this string must be JSON text that specifies the permissions that admins in
          attached accounts can delegate to their users, groups, and roles. For more information
          about the SCP syntax, see `Service Control Policy Syntax
    ...
    def detach_policy(self, PolicyId: str, TargetId: str) -> None:
         ...
         **Note:** Every root, OU, and account must have at least one SCP attached. If you want to
         replace the default ``FullAWSAccess`` policy with one that limits the permissions that can
         be delegated, you must attach the replacement policy before you can remove the default one.
         This is the authorization strategy of `whitelisting
         <https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_policies_about-scps.html#orgs_policies_whitelist>`__
         . If you instead attach a second SCP and leave the ``FullAWSAccess`` SCP still attached,
         and specify ``"Effect": "Deny"`` in the second SCP to override the ``"Effect": "Allow"`` in
         the ``FullAWSAccess`` policy (or any other attached SCP), you're using the authorization
         strategy of `blacklisting
    ...
    def disable_aws_service_access(self, ServicePrincipal: str) -> None:
        ...
        *new* accounts in your organization. This means the service can't perform operations on your
        ...
        longer perform operations in your organization's accounts unless the operations are
        explicitly permitted by the IAM policies that are attached to your roles.
    ...
    def disable_policy_type(
        ...
        """
        Disables an organizational control policy type in a root. A policy of a certain type can be
    ...
    def enable_all_features(
        ...
        .. warning::
          ...
          the change by accepting the handshake.
    ...
    def invite_account_to_organization(
        ...
        .. warning::
          ...
          * You can invite AWS accounts only from the same seller as the master account. For
          ...
          Pvt. Ltd (AISPL), an AWS seller in India, you can invite only other AISPL accounts to your
          organization. You can't combine accounts from AISPL and AWS or from any other AWS seller.
          ...
          * If you receive an exception that indicates that you exceeded your account limits for the
          ...
          wait one hour and then try again. If the error persists after an hour, contact `AWS
    ...
    def leave_organization(self, *args: Any, **kwargs: Any) -> None:
        ...
        .. warning::
          ...
          * The master account in an organization with all features enabled can set service control
          ...
          preventing them from successfully calling ``LeaveOrganization`` and leaving the
          organization.
          ...
          * You can leave an organization as a member account only if the account is configured with
          ...
          an organization using the AWS Organizations console, API, or CLI commands, the information
          required of standalone accounts is *not* automatically collected. For each account that
          you want to make standalone, you must accept the end user license agreement (EULA), choose
          a support plan, provide and verify the required contact information, and provide a current
          payment method. AWS uses the payment method to charge for any billable (not free tier) AWS
              ...
              - *(dict) --*
                ...
                - **Id** *(string) --*
                  ...
                  * Organizational unit (OU): a string that begins with "ou-" followed by from 4 to
                  32 lower-case letters or digits (the ID of the root that contains the OU) followed
    ...
    def list_handshakes_for_account(
        ...
        **Request Syntax**
        ...
        :param Filter:
          ...
          ``INVITE`` , ``ENABLE_ALL_FEATURES`` , or ``APPROVE_ALL_FEATURES`` . Alternatively, for
          ...
          - *(dict) --*
            ...
            - **Handshakes** *(list) --*
              ...
              - *(dict) --*
                ...
                master account (the originator) invites another account (the recipient) to join its
    ...
    def list_handshakes_for_organization(
        ...
        **Request Syntax**
        ...
        :param Filter:
          ...
          as ``INVITE`` , ``ENABLE-ALL-FEATURES`` , or ``APPROVE-ALL-FEATURES`` . Alternatively, for
          the ``ENABLE-ALL-FEATURES`` handshake that generates a separate child handshake for each
          member account, you can specify the ``ParentHandshakeId`` to see only the handshakes that
            ...
            - **Parents** *(list) --*
              ...
              - *(dict) --*
                ...
                - **Id** *(string) --*
                  ...
                  * Organizational unit (OU): a string that begins with "ou-" followed by from 4 to
                  32 lower-case letters or digits (the ID of the root that the OU is in) followed by
    ...
    def remove_account_from_organization(self, AccountId: str) -> None:
        ...
        .. warning::
          ...
          want to make standalone, you must accept the end user license agreement (EULA), choose a
          support plan, provide and verify the required contact information, and provide a current
          payment method. AWS uses the payment method to charge for any billable (not free tier) AWS
          activity that occurs while the account isn't attached to an organization. To remove an
          account that doesn't yet have this information, you must sign in as the member account and
          follow the steps at `To leave an organization when all required account information has
