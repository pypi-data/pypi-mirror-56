...
def check_output(cmd, folder=None, return_code=False, stderr=None):
    ...
    try:
        ...
        logger.debug("Command return code: {}".format(int(return_code)))
        logger.debug("Output in file: {}".format(output))
        logger.debug("Output stdout: {}".format(stdout))
        logger.debug("Output stderr: {}".format(stderr))
        return output
    finally:
        try:
            os.unlink(tmp_file)
