...
from futura_ui.app.ui.dialogs.run_dialog import RunDialog
from futura_ui.app.ui.dialogs.project_choice import ProjectDialog
...
class Controller(object):
    ...
    def __init__(self):
        ...
        # signals.project_selected.emit()     # emit the project_selected signal - this runs the self.ensure_sqlite_indices function
        # signals.run_dialog.emit(None)           # emit the run_dialog signal
    ...
    def connect_signals(self):
        signals.run_dialog.connect(self.run_dialog)
        signals.load_tree_data.connect(self.load_tree_data)

        # Note - these need to be set up in the signals file too

        ## SLOTS
        ### Project
        signals.project_selected.connect(self.project_selected)
...
# Specific functions to do things go here, within the controller class
    ...
    #        self.change_project(futura_settings.startup_project, reload=True)
...
#         signals.database_changed.emit(exchange['output'][0])
    def run_dialog(self, parent=None):
        print('run_dialog')
        rundialog = RunDialog(parent)
    ...
    def load_tree_data(self):
        ...
        # might need to send this to a different thread maybe singleshot QTimer?
        #from futura.constants import TREE_DATA
        print('import finished')
        #self.tree_model = TreeModel(TREE_DATA)
        #print('treemodel created')
    ...
    def project_selected(self):
        ...
        print(sorted([d for d in bw.databases]))
