...
class SN:
    ...
    def nodemapping2(self, vnr, v2sindex):
        # v2sindex=[[v,s],[]...]
        vn = len(v2sindex)
        for i in range(vn):
            if self.snode[v2sindex[i][1]].lastcpu < vnr.vnode[v2sindex[i][0]].cpu or vnr.id in [id_vnode[0] for id_vnode in self.snode[v2sindex[i][1]].vnodeindexs]:
                raise Exception("CPU约束不满足，无法映射虚拟节点 或者同一个虚拟网的多个虚拟节点映射到同一个物理节点上")
        ...
        print('map  vnr node  ', vnr.id, ' success')
    ...
    def edegemapping(self, vnr, ve2seindex):
        i = 0
        for path in ve2seindex:
            ...
            i = i + 1
        print('map vnr edege ', vnr.id, ' success')
