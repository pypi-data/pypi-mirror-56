...
######################### gaerpytz monkey patch START #########################
from cStringIO import StringIO
import zipfile
########################## gaerpytz monkey patch END ##########################
...
##     except IOError:
##         return False
_zoneinfo_path = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                              'zoneinfo.zip'))
"""String path to the zoneinfo zip file relative to __init__.py."""

_zoneinfo = None

def _get_zoneinfo():
    """Cache the zoneinfo ZipFile in the module."""
    global _zoneinfo
    if _zoneinfo is None:
        _zoneinfo = zipfile.ZipFile(_zoneinfo_path, 'r')
    return _zoneinfo

def open_resource(name):
    """Open a resource from the zoneinfo zip file for reading.

    Monkey patched version of open_resource() to read from a ZipFile.
    """
    name_parts = name.lstrip('/').split('/')
    for part in name_parts:
        if part == os.path.pardir or os.path.sep in part:
            raise ValueError('Bad path segment: %r' % part)
    name_parts.insert(0, 'zoneinfo')
...
def resource_exists(name):
    """Return True if name is in the zoneinfo zip file.

    Monkey patched version of resource_exists() to read from a ZipFile.
    """
    global _name_set
    if _name_set is None:
        _name_set = frozenset(_get_zoneinfo().namelist())
    name = 'zoneinfo/' + name
