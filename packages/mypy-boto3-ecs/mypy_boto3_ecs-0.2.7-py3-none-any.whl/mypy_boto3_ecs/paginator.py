"Main interface for ecs Paginators"
...
class ListAccountSettingsPaginator(Boto3Paginator):
    ...
    def paginate(
        ...
        :py:meth:`ECS.Client.list_account_settings`.
        ...
        **Request Syntax**
        ...
        :param effectiveSettings:
          ...
          Specifies whether to return the effective settings. If ``true`` , the account settings for the
          root user or the default setting for the ``principalArn`` are returned. If ``false`` , the account
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param attributeValue:
          ...
          The value of the attribute with which to filter results. You must also specify an attribute name
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param filter:
          ...
          statements. For more information, see `Cluster Query Language
          <https://docs.aws.amazon.com/AmazonECS/latest/developerguide/cluster-query-language.html>`__ in
          the *Amazon Elastic Container Service Developer Guide* .

        :type status: string
        :param status:

          Filters the container instances by status. For example, if you specify the ``DRAINING`` status,
          the results include only container instances that have been set to ``DRAINING`` using
          UpdateContainerInstancesState . If you do not specify this parameter, the default is to include
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param status:
          ...
          families that do not have any ``ACTIVE`` task definition revisions are returned. If you paginate
          the resulting output, be sure to keep the ``status`` value constant in each subsequent request.
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param status:
          ...
          only ``ACTIVE`` task definitions are listed. By setting this parameter to ``INACTIVE`` , you can
          view task definitions that are ``INACTIVE`` as long as an active task or service still references
          them. If you paginate the resulting output, be sure to keep the ``status`` value constant in each
          subsequent request.
        ...
        :param sort:
          ...
          numerical order by revision so that the newest task definitions in a family are listed last.
          Setting this parameter to ``DESC`` reverses the sort order on family name and revision so that the
    ...
    def paginate(
        ...
        **Request Syntax**
        ...
        :param cluster:
          ...
          The short name or full Amazon Resource Name (ARN) of the cluster that hosts the tasks to list. If
        ...
        :param containerInstance:
          ...
          ``ListTasks`` results. Specifying a ``containerInstance`` limits the results to tasks that belong
        ...
        :param desiredStatus:
          ...
          ``desiredStatus`` of ``STOPPED`` limits the results to tasks that Amazon ECS has set the desired
          status to ``STOPPED`` . This can be useful for debugging tasks that are not starting properly or
          have died or finished. The default status filter is ``RUNNING`` , which shows tasks that Amazon
          ECS has set the desired status to ``RUNNING`` .

          .. note::

            Although you can filter results based on a desired status of ``PENDING`` , this does not return
