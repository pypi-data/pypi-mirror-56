...
df = r.get_device_data(serial_number, take_count=TAKE_COUNT, all_cols=True)
assert type(df.index) == pd.core.indexes.datetimes.DatetimeIndex, f'the type of df.index is {type(df.index)}'
