...
class ActionAck(Action):
    ...
    def __call__(self, *args, **kwargs):
        reply_to = self.header_frame.headers.get("reply_to")

        # 当有replay_to的时候，向replay_to发送信息
        if reply_to:
            properties = pika.BasicProperties(
                correlation_id=self.header_frame.get("correlation_id"),
...
class ActionNextTo(Action):
    ...
    def __call__(self, *args, **kwargs):
        ...
        data = self.new_message_data or self.context.body
        properties = pika.BasicProperties(
            headers=headers,
    ...
    def get_headers(self):
        ...
        if not self.new_message_headers:
            headers = getattr(self.context.header_frame, "headers", {})
        else:
            headers = self.new_message_headers
            for i, v in self.context.header_frame.get("headers", {}):
        ...
        headers["traceID"] = self.context.header_frame.get("headers", {}).get("traceID")
        return headers
