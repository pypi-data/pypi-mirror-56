...
class Client(BaseClient):
                ...
                'recentIntentSummaryView': [
                    {
                        ...
                        'dialogActionType':
                        'ElicitIntent'|'ConfirmIntent'|'ElicitSlot'|'Close'
                        |'Delegate',
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **recentIntentSummaryView** *(list) --*
              ...
              If you set the ``checkpointLabelFilter`` parameter in the request, the array contains
              ...
              - *(dict) --*
                ...
                - **checkpointLabel** *(string) --*
                  ...
                  to return to a previous intent.
            ...
            - **sessionAttributes** *(dict) --*
              ...
              contains application information passed between Amazon Lex and a client application.
            ...
            - **dialogAction** *(dict) --*
              ...
              - **type** *(string) --*
                ...
                * ``ElicitIntent`` - The next action is to determine the intent that the user wants
                to fulfill.
    ...
    def post_content(
        ...
        requests to Amazon Lex at runtime. Amazon Lex interprets the user input using the machine
        learning model that it built for the bot.

        The ``PostContent`` operation supports audio input at 8kHz and 16kHz. You can use 8kHz audio
        to achieve higher speech recognition accuracy in telephone audio applications.

        In response, Amazon Lex returns the next message to convey to the user. Consider the
        ...
        * If the message is to elicit slot data, Amazon Lex returns the following context
        information:
          ...
          * ``x-amz-lex-slots`` header set to a map of slots configured for the intent with their
          current values
        ...
        * If the message is a clarification prompt configured for the intent, indicating that the
        user intent is not understood, the ``x-amz-dialog-state`` header is set to ``ElicitIntent``
        and the ``x-amz-slot-to-elicit`` header is omitted.

        In addition, Amazon Lex also returns your application-specific ``sessionAttributes`` . For
        more information, see `Managing Conversation Context
        ...
        **Request Syntax**
        ...
        :param userId: **[REQUIRED]**
          ...
          The ID of the client application user. Amazon Lex uses this to identify a user's
          conversation with your bot. At runtime, each request must contain the ``userID`` field.

          To decide the user ID to use for your application, consider the following factors.

          * The ``userID`` field must not contain any personally identifiable information of the
          user, for example, name, personal identification numbers, or other end user personal
          information.
        ...
        :param sessionAttributes:
          ...
          Application-specific information passed between Amazon Lex and a client application. The
          value must be a JSON serialized and base64 encoded map with string keys and values. The
          total size of the ``sessionAttributes`` and ``requestAttributes`` headers is limited to 12
          KB.
        ...
        :param requestAttributes:
          ...
          Request-specific information passed between Amazon Lex and a client application. The value
          must be a JSON serialized and base64 encoded map with string keys and values. The total
          size of the ``requestAttributes`` and ``sessionAttributes`` headers is limited to 12 KB.
        ...
        :param accept:
          ...
          * If the value begins with ``audio/`` , Amazon Lex returns speech in the response. Amazon
          Lex uses Amazon Polly to generate the speech (using the configuration you specified in the
          ``Accept`` header). For example, if you specify ``audio/mpeg`` as the value, Amazon Lex
        ...
        :param inputStream: **[REQUIRED]**
          ...
          You can stream audio data to Amazon Lex or you can create a local buffer that captures all
          of the audio data before sending. In general, you get better performance if you stream
          ...
          **Response Syntax**
          ...
          ::

            {
                ...
                'messageFormat': 'PlainText'|'CustomPayload'|'SSML'|'Composite',
                'dialogState':
                'ElicitIntent'|'ConfirmIntent'|'ElicitSlot'|'Fulfilled'
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **slots** (JSON serializable) --
              ...
              that it returns is determined by the ``valueSelectionStrategy`` selected when the slot
              type was created or updated. If ``valueSelectionStrategy`` is set to
              ``ORIGINAL_VALUE`` , the value provided by the user is returned, if the user value is
              similar to the slot values. If ``valueSelectionStrategy`` is set to ``TOP_RESOLUTION``
              Amazon Lex returns the first value in the resolution list or, if there is no
              resolution list, null. If you don't specify a ``valueSelectionStrategy`` , the default
              is ``ORIGINAL_VALUE`` .
            ...
            - **message** *(string) --*
              ...
              returned ``Delegate`` as the ``dialogAction.type`` in its response, Amazon Lex decides
              on the next course of action and selects an appropriate message from the bot's
              ...
              assigned to groups Amazon Lex returns one message from each group in the response. The
              message field is an escaped JSON string containing the messages. For more information
              about the structure of the JSON string returned, see  msg-prompts-formats .
            ...
            - **messageFormat** *(string) --*
              ...
              * ``Composite`` - The message contains an escaped JSON object containing one or more
              messages from the groups that messages were assigned to when the intent was created.

            - **dialogState** *(string) --*

              Identifies the current state of the user interaction. Amazon Lex returns one of the
              following values as ``dialogState`` . The client can optionally use this information
              to customize the user interface.

              * ``ElicitIntent`` - Amazon Lex wants to elicit the user's intent. Consider the
              following examples:  For example, a user might utter an intent ("I want to order a
              pizza"). If Amazon Lex cannot infer the user intent from this utterance, it will
              return this dialog state.

              * ``ConfirmIntent`` - Amazon Lex is expecting a "yes" or "no" response.  For example,
              Amazon Lex wants user confirmation before fulfilling an intent. Instead of a simple
              "yes" or "no" response, a user might respond with additional information. For example,
              "yes, but make it a thick crust pizza" or "no, I want to order a drink." Amazon Lex
              can process such additional information (in these examples, update the crust type slot
              or change the intent from OrderPizza to OrderDrink).
              ...
              * ``Failed`` - Conveys that the conversation with the user failed.  This can happen
              for various reasons, including that the user does not provide an appropriate response
              to prompts from the service (you can configure how many times Amazon Lex can prompt a
            ...
            - **inputTranscript** *(string) --*
              ...
              recognize intents and slot values. You can use this information to determine if Amazon
              Lex is correctly processing the audio that you send.

            - **audioStream** (:class:`.StreamingBody`) --

              The prompt (or statement) to convey to the user. This is based on the bot
              configuration and context. For example, if Amazon Lex did not understand the user
              intent, it sends the ``clarificationPrompt`` configured for the bot. If the intent
              ...
              successfully fulfilled the intent, and sent a message to convey to the user. Then
    ...
    def post_text(
        ...
        * After the user replies to a confirmation prompt with a "yes", Amazon Lex might return a
        ...
        addition to the ``message`` , Amazon Lex provides additional context about the message in
        the response that you might use to enhance client behavior, for example, to display the
        appropriate client user interface. These are the ``slotToElicit`` , ``dialogState`` ,
        ``intentName`` , and ``slots`` fields in the response. Consider the following examples:
        ...
        * If the message is a clarification prompt (configured for the intent) that indicates that
        user intent is not understood, the ``dialogState`` is set to ElicitIntent and
        ``slotToElicit`` is set to null.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **message** *(string) --*
              ...
              If the intent is not configured with a Lambda function, or if the Lambda function
              returned ``Delegate`` as the ``dialogAction.type`` its response, Amazon Lex decides on
              the next course of action and selects an appropriate message from the bot's
              configuration based on the current interaction context. For example, if Amazon Lex
              isn't able to understand user input, it uses a clarification prompt message.
            ...
            - **dialogState** *(string) --*
              ...
              * ``ElicitIntent`` - Amazon Lex wants to elicit user intent.  For example, a user
              might utter an intent ("I want to order a pizza"). If Amazon Lex cannot infer the user
              ...
              * ``ElicitSlot`` - Amazon Lex is expecting a slot value for the current intent.  For
              example, suppose that in the response Amazon Lex sends this message: "What size pizza
              would you like?". A user might reply with the slot value (e.g., "medium"). The user
              might also provide additional information in the response (e.g., "medium thick crust
              ...
              * ``Failed`` - Conveys that the conversation with the user failed.  This can happen
              for various reasons including that the user did not provide an appropriate response to
              prompts from the service (you can configure how many times Amazon Lex can prompt a
              user for specific information), or the Lambda function failed to fulfill the intent.
            ...
            - **responseCard** *(dict) --*
              ...
              Card can come from the bot configuration (in the Amazon Lex console, choose the
              settings button next to a slot) or from a code hook (Lambda function).
    ...
    def put_session(
        ...
        **Request Syntax**
        ...
        :param recentIntentSummaryView:
          ...
          checkpoint label on an intent and modify attributes of intents. You can also use it to
          remove or add intent summary objects to the list.
          ...
          * ``slotToElict``
          ...
          contents of the new summary view replaces the old summary view. For example, if a
          ``GetSession`` request returns three intents in the summary view and you call
          ``PutSession`` with one intent in the summary view, the next call to ``GetSession`` will
          only return one intent.

          - *(dict) --*

            Provides information about the state of an intent. You can use this information to get
            the current state of an intent so that you can process the intent, or so that you can
            return the intent to its previous state.
            ...
            - **confirmationStatus** *(string) --*
              ...
              user confirms the intent, Amazon Lex sets this field to ``Confirmed`` . If the user
              denies the intent, Amazon Lex sets this value to ``Denied`` . The possible values are:
        ...
        :param accept:
          ...
          * If the value begins with ``audio/`` , Amazon Lex returns speech in the response. Amazon
          Lex uses Amazon Polly to generate the speech in the configuration that you specify. For
          example, if you specify ``audio/mpeg`` as the value, Amazon Lex returns speech in the MPEG
          format.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **dialogState** *(string) --*
              ...
              * ``ConfirmIntent`` - Amazon Lex is expecting a "yes" or "no" response to confirm the
              intent before fulfilling an intent.

              * ``ElicitIntent`` - Amazon Lex wants to elicit the user's intent.

              * ``ElicitSlot`` - Amazon Lex is expecting the value of a slot for the current intent.

              * ``Failed`` - Conveys that the conversation with the user has failed. This can happen
              for various reasons, including the user does not provide an appropriate response to
              prompts from the service, or if the Lambda function fails to fulfill the intent.
