...
def import_auth():
 try:
  ...
  raise aQcnm('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=aQcnK)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=aQcnM)
def get_params(args,names=[],mandatory=aQcnK):
 result=()
 for name in names:
  value=aQcnv
  if name in args:
  ...
  if not value and mandatory:
   raise aQcnm('Please provide %s=... parameter'%name)
  result+=(value)
 if aQcnN(result)==1:
  return result[0]
 return result
