...
def import_auth():
 try:
  ...
  raise JdXAU('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=JdXAp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=JdXAB)
def get_params(args,names=[],mandatory=JdXAp):
 result=()
 for name in names:
  value=JdXAI
  if name in args:
  ...
  if not value and mandatory:
   raise JdXAU('Please provide %s=... parameter'%name)
  result+=(value)
 if JdXAn(result)==1:
  return result[0]
 return result
