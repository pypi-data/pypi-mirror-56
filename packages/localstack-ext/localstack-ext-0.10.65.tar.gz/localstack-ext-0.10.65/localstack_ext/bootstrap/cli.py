...
def import_auth():
 try:
  ...
  raise vEGsl('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=vEGsq)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=vEGsW)
def get_params(args,names=[],mandatory=vEGsq):
 result=()
 for name in names:
  value=vEGsf
  if name in args:
  ...
  if not value and mandatory:
   raise vEGsl('Please provide %s=... parameter'%name)
  result+=(value)
 if vEGsR(result)==1:
  return result[0]
 return result
