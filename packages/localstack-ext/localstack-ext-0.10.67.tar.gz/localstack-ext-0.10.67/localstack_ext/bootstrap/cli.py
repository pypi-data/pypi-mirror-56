...
def import_auth():
 try:
  ...
  raise knQMl('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=knQMw)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=knQMo)
def get_params(args,names=[],mandatory=knQMw):
 result=()
 for name in names:
  value=knQMp
  if name in args:
  ...
  if not value and mandatory:
   raise knQMl('Please provide %s=... parameter'%name)
  result+=(value)
 if knQME(result)==1:
  return result[0]
 return result
