...
def import_auth():
 try:
  ...
  raise iyxoF('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=iyxoY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=iyxod)
def get_params(args,names=[],mandatory=iyxoY):
 result=()
 for name in names:
  value=iyxoU
  if name in args:
  ...
  if not value and mandatory:
   raise iyxoF('Please provide %s=... parameter'%name)
  result+=(value)
 if iyxob(result)==1:
  return result[0]
 return result
