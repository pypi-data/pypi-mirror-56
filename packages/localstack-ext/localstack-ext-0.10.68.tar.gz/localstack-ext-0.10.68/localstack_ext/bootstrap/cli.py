...
def import_auth():
 try:
  ...
  raise dAFrn('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=dAFrE)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=dAFrp)
def get_params(args,names=[],mandatory=dAFrE):
 result=()
 for name in names:
  value=dAFra
  if name in args:
  ...
  if not value and mandatory:
   raise dAFrn('Please provide %s=... parameter'%name)
  result+=(value)
 if dAFrl(result)==1:
  return result[0]
 return result
