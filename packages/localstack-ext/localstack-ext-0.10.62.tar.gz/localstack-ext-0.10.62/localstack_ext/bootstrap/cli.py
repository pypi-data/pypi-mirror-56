...
def import_auth():
 try:
  ...
  raise vJCRX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=vJCRg)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=vJCRa)
def get_params(args,names=[],mandatory=vJCRg):
 result=()
 for name in names:
  value=vJCRO
  if name in args:
  ...
  if not value and mandatory:
   raise vJCRX('Please provide %s=... parameter'%name)
  result+=(value)
 if vJCRp(result)==1:
  return result[0]
 return result
