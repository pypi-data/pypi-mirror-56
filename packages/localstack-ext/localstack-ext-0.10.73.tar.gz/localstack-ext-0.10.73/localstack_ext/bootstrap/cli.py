...
def import_auth():
 try:
  ...
  raise DYQyt('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=DYQyT)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=DYQyU)
def get_params(args,names=[],mandatory=DYQyT):
 result=()
 for name in names:
  value=DYQyB
  if name in args:
  ...
  if not value and mandatory:
   raise DYQyt('Please provide %s=... parameter'%name)
  result+=(value)
 if DYQyM(result)==1:
  return result[0]
 return result
