...
def import_auth():
 try:
  ...
  raise RomuD('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RomuN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=Romuf)
def get_params(args,names=[],mandatory=RomuN):
 result=()
 for name in names:
  value=RomuQ
  if name in args:
  ...
  if not value and mandatory:
   raise RomuD('Please provide %s=... parameter'%name)
  result+=(value)
 if RomuW(result)==1:
  return result[0]
 return result
