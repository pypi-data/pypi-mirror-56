...
def import_auth():
 try:
  ...
  raise mlLHr('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=mlLHM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=mlLHN)
def get_params(args,names=[],mandatory=mlLHM):
 result=()
 for name in names:
  value=mlLHf
  if name in args:
  ...
  if not value and mandatory:
   raise mlLHr('Please provide %s=... parameter'%name)
  result+=(value)
 if mlLHs(result)==1:
  return result[0]
 return result
