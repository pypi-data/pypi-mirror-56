...
def import_auth():
 try:
  ...
  raise XnDBu('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=XnDBm)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=XnDBE)
def get_params(args,names=[],mandatory=XnDBm):
 result=()
 for name in names:
  value=XnDBs
  if name in args:
  ...
  if not value and mandatory:
   raise XnDBu('Please provide %s=... parameter'%name)
  result+=(value)
 if XnDBC(result)==1:
  return result[0]
 return result
