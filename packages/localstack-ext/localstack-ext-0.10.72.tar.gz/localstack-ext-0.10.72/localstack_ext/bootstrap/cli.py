...
def import_auth():
 try:
  ...
  raise QBMhn('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=QBMha)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=QBMhc)
def get_params(args,names=[],mandatory=QBMha):
 result=()
 for name in names:
  value=QBMhe
  if name in args:
  ...
  if not value and mandatory:
   raise QBMhn('Please provide %s=... parameter'%name)
  result+=(value)
 if QBMhE(result)==1:
  return result[0]
 return result
