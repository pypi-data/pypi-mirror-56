...
class Icosahedron(BaseParticle):
    ...
    natoms
        ...
        match this value.
    lattice_paramter
        Lattice parameter (side length of the conventional cubic FCC cell).
    chemical_symbol
        The chemical symbol to be used if atomic structure is returned.
    tol
    ...
    `Icosahedron` object::
        ...
        surface_energies = {(1, 1, 1): 1.0, (1, 1, 0): 1.14}
        particle = Icosahedron(surface_energies, twin_energy=0.05)
        particle.view()
    ...
    def __init__(self,
                 ...
                 chemical_symbol: str = 'Au',
                 tol: float = 1e-5):

        prim = bulk(chemical_symbol, crystalstructure='fcc',
                    a=lattice_parameter, cubic=True)
        cubic_symmetries = get_symmetries(prim)

        symmetries = break_symmetry(cubic_symmetries, [(1, 1, 1)])
