...
class Form():
    ...
    @property
    def area(self) -> float:
        """Return the total area of the facets in this form."""
        return len(self.facets) * self.facets[0].area

    @property
    def surface_energy(self) -> float:
        """Return the total surface energy of the facets in this form."""
        return len(self.facets) * self.facets[0].surface_energy

    @property
    def volume(self) -> float:
        """
        Return the total volume formed by pyramids formed by each
...
def setup_forms(surface_energies: Dict,
    ...
    ----------
    surface_energies
        keys are either tuples with three integer (describing a form in the
