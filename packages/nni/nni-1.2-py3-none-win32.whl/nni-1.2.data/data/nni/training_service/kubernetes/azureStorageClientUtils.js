...
(function (AzureStorageClientUtility) {
    async function createShare(fileServerClient, azureShare) {
        ...
        fileServerClient.createShareIfNotExists(azureShare, (error, result, response) => {
            if (error) {
                log_1.getLogger()
    ...
    async function createDirectory(fileServerClient, azureFoler, azureShare) {
        const deferred = new ts_deferred_1.Deferred();
        fileServerClient.createDirectoryIfNotExists(azureShare, azureFoler, (error, result, response) => {
            if (error) {
                log_1.getLogger()
    ...
    async function createDirectoryRecursive(fileServerClient, azureDirectory, azureShare) {
        ...
        for (const directory of directories) {
            ...
            let result = await createDirectory(fileServerClient, rootDirectory, azureShare);
            if (!result) {
                deferred.resolve(false);
                return deferred.promise;
            }
    ...
    async function uploadFileToAzure(fileServerClient, azureDirectory, azureFileName, azureShare, localFilePath) {
        const deferred = new ts_deferred_1.Deferred();
        await fileServerClient.createFileFromLocalFile(azureShare, azureDirectory, azureFileName, localFilePath, (error, result, response) => {
            if (error) {
                log_1.getLogger()
    ...
    async function downloadFile(fileServerClient, azureDirectory, azureFileName, azureShare, localFilePath) {
        const deferred = new ts_deferred_1.Deferred();
        await fileServerClient.getFileToStream(azureShare, azureDirectory, azureFileName, fs.createWriteStream(localFilePath), (error, result, response) => {
            if (error) {
                log_1.getLogger()
    ...
    async function uploadDirectory(fileServerClient, azureDirectory, azureShare, localDirectory) {
        ...
        let result = await createDirectoryRecursive(fileServerClient, azureDirectory, azureShare);
        if (!result) {
            deferred.resolve(false);
            return deferred.promise;
        }
    ...
    async function downloadDirectory(fileServerClient, azureDirectory, azureShare, localDirectory) {
        ...
        fileServerClient.listFilesAndDirectoriesSegmented(azureShare, azureDirectory, 'null', async (error, result, response) => {
