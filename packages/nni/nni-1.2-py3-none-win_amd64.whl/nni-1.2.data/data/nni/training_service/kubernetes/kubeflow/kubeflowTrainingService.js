...
let KubeflowTrainingService = class KubeflowTrainingService extends kubernetesTrainingService_1.KubernetesTrainingService {
    ...
    async uploadCodeFiles(trialJobId, trialLocalTempFolder) {
        ...
        if (this.kubeflowClusterConfig.storage === 'azureStorage') {
            ...
            await cpp.exec(`cp -r ${this.kubeflowTrialConfig.codeDir}/* ${this.trialLocalNFSTempFolder}/nni/${experimentStartupInfo_1.getExperimentId()}/${trialJobId}/.`);
    ...
    async generateKubeflowJobConfig(trialJobId, trialWorkingFolder, kubeflowJobName, workerPodResources, nonWorkerPodResources) {
        ...
        if (this.kubeflowTrialConfig.operatorType === 'tf-operator') {
            ...
            if (tensorflowTrialConfig.ps !== undefined) {
                let privateRegistrySecretName = await this.createRegistrySecret(tensorflowTrialConfig.ps.privateRegistryAuthPath);
            ...
            if (pytorchTrialConfig.worker !== undefined) {
                let privateRegistrySecretName = await this.createRegistrySecret(pytorchTrialConfig.worker.privateRegistryAuthPath);
            ...
            let privateRegistrySecretName = await this.createRegistrySecret(pytorchTrialConfig.master.privateRegistryAuthPath);
