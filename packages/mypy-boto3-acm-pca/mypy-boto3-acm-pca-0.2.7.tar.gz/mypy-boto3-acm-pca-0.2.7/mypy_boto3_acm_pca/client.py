...
class Client(BaseClient):
    ...
    def create_certificate_authority(
        ...
        configuration, the certificate revocation list (CRL) configuration, the CA type, and an optional
        idempotency token to avoid accidental creation of multiple CAs. The CA configuration specifies the
        name of the algorithm and key size to be used to create the CA private key, the type of signing
        algorithm that the CA uses, and X.500 subject information. The CRL configuration specifies the CRL
        expiration period in days (the validity period of the CRL), the Amazon S3 bucket that will contain
        the CRL, and a CNAME alias for the S3 bucket that is included in certificates issued by the CA. If
        ...
        **Request Syntax**
        ::
          ...
          response = client.create_certificate_authority(
              CertificateAuthorityConfiguration={
                  ...
                  'SigningAlgorithm':
                  'SHA256WITHECDSA'|'SHA384WITHECDSA'|'SHA512WITHECDSA'|'SHA256WITHRSA'|'SHA384WITHRSA'
                  |'SHA512WITHRSA',
                  'Subject': {
                      'Country': 'string',
                      'Organization': 'string',
        ...
        :param CertificateAuthorityConfiguration: **[REQUIRED]**
          ...
          - **KeyAlgorithm** *(string) --* **[REQUIRED]**
            ...
            issues a certificate. When you create a subordinate CA, you must use a key algorithm supported
            by the parent CA.
        ...
        :param IdempotencyToken:
          ...
          **CreateCertificateAuthority** . Idempotency tokens time out after five minutes. Therefore, if you
          ...
          minute period, ACM Private CA recognizes that you are requesting only one certificate. As a
          result, ACM Private CA issues only one. If you change the idempotency token for each call,
          however, ACM Private CA recognizes that you are requesting multiple certificates.

        :type Tags: list
        :param Tags:

          Key-value pairs that will be attached to the new private CA. You can associate up to 50 tags with
          a private CA. For information using tags with
          ...
          - *(dict) --*
            ...
            Tags are labels that you can use to identify and organize your private CAs. Each tag consists of
            a key and an optional value. You can associate up to 50 tags with a private CA. To add one or
            more tags to a private CA, call the  TagCertificateAuthority action. To remove a tag, call the
    ...
    def create_certificate_authority_audit_report(
        ...
        """
        Creates an audit report that lists every time that your CA private key is used. The report is saved
        in the Amazon S3 bucket that you specify on input. The  IssueCertificate and  RevokeCertificate
    ...
    def create_permission(
        ...
        """
        Assigns permissions from a private CA to a designated AWS service. Services are specified by their
        service principals and can be given permission to create and retrieve certificates on a private CA.
        Services can also be given permission to list the active permissions that the private CA has
        granted. For ACM to automatically renew your private CA's certificates, you must assign all possible
        permissions from the CA to the ACM service principal.

        At this time, you can only assign permissions to ACM (``acm.amazonaws.com`` ). Permissions can be
        revoked with the  DeletePermission action and listed with the  ListPermissions action.
        ...
        **Request Syntax**
        ...
        :param CertificateAuthorityArn: **[REQUIRED]**
          ...
          calling the  ListCertificateAuthorities action. This must have the following form:
        ...
        :param Principal: **[REQUIRED]**
          ...
          The AWS service or identity that receives the permission. At this time, the only valid principal
    ...
    def delete_certificate_authority(
        ...
        Additionally, you can delete a CA if you are waiting for it to be created (that is, the status of
        the CA is ``CREATING`` ). You can also delete it if the CA has been created but you haven't yet
        imported the signed certificate into ACM Private CA (that is, the status of the CA is
        ``PENDING_CERTIFICATE`` ).

        When you successfully call  DeleteCertificateAuthority , the CA's status changes to ``DELETED`` .
        However, the CA won't be permanently deleted until the restoration period has passed. By default, if
        you do not set the ``PermanentDeletionTimeInDays`` parameter, the CA remains restorable for 30 days.
        You can set the parameter from 7 to 30 days. The  DescribeCertificateAuthority action returns the
        time remaining in the restoration window of a private CA in the ``DELETED`` state. To restore an
        ...
        **Request Syntax**
        ...
        :param PermanentDeletionTimeInDays:
          ...
          The number of days to make a CA restorable after it has been deleted. This can be anywhere from 7
          to 30 days, with 30 being the default.
    ...
    def delete_permission(
        ...
        """
        Revokes permissions that a private CA assigned to a designated AWS service. Permissions can be
        created with the  CreatePermission action and listed with the  ListPermissions action.
        ...
        **Request Syntax**
        ...
        :param Principal: **[REQUIRED]**
          ...
          The AWS service or identity that will have its CA permissions revoked. At this time, the only
    ...
    def describe_certificate_authority(
        ...
        """
        Lists information about your private certificate authority (CA). You specify the private CA on input
        by its ARN (Amazon Resource Name). The output contains the status of your CA. This can be any of the
        following:
        ...
        * ``FAILED`` - Your private CA has failed. Your CA can fail because of problems such a network
        outage or backend AWS failure or other errors. A failed CA can never return to the pending state.
        You must create a new CA.

        * ``DELETED`` - Your private CA is within the restoration period, after which it is permanently
        deleted. The length of time remaining in the CA's restoration period is also included in this
        action's output.
              ...
              - **RevocationConfiguration** *(dict) --*
                ...
                - **CrlConfiguration** *(dict) --*
                  ...
                  - **CustomCname** *(string) --*
                    ...
                    the use of an alias for the CRL distribution point. Use this value if you don't want the
                    name of your S3 bucket to be public.
                  ...
                  - **S3BucketName** *(string) --*
                    ...
                    **CustomCname** argument, the name of your S3 bucket is placed into the **CRL
                    ...
                    your bucket by calling the  UpdateCertificateAuthority action. You must specify a bucket
    ...
    def get_certificate(
        ...
        """
        Retrieves a certificate from your private CA. The ARN of the certificate is returned when you call
        the  IssueCertificate action. You must specify both the ARN of your private CA and the ARN of the
        issued certificate when calling the **GetCertificate** action. You can retrieve the certificate if
        it is in the **ISSUED** state. You can call the  CreateCertificateAuthorityAuditReport action to
        create a report that contains information about all of the certificates issued and revoked by your
        private CA.
        ...
        **Request Syntax**
        ...
        :param CertificateArn: **[REQUIRED]**
          ...
          The ARN of the issued certificate. The ARN contains the certificate serial number and must be in
          the following form:
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **CertificateChain** *(string) --*
              ...
              The base64 PEM-encoded certificate chain that chains up to the on-premises root CA certificate
    ...
    def get_certificate_authority_certificate(
        ...
        """
        Retrieves the certificate and certificate chain for your private certificate authority (CA). Both
        the certificate and the chain are base64 PEM-encoded. The chain does not include the CA certificate.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **CertificateChain** *(string) --*
              ...
              root on-premises certificate that you used to sign your private CA certificate. The chain does
    ...
    def get_certificate_authority_csr(
        ...
        is created when you call the  CreateCertificateAuthority action. Sign the CSR with your ACM Private
        CA-hosted or on-premises root or subordinate CA. Then import the signed certificate back into ACM
        Private CA by calling the  ImportCertificateAuthorityCertificate action. The CSR is returned as a
    ...
    def import_certificate_authority_certificate(
        ...
        """
        Imports a signed private CA certificate into ACM Private CA. This action is used when you are using
        a chain of trust whose root is located outside ACM Private CA. Before you can call this action, the
        following preparations must in place:

        * In ACM Private CA, call the  CreateCertificateAuthority action to create the private CA that that
        ...
        * Sign the CSR using a root or intermediate CA hosted either by an on-premises PKI hierarchy or a
        commercial CA..

        * Create a certificate chain and copy the signed certificate and the certificate chain to your
        working directory.
        ...
        * Your ACM Private CA-hosted or on-premises CA certificate must be the last certificate in your
        ...
        **Request Syntax**
        ...
        :param Certificate: **[REQUIRED]**
          ...
          The PEM-encoded certificate for a private CA. This may be a self-signed certificate in the case of
          a root CA, or it may be signed by another CA that you control.
        ...
        :param CertificateChain:
          ...
          importing, chaining up to your root CA. Your ACM Private CA-hosted or on-premises root certificate
          is the last in the chain, and each certificate in the chain signs the one preceding.

          This parameter must be supplied when you import a subordinate CA. When you import a root CA, there
          is no chain.
    ...
    def issue_certificate(
        ...
        """
        Uses your private certificate authority (CA) to issue a client certificate. This action returns the
        Amazon Resource Name (ARN) of the certificate. You can retrieve the certificate by calling the
        ...
        **Request Syntax**
        ...
        :param Csr: **[REQUIRED]**
          ...
          The certificate signing request (CSR) for the certificate you want to issue. You can use the
          ...
          block in the configuration file contains your X509 version 3 extensions.
           ...
           ``openssl req -new -config openssl_rsa.cnf -extensions usr_cert -newkey rsa:2048 -days -365
           -keyout private/test_cert_priv_key.pem -out csr/test_cert_.csr``
        ...
        :param TemplateArn:
          ...
          not provided, ACM Private CA defaults to the ``EndEntityCertificate/V1`` template.
        ...
        :param IdempotencyToken:
          ...
          requesting only one certificate and will issue only one. If you change the idempotency token for
    ...
    def list_certificate_authorities(
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          Use this parameter when paginating results in a subsequent request after you receive a response
          with truncated results. Set it to the value of the ``NextToken`` parameter from the response you
          just received.
    ...
    def list_permissions(
        ...
        """
        Lists all the permissions, if any, that have been assigned by a private CA. Permissions can be
        granted with the  CreatePermission action and revoked with the  DeletePermission action.
        ...
        **Request Syntax**
        ...
        :param MaxResults:
          ...
          When paginating results, use this parameter to specify the maximum number of items to return in
          the response. If additional items exist beyond the number you specify, the **NextToken** element
          ...
          - *(dict) --*
            ...
            - **Permissions** *(list) --*
              ...
              - *(dict) --*
                ...
                ``ListPermissions`` ). Permissions can be assigned with the  CreatePermission action,
    ...
    def list_tags(
        ...
        """
        Lists the tags, if any, that are associated with your private CA. Tags are labels that you can use
        to identify and organize your CAs. Each tag consists of a key and an optional value. Call the
        TagCertificateAuthority action to add one or more tags to your CA. Call the
          ...
          - *(dict) --*
            ...
            - **Tags** *(list) --*
              ...
              - *(dict) --*
                ...
                To add one or more tags to a private CA, call the  TagCertificateAuthority action. To remove
                a tag, call the  UntagCertificateAuthority action.
    ...
    def restore_certificate_authority(self, CertificateAuthorityArn: str) -> None:
        ...
        the period that you defined in the **PermanentDeletionTimeInDays** parameter of the
        ...
        **RestoreCertificateAuthority** action returns. To change its status to ``ACTIVE`` , call the
        UpdateCertificateAuthority action. If the private CA was in the ``PENDING_CERTIFICATE`` state at
        deletion, you must use the  ImportCertificateAuthorityCertificate action to import a certificate
        authority into the private CA before it can be activated. You cannot restore a CA after the
    ...
    def revoke_certificate(
        ...
        """
        Revokes a certificate that was issued inside ACM Private CA. If you enable a certificate revocation
        list (CRL) when you create or update your private CA, information about the revoked certificates
        will be included in the CRL. ACM Private CA writes the CRL to an S3 bucket that you specify. For
        more information about revocation, see the  CrlConfiguration structure. ACM Private CA also writes
        ...
        **Request Syntax**
        ::
          ...
          response = client.revoke_certificate(
              ...
              RevocationReason=
                  'UNSPECIFIED'|'KEY_COMPROMISE'|'CERTIFICATE_AUTHORITY_COMPROMISE'|'AFFILIATION_CHANGED'
                  |'SUPERSEDED'|'CESSATION_OF_OPERATION'|'PRIVILEGE_WITHDRAWN'|'A_A_COMPROMISE'
          )
        :type CertificateAuthorityArn: string
        :param CertificateAuthorityArn: **[REQUIRED]**

          Amazon Resource Name (ARN) of the private CA that issued the certificate to be revoked. This must
          be of the form:
        ...
        :param CertificateSerial: **[REQUIRED]**
          ...
          retrieve the serial number by calling  GetCertificate with the Amazon Resource Name (ARN) of the
          certificate you want and the ARN of your private CA. The **GetCertificate** action retrieves the
          certificate in the PEM format. You can use the following OpenSSL command to list the certificate
          ...
          <https://docs.aws.amazon.com/acm/latest/APIReference/API_DescribeCertificate.html>`__ action in
    ...
    def tag_certificate_authority(
        ...
        your AWS resources. Each tag consists of a key and an optional value. You specify the private CA on
        input by its Amazon Resource Name (ARN). You specify the tag by using a key-value pair. You can
        apply a tag to just one private CA if you want to identify a specific characteristic of that CA, or
        you can apply the same tag to multiple private CAs if you want to filter for a common relationship
        among those CAs. To remove one or more tags, use the  UntagCertificateAuthority action. Call the
    ...
    def untag_certificate_authority(
        ...
        To add tags to a private CA, use the  TagCertificateAuthority . Call the  ListTags action to see
    ...
    def update_certificate_authority(
        ...
        in the ``ACTIVE`` or ``DISABLED`` state before you can update it. You can disable a private CA that
        is in the ``ACTIVE`` state or make a CA that is in the ``DISABLED`` state active again.
