...
from rqdatac.utils import to_datetime, today_int, to_date
...
HOLDING_FIELDS = ["order_book_id", "type", "date", "weight", "shares", "market_value", "symbol"]


@export_as_api(namespace="fund")
def get_holdings(order_book_id, date=None, market="cn"):
    """获取距离指定日期最近发布的基金持仓信息
    ...
    df = get_client().execute("fund.get_holdings_v2", order_book_id, date, market=market)
    if not df:
        return
    ...
    if 'category' in df.columns:
        ...
        fields = HOLDING_FIELDS + ["category"]
    else:
        fields = HOLDING_FIELDS + ["region"]

    df.sort_values(["date", "type", "order_book_id"], inplace=True)
...
def get_asset_allocation(order_book_id, date=None, market="cn"):
    ...
    df = pd.DataFrame(df, columns=["stock", "bond", "fund", "cash", "nav", "datetime"])
