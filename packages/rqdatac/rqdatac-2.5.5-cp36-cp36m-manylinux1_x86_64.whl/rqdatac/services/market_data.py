...
from rqdatac.utils import pf_fill_nan


@export_as_api
@compatible_with_parm(name="country", value="cn", replace="market")
def get_split(order_book_id, start_date=None, end_date=None, market="cn"):
    """获取拆分信息

    :param order_book_id: 股票 order_book_id or order_book_id list
    :param start_date: 开始日期；默认为上市首日
    :param end_date: 结束日期；默认为今天
    :param market:  (Default value = "cn")

    """
    order_book_ids = ensure_order_book_ids(order_book_id, market=market)
...
@export_as_api
@compatible_with_parm(name="country", value="cn", replace="market")
def get_dividend(order_book_id, start_date=None, end_date=None, adjusted=False, market="cn"):
    """获取分红信息
    ...
    order_book_ids = ensure_order_book_ids(order_book_id)
    if start_date is not None:
        start_date = ensure_date_int(start_date)
    if end_date is not None:
        end_date = ensure_date_int(end_date)
...
def get_ex_factor(order_book_id, start_date=None, end_date=None, market="cn"):
    ...
    :param order_book_id: 如'000001.XSHE'
...
@export_as_api
@compatible_with_parm(name="country", value="cn", replace="market")
def get_yield_curve(start_date="2013-01-04", end_date="2014-01-04", tenor=None, market="cn"):
    """获取国债收益率曲线
