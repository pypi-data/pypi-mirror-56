...
from rqdatac.services.basic import instruments, all_instruments
from rqdatac.services.calendar import get_next_trading_date, get_previous_trading_date
...
def get_tick_price(order_book_id, start_date, end_date, fields, expect_df, market):
    ensure_string(order_book_id, "order_book_id")
    ins = instruments(order_book_id)
    if not ins:
        return
    if ins.type == "Convertible":
    ...
    if ins.type in ("Future", "Option"):
        ...
        change_rate_base_field = "prev_settlement"
    else:
        base_fields = EQUITIES_TICK_FIELDS
        base_columns = EQUITIES_TICK_COLUMNS
        change_rate_base_field = "prev_close"
    ...
    data = get_client().execute("get_tickbar", order_book_id, start_date, end_date, list(gtw_fields), market=market)
    today = today_int()
    df_list = []
    if data:
        data = [(obid, {k: np.frombuffer(*v) for k, v in d.items()}) for obid, d in data]
        for obid, d in data:
            dates = d.pop("date")
            times = d.pop("time")
            d["datetime"] = [_convert_int_to_datetime(dt, tm) for dt, tm in zip(dates, times)]
            date = to_datetime(get_current_trading_date(int17_to_datetime(d["datetime"][-1])))
            df = pd.DataFrame(d)
            df["trading_date"] = date

            if daybar is not None:
                try:
                    last = daybar.loc[date]
                except KeyError:
                    continue
                day_open = last["open"]
                df["open"] = df["volume"].apply(lambda x: day_open if x > 0 else 0.0)
                df["prev_close"] = last["prev_close"]

                if ins.type in ("CS", "Future"):
                    df["limit_up"] = last["limit_up"]
                    df["limit_down"] = last["limit_down"]

                if ins.type in ("Future", "Option"):
                    df["prev_settlement"] = last["prev_settlement"]

                df["change_rate"] = df["last"] / df[change_rate_base_field] - 1
    ...
    if df_list:
        ...
        history_latest_date = date_to_int8(history_latest_date)
    else:
        df = None
    ...
    try:
        live_df = get_ticks(order_book_id, start_date=live_date, end_date=live_date, expect_df=expect_df, market=market)
    except (PermissionDenied, MarketNotSupportError):
        live_df = None
    ...
    live_df["change_rate"] = live_df["last"] / live_df[change_rate_base_field] - 1
    ...
    if isinstance(key, (list, tuple, set, frozenset)):
        ...
        for i in key:
            if i in FIELDS_MAPPING:
                new_key.append(FIELDS_MAPPING[i])
            else:
                new_key.append(i)
        return new_key
...
class HybridDataFrame(pd.DataFrame):
    def __getitem__(self, key):
        try:
            ...
            key = get_key(key)
            ret = super(HybridDataFrame, self).__getitem__(key)
            if isinstance(ret, pd.DataFrame):
                return HybridDataFrame(ret._data)
            return ret

    def __getattr__(self, name):
        try:
            return super(HybridDataFrame, self).__getattr__(name)
        except AttributeError:
            return super(HybridDataFrame, self).__getattr__(get_key(name))

    def __contains__(self, key):
        return super(HybridDataFrame, self).__contains__(get_key(key))

    def to_pickle(self, path, compression='infer', protocol=pickle.HIGHEST_PROTOCOL):
        pd.DataFrame(self._data).to_pickle(path, compression=compression, protocol=protocol)


class HybridPanel(pd.Panel):
    def __getitem__(self, key):
        try:
            return super(HybridPanel, self).__getitem__(key)
        except KeyError:
            key = get_key(key)
            ret = super(HybridPanel, self).__getitem__(key)
    ...
    def __getattr__(self, name):
        try:
            return super(HybridPanel, self).__getattr__(name)
        except AttributeError:
            return super(HybridPanel, self).__getattr__(get_key(name))

    def __contains__(self, key):
        return super(HybridPanel, self).__contains__(get_key(key))

    def minor_xs(self, key):
        result = pd.Panel.minor_xs(self, key)
        return HybridDataFrame(result._data)

    def to_pickle(self, path, compression='infer', protocol=pickle.HIGHEST_PROTOCOL):
        pd.Panel(self._data).to_pickle(path, compression=compression, protocol=protocol)


def compatible_with_camelcase(df_or_pl):
    if isinstance(df_or_pl, pd.Panel):
        warnings.warn(
            "data returned as panel will be deprecated in future, please set 'expect_df=True' so to return dataframe for ongoing usage.")
        return HybridPanel(df_or_pl._data)
    return HybridDataFrame(df_or_pl._data)
