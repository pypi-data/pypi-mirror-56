...
share_fields = {
    ...
    "management_circulation": "a_man_cir_shares",
...
def get_shares(order_book_ids, start_date=None, end_date=None, fields=None, expect_df=False, market="cn"):
    ...
    :param start_date: 开始日期, 如'2013-01-04' (Default value = None)
    :param end_date: 结束日期，如'2014-01-04' (Default value = None)
    :param fields: 如'total', 'circulation_a', 'management_circulation' (Default value = None)
    :param expect_df: 返回 MultiIndex DataFrame (Default value = False)
    :param market: 地区代码，如'cn' (Default value = "cn")
    ...
    df = df[fields]
    if "a_man_cir_shares" in fields:
        idx = pd.IndexSlice
        for order_book_id in order_book_ids:
            df.loc[:, idx["a_man_cir_shares", order_book_id]] = df["a_man_cir_shares"][
                order_book_id
    ...
    if expect_df:
        ...
        df = df.rename(anti_fields, axis=1)
        return df
