...
from rqdatac.services.calendar import get_trading_dates_in_type, get_previous_trading_date
from rqdatac.utils import to_datetime, int8_to_date, int9_to_time, int14_to_datetime, get_tick_value
from rqdatac.client import get_client
from rqdatac.decorators import export_as_api, ttl_cache
from rqdatac.validators import ensure_instruments_in_order, ensure_order_book_ids
...
def current_snapshot(order_book_ids, market="cn"):
    ...
    for ins in _instruments:
        if ins.type == "Future" and ins.order_book_id.endswith(("88", "89")):
            real_contract = _to_real_contract(ins.underlying_symbol, market)
...
class TickObject(object):
    ...
    @property
    def iopv(self):
        return self._data.get("iopv", 0)

    @property
    def prev_iopv(self):
        return self._data.get("prev_iopv", 0)
...
def get_ticks(order_book_id, start_date=None, end_date=None, expect_df=False, market="cn"):
    ...
    is_future = instrument.type == "Future"
    dates = get_trading_dates_in_type(
        start_date or datetime.date.today(),
    ...
    bars = np.array([tuple([get_tick_value(t, f) for f in fields]) for t in chain(*ticks)], dtype=dtype)
...
def current_minute(order_book_ids, skip_suspended=False, fields=None, market="cn"):
    ...
    """
    from rqdatac.services.get_price import classify_order_book_ids, MINBAR_FIELDS, _ensure_fields
    ...
    fields, _ = _ensure_fields(fields, MINBAR_FIELDS, stocks, funds, futures, spots, options, convertibles, indexes, repos)
