...
@export_as_api(namespace="econ")
def get_factors(start_date, end_date, factors, market="cn"):
    start_date, end_date = ensure_date_range(start_date, end_date)
    factors = ensure_list_of_string(factors, "factors")
    data = get_client().execute("econ.get_factors", start_date, end_date, factors, market=market)
    if not data:
        return
...
def get_trading_hours(order_book_id, date=None, expected_fmt="str", frequency="1m", market="cn"):
    ...
    elif ins.type not in ("Future", "Option") or (ins.type == "Option" and ins.underlying_symbol == "510050.XSHG"):
...
def get_quota():
    ...
    :return dict
        ...
        license_type：账户类型(FULL: 付费类型，TRIAL: 试用类型， OTHER: 其他类型)
    """
    return get_client().execute("user.get_quota")
