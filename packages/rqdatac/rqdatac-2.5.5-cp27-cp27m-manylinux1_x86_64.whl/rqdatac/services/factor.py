...
def get_factor(order_book_ids, factor, start_date=None, end_date=None, universe=None, expect_df=False, **kwargs):
    ...
    data = get_client().execute_with_timeout(
        15 * 60, "get_factor_from_store", order_book_ids, factor, start_date, end_date, universe=universe
    )  # factor real time calc server is too slow. so set default timeout to 15 minutes
    if not data:
        return
    ...
    if expect_df or len(factor) > 1:
        ...
        for order_book_id, factor_name, values in data:
            ...
            slice_ = slice(order_book_id_index * len(dates), (order_book_id_index + 1) * len(dates), None)
...
def get_factor_return(
    start_date, end_date, factors=None, universe="whole_market", method="implicit", market="cn",
    **kwargs
):
    """获取因子收益率数据
    ...
    industry_mapping = kwargs.get('industry_mapping', True)
    ...
    if not df:
        return None
    df = pd.DataFrame(df).set_index("date").sort_index()
    return df.pivot(index=df.index, columns="factor")[universe]
...
@export_as_api
def get_factor_exposure(order_book_ids, start_date=None, end_date=None, factors=None, market="cn",
                        **kwargs):
    """获取因子暴露度
    ...
    if len(no_data_book_id) is not 0:
