...
def index_components(index_name, date=None, start_date=None, end_date=None, market="cn"):
    """获取指数成分
    :param index_name: 指数 id
    ...
    """
    index_name = ensure_order_book_id(index_name)
    ...
    if start_date:
        ...
        data = get_client().execute("index_components_between", index_name, start_date, end_date, market=market)
        if not data:
            return
        return {int8_to_datetime(d["trade_date"]): d["component_ids"] for d in data}

    if date:
        date = ensure_date_int(date)
    return get_client().execute("index_components", index_name, date, market=market)


@export_as_api
def index_weights(index_name, date=None, market="cn"):
    """获取指数的权重

    :param index_name: 如'000300.XSHG'和'000905.XSHG'
