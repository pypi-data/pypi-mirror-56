...
@export_as_api
def get_dominant_future(underlying_symbol, start_date=None, end_date=None, rule=0, market="cn"):
    import warnings

    msg = "'get_dominant_future' is deprecated, please use 'futures.get_dominant' instead"
    warnings.warn(msg, category=DeprecationWarning, stacklevel=2)
    return get_dominant(underlying_symbol, start_date, end_date, rule, market)



@export_as_api(namespace='futures')
def get_dominant(underlying_symbol, start_date=None, end_date=None, rule=0, market="cn"):
    """获取指定期货品种当日对应的主力合约
...
def get_member_rank(obj, trading_date=None, rank_by='volume', **kwargs):
    ...
    df = pd.DataFrame(ret).set_index('trading_date').sort_values(by=['trading_date', 'rank'])
    return df
