...
_ClientAddTagsResponseTypeDef = TypedDict(
    "_ClientAddTagsResponseTypeDef", {"ResourceId": str, "ResourceType": str}, total=False
)
...
class ClientCreateDataSourceFromRdsRDSDataTypeDef(
    ...
    * SecurityInfo - The security information to use to access an RDS DB instance. You need to set
    up appropriate ingress rules for the security entity IDs provided to allow access to the Amazon
    RDS instance. Specify a [``SubnetId`` , ``SecurityGroupIds`` ] pair for a VPC-based RDS DB
    instance.
    ...
    * DataRearrangement - A JSON string that represents the splitting and rearrangement requirements
    ...
    - **DataRearrangement** *(string) --*
      ...
      * **``percentBegin``**  Use ``percentBegin`` to indicate the beginning of the range of the
      data used to create the Datasource. If you do not include ``percentBegin`` and ``percentEnd``
      ...
      * **``complement``**  The ``complement`` parameter instructs Amazon ML to use the data that is
      ...
      the following two datasources do not share any data, and can be used to train and evaluate a
      model. The first datasource has 25 percent of the data, and the second one has 75 percent of
      the data. Datasource for evaluation: ``{"splitting":{"percentBegin":0, "percentEnd":25}}``
      ...
      * **``strategy``**  To change how Amazon ML splits the data for a datasource, use the
      ...
      input data. The following two ``DataRearrangement`` lines are examples of sequentially ordered
      ...
      "complement":"true"}}``  To randomly split the input data into the proportions indicated by
      the percentBegin and percentEnd parameters, set the ``strategy`` parameter to ``random`` and
      provide a string that is used as the seed value for the random data splitting (for example,
      you can use the S3 path to your data as the random seed string). If you choose the random
      split strategy, Amazon ML assigns each row of data a pseudo-random number between 0 and 100,
      and then selects the rows that have an assigned number between ``percentBegin`` and
      ``percentEnd`` . Pseudo-random numbers are assigned using both the input seed string value and
      the byte offset as a seed, so changing the data results in a different split. Any existing
      ordering is preserved. The random splitting strategy ensures that variables in the training
      and evaluation data are distributed similarly. It is useful in the cases where the input data
      may have an implicit sort order, which would otherwise result in training and evaluation
      datasources containing non-similar data records. The following two ``DataRearrangement`` lines
      are examples of non-sequentially ordered training and evaluation datasources: Datasource for
      evaluation: ``{"splitting":{"percentBegin":70, "percentEnd":100, "strategy":"random",
    ...
    - **DataSchema** *(string) --*
      ...
      }, { "fieldName": "F7", "fieldType": "WEIGHTED_INT_SEQUENCE" }, { "fieldName": "F8",
      "fieldType": "WEIGHTED_STRING_SEQUENCE" } ],
    ...
    - **ServiceRole** *(string) --* **[REQUIRED]**
      ...
      templates <http://docs.aws.amazon.com/datapipeline/latest/DeveloperGuide/dp-iam-roles.html>`__
      for data pipelines.
    ...
    - **SecurityGroupIds** *(list) --* **[REQUIRED]**
      ...
      appropriate ingress rules set up to allow access to the RDS DB instance. This attribute is
      used by Data Pipeline to carry out the copy operation from Amazon RDS to an Amazon S3 task.
...
_ClientDeleteTagsResponseTypeDef = TypedDict(
    "_ClientDeleteTagsResponseTypeDef", {"ResourceId": str, "ResourceType": str}, total=False
)
...
class ClientDescribeBatchPredictionsResponseResultsTypeDef(
    ...
    - **BatchPredictionId** *(string) --*
      ...
      The ID assigned to the ``BatchPrediction`` at creation. This value should be identical to the
...
class ClientDescribeDataSourcesResponseResultsRDSMetadataTypeDef(
    ...
    - **ResourceRole** *(string) --*
      ...
      The role (DataPipelineDefaultResourceRole) assumed by an Amazon EC2 instance to carry out the
    ...
    - **DataPipelineId** *(string) --*
      ...
      Amazon S3. You can use the ID to find details about the instance in the Data Pipeline console.
    """
...
class ClientDescribeDataSourcesResponseResultsRedshiftMetadataRedshiftDatabaseTypeDef(
    ...
    """
    Type definition for `ClientDescribeDataSourcesResponseResultsRedshiftMetadata`
    `RedshiftDatabase`
...
class ClientDescribeDataSourcesResponseResultsTypeDef(
    ...
    - **DataLocationS3** *(string) --*
      ...
      The location and name of the data in Amazon Simple Storage Service (Amazon S3) that is used by
      a ``DataSource`` .
...
class ClientDescribeEvaluationsResponseResultsTypeDef(
    ...
    - **InputDataLocationS3** *(string) --*
      ...
      The location and name of the data in Amazon Simple Storage Server (Amazon S3) that is used in
      the evaluation.
...
class ClientDescribeMlModelsResponseResultsTypeDef(_ClientDescribeMlModelsResponseResultsTypeDef):
    ...
    - **Status** *(string) --*
      ...
      * ``FAILED`` - The request to create an ``MLModel`` didn't run to completion. The model isn't
      usable.
    ...
    - **TrainingParameters** *(dict) --*
      ...
      * ``sgd.l1RegularizationAmount`` - The coefficient regularization L1 norm, which controls
      ...
      specifying a small value, such as ``1.0E-08`` . The value is a double that ranges from ``0``
      to ``MAX_DOUBLE`` . The default is to not use L1 normalization. This parameter can't be used
    ...
    - **MLModelType** *(string) --*
      ...
      * ``BINARY`` - Produces one of two possible results. For example, "Is this a child-friendly
      web site?".
...
_ClientDescribeTagsResponseTypeDef = TypedDict(
    ...
    {"ResourceId": str, "ResourceType": str, "Tags": List[ClientDescribeTagsResponseTagsTypeDef]},
    total=False,
)
...
class ClientPredictResponsePredictionTypeDef(_ClientPredictResponsePredictionTypeDef):
    ...
    * ``PredictedValue`` - Present for a ``REGRESSION``  ``MLModel`` request.
    ...
    - **details** *(dict) --* Provides any additional details regarding the prediction.
      ...
      Indicates the type of the ``MLModel`` . ``Algorithm`` - Indicates the algorithm that was used
      for the ``MLModel`` .
...
class DescribeDataSourcesPaginateResponseResultsRedshiftMetadataRedshiftDatabaseTypeDef(
    ...
    """
    Type definition for `DescribeDataSourcesPaginateResponseResultsRedshiftMetadata`
    `RedshiftDatabase`
