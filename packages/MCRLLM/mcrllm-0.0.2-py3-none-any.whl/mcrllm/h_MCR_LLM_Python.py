...
from init import nKmeansInit, KmeansInit
from sklearn import decomposition
from sklearn import datasets
from sklearn.cluster import KMeans
from mpl_toolkits.mplot3d import Axes3D
from SI_File_Toolbox import normalize
#from Functions.SI_File_Toolbox import dataBin
...
class h_MCR_LLM:
    ...
    def h_mcr_llm(cls, nb_i, min_pixels, max_pixels, Max_level, Xraw):
        ...
        print("\n### LEVEL 0 ###")
        init = nKmeansInit.initialisation(X, 2) 
        [C,S] = HyperspectralSegmentation_MCR_LLM.mcr_llm(Xraw, 2, init, nb_i) 
        ...
        for level in range(Max_level):
                ...
                for i in range (len(all_p)):
                    ...
                    if all_p[i].shape[0] >= max_pixels: # Si le jeu i contient assez de spectre, on le sépare en deux
                        ...
                        init = KmeansInit.initialisation(X[all_p[i],:], 2, n_init=1) 
                        [C,S] = HyperspectralSegmentation_MCR_LLM.mcr_llm(Xraw[all_p[i],:], 2, init, nb_i)            
