...
from .forms import UserForm, UserProfileParentsForm, UserAvatarForm, UserSearchForm, profile_inline_formset, \
    ...
    UserLevelUpRecordUpdateForm, UserLevelUpRecordSearchForm, CustomUserLevelUpRecordUpdateForm
...
class StatusSave(TemplateView):
    def post(self, request, *args, **kwargs):
        ...
        _info = '【' + self.request.user.first_name + "】于【" + localtime().strftime(
            "%Y-%m-%d %H:%M:%S") + "】操作"
        update_user_active_pause_signal.send(sender=UserProfile, user=user, active=is_active, pause=is_pause,info=_info)
...
class CustomLevelRecrodApply(LevelRecrodUpdate):
    model = UserLevelUpRecord
    form_class = CustomUserLevelUpRecordUpdateForm
    ...
    def form_valid(self, form):
        ...
        if not record.status in [-1, 2]:
            messages.error(self.request, '已经提交过申请，请等待审核结果')
            return redirect(reverse("bee_django_user:custom_level_record_apply", kwargs=self.kwargs))
    ...
    def get_success_url(self):
        return reverse("bee_django_user:custom_level_record_apply", kwargs=self.kwargs)
