...
class UserProfile(models.Model):
    ...
    def get_class_list(self):
        ...
        if self.is_role("agent"):
            filter1 = Q(user_class___agent=self)
        else:
            filter1 = Q(pk=None)
        # 讲师
        if self.is_role("lecturer"):
            filter2 = Q(user_class__lecturer=self)
        else:
            filter2 = Q(pk=None)
        # 助教
        if self.is_role("mentor"):
            filter3 = Q(user_class__assistant__userprofile=self)
        else:
            filter3 = Q(pk=None)
...
USER_LEVEL_UP_STATUS_CHOICES = ((-1, "未申请"), (-2, "已申请"), (1, '通过'), (2, '未通过'), (3, '关闭'))
...
class UserLevelUpRecord(models.Model):
    ...
    level = models.ForeignKey("bee_django_user.UserLevel", related_name='bee_django_user_level', null=True,
                              on_delete=models.SET_NULL, verbose_name='顺序')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    # 可以通过的状态
    def can_chenge_status(self):
        if self.status in [-1, -2, 2, 3]:
            return True

    # 变更状态
    # _type：create/apply/pass/reject/close/open
    @transaction.atomic
