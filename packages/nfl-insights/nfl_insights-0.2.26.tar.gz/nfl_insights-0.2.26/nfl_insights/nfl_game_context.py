...
class NFLGameContext:
    ...
    def _get_sentences(self, context, threshold=0.5):
        ...
        for teamPref in ['pos', 'def']:
            ...
            self._sentencesDF['isin'] = self._sentencesDF.apply(lambda x: True if len(x['Dims'])==0 or len(x['Dims'] & _dimset) > 0 else False, axis=1)
            _df = self._sentencesDF.query('isin & (teamName == "{}") & (statObject == "{}") & (Interest>{})'.format(_teamName, _statObject, threshold)).copy()
        ...
        logger.debug('Sentences table:\n%s', _filteredSentencesDF)
        _ret = list()
