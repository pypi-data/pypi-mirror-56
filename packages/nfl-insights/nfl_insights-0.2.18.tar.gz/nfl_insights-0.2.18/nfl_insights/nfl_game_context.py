...
class NFLGameContext:
    ...
    def _get_sentences(self, context, threshold=0.5):
        ...
        teams = str([context['Home Team'], context['Away Team']])
        _df = self._sentencesDF.query('isin & (Team in {}) & (Interest>{})'.format(teams, threshold))
        return _df.shape[0], (_df.reset_index().to_html(columns=['Interest', 'Dims', 'Team', 'Text'], justify='left'))
    ...
    def get_context(self, sentences=True) -> dict:
        ...
        context['Away Team'] = self.teams[self._pbpContext._playContext.awayTeam]['teamFullname']
        context['Home Team'] = self.teams[self._pbpContext._playContext.homeTeam]['teamFullname']
        context['Score'] = self._pbpContext._playContext.score
        context['Play-Type'] = self._pbpContext._playContext.lastPlayType
        ...
        # get the sentences
        if sentences:
            _len, _sentences = self._get_sentences(context)
            context['Num sentences (over 0.5)'] = _len
