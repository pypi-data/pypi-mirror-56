...
class Client(BaseClient):
    ...
    def create_event_bus(
        ...
        use to receive events from your own custom applications and services, or it can be a partner
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          ``default`` for a custom event bus because this name is already used for your account's
          default event bus.

          If this is a partner event bus, the name must exactly match the name of the partner event
          source that this event bus is matched to. This name will include the ``/`` character.

        :type EventSourceName: string
        :param EventSourceName:

          If you're creating a partner event bus, this specifies the partner event source that the
    ...
    def create_partner_event_source(
        ...
        Each partner event source can be used by one AWS account to create a matching partner event
        bus in that AWS account. A SaaS partner must create one partner event source for each AWS
        ...
        * For *event_namespace* , we recommend that partners use a string that identifies the AWS
        customer within the partner's system. This should not be the customer's AWS account ID.

        * *event_name* is determined by the partner, and should uniquely identify an
        event-generating resource within the partner system. This should help AWS customers decide
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          this partner event source must create a partner event bus with a name that matches the
          name of the partner event source.

        :type Account: string
        :param Account: **[REQUIRED]**

          The AWS account ID of the customer who is permitted to create a matching partner event bus
    ...
    def deactivate_event_source(self, Name: str) -> None:
        """
        An AWS customer uses this operation to temporarily stop receiving events from the specified
        partner event source. The matching event bus isn't deleted.

        When you deactivate a partner event source, the source goes into ``PENDING`` state. If it
        remains in ``PENDING`` state for more than two weeks, it's deleted.
    ...
    def delete_event_bus(self, Name: str) -> None:
        ...
        event bus are also deleted. You can't delete your account's default event bus.
    ...
    def delete_rule(self, Name: str, EventBusName: str = None, Force: bool = None) -> None:
        ...
        rules are created by those other AWS services to support functionality in those services.
        You can delete these rules using the ``Force`` option, but you should do so only if you're
        sure that the other service isn't still using that rule.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **State** *(string) --*
              ...
              is deactivated. If it's ``DELETED`` , you have created a matching event bus, but the
    ...
    def describe_rule(
        ...
        Describes the specified rule.
         ...
         ``DescribeRule`` doesn't list the targets of a rule. To see the targets associated with a
    ...
    def list_event_buses(
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          also returns a ``NextToken`` that you can use in a subsequent operation to retrieve the
          next set of results.
          ...
          - *(dict) --*
            ...
            - **EventBuses** *(list) --*
              ...
              - *(dict) --*
                ...
                custom event bus can receive rules from AWS services as well as your custom
                applications and services. A partner event bus receives events from an event source
                created by an SaaS partner. These events come from the partners services or
                applications.
            ...
            - **EventSources** *(list) --*
              ...
              - *(dict) --*
                ...
                - **State** *(string) --*
                  ...
                  The state of the event source. If it's ``ACTIVE`` , you have already created a
                  matching event bus for this event source, and that event bus is active. If it's
                  ``PENDING`` , either you haven't yet created a matching event bus, or that event
                  bus is deactivated. If it's ``DELETED`` , you have created a matching event bus,
                  but the event source has since been deleted.
    ...
    def list_partner_event_sources(
        ...
        **Request Syntax**
        ...
        :param Limit:
          ...
          returns a ``NextToken`` that you can use in a subsequent operation to retrieve the next
          set of results.
    ...
    def list_rules(
        ...
        """
        Lists your EventBridge rules. You can either list all the rules or provide a prefix to match
        to the rule names.

         ``ListRules`` doesn't list the targets of a rule. To see the targets associated with a
         rule, use  ListTargetsByRule .
          ...
          - *(dict) --*
            ...
            - **Targets** *(list) --*
              ...
              - *(dict) --*
                ...
                the account ID, you must specify a ``RoleArn`` with proper permissions in the
                ...
                - **Id** *(string) --*
                  ...
                  A name for the target. Use a string that will help you identify the target. Each
                  target associated with a rule must have an ``Id`` unique for that rule.
                ...
                - **Input** *(string) --*
                  ...
                  (JSON) Data Interchange Format <http://www.rfc-editor.org/rfc/rfc7159.txt>`__ .
                ...
                - **InputPath** *(string) --*
                  ...
                  <http://goessner.net/articles/JsonPath/>`__ .
                ...
                - **InputTransformer** *(dict) --*
                  ...
                  - **InputPathsMap** *(dict) --*
                    ...
                    the template in ``InputTemplate`` to produce the output to be sent to the
                    target.

                     ``InputPathsMap`` is an array key-value pairs, where each value is a valid JSON
                     path. You can have as many as 10 key-value pairs. You must use JSON dot
                ...
                - **KinesisParameters** *(dict) --*
                  ...
                  The custom parameter that you can use to control the shard assignment when the
                  target is a Kinesis data stream. If you don't include this parameter, the default
                  ...
                  - **PartitionKeyPath** *(string) --*
                    ...
                    <https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html#partition-key>`__
                ...
                - **EcsParameters** *(dict) --*
                  ...
                  - **LaunchType** *(string) --*
                    ...
                    Amazon ECS
                    <https://docs.aws.amazon.com/AmazonECS/latest/developerguide/AWS-Fargate.html>`__
                  ...
                  - **NetworkConfiguration** *(dict) --*
                    ...
                    - **awsvpcConfiguration** *(dict) --*
                      ...
                      - **SecurityGroups** *(list) --*
                        ...
                        groups. If you don't specify a security group, the default security group
                        for the VPC is used.
                  ...
                  - **PlatformVersion** *(string) --*
                    ...
                    information about valid platform versions, see `AWS Fargate Platform Versions
                ...
                - **BatchParameters** *(dict) --*
                  ...
                  <https://docs.aws.amazon.com/batch/latest/userguide/jobs.html>`__ in the *AWS
                  Batch User Guide* .
    ...
    def put_events(
        ...
        """
        Sends custom events to EventBridge so that they can be matched to rules. These events can be
        ...
        **Request Syntax**
        ...
        :param Entries: **[REQUIRED]**
          ...
          entry such as the source and type of the event, resources associated with the event, and
          so on.
          ...
          - *(dict) --*
            ...
            - **Time** *(datetime) --*
              ...
              <https://www.rfc-editor.org/rfc/rfc3339.txt>`__ . If no timestamp is provided, the
              timestamp of the  PutEvents call is used.
    ...
    def put_partner_events(
        ...
        .. note::
          ...
          AWS customers do not use this operation. Instead, AWS customers can use  PutEvents to
          write custom events from their own applications to an event bus.
    ...
    def put_permission(
        ...
        """
        Running ``PutPermission`` permits the specified AWS account or AWS organization to put
        events to the specified *event bus* . Rules in your account are triggered by these events
        arriving to an event bus in your account.

        For another account to send events to your account, that external account must have a rule
        with your account's event bus as a target.

        To enable multiple AWS accounts to put events to an event bus, run ``PutPermission`` once
        for each of these accounts. Or, if all the accounts are members of the same AWS
        organization, you can run ``PutPermission`` once specifying ``Principal`` as "*" and
        specifying the AWS organization ID in ``Condition`` , to grant permissions to all accounts
        in that organization.

        If you grant permissions using an organization, then accounts in that organization must
        specify a ``RoleArn`` with proper permissions when they use ``PutTarget`` to add your
        account's event bus as a target. For more information, see `Sending and Receiving Events
        Between AWS Accounts
        <https://docs.aws.amazon.com/eventbridge/latest/userguide/eventbridge-cross-account-event-delivery.html>`__
        in the *Amazon EventBridge User Guide* .
        ...
        **Request Syntax**
        ...
        :param Principal: **[REQUIRED]**
          ...
          If you specify "*" without specifying ``Condition`` , avoid creating rules that might
          match undesirable events. To create more secure rules, make sure that the event pattern
          for each rule contains an ``account`` field with a specific account ID to receive events
          from. Rules that have an account field match events sent only from accounts that are
          listed in the rule's ``account`` field.

        :type StatementId: string
        :param StatementId: **[REQUIRED]**

          An identifier string for the external account that you're granting permissions to. If you
        ...
        :param Condition:
          ...
          about AWS Organizations, see `What Is AWS Organizations?
          <https://docs.aws.amazon.com/organizations/latest/userguide/orgs_introduction.html>`__ in
          ...
          The ``Condition`` is a JSON string that must contain ``Type`` , ``Key`` , and ``Value``
          fields.
    ...
    def put_rule(
        ...
        state. You can disable a rule using  DisableRule .
        ...
        services, you can specify whether their events go to your default event bus or a custom
        event bus that you have created. For more information, see  CreateEventBus .

        If you're updating an existing rule, the rule is replaced with what you specify in this
        ``PutRule`` command. If you omit arguments in ``PutRule`` , the old values for those
        arguments aren't kept. Instead, they're replaced with null values.
        ...
        (ARNs). However, EventBridge uses an exact match in event patterns and rules. Be sure to use
        the correct ARN characters when creating event patterns so that they match the ARN syntax in
        the event that you want to match.

        In EventBridge, you could create rules that lead to infinite loops, where a rule is fired
        repeatedly. For example, a rule might detect that ACLs have changed on an S3 bucket, and
        trigger software to change them to the desired state. If you don't write the rule carefully,
        the subsequent change to the ACLs fires the rule again, creating an infinite loop.

        To prevent this, write the rules so that the triggered actions don't refire the same rule.
        ...
        see `Managing Your Costs with Budgets
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          A rule can't have the same name as another rule in the same Region or on the same event
          bus.
    ...
    def put_targets(
        ...
        * The default event bus of another AWS account
        ...
        Creating rules with built-in targets is supported only on the AWS Management Console. The
        ...
        For some target types, ``PutTargets`` provides target-specific parameters. If the target is
        a Kinesis data stream, you can optionally specify which shard the event goes to by using the
        ...
        resource-based policies. For EC2 instances, Kinesis data streams, and AWS Step Functions
        state machines, EventBridge relies on IAM roles that you specify in the ``RoleARN`` argument
        ...
        target of the rules in your account. To send the matched events to the other account,
        ...
        isn't charged. For more information, see `Amazon EventBridge Pricing
        <https://aws.amazon.com/eventbridge/pricing/>`__ .
        ...
        * If none of the following arguments are specified for a target, the entire event is passed
        to the target in JSON format (unless the target is Amazon EC2 Run Command or Amazon ECS
        ...
        * If ``InputPath`` is specified in the form of JSONPath (for example, ``$.detail`` ), only
        ...
        * If ``InputTransformer`` is specified, one or more specified JSONPaths are extracted from
        ...
        happens, ``FailedEntryCount`` is nonzero in the response, and each entry in
        ...
        **Request Syntax**
        ...
        :param Targets: **[REQUIRED]**
          ...
          - *(dict) --*
            ...
            structure. For more information, see `Sending and Receiving Events Between AWS Accounts
            <https://docs.aws.amazon.com/eventbridge/latest/userguide/eventbridge-cross-account-event-delivery.html>`__
            in the *Amazon EventBridge User Guide* .
            ...
            - **InputTransformer** *(dict) --*
              ...
              - **InputTemplate** *(string) --* **[REQUIRED]**
                ...
                the keys from ``InputPathsMap`` to customize the data sent to the target. Enclose
                each ``InputPathsMaps`` value in brackets: <*value* >. The InputTemplate must be
                valid JSON.
            ...
            - **RunCommandParameters** *(dict) --*
              ...
              - **RunCommandTargets** *(list) --* **[REQUIRED]**
                ...
                - *(dict) --*
                  ...
                  this key can specify multiple values.
            ...
            - **EcsParameters** *(dict) --*
              ...
              is an Amazon ECS task. For more information about Amazon ECS tasks, see `Task
              Definitions
              <https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_defintions.html>`__
              in the *Amazon EC2 Container Service Developer Guide* .
            ...
            - **BatchParameters** *(dict) --*
              ...
              <https://docs.aws.amazon.com/batch/latest/userguide/jobs.html>`__ in the *AWS Batch
              User Guide* .
              ...
              - **ArrayProperties** *(dict) --*
                ...
                size can be between 2 and 10,000. If you specify array properties for a job, it
    ...
    def remove_permission(self, StatementId: str, EventBusName: str = None) -> None:
        ...
        event bus. Specify the account to revoke by the ``StatementId`` value that you associated
        with the account when you granted it permission with ``PutPermission`` . You can find the
    ...
    def remove_targets(
        ...
        **Request Syntax**
        ...
        :param Force:
          ...
          If this is a managed rule created by an AWS service on your behalf, you must specify
          ``Force`` as ``True`` to remove targets. This parameter is ignored for rules that aren't
          managed rules. You can check whether a rule is a managed rule by using ``DescribeRule`` or
          ``ListRules`` and checking the ``ManagedBy`` field of the response.
    ...
    def tag_resource(
        ...
        tag values. In EventBridge, rules can be tagged.

        Tags don't have any semantic meaning to AWS and are interpreted strictly as strings of
        characters.

        You can use the ``TagResource`` action with a rule that already has tags. If you specify a
        new tag key for the rule, this tag is appended to the list of tags associated with the rule.
        If you specify a tag key that is already associated with the rule, the new tag value that
    ...
    def untag_resource(self, ResourceARN: str, TagKeys: List[str]) -> Dict[str, Any]:
        """
        Removes one or more tags from the specified EventBridge resource. In EventBridge, rules can
        be tagged.
