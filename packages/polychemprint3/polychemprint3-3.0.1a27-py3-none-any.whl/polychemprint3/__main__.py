...
def io_loadPCP(objType):
    ...
    for objFile in pcpObjFiles:
        try:
            ...
            print('\t%-15s passes syntax check' % objFile)
            objValidDict.update({objFile[:-3]: objDir / objFile})
        except Exception:
            print(Fore.LIGHTRED_EX + '\t' + objFile + "\tfailed syntax check")
    ...
    for objName in objValidDict:
        try:
            ...
            vars()[objName] = obj(__verbose__)
            objDict.update({"S%s" % objNum: vars()[objName]})
            objNum += 1
            print('\t%-15s loaded successfully' % (objName + ".py"))
