...
from .spec import Spec, Device, Message, Signal, Log, Command, Argument, Config, make_sid, decompose_id
...
def default_spec_logs(spec):
    log_error = Log(id=0, name="wrong_log_id", n_args=0, string="Log code was not found")
    spec.add_log(log_error)

    cfg_error = Log(id=1, name="wrong_cfg_id", n_args=0, string="Cfg code was not found")
...
def read_dbc(dbc, json_file, device_config, logger):
    ...
    for message in db.messages:
        ...
        for signal in message.signals:
            ...
            logger.debug("add signal " + str(sig.name) + " " + str(err) + " " + str(sig))
