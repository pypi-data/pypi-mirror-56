...
def check_message(message):
    if len(message.signals.keys()) == 0:
        return False, f"Message without signals {message.name}"
...
def check_signal(key, signal):
    if signal.length > 64:
    ...
    elif not (signal.type == "unsigned" or signal.type == "signed" or signal.type == "double" or signal.type == "float"):
        return False, f"{signal.name}: Unsuported signal type"
    ...
    elif not (signal.byte_order == "little_endian" or signal.byte_order == "big_endian"):
        ...
        return False, f"{signal.name}: signal offset is different than 0 for a scale of 1"
    elif signal.scale != 1 and signal.length > 32:
        return False, f"{signal.name}: scaling for variables bigger than 32 bit is unsuported"
