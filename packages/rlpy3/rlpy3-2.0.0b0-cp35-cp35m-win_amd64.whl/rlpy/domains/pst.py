...
class PST(Domain):
    ...
    **REFERENCE:**
    ...
    def show_domain(self, a=0):
        ...
        if self.domain_fig is None:
            ...
            self.domain_fig = plt.figure(
                1,
                (UAVLocation.SIZE * self.dist_between_locations + 1, self.NUM_UAV + 1),
            )
            plt.show()
        plt.clf()
        ...
        self.subplot_axes = self.domain_fig.add_axes(
            [0, 0, 1, 1], frameon=False, aspect=1.0
        )
        ...
        self.location_rect_vis.append(
            mpatches.Rectangle(
                [crashLocationX, 0], self.LOCATION_WIDTH, self.NUM_UAV * 2, fc="w"
            )
        )
        ...
        self.uav_sensor_vis = [
            mpatches.Wedge(
                (uav_x + self.SENSOR_REL_X, 1 + uav_id), self.SENSOR_LENGTH, -30, 30
            )
            for uav_id in range(0, self.NUM_UAV)
        ]
        self.uav_actuator_vis = [
            mpatches.Wedge(
                (uav_x, 1 + uav_id + self.ACTUATOR_REL_Y), self.ACTUATOR_HEIGHT, 60, 120
            )
            for uav_id in range(0, self.NUM_UAV)
        ]
        ...
        for uav_id in range(0, self.NUM_UAV):
            ...
            self.uav_text_vis[uav_id] = plt.text(uav_x - 0.05, uav_y - 0.05, uav_fuel)
        ...
        numHealthySurveil = np.sum(
            np.logical_and(sStruct.locations == UAVLocation.SURVEIL, sStruct.sensor)
        )
        ...
        if any(sStruct.locations == UAVLocation.COMMS):
            ...
            if numHealthySurveil > 0:
                self.location_rect_vis[len(self.location_rect_vis) - 1].set_color(
                    "green"
                )
    ...
    def step(self, a):
        ...
        fuelBurnedBool = [
            (
                actionVector[i] == UAVAction.LOITER
                and (
                    nsStruct.locations[i] == UAVLocation.REFUEL
                    or nsStruct.locations[i] == UAVLocation.BASE
                )
            )
            for i in range(self.NUM_UAV)
        ]
        fuelBurnedBool = np.array(fuelBurnedBool) == 0.0
        ...
        distanceTraveled = np.sum(np.logical_and(nsStruct.locations, sStruct.locations))

        # Actuator failure transition
        randomFails = np.array(
            [self.random_state.random_sample() for dummy in range(self.NUM_UAV)]
        )
        ...
        self.numHealthySurveil = sum(np.logical_and(surveillanceBool, sStruct.sensor))
    ...
    def possible_actions(self):
        ...
        for uav_id in range(0, self.NUM_UAV):
            ...
            if (
                ...
                or sStruct.locations[uav_id] == UAVLocation.REFUEL
                or sStruct.locations[uav_id] == UAVLocation.BASE
            ):
            ...
            if sStruct.fuel[uav_id] > 0:  # This UAV is not crashed
                ...
                if (
                    sStruct.locations[uav_id] != UAVLocation.SURVEIL
                    and sStruct.actuator[uav_id] == ActuatorState.RUNNING
                ):
