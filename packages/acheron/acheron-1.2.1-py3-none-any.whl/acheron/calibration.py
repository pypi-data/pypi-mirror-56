...
from .unit_selection_dialog import UnitSelectionDialog
from .unit_formatter_spinbox import UnitFormatterDoubleSpinBox

logger = logging.getLogger(__name__)


class CalibrationChannel(QtGui.QWidget, Ui_CalibrationChannel):
    ...
    def linear_capture(self):
        ...
        if math.isfinite(mean) and math.isfinite(std_dev):
            ...
            delete = QtGui.QPushButton(self.tr("Delete"))
...
class CalibrationPanel(QtGui.QGroupBox, Ui_CalibrationPanel):
    ...
    def get_channel_setting_values(self, channel_widget):
        cal = channel_widget.calibration_info

        unit_type = channel_widget.unit_info[0]
        ...
        if cal.resolution_setting_index < len(self.device_info['settings']):
    ...
    def save(self):
        ...
        for _name, channel_widget in self.channel_widgets:
            if channel_widget.calibrationEnabled.isChecked():
                if channel_widget.is_valid():
                    u, f = self.get_channel_setting_values(channel_widget)
                    unit_settings.update(u)
                    float_settings.update(f)

        nvm = bytearray(self.device_info['nvm'])

        for setting_index, value in unit_settings.items():
            setting = self.device_info['settings'][setting_index]
