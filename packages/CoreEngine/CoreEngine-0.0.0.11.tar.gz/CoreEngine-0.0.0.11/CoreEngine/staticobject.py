...
class StaticObject(pygame.sprite.DirtySprite):
    ...
    def get_object_angle(self, entity_2):
        """
...
class SimpleStaticObject(StaticObject):
    """
    simple Static None-animated object
    """

    def __init__(self, pos, size, image):
        StaticObject.__init__(self, pos, size)
        self.image = image
        self.dirty = 2


class ComplexStaticObject(StaticObject):
    def __init__(self, animationset, position, size, startanimation="IDLE"):
        StaticObject.__init__(self, position, size)
        ...
        try:
            ...
            print(err)
            print("Check AnimationSet Syntax with print(CoreEngine.animationSyntax)  or use SimpleStaticObject instead")
            raise SystemExit
