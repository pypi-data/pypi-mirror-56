...
import shutil # structure skeleton
...
if int(pygame.__version__.split(".")[0]) < 2:
    print("[WARNING] pygame 2 is recommend")
...
class VideoPlayer(object):
    ...
    if operating_system == "Windows":
        FFMPEG_BINARY = "binaries/WINDOWS/ffmpeg.exe"  # hard code ffmpeg
        FFPROBE_BINARY = "binaries/WINDOWS/ffprobe.exe"  # hard code ffprobe

    elif operating_system == "Linux":
        FFMPEG_BINARY = "binaries/LINUX/ffmpeg"  # hard code ffmpeg
        FFPROBE_BINARY = "binaries/LINUX/ffprobe"  # hard code ffprobe

    elif operating_system == "Darwin":
        FFMPEG_BINARY = "binaries/MACOSX/ffmpeg"  # hard code ffmpeg
        FFPROBE_BINARY = "binaries/MACOSX/ffprobe"  # hard code ffprobe
    ...
    def __init__(self, filename, path="", position=(0, 0), resolution=None, doVideoResize=True, doVideoConvert=True,
        ...
        if not self._filename.endswith(".mp4"):
            ...
            if not os.path.isfile(os.path.join(self._destination_path,self._filename)):
                shutil.copy(os.path.join(self._path,self._filename),os.path.join(self._destination_path,self._filename))
    ...
    def update(self):
        ...
        if self._video_is_playing:
            if self._hasSound:
                if not self._audio.is_playing and not self._audio.is_muted:
            ...
            if int(self._video_cursor) != self._last_video_cursor:
                ...
                if self._last_video_cursor < self._total_frames:
                    ...
                    if difference > 1:
                        self._pipe.stdout.read(self._frame_buffer_size * (difference - 1))  # skip frames
    ...
    def resize(self, resolution):
        ...
        self._frame_buffer_size = self._frame_buffer_size = resolution[0] * self._resolution[1] * 3
    ...
    def _resize_video(self, size):
        ...
        :return: None
        """
        print("[INFO] Resizing video > ", self._path, self._filename, " > ", size)
    ...
    def _convert_video(self):
        """
        convert video to mp4 to make it easier to access its data
        :return: None
        """
