        ...
        if self.finded == True:
            ...
            if response.status_code == 200:
                ...
                for i in range(len([i for i in self.tree.findall('by/citation')])): # to avoid lxml warning
                    self.works[str(i + 1)] = [
                        self.tree.findall('by/citation|about/citation')[i].find('title').text if self.tree.findall('by/citation|about/citation')[i].find('title').text is not None else 'NA',
                        self.tree.findall('by/citation|about/citation')[i].find('languages').attrib['count'] if self.tree.findall('by/citation|about/citation')[i].find('languages').attrib['count'] is not None else 'NA',
                        self.tree.findall('by/citation|about/citation')[i].find('holdings').text if self.tree.findall('by/citation|about/citation')[i].find('holdings').text is not None else 'NA',
                        self.tree.findall('by/citation|about/citation')[i].find('numEditions').text if self.tree.findall('by/citation|about/citation')[i].find('numEditions').text is not None else 'NA',
                        self.tree.findall('by/citation|about/citation')[i].find('recordType').text if self.tree.findall('by/citation|about/citation')[i].find('recordType').text is not None else 'NA'
                    ]
            else:
