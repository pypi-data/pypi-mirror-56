    ...
    try:
        ...
        while inputOK is False:
            if (calPath is None):
                ...
                calPath = requestDialog("Enter Report Path", "Enter the desired save path for the calibration report. Press Enter to defaut to: ["+calPath+"] :",desiredType = "path", defaultUserInput=os.path.expanduser("~"))
        ...
        while(True):                   
            ...
            if (calAction != None and 'select' in calAction) or (ppmAddress == None):  
                ...
                niceDisplay = True if userMode in "console" else False
                if  niceDisplay: printText("in console mode")
                # Request user to select the (QTL1999) PPM to calibrate
                ppmAddress = userSelectDevice (scanFilterStr = ["QTL1999","QTL1995","QTL1944","QTL2312"], nice=niceDisplay)
