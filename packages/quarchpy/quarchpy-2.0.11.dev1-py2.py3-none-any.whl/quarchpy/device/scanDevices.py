...
def list_network(target_conn="all", debugPring=False, lanTimeout=1, ipAddressLookup=None):
    ...
    if ipAddressLookup is not None:
        ...
        try:
            print("Attempting ipaddress lookup" + ipAddressLookup)
...
def userSelectDevice(scanDictionary=None, scanFilterStr=None,favouriteOnly=True, message=None, title=None, ui="console", nice=False):
    ...
    while (True):
        ...
        if nice:
            ...
            tempList.append(["Rescan", "r"])
            tempList.append(["All Conn Types", "a"])
            tempList.append(["Quit", "q"])


            userStr = listSelection(title, message, tempList, nice=nice)
            ...
            devicesString.append("r=Rescan")
            devicesString.append("a=All connections")
            devicesString.append("q=Quit")

            devicesString = ','.join(devicesString)
            userStr = listSelection("Select a Quarch Device", "Please select a quarch device", devicesString)
