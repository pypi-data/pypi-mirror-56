...
def userSelectDevice(scanDictionary=None, scanFilterStr=None,favouriteOnly=True, message=None, title=None, nice=False, additionalOptions = None):
    #Initiate values. Originals must be stored for the case of a rescan.
    originalOptions = None
    originalOptions = additionalOptions
    originalTableHeaders = None
    ...
    while (True):
        ...
        if nice: #Prepair the data for niceListSelection using displayTable().
            ...
            for option in additionalOptions:
                templist.append([option]*2)
            additionalOptions = templist
            userStr = listSelection(title, message, tempList, additionalOptions=additionalOptions, indexReq=True, nice=nice, tableHeaders=["Serial", "ConnTarget"])
