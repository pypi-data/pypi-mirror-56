...
def importUSB(version = "None"):
    if os.name == 'nt': 
        ...
        if (version is not "None"):
            ...
            if (version is not "64" and version is not "32"):
    ...
    #now that the DLLs are in the memory and the folde ris in the path, import usb1
    import usb1
    return usb1
...
class TQuarchUSB_IF:
    ...
    def ClosePort(self):
        if self.QCmdEPSize> 64: #If this is and Pic32
            self.lastError = ''
            self.isOpen = 0
            return 1
    ...
    #   on error: returns ''
    def RunCommand( self, command ):      #, timeout ):  
        if self.SendCommand( command ) == -1:
            self.lastError = 'Run: Send Fail'
            return ''

        return self.FetchCmdReply()
