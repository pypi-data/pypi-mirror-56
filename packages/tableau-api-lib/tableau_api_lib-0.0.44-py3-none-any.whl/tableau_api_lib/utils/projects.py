...
def get_source_user_df(conn):
    ...
    source_users_df = get_users_df(conn)
...
def get_target_user_df(conn):
    ...
    """
    cols = ['email', 'fullName', 'id']
    target_users_df = get_users_df(conn)
...
def add_target_project_ids(project_details_df, conn_target):
    """
    Join the project details and the target site's project IDs
    :param project_details_df:
    :param conn_target:
    :return:
    """
...
def update_project_hierarchies(project_details_df, conn_target):
    ...
    for index, row in child_projects_df.iterrows():
        print(row)
        print(row['target_project_id'])
        print(row['target_project_parent_id'])
        response = conn_target.update_project(
            project_id=row['target_project_id'],
            parent_project_id=row['target_project_parent_id']
        )
        print('update response: ', response.content)
        print('request body: ', conn_target.active_request)
...
def clone_projects(conn_source, conn_target, project_names=None):
    ...
    project_names = get_project_names(project_details_df, project_names)
    create_projects(project_details_df)
