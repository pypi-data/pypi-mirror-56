...
def _all_outputs(datadriftdetector, start_time, end_time, run_id=None):
    ...
    logger = datadriftdetector._logger

    # Filter based on input params
    if run_id:
        metrics_got = _get_metrics(datadriftdetector, start_time, end_time, run_id, daily_latest_only=False)
        metrics_valid = [m for m in metrics_got if _properties(m.get_extended_properties())[RUN_ID] == run_id]
        if len(metrics_valid) < 1:
            logger.error("No result found for run id {}, please make sure it's a valid run id.".format(run_id))
            raise ValueError("No result found for run id {}, please make sure it's a valid run id.".format(run_id))
        else:
    ...
    # if run_id is None, pick only scheduled runs' results
        ...
        metrics_got = _get_metrics(datadriftdetector, start_time, end_time, run_id, daily_latest_only=True)
        if len(metrics_got) < 1:
            ...
            logger.debug("Filtered in {} records of scheduled/backfill runs.".format(len(metrics_got)))
        ...
        if len(metrics_valid) < 1:
            logger.error("No results found between [{}, {}].".format(start_time, end_time))
            raise ValueError("No results found between [{}, {}].".format(start_time, end_time))
        else:
...
def _get_metrics(datadriftdetector, start_time, end_time, run_id, daily_latest_only=True, with_adhoc=False):
    """
    ...
    if daily_latest_only is True:
        ...
        if len(metrics) > 0:
            ...
            error_msg = "No Scheduled/backfill run outputs found between {} to {}.".format(start_time, end_time)
...
def _download_from_blob_metrics(datadriftdetector, local_temp_root, service, start_time, end_time, run_id, with_adhoc):
    ...
    logger.info("Searching results under blob {} in container {}".format(metrics_rel_base, data_store.container_name))
    metrics_list = []
    ...
    for f in blob_list:
        ...
        if metrics_rel_base in f_name and f_name.endswith(".json"):
            ...
            if (run_id and run_id in f.name) or (not run_id and start_time <= dt <= end_time):
                ...
                metrics_list.append(os.path.join(local_temp_root, *f_name.split('/')))
                # no more check is needed if it's for a specific run id
                if run_id:
                    break
    logger.info("Downloading {} result files under blob {} in container {}".
    ...
    if count > 0:
        logger.debug("Before applying filters, downloaded {} rows of data diff results under path {}."
    ...
    # find nothing, which means no files on blob storage, means outputs were not generated.
        ...
        if drift_type == DATADRIFT_TYPE_DATASET:
            ...
            error_msg += "model base data drift detector {} of model: {} with version: {}.".\
                format(datadrift_id, model_name, model_version)
...
def _show(datadriftdetector, start_time=datetime.min, end_time=datetime.max, with_details=False):
    ...
    metrics = _get_metrics(datadriftdetector, start_time, end_time, run_id=None,
                           daily_latest_only=True, with_adhoc=True)
