...
class OpenEphysNWB(object):
    ...
    def loadBinary(self, pname_root: str, recording_name='recording1'):
        '''
        ...
        '''
        import os
        path2APdata = os.path.join(pname_root, recording_name, 'continuous', 'Neuropix-3a-101.0')
        path2PosData = os.path.join(pname_root, recording_name, 'events', 'Pos_Tracker-100.0', 'BINARY_group_1')
        ...
        settings = Settings(os.path.join(fname_root), 'settings.xml')
        settings.parse()
        self.settings = settings
        sample_rate = 30000
        n_channels = 384
