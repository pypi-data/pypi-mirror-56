...
class CourseTableSync(BaseSync):
    def __init__(self):
        ...
        self.yjs_xq = '第一学期'
        self.xq = '1'
        self.default_subject_name = '公选课'
        self.default_subject_number = b64encode(self.default_subject_name)
    ...
    def read_campus_excel(self):
        ...
        for value in df.ix[:, ['楼名称', '校区']].values:
            self.campus_map[value[0]] = value[1]

    def analyse_building(self, classroom_name):
        building, floor = '教学楼', None
        if classroom_name:
            key_words = ["楼", "区"]
            for kw in key_words:
                if kw in classroom_name:
                    ...
                    try:
                        ...
                        pass
                    break
        campus = self.campus_map.get(building, '本部')
    ...
    def get_yjs_course(self):
        ...
        for row in rows:
            ...
            if not (time_info and classroom_name):
                self.write_wrong_data(row, "no time_info")
                continue
            time_result = self.analyse_sksj(time_info)
            if not time_result:
                self.write_wrong_data(row, "analyse time_info failed")
                continue
            try:
                course_name = classroom_name + subject_number + teacher_number
            except (Exception,):
                self.write_wrong_data(row, "no teacher_number")
                continue
    ...
    def get_bk_course(self):
        ...
        for index in range(total_page):
            ...
            rows = self.cur.fetchall()
            for row in rows:
                subject_number = row[0] or self.default_subject_number
                ...
                if not week_range:
                    self.write_wrong_data(row, "no week_range")
                    continue
                ...
                try:
                    ...
                    self.write_wrong_data(row, "no info")
                    continue
        ...
        for index in range(total_page):
            ...
            for row in rows:
                ...
                try:
                    ...
                    self.write_wrong_data(row, "no num/week/single")
                    print(row)
                    continue
    ...
    def sync(self):
        self.write_wrong_data("Start sync", "Start")
        print(">>>Start course sync")
        t1 = time.time()
        self.read_campus_excel()
        self.get_subjects()
        self.get_bk_course()
    ...
    def write_wrong_data(self, data, reason=None):
        dir_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        file_path = os.path.join(dir_path, "wrong_data.txt")
