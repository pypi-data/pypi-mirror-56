...
def plotSeisILXLZSliceFrom3DMat(seis3dmat, inlsls=None, xlsls=None, zsls=None,
                                ...
                                verbose=True):
    """
    Plot seismic inline, crossline, and z slices from 3D matrix
    Argus:
        seis3dmat:  3D matrix representing seismic data [Z/XL/IL]
        inlsls:     inline No. for plotting
    ...
    # Check input 3D seismic matrix
    if np.ndim(seis3dmat) != 3:
        print('ERROR in plotSeisILXLZSliceFrom3DMat: 3D seismic matrix expected')
        sys.exit()

    if seisinfo is None:
        print('WARNING in plotSeisILXLZSliceFrom3DMat: Survey info auto-generated from 3D seismic matrix')
        seisinfo = seis_ays.createSeisInfoFrom3DMat(seis3dmat)
    ...
    if inlsls is None:
        print('WARNING in plotSeisILXLZSliceFrom3DMat: to plot the middle inline slice in 3D seismic matrix')
        inlsls = inlrange[int(inlnum/2):int(inlnum/2)+1]

    if xlsls is None:
        print('WARNING in plotSeisILXLZSliceFrom3DMat: to plot the middle crossline slice in 3D seismic matrix')
        xlsls = xlrange[int(xlnum/2):int(xlnum/2)+1]

    if zsls is None:
        print('WARNING in plotSeisILXLZSliceFrom3DMat: to plot the middle z slice in 3D seismic matrix')
        zsls = zrange[int(znum/2):int(znum/2)+1]

    if np.ndim(inlsls) != 1:
        print('ERROR in plotSeisILXLZSliceFrom3DMat: 1D array of inline slices expected')
        sys.exit()

    if np.ndim(xlsls) != 1:
        print('ERROR in plotSeisILXLZSliceFrom3DMat: 1D array of crossline slices expected')
        sys.exit()

    if np.ndim(zsls) != 1:
        print('ERROR in plotSeisILXLZSliceFrom3DMat: 1D array of z slices expected')
        sys.exit()
