...
The log module reads-in the mc_logging.json file and sets up logging for Sesh.
...
from config.base_config import MC_LOG_CONFIG, MC_LOG_FILE, MC_LOG_LEVEL
from sesh.error import ExitCode


def _get_logging_config(env_log_level='MC_LOG_LEVEL',
                        env_log_config='MC_LOG_CONFIG',
                        env_log_file='MC_LOG_FILE'):
    """Setup logging configuration."""
    value = os.getenv(env_log_config)
    if value:
        log_config_path = value
    else:
        log_config_path = MC_LOG_CONFIG
    ...
    if value:
        ...
        mc_log_file = MC_LOG_FILE
    logging_config['handlers']['default_handler']['filename'] = mc_log_file
    ...
    logging_config['loggers']['sesh']['level'] = mc_log_level
