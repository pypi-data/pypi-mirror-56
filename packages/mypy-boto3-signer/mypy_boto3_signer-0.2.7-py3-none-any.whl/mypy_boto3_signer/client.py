"Main interface for signer Client"
...
class Client(BaseClient):
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **signingParameters** *(dict) --*
              ...
              Map of user-assigned key-value pairs used during signing. These values contain any information
              that you specified for use in your signing job.
    ...
    def list_signing_jobs(
        ...
        jobs that are returned in the response. If additional jobs remain to be listed, code signing returns
        a ``nextToken`` value. Use this value in subsequent calls to ``ListSigningJobs`` to fetch the
        remaining values. You can continue calling ``ListSigningJobs`` with your ``maxResults`` parameter
        and with new values that code signing returns in the ``nextToken`` parameter until all of your
        ...
        **Request Syntax**
        ...
        :param maxResults:
          ...
          paginating results. If additional items exist beyond the number you specify, the ``nextToken``
          element is set in the response. Use the ``nextToken`` value in a subsequent request to retrieve
          additional items.

        :type nextToken: string
        :param nextToken:

          String for specifying the next set of paginated results to return. After you receive a response
          with truncated results, use this parameter in a subsequent request. Set it to the value of
    ...
    def list_signing_platforms(
        ...
        additional jobs remain to be listed, code signing returns a ``nextToken`` value. Use this value in
        subsequent calls to ``ListSigningJobs`` to fetch the remaining values. You can continue calling
        ``ListSigningJobs`` with your ``maxResults`` parameter and with new values that code signing returns
        in the ``nextToken`` parameter until all of your signing jobs have been returned.
        ...
        **Request Syntax**
        ...
        :param nextToken:
          ...
          Value for specifying the next set of paginated results to return. After you receive a response
          with truncated results, use this parameter in a subsequent request. Set it to the value of
          ``nextToken`` from the response that you just received.
          ...
          - *(dict) --*
            ...
            - **platforms** *(list) --*
              ...
              - *(dict) --*
                ...
                Contains information about the signing configurations and parameters that is used to perform
                a code signing job.
    ...
    def list_signing_profiles(
        ...
        status unless the ``includeCanceled`` request field is set to ``true`` . If additional jobs remain
        to be listed, code signing returns a ``nextToken`` value. Use this value in subsequent calls to
        ``ListSigningJobs`` to fetch the remaining values. You can continue calling ``ListSigningJobs`` with
        your ``maxResults`` parameter and with new values that code signing returns in the ``nextToken``
    ...
    def start_signing_job(
        ...
        ``ListSigningJobs`` operation for two years after they are performed. Note the following
        requirements:

        * You must create an Amazon S3 source bucket. For more information, see `Create a Bucket
        <http://docs.aws.amazon.com/AmazonS3/latest/gsg/CreatingABucket.html>`__ in the *Amazon S3 Getting
        Started Guide* .

        * Your S3 source bucket must be version enabled.

        * You must create an S3 destination bucket. Code signing uses your S3 destination bucket to write
        your signed code.
        ...
        **Request Syntax**
        ...
        :param destination: **[REQUIRED]**
          ...
          - **s3** *(dict) --*
            ...
            - **prefix** *(string) --*
              ...
              An Amazon S3 prefix that you can use to limit responses to those that begin with the specified
              prefix.
        ...
        :param clientRequestToken: **[REQUIRED]**
          ...
          String that identifies the signing request. All calls after the first that use this token return
          the same response as the first call.
    ...
    def tag_resource(self, resourceArn: str, tags: Dict[str, str]) -> Dict[str, Any]:
        ...
        organize your AWS resources. Each tag consists of a key and an optional value. You specify the
        signing profile using its Amazon Resource Name (ARN). You specify the tag by using a key-value pair.
