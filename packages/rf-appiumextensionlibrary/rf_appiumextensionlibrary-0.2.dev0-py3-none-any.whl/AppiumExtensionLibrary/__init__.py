...
class _ApplicationManagementExtendedKeywords(_ApplicationManagementKeywords):
    ...
    def get_window_handles(self):
        """ Retrieves the list of all window handles available to the session (Web context only).  
    ...
    def get_cookies(self):
        """ Retrieves all cookies visible to the current page (Web context only).  
            *AppiumExtensionLibrary Only Keyword*.
        """
    ...
    def add_cookie(self, **kwargs):
        """Set a cookie (Web context only).
           ...
           NOTE: ``name`` and ``value`` arguments are required, ``path``, ``domain``, ``expiry`` and ``secure`` are optional.
           | Set Cookie     | name=_cookieName          | value=someValue       |
           | Set Cookie     | name=_cookieName          | value=someValue       | path=some/path    | domain=.cookie.com    |  
           *AppiumExtensionLibrary Only Keyword*.
        """
        try:
...
class AppiumExtensionLibrary(
    ...
    """AppiumExtensionLibrary is a Mobile App testing library for Robot Framework.
