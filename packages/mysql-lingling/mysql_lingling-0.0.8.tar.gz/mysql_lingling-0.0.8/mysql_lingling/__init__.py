...
def errlog(msg, level=1):
    with open('./log/mysql-err.log', 'a', encoding='utf-8')as f:
        f.write('%s||%s：%s\n' % (
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
            sys._getframe(level).f_code.co_name,  # 执行errlog这个函数的函数名字，即上一级函数
            msg
        ))


def sqllog(sql, args=[]):
    with open('./log/mysql.log', 'a', encoding='utf-8')as f:
        f.write('%s||%s：%s\n' % (
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
...
class MySQLTool(object):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if exc_type:
            e = 'mysql-lingling exit Error:%s' % str(exc_type)
            print(e)
            errlog(e, 2)
        else:
            self.close()
