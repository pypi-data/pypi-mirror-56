...
from deepcinac.utils.utils import load_movie, get_source_profile_param, scale_polygon_to_source, \
    get_continous_time_periods, smooth_convolve
from deepcinac.utils.cinac_file_utils import CinacFileReader, CinacFileWriter
from keras.models import model_from_json

from deepcinac.cinac_predictor import CinacRecording, CinacDataMovie, predict_transient_from_model
...
class RawFormatOptionModule(tk.Frame):
    ...
    def get_config_as_dict(self):
        ...
        if self.with_option_menu:
            ...
            if self.with_check_box:
                config_dict["check_box_var"] = self.check_box_var.get()
...
class ChoiceRawFormatFrame(tk.Frame):
    ...
    def launch_exploratory_gui(self):
        ...
        if self.modules_dict["caiman_coords"].activated:
            ...
            matlab_indexing = bool(self.modules_dict["caiman_coords"].check_box_var.get())
        ...
        elif self.modules_dict["suite2p_iscell"].activated:
    ...
    def load_config(self):
        ...
        Returns: None if no file has been loaded, or the path from where the file was loaded otherwise
    ...
    def update_launch_gui_button(self):
        ...
        if self.modules_dict["movie"].activated and (self.modules_dict["caiman_coords"].activated or
                                                     (self.modules_dict["suite2p_iscell"].activated and
                                                      self.modules_dict["suite2p_stat"].activated)):
...
class ManualOnsetFrame(tk.Frame):
    ...
    def __init__(self, data_and_param, default_path=None, segment_mode=False, parent=None, title=None):
        ...
        self.nb_times = self.data_and_param.tiff_movie.shape[0]

        # if True, smooth_traces  will be displayed next to raw smooth_traces (fluorescence signal)
        self.display_smooth_traces = False
        ...
        if self.data_and_param.classifier_predictions is not None:
            print(f"Classifier predictions available")
            for cell in np.arange(self.nb_neurons):
        ...
        if not self.segment_mode:
            ...
            self.spin_box_pixels_around_cell = Spinbox(add_segment_frame,
                                                       values=list(np.arange(5, max(n_pixels_x, n_pixels_y), 1)),
                                                       fg=self.color_text_gui_default, justify=CENTER,
                                                       width=4, textvariable=var,
                                                       state="readonly")  # , textvariable=self.spin_box_pixels_around_cell)
            if max(n_pixels_x, n_pixels_y) >= self.segment_window_in_pixels:
                var.set(f"{self.segment_window_in_pixels}")
            else:
                var.set(f"{max(n_pixels_x, n_pixels_y)}")
                self.segment_window_in_pixels = max(n_pixels_x, n_pixels_y)
    ...
    def change_prediction_transient_to_look_at(self, period):
        ...
        if self.display_smooth_traces:
            new_top_y_limit = np.max(self.raw_traces[new_neuron, new_left_x_limit:new_right_x_limit + 1]) + 0.5
            new_bottom_y_limit = np.min(self.smooth_traces[new_neuron, new_left_x_limit:new_right_x_limit + 1]) - 0.5
        else:
            new_top_y_limit = np.max(self.raw_traces[new_neuron, new_left_x_limit:new_right_x_limit + 1]) + 0.5
    ...
    def onrelease(self, event):
        ...
        if self.center_segment_mode:
            ...
            if self.display_smooth_traces:
                ...
                new_bottom_y_limit = np.min(self.smooth_traces[self.current_neuron,
                                            new_left_x_limit:new_right_x_limit + 1]) - 0.5
            else:
                new_top_y_limit = np.max(self.raw_traces[self.current_neuron,
                                         new_left_x_limit:new_right_x_limit + 1]) + 0.5
    ...
    def set_transient_classifier_prediction_for_cell(self, cell):
        ...
        if (self.cinac_recording is None) or (self.cinac_recording is None):
            return

        use_data_augmentation = False
        overlap_value = 0

        predictions = predict_transient_from_model(cinac_recording=self.cinac_recording,
                                                   cell=cell, model=self.classifier_model,
    ...
    def get_square_coord_around_cell(self, cell, x_len_max, y_len_max):
        """
        ...
        """
        if self.segment_window_in_pixels in self.square_coord_around_cell_dict:
            if cell in self.square_coord_around_cell_dict[self.segment_window_in_pixels]:
                return self.square_coord_around_cell_dict[self.segment_window_in_pixels][cell]
        ...
        min_x_centroid = centroid_cell[0] - int(self.segment_window_in_pixels // 2)
        min_y_centroid = centroid_cell[1] - int(self.segment_window_in_pixels // 2)

        # poly_gon_cell_of_interest = self.data_and_param.coord_obj.cells_polygon[cell]
        # minx_cell, miny_cell, maxx_cell, maxy_cell = np.array(list(poly_gon_cell_of_interest.bounds)).astype(int)
        ...
        # if the cell in not entirely in the frame, then we center the frame around the cell centroid
        # if (minx_cell < minx) or (miny_cell < miny) or (maxx_cell > maxx) or (maxy_cell > maxy):
        #     minx = min_x_centroid
        #     miny = min_y_centroid

        # centering the window on the cell
        if (min_x_centroid <= minx) and \
                ((min_x_centroid + self.segment_window_in_pixels) >= maxx):
            minx = min_x_centroid

        if (min_y_centroid <= miny) and \
                ((min_y_centroid + self.segment_window_in_pixels) >= maxy):
            miny = min_y_centroid

        # then we make sure we don't over go the border
        if minx + self.segment_window_in_pixels >= x_len_max:
            minx = x_len_max - self.segment_window_in_pixels - 1
        if miny + self.segment_window_in_pixels >= y_len_max:
            miny = y_len_max - self.segment_window_in_pixels - 1

        # pixels_around is set at 0
        # minx = max(0, minx - pixels_around)
        # miny = max(0, miny - pixels_around)

        if self.segment_window_in_pixels not in self.square_coord_around_cell_dict:
            self.square_coord_around_cell_dict[self.segment_window_in_pixels] = dict()
        self.square_coord_around_cell_dict[self.segment_window_in_pixels][cell] = (max(0, minx), max(0, miny))
    ...
    def save_segments(self, and_close=False):
        if self.save_file_name is None:
            ...
            return
        print(f"self.save_file_name {self.save_file_name}")
        # self.save_segments_button['state'] = DISABLED
    ...
    def plot_magnifier(self, first_time=False, mouse_x_position=None, mouse_y_position=None):
        ...
        if self.x_center_magnified is not None:
            ...
            if self.display_smooth_traces:
                max_value = max(np.max(self.smooth_traces[self.current_neuron, pos_beg_x:pos_end_x]),
                                np.max(self.raw_traces[self.current_neuron, pos_beg_x:pos_end_x]))
                min_value = min(np.min(self.smooth_traces[self.current_neuron, pos_beg_x:pos_end_x]),
            ...
            if self.display_smooth_traces:
                self.line1, = self.axe_plot_magnifier.plot(np.arange(nb_times_to_display),
                                                           self.smooth_traces[self.current_neuron, pos_beg_x:pos_end_x],
        ...
        if self.display_background_img:
            ...
            if self.play_movie:
                ...
                if self.display_classifier_frame_around_cell:
                    ...
                    square_coords.append((max(min_x + self.segment_window_in_pixels, 0), min_y))
                    square_coords.append((max(min_x + self.segment_window_in_pixels, 0),
                                          min(dimensions_movie[1], min_y + self.segment_window_in_pixels)))
                    square_coords.append((min_x, min(dimensions_movie[1], min_y + self.segment_window_in_pixels)))
    ...
    def plot_graph(self, y_max_lim=None, first_time=False):
        ...
        if self.display_smooth_traces:
            self.line1, = self.axe_plot.plot(np.arange(self.nb_times_traces),
                                             self.smooth_traces[self.current_neuron, :],
                                             color=self.color_trace, zorder=10)
        ...
        if self.show_transient_classifier:
            if self.current_neuron in self.transient_prediction:
                ...
                for i_ap, active_period in enumerate(active_periods):
                    ...
                    min_traces = np.min(self.raw_traces[self.current_neuron, period])  # - 0.1
                    y2 = np.repeat(min_traces, len(period))
                    self.axe_plot.fill_between(x=period, y1=self.raw_traces[self.current_neuron, period], y2=y2,
                ...
                if False:
                    ...
                    if len(predictions.shape) == 1:
                        ...
                        real_transient_frames = np.logical_and((predictions[:, 0] >= threshold_tc),
                                                               (predictions[:, 0] == max_pred_by_frame))
                    predicted_raster_dur[real_transient_frames] = 1
                    frames_stat, transients_stat, _ = classification_stat.compute_stats(raster_dur,
        ...
        if self.display_smooth_traces:
            max_value = max(np.max(self.raw_traces[self.current_neuron, :]),
                            np.max(self.smooth_traces[self.current_neuron, :]))
            min_value = min(np.min(self.raw_traces[self.current_neuron, :]),
                            np.min(self.smooth_traces[self.current_neuron, :]))
        else:
        ...
        # #################### Infered neuronal activity from other sources ####################
        ...
        if (self.other_rasters is not None) and len(self.other_rasters) > 0:
            for raster_index, raster in enumerate(self.other_rasters):
                self.axe_plot.vlines(np.where(raster[self.current_neuron])[0],
                                     ...
                                     color=self.other_rasters_colors[raster_index % len(self.other_rasters_colors)],
        ...
        if self.display_threshold:
            threshold = self.get_threshold()
            peaks_under_threshold = np.where(self.raw_traces[self.current_neuron, peaks] < threshold)[0]
            if len(peaks_under_threshold) == 0:
                ...
                peaks_under_threshold_value = self.raw_traces[self.current_neuron, peaks][peaks_under_threshold]
            ...
            peaks_over_threshold = np.where(self.raw_traces[self.current_neuron, peaks] >= threshold)[0]
            if len(peaks_over_threshold) == 0:
                ...
                peaks_over_threshold_value = self.smooth_traces[self.current_neuron, peaks][peaks_over_threshold]
            ...
            if self.correlation_for_each_peak_option and (self.peaks_correlation is not None):
                ...
                if len(peaks_over_threshold_index) == 0:
                    ...
                    peaks_over_threshold_value = self.smooth_traces[self.current_neuron, peaks_over_threshold_index]
                    peaks_over_threshold_value_raw = self.raw_traces[self.current_neuron, peaks_over_threshold_index]
                ...
                if len(overlapping_cells) == 0:
                    ...
                    peaks_under_threshold_value = self.smooth_traces[self.current_neuron, peaks_under_threshold_index]
                    peaks_under_threshold_value_raw = self.raw_traces[self.current_neuron, peaks_under_threshold_index]
                    ...
                    if len(peaks_under_threshold_index) == 0:
                        ...
                        peaks_under_threshold_value = self.smooth_traces[
                            self.current_neuron, peaks_under_threshold_index]
                        peaks_under_threshold_value_raw = self.raw_traces[
                            self.current_neuron, peaks_under_threshold_index]
                    ...
                    if len(peaks_undetermined_index) == 0:
                        ...
                        peaks_undetermined_value = self.smooth_traces[self.current_neuron, peaks_undetermined_index]
                        peaks_undetermined_value_raw = self.raw_traces[self.current_neuron, peaks_undetermined_index]
                # use to remove them
                self.peaks_under_threshold_index = peaks_under_threshold_index
            ...
            # z_order=10 indicate that the scatter will be on top
            if self.display_smooth_traces:
                self.ax1_bottom_scatter = self.axe_plot.scatter(peaks, self.smooth_traces[self.current_neuron, peaks],
                                                                marker='o', c=self.color_peak,
        ...
        if self.to_agree_peak_nums is not None:
            ...
            peaks_amplitude = self.raw_traces[self.current_neuron, to_agree_peaks]
    ...
    def update_plot(self, new_neuron=False, amplitude_zoom_fit=True,
        ...
        self.axe_plot.clear()
        # self.line1.set_ydata(self.smooth_traces[self.current_neuron, :])
        if amplitude_zoom_fit:
            y_max_lim = self.current_max_amplitude()
        elif new_neuron:
            y_max_lim = math.ceil(np.max(self.raw_traces))
        else:
