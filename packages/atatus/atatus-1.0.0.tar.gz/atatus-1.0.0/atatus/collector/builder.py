...
class Builder(object):
    def __init__(self, config, metadata):
        ...
        if not self._hostid:
            self._hostid = socket.gethostname()
    ...
    def _build_hostinfo_env_host_obj(self):
        hostinfo = {}
        hostinfo["hostname"] = self._config.hostname
        if "hwinfo" in self._metadata:
    ...
    def _build_metric(self, name, value):
        ...
        metric = {
            ...
            "type": value.type,
            "kind": value.kind,
            "durations": [value.count, value.min, value.max, value.total]
        }
    ...
    def _build_traces_obj(self, traces_data):
        ...
        for txn in traces_data:
            ...
            if self._config.framework_name:
                trace["type"] = self._config.framework_name
            else:
                trace["type"] = atatus.PYTHON_AGENT
    ...
    def _build_error_metrics_obj(self, metrics_data):
        ...
        for t, v in metrics_data.items():
            ...
            error_metric["name"] = t
            if self._config.framework_name:
                error_metric["type"] = self._config.framework_name
            else:
                error_metric["type"] = atatus.PYTHON_AGENT
    ...
    def _build_error_requests_obj(self, requests_data):
        ...
        for v in requests_data:
            ...
            if self._config.framework_name:
                error_request["type"] = self._config.framework_name
            else:
                error_request["type"] = atatus.PYTHON_AGENT
