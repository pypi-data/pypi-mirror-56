...
class Range:
    ...
    **ALL INEQUALITIES SHOULD BE IN THE FORM:**
    ...
    def _get_intersection_points(self, intersection, tol):
        N_in = len(self._coeff_inequalities_tmp)
        N_var = len(intersection)
        bounds_max = [bound[1] for bound in self.bounds]
        Ac = np.zeros((N_in + N_var, N_var))
        bc = np.zeros(N_in + N_var)
        for i in range(N_in):
            arr = np.array(self._coeff_inequalities_tmp[i])
            Ac[i] = arr[intersection]
            bc[i] = self._const_inequalities_tmp[i]
        for i in range(N_var):
            arr = np.zeros(N_var)
            arr[i] = 1
            Ac[N_in + i] = arr
            bc[N_in + i] = bounds_max[intersection[i]]

        points_in = []
        for i in range(N_in + N_var):
            for j in range(i + 1, N_in + N_var):
                A = np.zeros((2, N_var))
                b = np.zeros(N_var)
                A[0] = Ac[i]
                A[1] = Ac[j]
                ...
                b[1] = bc[j]
                try:
                    points_in.append(np.linalg.solve(A, b))
                except np.linalg.linalg.LinAlgError:
                    # two phases with same stoichiometry
                    pass

        # find outer points
        points_out = []
        for pi in range(len(points_in)):
            x, y = points_in[pi]
            candidates = []
            for i in range(N_in + N_var):
                a, b = Ac[i]
                c = bc[i]
                if b != 0:
                    candidates.append(c/b - a*x/b)
            if (y - np.array(candidates) < tol).all():
                points_out.append((x, y))
        self._boundary_points = tuple(points_out)
        return points_out
    ...
    def _get_limit_lines(self, edges_points, compounds_dict, tol):
        ...
        for ii in range(len(xx) - 2):
            ...
            if np.abs(x2-x1) > 1e-6:
                slope = (y2-y1)/(x2-x1)
                q = y1 - slope*x1
                if np.abs(slope) > tol: # regular case
                    b = -1/slope
                    c = q*b
                else: # y = something
                    b = np.inf
            ...
            for key in compounds_dict.keys():
                ...
                for cc in comp:
                    ...
                    if key[0] != 0:
                        ...
                        b_1 = np.inf
                        c_1 = np.inf
                    # first condition for b=c=np.inf
                    if ((b == b_1 and c == c_1) or
                        (np.abs(b_1 - b) < tol and np.abs(c_1 - c) < tol)):
                        to_append = True
                    else:
                        to_append = False
                    if to_append:
    ...
    def _intersect_values(self, plane_axes, plane_values=None):
        ...
        self._const_inequalities_tmp = self.const_inequalities - values.sum(axis=1)
        values = self._coeff_equalities_tmp[:, rem_args]*plane_values
        self._const_equalities_tmp = self.const_equalities - values.sum(axis=1)
    ...
    def plot_feasible_region_on_plane(self, plane_axes,
        ...
        # create the polygon representing the feasible region
        xx = []; yy = []
        for point in points:
            x, y = point
            xx.append(x); yy.append(y)
        xx.append(x_val_min)
        yy.append(bounds_max[plane_axes[1]])
        xx.append(bounds_max[plane_axes[0]])
        yy.append(y_val_min)
        xx = np.array(xx); yy = np.array(yy)
        sorted_index = np.argsort(xx)
        xx = xx[sorted_index]
        yy = yy[sorted_index]
        xx = list(xx); yy = list(yy)
        xx.append(x_val_min)
        yy.append(y_val_min)
        verts = np.zeros((len(xx), 2))
        verts[:, 0] = xx; verts[:, 1] = yy
        poly = Polygon(verts, True, alpha=0.2, color='green')
        ax.add_patch(poly)

        # find the compounds giving the edges of the polygon
        lines = self._get_limit_lines((xx, yy), compounds_dict, tol)
        ...
        try:
            values = self._find_variables_extrema(ineq_coeffs, ineq_const,
                                                  eq_coeffs, eq_const,
                                                  bounds, 2)
            ax.scatter(values[0, 0], values[1, 1], color='black')
            ax.scatter(values[0, 1], values[1, 0], color='black')
        except ValueError:
            pass

        plt.legend(prop={'size':fsize-4}, loc=3)
