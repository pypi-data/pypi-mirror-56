    ...
    def _get_values(self):
        ...
        with np.errstate(divide='ignore', over='raise'):
            ...
            try:
                ...
                fd = (1/(1 + np.exp((energy.astype(np.float128) - mu)/
                                    (self.kb * temper))))
...
class EquilibriumConcentrations:
    ...
    form_energy_vbm : dict of arrays or None
        ...
        for a Fermi level equal to :data:`vbm`.
    ...
    """
    def __init__(self, charge_states, form_energy_vbm, vbm, e_gap, N_conc,
                 dos, N_eff, T_range, g=None, units_energy='eV',
                 dos_down=None):
        ...
        self._N_conc_dict = copy.copy(N_conc)
        # tuples instead of dictionaries, the order of the defects is contained
        # in self._defect_order
        self._charge_states = tuple()
        self._formation_energies_vbm = ()
        self._N_conc = () # hold site concentrations only for atomic defects
        self._Ne = self._N_conc_dict.pop('electron')
        self._Nh = self._N_conc_dict.pop('hole')
        if charge_states is None:
            ...
            for key in charge_states:
                ...
                N_defect = [N_conc[key]]*len(charge_states[key])
                self._N_conc += tuple(N_defect)
                self._g += g_value
    ...
    @property
    def N_conc(self):
        return np.copy(self._N_conc_dict)
    ...
    def _make_eq_defects_dict(self):
        ...
        for key in self.defect_order:
            ...
            for charge in charges:
                ...
                defects_dict[key][charge] = this_conc
                count += 1
        return defects_dict
