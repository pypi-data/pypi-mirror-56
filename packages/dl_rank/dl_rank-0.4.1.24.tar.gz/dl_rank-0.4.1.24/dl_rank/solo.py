...
class EstimatorManager(object):
    ...
    def input_fn(self, mode, filter_func, distributionStrategy):
        def wrapper(parse_fn, batch_size, data_source, TOTAL_WORKERS=1, WORKER_INDEX=1, epochs=1, num_parallel_calls=1,
            ...
            if isinstance(data_source, str) or isinstance(data_source, pathStr):
    ...
    def train_and_eval(self, parse_fn):
        ...
        train_input_fn = self.input_fn("train",self.parser.data_file_filter,self.run_conf['train_distribute'])
        eval_input_fn = self.input_fn("evaluation",self.parser.data_file_filter,self.run_conf['train_distribute'])
    ...
    def predict(self, parse_fn, bake_path=None):
        ...
        input_fn = self.input_fn("infer",self.parser.data_file_filter,"")
...
class Utils(object):
    @staticmethod
    def get_files_under_dates(data_file, filter_func=None, part=-1, total_part=-1, tmp_path="/dev/shm/data"):
        ...
        if total_part>0 and tmp_path is not None:
            if not wfile.Exists(tmp_path):
                wfile.MakeDirs(tmp_path)
            fileList = fileList[part:: total_part]
            outFileList = [os.path.join(tmp_path, file.split('/')[-1]) for file in fileList]
