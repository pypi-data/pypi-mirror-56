...
class BaseParser(object):
                    ...
                    with tf.variable_scope(scope):
                        for feature in features:
                            if feature in feature_unused:
                                continue
                            input_dict.update({feature: tf.compat.v1.placeholder(dtype=tf.string, shape=[None], name=feature)})
    ...
    def parse_fn_bake(self, isPred=False, na_value='', tail=''):
        csv_defaults = self.column_defaults
        feature_unused = self.feature_unused
    ...
    def model_input_parse_fn(self, model_secondary_parse_fn):
        ...
        def wrapper(features, params, dims, model_struct):
            sparse_emb, deep_emb, dense_emb, mask = first_parser(features, params, csv_defaults, feature_unused, secondary_delim, teriary_delim)
            features = second_parser(sparse_emb, deep_emb, dense_emb, mask, model_struct)
            return dims, features
        return wrapper

    @staticmethod
    def first_parse_fn(features, params, csv_defaults, feature_unused, secondary_delim, teriary_delim):
        '''
        ...
        decode_features = tf.io.decode_csv(
            records=features, record_defaults=list(csv_defaults.values()),
            field_delim=secondary_delim, use_quote_delim=False, na_value='')
        decode_features = dict(zip(csv_defaults.keys(), decode_features))
        for f in feature_unused:
            decode_features.pop(f)
