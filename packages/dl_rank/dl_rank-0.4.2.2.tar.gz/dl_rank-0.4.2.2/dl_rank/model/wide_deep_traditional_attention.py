...
class wide_and_deep(baseModel):
    ...
    def _forward_model(self, is_training, *args, **kwargs):
        ...
        with tf.variable_scope('trunk'):
            ...
            if self.mode == 'train':
                ...
                self.sequence = tf.identity(sequence_emb, name="seq_input_node")                    # placeholder: embedding_lookup_2/Reshape_1
                self.itemid = tf.squeeze(itemid_emb, axis=1, name="click_input_node")    # placeholder: trunk/Squeeze
            ...
            with tf.variable_scope("wide_layer") as scope:
                if self.mode == 'train':
                    self.add_sum = tf.reduce_sum(sparse_features, axis=1, name='wide_value')
            ...
            with tf.variable_scope("deep_layer") as scope:
                if self.mode == 'train':
                    ...
                    self.logits_deep = self.bulid_attention_layers(is_training, sequence_emb, sequence_token_mask, itemid_emb,
            ...
            # self.tfprint = tf.print('logits_deep', self.logits_deep, summarize=10000)

            with tf.variable_scope("output_layer"):
                if self.mode == 'train':
                    # pt = tf.concat([self.logits_deep, self.logits_wide], axis=1)
                    # tfprint = tf.print(pt, summarize=10000)
    ...
    def bulid_optimize(self,learning_rate):
        ...
        if self.optimizer_name.lower() == 'adadelta':
            ...
            opt = [wide_opt, deep_opt]
            #TODO: DELETE
            a = len(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "wide_layer"))
        else:
