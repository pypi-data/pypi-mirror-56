...
class baseModel(metaclass=ABCMeta):
    ...
    def build_embedding(params, num_shards):
        ...
        for feature in feature_list:
            ...
            feature_initializer = conf['initializer'] if 'initializer' in conf else dict()
            ...
            with tf.variable_scope(feature_scope, reuse=tf.AUTO_REUSE, partitioner=partitioner) as scope:
                if f_type == 'category':
                    ...
                    if f_tran == 'vocabulary_list':
                        ...
                        vocabulary = vocabulary ##########['DEFAULT'] + vocabulary
                        default_value = 0 if 'default' not in f_param \
                            else vocabulary.index(f_param['default']) \
                            if isinstance(f_param['default'], str) else f_param['default']
                        tail_value = 0 if 'tail' not in f_param \
                            else vocabulary.index(f_param['tail']) \
                            if isinstance(f_param['tail'], str) else f_param['tail']
                        fill_value = '' if 'fill' not in f_param \
                            else vocabulary[f_param['fill']] \
                            if isinstance(f_param['fill'], int) else f_param['fill']
                    ...
                    elif f_tran == 'vocabulary_file':
                        default_value = 0 if 'default' not in f_param else f_param['default']
                        tail_value = 0 if 'tail' not in f_param else f_param['tail']
                        fill_value = '' if 'fill' not in f_param else f_param['fill']

                        table.update(
                            {feature: lookup.index_table_from_file(
                                vocabulary_file=file_path, vocab_size=vocab_size, default_value=default_value,delimiter='\t')
                            })
                        f_dim = vocab_size * f_num
                        f_size = vocab_size

                    elif f_tran == 'tabled':
                        f_dim = f_param['size']*f_num
                        f_size = f_param['size']
                        fill_value = f_param['fill'] if 'fill' in f_param else ''
                    ...
                    if 'embedding' in conf['style']:
                        deep.update({feature: tf.get_variable(initializer=get_initializer(**feature_initializer),
                                                              shape=[f_size, feature_embed_dim+first_order],
                        ...
                        if f_num >=1:
                            if f_multi['combiner'] == 'none':
                                if f_multi['same']:
                                    deep.update({feature:
                                        tf.get_variable(
                                            initializer=get_initializer(**feature_initializer),
                                            shape=[1, embed_dim + first_order],
                                            name='{}'.format(feature_name))})
                                else:
                                    deep.update({feature: tf.get_variable(
                                        initializer=get_initializer(**feature_initializer),
                                        shape=[f_num, embed_dim + first_order],
                                        name='{}'.format(feature_name))})
                                con_num += f_num
                                all_num += f_num
                                con_deep_num += f_num
                            ...
                            if f_multi['combiner'] == 'none':
                                ...
                                if f_multi['same']:
                                    ...
                                    deep.update({feature: tf.get_variable(initializer=get_initializer(**feature_initializer),
                                                                          shape=[f_num, embed_dim+first_order],
                                                                          name='{}_embedding'.format(feature))})
                                con_num += f_num
                                all_num += f_num
                                con_deep_num += f_num
                                deep_num += f_num
                    ...
                    f_dim = -1
                    default_value = 0   # no used
                    if 'fill' in f_param:
                        fill_value = f_param['fill']
                        if isinstance(fill_value, str):
                            fill_value = tf.get_variable(initializer=get_initializer(**feature_initializer),
                                                         shape=[],
                                                         name='{}_fill_value'.format(fill_value))
                    else:
                        fill_value = ''
                    if 'tail' in f_param:
                        tail_value = f_param['tail']
                        if isinstance(tail_value, str):
                            tail_value = tf.get_variable(initializer=get_initializer(**feature_initializer),
                                                         shape=[],
                                                         name='{}_tail_value'.format(tail_value))
                    else:
                        tail_value = 0
                else:
...
def get_initializer(name=None, param=None):
    param = dict() if param is None else param
    try:
        return eval(name)(**param)
