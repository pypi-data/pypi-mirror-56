...
class EstimatorManager(object):
    ...
    def input_fn(self):
        ...
        distributionStrategy = self.run_conf['train_distribute']
        def wrapper(parse_fn, batch_size, data_source, TOTAL_WORKERS=1, WORKER_INDEX=1, epochs=1, num_parallel_calls=1,
            ...
            if distributionStrategy and _args_mode == "train":
                return dataset
            else:
    ...
    def train_and_eval(self, parse_fn):
        ...
        train_spec = tf.estimator.TrainSpec(
            input_fn=lambda: input_fn(parse_fn, batch_size, train_data, self.args['tw'], self.args['wi'],
                                      self.train_conf['train_epochs'], self.train_conf['num_parallel_calls'],
                                      self.train_conf['shuffle_buffer_size']),
            max_steps=max_steps)
        eval_spec = tf.estimator.EvalSpec(
            input_fn=lambda: input_fn(parse_fn, batch_size, eval_data, self.args['tw'], self.args['wi'],
