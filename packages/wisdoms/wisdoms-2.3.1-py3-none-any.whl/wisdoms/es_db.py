...
class EsSearch:
    ...
    def es_search(self, index, p=None, ns_key='id', **kwargs):
        ...
        for key in kwargs:
            if isinstance(kwargs[key], dict):
                s = s.query('nested', path=key, query=Q('match', **{str(key) + '__' + ns_key: kwargs[key].get(ns_key)}))
            elif isinstance(kwargs[key], list):
                s = s.query('range', **{key: {'gte': kwargs[key][0], 'lte': kwargs[key][1]}})
            else:
                s = s.filter('wildcard', **{key + '.keyword': '*' + kwargs[key] + '*'})
