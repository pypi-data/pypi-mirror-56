...
class Output:
    ...
    def __init__(self, target, results, given):
        ...
        for p, quantity in self.__results.items():
            ...
            if not isinstance(v_lower, np.ndarray) and not isinstance(v_upper, np.ndarray):
    ...
    def implot(self,
        ...
        if not self.__grid_results:
            raise CavCalcError("None of the computed parameters have been evaluated on a grid, cannot plot results.")

        if zparam is None:
            if len(self.__grid_results) > 1:
                raise CavCalcError("Multiple computed parameters have been evaluated on grids, zparam must be "
        ...
        if zparam in self.__grid_results:
            ...
            raise CavCalcError(f"Parameter: {zparam} not in results dictionary of output.")
        ...
        for i, z_values in enumerate(zs_quads):
            ...
            fig, ax = density_plot(
                ...
                zlabel=get_label(zparam) + quad_label + (f" [{z_units}]" if z_units else ''),
