...
class TestPropertyUpdates(unittest.TestCase):
    ...
    def test_update_delta_altitude(self):
        ...
        self.assertEqual(target_alt - alt, self.env.sim.get_property_value(c.delta_altitude), 'Delta altitude incorrect')
        new_alt = 11000
        self.env.sim.set_property_value(c.position_h_sl_ft, new_alt)
        self.assertEqual(target_alt - new_alt, self.env.sim.get_property_value(c.delta_altitude), 'Delta altitude incorrect')
    ...
    def test_update_detect_extreme_state(self):
        self.assertFalse(bool(self.env.sim.get_property_value(c.detect_extreme_state)), 'Should not detect extreme state')
    ...
    def test_update_center_brake(self):
        ...
        self.assertEqual(new_brake_cmd, self.env.sim.get_property_value(c.fcs_left_brake_cmd_norm), 'Left brake was not updated')
        self.assertEqual(new_brake_cmd, self.env.sim.get_property_value(c.fcs_right_brake_cmd_norm), 'Right brake was not updated')
