...
class TaxiapControlTask(Task):
    ...
    init_conditions = { c.ic_h_sl_ft: INITIAL_ALTITUDE_FT,
    ...
    def is_terminal(self, state, sim):
        ...
        terminal = sim.get_property_value(c.simulation_sim_time_sec)>=450 or math.fabs(sim.get_property_value(c.shortest_dist)) >= 10 or math.fabs(sim.get_property_value(c.velocities_vc_fps)) <= 5.0*self.k2f
