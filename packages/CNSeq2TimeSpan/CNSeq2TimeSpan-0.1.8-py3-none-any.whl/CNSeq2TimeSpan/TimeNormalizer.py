...
class TimeNormalizer:
    ...
    def _filter(self, input_query):
        ...
        if match == None:
            if "个" in input_query:
                ...
                translatedWords.append(translatedWord("个", ""))

        if "中旬" in input_query:
            input_query = input_query.replace('中旬', '15号')
            translatedWords.append(translatedWord('中旬', '15号'))
        if "中旬" in input_query:
            input_query = input_query.replace('傍晚', '午后')
            translatedWords.append(translatedWord('傍晚', '午后'))
        if "大年" in input_query:
            input_query = input_query.replace('大年', '')
            translatedWords.append(translatedWord('大年', ''))
        if "五一" in input_query:
            input_query = input_query.replace('五一', '劳动节')
            translatedWords.append(translatedWord('五一', '劳动节'))
        if "白天" in input_query:
            input_query = input_query.replace('白天', '早上')
            translatedWords.append(translatedWord('白天', '早上'))
        if "：" in input_query:
            input_query = input_query.replace('：', ':')
            translatedWords.append(translatedWord('：', ':'))
        return input_query, translatedWords
    ...
    def __preHandling(self):
        ...
        self.target, translatedWords = StringPreHandler.numberTranslator(self.target)  # 大写数字转化
        self.translatedWords.extend(translatedWords)
    ...
    def __recoveryWord(self, word, translatedWords, origin, n):
        '''
        通过递归 将正则后的词语还原成原词
        :param word: 要还原的词
        :param translatedWords: 词替换列表
        :param target: 原语句
        :param n: 选择第n个词进行替换
        :return:
        '''
        if n >= len(translatedWords):
            return word
        word_change = word.replace(translatedWords[n][1], translatedWords[n][0])
        if word in origin:
            return word
        elif word_change in origin:
            return word_change
        else:
            word_no = self.__recoveryWord(word,translatedWords,origin,n+1)
            word_yes = self.__recoveryWord(word_change, translatedWords, origin, n + 1)
            if word_no in origin:
                return word_no
