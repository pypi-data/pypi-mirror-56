...
class ClientDescribeTrailsResponsetrailListTypeDef(_ClientDescribeTrailsResponsetrailListTypeDef):
    ...
    - **CloudWatchLogsLogGroupArn** *(string) --*
      ...
      Specifies an Amazon Resource Name (ARN), a unique identifier that represents the log group to
...
class ClientGetEventSelectorsResponseEventSelectorsDataResourcesTypeDef(
    ...
    The Amazon S3 buckets or AWS Lambda functions that you specify in your event selectors for your
    trail to log data events. Data events provide information about the resource operations
    performed on or within a resource itself. These are also known as data plane operations. You can
    ...
    * The ``PutObject`` API operation is an Amazon S3 object-level API. It is recorded as a data
    ...
    events that occur on any object in that bucket are logged. The trail processes and logs the
    event.
    ...
    * The ``PutObject`` API operation occurred for an object in an S3 bucket that the CloudTrail
    ...
    data events for a Lambda function named *MyLambdaFunction* , but not for all AWS Lambda
    functions.

    * A user runs a script that includes a call to the *MyLambdaFunction* function and the
    *MyOtherLambdaFunction* function.

    * The ``Invoke`` API operation on *MyLambdaFunction* is an AWS Lambda API. It is recorded as a
    data event in CloudTrail. Because the CloudTrail user specified logging data events for
    *MyLambdaFunction* , any invocations of that function are logged. The trail processes and logs
    the event.
    ...
    * The ``Invoke`` API operation on *MyOtherLambdaFunction* is an AWS Lambda API. Because the
    ...
    - **Values** *(list) --*
      ...
      * To log data events for all objects in an S3 bucket, specify the bucket and an empty object
      prefix such as ``arn:aws:s3:::bucket-1/`` . The trail logs data events for all objects in this
      S3 bucket.
      ...
      * To log data events for all functions in your AWS account, specify the prefix as
      ...
      .. note::
         ...
         AWS account, even if that activity is performed on a function that belongs to another AWS
         account.
...
class ClientGetInsightSelectorsResponseInsightSelectorsTypeDef(
    ...
    """
    Type definition for `ClientGetInsightSelectorsResponse` `InsightSelectors`
...
class ClientGetInsightSelectorsResponseTypeDef(_ClientGetInsightSelectorsResponseTypeDef):
    """
    Type definition for `ClientGetInsightSelectors` `Response`
...
_ClientLookupEventsLookupAttributesTypeDef = TypedDict(
    "_ClientLookupEventsLookupAttributesTypeDef", {"AttributeKey": str, "AttributeValue": str}
)


class ClientLookupEventsLookupAttributesTypeDef(_ClientLookupEventsLookupAttributesTypeDef):
    """
...
class ClientLookupEventsResponseEventsResourcesTypeDef(
    ...
    - **ResourceType** *(string) --*
      ...
      **Trail** for CloudTrail, **DBInstance** for RDS, and **AccessKey** for IAM. To learn more
      about how to look up and filter events by the resource types supported for a service, see
    ...
    - **ResourceName** *(string) --*
      ...
      The name of the resource referenced by the event returned. These are user-created names whose
      values will depend on the environment. For example, the resource name might be
...
class ClientPutInsightSelectorsInsightSelectorsTypeDef(
    ...
    """
    Type definition for `ClientPutInsightSelectors` `InsightSelectors`
...
class ClientPutInsightSelectorsResponseInsightSelectorsTypeDef(
    ...
    """
    Type definition for `ClientPutInsightSelectorsResponse` `InsightSelectors`
...
class ClientPutInsightSelectorsResponseTypeDef(_ClientPutInsightSelectorsResponseTypeDef):
    """
    Type definition for `ClientPutInsightSelectors` `Response`
...
_LookupEventsPaginateLookupAttributesTypeDef = TypedDict(
    "_LookupEventsPaginateLookupAttributesTypeDef", {"AttributeKey": str, "AttributeValue": str}
)


class LookupEventsPaginateLookupAttributesTypeDef(_LookupEventsPaginateLookupAttributesTypeDef):
    """
