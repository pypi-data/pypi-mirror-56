"Main interface for cloudtrail Client"
...
class Client(BaseClient):
    ...
    def add_tags(
        ...
        """
        Adds one or more tags to a trail, up to a limit of 50. Overwrites an existing tag's value when a new
        value is specified for an existing tag key. Tag key names must be unique for a trail; you cannot
        have two keys with the same name but different values. If you specify a key without a value, the tag
        will be created with the specified key and a value of null. You can tag a trail that applies to all
        AWS Regions only from the Region in which the trail was created (also known as its home region).
        ...
        **Request Syntax**
        ...
        :param EnableLogFileValidation:
          ...
          .. note::
            ...
            hour. CloudTrail will not create digest files for log files that were delivered during a period
    ...
    def describe_trails(
        ...
        **Request Syntax**
        ...
        :param trailNameList:
          ...
          * If an empty list is specified and ``IncludeShadowTrails`` is false, then information for all
          trails in the current region is returned.

          * If an empty list is specified and IncludeShadowTrails is null or true, then information for all
          trails in the current region and any associated shadow trails in other regions is returned.

          .. note::

            If one or more trail names are specified, information is returned only if the names match the
            names of trails belonging only to the current region. To return information about a trail in
    ...
    def get_event_selectors(
        ...
        returned for your event selectors includes the following:

        * If your event selector includes read-only events, write-only events, or all events. This applies
        to both management events and data events.

        * If your event selector includes management events.

        * If your event selector includes data events, the Amazon S3 objects or AWS Lambda functions that
        you are logging for data events.
        ...
        **Request Syntax**
        ...
        :param TrailName: **[REQUIRED]**
          ...
          Specifies the name of the trail or trail ARN. If you specify a trail name, the string must meet
            ...
            - **EventSelectors** *(list) --*
              ...
              - *(dict) --*
                ...
                - **ReadWriteType** *(string) --*
                  ...
                  example, the EC2 ``GetConsoleOutput`` is a read-only API operation and ``RunInstances`` is
                ...
                - **DataResources** *(list) --*
                  ...
                  data resources cannot exceed 250 across all event selectors in a trail. This limit does
                      ...
                      * To log data events for a specific Lambda function, specify the function ARN.
                      ...
                      .. note::
                         ...
                         *arn:aws:lambda:us-west-2:111111111111:function:helloworld* , data events will only
                         ...
                         will not be logged for *arn:aws:lambda:us-west-2:111111111111:function:helloworld2*
                         .
                ...
                - **ExcludeManagementEventSources** *(list) --*
                  ...
                  it can filter out AWS Key Management Service events by containing ``"kms.amazonaws.com"``
                  . By default, ``ExcludeManagementEventSources`` is empty, and AWS KMS events are included
    ...
    def get_insight_selectors(
        ...
        ``GetInsightSelectors`` shows if CloudTrail Insights event logging is enabled on the trail, and if
        it is, which insight types are enabled. If you run ``GetInsightSelectors`` on a trail that does not
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **StartLoggingTime** *(datetime) --*
              ...
              Specifies the most recent date and time when CloudTrail started recording API calls for an AWS
              account.

            - **StopLoggingTime** *(datetime) --*

              Specifies the most recent date and time when CloudTrail stopped recording API calls for an AWS
              account.

            - **LatestCloudWatchLogsDeliveryError** *(string) --*

              Displays any CloudWatch Logs error that CloudTrail encountered when attempting to deliver logs
              to CloudWatch Logs.
    ...
    def list_trails(self, NextToken: str = None) -> ClientListTrailsResponseTypeDef:
        ...
        **Request Syntax**
        ...
        :param NextToken:
          ...
          passed in with the same parameters that were specified in the the original call. For example, if
          the original call specified an AttributeKey of 'Username' with a value of 'root', the call with
    ...
    def lookup_events(
        ...
        that are captured by CloudTrail. You can look up events that occurred in a region within the last 90
        days. Lookup supports the following attributes for management events:
        ...
        * Event source
        ...
        All attributes are optional. The default number of results returned is 50, with a maximum of 50
        possible. The response includes a token that you can use to get the next page of results.

        .. warning::

          The rate of lookup requests is limited to two per second per account. If this limit is exceeded, a
                ...
                - **Resources** *(list) --*
                  ...
                  - *(dict) --*
                    ...
                    - **ResourceType** *(string) --*
                      ...
                      be determined, null is returned. Some examples of resource types are: **Instance** for
                      EC2, **Trail** for CloudTrail, **DBInstance** for RDS, and **AccessKey** for IAM. To
                      learn more about how to look up and filter events by the resource types supported for
                      a service, see `Filtering CloudTrail Events
                    ...
                    - **ResourceName** *(string) --*
                      ...
                      might be "auto-scaling-test-group" for an Auto Scaling Group or "i-1234567" for an EC2
                      Instance.
    ...
    def put_event_selectors(
        self,
        TrailName: str,
        EventSelectors: List[ClientPutEventSelectorsEventSelectorsTypeDef],
    ) -> ClientPutEventSelectorsResponseTypeDef:
        """
        Configures an event selector for your trail. Use event selectors to further specify the management
        and data event settings for your trail. By default, trails created without specific event selectors
        will be configured to log all read and write management events, and no data events.

        When an event occurs in your account, CloudTrail evaluates the event selectors in all trails. For
        each trail, if the event matches any event selector, the trail processes and logs the event. If the
        ...
        * The ``RunInstances`` is a write-only event and it matches your event selector. The trail logs the
        event.
        ...
        * The ``GetConsoleOutput`` is a read-only event but it doesn't match your event selector. The trail
        ...
        You can configure up to five event selectors for each trail. For more information, see `Logging Data
        and Management Events for Trails
        <https://docs.aws.amazon.com/awscloudtrail/latest/userguide/logging-management-and-data-events-with-cloudtrail.html>`__
        and `Limits in AWS CloudTrail
        <https://docs.aws.amazon.com/awscloudtrail/latest/userguide/WhatIsCloudTrail-Limits.html>`__ in the
        ...
        **Request Syntax**
        ...
        :param EventSelectors: **[REQUIRED]**
          ...
          Specifies the settings for your event selectors. You can configure up to five event selectors for
          a trail.
          ...
          - *(dict) --*
            ...
            - **DataResources** *(list) --*
              ...
              specify up to 250 resources for an individual event selector, but the total number of data
              resources cannot exceed 250 across all event selectors in a trail. This limit does not apply
              if you configure resource logging for all data events.
              ...
              - *(dict) --*
                ...
                your trail to log data events. Data events provide information about the resource operations
                performed on or within a resource itself. These are also known as data plane operations. You
                ...
                * The ``Invoke`` API operation on *MyLambdaFunction* is an AWS Lambda API. It is recorded as
                a data event in CloudTrail. Because the CloudTrail user specified logging data events for
                ...
                * The ``Invoke`` API operation on *MyOtherLambdaFunction* is an AWS Lambda API. Because the
                ...
                - **Values** *(list) --*
                  ...
                  * To log data events for specific objects, specify the S3 bucket and object prefix such as
                  ``arn:aws:s3:::bucket-1/example-images`` . The trail logs data events for objects in this
                  S3 bucket that match the prefix.
            ...
            - **ExcludeManagementEventSources** *(list) --*
              ...
              filter out AWS Key Management Service events by containing ``"kms.amazonaws.com"`` . By
              default, ``ExcludeManagementEventSources`` is empty, and AWS KMS events are included in events
              that are logged to your trail.
    ...
    def put_insight_selectors(
        ...
        on an existing trail. You also use ``PutInsightSelectors`` to turn off Insights event logging, by
        passing an empty list of insight types. In this release, only ``ApiCallRateInsight`` is supported as
        an Insights selector.
    ...
    def start_logging(self, Name: str) -> Dict:
        """
        Starts the recording of AWS API calls and log file delivery for a trail. For a trail that is enabled
        in all regions, this operation must be called from the region in which the trail was created. This
        operation cannot be called on the shadow trails (replicated trails in other regions) of a trail that
    ...
    def stop_logging(self, Name: str) -> Dict:
        ...
        circumstances, there is no need to use this action. You can update a trail without stopping it
        first. This action is the only way to stop recording. For a trail enabled in all regions, this
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          Specifies the name or the CloudTrail ARN of the trail for which CloudTrail will stop logging AWS
    ...
    def update_trail(
        ...
        """
        Updates the settings that specify delivery of log files. Changes to a trail do not require stopping
        the CloudTrail service. Use this action to designate an existing bucket for log delivery. If the
        existing bucket has previously been a target for CloudTrail log files, an IAM policy exists for the
        ...
        **Request Syntax**
        ...
        :param Name: **[REQUIRED]**
          ...
          Specifies the name of the trail or trail ARN. If ``Name`` is a trail name, the string must meet
          the following requirements:
        ...
        :param IsOrganizationTrail:
          ...
          Specifies whether the trail is applied to all accounts in an organization in AWS Organizations, or
          ...
          If the trail is not an organization trail and this is set to true, the trail will be created in
          all AWS accounts that belong to the organization. If the trail is an organization trail and this
          is set to false, the trail will remain in the current AWS account but be deleted from all member
