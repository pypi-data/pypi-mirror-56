...
def initial_summary_stats(game, metadata, detailed_info):
    ...
    if not mmr_data[0]['m_scaledRating'] or not mmr_data[1]['m_scaledRating']:
        logging.critical('One or more players has no MMR')
        return None
...
def setup(filename):
    ...
    for i in range(0, 5):
        try:
            ...
            logging.error('An error occurred while decoding')
            pass
...
def parse_replay(filename):
    ...
    if not players:
        logging.critical('Aborting replay due to <2 players...')
        return None, None, None, None
    ...
    if summary_stats is None:
        logging.critical('Aborting replay due to missing MMR value(s)')
        return None, None, None, None
