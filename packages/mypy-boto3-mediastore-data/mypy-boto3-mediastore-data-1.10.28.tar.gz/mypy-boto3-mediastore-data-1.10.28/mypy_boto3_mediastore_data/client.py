...
class Client(BaseClient):
    ...
    def get_object(self, Path: str, Range: str = None) -> ClientGetObjectResponseTypeDef:
        ...
        ``streaming`` , AWS Elemental MediaStore downloads the object even if it’s still uploading
        the object.
        ...
        **Request Syntax**
        ...
        :param Path: **[REQUIRED]**
          ...
          ``premium/canada`` , the service creates a ``canada`` subfolder in the ``premium`` folder.
          You then have two subfolders, ``usa`` and ``canada`` , in the ``premium`` folder.
          ...
          Elemental MediaStore User Guide <http://docs.aws.amazon.com/mediastore/latest/ug/>`__ .
        ...
        :param Range:
          ...
          The range bytes of an object to retrieve. For more information about the ``Range`` header,
          see `http\\://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35
          <http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35>`__ . AWS Elemental
    ...
    def list_items(
        ...
        **Request Syntax**
        ...
        :param MaxResults:
          ...
          ``ListItems`` request with ``MaxResults`` set at 500. Although 2,000 items match your
          request, the service returns no more than the first 500 items. (The service also returns a
          ``NextToken`` value that you can use to fetch the next batch of results.) The service
        ...
        :param NextToken:
          ...
          The token that identifies which batch of results that you want to see. For example, you
          submit a ``ListItems`` request with ``MaxResults`` set at 500. The service returns the
          first batch of results (up to 500) and a ``NextToken`` value. To see the next batch of
          results, you can submit the ``ListItems`` request a second time and specify the
          ``NextToken`` value.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextToken** *(string) --*
              ...
              you submit a ``ListItems`` request that matches 2,000 items with ``MaxResults`` set at
              500. The service returns the first batch of results (up to 500) and a ``NextToken``
    ...
    def put_object(
        ...
        **Request Syntax**
        ...
        :param StorageClass:
          ...
          Indicates the storage class of a ``Put`` request. Defaults to high-performance temporal
          storage class, and objects are persisted into durable storage shortly after being
          received.

        :type UploadAvailability: string
        :param UploadAvailability:

          Indicates the availability of an object while it is still uploading. If the value is set
          to ``streaming`` , the object is available for downloading after some initial buffering
          but before the object is uploaded completely. If the value is set to ``standard`` , the
          object is available for downloading only when it is uploaded completely. The default value
