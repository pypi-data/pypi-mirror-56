...
class BulkProcesses(object):
    ...
    def __init__(self, configfile=None, logfile = None, loglevelname = None):
        """Initialise class BulkProcesses"""
        ...
        self.host = asseticsdk.conf.host
    ...
    def upload_document_photo(self, excelDataFileToRead,groupid=None,
        ...
        :param excelDataFileToRead:
            ...
            Is Key-photo (TRUE/FALSE)|External ID
        ...
        try:
            ...
            sheet = wb.active
            sheet['F' + str(1)] = 'Migrated Document Id'
            sheet['G' + str(1)] = 'Operation Performed On'
            sheet['H' + str(1)] = 'Status'
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            msg += ";" + result[0]
            msg = msg.strip(";")
            self._log_message_to_sheet(sheet, row, result[1], msg)
    ...
    def _process_upload_row(self, fileNameToUpload, parent_id, groupid=None
        ...
        if not aslink:
            ...
            document.label = 'Importing complex asset document/photo' \
                             "- [' + fileToUploadName + '] on {0}" \
    ...
    def _log_message_to_sheet(self, sheet, row, docid, status):
        ...
        """
        sheet['F' + str(row)] = docid
        sheet['G' + str(row)] = time.strftime("%c")
        sheet['H' + str(row)] = status
    ...
    def document_get(self, fileStorageDirectoryLocation, docid):
        ...
        if fileStorageDirectoryLocation is not None and not \
           ...
           and docid is not None and not docid.isspace():
            ...
            getfile = None
            try:
                getfile = self.docapi_get.document_get_document_file_with_http_info(docid)
            except ApiException as e:
