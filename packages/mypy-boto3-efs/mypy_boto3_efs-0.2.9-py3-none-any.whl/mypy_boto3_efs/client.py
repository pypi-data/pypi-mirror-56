...
class Client(BaseClient):
    ...
    def create_file_system(
        ...
        caller's AWS account with the specified creation token, this operation does the following:
        ...
        * Returns with the description of the created file system.
        ...
        systems using the ``maxIO`` performance mode can scale to higher levels of aggregate
        throughput and operations per second with a tradeoff of slightly higher latencies for most
        file operations. The performance mode can't be changed after the file system has been
        ...
        For more information, see  CreateMountTarget . You mount your Amazon EFS file system on an
        EC2 instances in your VPC by using the mount target. For more information, see `Amazon
        EFS\\: How it Works <https://docs.aws.amazon.com/efs/latest/ug/how-it-works.html>`__ .
        ...
        **Request Syntax**
        ...
        :param PerformanceMode:
          ...
          for most file systems. File systems using the ``maxIO`` performance mode can scale to
          higher levels of aggregate throughput and operations per second with a tradeoff of
        ...
        :param Encrypted:
          ...
          encrypted file system, you have the option of specifying  CreateFileSystemRequest$KmsKeyId
          for an existing AWS Key Management Service (AWS KMS) customer master key (CMK). If you
          don't specify a CMK, then the default CMK for Amazon EFS, ``/aws/elasticfilesystem`` , is
        ...
        :param ThroughputMode:
          ...
          Provisioned Throughput mode or change between the throughput modes as long as it’s been
          more than 24 hours since the last decrease or throughput mode change. For more, see
          `Specifying Throughput with Provisioned Mode
          <https://docs.aws.amazon.com/efs/latest/ug/performance.html#provisioned-throughput>`__ in
        ...
        :param ProvisionedThroughputInMibps:
          ...
          you're creating. Valid values are 1-1024. Required if ``ThroughputMode`` is set to
          ``provisioned`` . The upper limit for throughput is 1024 MiB/s. You can get this limit
          increased by contacting AWS Support. For more information, see `Amazon EFS Limits That You
          Can Increase <https://docs.aws.amazon.com/efs/latest/ug/limits.html#soft-limits>`__ in the
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **SizeInBytes** *(dict) --*
              ...
              consistent snapshot of the file system, but it is eventually consistent when there are
              no writes to the file system. That is, ``SizeInBytes`` represents actual size only if
              the file system is not modified for a period longer than a couple of hours. Otherwise,
            ...
            - **ThroughputMode** *(string) --*
              ...
              for your file system: ``bursting`` and ``provisioned`` . If you set ``ThroughputMode``
              ...
              change between the throughput modes as long as it’s been more than 24 hours since the
            ...
            - **ProvisionedThroughputInMibps** *(float) --*
              ...
              values are 1-1024. Required if ``ThroughputMode`` is set to ``provisioned`` . The
              limit on throughput is 1024 MiB/s. You can get these limits increased by contacting
              AWS Support. For more information, see `Amazon EFS Limits That You Can Increase
              <https://docs.aws.amazon.com/efs/latest/ug/limits.html#soft-limits>`__ in the *Amazon
              EFS User Guide.*
    ...
    def create_mount_target(
        ...
        to access their file system. For more information, see `Amazon EFS\\: How it Works
        ...
        target and the file system's lifecycle state must be ``available`` . For more information,
        ...
        * IP address range from which Amazon EFS selects the IP address of the mount target (if you
        don't specify an IP address in the request)
        ...
        system in an EC2 instance. You can also use the mount target's DNS name when mounting the
        file system. The EC2 instance on which you mount the file system by using the mount target
        can resolve the mount target's DNS name to its IP address. For more information, see `How it
        ...
        * Also creates a new network interface in the subnet as follows:
          ...
          * If the request provides an ``IpAddress`` , Amazon EFS assigns that IP address to the
          network interface. Otherwise, Amazon EFS assigns a free address in the subnet (in the same
          way that the Amazon EC2 ``CreateNetworkInterface`` call does when a request does not
        ...
        the mount target's description to the network interface ID, and the ``IpAddress`` field to
        its address. If network interface creation fails, the entire ``CreateMountTarget`` operation
        fails.

        .. note::

          The ``CreateMountTarget`` call returns only after creating the network interface, but
          while the mount target state is still ``creating`` , you can check the mount target
          creation status by calling the  DescribeMountTargets operation, which among other things
        ...
        created in another Availability Zone. For more information, see `Amazon EFS
        <http://aws.amazon.com/efs/>`__ . In addition, by always using a mount target local to the
        instance's Availability Zone, you eliminate a partial failure scenario. If the Availability
        Zone in which your mount target is created goes down, then you can't access your file system
        ...
        **Request Syntax**
        ...
        :param SecurityGroups:
          ...
          Up to five VPC security group IDs, of the form ``sg-xxxxxxxx`` . These must be for the
    ...
    # pylint: disable=arguments-differ,redefined-outer-name,redefined-builtin
    def create_tags(self, FileSystemId: str, Tags: List[ClientCreateTagsTagsTypeDef]) -> None:
        """
    ...
    def delete_file_system(self, FileSystemId: str) -> None:
        ...
        You can't delete a file system that is in use. That is, if the file system has any mount
        ...
        .. note::
          ...
          ID or creation token for the deleted file system, the  DescribeFileSystems returns a ``404
    ...
    def delete_mount_target(self, MountTargetId: str) -> None:
        ...
        * ``elasticfilesystem:DeleteMountTarget``
        ...
        .. note::
          ...
          The ``DeleteMountTarget`` call returns while the mount target state is still ``deleting``
          . You can check the mount target deletion by calling the  DescribeMountTargets operation,
          which returns a list of mount target descriptions for the given file system.

        The operation also requires permissions for the following Amazon EC2 action on the mount
    ...
    def delete_tags(self, FileSystemId: str, TagKeys: List[str]) -> None:
        """
        Deletes the specified tags from a file system. If the ``DeleteTags`` request includes a tag
        key that doesn't exist, Amazon EFS ignores it and doesn't cause an error. For more
        information about tags and related restrictions, see `Tag Restrictions
    ...
    def describe_file_systems(
        ...
        all file systems owned by the caller's AWS account in the AWS Region of the endpoint that
        you're calling.

        When retrieving all file system descriptions, you can optionally specify the ``MaxItems``
        parameter to limit the number of descriptions in a response. Currently, this number is
        automatically set to 10. If more file system descriptions remain, Amazon EFS returns a
        ``NextMarker`` , an opaque token, in the response. In this case, you should send a
        subsequent request with the ``Marker`` request parameter set to the value of ``NextMarker``
        .

        To retrieve a list of your file system descriptions, this operation is used in an iterative
    ...
    def describe_lifecycle_configuration(
        ...
        """
        Returns the current ``LifecycleConfiguration`` object for the specified Amazon EFS file
        system. EFS lifecycle management uses the ``LifecycleConfiguration`` object to identify
        which files to move to the EFS Infrequent Access (IA) storage class. For a file system
        ...
        **Request Syntax**
        ...
        :param FileSystemId: **[REQUIRED]**
          ...
          The ID of the file system whose ``LifecycleConfiguration`` object you want to retrieve
          (String).
            ...
            - **LifecyclePolicies** *(list) --*
              ...
              - *(dict) --*
                ...
                - **TransitionToIA** *(string) --*
                  ...
                  it transitions to the IA storage class. Metadata operations such as listing the
    ...
    def describe_mount_target_security_groups(
        ...
        * ``elasticfilesystem:DescribeMountTargetSecurityGroups`` action on the mount target's file
        system.
    ...
    def describe_tags(
        ...
        **Request Syntax**
        ...
        :param MaxItems:
          ...
          this number is automatically set to 10, and other values are ignored. The response is
          paginated at 10 per page if you have more than 10 tags.
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **NextMarker** *(string) --*
              ...
              provide the value of ``NextMarker`` as the value of the ``Marker`` parameter in your
              next request to retrieve the next set of tags.
    ...
    def modify_mount_target_security_groups(
        ...
        information, see  CreateMountTarget . This operation replaces the security groups in effect
        ...
        target has been created and the lifecycle state of the mount target is not ``deleted`` .

        The operation requires permissions for the following actions:

        * ``elasticfilesystem:ModifyMountTargetSecurityGroups`` action on the mount target's file
        system.
    ...
    def put_lifecycle_configuration(
        self,
        FileSystemId: str,
        LifecyclePolicies: List[ClientPutLifecycleConfigurationLifecyclePoliciesTypeDef],
    ) -> ClientPutLifecycleConfigurationResponseTypeDef:
        """
        ...
        * A ``LifecyclePolicies`` array of ``LifecyclePolicy`` objects that define when files are
        moved to the IA storage class. The array can contain only one ``LifecyclePolicy`` item.
        ...
        AWS Key Management Service (AWS KMS) permissions as when you created the encrypted file
        system.
        ...
        **Request Syntax**
        ...
        :param LifecyclePolicies: **[REQUIRED]**
          ...
          management when to transition files from the Standard storage class to the Infrequent
          ...
          - *(dict) --*
            ...
            - **TransitionToIA** *(string) --*
              ...
              transitions to the IA storage class. Metadata operations such as listing the contents
    ...
    def update_file_system(
        ...
        **Request Syntax**
        ...
        :param ProvisionedThroughputInMibps:
          ...
          system. Valid values are 1-1024. Required if ``ThroughputMode`` is changed to
          ``provisioned`` on update. If you're not updating the amount of provisioned throughput for
