...
class Client(BaseClient):
    ...
    def allocate_hosted_connection(
        ...
        connection on the specified interconnect or LAG of interconnects. AWS polices the hosted
        connection for the specified capacity and the AWS Direct Connect Partner must also police
          ...
          - *(dict) --*
            ...
            - **virtualInterfaceState** *(string) --*
              ...
              * ``confirming`` : The creation of the virtual interface is pending confirmation from
              the virtual interface owner. If the owner of the virtual interface is different from
              the owner of the connection on which it is provisioned, then the virtual interface
          ...
          ::

            {
                'virtualInterface': {
                    ...
                    'addressFamily': 'ipv4'|'ipv6',
                    'virtualInterfaceState':
                    'confirming'|'verifying'|'pending'|'available'|'down'|'deleting'
            ...
            - **virtualInterface** *(dict) --*
              ...
              - **virtualInterfaceState** *(string) --*
                ...
                * ``confirming`` : The creation of the virtual interface is pending confirmation
                ...
                interface will remain in this state until it is confirmed by the virtual interface
                owner.
                ...
                - *(dict) --*
                  ...
                  - **bgpStatus** *(string) --*
                    ...
                    * ``up`` : The BGP peer is established. This state does not indicate the state
                    of the routing function. Ensure that you are receiving routes over the BGP
                    session.
    ...
    def associate_connection_with_lag(
        ...
        the interconnect was originally associated with a different LAG, the hosted connections
    ...
    def associate_hosted_connection(
        ...
        interrupts the hosted connection's connectivity to AWS as it is being migrated.
    ...
    def associate_virtual_interface(
        ...
        number or a conflicting IP address, the operation fails.

        Virtual interfaces associated with a hosted connection cannot be associated with a LAG;
        hosted connections must be migrated along with their virtual interfaces using
    ...
    def confirm_connection(self, connectionId: str) -> ClientConfirmConnectionResponseTypeDef:
        ...
        Upon creation, the hosted connection is initially in the ``Ordering`` state, and remains in
        this state until the owner confirms creation of the hosted connection.
    ...
    def create_bgp_peer(
        ...
        already whitelisted for the virtual interface.
    ...
    def create_connection(
        ...
        a LAG ID in the request. This ensures that the new connection is allocated on the same AWS
    ...
    def create_direct_connect_gateway(
        ...
        different AWS Regions. This enables you to connect to a VPC in any Region, regardless of the
    ...
    def create_direct_connect_gateway_association_proposal(
        ...
        gateway when the account that owns the Direct Connect gateway and the account that owns the
        virtual private gateway or transit gateway have the same AWS Payer ID.
    ...
    def create_interconnect(
        ...
        customers through their own network services. Like a standard connection, an interconnect
        links the partner's network to an AWS Direct Connect location over a standard Ethernet
        fiber-optic cable. One end is connected to the partner's router, the other to an AWS Direct
        Connect router.

        You can automatically add the new interconnect to a link aggregation group (LAG) by
        specifying a LAG ID in the request. This ensures that the new interconnect is allocated on
        the same AWS Direct Connect endpoint that hosts the specified LAG. If there are no available
        ports on the endpoint, the request fails and no interconnect is created.

        For each end customer, the AWS Direct Connect Partner provisions a connection on their
        interconnect by calling  AllocateHostedConnection . The end customer can then connect to AWS
        resources by creating a virtual interface on their connection, using the VLAN assigned to
        them by the AWS Direct Connect Partner.
    ...
    def create_lag(
        ...
        connections between the customer network and a specific AWS Direct Connect location. A LAG
        is a logical interface that uses the Link Aggregation Control Protocol (LACP) to aggregate
        multiple interfaces, enabling you to treat them as a single interface.

        All connections in a LAG must use the same bandwidth and must terminate at the same AWS
        ...
        You can specify an existing physical connection or interconnect to include in the LAG (which
        counts towards the total number of connections). Doing so interrupts the current physical
        connection or hosted connections, and re-establishes them as a member of the LAG. The LAG
        will be created on the same AWS Direct Connect endpoint to which the connection terminates.
        Any virtual interfaces associated with the connection are automatically disassociated and
        re-associated with the LAG. The connection ID does not change.

        If the AWS account used to create a LAG is a registered AWS Direct Connect Partner, the LAG
        ...
        **Request Syntax**
        ...
        :param connectionsBandwidth: **[REQUIRED]**
          ...
          values are 50Mbps, 100Mbps, 200Mbps, 300Mbps, 400Mbps, 500Mbps, 1Gbps, 2Gbps, 5Gbps, and
          10Gbps.
                ...
                'connections': [
                    {
                        ...
                        'connectionName': 'string',
                        'connectionState':
                        'ordering'|'requested'|'pending'|'available'|'down'
    ...
    def create_private_virtual_interface(
        ...
        Connect gateway or a Virtual Private Gateway (VGW). Connecting the private virtual interface
        to a Direct Connect gateway enables the possibility for connecting to multiple VPCs,
        including VPCs in different AWS Regions. Connecting the private virtual interface to a VGW
    ...
    def create_transit_virtual_interface(
        ...
        .. warning::
          ...
          Autonomous System Number (ASN) used by the transit gateway and the Direct Connect gateway
          must be different. For example, if you use the default ASN 64512 for both your the transit
          gateway and Direct Connect gateway, the association request fails.
    ...
    def delete_connection(self, connectionId: str) -> ClientDeleteConnectionResponseTypeDef:
        ...
        If you are partnering with any third parties to connect with the AWS Direct Connect
        location, you must cancel your service with them separately.
    ...
    def describe_connection_loa(
        ...
        your APN partner or service provider uses when establishing your cross connect to AWS at the
        colocation facility. For more information, see `Requesting Cross Connects at AWS Direct
        Connect Locations
        <https://docs.aws.amazon.com/directconnect/latest/UserGuide/Colocation.html>`__ in the *AWS
    ...
    def describe_direct_connect_gateway_associations(
        ...
        You must specify a Direct Connect gateway, a virtual private gateway, or both. If you
        specify a Direct Connect gateway, the response contains all virtual private gateways
        associated with the Direct Connect gateway. If you specify a virtual private gateway, the
        response contains all Direct Connect gateways associated with the virtual private gateway.
        If you specify both, the response contains the association between the Direct Connect
        gateway and the virtual private gateway.
                ...
                'interconnects': [
                    {
                        ...
                        'interconnectName': 'string',
                        'interconnectState':
                        'requested'|'pending'|'available'|'down'|'deleting'
                        |'deleted'|'unknown',
                        'region': 'string',
                        'location': 'string',
                        'bandwidth': 'string',
                        ...
                        'connections': [
                            {
                                ...
                                'connectionName': 'string',
                                'connectionState':
                                'ordering'|'requested'|'pending'|'available'
    ...
    def describe_loa(
        ...
        **Request Syntax**
        ...
        :param providerName:
          ...
          The name of the service provider who establishes connectivity on your behalf. If you
          specify this parameter, the LOA-CFA lists the provider name alongside your company name as
          the requester of the cross connect.
    ...
    def describe_virtual_interfaces(
        ...
        A virtual interface (VLAN) transmits the traffic between the AWS Direct Connect location and
                ...
                'virtualInterfaces': [
                    {
                        ...
                        'addressFamily': 'ipv4'|'ipv6',
                        'virtualInterfaceState':
                        'confirming'|'verifying'|'pending'|'available'|'down'
                        |'deleting'|'deleted'|'rejected'|'unknown',
                        'customerRouterConfig': 'string',
                        'mtu': 123,
                        'jumboFrameCapable': True|False,
                        ...
                        'bgpPeers': [
                            {
                                ...
                                'customerAddress': 'string',
                                'bgpPeerState':
                                'verifying'|'pending'|'available'|'deleting'
                                |'deleted',
                                'bgpStatus': 'up'|'down'|'unknown',
                                'awsDeviceV2': 'string'
                            },
    ...
    def disassociate_connection_from_lag(
        ...
        delete the connection, use the  DeleteConnection request). If the LAG has associated virtual
        interfaces or hosted connections, they remain associated with the LAG. A disassociated
        connection owned by an AWS Direct Connect Partner is automatically converted to an
        interconnect.
        ...
        the LAG. If all connections are disassociated, the LAG continues to exist as an empty LAG
        with no physical connections.
    ...
    def tag_resource(
        ...
        """
        Adds the specified tags to the specified AWS Direct Connect resource. Each resource can have
    ...
    def update_lag(
        ...
        * The value for the minimum number of connections that must be operational for the LAG
        ...
        is zero (0). If you update this value and the number of operational connections falls below
        the specified value, the LAG automatically goes down to avoid over-utilization of the
        remaining connections. Adjust this value with care, as it could force the LAG down if it is
        set higher than the current number of operational connections.
