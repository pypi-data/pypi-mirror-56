...
from .api.trex_stl_types import StatNotAvailable
from .text_opts import format_num, format_text, format_threshold, TRexTextInfo, TRexTextTable
...
ALL_STATS_OPTS = [GLOBAL_STATS, PORT_STATS, PORT_STATUS, STREAMS_STATS, LATENCY_STATS,
                  PORT_GRAPH, LATENCY_HISTOGRAM, CPU_STATS, MBUF_STATS, EXTENDED_STATS,
...
class WatchedField(object):
    ...
    def update(self, value):
        ...
        if value > self.high_th and not self.hot:
            self.events_handler.log_warning(
                "{0} is high: {1}{2}".format(self.name, value, self.suffix))
            self.hot = True
...
class CTRexInfoGenerator(object):
    ...
    def __init__(self, global_stats_ref, ports_dict_ref, rx_stats_ref, latency_stats_ref,
                 util_stats_ref, xstats_ref, async_monitor, pgid_ref, stl_client):
    ...
    def generate_streams_info(self, port_id_list, stream_id_list):
        ...
        for port_obj in relevant_ports:
            streams_data = self._generate_single_port_streams_info(
                port_obj, stream_id_list)
            if not streams_data:
                continue
    ...
    def _generate_global_stats(self):
        ...
        stats_data_left = OrderedDict([("connection", "{host}, Port {port}"
                                        .format(host=global_stats.connection_info.get("server"),
                                                port=global_stats.connection_info.get("sync_port")
                                                )),
                                       ("version", "{ver}".format(
                                           ver=global_stats.server_version.get("version", "N/A"))),

                                       ("cpu_util.", "{0}% @ {2} cores({3} per port) {1}"
                                        .format(format_threshold(round_float(global_stats
                                                                             .get("m_cpu_util")),
                                                [85, 100], [0, 85]),
                                                global_stats.get_trend_gui("m_cpu_util",
                                                                           use_raw=True),
                                                global_stats.system_info.get('dp_core_count'),
                                                global_stats.system_info.get(
                                                    'dp_core_count_per_port'),)),

                                       ("rx_cpu_util.", "{0}% / {1} {2}"
                                        .format(format_threshold(round_float(global_stats
                                                .get("m_rx_cpu_util")), [85, 100], [0, 85]),
                                                global_stats.get("m_rx_core_pps",
                                                                 format=True, suffix="pkt/sec"),
                                                global_stats.get_trend_gui("m_rx_cpu_util",
                                                                           use_raw=True),)),

                                       ("async_util.", "{0}% / {1}"
                                        .format(format_threshold(round_float(self._async_monitor
                                                .get_cpu_util()), [85, 100], [0, 85]),
                                                format_num(self._async_monitor.get_bps() / 8.0,
                                                           suffix="B/sec"))),
                                       ])

        stats_data_right = OrderedDict([
            ("total_tx_L2", "{0} {1}".format(global_stats.get("m_tx_bps", format=True,
                                                              suffix="b/sec"),
                                             global_stats.get_trend_gui("m_tx_bps"))),

            ("total_tx_L1", "{0} {1}".format(global_stats.get("m_tx_bps_L1", format=True,
                                                              suffix="b/sec"),
            ...
            ("total_pps", "{0} {1}".format(global_stats.get("m_tx_pps", format=True,
                                                            suffix="pkt/sec"),
                                           global_stats.get_trend_gui("m_tx_pps"))),

            ("drop_rate", "{0}".format(format_num(global_stats.get("m_rx_drop_bps"),
                                                  suffix='b/sec',
                                                  opts='green' if(global_stats.get("m_rx_drop_bps")
                                                                  == 0) else 'red'),
                                       )),
            ...
            ("queue_full", "{0}".format(format_num(global_stats.get_rel("m_total_queue_full"),
                                                   ...
                                                   compact=False,
                                                   opts='green'
                                                   if(global_stats.get_rel("m_total_queue_full")
                                                      == 0) else 'red'))),
        ])

        # build table representation
        stats_table = TRexTextInfo()
    ...
    def _generate_streams_stats(self):
        ...
        # maximum 4
        for pg_id in pg_ids:
            sstats_data['Tx pps'].append(self.pgid_ref.get(
                ['flow_stats', pg_id, 'tx_pps', 'total'], format=True, suffix="pps"))
            sstats_data['Tx bps L2'].append(self.pgid_ref.get(
                ['flow_stats', pg_id, 'tx_bps', 'total'], format=True, suffix="bps"))
            sstats_data['Tx bps L1'].append(self.pgid_ref.get(
                ['flow_stats', pg_id, 'tx_bps_l1', 'total'], format=True, suffix="bps"))
            sstats_data['Rx pps'].append(self.pgid_ref.get(
                ['flow_stats', pg_id, 'rx_pps', 'total'], format=True, suffix="pps"))
            sstats_data['Rx bps'].append(self.pgid_ref.get(
                ['flow_stats', pg_id, 'rx_bps', 'total'], format=True, suffix="bps"))
            sstats_data['opackets'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'tx_pkts', 'total']))
            sstats_data['ipackets'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'rx_pkts', 'total']))
            sstats_data['obytes'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'tx_bytes', 'total']))
            sstats_data['ibytes'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'rx_bytes', 'total']))
            sstats_data['tx_pkts'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'tx_pkts', 'total'], format=True, suffix="pkts"))
            sstats_data['rx_pkts'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'rx_pkts', 'total'], format=True, suffix="pkts"))
            sstats_data['tx_bytes'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'tx_bytes', 'total'], format=True, suffix="B"))
            sstats_data['rx_bytes'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'rx_bytes', 'total'], format=True, suffix="B"))

        stats_table = TRexTextTable()
        ...
        disp_str = {'tx_pkts': "opackets ", 'rx_pkts': "ipackets ",
                    'rx_bytes': "ibytes ", 'tx_bytes': "obytes "}
        sstats_new_data = OrderedDict([(disp_str[k], v) if k in disp_str else(
            k, v) for k, v in list(sstats_data.items())])
        stats_table.add_rows([[k] + v
                              for k, v in list(sstats_new_data.items())],
                             header=False)
    ...
    def _generate_latency_stats(self):
        ...
        to_delete = []
        for id in list(self.max_hist.keys()):
            if id not in pg_ids:
                to_delete.append(id)
        ...
        for id in pg_ids:
            if id not in list(self.max_hist.keys()):
        ...
        for pg_id in pg_ids:
            last_max = self.pgid_ref.get(
                ['latency', pg_id, 'latency', 'last_max'])
            lstats_data['TX pkts'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'tx_pkts', 'total']))
            lstats_data['RX pkts'].append(self.pgid_ref.get_rel(
                ['flow_stats', pg_id, 'rx_pkts', 'total']))
            lstats_data['Avg latency'].append(
                try_int(self.pgid_ref.get(['latency', pg_id, 'latency', 'average'])))
            lstats_data['Max latency'].append(
                try_int(self.pgid_ref.get(['latency', pg_id, 'latency', 'total_max'])))
            lstats_data['Last max'].append(last_max)
            self.max_hist[pg_id][self.max_hist_index] = last_max
            for i in range(1, self.latency_window_size):
                val = self.max_hist[pg_id][(
                    self.max_hist_index - i) % self.latency_window_size]
                if val != -1:
                    lstats_data['Last-%s' % i].append(val)
                else:
                    lstats_data['Last-%s' % i].append(" ")
            lstats_data['Jitter'].append(self.pgid_ref.get(
                ['latency', pg_id, 'latency', 'jitter']))
            errors = 0
            seq_too_low = self.pgid_ref.get_rel(
                ['latency', pg_id, 'err_cntrs', 'seq_too_low'])
            if int(seq_too_low):
                errors += seq_too_low
            seq_too_high = self.pgid_ref.get_rel(
        ...
        stats_table.set_cols_dtype(['t'] + ['t'] * stream_count)
        stats_table.add_rows([[k] + v
                              for k, v in list(lstats_data.items())],
                             header=False)
    ...
    def _generate_latency_histogram(self):
        ...
        for key in list(reversed(sorted(merged_histogram.keys())))[:histogram_size]:
            ...
            for pg_id in pg_ids:
                hist_vals.append(
                    lat_stats[pg_id]['latency']['histogram'].get(key, ' '))
            ...
            for err_cntr in sorted(lat_stats[pg_id]['err_cntrs'].keys()):
                if err_cntr not in err_cntrs_dict:
                    err_cntrs_dict[err_cntr] = [
                        lat_stats[pg_id]['err_cntrs'][err_cntr]]
                else:
                    err_cntrs_dict[err_cntr].append(
                        lat_stats[pg_id]['err_cntrs'][err_cntr])
        for err_cntr, val_list in list(err_cntrs_dict.items()):
    ...
    def _generate_cpu_util_stats(self):
        ...
        if util_stats:
            if 'cpu' not in util_stats:
                raise Exception(
                    "Excepting 'cpu' section in stats %s" % util_stats)
            cpu_stats = util_stats['cpu']
            hist_len = len(cpu_stats[0]["history"])
            avg_len = min(5, hist_len)
            show_len = min(15, hist_len)
            stats_table.header(['Thread', 'Avg', 'Latest'] +
                               list(range(-1, 0 - show_len, -1)))
            ...
            for i in range(min(18, len(cpu_stats))):
                ...
                if ports == [-1, -1]:
                    ...
                    interfaces = "({:})".format(
                        ports[0] if ports[0] != -1 else ports[1])
    ...
    def _generate_mbuf_util_stats(self):
        ...
        if util_stats:
            if 'mbuf_stats' not in util_stats:
                raise Exception(
                    "Excepting 'mbuf_stats' section in stats %s" % util_stats)
            mbuf_stats = util_stats['mbuf_stats']
            for mbufs_per_socket in list(mbuf_stats.values()):
            ...
            stats_table.header(
                [''] + self._util_stats_ref.mbuf_types_list + ['RAM(MB)'])
            total_list = []
            sum_totals = 0
            for mbuf_type in self._util_stats_ref.mbuf_types_list:
                sum_totals += first_socket_mbufs[mbuf_type][1] * \
                    get_number_of_bytes(mbuf_type) + 64
            ...
            for socket_name in sorted(list(mbuf_stats.keys())):
                ...
                socket_show_name = socket_name.replace(
                    'cpu-', '').replace('-', ' ').capitalize() + ':'
                sum_used = 0
                used_list = []
                percentage_list = []
                for mbuf_type in self._util_stats_ref.mbuf_types_list:
                    ...
                    percentage_list.append('%s%%' % int(
                        100 * used / mbufs[mbuf_type][1]))
    ...
    def _generate_xstats(self, port_id_list, include_zero_lines=False):
        relevant_ports = [
            port.port_id for port in self.__get_relevant_ports(port_id_list)]
        # get the data on relevant ports
        xstats_data = OrderedDict()
        for port_id in relevant_ports:
            for key, val in list(self._xstats_ref.get_stats(port_id).items()):
        ...
        # put into table
        stats_table = TRexTextTable()
        stats_table.header(['Name:'] + ['Port %s:' %
                                        port_id for port_id in relevant_ports])
        ...
        for key, arr in list(xstats_data.items()):
            if include_zero_lines or list([_f for _f in arr if _f]):
                key = key[:28]
    ...
    def _generate_port_graph(self, port_id_list):
        ...
        util_tx_hist = [0] * (hist_maxlen - hist_len) + [round(
            relevant_port.port_stats.history[i]['tx_percentage']) for i in range(hist_len)]
        util_rx_hist = [0] * (hist_maxlen - hist_len) + [round(
            relevant_port.port_stats.history[i]['rx_percentage']) for i in range(hist_len)]
        ...
        for y in range(95, -1, -5):
            stats_table.add_row([y, ''.join([self._get_rational_block_char(util_tx, y, 5)
                                for util_tx in util_tx_hist]),
                                    ''.join([self._get_rational_block_char(util_rx, y, 5)
                                            for util_rx in util_rx_hist])])
    ...
    def _generate_port_stats(self, port_id_list):
        ...
        if(total_cols > 1):
            self.__update_per_field_dict(
                total_stats.generate_stats(), per_field_stats)
            header += ['total']
            total_cols += 1
        ...
        disp_str = {'tx-pkts': "opackets ", 'rx-pkts': "ipackets ",
                    'rx-bytes': "ibytes ", 'tx-bytes': "obytes "}
        disp_data = OrderedDict([(disp_str[k], v) if k in disp_str else(
            k, v) for k, v in list(per_field_stats.items())])

        stats_table.add_rows([[k] + v
                              for k, v in list(disp_data.items())],
                             header=False)
    ...
    def _generate_port_status(self, port_id_list):
        ...
        stats_table.add_rows([[k] + v
                              for k, v in list(per_field_status.items())],
                             header=False)
    ...
    def _generate_single_port_streams_info(self, port_obj, stream_id_list):
        ...
        for stream_id, stream_id_sum in list(return_streams_data['streams'].items()):
            stream_id_sum['packet_type'] = self._trim_packet_headers(
                stream_id_sum['packet_type'], 30)
            p_type_field_len = max(p_type_field_len, len(
                stream_id_sum['packet_type']))

        info_table = TRexTextTable()
        info_table.set_cols_align(
            ["c"] + ["l"] + ["r"] + ["c"] + ["r"] + ["c"])
        info_table.set_cols_width(
            [10] + [p_type_field_len] + [8] + [16] + [15] + [12])
        info_table.set_cols_dtype(
            ["t"] + ["t"] + ["t"] + ["t"] + ["t"] + ["t"])

        info_table.add_rows([list(v.values())
                             for k, v in list(return_streams_data['streams'].items())],
                            header=False)
        info_table.header(["ID", "packet type", "length",
                           "mode", "rate", "next stream"])
    ...
    def __get_relevant_ports(self, port_id_list):
        # fetch owned ports
        ports = [port_obj
                 for _, port_obj in list(self._ports_dict.items())
                 if port_obj.port_id in port_id_list]

        # display only the first FOUR options, by design
        if len(ports) > 4:
            # self.logger is not defined
            # self.logger.log(format_text("[WARNING]: ", 'magenta', 'bold'),
            #                 format_text("displaying up to 4 ports", 'magenta'))
            ports = ports[:4]
        return ports

    def __update_per_field_dict(self, dict_src_data, dict_dest_ref):
        for key, val in list(dict_src_data.items()):
...
class CTRexStats(object):
    ...
    def get_trend(self, field, use_raw=False, percision=10.0):
        ...
        with self.lock:
            field_samples = [sample[field]
                             for sample in list(self.history)[-5:]]
    ...
    def get_trend_gui(self, field, show_value=False, use_raw=False, up_color='red',
                      down_color='green'):
...
class CGlobalStats(CTRexStats):
    ...
    def __init__(self, connection_info, server_version, ports_dict_ref, events_handler):
        ...
        self.watched_cpu_util = WatchedField(
            'CPU util.', '%', 85, 60, events_handler)
        self.watched_rx_cpu_util = WatchedField(
            'RX core util.', '%', 85, 60, events_handler)
...
class CPortStats(CTRexStats):
    ...
    def _update(self, snapshot):
        ...
        diff_tx_pkts = snapshot.get(
            'opackets', 0) - self.latest_stats.get('opackets', 0)
        diff_tx_bytes = snapshot.get(
            'obytes', 0) - self.latest_stats.get('obytes', 0)
        tx_bps_L1 = calc_bps_L1(8.0 * diff_tx_bytes /
                                ts_diff, float(diff_tx_pkts) / ts_diff)
        if speed:
        ...
        diff_rx_pkts = snapshot.get(
            'ipackets', 0) - self.latest_stats.get('ipackets', 0)
        diff_rx_bytes = snapshot.get(
            'ibytes', 0) - self.latest_stats.get('ibytes', 0)
        rx_bps_L1 = calc_bps_L1(8.0 * diff_rx_bytes /
                                ts_diff, float(diff_rx_pkts) / ts_diff)
        if speed:
    ...
    def generate_stats(self):
        ...
        rate_format = {'bpsl1': None, 'bps': None,
                       'pps': None, 'percentage': 'bold'}
        ...
        return {"owner": owner,
                ...
                "CPU util.": "{0} {1}%".format(self.get_trend_gui("m_cpu_util", use_raw=True),
                                               format_threshold(round_float(self.get("m_cpu_util")),
                                               [85, 100], [0, 85])) if self._port_obj else '',
                "--": " ",
                "---": " ",
                "----": " ",
                "-----": " ",

                "Tx bps L1": "{0} {1}".format(self.get_trend_gui("m_total_tx_bps_L1",
                                              show_value=False),
                                              self.get("m_total_tx_bps_L1", format=True,
                                              suffix="bps", opts=rate_format['bpsl1'])),

                "Tx bps L2": "{0} {1}".format(self.get_trend_gui("m_total_tx_bps",
                                              show_value=False),
                                              self.get("m_total_tx_bps", format=True,
                                              suffix="bps", opts=rate_format['bps'])),

                "Line Util.": "{0} {1}".format(self.get_trend_gui("m_tx_util", show_value=False)
                                               if self._port_obj else "",
                                               self.get("m_tx_util", format=True, suffix="%",
                                               opts=rate_format['percentage'])
                                               if self._port_obj else ""),
...
class CLatencyStats(CTRexStats):
    ...
    def _update(self, snapshot):
        ...
        pg_ids = list(filter(is_intable, list(snapshot.keys())))
            ...
            if 'latency' in current_pg:
                for field in ['jitter', 'average', 'total_max', 'last_max']:
                    if field in current_pg['latency']:
                        ...
                        output[int_pg_id]['latency'][field] = StatNotAvailable(
                            field)

                if 'histogram' in current_pg['latency']:
                    output[int_pg_id]['latency']['histogram'] = {int(elem): current_pg['latency']
                                                                 ['histogram'][elem]
                                                                 for elem in current_pg['latency']
                                                                 ['histogram']}
                    min_val = min(output[int_pg_id]
                                  ['latency']['histogram'].keys())
                    if min_val == 0:
                        min_val = 2
                    output[int_pg_id]['latency']['total_min'] = min_val
                else:
                    output[int_pg_id]['latency']['total_min'] = StatNotAvailable(
                        'total_min')
...
class CRxStats(CTRexStats):
    ...
    def calculate_diff_sec(self, current, prev):
        if 'ts' not in current:
            raise ValueError(
                "INTERNAL ERROR: RX stats snapshot MUST contain 'ts' field")

        if prev:
            prev_ts = prev['ts']
            now_ts = current['ts']
            diff_sec = (now_ts['value'] - prev_ts['value']
                        ) / float(now_ts['freq'])
        else:
            diff_sec = 0.0
    ...
    def process_single_pg(self, current_pg, prev_pg):
        ...
        for field in ['tx_pkts', 'tx_bytes', 'rx_pkts', 'rx_bytes']:
            ...
            if field in current_pg:
                for port, pv in list(current_pg[field].items()):
            ...
            # sum up
            total = None
            for port, pv in list(output[field].items()):
                if not is_intable(port):
                    continue
                if total is None:
                    total = 0
    ...
    def process_snapshot(self, current, prev):
        ...
        for field in ['rx_err', 'tx_err']:
            ...
            if 'global' in current and field in current['global']:
                for port in current['global'][field]:
                    output['global'][field][int(
                        port)] = current['global'][field][port]

        # we care only about the current active keys
        pg_ids = list(filter(is_intable, list(current.keys())))
        ...
        ref_pg_ids = list(
            filter(is_intable, list(self.reference_stats.keys())))
    ...
    def calculate_bw_for_pg(self, pg_current, pg_prev=None, diff_sec=0.0):
        ...
        # TX
        for port in list(pg_current['tx_pkts'].keys()):

            prev_tx_pps = pg_prev['tx_pps'].get(port)
            now_tx_pkts = pg_current['tx_pkts'].get(port)
            prev_tx_pkts = pg_prev['tx_pkts'].get(port)
            pg_current['tx_pps'][port], pg_current['tx_pps_lpf'][port] = self.calc_pps(
                prev_tx_pps, now_tx_pkts, prev_tx_pkts, diff_sec)

            prev_tx_bps = pg_prev['tx_bps'].get(port)
            now_tx_bytes = pg_current['tx_bytes'].get(port)
            prev_tx_bytes = pg_prev['tx_bytes'].get(port)

            pg_current['tx_bps'][port], pg_current['tx_bps_lpf'][port] = self.calc_bps(
                prev_tx_bps, now_tx_bytes, prev_tx_bytes, diff_sec)

            if (pg_current['tx_bps'].get(port) is not None
                    and pg_current['tx_pps'].get(port) is not None):
                pg_current['tx_bps_L1'][port] = calc_bps_L1(
                    pg_current['tx_bps'][port], pg_current['tx_pps'][port])
                pg_current['tx_bps_L1_lpf'][port] = calc_bps_L1(
                    pg_current['tx_bps_lpf'][port], pg_current['tx_pps_lpf'][port])
            else:
                pg_current['tx_bps_L1'][port] = None
                pg_current['tx_bps_L1_lpf'][port] = None

        # RX
        for port in list(pg_current['rx_pkts'].keys()):

            prev_rx_pps = pg_prev['rx_pps'].get(port)
            now_rx_pkts = pg_current['rx_pkts'].get(port)
            prev_rx_pkts = pg_prev['rx_pkts'].get(port)
            pg_current['rx_pps'][port], pg_current['rx_pps_lpf'][port] = self.calc_pps(
                prev_rx_pps, now_rx_pkts, prev_rx_pkts, diff_sec)

            prev_rx_bps = pg_prev['rx_bps'].get(port)
            now_rx_bytes = pg_current['rx_bytes'].get(port)
            prev_rx_bytes = pg_prev['rx_bytes'].get(port)
            pg_current['rx_bps'][port], pg_current['rx_bps_lpf'][port] = self.calc_bps(
                prev_rx_bps, now_rx_bytes, prev_rx_bytes, diff_sec)
            if (pg_current['rx_bps'].get(port) is not None
                    and pg_current['rx_pps'].get(port) is not None):
                pg_current['rx_bps_L1'][port] = calc_bps_L1(
                    pg_current['rx_bps'][port], pg_current['rx_pps'][port])
                pg_current['rx_bps_L1_lpf'][port] = calc_bps_L1(
                    pg_current['rx_bps_lpf'][port], pg_current['rx_pps_lpf'][port])
            else:
    ...
    def get_stats(self):
        ...
        for pg_id, value in list(self.latest_stats.items()):
            # skip non ints
            if not is_intable(pg_id):
                # 'global' stats are in the same level of the pg_ids.
                # We do want them to go to the user
                if pg_id == 'global':
                    stats[pg_id] = value
                continue
            # bare counters
            ...
            for field in ['tx_pkts', 'tx_bytes', 'rx_pkts', 'rx_bytes']:
                ...
                stats[int(pg_id)][field] = {
                    'total': val if val != 'N/A' else StatNotAvailable(field)}
                for port in list(value[field].keys()):
                    if is_intable(port):
                        val = self.get_rel([pg_id, field, port])
                        stats[int(pg_id)][field][int(
                            port)] = val if val != 'N/A' else StatNotAvailable(field)
...
class CXStats(CTRexStats):
    ...
    def clear_stats(self, port_id=None):
        ...
        for port_id in ports:
            self.reference_stats[port_id] = self.get_stats(
                port_id, relative=False)
    ...
    def get_stats(self, port_id, use_1sec_cache=False, relative=True):
        ...
        if self.last_update_ts + 1 < time_now or not self.latest_stats or not use_1sec_cache:
            if self.client.is_connected():
                rc = self.client._transmit(
                    'get_port_xstats_values', params={'port_id': port_id})
                if not rc:
                    raise STLError(rc)
                self.last_update_ts = time_now
                ...
                if len(values) != len(self.names):  # need to update names("keys")
                    rc = self.client._transmit(
                        'get_port_xstats_names', params={'port_id': port_id})
                    if not rc:
                        raise STLError(rc)
                    self.names = rc.data().get('xstats_names', [])
                if len(values) != len(self.names):
                    raise STLError('Length of get_xstats_names: %s '
                                   + 'and get_port_xstats_values: %s'
                                   % (len(self.names), len(values)))
                self.latest_stats[port_id] = OrderedDict(
                    [(key, val) for key, val in zip(self.names, values)])

        stats = OrderedDict()
        for key, val in list(self.latest_stats[port_id].items()):
            if relative:
                ...
                if pg_id in self.ref['flow_stats'] and field in self.ref['flow_stats'][pg_id]:
                    for port in ret['flow_stats'][pg_id][field]:
                        if port in self.ref['flow_stats'][pg_id][field]:
                            ...
                            try:
                                ret['flow_stats'][pg_id][field][port] -= (self.ref['flow_stats']
                                                                          [pg_id][field][port])
                            except Exception:
                                pass

        if 'latency' not in ret:
            return ret
        ...
        for pg_id in ret['latency']:
            ...
            if 'latency' in ret['latency'][pg_id]:
                ...
                for key in ret['latency'][pg_id]['latency']['histogram']:
                    if pg_id in self.ref['latency'] and key in (self.ref['latency'][pg_id]
                                                                ['latency']['histogram']):
                        ret['latency'][pg_id]['latency']['histogram'][key] -= (self.ref['latency']
                                                                               [pg_id]['latency']
                                                                               ['histogram'][key])
                ...
                # special handling for 'total_max' field. We want to take it at first,
                # from server reported 'total_max'
                # After running clear_stats, we zero it, and start calculating ourselves
                # by looking at 'last_max' in each sampling
                if 'total_max' in self.ref['latency'][pg_id]['latency']:
                    ret['latency'][pg_id]['latency']['total_max'] = (self.ref['latency'][pg_id]
                                                                     ['latency']['total_max'])
                if 'err_cntrs' in ret['latency'][pg_id] and 'err_cntrs' in (self.ref['latency']
                                                                            [pg_id]):
                    for key in ret['latency'][pg_id]['err_cntrs']:
                        if key in self.ref['latency'][pg_id]['err_cntrs']:
                            ret['latency'][pg_id]['err_cntrs'][key] -= (self.ref['latency'][pg_id]
                                                                        ['err_cntrs'][key])
        return ret
        ...
        if 'latency' in self.ref and 'latency' in self.last:
            for pg_id in self.ref['latency']:
                if 'latency' in self.ref['latency'][pg_id] and pg_id in self.last['latency']:
                    if 'total_max' in self.ref['latency'][pg_id]['latency']:
                        if (self.last['latency'][pg_id]
                            ['latency']['last_max']) > (self.ref['latency']
                                                        [pg_id]['latency']['total_max']):
                                (self.ref['latency'][pg_id]
                                 ['latency']['total_max']) = (self.last['latency'][pg_id]
                                                              ['latency']['last_max'])
                    else:
                        self.ref['latency'][pg_id]['latency']['total_max'] = self._get(
                            stats, ['latency', pg_id, 'latency', 'total_max'], default=0)
                    (self.last['latency'][pg_id]
                     ['latency']['total_max']) = self.ref['latency'][pg_id]['latency']['total_max']
