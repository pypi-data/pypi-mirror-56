...
#   File "/home/dedie/workspace/slsecure/test/system/libraries/PerformanceKeywords.py", line 47, in create_stream_with_profile
#     print("\nError while loading profile {0}, error: {1}\n".format(profile_file, err))
#   File "/home/dedie/.local/lib/python3.7/site-packages/trex_stl_lib/trex_stl_exceptions.py", line 33, in __str__
...
class STLError(Exception):
    ...
    def __str__(self):
        ...
        for i, line in enumerate(self.tb):
            ...
            s += "#{:<2}    {:<50} - '{}'\n".format(len(self.tb) - i - 1,
                                                    format_text(
                fname, 'bold') + ':' + format_text(lineno, 'bold'),
...
class STLPortStateError(STLError):
    def __init__(self, port, op, state):
        self.msg = ("Operation '{0}' on port(s) '{1}" +
                    "' is not valid while port(s) '{2}'".format(op,
                                                                port,
                                                                state))
...
class STLTypeError(STLError):
    def __init__(self, arg_name, arg_type, valid_types):
        self.tb = traceback.extract_stack()
        self.msg = ("Argument: '%s' invalid type: '%s', expecting type(s): %s."
                    % (arg_name, arg_type.__name__, [t.__name__
...
class STLTimeoutError(STLError):
    def __init__(self, timeout):
        self.tb = traceback.extract_stack()
        self.msg = "Timeout: operation took more than '{0}' seconds".format(
            timeout)
