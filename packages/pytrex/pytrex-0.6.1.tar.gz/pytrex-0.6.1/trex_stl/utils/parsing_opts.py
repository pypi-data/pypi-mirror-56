...
from .common import (is_valid_ipv4, is_valid_ipv6, is_valid_mac, list_difference, list_remove_dup)
...
'''
# Wacky constant assigning code, replacing with enum
#
...
PORT_LIST = 4
ALL_PORTS = 5
PORT_LIST_WITH_ALL = 6
FILE_PATH = 7
FILE_FROM_DB = 8
SERVER_IP = 9
STREAM_FROM_PATH_OR_FILE = 10
DURATION = 11
TIMEOUT = 12
FORCE = 13
READONLY = 14
...
MBUF_STATS = 71
EXTENDED_STATS = 72
EXTENDED_INC_ZERO_STATS = 73
STREAMS_MASK = 75
CORE_MASK_GROUP = 76
CAPTURE_PORTS_GROUP = 77
MONITOR_TYPE_VERBOSE = 79
MONITOR_TYPE_PIPE = 80
MONITOR_TYPE = 81
BPF_FILTER = 82
VLAN_TAGS = 84
CLEAR_VLAN = 85
...
def check_negative(value):
    ...
    if ivalue < 0:
        raise argparse.ArgumentTypeError(
            "non positive value provided: '{0}'".format(value))
    return ivalue
...
match_multiplier_help = """Multiplier should be passed in the following format:
                          ...
                          no suffix will provide an absoulute
                          ...
                          '-m 23mbps': is 23mbps in L2 bandwidth
                                       (including FCS+4)
...
def hex_int(val):
    ...
    if not re.match(pattern, val):
        raise argparse.ArgumentTypeError(
            "{0} is not a valid positive HEX formatted number".format(val))
...
def is_valid_file(filename):
    if os.path.isdir(filename):
        raise argparse.ArgumentTypeError(
            "Given path '%s' is a directory" % filename)
...
class ScapyDecoder(object):
    ...
    def init():
        ...
        ScapyDecoder.scapy_layers = {k: v for k, v in list(
            raw.items()) if (inspect.isclass(v)
                             and issubclass(v, scapy.all.Packet))}
    ...
    def to_scapy(scapy_str):
        ...
        try:
            scapy_obj = eval(scapy_str, {
                             '__builtins__': {}, 'True': True, 'False': False},
                             ScapyDecoder.scapy_layers)
            len(scapy_obj)
            return scapy_obj
        except Exception as e:
            raise argparse.ArgumentTypeError(
                "invalid scapy expression: '{0}' - {1}"
                .format(scapy_str, str(e)))
...
def check_ipv4_addr(ipv4_str):
    if not is_valid_ipv4(ipv4_str):
        raise argparse.ArgumentTypeError(
            "invalid IPv4 address: '{0}'".format(ipv4_str))
...
def check_ip_addr(addr):
    if not(is_valid_ipv4(addr) or is_valid_ipv6(addr)):
        raise argparse.ArgumentTypeError(
            "invalid IPv4/6 address: '{0}'".format(addr))
...
def check_pkt_size(pkt_size):
    try:
        ...
        raise argparse.ArgumentTypeError(
            "invalid packet size type: '{0}'".format(pkt_size))

    if(pkt_size < 64) or(pkt_size > 9216):
        raise argparse.ArgumentTypeError(
            "invalid packet size: '{0}' - valid range is 64 to 9216"
            .format(pkt_size))
...
def check_mac_addr(addr):
    if not is_valid_mac(addr):
        raise argparse.ArgumentTypeError(
            "not a valid MAC address: '{0}'".format(addr))
...
def decode_tunables(tunable_str):
    ...
    for token in tokens:
        ...
        if not m:
            raise argparse.ArgumentTypeError(
                "bad syntax for tunables: {0}".format(token))
        val = m.group(2)           # string
        # need to remove the quotes from value
...
OPTIONS_DB = {MULTIPLIER: ArgumentPack(['-m', '--multiplier'],
              ...
              CAPTURE_PORTS_GROUP: ArgumentGroup(NON_MUTEX, [TX_PORT_LIST,
                                                             RX_PORT_LIST],
                                                 {}),
...
class _MergeAction(argparse._AppendAction):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if not items:
            ...
            raise Exception(
                "Argparser 'merge' option should be used on dict or list.")
...
class CCmdArgParser(argparse.ArgumentParser):
    ...
    def add_subparsers(self, *args, **kwargs):
        ...
        def remove_parser(name):
            if name in sub._name_parser_map:
                ...
                self._print_message(
                    bold('Subparser "%s" does not exist!' % name))
    ...
    def parse_args(self, args=None, namespace=None, default_ports=None, verify_acquired=False):
        try:
            ...
            if(getattr(opts, "all_ports", None) is True) or(getattr(opts, "ports", None) == []):
            ...
            invalid_ports = list_difference(
                opts.ports, self.stateless_client.get_all_ports())
            if invalid_ports:

                if len(invalid_ports) > 1:
                    msg = "{0}: port(s) {1} are not valid port IDs".format(
                        self.cmd_name, invalid_ports)
                else:
                    msg = "{0}: port {1} is not a valid port ID".format(
                        self.cmd_name, invalid_ports[0])
        ...
        try:
            ...
            if isinstance(argument, ArgumentGroup):
                if argument.type == MUTEX:
                    ...
                    raise Exception(
                        'Invalid ArgumentGroup type, should be either MUTEX or NON_MUTEX')
