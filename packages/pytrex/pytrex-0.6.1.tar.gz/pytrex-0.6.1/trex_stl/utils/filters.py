...
class ToggleFilter(object):
    """
    This class provides a "sticky" filter, that works by "toggling" items
    of the original database on and off.
    """
    ...
    def __init__(self, db_ref, show_by_default=True):
        ...
        :parameters:
             db_ref : iterable
                an iterable object(i.e. list, set etc) that would serve as
                the reference db of the instance.
                Changes in that object will affect the output of
                ToggleFilter instance.

             show_by_default: bool
                decide if by default all the items are "on", i.e. these
                items will be presented if no other
                toggling occurred.
    ...
    def toggle_item(self, item_key):
        ...
        :parameters:
             item_key :
                an item the by its value the filter can decide to toggle
                or not.
        ...
        :raises:
            + KeyError, in case if item key is not part of the toggled
            list and not part of the referenced db.
        ...
        if item_key in self._toggle_db:
            ...
            raise KeyError("Provided item key isn't a key " +
                           "of the referenced data structure.")
    ...
    def toggle_items(self, *args):
        ...
        :parameters:
             args : iterable
                an iterable object containing all item keys to be
                toggled in/out

        :return:
            + **True** if all toggled items were toggled **into**
            the filtered items
            + **False** if at least one of the items was toggled
            **out from** the filtered items

        :raises:
            + KeyError, in case if ont of the item keys was not part of
            the toggled list and not part of the referenced db.

        """
        # in python 3, 'map' returns an iterator, so wrapping with 'list'
        # call creates same effect for both python 2 and 3
        return all(list(map(self.toggle_item, args)))

    def filter_items(self):
        """
        Filters the pointed database by showing only the
        items mapped at toggle_db set.
    ...
    def __set_initial_state(self, show_by_default):
        try:
            ...
            if show_by_default:
                # assuming all relevant data with unique identifier
                self._toggle_db = set(shallow_copy(self._data))
            return _state
        except TypeError:
    ...
    def list_filter(function, iterable):
        # in python 3, filter returns an iterator, so wrapping with
        # list creates same effect for both python 2 and 3
