...
from .trex_stl_packet_builder_scapy import RawPcapReader
...
class STLSim(object):
    ...
    def run(self,
        ...
        # listify
        input_list = input_list if isinstance(
            input_list, list) else [input_list]
        ...
        for stream in stream_list:
            ...
            if next:
                if next not in lookup:
                    raise STLError(
                        "stream dependency error - unable to find '{0}'".format(next))
                next_id = lookup[next]
        ...
        if mode == 'json':
            print((json.dumps(cmds_json, indent=4,
    ...
    def execute_bp_sim(self, json_filename):
        ...
        if not os.path.exists(exe):
            raise STLError(
                "'{0}' does not exists, please build it before calling the simulation".format(exe))
    ...
    def merge_results(self):
        ...
        inputs = ["{0}-{1}".format(self.outfile, index)
                  for index in range(0, self.dp_core_count)]
...
def setParserOptions():
    ...
    group.add_argument('--native',
                       help=('generate Python code with stateless profile '
                             + 'from input file [default is False]'),
                       action='store_true',
                       default=False)
...
def validate_args(parser, options):
    ...
    if options.dp_core_index:
        if options.dp_core_index not in range(0, options.dp_core_count):
            parser.error("DP core index valid range is 0 to {0}".format(
                options.dp_core_count - 1))
...
def test_multi_core(r, options):
    ...
    for core_count in range(1, 9):
        print((format_text("comparing {0} cores to 1 core:\n".format(
            core_count), 'underline')))
        rc = compare_caps_strict(
            'generated/1.cap', 'generated/{0}.cap'.format(core_count))
        if rc:
            print("[Passed]\n")
