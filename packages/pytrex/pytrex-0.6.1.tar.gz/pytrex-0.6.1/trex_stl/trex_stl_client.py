...
from .trex_stl_psv import (PSV_ACQUIRED, PSV_IDLE, PSV_L3, PSV_NON_SERVICE, PSV_RESOLVED, PSV_SERVICE, PSV_UP,
                           PortStateValidator)
from .trex_stl_stats import (ALL_STATS_OPTS, COMPACT, CGlobalStats, CLatencyStats, CPgIdStats, CRxStats,
                             CTRexInfoGenerator, CUtilStats, CXStats)
from .trex_stl_streams import STLProfile, STLStream
from .trex_stl_types import RC, RC_ERR, RC_OK, StatNotAvailable, listify, validate_type
from .trex_stl_vlan import VLAN
from .utils import common, parsing_opts, text_tables
from .utils.common import (PassiveTimer, is_sub_list, is_valid_ipv4, is_valid_ipv6, is_valid_mac, list_difference,
...
class LoggerApi(object):
    ...
    def set_verbose(self, level):
        if level not in range(self.VERBOSE_QUIET, self.VERBOSE_HIGH + 1):
...
class STLClient(object):
    ...
    def __init__(self,
        ...
        .. code-block:: python
            ...
            # change user name
            c = STLClient(username = "root",server = "10.0.0.10",
    ...
    def __push_remote(self, pcap_filename, port_id_list, ipg_usec, speedup, count,
        ...
        for port_id in port_id_list:
            ...
            slave_handler = self.ports[port_id ^
                                       0x1].handler if is_dual else ""
    ...
    def __clear_stats(self, port_id_list, clear_global, clear_flow_stats,
                      clear_latency_stats, clear_xstats):
        ...
        if clear_flow_stats or clear_latency_stats:
            self.pgid_stats.clear_stats(
                clear_flow_stats=clear_flow_stats, clear_latency_stats=clear_latency_stats)

        if clear_xstats:
            self.xstats.clear_stats()

        self.logger.log_cmd(
            "Clearing stats on port(s) {0}:".format(port_id_list))
    ...
    def __get_stats(self, port_id_list):
        ...
        for port_id in port_id_list:
            ...
            for k, v in list(port_stats.items()):
                if k not in total:
                    total[k] = v
                else:
                    total[k] += v
    ...
    def __decode_core_mask(self, ports, core_mask):
        available_modes = [self.CORE_MASK_PIN,
                           self.CORE_MASK_SPLIT, self.CORE_MASK_SINGLE]

        # predefined modes
        if isinstance(core_mask, int):
            if core_mask not in available_modes:
                raise STLError(
                    "'core_mask' can be either %s or a list of masks" % ', '.join(available_modes))
            ...
            for port in ports:
                ...
                if(core_mask == self.CORE_MASK_PIN) and((port ^ 0x1) in ports):
                    decoded_mask[port] = 0x55555555 if(
                        port % 2) == 0 else 0xAAAAAAAA
    ...
    def _validate_port_list(self, port_id_list, allow_empty=False):
        ...
        for port_id in port_id_list:
            if port_id not in valid_ports:
                raise STLError(
                    "Port ID '{0}' is not a valid port ID - valid values: {1}"
                    .format(port_id, valid_ports))
    ...
    def _get_formatted_stats(self, port_id_list, stats_mask=COMPACT):
        ...
        for stats_type in stats_opts:
            stats_obj.update(self.stats_generator.generate_single_statistic(
    ...
    def _get_streams(self, port_id_list, streams_mask=set()):
        ...
        streams_obj = self.stats_generator.generate_streams_info(
            port_id_list, streams_mask)
    ...
    def _remove_rx_filters(self, ports, rx_delay_ms):
        ...
        rx_ports = [
            port_id for port_id in ports if self.ports[port_id].has_rx_enabled()]

        if not rx_ports:
            return RC_OK()

        # block while any RX configured port has not yet have it's delay expired
        while any([not self.ports[port_id].has_rx_delay_expired(rx_delay_ms)
    ...
    def __api_check(connected=True):
        ...
        def wrap(f):
            ...
            def wrap2(*args, **kwargs):
                ...
                try:
                    # before we enter the API, set the async thread to signal
                    ...
                    if connected and not client.is_connected():
                        ...
                        if client.conn.is_marked_for_disconnect():
                            ...
                            raise STLError("'{0}' - connection to the server had been lost: '{1}'"
                                           .format(func_name,
                                                   client.conn.get_disconnection_cause()))
                        else:
                            # simply was called while disconnected
                            raise STLError(
                                "'{0}' - is not valid while disconnected".format(func_name))

                    # call the API
                    ret = f(*args, **kwargs)

                except KeyboardInterrupt:
                    # SIGINT can be either from ctrl + c or from the async thread to
                    # interrupt the main thread
                    if client.conn.is_marked_for_disconnect():
                        raise STLError("'{0}' - connection to the server had been lost: '{1}'"
                                       .format(func_name, client.conn.get_disconnection_cause()))
                    else:
                        raise STLError(
                            "'{0}' - interrupted by a keyboard signal(probably ctrl + c)"
                            .format(func_name))
    ...
    def get_port(self, port_id):
        ...
        if(port is not None):
            ...
            raise STLArgumentError(
                'port id', port_id, valid_values=self.get_all_ports())
    ...
    def get_acquired_ports(self):
        return [port_id
                for port_id, port_obj in list(self.ports.items())
    ...
    def get_stats(self, ports=None, sync_now=True):
        ...
        :parameters:
          ...
          sync_now - Boolean - If true, create a call to the server to get latest stats,
          and wait for result to arrive. Otherwise, return last stats saved in client cache.
                            Downside of putting True is a slight delay(few 10th msecs)
                            in getting the result. For practical uses, value should be True.
        :return:
            Statistics dictionary of dictionaries with the below format.

            **Note** that for getting latency and flow_stats statistics there
            is now a newer and more efficient API
            :ref:`get_pgid_stats<get_pgid_stats>`. In the future, flow stats and
            latency info might be removed
            ...
            **total** and per port statistics contain dictionary with following format.
            ...
            Most of the bytes counters(unless specified otherwise) are in L2 layer,
            including the Ethernet FCS. e.g. minimum packet size is 64 bytes
            ...
            **flow_stats** contains :ref:`global dictionary <flow_stats_global_o>`,
            and dictionaries per packet group id(pg id). See structures below.
            ...
            **global flow stats** dictionary has the following structure:
            ...
            key                 Meaning
            =================   ===============
            rx_err              Number of flow statistics packets received
            that we could not associate to any pg_id. This can happen if
            latency on the used setup is large. See :ref:`wait_on_traffic
            <wait_on_traffic>` rx_delay_ms parameter for details.
            tx_err              Number of flow statistics packets transmitted
            that we could not associate to any pg_id. This is never expected.
            If you see this different than 0, please report.
            =================   ===============
            ...
            **global**
            ...
            key                 Meaning
            =================   ===============
            bw_per_core         Estimated bit rate Trex can support per core.
            This is calculated by extrapolation of current rate and load on transmitting cores.
            cpu_util            Estimate of the average utilization percentage of
            the transimitting cores
            queue_full          Total number of packets transmitted while the NIC TX queue was full.
            The packets will be transmitted, eventually, but will create high CPU%due to
            polling the queue.  This usually indicates that the rate we trying to transmit
            is too high for this port.
            rx_cpu_util         Estimate of the utilization percentage of the core
            handling RX traffic. Too high value of this CPU utilization could cause
            drop of latency streams.
            ...
            **latency** contains :ref:`global dictionary <lat_stats_global_o>`,
            and dictionaries per packet group id(pg id). Each one with the following structure.
            ...
            **per pg_id latency stat** dictionaries have following structure:
            ...
            key                                    Meaning
            =============================          ===============
            :ref:`err_cntrs<err-cntrs_o>`          Counters describing errors that occured with
            this pg id
            ...
            **err-cntrs**
            ...
            For calculating packet error events, we add sequence number to each packet's payload.
            We decide what went wrong only according to sequence number
            of last packet received and that of the previous packet. 'seq_too_low' and
            'seq_too_high' count events we see. 'dup', 'out_of_order' and 'dropped'
            are heuristics we apply to try and understand what happened. They will be accurate
            in common error scenarios.
            We describe few scenarios below to help understand this.

            Scenario 1: Received packet with seq num 10, and another one with seq num 10.
            We increment 'dup' and 'seq_too_low' by 1.

            Scenario 2: Received pacekt with seq num 10 and then packet with seq num 15.
            We assume 4 packets were dropped, and increment 'dropped' by 4, and 'seq_too_high' by 1.
            We expect next packet to arrive with sequence number 16.

            Scenario 2 continue: Received packet with seq num 11. We increment 'seq_too_low' by 1.
            We increment 'out_of_order' by 1. We *decrement* 'dropped' by 1.
           (We assume here that one of the packets we considered as dropped before,
           actually arrived out of order).
            ...
            **latency**
            ...
            key                 Meaning
            =================   ===============
            average             Average latency over the stream lifetime(usec).Low pass filter
            is applied to the last window average.It is computed each sampling period by
            following formula: <average> = <prev average>/2 + <last sampling period average>/2
            histogram           Dictionary describing logarithmic distribution histogram
            of packet latencies. Keys in the dictionary represent range of latencies(in usec).
            Values are the total number of packets received in this latency range.
            For example, an entry {100:13} would mean that we saw 13 packets with latency
            in the range between 100 and 200 usec.
            jitter              Jitter of latency samples, computed as described in :
            rfc:`3550#appendix-A.8`
            last_max            Maximum latency measured between last two data reads
            from server(0.5 sec window).
            ...
            **global latency stats** dictionary has the following structure:
            ...
            key                 Meaning
            =================   ===============
            old_flow            Number of latency statistics packets received that
            we could not associate to any pg_id. This can happen if latency on the
            used setup is large. See :ref:`wait_on_traffic <wait_on_traffic>` rx_delay_ms
            parameter for details.
            bad_hdr             Number of latency packets received with bad latency data.
            This can happen becuase of garbage packets in the network,
            or if the DUT causes packet corruption.
            =================   ===============
    ...
    def set_verbose(self, level):
        ...
        """
        modes = {'low': LoggerApi.VERBOSE_QUIET,
                 'normal': LoggerApi.VERBOSE_REGULAR, 'high': LoggerApi.VERBOSE_HIGH}

        if level not in list(modes.keys()):
    ...
    def disconnect(self, stop_traffic=True, release_ports=True):
        ...
        self.logger.pre_cmd("Disconnecting from server at '{0}':'{1}'"
                            .format(self.connection_info['server'],
        ...
        for port_id in ports:
            if not self.ports[port_id].is_resolved():
                self.logger.log(format_text(
                    '*** Warning - Port {0} destination is unresolved ***'.format(port_id), 'bold'))
    ...
    def ping_rpc_server(self):
        ...
        self.logger.pre_cmd("Pinging the server on '{0}' port '{1}': "
                            .format(self.connection_info['server'],
                                    self.connection_info['sync_port']))
    ...
    def set_l2_mode(self, port, dst_mac):
        ...
        if not is_valid_mac(dst_mac):
            raise STLError(
                "dest_mac is not a valid MAC address: '{0}'".format(dst_mac))
    ...
    def set_l3_mode(self, port, src_ipv4, dst_ipv4, vlan=None):
        ...
        if not is_valid_ipv4(src_ipv4):
            raise STLError(
                "src_ipv4 is not a valid IPv4 address: '{0}'".format(src_ipv4))

        if not is_valid_ipv4(dst_ipv4):
            raise STLError(
                "dst_ipv4 is not a valid IPv4 address: '{0}'".format(dst_ipv4))
    ...
    def ping_ip(self, src_port, dst_ip, pkt_size=64, count=5, interval_sec=1, vlan=None):
        """
            ...
            :parameters:
                 ...
                 vlan         - one or two VLAN tags o.w it will be taken from
                                the src port configuration
            ...
            :returns:
                ...
                * formatted_string - string, human readable output, for example:
                'Request timed out.'
        ...
        if not(is_valid_ipv4(dst_ip) or is_valid_ipv6(dst_ip)):
            raise STLError(
                "PING - dst_ip is not a valid IPv4/6 address: '{0}'".format(dst_ip))

        if(pkt_size < 64) or(pkt_size > 9216):
            raise STLError(
                "PING - pkt_size should be a value between 64 and 9216: '{0}'".format(pkt_size))
        ...
        if is_valid_ipv4(dst_ip):
            self.psv.validate('PING IPv4', src_port,
                              (PSV_ACQUIRED, PSV_SERVICE, PSV_L3))
        else:
            self.psv.validate('PING IPv6', src_port,
                              (PSV_ACQUIRED, PSV_SERVICE))

        # if vlan was given use it, otherwise generate it from the port
        vlan = VLAN(self.ports[src_port].get_vlan_cfg()
                    if vlan is None else vlan)

        if vlan:
            self.logger.pre_cmd("Pinging {0} from port {1} over {2} with {3} bytes of data:"
                                .format(dst_ip, src_port, vlan.get_desc(), pkt_size))
        else:
            self.logger.pre_cmd("Pinging {0} from port {1} with {2} bytes of data:"
                                .format(dst_ip, src_port, pkt_size))
    ...
    def _ping_ipv6(self, src_port, vlan, dst_ip, pkt_size, count, interval_sec):
        ...
        with self.logger.supress(level=LoggerApi.VERBOSE_REGULAR_SYNC):
            ...
            for i in range(count):
                rc = self.ports[src_port].ping_ipv6(
                    ping_ip=dst_ip, pkt_size=pkt_size, dst_mac=dst_mac)
                if not rc:
                    raise STLError(rc)
    ...
    def server_shutdown(self, force=False):
        ...
        rc = self._transmit("shutdown", params={
                            'force': force, 'user': self.username})
    ...
    def get_active_pgids(self):
        """
            ...
            :returns:
                Dict with entries 'latency' and 'flow_stats'.
                Each entry contains list of used packet group IDs
                of the given type.
    ...
    def get_pgid_stats(self, pgid_list=[]):
        ...
        :parameters:
            pgid_list: list
                ...
                Allows to get statistics for 1024 flows in one
                call(will return error if asking for more).
        :return:
            Return dictionary containing packet group id statistics
            ...
            **flow_stats** contains :ref:`global dictionary <flow_stats_global>`,
            and dictionaries per packet group id(pg id). See structures below.
            ...
            **global flow stats** dictionary has the following structure:
            ...
            key                 Meaning
            =================   ===============
            rx_err              Number of flow statistics packets received that we could
                                not associate to any pg_id. This can happen if latency on
                                the used setup is large. See :ref:`wait_on_traffic
                                <wait_on_traffic>` rx_delay_ms parameter for details.
            tx_err              Number of flow statistics packets transmitted that we could
                                not associate to any pg_id. This is never expected.
                                If you see this different than 0, please report.
            =================   ===============

            .. _latency:

            **latency** contains :ref:`global dictionary <lat_stats_global>`,
            and dictionaries per packet group id(pg id). Each one with the following structure.
            ...
            **per pg_id latency stat** dictionaries have following structure:
            ...
            key                                  Meaning
            ===========================          ===============
            :ref:`err_cntrs<err-cntrs>`          Counters describing errors that occured
                                                 with this pg id
            ...
            **err-cntrs**
            ...
            For calculating packet error events, we add sequence number to each packet's payload.
            We decide what went wrong only according to sequence number
            of last packet received and that of the previous packet.
            'seq_too_low' and 'seq_too_high' count events we see.
            'dup', 'out_of_order' and 'dropped' are heuristics we apply to try and
            understand what happened. They will be accurate in common error scenarios.
            We describe few scenarios below to help understand this.
            ...
            **latency**
            ...
            average             Average latency over the stream lifetime(usec).Low pass filter
                                ...
                                period by following formula:
                                <average> = <prev average>/2 + <last sampling period average>/2
            histogram           Dictionary describing logarithmic distribution histogram of
                                packet latencies. Keys in the dictionary represent range of
                                latencies(in usec). Values are the total number of packets received
                                in this latency range. For example, an entry {100:13} would mean
                                that we saw 13 packets with latency in the range between
                                100 and 200 usec.
            jitter              Jitter of latency samples, computed as described in:
                                rfc:`3550#appendix-A.8`
            last_max            Maximum latency measured between last two data reads from
                                server(0.5 sec window).
            total_max           Maximum latency measured over the stream lifetime(in usec).
            total_min           Minimum latency measured over the stream lifetime(in usec).
            =================   ===============
            ...
            **global latency stats** dictionary has the following structure:
            ...
            key                 Meaning
            =================   ===============
            old_flow            Number of latency statistics packets received that we could
                                not associate to any pg_id. This can happen if latency on the
                                used setup is large. See :ref:`wait_on_traffic <wait_on_traffic>`
                                rx_delay_ms parameter for details.
            bad_hdr             Number of latency packets received with bad latency data.
                                This can happen becuase of garbage packets in the network,
                                or if the DUT causes packet corruption.
            =================   ===============
        ...
        while index <= pgid_list_len:
            ...
            rc = self._transmit("get_pgid_stats", params={
                                'pgids': curr_pgid_list})

            if not rc:
                raise STLError(rc)

            for key in list(rc.data().keys()):
                if key in ans_dict:
                    try:
        ...
        j_to_p_lat = {'jit': 'jitter', 'average': 'average',
                      'total_max': 'total_max', 'last_max': 'last_max'}
        j_to_p_err = {'drp': 'dropped', 'ooo': 'out_of_order',
                      'dup': 'dup', 'sth': 'seq_too_high', 'stl': 'seq_too_low'}
        ...
        if 'latency' in list(ans_dict.keys()) and ans_dict['latency'] is not None:
            new['latency'] = {}
            new['latency']['global'] = {}
            for key in list(j_to_p_global.keys()):
            ...
            for pg_id in ans_dict['latency']:
                ...
                if 'er' in ans_dict['latency'][pg_id]:
                    for key in list(j_to_p_err.keys()):
                        if key in ans_dict['latency'][pg_id]['er']:
                            new['latency'][int_pg_id]['err_cntrs'][j_to_p_err[key]
                                                                   ] = ans_dict[
                                                                       'latency'
                                                                       ][pg_id]['er'][key]
                        else:
                ...
                for field in list(j_to_p_lat.keys()):
                    if field in ans_dict['latency'][pg_id]['lat']:
                        new['latency'][int_pg_id]['latency'][j_to_p_lat[field]
                                                             ] = ans_dict[
                                                                 'latency'
                                                                 ][pg_id]['lat'][field]
                    else:
                        new['latency'][int_pg_id]['latency'][j_to_p_lat[field]
                                                             ] = StatNotAvailable(field)
                ...
                if 'histogram' in ans_dict['latency'][pg_id]['lat']:
                    ...
                    min_val = min(new['latency'][int_pg_id]
                                  ['latency']['histogram'])
                    if min_val == 0:
                        min_val = 2
                    new['latency'][int_pg_id]['latency']['total_min'] = min_val
                else:
                    new['latency'][int_pg_id]['latency']['total_min'] = StatNotAvailable(
                        'total_min')
                    new['latency'][int_pg_id]['latency']['histogram'] = {}

        # translate json 'flow_stats' to python API 'flow_stats'
        if 'flow_stats' in list(ans_dict.keys()) and ans_dict['flow_stats'] is not None:
            new['flow_stats'] = {}
            ...
            for pg_id in ans_dict['flow_stats']:
                ...
                # do this only once
                if all_ports == []:
                    # if field does not exist, we don't know which ports we have.
                    # We assume 'tp' will always exist
                    for port in ans_dict['flow_stats'][pg_id]['tp']:
                        all_ports.append(int(port))

                new['flow_stats'][int_pg_id] = {}
                for field in list(j_to_p_f_stat.keys()):
                    ...
                    if field in ans_dict['flow_stats'][pg_id]:
                        for port in ans_dict['flow_stats'][pg_id][field]:
                            new['flow_stats'][int_pg_id][j_to_p_f_stat[field]][int(
                                port)] = ans_dict['flow_stats'][pg_id][field][port]
                            total += new['flow_stats'][int_pg_id][j_to_p_f_stat[field]
                                                                  ][int(port)]
                        new['flow_stats'][int_pg_id][j_to_p_f_stat[field]
                                                     ]['total'] = total
                    else:
                        for port in all_ports:
                            new['flow_stats'][int_pg_id][j_to_p_f_stat[field]][int(
                                port)] = StatNotAvailable(j_to_p_f_stat[field])
                        new['flow_stats'][int_pg_id][j_to_p_f_stat[field]
                                                     ]['total'] = StatNotAvailable('total')
                ...
                for port in new['flow_stats'][int_pg_id]['rx_pkts']:
                    ...
                    new['flow_stats'][int_pg_id]['rx_bps_l1'][port] = float(
                        new['flow_stats'][int_pg_id]['rx_bps'][port]) + float(new['flow_stats']
                                                                              [int_pg_id]
                                                                              ['rx_pps']
                                                                              [port]) * 20 * 8
                    new['flow_stats'][int_pg_id]['tx_bps_l1'][port] = float(
                        new['flow_stats'][int_pg_id]['tx_bps'][port]) + float(new['flow_stats']
                                                                              [int_pg_id]
                                                                              ['tx_pps']
                                                                              [port]) * 20 * 8

            if 'g' in ans_dict['flow_stats']:
                for field in list(j_to_p_g_f_err.keys()):
                    if field in ans_dict['flow_stats']['g']:
                        new['flow_stats']['global'][j_to_p_g_f_err[field]
                                                    ] = ans_dict['flow_stats']['g'][field]
                    else:
                        new['flow_stats']['global'][j_to_p_g_f_err[field]] = {}
                        for port in all_ports:
                            new['flow_stats']['global'][j_to_p_g_f_err[field]
    ...
    def get_xstats(self, port_id):
        """
            ...
            :returns:
                Dict with names of counters as keys and values of uint64.
                Actual keys may vary per NIC.
    ...
    def remove_all_streams(self, ports=None):
        ...
        self.logger.pre_cmd(
            "Removing all streams from port(s) {0}:".format(ports))
    ...
    def add_streams(self, streams, ports=None):
        ...
        self.logger.pre_cmd(
            "Attaching {0} streams to port(s) {1}:".format(len(streams), ports))
    ...
    def remove_streams(self, stream_id_list, ports=None):
        ...
        # remove streams
        self.logger.pre_cmd("Removing {0} streams from port(s) {1}:".format(
            len(stream_id_list), ports))
    ...
    def start(self,
        ...
        """
            ...
            :parameters:
                ...
                force : bool
                    If the ports are not in stopped mode or do not have sufficient bandwidth
                    for the traffic, determines whether to stop the current traffic and force start.
                    True: Force start
                ...
                total : bool
                    Determines whether to divide the configured bandwidth among the ports,
                    or to duplicate the bandwidth for each port.
                    True: Divide bandwidth among the ports
                    False: Duplicate

                core_mask: CORE_MASK_SPLIT, CORE_MASK_PIN, CORE_MASK_SINGLE or a
                list of masks(one per port)
        ...
        if synchronized:
            ...
            if len(ports) % 2:
                raise STLError(
                    'Must use even number of ports in synchronized mode')
            for port in ports:
                if port ^ 1 not in ports:
                    raise STLError(
                        'Must use adjacent ports in synchronized mode. Port "%s" has not pair.'
                        % port)

            start_time = time.time()
            with self.logger.supress():
                ping_data = self.ping_rpc_server()
            start_at_ts = ping_data['ts'] + \
                max((time.time() - start_time), 0.5) * len(ports)
        ...
        # start traffic
        self.logger.pre_cmd(
            "Starting {}traffic on port(s) {}:".format(synchronized_str, ports))
        rc = self.__start(mult_obj, duration, ports,
                          force, decoded_mask, start_at_ts)
        self.logger.post_cmd(rc)
    ...
    def update(self, ports=None, mult="1", total=False, force=False):
        """
            ...
            :parameters:
                ...
                force : bool
                    If the ports are not in stopped mode or do not have sufficient
                    bandwidth for the traffic, determines whether to stop the current
                    traffic and force start.
    ...
    def pause(self, ports=None):
        """
            Pause traffic on port(s). Works only for ports that are active,
            and only if all streams are in Continuous mode.
    ...
    def push_remote(self,
        ...
        if force:
            active_ports = list(
                set(self.get_active_ports()).intersection(ports))
            if active_ports:
                self.stop(active_ports)
        ...
        if is_dual:
            if not pcap_filename.endswith('erf'):
                raise STLError(
                    "dual mode: only ERF format is supported for dual mode")
            ...
            for port in ports:
                ...
                if slave in ports:
                    raise STLError(
                        "dual mode: cannot provide adjacent ports({0}, {1}) in a batch"
                        .format(master, slave))

                if slave not in self.get_acquired_ports():
                    raise STLError(
                        "dual mode: adjacent port {0} must be owned during dual mode".format(slave))

        self.logger.pre_cmd(
            "Pushing remote PCAP on port(s) {0}:".format(ports))
        rc = self.__push_remote(
            pcap_filename, ports, ipg_usec, speedup, count, duration, is_dual, min_ipg_usec)
        self.logger.post_cmd(rc)
    ...
    def push_pcap(self,
        ...
        if all([ipg_usec, min_ipg_usec]):
            raise STLError(
                'Please specify either ipg or minimal ipg, not both.')
        ...
        if not force and os.path.getsize(pcap_filename) > (1024 * 1024):
            raise STLError("PCAP size of {:} is too big for local push - "
                           + "consider using remote push or provide 'force'"
                           .format(format_num(os.path.getsize(pcap_filename), suffix='B')))
        ...
        if is_dual:
            for port in ports:
                ...
                if slave in ports:
                    raise STLError(
                        "dual mode: please specify only one of adjacent ports({0}, {1}) in a batch"
                        .format(master, slave))
        ...
        if not is_dual:
            ...
            try:
                self.logger.pre_cmd(
                    "Converting '{0}' to streams:".format(pcap_filename))
            ...
            self.add_streams(profile.get_streams(), ports)
            ...
            if(ipg_usec and ipg_usec < 1000 * speedup) or(min_ipg_usec and min_ipg_usec < 1000):
                self.event_handler.log_warning('In order to get synchronized traffic, '
                                               + 'ensure that effective ipg is at least 1000 usec ')

            try:
                self.logger.pre_cmd("Analyzing '{0}' for dual ports based on {1}:".format(
                    pcap_filename, split_mode))
    ...
    def push_packets(self, pkts, ports=None, force=False, ipg_usec=0):
        ...
        for pkt in pkts:
            if not isinstance(pkt, (Ether, bytes, PacketBuffer)):
                raise STLTypeError('pkts', type(
                    pkt), (Ether, bytes, PacketBuffer))
        ...
        self.logger.pre_cmd(
            "Pushing {0} packets on port(s) {1}:".format(len(pkts), ports))
    ...
    def clear_stats(self, ports=None, clear_global=True, clear_flow_stats=True,
                    clear_latency_stats=True, clear_xstats=True):
        """
        ...
        rc = self.__clear_stats(
            ports, clear_global, clear_flow_stats, clear_latency_stats, clear_xstats)
        if not rc:
            raise STLError(rc)
    ...
    def wait_on_traffic(self, ports=None, timeout=None, rx_delay_ms=None):
        """
            ...
            :parameters:
                ...
                rx_delay_ms : int
                    Time to wait(in milliseconds) after last packet was sent,
                    until RX filters used for measuring flow statistics and
                    latency are removed.
                    This value should reflect the time it takes packets which were
                    transmitted to arrive to the destination.
                    After this time, RX filters will be removed,
                    and packets arriving for per flow statistics
                    feature and latency flows will be counted as errors.
    ...
    def set_port_attr(self,
        ...
        """
            ...
            :parameters:
                ...
                flow_ctrl        - 0: disable all
                                   1: enable tx side
                                   2: enable rx side
                                   3: full enable
                resolve          - if true, in case a destination address is configured as
                                   IPv4 try to resolve it
        ...
        self.logger.pre_cmd(
            "Applying attributes on port(s) {0}:".format(ports))
    ...
    def set_service_mode(self, ports=None, enabled=True):
        ...
        if enabled:
            self.logger.pre_cmd(
                'Enabling service mode on port(s) {0}:'.format(ports))
        else:
            self.logger.pre_cmd(
                'Disabling service mode on port(s) {0}:'.format(ports))
    ...
    def resolve(self, ports=None, retries=0, verbose=True, vlan=None):
        ...
        ports = self.psv.validate(
            'ARP', ports, (PSV_ACQUIRED, PSV_SERVICE, PSV_L3))
        ...
        if vlan:
            self.logger.pre_cmd(
                'Resolving destination over {0} on port(s) {1}:'.format(vlan.get_desc(), ports))
        else:
            self.logger.pre_cmd(
                'Resolving destination on port(s) {0}:'.format(ports))
        ...
        for port in ports:
            ...
            port_vlan = self.ports[port].get_vlan_cfg(
            ) if vlan.is_default() else vlan

            ctx = self.create_service_ctx(port)

            # retries
            for i in range(retries + 1):
                arp = STLServiceARP(ctx, dst_ip=dst_ipv4,
                                    src_ip=src_ipv4, vlan=port_vlan)
                ctx.run(arp)
                if arp.get_record():
                    self.ports[port].set_l3_mode(
                        src_ipv4, dst_ipv4, arp.get_record().dst_mac)
                    break
        ...
        for port, arp in zip(ports, arps):
            if arp.get_record():
                self.logger.log(format_text(
                    "Port {0} - {1}".format(port, arp.get_record()), 'bold'))
            else:
                self.logger.log(format_text(
                    "Port {0} - *** {1}".format(port, arp.get_record()), 'bold'))
    ...
    def scan6(self, ports=None, timeout=5, verbose=False):
        """
            ...
            :parameters:
                ports          - for which ports to apply a unique sniffer
                                 (each port gets a unique file)
        ...
        if ports is None:
            ...
            if not_acquired:
                raise STLError('Following ports are not acquired: %s' %
                               ', '.join(map(str, not_acquired)))

        not_in_service = list_difference(
            ports, self.get_service_enabled_ports())
        if not_in_service:
            raise STLError('Following ports are not in service mode: %s' %
                           ', '.join(map(str, not_in_service)))

        self.logger.pre_cmd(
            'Scanning network for IPv6 nodes on port(s) {0}:'.format(ports))
        ...
        err = RC()
        replies_per_port = {}
        for port, rc in list(rc_per_port.items()):
            if not rc:
                err.add(rc)
            else:
        ...
        if verbose:
            for port, replies in list(replies_per_port.items()):
                if replies:
                    max_ip_len = 0
                    for resp in replies:
                    ...
                    for key in sorted(node_types.keys()):
                        for reply in node_types[key]:
                            scan_table.add_row(
                                [key, reply['mac'], reply['ipv6']])
                    self.logger.log(scan_table.draw())
                    self.logger.log('')
                else:
                    self.logger.log(format_text(
                        'Port %s: no replies! Try to ping with explicit address.' % port, 'bold'))
    ...
    def start_capture(self, tx_ports=None, rx_ports=None, limit=1000, mode='fixed', bpf_filter=''):
        ...
        # verify service mode
        non_service_ports = list_difference(
            merge_ports, self.get_service_enabled_ports(owned=False))
        if non_service_ports:
            raise STLError("Port(s) {0} are not under service mode. "
                           + "packet capturing requires all "
                           + "ports to be in service mode"
                           .format(non_service_ports))

        # actual job
        self.logger.pre_cmd(
            "Starting packet capturing up to {0} packets".format(limit))
    ...
    def stop_capture(self, capture_id, output=None):
        ...
        rc = self._transmit("capture", params={
                            'command': 'stop', 'capture_id': capture_id})
        self.logger.post_cmd(rc)
        if not rc:
            raise STLError(rc)
        ...
        # remove
        self.logger.pre_cmd(
            "Removing PCAP capture {0} from server".format(capture_id))
        rc = self._transmit("capture", params={
                            'command': 'remove', 'capture_id': capture_id})
        self.logger.post_cmd(rc)
        if not rc:
            raise STLError(rc)
    ...
    def fetch_capture_packets(self, capture_id, output, pkt_count=1000):
        ...
        write_to_file = isinstance(output, str)

        self.logger.pre_cmd("Writing up to {0} packets to '{1}'".format(
            pkt_count, output if write_to_file else 'list'))
        ...
        while pending > 0 and pkt_count > 0:
            rc = self._transmit("capture", params={
                                'command': 'fetch', 'capture_id': capture_id,
                                'pkt_limit': min(50, pkt_count)})
            if not rc:
                self.logger.post_cmd(rc)
                raise STLError(rc)
            ...
            for pkt in pkts:
                ...
                if write_to_file:
                    ...
                    writer._write_packet(
                        pkt['binary'], sec=ts_sec, usec=ts_usec)
                else:
                    output.append(pkt)
    ...
    def remove_all_captures(self):
        ...
        for capture_id in list(captures.keys()):
            # remove
            rc = self._transmit("capture", params={
                                'command': 'remove', 'capture_id': capture_id})
            if not rc:
    ...
    def ping_line(self, line):
        ...
        self.ping_ip(opts.ports[0], opts.ping_ip,
                     opts.pkt_size, opts.count, vlan=opts.vlan)
    ...
    def acquire_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_all_ports())
        if not opts:
            return opts
        ...
        if not ports:
            msg = "acquire - all of port(s) {0} are already acquired".format(
                opts.ports)
    ...
    def release_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_acquired_ports())
        if not opts:
            return opts
        ...
        if not ports:
            if not opts.ports:
                ...
                msg = "release - none of port(s) {0} are acquired".format(
                    opts.ports)
                self.logger.log(format_text(msg, 'bold'))
                return RC_ERR(msg)
    ...
    def reacquire_line(self, line):
        ...
        my_unowned_ports = list_difference([k for k, v in list(
            self.ports.items()) if v.get_owner() == self.username], self.get_acquired_ports())
        if not my_unowned_ports:
            msg = "reacquire - no unowned ports under '{0}'".format(
                self.username)
            self.logger.log(msg)
            return RC_ERR(msg)
    ...
    def reset_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_acquired_ports(), verify_acquired=True)
        if not opts:
            return opts
    ...
    def stop_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_active_ports(), verify_acquired=True)
        if not opts:
            return opts
        ...
        if not ports:
            if not opts.ports:
                ...
                msg = 'stop - no active traffic on ports {0}'.format(
                    opts.ports)
    ...
    def update_line(self, line):
        ...
        if not ports:
            if not opts.ports:
                ...
                msg = 'update - no active traffic on ports {0}'.format(
                    opts.ports)
    ...
    def pause_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_transmitting_ports(), verify_acquired=True)
        if not opts:
            return opts
        ...
        if not ports:
            if not opts.ports:
                ...
                msg = 'pause - none of ports {0} are transmitting'.format(
                    opts.ports)
    ...
    def resume_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_paused_ports(), verify_acquired=True)
        if not opts:
            return opts
        ...
        if not ports:
            if not opts.ports:
                ...
                msg = 'resume - none of ports {0} are paused'.format(
                    opts.ports)
    ...
    def show_stats_line(self, line):
        ...
        for stat_type, stat_data in list(stats.items()):
            text_tables.print_table_with_header(
                stat_data.text_table, stat_type)
    ...
    def show_streams_line(self, line):
        ...
        if not streams:
            ...
            # print stats to screen
            for stream_hdr, port_streams_data in list(streams.items()):
                text_tables.print_table_with_header(port_streams_data.text_table,
                                                    header=stream_hdr.split(":")[
                                                        0] + ":",
    ...
    def push_line(self, line):
        ...
        parser = parsing_opts.gen_parser(
            *(args + [parsing_opts.FILE_PATH_NO_CHECK]))
    ...
    def set_port_attr_line(self, line):
        ...
        if not list([opt for opt in opts._get_kwargs() if opt[0] not in('all_ports', 'ports')
                     and opt[1] is not None]):
            ports = opts.ports if opts.ports else self.get_all_ports()
            self.show_stats_line(
                "--ps --port {0}".format(' '.join(str(port) for port in ports)))
            return

        if opts.supp:
            # assume for now all ports are same
            info = self.ports[0].get_formatted_info()
            print('')
    ...
    def resolve_line(self, line):
        ...
        opts = parser.parse_args(
            line.split(), default_ports=self.get_resolvable_ports(), verify_acquired=True)
        if not opts:
            return opts
    ...
    def scan6_line(self, line):
        ...
        rc_per_port = self.scan6(
            ports=opts.ports, timeout=opts.timeout, verbose=True)
    ...
    def set_l3_mode_line(self, line):
        ...
        # source ports maps to ports as a single port
        self.set_l3_mode(
            opts.ports[0], src_ipv4=opts.src_ipv4, dst_ipv4=opts.dst_ipv4, vlan=opts.vlan)
    ...
    def show_profile_line(self, line):
        ...
        self.logger.log(
            'Stream count:     {:^12}'.format(info['stream_count']))
        ...
        if profile_type == 'python':
            ...
            self.logger.log('Tunables:         {:^12}'.format(
                str(['{0} = {1}'.format(k, v) for k, v in list(info['tunables'].items())])))
        ...
        if not self.is_connected():
            ...
            if self.get_service_enabled_ports():
                if self.get_service_enabled_ports() == self.get_acquired_ports():
                    ...
                    p += '(service: {0})'.format(', '.join(map(str,
                                                               self.get_service_enabled_ports())))
    ...
    def pkt_line(self, line):
        ...
        if opts.dry:
            self.logger.log(format_text('\nPacket(Size: {0}):\n'.format(
                format_num(len(opts.scapy_pkt), suffix='B')), 'bold', 'underline'))
            opts.scapy_pkt.show2()
            self.logger.log(format_text(
                '\n*** DRY RUN - no traffic was injected ***\n', 'bold'))
            return

        self.push_packets(pkts=opts.scapy_pkt,
                          ports=opts.ports, force=opts.force)
    ...
    def debug_line(self, line, console_obj):
        ...
        self.logger.log(format_text("\n*** Starting IPython... use 'client' as client object, "
                                    + "Ctrl + D to exit ***\n", 'bold'))
