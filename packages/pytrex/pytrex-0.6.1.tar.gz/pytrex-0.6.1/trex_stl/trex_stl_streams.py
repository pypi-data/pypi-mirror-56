...
class STLStream(object):
    ...
    def to_code(self):
        ...
        while layer:
            ...
            if layer_class not in vars(scapy.layers.all):  # custom import
                ...
                for module_path, module in list(sys.modules.items()):
                    if not module_path.startswith(('scapy.layers', 'scapy.contrib')):
                        continue
                    import_string = 'from %s import %s' % (
                        module_path, layer_class)
                ...
                if not found_import:
                    raise STLError(
                        'Could not determine import of layer %s' % layer.name)
            ...
            if isinstance(layer, Raw):
                ...
                if payload_data == payload_data[0:1] * len(payload_data):
                    layer_command = '%s * %s)' % (
                        Raw(payload_data[0:1]).command().rstrip(')'), len(payload_data))
                else:
        ...
        if default_STLStream.fields['flow_stats'] != self.fields['flow_stats']:
            stream_params_list.append(
                'flow_stats = STLFlowStats(%s)' % self.fields['flow_stats']['stream_id'])
        ...
        if self.mode_desc == STLTXCont.__str__():
            ...
            stream_params_list.append(
                'mode = STLTXSingleBurst(%s)' % mode_args)
...
class YAMLLoader(object):
    ...
    def __parse_stream(self, yaml_object):
        ...
        stream = STLStream(name=yaml_object.get('name'),
                           ...
                           enabled=s_obj.get(
                               'enabled', defaults.fields['enabled']),
                           self_start=s_obj.get(
                               'self_start', defaults.fields['self_start']),
                           isg=s_obj.get('isg', defaults.fields['isg']),
                           next=yaml_object.get('next'),
                           action_count=s_obj.get(
                               'action_count', defaults.fields['action_count']),
                           mac_src_override_by_pkt=s_obj.get(
                               'mac_src_override_by_pkt', 0),
                           mac_dst_override_mode=s_obj.get(
                               'mac_src_override_by_pkt', 0)
                           )
    ...
    def load_pcap(pcap_file,
        ...
        if(ipg_usec is not None and(ipg_usec < 0.001 * speedup) and
           (min_ipg_usec is None or min_ipg_usec < 0.001)):
            raise STLError(
                "ipg_usec cannot be less than 0.001 usec: '{0}'".format(ipg_usec))
        ...
        try:
            if split_mode is None:
                ...
                if not(pkts_a or pkts_b):
                    ...
                    raise STLError(
                        "'%s' contains only one direction." % pcap_file)
    ...
    def load(filename, direction=0, port_id=0, **kwargs):
        ...
        if suffix == 'py':
            profile = STLProfile.load_py(
                filename, direction, port_id, **kwargs)
        ...
        profile.meta['stream_count'] = len(profile.get_streams()) if isinstance(
            profile.get_streams(), list) else 1
        return profile
    ...
    def generate_mac_groups(self):
        for i, (pkt, _) in enumerate(self.pkts_arr):
            if not isinstance(pkt, (Ether, Dot3)):
                raise STLError(
                    "Packet #{0} has an unknown L2 format: {1}".format(i, type(pkt)))
    ...
    def split(self):
        ...
        while len(nodes) > 0:
            ...
            for friend in friends:
                self.log("<FRIEND> {0} --> {1}".format(friend,
                                                       'A' if other_color is color_a else 'B'))
