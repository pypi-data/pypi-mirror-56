...
class RXServiceIPv6(RXServiceAPI):
    ...
    def execute(self, *a, **k):
        ...
        try:
            if mult != True:
                # response might be multicast
                self.port.set_attr(multicast=True)
...
class RXServiceIPv6Scan(RXServiceIPv6):
    ...
    def on_pkt_rx(self, pkt, start_ts):
        ...
        if scapy_pkt.haslayer('ICMPv6ND_NS') and scapy_pkt.haslayer('ICMPv6NDOptSrcLLAddr'):
            ...
            if node_ip not in self.responses and dst_ip == self.src_ip:
                self.responses[node_ip] = {
                    'type': 'Router' if is_router else 'Host', 'mac': node_mac}
    ...
    def on_timeout(self):
        return self.port.ok([{'type': v['type'], 'mac':  v['mac'], 'ipv6': k} for k, v in list(self.responses.items())])
...
class RXServiceICMPv6(RXServiceIPv6):
    ...
    def on_pkt_rx(self, pkt, start_ts):
        ...
        if scapy_pkt.haslayer('ICMPv6EchoReply'):
            ...
            dt = pkt['ts'] - start_ts
            self.result['formatted_string'] = 'Reply from {0}: bytes={1}, time={2:.2f}ms, hlim={3}'.format(
                node_ip, len(pkt['binary']), dt * 1000, hlim)
        ...
        if scapy_pkt.haslayer('ICMPv6DestUnreach'):
            ...
            if dst_ip != self.src_ip:  # not our ping
                return
            self.result['formatted_string'] = 'Reply from {0}: Destination host unreachable'.format(
                node_ip)
