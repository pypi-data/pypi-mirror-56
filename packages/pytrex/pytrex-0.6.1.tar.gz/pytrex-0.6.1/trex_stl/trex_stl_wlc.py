...
class AP:
    ...
    def __init__(self, ssl_ctx, logger, trex_port, name, mac, ip, port, radio_mac,
                 verbose_level=VERB_WARN, rsa_priv_file=None, rsa_cert_file=None):
...
'''
    ...
    def wrap_capwap_pkt(self, capwap_pkt, dst_mac=None, dst_ip=None, dst_port=5246):
        if self.mac_dst and not dst_mac:
            if 'ether' not in self._scapy_cache:
                self._scapy_cache['ether'] = bytes(
                    Ether(src=self.mac_src, dst=self.mac_dst, type=0x0800))
    ...
    def wrap_pkt_by_wlan(self, client, pkt):
        assert type(
            pkt) is Ether, 'wrap_pkt_by_wlan() expects Ethernet packet, got: %s' % type(pkt)
    ...
    def get_arp_pkt(self, client, op, mac_dst, ip_dst):
        return(
            ...
            ARP(
                op=op,
                hwsrc=client.mac,
                hwdst='00:00:00:00:00:00' if(
                    op == 'who-has' and client.ip != ip_dst) else mac_dst,
                psrc=client.ip,
                pdst=ip_dst,
            )
        )
...
class AP_Manager:
    def __init__(self, trex_client=None, server=None):
        ...
        if not(bool(server) ^ bool(trex_client)):
            raise STLError(
                'Please specify either trex_client or server argument.')
        if not server:
        ...
        self.service_ctx = {}
        self.base_file_path = '/tmp/trex/console/%s_%s.wlc_base' % (
            get_current_user(), server)
    ...
    def init(self, trex_port_ids):
        ...
        for trex_port_id in trex_port_ids:
            ...
            if port_id in self.service_ctx:
                raise Exception(
                    'AP manager already initialized on port %s. Close it to proceed.' % port_id)
            success = False
            try:
                ...
                if not self.trex_client.get_port_attr(port=port_id)['prom'] == 'on':
                    self.trex_client.set_port_attr(
                        ports=port_id, promiscuous=True)
                self.service_ctx[port_id]['synced'] = True
                self.service_ctx[port_id]['bg'] = STLServiceApBgMaintenance(
                    self, port_id)
                self.service_ctx[port_id]['fg'] = STLServiceBufferedCtx(
                    self.trex_client, port_id)
    ...
    def remove_ap(self, ap_id):
        ...
        if ap.is_dtls_established():
            self.service_ctx[ap.port_id]['fg'].run(
                [STLServiceApShutdownDTLS(ap)])
        with self.bg_lock:
            for client in ap.clients:
                for key, val in list(dict(self.client_by_id).items()):
                    if val == client:
    ...
    def _compare_aps(self, good_aps, aps, err, show_success=True):
        if len(good_aps) != len(aps):
            ...
            raise Exception('Following AP(s) could not %s: %s' % (err, ', '.join(
                sorted([ap.name for ap in bad_aps], key=natural_sorted_key))))
        if show_success:
    ...
    def join_aps(self, ids=None):
        ...
        if len(aps) > MAX_JOINS:
            raise Exception(
                'Can not join more than %s at once, please split the joins' % MAX_JOINS)

        # discover
        self.trex_client.logger.pre_cmd('Discovering WLC')
        for port_id, aps_of_port in list(self._get_ap_per_port(aps).items()):
            self.service_ctx[port_id]['fg'].run(
                [STLServiceApDiscoverWLC(ap) for ap in aps_of_port])
        ...
        for port_id, aps_of_port in list(self._get_ap_per_port(aps).items()):
            self.service_ctx[port_id]['fg'].run(
                [STLServiceApEstablishDTLS(ap) for ap in aps_of_port])
        ...
        for port_id, aps_of_port in list(self._get_ap_per_port(aps).items()):
            self.service_ctx[port_id]['fg'].run(
                [STLServiceApJoinWLC(ap) for ap in aps_of_port])

        # check results
        good_aps = [ap for ap in aps if ap.SSID]
        self._compare_aps(good_aps, aps, 'join or get SSID',
                          show_success=False)
    ...
    def join_clients(self, ids=None):
        ...
        for client in clients:
            ...
            if client.ap not in clients_per_ap_per_port[client.ap.port_id]:
                clients_per_ap_per_port[client.ap.port_id][client.ap] = [
                    client]
            else:
                clients_per_ap_per_port[client.ap.port_id][client.ap].append(
                    client)
            if len(clients_per_ap_per_port) >= 100:
                for port_id, clients_per_ap in list(clients_per_ap_per_port.items()):
                    self.service_ctx[port_id]['fg'].run(
                        [STLServiceApAddClients(ap, c) for ap, c in list(clients_per_ap.items())])
                clients_per_ap_per_port = {}
        ...
        # check results
        no_assoc_clients = [
            client.ip for client in clients if not client.is_associated]
        if no_assoc_clients:
            self.trex_client.logger.post_cmd(False)
            raise Exception(
                'Following client(s) could not be associated: %s' % ', '.join(no_assoc_clients))
        no_resp_clients = [
            client.ip for client in clients if not client.seen_arp_reply]
        if no_resp_clients:
            self.trex_client.logger.post_cmd(False)
            raise Exception(
    ...
    def add_streams(self, client_id, streams):
        ...
        streams = [client.ap.patch_stream(client, stream)
    ...
    def close(self, ports=None):
        ...
        ap_per_port = self._get_ap_per_port(
            [ap for ap in self.aps if ap.port_id in ports])

        for port_id in ports:
            if port_id not in self.service_ctx:
                raise Exception(
                    'AP manager is not initialized on port %s!' % port_id)
    ...
    def _gen_ap_params(self):
        ...
        while self.next_ap_name in self.ap_by_id:
            ...
            if not m:
                raise Exception(
                    'Bad AP base name, should end with number, got: %s' % self.next_ap_name)
    ...
    def set_base_values(self, name=None, mac=None, ip=None, udp=None, radio=None,
        ...
        if name:
            if name[-1] not in string.digits:
                raise Exception(
                    'Base name of AP should end with number, for example: test-ap-1')
        if udp:
        ...
        if radio:
            if radio.split(':')[-1] != '00':
                raise Exception(
                    'Radio MACs should end with zero, got: %s' % radio)
