...
class STLServiceDHCP(STLService):
    ...
    # DHCP states
    INIT, SELECTING, REQUESTING, BOUND = list(range(4))
    ...
    def _acquire(self, pipe):
        ...
        while True:
            ...
            # SELECTING state
                ...
                offers = [
                    offer for offer in offers if offer.options['message-type'] == parser.OFFER]

                if not offers:
                    self.log('DHCP: {0} *** timeout on offers - retries left: {1}'.format(
                        self.mac, self.retries), level=STLService.ERROR)
                    self.state = 'INIT'
                    continue

                offer = offers[0]
                self.log("DHCP: {0} <--- OFFER from '{1}' with address '{2}' ".format(
                    self.mac, ipv4_num_to_str(offer.options['server_id']), ipv4_num_to_str(offer.yiaddr)))
            ...
            # REQUEST state
                ...
                acknacks = [
                    acknack for acknack in acknacks if acknack.options['message-type'] in(parser.ACK, parser.NACK)]

                if not acknacks:
                    self.log('DHCP: {0} *** timeout on ack - retries left: {1}'.format(
                        self.mac, self.retries), level=STLService.ERROR)
                    self.state = 'INIT'
                    continue
                ...
                if acknack.options['message-type'] == parser.ACK:
                    self.log("DHCP: {0} <--- ACK from '{1}' to address '{2}' ".format(
                        self.mac, ipv4_num_to_str(offer.options['server_id']), ipv4_num_to_str(offer.yiaddr)))
                    self.state = 'BOUND'
                else:
                    self.log("DHCP: {0} <--- NACK from '{1}'".format(self.mac,
                                                                     ipv4_num_to_str(offer.options['server_ip'])))
                    self.state = 'INIT'
    ...
    class DHCPRecord(object):
        ...
        def __init__(self, offer):
            ...
            self.server_ip = ipv4_num_to_str(
                options['server_id']) if 'server_id' in options else 'N/A'
            self.subnet = ipv4_num_to_str(
                options['subnet_mask']) if 'subnet_mask' in options else 'N/A'
