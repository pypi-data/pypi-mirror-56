...
class STLServiceFilterARP(STLServiceFilter):
    ...
    def add(self, service):
        ...
        self.services[(service.src_ip, service.dst_ip,
...
class STLServiceARP(STLService):
    ...
    def run(self, pipe):
        ...
        self.log(
            "ARP: ---> who has '{0}' ? tell '{1}' ".format(self.dst_ip, self.src_ip))

        pkt = Ether(dst="ff:ff:ff:ff:ff:ff")/ARP(psrc=self.src_ip,
                                                 pdst=self.dst_ip, hwsrc=self.src_mac)
        ...
        self.log(
            "ARP: <--- '{0} is at '{1}'".format(self.record.dst_ip, self.record.dst_mac))
