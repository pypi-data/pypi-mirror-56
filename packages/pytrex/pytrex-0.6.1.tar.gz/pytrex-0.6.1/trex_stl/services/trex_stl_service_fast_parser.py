...
class ParserInstance(object):
    ...
    def __getattr__(self, name):
        ...
        if not name in self.fields:
            raise ValueError(
                "field '{0}' is not registered under the parser".format(name))
    ...
    def __setattr__(self, name, value):
        ...
        info = self.fields[name]
        self.__dict__['pkt_bytes'] = info['setter'](
            self.pkt_bytes, info, value)
    ...
    def fix_chksum(self):
        if not 'ihl' in self.fields:
            raise ParserError(
                "'ihl' field must be registered under the parser for checksum fix")
        ...
        for i in range(0, ihl * 4, 2):
            word = struct.unpack_from(
                "!H", self.pkt_bytes, offset=ip_start + i)[0]
            sum += word
    ...
    def def_setter(pkt_bytes, info, value):
        ...
        if len(pkt_bytes) < min_size:
            raise ParserError("packet length is '{0}' but setting '{1}' requires at least '{2}' bytes".format(
                len(pkt_bytes), info['name'], min_size))
