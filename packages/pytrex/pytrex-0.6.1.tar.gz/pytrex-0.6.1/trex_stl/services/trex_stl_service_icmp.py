...
class STLServiceFilterICMP(STLServiceFilter):
    ...
    def add(self, service):
        ...
        self.services[(service.src_ip, service.id, service.seq,
                       tuple(service.vlan))].append(service)
...
class STLServiceICMP(STLService):
    ...
    def __init__(self, ctx, dst_ip, src_ip=None, pkt_size=64, timeout_sec=3, verbose_level=STLService.ERROR, vlan=None):
        ...
        if src_ip is None:
            ...
            if not src_ip:
                raise STLError('PING: port {} does not have an IPv4 address. please manually provide source IPv4'.format(
                    ctx.get_port_id()))
    ...
    def run(self, pipe):
        ...
        self.log(
            "ICMP: {:<15} ---> Pinging '{}'".format(self.src_ip, self.dst_ip))
        ...
        # parse record
        self.record = self.PINGRecord(
            Ether(response['pkt']), tx_info['ts'], response['ts'])
    ...
    class PINGRecord(object):
        TIMEOUT, UNREACHABLE, SUCCESS = list(range(3))
