...
from .trex_stl_stats import CPortStats
...
class Port(object):
    ...
    def writeable(func):
        def func_wrapper(*args, **kwargs):
            ...
            if not port.is_writeable():
                return port.err("{0} - port is active, please "
                                + "stop the port before executing command".format(func.__name__))
    ...
    def is_active(self):
        return((self.state == self.STATE_TX) or (self.state == self.STATE_PAUSE)
               or (self.state == self.STATE_PCAP_TX))
    ...
    def sync(self):
        ...
        if port_state == "DOWN":
            ...
            raise Exception("port {0}: bad state received from server '{1}'".format(
    ...
    def add_streams(self, streams_list):
        ...
        for stream in streams_list:
            ...
            # allocate stream id
            stream_id = stream.get_id() if stream.get_id(
            ) is not None else self.__allocate_stream_id()
        ...
        self.state = self.STATE_STREAMS if(
            len(self.streams) > 0) else self.STATE_IDLE
    ...
    def remove_streams(self, stream_id_list):
        ...
        if not all([stream_id in self.streams for stream_id in stream_id_list]):
            ...
            return self.err("stream does not exists")
        ...
        self.has_rx_streams = any([stream['has_flow_stats']
    ...
    def has_rx_delay_expired(self, rx_delay_ms):
        ...
        return (not self.tx_stopped_ts or(datetime.now() - self.tx_stopped_ts)
                > timedelta(milliseconds=rx_delay_ms))
    ...
    def set_l2_mode(self, dst_mac):
        if not self.is_service_mode_on():
            return self.err('port service mode must be enabled for configuring L2 mode. '
                            + 'Please enable service mode')
    ...
    def set_l3_mode(self, src_addr, dst_addr, resolved_mac=None):
        if not self.is_service_mode_on():
            return self.err('port service mode must be enabled for configuring L3 mode.'
                            + ' Please enable service mode')
    ...
    def set_vlan(self, vlan):
        if not self.is_service_mode_on():
            return self.err('port service mode must be enabled for configuring VLAN.'
                            + ' Please enable service mode')
    ...
    def set_attr(self, **kwargs):
        ...
        if kwargs.get('flow_ctrl_mode') is not None:
            json_attr['flow_ctrl_mode'] = {
                'mode': kwargs.get('flow_ctrl_mode')}

        if kwargs.get('rx_filter_mode') is not None:
            json_attr['rx_filter_mode'] = {
                'mode': kwargs.get('rx_filter_mode')}
    ...
    @writeable
    def push_remote(self, pcap_filename, ipg_usec, speedup, count, duration, is_dual,
                    slave_handler, min_ipg_usec):
    ...
    def get_formatted_info(self, sync=True):
        ...
        if layer_cfg['ipv4']['state'] == 'none':
            ...
            info['src_ipv4'] = '-'
            info['dest'] = layer_cfg['ether']['dst'] if (layer_cfg['ether']['state']
        ...
        # RX queue
        queue = rx_info['queue']
        info['rx_queue'] = '[{0} / {1}]'.format(
            queue['count'], queue['size']) if queue['is_active'] else 'off'

        # Grat ARP
        grat_arp = rx_info['grat_arp']
        if grat_arp['is_active']:
            info['grat_arp'] = "every {0} seconds".format(
                grat_arp['interval_sec'])
        else:
            info['grat_arp'] = "off"
