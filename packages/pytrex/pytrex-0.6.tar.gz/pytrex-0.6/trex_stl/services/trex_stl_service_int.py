...
# ######## internal functions              #########
    ...
    def _run(self, services):
        ...
        self.tx_buffer = TXBuffer(
            self.env, self.client, self.port, self.tx_batch_size, self.ipg_usec)
        ...
        is_promiscuous = self.client.get_port_attr(port=self.port)[
            'prom'] == "on"
        ...
        try:
            ...
            for f in list(self.filters.values()):
                ...
                f['capture_id'] = self.client.start_capture(
                    rx_ports=self.port, bpf_filter=f['inst'].get_bpf_filter(), limit=limit)['id']
...
class TXBuffer(object):
    ...
    def send_all(self):
        ...
        if self.queue:
            ...
            rc = self.client.push_packets(
                ports=self.port, pkts=buffer.pkts, ipg_usec=self.ipg_usec, force=True)
            tx_ts = rc.data()['ts']
