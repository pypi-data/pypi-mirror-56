...
class STLServiceBufferedCtx(STLServiceCtx):
    ...
    def _tick_process(self):
        while True:
            ...
            for ap, services in list(self.filter.services_per_ap.items()):
                for _ in range(len(ap.rx_buffer)):
                    ...
                    for service in services:
                        self.services[service]['pipe']._on_rx_pkt(
                            scapy_pkt, None)
...
##################
    ...
    def main_loop_wrapper(self):
        err_msg = ''
        try:
            self.capture_id = self.bg_client.start_capture(
                rx_ports=self.port_id, bpf_filter=self.bpf_filter, limit=5000)['id']
            self.main_loop()
            ...
            if self.port_id in self.ap_mngr.service_ctx:
                if self.ap_per_ip:
                    self.err('Background thread on port %s died%s. Disconnecting APs.' % (
                        self.port_id, err_msg))
                else:
                    self.err('Background thread on port %s died%s.' %
                             (self.port_id, err_msg))
                for ap in list(self.ap_per_ip.values()):
                    self.shutdown_ap(ap)
    ...
    def handle_ap_arp(self, rx_bytes, send_pkts):
        ...
        if dst_mac not in(b'\xff\xff\xff\xff\xff\xff', ap.mac_src):  # check MAC
            ap.err('Bad MAC(%s), although IP of AP(%s)' %
    ...
    def process_capwap_ctrl(self, rx_bytes, ap, send_pkts):
        ...
        if rx_pkt.control_header.type == 7:  # Configuration Update Request
            ...
            CAPWAP_PKTS.parse_message_elements(
                rx_pkt, ap, self.ap_mngr)  # get info from incoming packet
            seq = rx_pkt.control_header.sequence_number
            tx_pkt = ap.get_config_update_capwap(seq)
            if ap.is_debug:
                CAPWAP_CTRL(tx_pkt.value).show2()
            send_pkts.append(ap.wrap_capwap_pkt(
                b'\1\0\0\0' + ap.encrypt(tx_pkt)))
    ...
    def handle_client_arp(self, dot11_bytes, ap, send_pkts):
        ...
        if client.ap is not ap:
            self.err('Got ARP to client %s via wrong AP(%s)' %
                     (client.ip, ap.name))
            return
        ...
        if dot11_bytes[40:42] == self.ARP_REQ:  # 'who-has'
            if dot11_bytes[48:52] == dot11_bytes[58:62]:  # GARP
                return
            tx_pkt = ap.wrap_pkt_by_wlan(client, ap.get_arp_pkt(
                client, 'is-at', dot11_bytes[42:48], str2ip(dot11_bytes[48:52])))
            send_pkts.append(tx_pkt)
    ...
    def main_loop(self):
        ...
        while True:
            ...
            try:
                ...
                if not self.ap_mngr.service_ctx[self.port_id]['synced']:
                    ...
                    self.ap_per_ip = dict(
                        [(ap.ip_src, ap) for ap in aps if ap.port_id == self.port_id])
                    self.client_per_ip = dict(
                        [(client.ip_bytes, client) for client in clients])
                    ...
                    if ap.is_connected:
                        if ap.echo_resp_timer and ap.echo_resp_timer.has_expired():  # retry echo
                            if ap.echo_resp_retry > 0:
                                ...
                                ap.echo_resp_timer = PassiveTimer(
                                    ap.echo_resp_timeout)
                                ap.echo_resp_retry -= 1
                                tx_pkt = ap.get_echo_capwap()
                                send_pkts.append(
                                    ap.get_echo_wrap(ap.encrypt(tx_pkt)))
                            else:
                                self.err(
                                    "Timeout in echo response for AP '%s', disconnecting" % ap.name)
                                self.shutdown_ap(ap)
                ...
                if ether_type == self.ARP_ETHTYPE:
                    ...
                    if ip_proto == self.ICMP_PROTO:
                        ...
                        if udp_port_str != ap.udp_port_str:  # check UDP port
                            ap.err('Bad UDP port(%s), although IP of AP(%s)' %
                                   (str2int(udp_port), str2ip(ip)))
                            continue
                        ...
                        if udp_src == self.CAPWAP_CTRL_PORT:
                            ...
                            # Client assoc. response
                            if dot11_bytes[:2] == self.WLAN_ASSOC_RESP:
                                ap.rx_buffer.append(Dot11_swapped(dot11_bytes))

                            elif dot11_bytes[32:34] == self.ARP_ETHTYPE:
                                self.handle_client_arp(
                                    dot11_bytes, ap, send_pkts)

                            elif dot11_bytes[32:34] == self.IP_ETHTYPE and dot11_bytes[43:44] == self.ICMP_PROTO:
                                self.handle_client_icmp(
                                    dot11_bytes, ap, send_pkts)
...
class STLServiceAp(STLService):
    ...
    def run(self, pipe):
        ...
        while True:
            if self.requires_dtls and not self.ap.is_dtls_established():
                self.log('DTLS session got closed for AP %s, exiting FSM' %
                         self.ap.name)
                break
            try:
    ...
    def run_with_buffer(self):
        ...
        for _ in range(max(2, self.ap.capwap_MaxRetransmit)):
            ...
            discovery_pkt = self.ap.wrap_capwap_pkt(CAPWAP_PKTS.discovery(
                self.ap), dst_mac='ff:ff:ff:ff:ff:ff', dst_ip='255.255.255.255')
...
class STLServiceApEstablishDTLS(STLServiceAp):
    ...
    def openssl_callback(ssl, where, ret):
        ...
        if libcrypto.BIO_ctrl_pending(ap.out_bio):
            ...
            if ssl_data:
                pkt = STLServiceAp.wrap_capwap_static(
                    ap, ap.mac_dst, ap.ip_dst, 5246, b'\1\0\0\0' + ssl_data)
        ...
        try:
            ...
            while not timer.has_expired():
                ...
                if self.ap.is_dtls_closed():
                    self.err('DTLS session got closed for ap %s' %
                             self.ap.name)
                    return
...
class STLServiceApJoinWLC(STLServiceApEncryptedControl):
    def run_with_buffer(self):
        ...
        ctrl_gen = self.control_round_trip(
            CAPWAP_PKTS.conf_status_req(self.ap), 6)
        send_data = None
        while True:
            action = ctrl_gen.send(send_data)
            if type(action) is tuple and action[0] == 'resp':
        ...
        ctrl_gen = self.control_round_trip(
            CAPWAP_PKTS.change_state(self.ap), 12)
        send_data = None
        while True:
            action = ctrl_gen.send(send_data)
            if type(action) is tuple and action[0] == 'resp':
...
class STLServiceApAddClients(STLServiceAp):
    def __init__(self, ap, clients, verbose_level=STLService.WARN):
        ...
        assert all([hasattr(c, 'mac') and hasattr(c, 'ip')
                    for c in clients]), 'Clients should have attributes mac and ip'
        self.clients = clients

    def run_with_buffer(self):
        self.log('Sending Association requests.')
        need_assoc_resp_clients = dict(
            [(client.mac, client) for client in self.clients])
        ...
        for _ in range(self.ap.capwap_MaxRetransmit):
            ...
            RetransmitInterval *= 2
            tx_pkts = []
            for client in list(need_assoc_resp_clients.values()):
                tx_pkt = CAPWAP_PKTS.client_assoc(
                    self.ap, slot_id=0, client_mac=client.mac)
            ...
            while not timer.has_expired():
                ...
                if rx_pkt and isinstance(rx_pkt, Dot11_swapped):
                    ...
                    if rx_pkt.type == 0 and rx_pkt.subtype == 1 and rx_pkt.addr1 in need_assoc_resp_clients:
                        self.log('Received Association response for %s.' %
                                 need_assoc_resp_clients[rx_pkt.addr1].ip)
        ...
        if need_assoc_resp_clients:
            yield('err', 'No Association response for clients: %s' % ', '.join([client.ip for client in list(need_assoc_resp_clients.values())]))

        need_arp_resp_clients = dict(
            [(client.ip, client) for client in self.clients])
        ...
        for _ in range(self.ap.capwap_MaxRetransmit):
            ...
            RetransmitInterval *= 2
            tx_pkts = []
            for client in list(need_arp_resp_clients.values()):
                tx_pkts.append(self.ap.wrap_pkt_by_wlan(
                    client, self.ap.get_garp_pkt(client)))
                tx_pkts.append(self.ap.wrap_pkt_by_wlan(client, self.ap.get_arp_pkt(
                    client, 'who-has', 'ff:ff:ff:ff:ff:ff', self.ap.ip_dst)))
            yield('put', tx_pkts)
            ...
            while not timer.has_expired():
                ...
                if rx_pkt and isinstance(rx_pkt, Dot11_swapped) and ARP in rx_pkt:
                    ...
                    if rx_arp.hwsrc != self.ap.mac_dst:
                        self.log('MAC address returned from ARP(%s) differs from WLC MAC(%s)' % (
                            rx_arp.hwsrc, self.ap.mac_dst))
