...
_PSV_ALL, PSV_UP, PSV_ACQUIRED, PSV_IDLE, PSV_RESOLVED, PSV_SERVICE, PSV_NON_SERVICE, PSV_L3 = list(
    range(0, 8))
...
class PortState(object):
    ...
    def validate(self, client, cmd_name, ports, custom_err_msg=None):
        ...
        if invalid_ports:
            ...
            raise STLError(
                '{0} - port(s) {1}: {2}'.format(cmd_name, invalid_ports, err_msg))
...
class PortStateIdle(PortState):
    ...
    def get_valid_ports(self, client):
        return [port_id for port_id in client.get_all_ports()
                if not client.ports[port_id].is_active()]
...
class PortStateNonService(PortState):
    ...
    def get_valid_ports(self, client):
        return [port_id for port_id in client.get_all_ports()
                if not client.ports[port_id].is_service_mode_on()]
...
class PortStateValidator(object):
    ...
    def validate(self, cmd_name, ports, states=None, allow_empty=False):
        ...
        for state, err_msg in list(states_map.items()):
            self.validators[state].validate(
                self.client, cmd_name, ports, err_msg)
