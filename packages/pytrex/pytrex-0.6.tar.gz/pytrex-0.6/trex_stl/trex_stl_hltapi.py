...
traffic_config_kwargs = {
    ...
    # ( split | duplicate | single ) TRex extention: split = split traffic
    # by cores, duplicate = duplicate traffic for all cores,
    # single = run only with sinle core(not implemented yet)
    'split_by_cores': 'split',
    # TRex extention: path to filename with stream profile
    # (stream builder parameters will be ignored, limitation: modify)
    'load_profile': None,
    # TRex extention: False(default) = random sequence will be
    # different every run, True = random sequence will be same every run
    'consistent_random': False,
    # TRex extention: True = use MACs from server configuration,
    # no MAC VM(workaround on lack of ARP)
    'ignore_macs': False,
    # TRex extention: True = don't use flow stats for this stream,
    # (workaround for limitation on type of packet for flow_stats)
    'disable_flow_stats': False,
    # TRex extention: uint, for use of STLHltStream,
    # specifies id for flow stats(see stateless manual for flow_stats details)
    'flow_stats_id': None,
    'port_handle': None,
    'port_handle2': None,
    'bidirectional': False,
...
def merge_kwargs(default_kwargs, user_kwargs):
    ...
    for key, value in list(user_kwargs.items()):
        if key in kwargs:
            kwargs[key] = value
        # Internal arguments
        elif key in('save_to_yaml', 'save_to_pcap', 'pg_id'):
            kwargs[key] = value
        else:
            print(("Warning: provided parameter '%s' is not supported" % key))
    return kwargs

# change MACs from formats 01-23-45-67-89-10 or 0123.4567.8910
# or {01 23 45 67 89 10} to Scapy format 01:23:45:67:89:10


def correct_macs(kwargs):
    list_of_mac_args = ['mac_src', 'mac_dst', 'mac_src2', 'mac_dst2']
    list_of_mac_steps = ['mac_src_step', 'mac_dst_step',
                         'mac_src2_step', 'mac_dst2_step']
    for mac_arg in list_of_mac_args + list_of_mac_steps:
        if mac_arg in kwargs:
            mac_value = kwargs[mac_arg]
            # Step can be a number
            if is_integer(mac_value) and mac_arg in list_of_mac_steps:
                continue
            if type(mac_value) is not str:
                raise STLError('Argument %s should be str' % mac_arg)
            mac_value = (mac_value.replace('{', '').replace('}', '')
                         .strip().replace('-', ' ').replace(':', ' ')
                         .replace('.', ' '))
            if mac_value[4] == ' ' and mac_value[9] == ' ':
                mac_value = ' '.join(
                    [mac_value[0:2], mac_value[2:7], mac_value[7:12],
                     mac_value[12:14]])
            ...
            try:
                ...
                raise STLError(
                    'Incorrect MAC %s=%s, please use 01:23:45:67:89:10 '
                    + 'or 01-23-45-67-89-10 or 0123.4567.8910 or '
                    + '{01 23 45 67 89 10}' % (mac_arg, kwargs[mac_arg]))


def is_true(input):
    if input in(True, 'True', 'true', 1, '1', 'enable', 'Enable', 'Yes',
                'yes', 'y', 'Y', 'enabled', 'Enabled'):
        return True
    return False
...
def print_brief_stats(res):
    ...
    tx_str = 'TX:'
    rx_str = 'RX:'
    for port_id, stat in list(res.items()):
...
# hlt_history = True:  act as dictionary(per port) stream_id
# -> hlt arguments used for build
...
class CStreamsPerPort(defaultdict):
    ...
    def get_stream_list(self, ports_list=None):
        if self.hlt_history:
            if ports_list is None:
                ports_list = list(self.keys())
            elif not isinstance(ports_list, list):
                ports_list = [ports_list]
            ret = {}
            for port in ports_list:
                ret[port] = list(self[port].keys())
            return ret
        else:
            return self
    ...
    def add_streams_from_res(self, res):
        if self.hlt_history:
            raise STLError(
                'CStreamsPerPort: this object is not meant for HLT history,'
                + ' try init with hlt_history = False')
        if not isinstance(res, dict):
            raise STLError('CStreamsPerPort: res should be dict')
        if res.get('status') != 1:
            raise STLError('CStreamsPerPort: res has status %s' %
                           res.get('status'))
        ...
        for port, port_stream_ids in list(res_streams.items()):
    ...
    def save_stream_args(self, ports_list, stream_id, stream_hlt_args):
        if stream_id is None:
            raise STLError('CStreamsPerPort: no stream_id in stream')
        if stream_hlt_args.get('load_profile'):
            return  # can't modify profiles, don't save
        if not self.hlt_history:
            raise STLError(
                'CStreamsPerPort: this object works only with HLT history,'
                + ' try init with hlt_history = True')
        if not is_integer(stream_id):
            raise STLError('CStreamsPerPort: stream_id should be number')
        if not isinstance(stream_hlt_args, dict):
    ...
    def remove_stream(self, ports_list, stream_id):
        ...
        for port in ports_list:
            ...
            if stream_id not in self[port]:
                raise STLError(
                    'CStreamsPerPort: stream_id %s not found at port %s'
                    % (port, stream_id))
            if self.hlt_history:
...
class CTRexHltApi(object):
    ...
    def __init__(self, verbose=0):
        ...
        # streams in format of HLT arguments for modify later
        self._streams_history = {}
...
###########################
    ...
    def connect(self, **user_kwargs):
        ...
        except Exception:  # give it another try
            try:
                device = socket.gethostbyname(device)
            except Exception as e:
                return HLT_ERR('Could not translate hostname "%s" to IP: %s'
                               % (device, e))

        try:
            self.trex_client = STLClient(
                kwargs['username'], device, verbose_level=self.verbose)
        except Exception as e:
            return HLT_ERR('Could not init stateless client %s: %s'
                           % (device, e if isinstance(e, STLError)
        ...
        try:
            ...
            self.trex_client = None
            return HLT_ERR('Could not connect to device %s: %s'
                           % (device, e if isinstance(e, STLError)
                              else traceback.format_exc()))

        # connection successfully created with server,
        # try acquiring ports of TRex
        try:
            port_list = self._parse_port_list(kwargs['port_list'])
            self.trex_client.acquire(
                ports=port_list, force=kwargs['break_locks'])
            for port in port_list:
                self._native_handle_by_pg_id[port] = {}
        except Exception as e:
            self.trex_client = None
            return HLT_ERR('Could not acquire ports %s: %s'
                           % (port_list, e if isinstance(e, STLError)
                              else traceback.format_exc()))
        ...
        if kwargs['reset']:
            ...
            try:
                ...
                self.trex_client = None
                return HLT_ERR('Error in reset traffic: %s'
                               % e if isinstance(e, STLError)
                               else traceback.format_exc())

        self._streams_history = CStreamsPerPort(hlt_history=True)
        return HLT_OK(port_handle=dict([(port_id, port_id)
    ...
    def cleanup_session(self, **user_kwargs):
        ...
        if not kwargs['maintain_lock']:
            ...
            try:
                ...
                return HLT_ERR('Unable to determine which ports to release: %s'
                               % e if isinstance(e, STLError)
                               else traceback.format_exc())
            try:
                self.trex_client.stop(port_list)
            except Exception as e:
                return HLT_ERR('Unable to stop traffic %s: %s'
                               % (port_list, e if isinstance(e, STLError)
                                  else traceback.format_exc()))
            try:
                self.trex_client.remove_all_streams(port_list)
            except Exception as e:
                return HLT_ERR('Unable to remove all streams %s: %s'
                               % (port_list, e if isinstance(e, STLError)
                                  else traceback.format_exc()))
            try:
                self.trex_client.release(port_list)
            except Exception as e:
                return HLT_ERR('Unable to release ports %s: %s'
                               % (port_list, e if isinstance(e, STLError)
                                  else traceback.format_exc()))
        try:
    ...
    def interface_config(self, port_handle, mode='config'):
        ...
        if mode not in ALLOWED_MODES:
            return HLT_ERR('Mode must be one of the following values: %s'
                           % ALLOWED_MODES)
...
###########################
    ...
    def traffic_config(self, **user_kwargs):
        ...
        try:
            ...
            return HLT_ERR(e if isinstance(e, STLError)
                           else traceback.format_exc())
        kwargs = merge_kwargs(traffic_config_kwargs, user_kwargs)
        stream_id = kwargs['stream_id']
        mode = kwargs['mode']
        pg_id = kwargs['flow_stats_id']
        port_handle = self._parse_port_list(kwargs['port_handle'])

        ALLOWED_MODES = ['create', 'modify',
        ...
        if mode == 'reset':
            try:
                ...
                return HLT_ERR('Could not reset streams at ports %s: %s'
                               % (port_handle, e if isinstance(e, STLError)
                                  else traceback.format_exc()))
        ...
        if mode == 'remove':
            ...
            if stream_id == 'all':
                try:
                    ...
                    return HLT_ERR('Could not remove all streams at ports %s: %s'
                                   % (port_handle, e
                                      if isinstance(e, STLError) else traceback.format_exc()))
            else:
                try:
                    self._remove_stream(stream_id, port_handle)
                except Exception as e:
                    return HLT_ERR('Could not remove streams with specified by %s, error: %s'
                                   % (stream_id, e if isinstance(e, STLError)
                                      else traceback.format_exc()))
            return HLT_OK()
        ...
        #        return HLT_ERR('This stream_id(%s) was not used before, please create new.'
        #                       % stream_id)
            ...
            if len(port_handle) > 1:
                for port in port_handle:
                    try:
                        ...
                        return HLT_ERR('Could not remove stream(s) %s from port(s) %s: %s'
                                       % (stream_id, port_handle, e if isinstance(e, STLError)
                                          else traceback.format_exc()))
                return HLT_OK()
            else:
                ...
                if pg_id not in self._streams_history[port]:
                    return HLT_ERR('This stream_id(%s) was not used before at port %s, '
                                   + 'please create new.' % (stream_id, port))
                new_kwargs = {}
        ...
        if mode == 'create' or mode == 'modify':
            ...
            # two streams with opposite directions
            if is_true(kwargs['bidirectional']):
                ...
                try:
                    if save_to_yaml and type(save_to_yaml) is str:
                        user_kwargs['save_to_yaml'] = save_to_yaml.replace(
                            '.yaml', '_bi1.yaml')
                    ...
                    if res2['status'] == 0:
                        raise STLError(
                            'Could not create bidirectional stream 2: %s' % res2['log'])
            ...
            try:
                ...
                if kwargs['load_profile']:
                    stream_obj = STLProfile.load_py(
                        kwargs['load_profile'], direction=kwargs['direction'])
                else:
                    user_kwargs['pg_id'] = pg_id
                    stream_obj = STLHltStream(**user_kwargs)
            except Exception as e:
                return HLT_ERR('Could not create stream: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            ...
            try:
                for port in port_handle:
                    stream_id_arr = self.trex_client.add_streams(streams=stream_obj,
                                                                 ports=port)
                    self._streams_history.save_stream_args(
                        port, pg_id, user_kwargs)
                    if type(stream_id_arr) is not list:
                        stream_id_arr = [stream_id_arr]
                    self._native_handle_by_pg_id[port][pg_id] = stream_id_arr
            except Exception as e:
                return HLT_ERR('Could not add stream to ports: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            if mode == 'create':
        ...
        return HLT_ERR('Got to the end of traffic_config, mode not implemented or '
                       + 'forgot "return" somewhere.')
    ...
    def traffic_control(self, **user_kwargs):
        ...
        ALLOWED_ACTIONS = ['clear_stats', 'run',
                           'stop', 'sync_run', 'poll', 'reset']
        if action not in ALLOWED_ACTIONS:
            return HLT_ERR('Action must be one of the following values: {actions}'
                           .format(actions=ALLOWED_ACTIONS))

        if action == 'run':
            try:
                self.trex_client.start(ports=port_handle)
            except Exception as e:
                return HLT_ERR('Could not start traffic: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            ...
            try:
                ...
                return HLT_ERR('Unable to do sync_run: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            ...
            try:
                ...
                return HLT_ERR('Could not stop traffic: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            ...
            try:
                ...
                return HLT_ERR('Could not reset traffic: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
            ...
            try:
                ...
                return HLT_ERR('Could not clear stats: %s' % e if isinstance(e, STLError)
                               else traceback.format_exc())
        ...
        try:
            is_traffic_active = self.trex_client.is_traffic_active(
                ports=port_handle)
        except Exception as e:
            return HLT_ERR('Unable to determine ports status: %s' % e if isinstance(e, STLError)
                           else traceback.format_exc())
    ...
    def traffic_stats(self, **user_kwargs):
        ...
        try:
            ...
            if mode in('all', 'streams'):
                for pg_id, pg_stats in list(stats['flow_stats'].items()):
                    try:
                        pg_id = int(pg_id)
                    except Exception:
                        continue
                    for port_id in port_handle:
                        ...
                        hlt_stats_dict[port_id]['stream'][pg_id] = {
                            'tx': {
                                ...
                                'line_rate_percentage': (pg_stats['tx_bps_l1'].get(port_id, 0)
                                                         * 100.0 / ports_speed[port_id])
                                if ports_speed[port_id] else 0,
                            },
                            'rx': {
                                ...
                                'line_rate_percentage': (pg_stats['rx_bps_l1'].get(port_id, 0)
                                                         * 100.0 / ports_speed[port_id])
                                if ports_speed[port_id] else 0,
                            },
                        }
        except Exception as e:
            return HLT_ERR('Could not retrieve stats: %s' % e if isinstance(e, STLError)
                           else traceback.format_exc())
    ...
    def wait_on_traffic(self, port_handle=None, timeout=60):
        try:
            ...
            return HLT_ERR('Unable to run wait_on_traffic: %s' % e if isinstance(e, STLError)
                           else traceback.format_exc())
...
###########################
    ...
    def _get_available_pg_id(self):
        ...
        used_pg_ids = list(self.trex_client.get_stats()['flow_stats'].keys())
    ...
    def _remove_stream(self, stream_id, port_handle):
        ...
        if stream_num is not None:  # exact value of int or str
            for port in port_handle:
                ...
                self.trex_client.remove_streams(
                    native_handles, port)                   # actual remove
        ...
        if type(stream_id) is str:  # range or list in string
            if ',' in stream_id:
                for each_stream_id_element in stream_id.split(','):
                    self._remove_stream(
                        each_stream_id_element, port_handle)            # recurse
                return
            if '-' in stream_id:
                ...
                if stream_id_min is None:
                    raise STLError(
                        '_remove_stream: wrong range param %s' % stream_id_min)
                if stream_id_max is None:
                    raise STLError(
                        '_remove_stream: wrong range param %s' % stream_id_max)
                if stream_id_max < stream_id_min:
                    raise STLError(
                        '_remove_stream: right range param is smaller than left one: %s-%s'
                        % (stream_id_min, stream_id_max))
                for each_stream_id in range(stream_id_min, stream_id_max + 1):
                    # recurse
                    self._remove_stream(each_stream_id, port_handle)
                return
    ...
    def _parse_port_list(port_list):
        ...
        raise STLError(
            'port_list should be string with ports, list, or single number')
...
def STLHltStream(**user_kwargs):
    ...
    if len(intersect_rate_args) > 1:
        raise STLError('More than one rate argument specified: %s' %
                       intersect_rate_args)
    try:
    ...
    if kwargs['length_mode'] == 'imix':  # several streams with given length
        ...
        if (kwargs['l3_imix1_size'] < 32 or kwargs['l3_imix2_size']
                                    < 32 or kwargs['l3_imix3_size']
                                    < 32 or kwargs['l3_imix4_size']
                                    < 32):
        ...
        if rate_key == 'rate_bps':  # ratio dependent on L2 size too
            ...
            imix1_weight = kwargs['l3_imix1_ratio'] * \
                (kwargs['l3_imix1_size'] + 20)
            imix2_weight = kwargs['l3_imix2_ratio'] * \
                (kwargs['l3_imix2_size'] + 20)
            imix3_weight = kwargs['l3_imix3_ratio'] * \
                (kwargs['l3_imix3_size'] + 20)
            imix4_weight = kwargs['l3_imix4_ratio'] * \
                (kwargs['l3_imix4_size'] + 20)
        total_weight = float(imix1_weight + imix2_weight +
                             imix3_weight + imix4_weight)
        if total_weight == 0:
    ...
    # stream generation
    try:
        rate_types_dict = {'rate_pps': 'pps',
                           'rate_bps': 'bps_L2', 'rate_percent': 'percentage'}
        ...
        if transmit_mode == 'continuous':
            ...
            transmit_mode_obj = STLTXSingleBurst(
                total_pkts=pkts_per_burst, **rate_stateless)
        elif transmit_mode == 'multi_burst':
            transmit_mode_obj = STLTXMultiBurst(pkts_per_burst=pkts_per_burst,
                                                count=int(kwargs['burst_loop_count']),
                                                ibg=kwargs['inter_burst_gap'], **rate_stateless)
        else:
            raise STLError('transmit_mode %s not supported/implemented')
    except Exception as e:
        raise STLError('Could not create transmit_mode object %s: %s' % (
            transmit_mode, e if isinstance(e, STLError) else traceback.format_exc()))
    ...
    try:
        ...
        stream = STLStream(packet=packet,
                           random_seed=1 if is_true(
                               kwargs['consistent_random']) else 0,
                           # enabled = True,
                           # self_start = True,
        ...
        raise STLError('Could not create stream: %s' %
                       e if isinstance(e, STLError) else traceback.format_exc())
...
def generate_packet(**user_kwargs):
    ...
    vm_cmds = []
    # we will keep in cache same variables(inc/dec, var size in bytes, number of steps, step)
    vm_variables_cache = {}
    ...
    if kwargs['l2_encap'] in('ethernet_ii', 'ethernet_ii_vlan'):
        ...
        if kwargs['mac_src_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['mac_src_mode'] == 'random':
                mac_src = ipv4_str_to_num(mac2str(kwargs['mac_src'])[
                                          2:])  # take only 32 lsb

                step = kwargs['mac_src_step']

                if type(step) is str:
                    step = ipv4_str_to_num(
                        mac2str(step)[2:])  # take only 32 lsb
                ...
                if kwargs['mac_src_mode'] == 'increment':
                    ...
                    vm_cmds.append(STLVmFlowVar(
                        name=var_name, size=4, op='random', max_value=0xffffffff))
                else:
                    raise STLError('mac_src_mode %s is not supported' %
                                   kwargs['mac_src_mode'])
                vm_cmds.append(STLVmWrFlowVar(
                    fv_name=var_name, pkt_offset='Ethernet.src', offset_fixup=2, add_val=add_val))
        ...
        if kwargs['mac_dst_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['mac_dst_mode'] == 'random':
                mac_dst = ipv4_str_to_num(mac2str(kwargs['mac_dst'])[
                                          2:])  # take only 32 lsb
                ...
                if kwargs['mac_dst_mode'] == 'increment':
                    ...
                    raise STLError('mac_dst_mode %s is not supported' %
                                   kwargs['mac_dst_mode'])
                vm_cmds.append(STLVmWrFlowVar(
                    fv_name=var_name, pkt_offset='Ethernet.dst', offset_fixup=2, add_val=add_val))

        if kwargs['l2_encap'] == ('ethernet_ii_vlan' or(kwargs['l2_encap'] == 'ethernet_ii'
                                                        and vlan_in_args(user_kwargs))):
            ...
            for i, vlan_kwargs in enumerate(split_vlan_args(kwargs)):
                ...
                if vlan_protocol_tag_id is not None:
                    try:  # to convert str to int
                        ...
                        pass
                    ALLOWED_VLAN_PROTO = [
                        0x8100, 0x88A8, 0x9100, 0x9200, 0x9300]
                    if vlan_protocol_tag_id not in ALLOWED_VLAN_PROTO:
                        raise STLError('vlan_protocol_tag_id argument(s) must be one of '
                                       + 'the following values: %s, got: 0x%04x' %
                                       (', '.join(map('0x{:04x}'.format, ALLOWED_VLAN_PROTO)),
                ...
                if vlan_id_mode != 'fixed':
                    ...
                    if count > 0 or vlan_id_mode == 'random':
                        ...
                        if vlan_id_mode == 'increment':
                            ...
                            if var_name not in vm_variables_cache:
                                vm_cmds.append(STLVmFlowVar(name=var_name, size=2, op='inc',
                                                            step=step,
                                                            min_value=0x7fff,
                            ...
                            var_name = '%s_%s_%s_%s' % ('dec', 2, count, step)
                            if var_name not in vm_variables_cache:
                                vm_cmds.append(STLVmFlowVar(name=var_name, size=2, op='dec',
                                                            step=step,
                            ...
                            vm_cmds.append(STLVmFlowVar(
                                name=var_name, size=2, op='random', max_value=0xffff))
                        else:
                            raise STLError(
                                'vlan_id_mode %s is not supported' % vlan_id_mode)
                        vm_cmds.append(STLVmWrMaskFlowVar(fv_name=var_name,
                                                          pkt_offset='802|1Q:%s.vlan' % i,
                                                          pkt_cast_size=2, mask=0xfff,
                                                          add_value=add_val))
    else:
        raise NotImplementedError(
            "l2_encap does not support the desired encapsulation '%s'" % kwargs['l2_encap'])
    base_pkt = l2_layer
    ...
    if kwargs['l3_protocol'] is None:
        ...
        if kwargs['ip_src_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['ip_src_mode'] == 'random':
                ...
                if type(ip_src_addr) is str:
                    ip_src_addr = ipv4_str_to_num(
                        is_valid_ipv4_ret(ip_src_addr))
                ...
                if kwargs['ip_src_mode'] == 'increment':
                    ...
                    raise STLError('ip_src_mode %s is not supported' %
                                   kwargs['ip_src_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='IP.src', add_val=add_val))
        ...
        if kwargs['ip_dst_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['ip_dst_mode'] == 'random':
                ...
                if type(ip_dst_addr) is str:
                    ip_dst_addr = ipv4_str_to_num(
                        is_valid_ipv4_ret(ip_dst_addr))
                ...
                if kwargs['ip_dst_mode'] == 'increment':
                    ...
                    raise STLError('ip_dst_mode %s is not supported' %
                                   kwargs['ip_dst_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='IP.dst', add_val=add_val))
        ...
        if kwargs['ipv6_src_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['ipv6_src_mode'] == 'random':
                ipv6_src_addr_num = ipv4_str_to_num(
                    is_valid_ipv6_ret(kwargs['ipv6_src_addr'])[-4:])
                ...
                if kwargs['ipv6_src_mode'] == 'increment':
                    ...
                    raise STLError('ipv6_src_mode %s is not supported' %
                                   kwargs['ipv6_src_mode'])
                vm_cmds.append(STLVmWrFlowVar(
                    fv_name=var_name, pkt_offset='IPv6.src', offset_fixup=12, add_val=add_val))
        ...
        if kwargs['ipv6_dst_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['ipv6_dst_mode'] == 'random':
                ipv6_dst_addr_num = ipv4_str_to_num(
                    is_valid_ipv6_ret(kwargs['ipv6_dst_addr'])[-4:])
                ...
                if kwargs['ipv6_dst_mode'] == 'increment':
                    ...
                    raise STLError('ipv6_dst_mode %s is not supported' %
                                   kwargs['ipv6_dst_mode'])
                vm_cmds.append(STLVmWrFlowVar(
                    fv_name=var_name, pkt_offset='IPv6.dst', offset_fixup=12, add_val=add_val))

    elif kwargs['l3_protocol'] is not None:
        raise NotImplementedError(
            "l3_protocol '%s' is not supported by TRex yet." % kwargs['l3_protocol'])
    ...
    if kwargs['l4_protocol'] == 'tcp':
        assert kwargs['l3_protocol'] in(
            'ipv4', 'ipv6'), 'TCP must be over ipv4/ipv6'
        ...
        if kwargs['tcp_src_port_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['tcp_src_port_mode'] == 'random':
                ...
                if kwargs['tcp_src_port_mode'] == 'increment':
                    ...
                    raise STLError(
                        'tcp_src_port_mode %s is not supported' % kwargs['tcp_src_port_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='TCP.sport', add_val=add_val))
        ...
        if kwargs['tcp_dst_port_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['tcp_dst_port_mode'] == 'random':
                ...
                if kwargs['tcp_dst_port_mode'] == 'increment':
                    ...
                    raise STLError(
                        'tcp_dst_port_mode %s is not supported' % kwargs['tcp_dst_port_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='TCP.dport', add_val=add_val))

    elif kwargs['l4_protocol'] == 'udp':
        assert kwargs['l3_protocol'] in(
            'ipv4', 'ipv6'), 'UDP must be over ipv4/ipv6'
        ...
        if kwargs['udp_src_port_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['udp_src_port_mode'] == 'random':
                ...
                if kwargs['udp_src_port_mode'] == 'increment':
                    ...
                    raise STLError(
                        'udp_src_port_mode %s is not supported' % kwargs['udp_src_port_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='UDP.sport', add_val=add_val))
        ...
        if kwargs['udp_dst_port_mode'] != 'fixed':
            ...
            if count > 0 or kwargs['udp_dst_port_mode'] == 'random':
                ...
                if kwargs['udp_dst_port_mode'] == 'increment':
                    ...
                    raise STLError(
                        'udp_dst_port_mode %s is not supported' % kwargs['udp_dst_port_mode'])
                vm_cmds.append(STLVmWrFlowVar(fv_name=var_name,
                                              pkt_offset='UDP.dport', add_val=add_val))
    elif kwargs['l4_protocol'] is not None:
        raise NotImplementedError(
            "l4_protocol '%s' is not supported by TRex yet." % kwargs['l4_protocol'])
    ...
    if length_mode == 'auto':
        ...
        raise STLError(
            "length_mode 'imix' should be treated at stream creating level.")
    elif length_mode in trim_dict:
        # size is determined by L2, higher priority over L3 size
        if 'frame_size_min' in user_kwargs or 'frame_size_max' in user_kwargs:
            if kwargs['frame_size_min'] < 44 or kwargs['frame_size_max'] < 44:
                raise STLError(
                    'frame_size_min and frame_size_max should be at least 44')
            if kwargs['frame_size_min'] > kwargs['frame_size_max']:
                raise STLError('frame_size_min is bigger than frame_size_max')
            if kwargs['frame_size_min'] != kwargs['frame_size_max']:
                fix_ipv4_checksum = True
                vm_cmds.append(STLVmFlowVar(name='pkt_len', size=2, op=trim_dict[length_mode],
                                            step=kwargs['frame_size_step'],
            ...
            if kwargs['l3_length_min'] < 40 or kwargs['l3_length_max'] < 40:
                raise STLError(
                    'l3_length_min and l3_length_max should be at least 40')
            if kwargs['l3_length_min'] > kwargs['l3_length_max']:
                raise STLError('l3_length_min is bigger than l3_length_max')
            if kwargs['l3_length_min'] != kwargs['l3_length_max']:
                fix_ipv4_checksum = True
                vm_cmds.append(STLVmFlowVar(name='pkt_len', size=2, op=trim_dict[length_mode],
                                            step=kwargs['l3_length_step'],
                                            min_value=kwargs['l3_length_min'] +
                                            len(l2_layer),
                                            max_value=kwargs['l3_length_max'] + len(l2_layer)))
            payload_len = kwargs['l3_length_max'] + \
                len(l2_layer) - len(base_pkt)
            vm_cmds.append(STLVmTrimPktSize('pkt_len'))

        if(l3_layer and l3_layer.name == 'IP'):
            vm_cmds.append(STLVmWrFlowVar(fv_name='pkt_len',
                                          pkt_offset='IP.len', add_val=-len(l2_layer)))
        if(l4_layer and l4_layer.name == 'UDP'):
            vm_cmds.append(STLVmWrFlowVar(
                fv_name='pkt_len', pkt_offset='UDP.len', add_val=-len(l2_layer) - len(l3_layer)))
    else:
        raise STLError('length_mode should be one of the following: %s' % [
                       'auto', 'fixed'] + list(trim_dict.keys()))

    if payload_len < 0:
        raise STLError(
            'Packet length is bigger than defined by frame_size* or l3_length*. '
            + 'We got payload size %s' % payload_len)
    ...
    if vm_cmds:
        ...
        if kwargs['split_by_cores'] == 'split':
            ...
            raise STLError("split_by_cores '%s' is not supported" %
                           kwargs['split_by_cores'])
...
def get_TOS(user_kwargs, kwargs):
    ...
    TOS1 = set(['ip_precedence', 'ip_delay', 'ip_throughput',
                'ip_reliability', 'ip_cost', 'ip_reserved'])
    TOS2 = set(['ip_dscp', 'ip_cu'])
    user_args = set(user_kwargs.keys())
    if user_args & (TOS1 - TOS0) and user_args & (TOS0 - TOS1):
        raise STLError('You have mixed %s and %s TOS parameters' %
                       (TOS0, TOS1))
    ...
    if user_args & (TOS1 - TOS2):
        return(kwargs['ip_precedence'] << 5) + (kwargs['ip_delay'] << 4)
        + (kwargs['ip_throughput'] << 3) + (kwargs['ip_reliability'] << 2)
        + (kwargs['ip_cost'] << 1) + kwargs['ip_reserved']
...
def split_vlan_arg(vlan_arg):
    ...
    raise STLError(
        'vlan argument invalid(expecting list, int, long, str, None): %s' % vlan_arg)
...
def split_vlan_args(kwargs):
    ...
    dot1q_headers_count = max([len(x) for x in list(vlan_args_dict.values())])
    vlan_args_per_header = [{} for _ in range(dot1q_headers_count)]
    for arg, value in list(vlan_args_dict.items()):
...
def correct_sizes(kwargs):
    for arg, value in list(kwargs.items()):
        if is_integer(value):
            if arg.endswith(('_length', '_size', '_size_min', '_size_max',
                             '_length_min', '_length_max')):
                kwargs[arg] -= 4
