...
def track_it(testdir, func):
    testmon = CoreTestmon(project_dirs=[testdir.tmpdir.strpath], testmon_labels=set())
    testmon_data = CoreTestmonData(testdir.tmpdir.strpath)
    testmon.start()
    func()
    testmon.stop()
    nodedata = testmon_data.node_data_from_cov(testmon.cov, "test_a.py::test_1")
...
class TestPytestReportHeader:
    ...
    def test_pytest_cov(self, testdir, test_a):
        tmp_cov = None


        result = testdir.runpytest_inprocess("--testmon", "--cov=.")
        result.stdout.fnmatch_lines(
            [
...
class TestVariant:
    def test_separation(self, testdir):
        make_testmon_testdata(
            {"test_a.py::test_1": {"test_a.py": encode_lines([])}}, environment=1
        )
        make_testmon_testdata(
            {"test_a.py::test_2": {"test_a.py": encode_lines([])}}, environment=2
        )
        td3 = CoreTestmonData(testdir.tmpdir.strpath, environment="2")
...
class TestmonDeselect(object):
    ...
    def test_interrupted(self, test_a, testdir):
        ...
        testdir.runpytest_subprocess("--testmon",)
    ...
    def test_nonfunc_class_2(self, testdir):
        ...
        result = testdir.runpytest_inprocess("-vv", "--collectonly", "--testmon",)
        result.stdout.fnmatch_lines(
            ["*test_one*",]
        )
...
class TestPrioritization:
    def test_simple(self, testdir):
        ...
        reports = {
            "test_simple.py::test_1": {
                "setup": {"duration": 1},
                "call": {"duration": 2},
                "teardown": {"duration": 1},
            },
            "test_simple.py::test_2": {
                "setup": {"duration": 1},
                "call": {"duration": 1},
                "teardown": {"duration": 1},
            },
        ...
        assert items[0].nodeid == "test_simple.py::test_2"
        assert items[1].nodeid == "test_simple.py::test_1"
