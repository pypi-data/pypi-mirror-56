...
It also provides three converter functions `fromParser`, `fromDict` and `fromFunction` that can create a parameter class compatible with this package from existing parameters in  `argparse.ArgumentParser` and `dict` formats or using a function's keyword arguments.
