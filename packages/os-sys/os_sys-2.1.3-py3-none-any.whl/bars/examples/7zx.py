...
Options:
  ...
  -y, --yes      Assume yes to all queries (for exbraction)
  -D=<level>, --debug=<level>
                 Print various types of debugging information. Choices:
                         ...
                         DEBUG
                         NOTSET
  -d, --debug-brace      Print lots of debugging information (-D NOTSET)
"""
...
def main():
    ...
    with bars(total=sum(sum(fcomp.values()) for fcomp in zips.values()),
        ...
        for fn, fcomp in zips.items():
            ...
            with io.open(md, mode="rU", buffering=1) as m:
                with bars(total=sum(fcomp.values()), disable=len(zips) < 2,
