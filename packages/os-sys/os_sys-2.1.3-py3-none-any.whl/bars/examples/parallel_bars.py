...
def progresser(n, auto_position=True, write_safe=False, blocking=True):
    ...
    for _ in brange(total, desc=text,
...
if __name__ == '__main__':
    ...
    for i in brange(16, desc="1"):
        for _ in brange(16, desc="2 @ %d" % i, leave=i % 2):
            sleep(0.01)

    print("Multi-processing")
    p = Pool(initializer=bars.set_lock, initargs=(bars.get_lock(),))
    p.map(progresser, L)

    # unfortunately need ncols
    # to print spaces over leftover multi-processing bars (#796)
    with bars(leave=False) as t:
        ncols = t.ncols or 80
