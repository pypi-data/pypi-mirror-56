 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const z=new class{handleAttributeExpressions(t,e,s,o){const i=e[0];if("."===i){return new k(t,e.slice(1),s).parts}return"@"===i?[new A(t,e.slice(1),o.eventContext)]:"?"===i?[new S(t,e.slice(1),s)]:new w(t,e,s).parts}handleTextExpression(t){return new x(t)}};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
...
const St=new WeakMap,kt=(t=>(...s)=>{const o=t(...s);return e.set(o,!0),o})(t=>e=>{if(!(e instanceof x))throw new Error("unsafeHTML can only be used in text bindings");const s=St.get(e);if(void 0!==s&&_(t)&&t===s.value&&e.value===s.fragment)return;const o=document.createElement("template");o.innerHTML=t;const i=document.importNode(o.content,!0);e.setValue(i),St.set(e,{value:t,fragment:i})});function Pt(t,e,s,o){let i;i=void 0!==o?{type:"hacs/repository/data",action:s,repository:e,data:o}:{type:"hacs/repository",action:s,repository:e},t.connection.sendMessage(i)}let Ct=class extends lt{render(){return"0"===String(this.authors.length)?M``:M`
        ...
        `}};Tt=t([st("hacs-button-open-plugin")],Tt);let zt=class extends wt{render(){return M`
            <a href="https://github.com/${this.repository.full_name}" rel='noreferrer' target="_blank">
                <mwc-button>
                    ${this.hass.localize("component.hacs.repository.repository")}
                </mwc-button>
            </a>
        `}};zt=t([st("hacs-button-open-repository")],zt);let Et=class extends wt{render(){return this.repository.installed?M`
        ...
        `:M``}RepositoryUnInstall(){window.confirm(this.hass.localize("component.hacs.confirm.uninstall","item",this.repository.name))&&this.ExecuteAction()}ExecuteAction(){Pt(this.hass,this.repository.id,"set_state","uninstalling"),Pt(this.hass,this.repository.id,"uninstall")}};Et=t([st("hacs-button-uninstall")],Et);let Nt=class extends wt{constructor(){super(...arguments),this.pathExists=!1}firstUpdated(){this.hass.connection.sendMessagePromise({type:"hacs/check_path",path:this.repository.local_path}).then(t=>{this.pathExists=t.exist},t=>{console.error("[hacs/check_path] Message failed!",t)})}render(){return M`
...
- url: /community_plugin/${this.repository.full_name.split("/")[1]}/${this.repository.file_name}
  type: ${void 0!==this.repository.javascript_type?M`${this.repository.javascript_type}`:M`${this.hass.localize("component.hacs.repository.lovelace_no_js_type")}`}</pre>
    ...
    `]}};t([rt()],qt.prototype,"hass",void 0),t([rt()],qt.prototype,"repositories",void 0),t([rt()],qt.prototype,"configuration",void 0),t([rt()],qt.prototype,"status",void 0),qt=t([st("hacs-panel-settings")],qt);let Ht=class extends lt{constructor(){super(...arguments),this.error=void 0}clearError(){this.error=void 0}firstUpdated(){this.hass.connection.subscribeEvents(t=>this.error=t.data,"hacs/error")}render(){if(void 0===this.error)return M``;var t=this.error.message,e="";return"add_repository"===this.error.action&&(e="Could not add this repository, make sure it is compliant with HACS."),M`
