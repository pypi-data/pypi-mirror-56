...
`;
const HacsStyle = [haStyle, navStyle, baseHacsStyles, mobileHacsStyles];

let HacsHelpButton = class HacsHelpButton extends LitElement {
    render() {
        return html `
        <a href="#">
            <ha-icon
    ...
    openHelp() {
        ...
        if (location === "plugin")
            location = "stores";
        if (location === "appdaemon")
            location = "stores";
        if (location === "python_script")
            location = "stores";
        if (location === "theme")
            location = "stores";
...
function AddedToLovelace(repository, lovelaceconfig, status) {
    ...
    if (lovelaceconfig !== undefined) {
        var loaded = false;
        var URL = `/community_plugin/${repository.full_name.split("/")[1]}/${repository.file_name}`;
        if (lovelaceconfig.resources !== undefined) {
            lovelaceconfig.resources.forEach((item) => {
                if (!loaded) {
                    if (item.url === URL)
                        loaded = true;
                }
...
let HacsPanelStore = class HacsPanelStore extends LitElement {
    constructor() {
        super(...arguments);
            ...
            var _repositories = this.repositories.filter(function (repo) {
                if (category !== "installed") {
                    ...
                    if (config.country !== "ALL" && repo.country !== undefined) {
                        if (config.country !== repo.country)
                            return false;
                    }
                }
                else {
                ...
                if (repo.category === category) {
                    if (SearchTerm !== "" || null) {
                        ...
                        if (repo.full_name.toLowerCase().includes(SearchTerm))
                            return true;
                        if (String(repo.authors).toLowerCase().includes(SearchTerm))
                            return true;
                        if (String(repo.topics).toLowerCase().includes(SearchTerm))
                            return true;
                        return false;
                    }
                    if (repo.new)
    ...
    <div class="card-group">
      ...
      ${newRepositories.sort((a, b) => (a.name > b.name) ? 1 : -1).map(repo => html `
    ...
    <div class="card-group">
    ${_repositories.sort((a, b) => (a.name > b.name) ? 1 : -1).map(repo => html `
    ...
    get _rootPath() {
        if (window.location.pathname.split("/")[1] === "hacs_dev")
            return "hacs_dev";
        return "hacs";
    }
    static get styles() {
...
__decorate([
    property()
], HacsPanelStore.prototype, "hass", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "repositories", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "configuration", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "route", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "panel", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "status", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "repository_view", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "repository", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "SearchTerm", void 0);
__decorate([
    property()
], HacsPanelStore.prototype, "lovelaceconfig", void 0);
HacsPanelStore = __decorate([
...
let HacsButtonUninstall = class HacsButtonUninstall extends HacsRepositoryButton {
    ...
    RepositoryUnInstall() {
        if (window.confirm(this.hass.localize("component.hacs.confirm.uninstall", "item", this.repository.name)))
            this.ExecuteAction();
    }
    ExecuteAction() {
...
let HacsButtonMainAction = class HacsButtonMainAction extends HacsRepositoryButton {
    ...
    RepositoryInstall() {
        if (!this.repository.can_install) {
            window.alert(`This repository version requires Home Assistant version ${this.repository.homeassistant}`);
            return;
        }
        if (this.pathExists && !this.repository.installed) {
            if (window.confirm(this.hass.localize("component.hacs.confirm.exsist", "item", this.repository.local_path)
...
- url: /community_plugin/${this.repository.full_name.split("/")[1]}/${this.repository.file_name}
  type: ${(this.repository.javascript_type !== undefined
    ...
    CopyToLovelaceExampleToClipboard(ev) {
...
let HacsButtonAddToLovelace = class HacsButtonAddToLovelace extends HacsRepositoryButton {
    render() {
        ...
        if (!this.repository.installed)
            return html ``;
        if (this.repository.javascript_type === null)
            return html ``;
        return html `
    ...
    RepositoryAddToLovelace() {
        if (!window.confirm("Do you want to add this to your lovelace resources?"))
            return;
        this.hass.connection.sendMessagePromise({
            type: 'lovelace/config', force: false
        }).then((resp) => {
            var currentConfig = resp;
            const cardConfig = {
                "type": this.repository.javascript_type,
                "url": `/community_plugin/${this.repository.full_name.split("/")[1]}/${this.repository.file_name}`
            };
            if (currentConfig.resources)
                currentConfig.resources.push(cardConfig);
            else
                currentConfig.resources = [cardConfig];
...
__decorate([
    property()
], HacsButtonAddToLovelace.prototype, "lovelaceconfig", void 0);
__decorate([
    property()
], HacsButtonAddToLovelace.prototype, "configuration", void 0);
    ...
    render() {
        ...
        if (this.repository.status == "pending-restart") {
            ...
            if (this.lovelaceconfig !== undefined && !this.status.background_task) {
                var loaded = AddedToLovelace(this.repository, this.lovelaceconfig, this.status);
                if (!loaded) {
                    type = "warning";
                    title = "Not Loaded";
                    message = `
...
let HacsPanelRepository = class HacsPanelRepository extends LitElement {
    constructor() {
        super(...arguments);
        this.repository_view = false;
    }
                    ...
                    label="${this.hass.localize(`component.hacs.repository.available`)}:
                     (${this.hass.localize(`component.hacs.repository.newest`)}: ${this.repo.releases[0]})">
                      <paper-listbox slot="dropdown-content" selected="-1">
                          ${this.repo.releases.map(release => html `<paper-item>${release}</paper-item>`)}
...
__decorate([
    property()
], HacsPanelRepository.prototype, "hass", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "repositories", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "configuration", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "repository", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "panel", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "route", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "status", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "repository_view", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "repo", void 0);
__decorate([
    property()
], HacsPanelRepository.prototype, "lovelaceconfig", void 0);
HacsPanelRepository = __decorate([
...
let CustomRepositories = class CustomRepositories extends LitElement {
    ...
    render() {
        ...
        <ha-card header="${this.hass.localize("component.hacs.settings.custom_repositories")}">
            ...
            ${this.custom.sort((a, b) => (a.full_name > b.full_name) ? 1 : -1).map(repo => html `
...
__decorate([
    property()
], CustomRepositories.prototype, "hass", void 0);
__decorate([
    property()
], CustomRepositories.prototype, "repositories", void 0);
__decorate([
    property()
], CustomRepositories.prototype, "custom", void 0);
__decorate([
    property()
], CustomRepositories.prototype, "status", void 0);
__decorate([
    property()
], CustomRepositories.prototype, "configuration", void 0);
__decorate([
    property()
], CustomRepositories.prototype, "SaveSpinner", void 0);
...
let HiddenRepositories = class HiddenRepositories extends LitElement {
    ...
    render() {
        ...
        <ha-card header="${this.hass.localize("component.hacs.settings.hidden_repositories").toUpperCase()}">
            ...
            ${this._hidden.sort((a, b) => (a.full_name > b.full_name) ? 1 : -1).map(repo => html `
                <div class="row" .repoID=${repo.id}>
                    <paper-item>
                    <ha-icon
...
__decorate([
    property()
], HiddenRepositories.prototype, "hass", void 0);
__decorate([
    property()
], HiddenRepositories.prototype, "repositories", void 0);
__decorate([
    property()
...
let HacsPanelSettings = class HacsPanelSettings extends LitElement {
    render() {
        if (this.status.reloading_data)
            ;
        return html `
...
__decorate([
    property()
], HacsPanelSettings.prototype, "hass", void 0);
__decorate([
    property()
], HacsPanelSettings.prototype, "repositories", void 0);
__decorate([
    property()
], HacsPanelSettings.prototype, "configuration", void 0);
__decorate([
    property()
], HacsPanelSettings.prototype, "status", void 0);
HacsPanelSettings = __decorate([
...
__decorate([
    property()
], HacsProgressbar.prototype, "status", void 0);
...
let HacsFrontendBase = class HacsFrontendBase extends LitElement {
    ...
    firstUpdated() {
        ...
        load_lovelace();
        // Event subscription
        this.hass.connection.subscribeEvents(() => this.getRepositories(), "hacs/repository");
        ...
        this.hass.connection.subscribeEvents(() => this.getLovelaceConfig(), "lovelace_updated");
    }
    _reload(e) {
...
__decorate([
    property()
], HacsFrontendBase.prototype, "repositories", void 0);
__decorate([
    property()
], HacsFrontendBase.prototype, "configuration", void 0);
__decorate([
    property()
], HacsFrontendBase.prototype, "status", void 0);
__decorate([
    property()
...
__decorate([
    property()
], HacsFrontendBase.prototype, "panel", void 0);
__decorate([
    property()
], HacsFrontendBase.prototype, "repository", void 0);
__decorate([
    property()
