...
def create_network():
    ...
    network = fastr.Network(id_='add')

    # Create a source node in the network
    source1 = network.create_source('Float', id_='source')

    # Create a new node in the network using toollist
    node = network.create_node('Add', id_="add")
    ...
    node.inputs['right_hand'] = [3.14]

    # Create a sink to save the data
    sink = network.create_sink(fastr.typelist['Float'], id_='sink')
