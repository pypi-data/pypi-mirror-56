...
class TestRailListener(object):
    """Fixing of testing results and update test case in
    [ http://www.gurock.com/testrail/ | TestRail ].
    ...
    == Preconditions ==
    1. [ http://docs.gurock.com/testrail-api2/introduction
       | Enable TestRail API] \n
    2. Create custom field "case_description" with type "text", which
    corresponds to the Robot Framework's test case documentation.
    ...
    | robot --listener TestRailListener.py:testrail_server:tester_user:tester_password:20:https:update  autotest.robot
    5. Test with case_id=10 will be marked as failed in TestRail
        with message "Test fail message" and defects "BUG-1, BUG-2".
    Also title, description and references of this test will be updated in TestRail.
        Parameter "update" is optional.
    """
    ...
    def __init__(self, server, user, password, run_id, protocol='http',
        ...
        *Args:*\n
            ...
            _juggler_disable_ - indicator to disable juggler logic; if exist,
                                then juggler logic will be disabled;\n
            _update_ - indicator to update test case in TestRail; if exist,
                                then test will be updated.
        """
    ...
    def end_test(self, name, attributes):
        ...
        *Args:* \n
        ...
        if not case_id:
            logger.warn("[TestRailListener] No case_id presented for test_case {name}."
                        .format(name=name))
            return

        if 'skipped' in [tag.lower() for tag in attributes['tags']]:
            logger.warn("[TestRailListener] SKIPPED test case \"{name}\" with testrailId={case_id} "
                        "will not be posted to Testrail".format(name=name, case_id=case_id))
            return
        ...
        try:
            ...
            logger.error("[TestRailListener] http error on case_id = {case_id}\n{error}"
                         .format(case_id=case_id, error=error))

    def _update_case_description(self, attributes, case_id, name,
                                 references):
        ...
        *Args:* \n
        ...
        description = "{}\nPath to test: {}"\
            .format(attributes['doc'], attributes['longname'])
        request_fields = {
        ...
        try:
            ...
            logger.info("[TestRailListener] result for method update_case: {result}"
                        .format(result=result))
        except requests.HTTPError as error:
            logger.error("[TestRailListener] http error, while execute request:\n{error}"
                         .format(error=error))

    def _prepare_test_result(self, attributes, defects, old_test_status_id,
                             case_id):
        ...
        *Returns:*\n
        ...
        comment = "Autotest name: {}\nMessage: {}\nTest time:" \
                  " {} s".format(attributes['longname'], attributes['message'], test_time)
        if link_to_report:
            comment += '\nLink to Report: {link_to_report}'.format(link_to_report=link_to_report)
        if self.juggler_disable:
            ...
            new_test_status_id = self._prepare_new_test_status_id(attributes['status'],
                                                                  old_test_status_id)
        test_result = {
    ...
    def _prepare_new_test_status_id(self, new_test_status, old_test_status_id):
        ...
        *Returns:*\n
            New test status id.
        """
        old_statuses_to_fail = (self.TESTRAIL_TEST_STATUS_ID_PASSED,
                                self.TESTRAIL_TEST_STATUS_ID_FAILED, None)
    ...
    def _get_url_report_by_case_id(self, case_id):
        ...
        *Returns:*\n
        ...
        report_uri = '{report_filename}#search?include=testrailid={case_id}'.\
            format(report_filename=report_filename, case_id=case_id)
        if 'TEAMCITY_HOST_URL' in self.vars_for_report_link:
            ...
            build_url = '{}/repository/download/{}/{}:id/{}'.\
                format(base_hostname, buildtype_id, build_id, report_artifact_path)
        elif 'JENKINS_BUILD_URL' in self.vars_for_report_link:
            build_url = self.vars_for_report_link['JENKINS_BUILD_URL'] + 'robot/report'
        return '{}/{}'.format(build_url, report_uri) if build_url else None
