...
class RarChecker:
    ...
    def _set_scheme(self) -> None:
        ...
        try:
            form_num = self.xml_content.xpath('/Файл/@ВерсФорм')[0]
            form_ver = self.xml_content.xpath('/Файл/ФормаОтч/@НомФорм')[0]
        except AttributeError:
            raise SchemeNotFound(self.filename)

        self.xsd_scheme = self.compendium[(form_num, *form_ver.split('.'))]
