...
class Bigoyster:
    ...
    def create_retail_device(self, location_id, device_type, device_ref, friendly_name=''):
        if not friendly_name:
            friendly_name = device_ref
        values = {
            'name': friendly_name,
            'location_id': location_id,
            'customer_ref': device_ref,
            'device_type': device_type
        }
        success, result = self.call_create_api('device', values=values)
        if success:
            return {'success': 'true', 'device_id': device_ref}
        else:
