        ...
        def hook(module, input, output):
            ...
            if hasattr(module, "weight") and hasattr(module.weight, "size"):
                ...
                if len(summary[m_key]["weight"] )>1:
                    summary[m_key]["flops"]+=np.prod(np.array(summary[m_key]["weight"]).astype(np.float64))*np.prod(np.array( summary[m_key]["input_shape"][2:]).astype(np.float64))
                else:
                    summary[m_key]["flops"] +=np.prod(np.array(summary[m_key]["weight"]).astype(np.float64))
