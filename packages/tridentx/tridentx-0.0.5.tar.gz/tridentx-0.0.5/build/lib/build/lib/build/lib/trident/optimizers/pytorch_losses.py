...
def make_one_hot(labels, classes):
    one_hot = torch.zeros((labels.size(0), classes, labels.size(2), labels.size(3))).to(_device)
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=None, ignore_index=255, size_average=True):
        ...
        self.gamma = gamma
        self.size_average = size_average
        self.CE_loss = nn.CrossEntropyLoss(reduction='mean', ignore_index=ignore_index, weight=alpha)
