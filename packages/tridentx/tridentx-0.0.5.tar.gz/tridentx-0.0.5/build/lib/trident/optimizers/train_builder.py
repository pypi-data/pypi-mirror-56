            ...
            try:
                for mbs,( input, target) in enumerate(data_loader):
                    ...
                    if (mbs) % self.print_progress_frequency == 0:
                        ...
                        progress_bar(mbs, len(dataset.batch_sampler), 'Loss: {0:.3f} | {1}'.format(np.array(losses).mean(), ','.join(['{0}: {1:.3%}'.format(snake2camel(k), v / len(metrics)) for k, v in base_metrics.items()])))
...
#     accu = 1 - np.mean(np.not_equal(np.argmax(output.cpu().detach().numpy(), -1).astype(np.int64),
#                                     target.cpu().detach().numpy().astype(np.int64)))
#     return accu
import trident as T
dataset=T.load_mnist('mnist','train',is_flatten=False)

plan = TrainingPlan()\
    .add_training_item(TrainingItem(lenet_model,'Adam',lr=0.01, betas=(0.9, 0.999), eps=1e-08,weight_decay=0.005)
                       .with_loss(nn.CrossEntropyLoss) \
                        .with_metrics(accuracy))\
    .with_data_loader(dataset)\
    .repeat_epochs(2)\
    .within_minibatch_size(32)\
    .print_progress_every(20)\
    .save_model_every(200,'Models/')\
    .start_now()
