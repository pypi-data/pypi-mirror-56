...
import trident as T
from trident.layers.pytorch_layers import GcdConv2d_1,Conv2d
# 是否使用GPU
...
for epoch in range(num_epochs):
    ...
    while mbs<=2000:
        ...
        gcd_loss = gcd_focal_criterion(gcd_output2, target) +gce_iou_criterion(gcd_output2,target.unsqueeze(1))+gcd_mse_criterion(gcd_output,T.make_one_hot(target.unsqueeze(1),2)[:,1:2,:,:])+ gcd_dice(gcd_output2, target).sum()
