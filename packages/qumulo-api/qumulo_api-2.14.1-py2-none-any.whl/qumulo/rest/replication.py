...
def replicate(conninfo, credentials, relationship):
    method = "POST"
    uri = "/v2/replication/source-relationships/{}/replicate".format(
        relationship)
...
def authorize(
    ...
    uri = UriBuilder(
        path="/v2/replication/target-relationships/{}/authorize".format(
            relationship_id))
...
def abort_replication(conninfo, credentials, relationship_id):
    method = "POST"
    uri = "/v2/replication/source-relationships/{}/abort-replication"
    return request.rest_request(
        conninfo, credentials, method, uri.format(relationship_id))
...
def release_queued_snapshot(
        conninfo, credentials, relationship_id, snapshot_id):
    method = "DELETE"
    uri = "/v2/replication/source-relationships/{}/queued-snapshots/{}"
    return request.rest_request(
