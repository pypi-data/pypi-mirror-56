...
class ReplicationLinkSnapshotPolicy(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument("--relationship-id", required=True,
            help="Unique identifier of the source replication relationship.")
        parser.add_argument("--snapshot-policy-id", required=True,
            help="Identifier of the snapshot policy to link.")
        parser.add_argument("-t", "--target-expiration", type=str,
            default=None, help=(
    ...
    def main(conninfo, credentials, args):
        ...
        snapshot_policies.append({
            'id': int(args.snapshot_policy_id),
            'target_expiration': target_ttl,
        })
...
class ReplicationUnlinkSnapshotPolicy(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        ...
        new_policies = [policy for policy in old_policies if \
            policy['id'] != int(args.snapshot_policy_id)]
...
class ReplicationListQueuedSnapshots(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = "List information for all snapshots awaiting replication " \
...
class ReplicationReleaseQueuedSnapshot(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = "Release a snapshot queued for replication from the queue " \
