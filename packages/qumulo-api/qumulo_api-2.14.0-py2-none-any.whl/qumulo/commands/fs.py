...
class ListLocksCommand(qumulo.lib.opts.Subcommand):
    NAME = "fs_list_locks"
    DESCRIPTION = "List file locks held by clients."
    ...
    def options(parser):
        ...
        filters.add_argument("--path", type=str,
            help="List locks on the file with this path.")
        filters.add_argument("--id", type=str,
            help="List locks on the file with this ID.")
        filters.add_argument("--ip", type=str,
            help="List all locks held by the client with this IP address.")
        filters.add_argument("--hostname", type=str,
            help="List all locks held by the client with this hostname. "
                 "Only available for NLM locks.")

        parser.add_argument("--snapshot", type=str,
            help="When a file is specified, "
                 "list locks held on a specific snapshot.")
        parser.add_argument(
            "--no-resolve", dest="resolve", default=True, action="store_false",
        ...
        parser.add_argument(
            "--sort", choices=["file", "client"], default="file",
    ...
    def fmt_lock_mode(lock):
        ...
        if 'offset' in lock:
            return '{} [{}, {})'.format(
                mode,
                lock['offset'],
                int(lock['offset']) + int(lock['size']))
        else:
            return mode

    @staticmethod
    def fmt_lock_table(locks, sort_field):
        headers = ['File', 'Client', 'Mode']
        table = [
            [
                l.get('path') or l['file_id'],
                l.get('owner_hostname') or l['owner_address'],
                ListLocksCommand.fmt_lock_mode(l),
            ]
            for l in locks
        ]
        sort_field_to_col = {
        ...
        table.sort(key=lambda x: (x[sort_field_to_col[sort_field]], x))
        return util.tabulate(table, headers)

    @staticmethod
    def resolve_paths(conninfo, credentials, locks, fs_mod=fs):
        paths, _etag = fs_mod.resolve_paths(
            conninfo, credentials, list({l['file_id'] for l in locks}))
        paths = {i['id']: i['path'] for i in paths}

        for l in locks:
            l['path'] = paths.get(l['file_id']) or None

    @staticmethod
    def resolve_hostnames(conninfo, credentials, locks, dns_mod=dns):
        hostnames, _etag = dns_mod.resolve_ips_to_names(conninfo, credentials,
            list({l['owner_address'] for l in locks}))
        hostnames = {
            i['ip_address']: i['hostname'] for i in hostnames
            if i['result'] == 'OK'}

        for l in locks:
            l['owner_hostname'] = hostnames.get(l['owner_address']) or None
    ...
    def main(conninfo, credentials, args):
        ...
        if args.path or args.id:
            locks = fs.list_all_locks_by_file(
                conninfo,
                credentials,
                args.protocol,
                args.lock_type,
            ...
            # Note that this function is misleadingly named; the API does not
            # require that a client be specified, and is intended to list all
            # locks in that case.
            locks = fs.list_all_locks_by_client(
                conninfo,
                credentials,
                args.protocol,
                args.lock_type,
        ...
        if args.resolve:
            ListLocksCommand.resolve_paths(conninfo, credentials, locks)
            ListLocksCommand.resolve_hostnames(conninfo, credentials, locks)

        if args.json:
            print(json.dumps(locks, indent=4))
        else:
            print(ListLocksCommand.fmt_lock_table(locks, args.sort))
...
class SetPermissionsSettingsCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(conninfo, credentials, args):
        print(fs.set_permissions_settings(
            conninfo, credentials, mode=args.mode))
...
def _expand_for_explain(conninfo, credentials, args, outfile):
    ...
    if res['type'] == auth.ID_TYPE_GROUP:
        raise ValueError(
            'User ID is actually a group: {}'.format(Identity(res['id'])))
    if res['id']['domain'] == AD_DOMAIN:
        ad_conf, _etag = ad.poll_ad(conninfo, credentials)
        if not ad_conf['use_ad_posix_attributes']:
            outfile.write('Warning: Asked to explain an AD user\'s rights, but '
    ...
    if not groups:
        raise ValueError(
            'No groups given or found. At least one group is required.')
...
class AclExplainRightsCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            "-u", "--user", nargs="+", metavar="ID", type=str_decode,
            dest="users", required=True,
            help="User for whom to explain rights. e.g. Alice, uid:1000, "
                 ...
                 "explanation.")
        parser.add_argument(
            "-g", "--groups", nargs="*", metavar="ID", type=str_decode,
            default=[],
            help="Groups that the user should be considered a member of for "
                 "the purpose of the explanation.")
        parser.add_argument(
            "--no-expand", dest="expand", action="store_false", default=True,
    ...
    def main(conninfo, credentials, args, outfile=sys.stdout):
        if args.expand:
            users, groups = _expand_for_explain(
                conninfo, credentials, args, outfile)
        elif not args.groups:
            sys.stderr.write(
                'At least one group must be given with --no-expand\n')
            sys.exit(2)
        else:
