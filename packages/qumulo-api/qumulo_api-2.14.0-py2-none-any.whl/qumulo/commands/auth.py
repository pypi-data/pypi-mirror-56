...
def verbose_name(_id, preferred_type=None):
    ...
    '''
    if _id.has_name():
        return u'{} ({})'.format(_id, _id.numeric_str(preferred_type))
    else:
        return u'{}'.format(_id.numeric_str(preferred_type))
...
def tabulate_id_list(ids):
    ...
    return util.tabulate(
        sorted([
            [unicode(i) if i.has_name() else u'', i.numeric_str()]
            for i in ids]),
        headers=["Name", "ID"])

def format_expanded_id(res):
    return '\n'.join([
        u'Identity:    {}'.format(verbose_name(id_util.Identity(res['id']))),
        u'Type:        {}'.format(res['type'].capitalize()),
        u'NFS Mapping: {}'.format(
            verbose_name(id_util.Identity(res['nfs_id']))
            if res['nfs_id'] else None),
        u'SMB Mapping: {}'.format(
            verbose_name(id_util.Identity(res['smb_id']), 'sid')
            if res['smb_id'] else None),
        u'',
        u'Equivalent Identities:',
        tabulate_id_list(
            [id_util.Identity(e) for e in res['equivalent_ids']]),
        u'',
        u'Group Membership:',
        tabulate_id_list(
            [id_util.Identity(e) for e in res['group_ids']])
    ])

class ExpandIdentityCommand(qumulo.lib.opts.Subcommand):
    NAME = 'auth_expand_identity'
    DESCRIPTION = 'Find equivalent identities and full group membership.'
...
class FindIdentityCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(conninfo, credentials, args):
        res = auth.find_identity(
            conninfo, credentials, **api_id_from_args(args))
        if args.json:
            print(res)
            return
...
class IdentityAttributesGetCommand(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = 'Get attributes related to the given identity.'
...
class IdentityAttributesSetCommand(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = 'Set attributes related to the given identity.'
    ...
    def main(conninfo, credentials, args):
        ...
        print(auth.set_identity_attributes(
            conninfo, credentials, auth_id, attrs))
