...
def redact_dicom_attr(header, tag):
    ...
    if isstr(value):
        ...
        isinstance(value, pydicom.valuerep.PersonName) or
        isinstance(value, pydicom.valuerep.PersonName3)
    ):
        header[tag].value = "XXXX"
...
def anonymise_dicom_dataset(dcm, actions=DICOM_ANON_MIN_SUPP_55):
    """
    ...
    while node_queue:
        ...
        for tag in header.keys():
            if check_tag(header, tag):
                ...
                template = ("deleting key {!r} with invalid data from header "
...
class RelativeTimeAnonymiser:
    ...
    def __init__(self):
        ...
        attributes_to_remove = [
            ...
            atr for atr_pair in self._datetime_attributes for atr in atr_pair
        ]
    ...
    def anonymise(self, filename):
        ...
        anonymise_dicom_dataset(header, self._restricted_actions)
        _updateMetaInfo(header)
