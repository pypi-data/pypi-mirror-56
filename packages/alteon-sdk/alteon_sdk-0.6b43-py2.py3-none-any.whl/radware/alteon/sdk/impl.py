...
class AlteonRest(DeviceAPI):
    ...
    def read(self, bean, retries=3):
        all_result = self.read_all(bean, retries)
        return self._return_read(bean, all_result)
    ...
    def update(self, bean, retries=3, dry_run=False):
        if not dry_run:
        ...
        if not dry_run:
            for x in range(0, retries):
                result = self._rest_client.put(url, data=data, **self._rest_params)
                if result.ok:
                    break
            self._alteon_response_processor(result, bean)
        return True
    ...
    def _read_all(self, bean, retries=3):
        log.debug('read bean from {0}, {1}'.format(self.server, bean))
        result = None
        for x in range(0, retries):
            result = self._rest_client.get(self._rest_url(bean), **self._rest_params)
            if result.ok:
                break
