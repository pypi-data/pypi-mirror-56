...
class KnotMap:
    def __init__(self, protlen, protstart=0, patches=1, missing_residues=[], broken_chain=[], rasterized=True,
        ...
        self.minoraxe = LocatableAxes(self.fig, divider.get_position())
...
#     pdb = argv[2].lower()
#     chain = argv[3]  # .upper()
#     z = fetchPDBinfo(pdb, chain)
#     miss = z.getMissing()
#     brk = z.getCalfaBreaks()
#
#     knotmap_data = WandaReader(knotfile, argv[4])
#     unique_knots = knotmap_data.getUniqueKnots()
#
#     knots_size = len(unique_knots)
#     knotmap = KnotMap(patches=knots_size, missing_residues=miss, broken_chain=brk, protstart=knotmap_data.chainStart(), \
#                       protlen=knotmap_data.chainEnd(), rasterized=True)
#     knotmap2 = KnotMap(patches=knots_size, missing_residues=miss, broken_chain=brk, protstart=knotmap_data.chainStart(), \
#                        protlen=knotmap_data.chainEnd(), rasterized=False, arrows=False)
#     for e in unique_knots:
#         d = knotmap_data.getKnot(e)
#         knotmap.add_patch_new(d)
#         knotmap2.add_patch_new(d)
#     knotmap2.saveFilePng(pdb + "_" + chain + ".png")
#     knotmap.saveFile(pdb + "_" + chain + ".svg")
#     knotmap2.saveFile(pdb + "_" + chain + "_vec.svg")
#     knotmap.close()
#     knotmap2.close()