"""
The 2D Polynomial definition from the bgabrovsek@github Laurent class
Adapted by Pawel Dabrowski-Tumanski
12.11.2018
"""
...
class term:
    ...
    def __call__(self, d):
        ...
        # if isinstance(self.coef, int) and all(isinstance(e, int) for e in d.values()) and all(
        #         abs(e) == 1 for e in d.values()):
        #     t.coef = int(round(t.coef))  # make int
        # t.canonical()
        return res * laurent(str(t))
...
class laurent:
    ...
    def print_short(self, double = False):
        ...
        if len(variables) == 0:
            ...
            return                  # TODO make exception or general case
        var = variables[0]
        coefficients = {}
        result = []
        for t in self.term:
    ...
    def understand_output(self, typ):
        ...
        """
        trans = {}
        trans['Yamada'] = self.understand_Yamada()
        trans['Jones'] = self.understand_Jones()
        try:
            return trans[typ]
        except KeyError:
            return "Unknown polynomial type"

    def understand_Jones(self):
        from Jones import Jones
        try:
            return Jones[self.print_short(double=True)]
        except KeyError:
            return "Unknown"
