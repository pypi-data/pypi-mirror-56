...
'''.format(FIST_TYPES)
    ...
    def start(self):
        args = list(sys.argv)
        args[1] = 'run'
        args += ['--disable_console_logger']  # direct output to /dev/null after the fist is ready
        cmd = ' '.join(args)
        temp_file = tempfile.NamedTemporaryFile(delete=False)
        subprocess.Popen(cmd, shell=True, stdout=temp_file.file, stderr=subprocess.STDOUT)
    ...
    def db(self):
        ...
        parser.add_argument('action', choices=['create', 'init', 'drop', 'print', 'exec'], help='actions to take')
        ...
        if args.action == 'create':
            ...
            try:
                import sqlalchemy
                res: sqlalchemy.engine.result.ResultProxy = engine.execute(sql)
            except Exception as e:
                print('[error]')
                print(e)
            else:
