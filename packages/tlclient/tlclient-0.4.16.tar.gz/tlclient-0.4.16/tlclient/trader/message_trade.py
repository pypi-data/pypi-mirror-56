...
class RtnTrade(ctypes.Structure):
    ...
    def __str__(self):
        return '<t tid={} oid={} ref={} tg={} exch={} tkr={} dir={} tp={} px={} vol={} comm={} t={}>'.format(
            self.trade_id, self.order_id, self.order_ref, self.tg_name, ExchangeID.read(self.exchange), self.ticker,
            Direction.read(self.direction), OrderType.read(self.order_type), self.price, self.volume, self.commission,
...
class PositionInfo:
    ...
    def __init__(self, d=None):
        ...
        self.margin_mode = None if d is None else MarginMode.parse(d['margin_mode'])
        self.asset_type = None if d is None else AssetType.parse(d['asset_type'])
        ...
        self.pnl = None if d is None else d['pnl']
        self.frozen = None if d is None else d['frozen']
        self.available = None if d is None else d['available']
        self.borrowed = None if d is None else d['borrowed']
        self.interest = None if d is None else d['interest']
        self.liquidation_price = None if d is None else d['liquidation_price']
        self.equity = None if d is None else d['equity']
        self.risk_rate = None if d is None else d['risk_rate']
    ...
    def __str__(self):
        return '<pos-{}-{} tkr={} src={} dir={} mm={} pos={} ydpos={} cost={} frz={} avail={} brw={} i={} lpx={} eqt={} rr={} pnl={}>'.format(
            ...
            self.frozen, self.available, self.borrowed, self.interest, self.liquidation_price, self.equity, self.risk_rate, self.pnl
        )
    ...
    def to_kdb(self, dt):
        ...
        return [
            ...
            self.liquidation_price, self.equity, self.risk_rate
        ]
...
class RspHistoryTrades:
    ...
    def __str__(self):
        s = '<trades tg={} #trades={}>'.format(self.tg_name, len(self.trades))
        return s
