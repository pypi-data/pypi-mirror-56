    ...
    def _proc_write_thread(self, res):
        try:
            ...
            if writes_time < 1:
                ...
                self.logger.error('[influxdb] end write_points. (res){} (time){}'.format(len(res), writes_time))
        except:
            self.logger.error('[influxdb] write_points error. (err_msg){}'.format(sys.exc_info()))

    def _write_queue_checker(self):
        while not self.is_stopped() or not self.write_queue.empty():
            res = []
            i = 0
            while i < 5000:
                try:
