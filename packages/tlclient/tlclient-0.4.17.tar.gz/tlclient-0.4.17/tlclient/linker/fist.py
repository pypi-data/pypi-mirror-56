...
class Fist:
    ...
    def _recv_req(self, socket, timeout=1000):
        poll = zmq.Poller()
    ...
    def req_master(self, frame):
        ...
        ret = self._recv_req(self.req_master_sock)
        if ret is None:
            raise Exception("Timeout in req_master!")
        return Frame(bytearray(ret))
    ...
    def run_sub(self):
        while not self.is_stopped():
            try:
                socks = dict(self.sub_poller.poll(1000))
                for sock in socks:
                    ret = sock.recv()
                    ret = bytearray(ret)
                    f = Frame(ret)
