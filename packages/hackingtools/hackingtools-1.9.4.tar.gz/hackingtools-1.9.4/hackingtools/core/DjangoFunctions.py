...
def getModulesFunctionsForMap():
    ...
    for module in ht.modules_loaded:
        for f in ht.modules_loaded[module]:
            ...
            conf_func = ht.Config.getConfig(parentKey='maps', key=module.split('.')[-1], subkey=functionName)
            if not conf_func:
                ht.Config.switch_function_for_map(ht.getModuleCategory(module.split('.')[-1]), module.split('.')[-1], functionName)
            else:
