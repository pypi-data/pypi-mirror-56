...
# === setAPIKey ===
def setAPIKey(api_name, api_key, django_session=None):
    if django_session:
        if not django_session in config['core']['__API_KEY__']:
            config['core']['__API_KEY__'][django_session] = {}
        config['core']['__API_KEY__'][django_session][api_name] = api_key
    else:
...
def loadRestAPIsFile(rest_api_file, password, django_session=None):
    with open(rest_api_file, 'r') as res:
        ...
        if django_session:
            if not django_session in config['core']['__API_KEY__']:
                config['core']['__API_KEY__'][django_session] = {}
            config['core']['__API_KEY__'][django_session] = api_keys
        else:
            config['core']['__API_KEY__'] = api_keys

def saveRestAPIsFile(rest_api_file, password, django_session=None):
    if django_session:
        if not django_session in config['core']['__API_KEY__']:
            config['core']['__API_KEY__'][django_session] = {}
        api_keys = config['core']['__API_KEY__'][django_session]
    else:
