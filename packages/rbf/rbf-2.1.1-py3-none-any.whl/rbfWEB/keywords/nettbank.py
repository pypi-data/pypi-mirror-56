...
class NettbankKeywords(LibraryComponent):
    ...
    @keyword
    def login_bankid(self, otp, password, one_time_code="Engangskode", personal_password="Personlig passord",  timeout=10):
        """
        *Login with Bank ID Authentication.*
        
        *Arguments*

        ``frame_bankid``: Location of Bank ID iframe.
        
        ``frame_element``: Location of div in iframe to provide user info.
                
        ``otp``: One time code.
		
        ``password``: Personal password.
		
        ``one_time_code``: Optional. Text to wait for in iframe. Engangskode by default. Needs to be changed for English version.
        
        ``personal_password``: Optional. Text to wait for in iframe. Personlig passord by default. Needs to be changed for English version.
        
        ``timeout``: Optional. Timeout will wait for 10 second by default to identify the elements on the page.
        
        *Examples*
              
        | `Login Bankid` | otp=***** | password=****** |

        """ 
        frame_bankid='//*[@id="bidclient"]/iframe'
        frame_element='class=full_width_height'
        self.info("Login with Bank ID Authentication. Where one_time_code: %s, personal_password: %s, timeout: %s" % (one_time_code, personal_password, timeout))
        WaitingKeywords.wait_until_page_contains_element(self, frame_bankid, timeout)
        FrameKeywords.select_frame(self, frame_bankid)
        self.info("Bank ID frame was selected. Waiting for one time code to be provided")
        
        time.sleep(2)
        self.wait_until_element_contains(frame_element, one_time_code, timeout)
        self._input_text_into_text_field_and_press_enter(frame_element, otp)
        self.info("One time code provided. Waiting for personal password to be provided")
        
        self.wait_until_element_contains(frame_element, personal_password, timeout)
        time.sleep(2)
        self._input_text_into_text_field_and_press_enter(frame_element, password)
        self.info("Personal password provided")
    ...
    def _input_text_into_text_field_and_press_enter(self, locator, text):
