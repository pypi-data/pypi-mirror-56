...
#            else: raise AssertionError(e)
    ...
    @keyword(name=None, tags=("Settings",))
    def capture_screen(self, height='410', width='670', filename=None):
        ...
        The default folder is the log folder of RobotFramework, if you want change see the `Set Screenshot Folder`.
           ...
           *Arguments*
           ...
           ``filename``: Optional. A name for the screenshot can be provided.   
           
           *Examples*
           
               | `Capture Screen` |
               | `Capture Screen` | filename=myfile |
               | `Capture Screen` | height=500 | width=700 |
               | `Capture Screen` | height=500 | width=700 | filename=myfile |
        """
        extension = 'html'
        if filename == None:           
            filename_prefix = 'screenshot'
            filename_sufix = str(int(round(time.time() * 1000)))
            filename = '%s_%s' % (filename_prefix, filename_sufix)
        filepath = os.path.join(self.imgfolder, '%s.%s' % (filename, extension))
        screenshot_path = "\"" + os.path.join(self.output_folder, filepath) + "\""
        self.mf.save_screen(screenshot_path)
