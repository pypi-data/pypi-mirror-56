...
class Notify:
    ...
    http_proxy  = "http://proxy:88"
    https_proxy = "https://proxy:88"
    ...
    def __init__(self, slackChannel, slackToken, testSuite, buildNum, buildBy):
        
        self.slackChannel = slackChannel
        self.slackToken = 'Bearer ' + slackToken
        self.testSuite = testSuite
        self.buildNum = os.environ['bamboo.buildNumber']
        self.buildBy = os.environ['bamboo.ManualBuildTriggerReason.userName']
    ...
    def _notify_slack(self):  
        ...
        self.payload["attachments"][0]["blocks"][3]["elements"][0]["text"] = buildDetails + " " + "by " + self.buildNum + " - " + self.buildBy
        ...
        response = requests.post(self.url, data=json.dumps(self.payload), headers=self.headers, proxies=proxyDict)
