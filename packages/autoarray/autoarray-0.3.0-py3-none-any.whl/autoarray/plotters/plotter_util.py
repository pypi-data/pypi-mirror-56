...
def output_figure(array, as_subplot, output_path, output_filename, output_format):
    ...
    -----------
    array : ndarray
        The 2D arrays of image to be output, required for outputting the image as a fits file.
    as_subplot : bool
...
def get_critical_curve_and_caustic(obj, grid, include_critical_curves, include_caustics):
    ...
    if include_critical_curves:
        critical_curves = obj.critical_curves(grid=grid)
    else:
        critical_curves = []

    if include_caustics:
        caustics = obj.caustics(grid=grid)
    else:
        caustics = []
...
def plot_lines(line_lists):
    """Plot the liness of the mask or the arrays on the figure.

    Parameters
    -----------t.
    mask : ndarray of data_type.arrays.mask.Mask
        The mask applied to the arrays, the edge of which is plotted as a set of points over the plotted arrays.
    plot_lines : bool
