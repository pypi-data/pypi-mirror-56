...
def include_grid(
    ...
    grid = convert_grid_units(
        grid_arcsec=grid, unit_label=unit_label, kpc_per_arcsec=kpc_per_arcsec
    )
...
def convert_grid_units(grid_arcsec, unit_label, kpc_per_arcsec):
    ...
    Parameters
    -----------
    grid_arcsec : ndarray or data_type.arrays.aa.Grid
    ...
    if unit_label in "scaled" or kpc_per_arcsec is None:
        return grid_arcsec
    elif unit_label in "kpc":
        return grid_arcsec * kpc_per_arcsec


def set_xy_labels(unit_label, kpc_per_arcsec, xlabelsize, ylabelsize, xyticksize):
    ...
    if unit_label in "scaled" or kpc_per_arcsec is None:
        ...
        raise exc.PlottingException(
            "The unit_label supplied to the plotted are not a valid string (must be pixels | "
            "arcsec | kpc)"
        )
...
def plot_points(grid, points, pointcolor):
    ...
    Parameters
    -----------
    grid : ndarray or data_type.arrays.aa.Grid
        The (y,x) coordinates of the grid, in an arrays of shape (total_coordinates, 2).
    points : []
        A set of points that are plotted in a different colour for emphasis (e.g. to show the mappings between \
        different planes).
