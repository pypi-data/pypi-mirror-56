...
from autoarray.structures.grids import grid_util
...
def mask_2d_centres_from_shape_pixel_scale_and_centre(shape, pixel_scales, centre):
    ...
    Parameters
     ----------
    shape : (int, int)
        The (y,x) shape of the 2D arrays the arc-second centre is computed for.
    pixel_scales : (float, float)
        The (y,x) arc-second to pixel scales of the 2D arrays.
    centre : (float, flloat)
    ...
    --------
    tuple (float, float)
        The (y,x) arc-second central coordinates of the input arrays.
...
def total_pixels_from_mask_2d(mask_2d):
    ...
    Parameters
     ----------
    mask_2d : ndarray
        A 2D arrays of bools, where *False* values are unmasked and included when counting pixels.
    ...
    mask = np.arrays([[True, False, True],
                 [False, False, False]
                 [True, False, True]])
...
@decorator_util.jit()
def total_sub_pixels_from_mask_2d_and_sub_size(mask_2d, sub_size):
    """Compute the total number of sub-pixels in unmasked pixels in a mask.
    
    Parameters
     ----------
    mask_2d : ndarray
        A 2D arrays of bools, where *False* values are unmasked and included when counting sub pixels.
    sub_size : int
        The size of the sub-grid that each pixel of the 2D mask arrays is divided into.
    ...
    """
    return total_pixels_from_mask_2d(mask_2d) * sub_size ** 2


@decorator_util.jit()
def total_sparse_pixels_from_mask_2d(mask_2d, unmasked_sparse_grid_pixel_centres):
...
def mask_2d_circular_from_shape_2d_pixel_scales_and_radius(
    shape_2d, pixel_scales, radius, centre=(0.0, 0.0)
):
    """Compute a circular mask from the 2D mask arrays shape and radius of the circle.

    This creates a 2D arrays where all values within the mask radius are unmasked and therefore *False*.
    ...
    --------
    ndarray
        The 2D mask arrays whose central pixels are masked as a circle.
    ...
    centres_arcsec = mask_2d_centres_from_shape_pixel_scale_and_centre(
...
def mask_2d_circular_annular_from_shape_2d_pixel_scales_and_radii(
    ...
    This creates a 2D arrays where all values within the inner and outer radii are unmasked and therefore *False*.
    ...
    --------
    ndarray
        The 2D mask arrays whose central pixels are masked as a annulus.
    ...
    for y in range(mask.shape[0]):
        for x in range(mask.shape[1]):
            ...
            if (
                ...
                or outer_radius_2_arcsec >= r_arcsec >= outer_radius
            ):
                mask[y, x] = False
...
def mask_2d_elliptical_from_shape_2d_pixel_scales_and_radius(
    ...
    This creates a 2D arrays where all values within the ellipse are unmasked and therefore *False*.
    ...
    --------
    ndarray
        The 2D mask arrays whose central pixels are masked as an ellipse.
...
def mask_2d_elliptical_annular_from_shape_2d_pixel_scales_and_radius(
    ...
    This creates a 2D arrays where all values within the elliptical annuli are unmasked and therefore *False*.
    ...
    --------
    ndarray
        The 2D mask arrays whose elliptical annuli pixels are masked
...
@decorator_util.jit()
def blurring_mask_2d_from_mask_2d_and_kernel_shape_2d(mask_2d, kernel_shape_2d):
    ...
    -----------
    mask_2d : ndarray
        A 2D arrays of bools, where *False* values are unmasked.
    ...
    --------
    ndarray
        The 2D blurring mask arrays whose unmasked values (*False*) correspond to where the mask will have PSF light \
        blurred into them.

    Examples
    --------
    mask = np.arrays([[True, True, True],
                     [True, False, True]
                     [True, True, True]])      
    ...
    blurring_mask = np.full(mask_2d.shape, True)
                    ...
                    for x1 in range(
                        ...
                        if (
                            ...
                            raise exc.MaskException(
                                ...
                                "of the mask - pad the datas arrays before masking"
                            )
...
@decorator_util.jit()
def mask_2d_from_shape_2d_and_mask_2d_index_for_mask_1d_index(
    shape_2d, mask_2d_index_for_mask_1d_index
):
    """For a 1D arrays that was computed by util unmasked values from a 2D arrays of shape (rows, columns), map its \
    indexes back to the original 2D arrays to create the origianl 2D mask.

    This uses a 1D arrays 'one_to_two' where each index gives the 2D pixel indexes of the 1D arrays's unmasked pixels, \
    for example:

    - If one_to_two[0] = [0,0], the first value of the 1D arrays maps to the pixel [0,0] of the 2D arrays.
    - If one_to_two[1] = [0,1], the second value of the 1D arrays maps to the pixel [0,1] of the 2D arrays.
    - If one_to_two[4] = [1,1], the fifth value of the 1D arrays maps to the pixel [1,1] of the 2D arrays.

    Parameters
     ----------
    shape_2d : (int, int)
        The shape of the 2D arrays which the pixels are defined on.
    mask_2d_index_for_mask_1d_index : ndarray
        An arrays describing the 2D arrays index that every 1D arrays index maps too.

    Returns
    --------
    ndarray
        A 2D mask arrays where unmasked values are *False*.

    Examples
    --------
    one_to_two = np.arrays([[0,1], [1,0], [1,1], [1,2], [2,1]])
    ...
    for index in range(len(mask_2d_index_for_mask_1d_index)):
        mask[
            mask_2d_index_for_mask_1d_index[index, 0],
            mask_2d_index_for_mask_1d_index[index, 1],
        ] = False
...
@decorator_util.jit()
def edge_1d_indexes_from_mask_2d(mask_2d):
    """Compute a 1D arrays listing all edge pixel indexes in the mask. An edge pixel is a pixel which is not fully \
    surrounding by False mask values i.e. it is on an edge."""

    edge_pixel_total = total_edge_pixels_from_mask_2d(mask_2d)
...
@decorator_util.jit()
def check_if_border_pixel(mask_2d, edge_pixel_1d, mask_2d_index_for_mask_1d_index):
    edge_pixel_index = int(edge_pixel_1d)

    y = int(mask_2d_index_for_mask_1d_index[edge_pixel_index, 0])
    x = int(mask_2d_index_for_mask_1d_index[edge_pixel_index, 1])
...
@decorator_util.jit()
def total_border_pixels_from_mask_2d_and_edge_pixels(
    mask_2d, edge_pixels, mask_2d_index_for_mask_1d_index
):
    """Compute the total number of borders-pixels in a mask."""
...
@decorator_util.jit()
def border_1d_indexes_from_mask_2d(mask_2d):
    """Compute a 1D arrays listing all borders pixel indexes in the mask. A borders pixel is a pixel which:

     1) is not fully surrounding by False mask values.
     2) Can reach the edge of the arrays without hitting a masked pixel in one of four directions (upwards, downwards,
     left, right).
    ...
    edge_pixels = edge_1d_indexes_from_mask_2d(mask_2d=mask_2d)
    mask_2d_index_for_mask_1d_index = sub_mask_2d_index_for_sub_mask_1d_index_via_mask_2d(
        mask_2d=mask_2d, sub_size=1
    )

    border_pixel_total = total_border_pixels_from_mask_2d_and_edge_pixels(
        mask_2d=mask_2d,
        edge_pixels=edge_pixels,
        mask_2d_index_for_mask_1d_index=mask_2d_index_for_mask_1d_index,
    )
...
def sub_border_pixel_1d_indexes_from_mask_2d_and_sub_size(mask_2d, sub_size):
    """Compute a 1D arrays listing all borders pixel indexes in the mask. A borders pixel is a pixel which:
    ...
    border_pixels = border_1d_indexes_from_mask_2d(mask_2d=mask_2d)

    sub_border_pixels = np.zeros(shape=border_pixels.shape[0])

    mask_1d_index_to_sub_mask_2d_indexes = sub_mask_1d_indexes_for_mask_1d_index_via_mask_2d(
        mask_2d=mask_2d, sub_size=sub_size
    )

    masked_sub_grid_1d = grid_util.grid_1d_via_mask_2d(
...
@decorator_util.jit()
def mask_1d_index_for_sub_mask_1d_index_via_mask_2d(mask_2d, sub_size):
    """"For pixels on a 2D arrays of shape (rows, colums), compute a 1D arrays which, for every unmasked pixel on \
    this 2D arrays, maps the 1D sub-pixel indexes to their 1D pixel indexes.

    For example, for a sub-grid size of 2, the following mappings from sub-pixels to 2D arrays pixels are:

    - mask_1d_index_for_sub_mask_1d_index[0] = 0 -> The first sub-pixel maps to the first unmasked pixel on the 2D arrays.
    - mask_1d_index_for_sub_mask_1d_index[3] = 0 -> The fourth sub-pixel maps to the first unmasked pixel on the 2D arrays.
    - mask_1d_index_for_sub_mask_1d_index[7] = 1 -> The eighth sub-pixel maps to the second unmasked pixel on the 2D arrays.

    The term 'grid' is used because the grid is defined as the grid of coordinates on the centre of every \
    pixel on the 2D arrays. Thus, this arrays maps sub-pixels on a sub-grid to pixels on a grid.
    ...
    total_sub_pixels = total_sub_pixels_from_mask_2d_and_sub_size(
        mask_2d=mask_2d, sub_size=sub_size
    )
...
def sub_mask_1d_indexes_for_mask_1d_index_via_mask_2d(mask_2d, sub_size):
    """"For pixels on a 2D arrays of shape (rows, colums), compute a 1D arrays which, for every unmasked pixel on \
    this 2D arrays, maps the 1D sub-pixel indexes to their 1D pixel indexes.
...
@decorator_util.jit()
def sub_mask_1d_index_for_sub_mask_2d_index_from_sub_mask_2d(sub_mask_2d):
    """Create a 2D arrays which maps every False entry of a 2D mask to its 1D mask arrays index 2D binned mask. Every \
    ...
    The mask_2d_to_mask_1d arrays would be:
    ...
    ----------
    sub_mask_2d : ndarray
        The 2D mask that the util arrays is created for.

    Returns
    -------
    ndarray
        The 2D arrays util 2D mask entries to their 1D masked arrays indexes.
    ...
    return sub_mask_1d_index_for_sub_mask_2d_index


@decorator_util.jit()
def sub_mask_2d_index_for_sub_mask_1d_index_via_mask_2d(mask_2d, sub_size):
    """Compute a 1D arrays that maps every unmasked sub-pixel to its corresponding 2d pixel using its (y,x) pixel indexes.

    For example, for a sub-grid size of 2, f pixel [2,5] corresponds to the first pixel in the masked 1D arrays:

    - The first sub-pixel in this pixel on the 1D arrays is grid_to_pixel[4] = [2,5]
    - The second sub-pixel in this pixel on the 1D arrays is grid_to_pixel[5] = [2,6]
    - The third sub-pixel in this pixel on the 1D arrays is grid_to_pixel[5] = [3,5]
