...
def padded_binning_array_2d_from_array_2d(array_2d, bin_up_factor, pad_value=0.0):
    """If an arrays is to be binned up, but the dimensions are not divisible by the bin-up factor, this routine pads \
    the arrays to make it divisible.

    For example, if the arrays is shape (5,5) and the bin_up_factor is 2, this routine will pad the arrays to shape \
    (6,6).

    Parameters
    ----------
    array_2d : ndarray
        The 2D arrays that is padded.
    bin_up_factor : int
        The factor which the arrays is binned up by (e.g. a value of 2 bins every 2 x 2 pixels into one pixel).
    pad_value : float
        If the arrays is padded, the value the padded edge values are filled in using.

    Returns
    -------
    ndarray
        The 2D arrays that is padded before binning up.
...
def bin_array_2d_via_mean(array_2d, bin_up_factor):
    """Bin up an arrays to coarser resolution, by binning up groups of pixels and using their mean value to determine \
     the value of the new pixel.

    If an arrays of shape (8,8) is input and the bin up size is 2, this would return a new arrays of size (4,4) where \
    every pixel was the mean of each collection of 2x2 pixels on the (8,8) arrays.

    If binning up the arrays leads to an edge being cut (e.g. a (9,9) arrays binned up by 2), the arrays is first \
    padded to make the division work. One must be careful of edge effects in this case.

    Parameters
    ----------
    array_2d : ndarray
        The 2D arrays that is binned up.
    bin_up_factor : int
        The factor which the arrays is binned up by (e.g. a value of 2 bins every 2 x 2 pixels into one pixel).
    ...
    -------
    ndarray
        The binned up 2D arrays from the input 2D arrays.
...
def bin_array_2d_via_quadrature(array_2d, bin_up_factor):
    """Bin up an arrays to coarser resolution, by binning up groups of pixels and using their quadrature value to \
    determine the value of the new pixel.

    If an arrays of shape (8,8) is input and the bin up size is 2, this would return a new arrays of size (4,4) where \
    every pixel was the quadrature of each collection of 2x2 pixels on the (8,8) arrays.
...
def bin_array_2d_via_sum(array_2d, bin_up_factor):
    """Bin up an arrays to coarser resolution, by binning up groups of pixels and using their sum value to determine \
     the value of the new pixel.

    If an arrays of shape (8,8) is input and the bin up size is 2, this would return a new arrays of size (4,4) where \
    every pixel was the sum of each collection of 2x2 pixels on the (8,8) arrays.
...
def bin_mask_2d(mask_2d, bin_up_factor):
    ...
    If binning up the arrays leads to an edge being cut (e.g. a (9,9) arrays binned up by 2), an arrays is first \
    extracted around the centre of that arrays.


    Parameters
    ----------
    mask_2d : ndarray
        The 2D arrays that is resized.
    new_shape : (int, int)
        The (y,x) new pixel dimension of the trimmed arrays.
    origin : (int, int)
        The oigin of the resized arrays, e.g. the central pixel around which the arrays is extracted.

    Returns
    -------
    ndarray
        The resized 2D arrays from the input 2D arrays.
...
def mask_1d_index_for_padded_mask_2d_index_via_mask_2d(mask_2d, bin_up_factor):
    ...
    -------
    ndarray
        The 2D arrays util padded 2D mask entries to their 1D masked arrays indexes.
...
def binned_mask_1d_index_for_padded_mask_2d_index_via_mask_2d(mask_2d, bin_up_factor):
    """Create a 2D arrays which maps every False entry of a 2D mask to its 1D binned mask index (created using the \
    *binned_upmask_from_mask_2d_and_bin_up_factor* method).

    We create an arrays the same shape as the 2D mask (after padding for the binnning up procedure), where each entry \
    gives the binned up mask's 1D masked arrays index.
    ...
    -------
    ndarray
        The 2D arrays util 2D mask entries to their 1D binned masked arrays indexes.
...
def binned_masked_array_1d_for_masked_array_1d_via_mask_2d(mask_2d, bin_up_factor):
    """Create a 1D arrays which maps every (padded) masked index to its corresponding 1D index in the binned 1D \
    mask.
    ...
    -------
    ndarray
        The 1D arrays util 1D binned mask entries to their corresponding 1D masked arrays index.
...
def masked_array_1d_for_binned_masked_array_1d_via_mask_2d(mask_2d, bin_up_factor):
    """Create a 1D arrays which maps every (padded) binned masked index to its correspond 1D index in the original 2D \
...
def masked_array_1d_for_binned_masked_array_1d_all_via_mask_2d(mask_2d, bin_up_factor):
    """Create a 2D arrays which maps every (padded) binned masked index to all of the corresponding 1D indexes of the \
