...
class Client(BaseClient):
    ...
    def create_user(
        ...
        **Request Syntax**
        ...
        :param Username: **[REQUIRED]**
          ...
          user name can include up to 20 characters. If you are using SAML for identity management,
          the user name can include up to 64 characters from [a-zA-Z0-9_-.\\@]+.

        :type Password: string
        :param Password:

          The password for the user account. A password is required if you are using Amazon Connect
        ...
        :param DirectoryUserId:
          ...
          The identifier of the user account in the directory used for identity management. If
          Amazon Connect cannot access the directory, you can specify this identifier to
    ...
    def describe_user(self, UserId: str, InstanceId: str) -> ClientDescribeUserResponseTypeDef:
        ...
        final part of the ARN). The console does not display the user IDs. Instead, list the users
    ...
    def get_current_metric_data(
        ...
        <https://docs.aws.amazon.com/connect/latest/adminguide/real-time-metrics-reports.html>`__ in
        ...
        **Request Syntax**
              ...
              CurrentMetrics=[
                  {
                      ...
                      |'AGENTS_ERROR'|'AGENTS_STAFFED'|'CONTACTS_IN_QUEUE'
                      |'OLDEST_CONTACT_AGE'|'CONTACTS_SCHEDULED'|'AGENTS_ON_CONTACT'
        ...
        :param Filters: **[REQUIRED]**
          ...
          retrieved only for the resources associated with the queues or channels included in the
          filter. You can include both queue IDs and queue ARNs in the same request. The only
        ...
        :param Groupings:
          ...
          metrics returned apply to each queue rather than aggregated for all queues. If you group
          by ``CHANNEL`` , you should include a Channels filter. The only supported channel is
          ``VOICE`` .
                        ...
                        'Collections': [
                            {
                                'Metric': {
                                    'Name':
                                    'AGENTS_ONLINE'|'AGENTS_AVAILABLE'
                                    |'AGENTS_ON_CALL'
                                    |'AGENTS_NON_PRODUCTIVE'
                                    |'AGENTS_AFTER_CONTACT_WORK'
                                    |'AGENTS_ERROR'|'AGENTS_STAFFED'
                                    |'CONTACTS_IN_QUEUE'
                                    |'OLDEST_CONTACT_AGE'
                                    |'CONTACTS_SCHEDULED'
                                    |'AGENTS_ON_CONTACT'|'SLOTS_ACTIVE'
                                    |'SLOTS_AVAILABLE',
                                    'Unit': 'SECONDS'|'COUNT'|'PERCENT'
                                },
                                'Value': 123.0
    ...
    def get_metric_data(
        ...
        <https://docs.aws.amazon.com/connect/latest/adminguide/historical-metrics.html>`__ in the
        ...
        **Request Syntax**
              ...
              HistoricalMetrics=[
                  {
                      ...
                      |'HANDLE_TIME'|'AFTER_CONTACT_WORK_TIME'|'QUEUED_TIME'
                      |'ABANDON_TIME'|'QUEUE_ANSWER_TIME'|'HOLD_TIME'
        ...
        :param StartTime: **[REQUIRED]**
          ...
          retrieval of historical metrics data. The time must be specified using a multiple of 5
        ...
        :param EndTime: **[REQUIRED]**
          ...
          retrieval of historical metrics data. The time must be specified using an interval of 5
          minutes, such as 11:00, 11:05, 11:10, and must be later than the start time timestamp.
        ...
        :param Groupings:
          ...
          queue, the metrics returned are grouped by queue. The values returned apply to the metrics
          for each queue rather than aggregated for all queues.
                        ...
                        'Collections': [
                            {
                                'Metric': {
                                    'Name':
                                    'CONTACTS_QUEUED'|'CONTACTS_HANDLED'
                                    |'CONTACTS_ABANDONED'
                                    |'CONTACTS_CONSULTED'
                                    |'CONTACTS_AGENT_HUNG_UP_FIRST'
                                    |'CONTACTS_HANDLED_INCOMING'
                                    ...
                                    |'CONTACTS_TRANSFERRED_IN'
                                    |'CONTACTS_TRANSFERRED_OUT'
                                    |'CONTACTS_TRANSFERRED_IN_FROM_QUEUE'|'CONTACTS_TRANSFERRED_OUT_FROM_QUEUE'|'CONTACTS_MISSED'|'CALLBACK_CONTACTS_HANDLED'|'API_CONTACTS_HANDLED'|'OCCUPANCY'|'HANDLE_TIME'|'AFTER_CONTACT_WORK_TIME'|'QUEUED_TIME'|'ABANDON_TIME'|'QUEUE_ANSWER_TIME'|'HOLD_TIME'|'INTERACTION_TIME'|'INTERACTION_AND_HOLD_TIME'
                                    |'SERVICE_LEVEL',
                                    'Threshold': {
                                        'Comparison': 'LT',
                                        'ThresholdValue': 123.0
                ...
                'ContactFlowSummaryList': [
                    {
                        ...
                        'Name': 'string',
                        'ContactFlowType':
                        'CONTACT_FLOW'|'CUSTOMER_QUEUE'|'CUSTOMER_HOLD'
                        |'CUSTOMER_WHISPER'|'AGENT_HOLD'|'AGENT_WHISPER'
                ...
                'PhoneNumberSummaryList': [
                    {
                        ...
                        |'KE'|'KI'|'KW'|'KG'|'LA'|'LV'|'LB'|'LS'|'LR'|'LY'|'LI'|'LT'
                        |'LU'|'MO'|'MK'|'MG'|'MW'|'MY'|'MV'|'ML'|'MT'|'MH'|'MR'|'MU'
                        |'YT'|'MX'|'FM'|'MD'|'MC'|'MN'|'ME'|'MS'|'MA'|'MZ'|'MM'|'NA'
                        |'NR'|'NP'|'NL'|'AN'|'NC'|'NZ'|'NI'|'NE'|'NG'|'NU'|'KP'|'MP'
                        |'NO'|'OM'|'PK'|'PW'|'PA'|'PG'|'PY'|'PE'|'PH'|'PN'|'PL'|'PT'
                        |'PR'|'QA'|'CG'|'RE'|'RO'|'RU'|'RW'|'BL'|'SH'|'KN'|'LC'|'MF'
                        |'PM'|'VC'|'WS'|'SM'|'ST'|'SA'|'SN'|'RS'|'SC'|'SL'|'SG'|'SX'
                        |'SK'|'SI'|'SB'|'SO'|'ZA'|'KR'|'ES'|'LK'|'SD'|'SR'|'SJ'|'SZ'
                        |'SE'|'CH'|'SY'|'TW'|'TJ'|'TZ'|'TH'|'TG'|'TK'|'TO'|'TT'|'TN'
                        |'TR'|'TM'|'TC'|'TV'|'VI'|'UG'|'UA'|'AE'|'GB'|'US'|'UY'|'UZ'
    ...
    def start_chat_contact(
        ...
        <https://docs.aws.amazon.com/connect-participant/latest/APIReference/API_CreateParticipantConnection.html>`__
        ...
        See also: `AWS API Documentation
        <https://docs.aws.amazon.com/goto/WebAPI/connect-2017-08-08/StartChatContact>`_
          ...
          **Response Structure**
          ...
          - *(dict) --*
            ...
            - **ParticipantId** *(string) --*
              ...
              The identifier for a chat participant. The participantId for a chat participant is the
    ...
    def start_outbound_voice_contact(
        ...
        **Request Syntax**
        ...
        :param ClientToken:
          ...
          A unique, case-sensitive identifier that you provide to ensure the idempotency of the
          request. The token is valid for 7 days after creation. If a contact is already started,
          the contact ID is returned. If the contact is disconnected, a new contact is started.
        ...
        :param SourcePhoneNumber:
          ...
          The phone number associated with the Amazon Connect instance, in E.164 format. If you do
        ...
        :param QueueId:
          ...
          phone number specified in the queue. If you do not specify a queue, the queue defined in
          the contact flow is used. If you do not specify a queue, you must specify a source phone
          number.
    ...
    def update_contact_attributes(
        ...
        can update the customer's name or the reason the customer called while the call is active,
        ...
        CRM application and save the data with the contact in Amazon Connect. You could also flag
        calls for additional analysis, such as legal review or identifying abusive callers.

        Contact attributes are available in Amazon Connect for 24 months, and are then deleted.

         **Important:** You cannot use the operation to update attributes for contacts that occurred
         prior to the release of the API, September 12, 2018. You can update attributes only for
         contacts that started after the release of the API. If you attempt to update attributes for
         a contact that occurred prior to the release of the API, a 400 error is returned. This
         applies also to queued callbacks that were initiated prior to the release of the API but
        ...
        **Request Syntax**
        ...
        :param Attributes: **[REQUIRED]**
          ...
          The Amazon Connect attributes. These attributes can be accessed in contact flows just like
