...
def _searchForLinks(input):
    """
    internal function
    check for links inside input and returns a list without internal links

    """
    i = 0
    while i < len(input):
        for parameter in input[i]['para']:
            if isinstance(input[i]['para'][parameter], str) and input[i]['para'][parameter][0] == '@':
...
class attributelist(list):
    ...
    def __init__(self, objekt):
        return list.__init__(self, objekt)
...
class dataListBase(list):
    ...
    def dlattr(self):
        """
        Show attributes of dataList without common attributes.

        """
        attr = [k for k in list(self.__dict__) if (k[0] != '_') and (k not in ('@name', 'raw_data'))]
        return sorted(attr)
    ...
    def savetxt(self, name=None, exclude=['comment', 'lastfit'], fmt='%.5e'):
        ...
        exclude : list of str, default ['comment','lastfit']
            List of dataList attributenames to exclude from being saved.
    ...
    def modelValues(self, **kwargs):
        ...
        **kwargs : parname=value
        ...
        # get args ad put into mappedArgs
    ...
    def fit(self, model, freepar={}, fixpar={}, mapNames={}, method='leastsq', xslice=slice(None), condition=None,
        ...
        method : default 'leastsq', 'differential_evolution', ‘BFGS’, ‘Nelder-Mead’  or from scipy.optimize.minimize
            ...
            - 'Nelder-Mead' allows optimization of integer variables .
        ...
        **Fit result attributes** ::
        ...
        if cov is not None:
            ...
            if self._nozeroerror and output:
                ...
                if output: print(CSIy +
                   'No Errors or zeros in Error!! Values are weighted equally! chi2 might not be closed to 1! ' + CSIe)
...
class dataList(dataListBase):
    ...
    def makeErrPlot(self, title=None, showfixpar=True, **kwargs):
        ...
        position = kwargs.pop('legpos', 'ur')
        if position in ['ll', 'ur', 'ul', 'lr'] or isinstance(position, (list, tuple)):
            self._errplotLegendPosition=position
...
class dataArrayBase(np.ndarray):
    ...
    def __new__(cls, input=None,
        ...
        **Rules for reading of ASCII files**
        ...
        if isinstance(input, str):  # if a filename is given
            ...
            input = _read(input, block=block, usecols=usecols, skiplines=skiplines, replace=replace, ignore=ignore,
                          delimiter=delimiter, takeline=takeline, lines2parameter=lines2parameter, encoding=encoding)
    ...
    def __array_wrap__(self, out_arr, context=None):
        ...
        # to transform the result into a new instance of the subclass. By default,
        # __array_wrap__ will call __array_finalize__, and the attributes will be inherited.
    ...
    def __reduce__(self):
        ...
        pickled_state = super(dataArrayBase, self).__reduce__()
    ...
    def __setstate__(self, state):
        ...
        super(dataArrayBase, self).__setstate__(state[0:-1])
    ...
    def setColumnIndex(self, *args, **kwargs):
        ...
        if args:
            if hasattr(args[0], '_isdataArray'):
                ...
                ixiyiey = (list(args[0]) + [''] * len(protectedIndicesNames))[:len(protectedIndicesNames)]
        else:
    ...
    def prune(self, lower=None, upper=None, number=None, kind='lin', col='X', weight='eY', keep=None, type='mean'):
        ...
        type : {None,'mean','error','mean+error','sum'} default 'mean'
            How to determine the value for a new point.
             ...
             - 'mean+std' calcs mean and adds error columns as standard deviation in intervals.
               Can be used if no errors are present to generate errors as std in intervals.
               For single values the error is interpolated from neighbouring values.
               ! For less pruned data error may be bad defined if only a few points are averaged.
        col : 'X','Y'....., or int, default 'X'
            Column to prune along X,Y,Z or index of column.
        weight : None, protectedNames as 'eY' or int
            Column for weight as 1/err**2 in 'mean' calculation,
            weight column gets new error sqrt(1/sum_i(1/err_i**2))
             - None is equal weight
             - If weight not existing or contains zeros equal weights are used.
        keep : list of int
            List of indices to keep in any case e.g. keep=np.r_[0:10,90:101]
        ...
        # We have to find the new intervals
        if kind[:3] == 'log':
    ...
    def interpolate(self, X, left=None, right=None, deg=1):
        ...
        if deg == 1:
            # outliers are nearest in np.interp
            result = dataArray(np.c_[X, np.interp(X, self.X[xsort], self.Y[xsort], left=left, right=right)].T)
        else:
            # outliers are handled above scipy 0.17.1 ; this will change later
            result = dataArray(np.c_[X, scipy.interpolate.interp1d(self.X[xsort], self.Y[xsort], kind=deg)(X)].T)
        # copy attributes
        result.setattr(self)
        return result
    ...
    def regrid(self, xgrid=None, zgrid=None, wgrid=None, method='nearest', fill_value=0):
        ...
        if wgrid is None:
            ...
            zz = zz.flatten()
            # ww = None
            f = griddata(points=self[[self._ix, self._iz]].T.array, values=self[self._iy].array,
                         xi=(xx, zz), method=method, fill_value=fill_value)
            if hasattr(self, '_iey'):
                # linear interpolate the errors if present
                fe = griddata(points=self[[self._ix, self._iz]].T.array, values=self[self._iey].array,
            ...
            zz = zz.flatten()
            ww = ww.flatten()
            f = griddata(points=self[[self._ix, self._iz, self._iw]].T.array, values=self[self._iy].array,
                         xi=(xx, zz, ww), method=method, fill_value=fill_value)
            if hasattr(self, '_iey'):
                fe = griddata(points=self[[self._ix, self._iz, self._iw]].T.array, values=self[self._iey].array,
    ...
    def extract_comm(self, iname=0, deletechars='', replace={}):
        ...
        for line in self.comment:
            ...
            numbers = [word for word in words if isinstance(word, numbers.Number)]
    ...
    def __repr__(self):
        ...
        try:
            ...
            comment = []
        desc = """dataArray->(X,Y,....=\n{0},\ncomment = {1}...,\nattributes = {2} ....,\nshape = {3} """
        return desc.format(shortprint(self, 49, 3) + '......', [a[:70] for a in comment], attr, np.shape(self))
