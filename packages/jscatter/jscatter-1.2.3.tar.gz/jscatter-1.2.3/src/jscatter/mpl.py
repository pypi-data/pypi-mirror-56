...
def contourImage(x, y=None, z=None, levels=None, fontsize=10, colorMap='jet', scale='norm', lineMap=None, title=None,
    ...
    Other examples ::
     ...
     fig=js.mpl.contourImage(xyz.X,xyz.Z,xyz.Y)
...
def showlastErrPlot2D(data, shape=None, scale='norm', colorMap='jet', method='nearest',
    ...
    ----------
    data : dataArray
        Dataarray with fit values in lastfit.
    shape : [int,int]
    ...
    ::
     ...
     data.fit(somepeaks,{'a':1,'b':3.8,'c':4},{},{'width':'X','height':'Z'})
     fig = js.mpl.showlastErrPlot2D(data)
    ...
    if shape is not None:
        ...
        lf_Y, _, _ = regrid(data.lastfit.Y, None, None, shape)
        ...
        lf_Y, _, _ = regrid(data.lastfit.Y, None, None,np.unique(data.X).shape[0])
        extend = [np.min(Z), np.max(Z), np.min(X), np.max(X)]
    else:
        ...
        lf_Y, _, _ = regrid(data.lastfit.regrid(nnx, nnz, 1, method=method, fill_value=0).Y, None, None, nnx)
    ...
    fig.colorbar(im2, ax=[ax1, ax2], pad=0.01, shrink=0.85)
    fig.colorbar(im3, ax=ax3, pad=0.01, shrink=0.85)
    ax1.set_ylabel('Z')
    ax2.set_xlabel('X')
    ax2.set_yticklabels([])
    ...
    fig.suptitle('Fit to model '+data.lastfit.func_name)
    txt=''
    txt += f'$\chi^2$        = {getattr(data.lastfit,"chi2"):.4G}\n'
    txt += f'dof       = {data.lastfit.dof:.4G}\n'
    if hasattr(data.lastfit,'cov'):
        txt += f'$cov_{{max}}$   = {np.max(data.lastfit.cov):.4G}\n'
    for attr in data._asdataList._freepar:
        val = getattr(data.lastfit, attr, None)
        err = getattr(data.lastfit, attr+'_err', None)
        txt += f'{attr:<10} = {val:.3G}'
        if err is not None:
            txt += f' ± {err:.3G}'
        txt += '\n'
    for attr in data._asdataList._fixpar:
        val = getattr(data.lastfit, attr)
        txt += f'{attr:<10} = {val:.3G} \n'

    fig.text(ax2.get_position().xmin*1.1, ax3.get_position().ymin, txt)
