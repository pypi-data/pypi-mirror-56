...
The scattering of a particle solution with particle concentration :math:`c_p` is

.. math:: I(q)= c_p I_p(q) = c_p I_0 F(q)
...
Particle form factor is (:math:`<>` indicates the ensemble average)
...
from .cloudscattering import cloudScattering, orientedCloudScattering
...
def sphereCoreShellGaussianCorona(q, Rc, Rs, Rg, Ncoil, thicknessCoils, coilSLD=0.64e-4, coreSLD=4.186e-4,
    ...
    ::
     ...
     p=js.grace()
     FF=js.ff.sphereCoreShellGaussianCorona(q,8,12,6,20,1.5)
     p.plot(FF,sy=[1,0.2],li=1)
     p.yaxis(label='I(q)',scale='l',min=1e-4,max=10)
     p.xaxis(label='q / nm\S-1',scale='l')
...
def multiShellDisc(q, radialthickness, shellthickness, shellSLD, solventSLD=0, alpha=None, nalpha=60):
    ...
    if isinstance(alpha, numbers.Number):
        ...
        result = formel.parQuadratureFixedGauss(_ff, alpha[0], alpha[1], 'angle', weights=w,
                                                n=nalpha, QQ=Q, R=radii, D=shelld)
...
def fuzzyCylinder(q, L, radius, sigmasurf, SLD=1e-3, solventSLD=0, alpha=None, nalpha=90):
    ...
    def _ff(QQ, r, L, angle, sig):
        # formfactor of a cylinder with orientation angle alpha
        QQ0 = np.r_[0, QQ]
        Pc = dSLD * _fqcylinder(QQ0, np.r_[r], L, angle)[:, 0] * np.exp(-sig ** 2 * QQ0 ** 2 / 2.)**2
...
def wormlikeChain(q, N, a, R=None, SLD=1, solventSLD=0, rtol=0.02):
    ...
    The forward scattering is :math:`I0=V^2(SLD-solventSLD)^2` with volume :math:`V=\pi R^2 N`.
...
def pearlNecklace(Q, N, Rc=None, l=None, A1=None, A2=None, A3=None, ms=None, mr=None, vmonomer=None, monomerlength=None):
    r"""
    ...
    # infinitely thin rod equ 16 selfterm ii
    ...
    # S_rs equ 21 in [1]
    Z3 = Y3*Y1*4 * ((N - 1) / (1 - np.sinc(QA)) - (1 - np.sinc(QA)**(N - 1)) / (1 - np.sinc(QA))**2 * np.sinc(QA))
...
def linearPearls(q, N, R, l, pearlSLD, cr, n=1, relError=0, rms=0, ffpolydispersity=0, ncpu=0,
    ...
    Positions are arranged along a line (x axis)  with positions :math:`x_{p=[0..N-1]}=p(2R+2d_{shell}+l)` for pearls and
    coils of radius :math:`r_c=l/(2n)` at positions :math:`x_{p=[0..N-1],c=[0..n-1]}=p(2R+l) + R +d_{shell}+ r_c +c 2r_c`
    ...
    ::
     ...
     for n in [0,0.5,1.3,2]:
         ...
         p.plot(fq,li=[1,2,-1],le='cr={0:.0f}'.format(n))
     p.plot(fq.formfactoramplitude_q,fq.formfactoramplitude[0],le='single sphere')
     p.plot(fq.formfactoramplitude_q,fq.formfactoramplitude[1],le='single gaussian')
    ...
    if d > 0 and shellSLD != solventSLD:
        fq = sphereCoreShell(q, Rc=R, Rs=R+d, bc=pearlSLD, bs=shellSLD, solventSLD=solventSLD)
    else:
        fq = sphere(q, R, pearlSLD-solventSLD)
    ...
    line[:N, 3] = fq.I0**0.5                  # scattering amplitude of pearls
    line[:N, 4] = 1                           # index formfactor
    # connectors as n Gaussian coils
    if n > 0:
        connectorSL = np.pi*cr**2*l*pearlSLD
        crg = l/2/n  # coil radius
        for m in range(M):
            line[N+m*n:N+(m+1)*n, 0]=line[m, 0] + R + d + crg + 2*crg*np.r_[0:n]
        line[N:, 3] = connectorSL / n
        line[N:, 4] = 2
        fq = fq.addColumn(1, gaussianChain(q, crg).Y)

    # use cloudscattering
    result = cloudScattering(q, line, relError=relError, formfactor=fq,
...
def ellipsoidFilledCylinder(q=1, R=10, L=0, Ra=1, Rb=2, eta=0.1, SLDcylinder=0.1, SLDellipsoid=1, SLDmatrix=0, alpha=90,
    ...
    ::
     ...
     p.plot(ff.X,ff[3],legend='cylinder formfactor')
     p.plot(ff.X,ff[4],legend='ellipsoid formfactor')
...
def superball(q, R, p, SLD=1, solventSLD=0, nGrid=15, returngrid=False):
    ...
    which results in a sphere for p=1.
    ...
    Visualisation as shown above ::
     ...
     for i,p in enumerate([0.2,0.5,1,1.3,20],1):
         ...
         grid=js.ff.superball(q,R,p=p,nGrid=20*(7-i),returngrid=True)
         grid.filter(lambda xyz: np.linalg.norm(xyz))
         grid.show(fig=fig, ax=ax)
    ...
    Compare to extreme cases of sphere (p=1) and cube (p->inf , use here 100) ::
     ...
     p[0].plot(ss,legend='superball p=1 nGrid=15 default')
     ss=js.ff.superball(q,R,p=1,nGrid=30)
     p[0].plot(ss,legend='superball p=1 nGrid=30')
     ...
     p[1].plot(cc,sy=[1,0.3,1],legend='superball p=100 nGrid=15')
     cc=js.ff.superball(q,R,p=100,nGrid=20)
     p[1].plot(cc,sy=[1,0.3,2],legend='superball p=100 nGrid=30')
    ...
    # The same is calculated in numpy.linalg.norm(xyz,ord=p2,axis=1)

    # pseudorandom grid
    grid = sf.pseudoRandomLattice([2*R, 2*R, 2*R], nGrid**3, b=0)
...
def multilayer(q, layerd, SLD, gaussw=None, solventSLD=0):
    ...
    **How to use in a fit model**.
    ...
    gaussw = np.atleast_1d([gw if isinstance(gw, numbers.Number) else 0 for gw in gaussw])
...
def multilamellarVesicles(Q, R, N, phi, displace=0, dR=0, dN=0, layers=0, gaussw=None, ds=0, SLD=1, solventSLD=0,
    ...
    """
    if isinstance(layers, numbers.Number): layers = [layers]
...
def decoratedCoreShell(q, Rcore, Rdrop, Ndrop, Hdrop, coreSLD, shellthickness=None, shellSLD=None, dropSLD=None,
    ...
    ::
     ...
     ax1.plot(drop.X,drop[3],'--', label='core shell')
     ax1.set_yscale('log')
     ax1.set_xscale('log')
     ax1.legend()
    ...
    """
    # use contrasts
    coreSLD = coreSLD - solventSLD
    if shellSLD is None:
        shellSLD = 0
    else:
        shellSLD = shellSLD - solventSLD
    if dropSLD is None:
        dropSLD = 0
    else:
        dropSLD = dropSLD - solventSLD

    if shellthickness is None: shellthickness=0

    # fq of coreshell and particles (root and norm is taken in cloudscattering)
    # this might be extended to complicated multishellSpheres
    if shellthickness ==0 or shellSLD ==0:
        fq = sphere(q, Rcore, coreSLD)
    elif Rcore ==0:
        fq = sphere(q, shellthickness, shellSLD)
    else:
        fq = multiShellSphere(q, [Rcore, shellthickness], [coreSLD, shellSLD], solventSLD=0)
    fq = fq.addColumn(1, 1)  # constant fa for points
    ...
    if typ == 'drop':
        ...
        # disc cuts in shell
        for point in pointsxyz:
            grid.alongLine(v=point, R=Rdrop, a=[0, 0, 0], direction=1, b=1)
    ...
    if show:
        ...
        fig.axes[0].plot_surface(x*Rcore, y*Rcore, z*Rcore,
                                 color='red', alpha=0.8)
        fig.axes[0].plot_surface(x*(Rcore+shellthickness), y*(Rcore+shellthickness), z*(Rcore+shellthickness),
                                 color='grey', alpha=0.2)
        return fig

    # complete the grid adding coreshell formfactor at center with respective scattering amplitude
    points = np.vstack([np.c_[grid.array, np.ones(grid.numberOfAtoms())*2], [0, 0, 0, fq.I0**0.5, 1]])
    res = cloudScattering(q, points, relError=relError, formfactor=fq, ncpu=0)
    result=res.addColumn(1, fq.Y)
...
def inhomogeneousSphere(q, Rcore, Rdrop, Ddrops, coreSLD, dropSLD=None, solventSLD=0, rms=0,
    ...
    r"""
    Scattering of a sphere filled with droplets.

    The model described spherical particle filled with drops.
    Drops are added only in the internal volume (drop center < Rcore) extending outside if radius is large enough.
    ...
    typ : 'drop',
        Type of the drops
         ...
         - 'gauss' gaussian coils. Coil scattering length is dropSLD*4/3pi Rdrop**3 .
    distribution : 'random','quasirandom','hcp'
        Distribution of drops as :
    ...
    ::
     ...
     ax1.plot(drop.X,drop[3],'--', label='sphere homogeneous')
     ...
     drop3 = js.ff.inhomogeneousSphere(q=q,Rcore=20, Rdrop=R, Ddrops=D, rms=4, coreSLD=0.1, dropSLD=1.5, typ='g')
     ax1.plot(drop3.X,drop3.Y, label='sphere with gaussian drops rms=4')
     ax1.set_yscale('log')
     ax1.set_xscale('log')
     ax1.legend()
     fig.set_size_inches(8,4)
    ...
    if shellthickness > 0 and shellSLD != 0 :
        fq = sphereCoreShell(q, Rc=Rcore, Rs=Rcore+shellthickness, bc= coreSLD,
                             bs=shellSLD, solventSLD=solventSLD)
    else:
        fq = sphere(q, Rcore, coreSLD)
    if typ[0] == 'gauss'[0]:
        fq = fq.addColumn(1, gaussianChain(q, Rdrop).Y)
    else:
        # default sphere
        fq = fq.addColumn(1, sphere(q, Rdrop, dropSLD).Y)  # drops

    # a grid for the droplets
    # volume fraction of sphere in cube is  (Rcore*2)**3/(np.pi/6*(2*Rcore)**3) = 6/np.pi = 1.9098593
    V = 4/3*np.pi*Rdrop**3
    if distribution[:1] == 'fcc'[:1]:
        size = Rcore/Ddrops * 1.2
        grid = sf.fccLattice(abc=Ddrops*2**0.5, size=size, b=0)
        grid.inSphere(Rcore, center=[0, 0, 0], b=dropSLD)
        grid.prune(~np.isclose(grid._points[:, 3], 0))  # prune all except spheres
        grid.set_b(dropSLD * V)
    elif distribution[:1] == 'random'[:1]:
        nOP = int((2*Rcore)**3/Ddrops**3)
        grid = sf.randomLattice(size=[Rcore*2, Rcore*2, Rcore*2], numberOfPoints=nOP, b=0)
        grid.move([-Rcore, -Rcore, -Rcore])
        grid.inSphere(Rcore, center=[0, 0, 0], b=dropSLD)
        grid.prune(~np.isclose(grid._points[:, 3], 0))  # prune all except spheres
        grid.set_b(dropSLD * V)
    else:
        nOP = int((2*Rcore)**3/Ddrops**3)
        grid = sf.pseudoRandomLattice(size=[Rcore*2, Rcore*2, Rcore*2], numberOfPoints=nOP, b=0)
        grid.move([-Rcore, -Rcore, -Rcore])
        grid.inSphere(Rcore, center=[0, 0, 0], b=dropSLD)
        grid.prune(~np.isclose(grid._points[:, 3], 0))  # prune all except spheres
        grid.set_b(dropSLD * V)
    ...
    # complete the grid adding coreshell formfactor at center with respective scattering amplitude
    points = np.vstack([np.c_[grid.array, np.ones(grid.numberOfAtoms())*2], [0, 0, 0, fq.I0**0.5, 1]])
    res = cloudScattering(q, points, relError=relError, formfactor=fq, rms=rms, ncpu=0)
    result=res.addColumn(1, fq.Y)
    result[1]=result[1]*result.I0
