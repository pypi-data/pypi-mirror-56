...
def sImemoize(cachesize=4, attrnames=[]):
    ...
    def _memoize(function):
        ...
        def _memoizer(*args, **kwargs):
            ...
            attributes =[getattr(cls, an) for an in attrnames]
...
class PickerDetPosition:
    ...
    def bstep(self, event):
        ...
        if self.dstepi >= len(self.dsteps):
            self.dstepi = 0
        self.buttons[0].label.set_text('step $\pm${0:}'.format(self.dsteps[self.dstepi]*1000))
    ...
    def bastep(self, event):
        ...
        if self.astepi >= len(self.asteps):
            self.astepi = 0
        self.buttons[13].label.set_text('step $\pm${0:}'.format(self.asteps[self.astepi]))
        self.figure.canvas.draw_idle()
    ...
    def bcstep(self, event):
        ...
        if self.cstepi >= len(self.csteps):
            self.cstepi = 0
        self.buttons[14].label.set_text('step $\pm${0:}'.format(self.csteps[self.cstepi]))
        self.figure.canvas.draw_idle()
    ...
    def update(self):
        ...
        Iradial = sf.radial3DLSF(self.image.pQ.reshape(-1, 3),
                                 lattice=self.lattice, domainsize=domainsize, rmsd=rmsd, hklmax=hklmax)
        ...
        latticeradial = sf.latticeStructureFactor(selfradial.X,
                                                  lattice=self.lattice, domainsize=domainsize, rmsd=rmsd, hklmax=hklmax)
        ...
        print('[hkl],  2theta :')
        for a, b in zip(Iradial.hkl[visible][:len(Iradial.theta2)], Iradial.theta2[visible[:len(Iradial.theta2)]]):
...
class sasImage(SubArray, np.ma.MaskedArray):
    ...
    def __new__(cls, file, detector_distance=None, center=None,
                alpha=None, beta=None, gamma=None, copy=None, maskbelow=0):
        r"""
        ...
        center : None, list 2xfloat, sasImage
    ...
    def pQ(self):
        r"""
        3D scattering vector :math:`q` for detector pixel with detector placed in non standard geometry
        (offset detector).
    ...
    def calibrateOffsetDetector(self, lattice=None, center=None, distance=None, alpha=None, beta=None, gamma=None,
        ...
        bcstep = Button(acstep, 'step $\pm$1', color='linen', hovercolor='green')
        ...
        bstep = Button(axstep, 'step $\pm$10', color='grey', hovercolor='green')
        bstep.label.set_fontsize(bfontsize)
        # angular step
        aastep = pyplot.axes([xx+4.*dx, yy+3*dy, dl, dly])
        bastep = Button(aastep, 'step $\pm$3', color='lightgrey', hovercolor='green')
        ...
        # looking at the lattice
        Iradial = sf.radial3DLSF(self.pQ.reshape(-1, 3), lattice=lattice, domainsize=domainsize,
                                 asym=asym, lg=lg, rmsd=rmsd, beta=None, hklmax=hklmax)
        I2d=Iradial.Y.reshape(self.shape)
        scaling=(selfradial.Y.max()-bgr)/(Iradial.Y.max()-Iradial.Y.min())
        I2d = I2d*scaling + bgr
        I2d[self.mask] = bgr
        latticeradial = sf.latticeStructureFactor(selfradial.X, lattice=lattice, domainsize=domainsize,
                                                  asym=asym, lg=lg, rmsd=rmsd, beta=None, hklmax=hklmax)
    ...
    def reduceSize(self, pixelsize=2, center=None, border=None):
        ...
        XResolution,YResolution,center are scaled correspondingly.
        ...
        # change some attributes according to pixelsize
        try:
            # increase pixel size
