...
if _platformname[0] != 'Windows':
    ...
    try:
        # here we only test if grace is installed by calling for the help
        p = subprocess.Popen(('which xmgrace',), bufsize=0, shell=True,
...
class GraceGraph:
    ...
    def gen_axis(self, axis_prefix, ax_min=None, ax_max=None,
        ...
        label : string, list or Label object
            ...
            If string this is the label string.
            If list it determines [label string,charsize,place,color]
            place : 'normal','both','opposite'
        invert : {True,False}
            invert axis
        formula : grace equation
            formula for ticklabel calculation eg. rescaling
        ...
        ticklabel : float, list, tickLabel object
            With [format,precision,charsize,placeon] or only charsize
    ...
    def xaxis(self, min=None, max=None, **kwargs):
        ...
        tick : Tick object or list, False True
            List of [major tick distance , minorticks number,majorsize,minorsize,position]
            position: 'normal','both','opposite' (first letter is enough)
            False/True switch Ticks on and off
            see Tick for more
        ticklabel : float, list, tickLabel object
            With [format,precision,charsize,placeon] or only charsize
    ...
    def legend(self, strings=None, x=None, y=None,
        ...
        strings : list of strings,string, default=None
            ...
            self.legend_strings is build from legend attributein plot
            ...
            if 'symbol' in kwargs or 'sy' in kwargs:
                ...
                if isinstance(kwargs[keyword], Symbol):
                    ...
                    for key, i, l in zip(keys, kwargs[keyword], range(len(keys))):
                        if key is 'fillcolor' and not isinstance(i, numbers.Integral):
            ...
            if 'line' in kwargs or 'li' in kwargs:
                ...
                if isinstance(kwargs[keyword], Line):
                    ...
                    for key, i, l in zip(keys, [1] + kwargs[keyword], range(len(keys))):
                        ...
                        if key is 'color' and not isinstance(kwargs[keyword][2], numbers.Integral):
                            self.lastline[l] = self.lastsymbol[2]
                        # avoid zero as white color
                        if key is 'color' and self.lastline[l] == 0:
                            self.lastline[l] = 1
...
class GracePlot:
    ...
    def __init__(self, width=None, height=None, auto_redraw=True, redraw_interval=0.1):
        """
        ...
        if width is not None and height is not None:
            args = self.command_args + ('-fixed', str(self.resolution * width), str(self.resolution * height))
            self.aspect = float(width) / height
        else:
            args = self.command_args + ('-free',)
            self.aspect = 1.0

        args += '-dpipe', '0'  # the -pipe method to get stdin freezes grace until EOF, -dpipe works right
        if _platformname[0] != 'Windows':
            try:
                self.grace = subprocess.Popen((self.grace_command,) + args, bufsize=65536,
    ...
    def save(self, filename, size=(1012, 760), dpi=300, format='agr'):
        """
        ...
        # code is basically Nate Gray's.  RB.
            ...
            self.send_commands('PAGE RESIZE 800,600', 'device "PNG" dpi 600',
