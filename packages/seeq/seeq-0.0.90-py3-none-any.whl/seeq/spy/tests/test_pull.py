...
def test_pull_empty_results():
    no_data_signal_df = spy.push(metadata=pd.DataFrame([{'Name': 'No Data Signal',
                                                         'Type': 'Signal'}]))

    no_data_condition_df = spy.push(metadata=pd.DataFrame([{'Name': 'No Data Condition',
                                                            'Maximum Duration': '1d',
                                                            'Type': 'Condition'}]))
...
def test_no_end_date():
    ...
    start = pd.datetime.now(tz=pytz.timezone('US/Pacific')) - pd.Timedelta(hours=1)
    df = spy.pull(search_results, start=start, grid=None)
...
def test_pull_condition_as_capsules():
    ...
    area_a_temperature_max['Statistic'] = 'max'
    with_signals_df = push_df.append([area_a_temperature_count, area_a_temperature_max]).reset_index(drop=True)
    ...
    assert 'Area A_Temperature (max)' in pull_df
