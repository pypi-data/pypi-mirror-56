...
class ItemWithOwnerAndAcl(Item):
    def decide_owner(self, datasource_maps, item_map, *, owner=PRESERVE_OWNER, current_owner_id=None):
        requires_admin = True
        if _common.is_guid(owner):
            owner_id = owner
        elif owner == PRESERVE_OWNER:
            ...
            identity_preview_list = users_api.autocomplete_users_and_groups(query=owner)  # type: IdentityPreviewListV1
            if len(identity_preview_list.items) == 0:
                raise RuntimeError('User "%s" not found' % owner)
            ...
            owner_id = identity_preview_list.items[0].id
...
class Folder(ItemWithOwnerAndAcl):
    ...
    def push(self, parent_folder_id, datasource_maps, item_map, *, owner=PRESERVE_OWNER, label=None,
             access_control=None):
        items_api = ItemsApi(_login.client)
        folders_api = FoldersApi(_login.client)
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def push(self, *, owner=PRESERVE_OWNER, folder_id=None, item_map=None, label=None,
    ...
    def push_containing_folders(self, item_map, use_full_path, parent_folder_id, owner, access_control):
    ...
    @staticmethod
    def find_item(item_id, label=None):
        """
