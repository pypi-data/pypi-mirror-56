...
def pull(items, *, start=None, end=None, grid='15min', header='__auto__', group_by=None, shape='auto',
    ...
    calculation : {str, pandas.DataFrame}
    ...
    _common.validate_argument_types([
        ...
        (calculation, 'calculation', (str, pd.DataFrame)),
    ...
    if 'ID' not in items or 'Type' not in items:
        raise RuntimeError("items DataFrame must include ID and Type columns")
    ...
    if shape not in ['auto', 'capsules', 'samples']:
        raise RuntimeError("shape must be one of 'auto', 'capsules', 'samples'")

    if capsules_as is not None:
        raise RuntimeError("capsules_as argument is deprecated. Use the following instead:\n"
    ...
    if capsule_properties is not None and not isinstance(capsule_properties, list):
        raise RuntimeError("capsules_properties must be a list of strings (capsule property names)")
    ...
    if shape == 'auto':
        shape = 'samples' if at_least_one_signal else 'capsules'
    column_names = dict()
    ...
    for phase in phases:
        ...
        try:
            ...
            for index, row in query_df.iterrows():
                ...
                try:
                    output_df = _process_query_row(
                        _drain_status_updates, at_least_one_signal, calculation, shape, capsule_properties,
                        column_names, final_column_names, formulas_api, grid, header, query_df, index, items_api,
                        output_df, pd_start, pd_end, phase, pulled_series, status, status_updates, threads, tz_convert,
                        bounding_values, group_by)
...
def _process_query_row(_drain_status_updates, at_least_one_signal, calculation, shape,
                       capsule_properties, column_names, final_column_names, formulas_api, grid, header, query_df,
                       index, items_api, output_df, pd_start, pd_end, phase, pulled_series, status, status_updates,
                       threads, tz_convert, bounding_values, group_by):
    row = query_df.loc[index]
    ...
    if phase == 'signals' and 'Signal' in item_type:
        ...
        if shape == 'samples' and len(output_df) == 0:
        ...
        output_df = _pull_condition(shape, capsule_properties, calculation_to_use, item_id,
                                    item_name, header, output_df, pulled_series, pd_start, pd_end, tz_convert,
                                    column_names, index, index_to_use, query_df, at_least_one_signal, status,
                                    _drain_status_updates)
...
def _pull_condition(shape, capsule_properties, calculation_to_use, item_id, item_name, header, output_df,
                    pulled_series, pd_start, pd_end, tz, column_names, index, index_to_use, query_df,
                    at_least_one_signal, status, _drain_status_updates):
    # noinspection PyBroadException
    try:
        ...
        while True:
            ...
            if shape == 'capsules' and at_least_one_signal:
                ...
                this_capsule_count, next_start, output_df = _pull_condition_via_table_builder_api(
                    capsule_properties, column_names, current_start,
                    query_df, item_id, item_name, header, offset, output_df, pd_end, tz, status)
            else:
                this_capsule_count, next_start, output_df = _pull_condition_via_formula_api(
                    calculation_to_use, shape, capsule_properties, column_names, current_start, index,
                    index_to_use, item_id, item_name, offset, output_df, pd_end, pulled_series, tz)
            ...
            if this_capsule_count < _common.DEFAULT_PULL_PAGE_SIZE:
                break
...
def _pull_condition_via_formula_api(calculation_to_use, shape, capsule_properties, column_names, current_start,
                                    index, index_to_use, item_id, item_name, offset, output_df, pd_end, pulled_series,
                                    tz):
    formulas_api = FormulasApi(_login.client)
        ...
        for capsule in capsules_output.capsules:  # type: CapsuleV1
            ...
            if check_for_dupes and \
                    ...
                    len(output_df.loc[(output_df['Condition'] == item_name) &
                                      (output_df['Capsule Start'] == pd_capsule_start) &
                                      (output_df['Capsule End'] == pd_capsule_end)]):
    ...
    return len(capsules_output.capsules), next_start, output_df


def _pull_condition_via_table_builder_api(capsule_properties, column_names,
                                          current_start, query_df, item_id, item_name,
                                          header, offset, output_df, pd_end, tz, status):
    ...
    for signal_index, signal_row in signals_df.iterrows():
        ...
        stat = signal_row['Statistic'] if 'Statistic' in signal_row else 'average'
        accessor = '.getValue()' if stat.lower() in ['max', 'min'] else ''
        formula = '$series.%s($capsule)%s on %s' % (stat, accessor, signal_item_id.lower())
        column_definitions['%s (%s)' % (signal_item_name, stat)] = formula
        status.df.at[signal_index, 'Result'] = 'Success'
    ...
    table_output = tables_api.build(
        ...
        offset=offset,
        limit=_common.DEFAULT_PULL_PAGE_SIZE
    )  # type: TableOutputV1
    ...
    return len(table_output.table), next_start, output_df
