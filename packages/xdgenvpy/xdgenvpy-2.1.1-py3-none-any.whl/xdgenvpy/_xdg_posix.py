...
def _get_dirs(var_name, default_value=None):
    ...
    Pythonic rather than return a single long string delimited by colons.
...
class XDGPosixPlatform(object):
    ...
    def XDG_DATA_HOME(self):
        ...
        return getenv('XDG_DATA_HOME',
                      Path(self._base_dir).joinpath('.local', 'share')
                                          .as_posix())
    ...
    def XDG_CONFIG_HOME(self):
        ...
        return getenv('XDG_CONFIG_HOME',
                      Path(self._base_dir).joinpath('.config').as_posix())
    ...
    def XDG_CACHE_HOME(self):
        ...
        return getenv('XDG_CACHE_HOME',
                      Path(self._base_dir).joinpath('.cache').as_posix())
    ...
    def XDG_RUNTIME_DIR(self):
        ...
        default_value = \
            Path().joinpath(sep, 'run', 'user', str(getuid())).as_posix()
    ...
    def XDG_DATA_DIRS(self):
        ...
        dirs = _get_dirs('XDG_DATA_DIRS', '/usr/local/share/:/usr/share/')
        if not dirs:
            dirs = (data_home,)
    ...
    def XDG_CONFIG_DIRS(self):
        ...
        dirs = _get_dirs('XDG_CONFIG_DIRS', '/etc/xdg')
        if not dirs:
            dirs = (config_home,)
