...
from logger_stackdriver import make_stackdriver_sink
from logger_stdout import make_stdout_sink
from logger_types import Message, Sequence, Trace
from stack_context import get_stackcontext, put_stackcontext
...
def log_root(sink=None):
    def wrapper(fn):
        @put_stackcontext(_STACK_MARKER, LogRoot(sink or _make_default_sink()))
...
class SequenceGuard:
    ...
    def __enter__(self):
        _get_root().save(Sequence(self.description))
        return self
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        if exc_type is None:
            root.head.resolution = Message.SUCCESS
        else:
...
def _make_default_sink():
    return make_stackdriver_sink() or make_stdout_sink()


_common_root = LogRoot(_make_default_sink())


def _get_root():
    return get_stackcontext(_STACK_MARKER, _common_root)
