...
class Algorithms:
    ...
    def downsample(self, frame, iterations=1):
        """
        Downsample the given Bayer raw frame by a factor of two in both directions,
        that is, to a quarter of its original size. This is repeated a given number
        of iterations. If iterations is zero, the frame is returned untouched.
        """
        if iterations >= 1:
            t0 = time.time()
    ...
    def lsc(self, frame, lscmap):
        ...
        if lscmap is not None:
            ...
            frame = frame * lscmap
            self._vprint(f"{_elapsed(t0)} - applying lens shading correction: range = {_minmax(frame)}")
        return frame
    ...
    def ccm(self, frame, matrix):
        ...
        if matrix is not None:
            ...
            with np.printoptions(formatter={'float': lambda x: f"{x:.2f}"}):
                sums = f"with row sums {np.sum(matrix, axis=1).T}"
